...
def download_cifar10(root='./data', train=True, download=True):
  url = 'https://www.cs.toronto.edu/~kriz/cifar-10-python.tar.gz'
  file_path = os.path.join(root, 'cifar-10-python.tar.gz')
  if download:
  ...
  if train:
    ...
    for i in range(1, 6):
      ...
      data_dict = unpickle(os.path.join(root, 'cifar-10-batches-py', filename))
      x += [data_dict[b'data']]
      y += [data_dict[b'labels']]
    x = np.concatenate(x, 0).reshape(-1, 3, 32, 32).astype(np.float32) / 255.0
    y = np.concatenate(y, 0).astype(np.int32)
  else:
    data_dict = unpickle(os.path.join(root, 'cifar-10-batches-py', 'test_batch'))
...
class CIFAR10(Dataset):
  def __init__(self, root='./data', train=True, download=True, transform=lambda x: x, target_transform=lambda t: t):
    ...
    self.x, self.y, self.classes = download_cifar10(root=root, train=train, download=download)
