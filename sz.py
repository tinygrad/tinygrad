...
def is_js_token(s):
  return len(s) and not s.startswith("//")
  ...
  for path, _, files in os.walk(os.path.join(base_path, "tinygrad")):
    for name in files:
      if not (name.endswith(".py") or name.endswith(".js")):
        continue
      if any(s in path.replace("\\", "/") for s in ["tinygrad/runtime/autogen", "tinygrad/viz/assets"]):
        continue
...
if __name__ == "__main__":
  ...
  if table:
    if len(sys.argv) == 3:
      ...
      print("```")
      print(
        tabulate([headers] + sorted(table, key=lambda x: -x[1]), headers="firstrow", intfmt=(..., "d", "+d"), floatfmt=(..., ..., ..., ".1f", "+.1f"))
        + "\n"
      )
