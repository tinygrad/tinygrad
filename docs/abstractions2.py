...
lib = ClangJITCompiler().compile(
    "void add(int *out, int *a, int *b) { out[0] = a[0] + b[0]; }"
)
...
a = (
    Buffer(DEVICE, 1, dtypes.int32)
    .allocate()
    .copyin(memoryview(bytearray(struct.pack("I", 2))))
)
b = (
    Buffer(DEVICE, 1, dtypes.int32)
    .allocate()
    .copyin(memoryview(bytearray(struct.pack("I", 3))))
)
...
st_0 = UOp(
    Ops.STORE, dtypes.void, (output_buf, ShapeTracker.from_shape((1,)).to_uop(), alu)
)
