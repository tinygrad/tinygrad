#!/usr/bin/env python3
"""Test Phase 4 runtime - COMPLETE IMPLEMENTATION"""
...
print("="*80)
print("PHASE 4: COMPLETE IMPLEMENTATION - Final Runtime Comparison")
print("="*80)
...
c_new, r_new = bench_runtime(x, w, 1, {'WINO': 1, 'WINO_OLD': 0}, "NEW (complete)")

print("\n" + "="*80)
print("PHASE 4 ANALYSIS - COMPLETE IMPLEMENTATIONS")
print("="*80)
print(f"Baseline runtime: {r_base:.3f}ms")
print(f"OLD full runtime: {r_old:.3f}ms ({r_old/r_base:.2f}× vs baseline)")
print(f"NEW full runtime: {r_new:.3f}ms ({r_new/r_base:.2f}× vs baseline)")
print(f"\n🎯 FINAL COMPARISON: NEW vs OLD")
print(f"Runtime: {r_new:.3f}ms vs {r_old:.3f}ms")
if r_new > r_old:
    print(f"❌ NEW is {(r_new-r_old)/r_old*100:.1f}% SLOWER runtime ({r_new/r_old:.2f}×)")
else:
    print(f"✅ NEW is {(r_old-r_new)/r_new*100:.1f}% FASTER runtime ({r_old/r_new:.2f}×)")

print(f"\nCompile time: {c_new:.1f}ms vs {c_old:.1f}ms")
if c_new > c_old:
    print(f"Compile: NEW is {(c_new-c_old)/c_old*100:.1f}% slower")
else:
    print(f"Compile: NEW is {(c_old-c_new)/c_new*100:.1f}% faster")
