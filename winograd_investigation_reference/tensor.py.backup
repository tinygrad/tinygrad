...
from tinygrad.helpers import IMAGE, WINO, Metadata, TRACEMETA, ceildiv, fetch, polyN, DEBUG, is_numpy_ndarray, FUSE_ATTENTION, WINO_OLD
...
class Tensor(MathTrait):
  ...
  def conv2d(self, weight:Tensor, bias:Tensor|None=None, groups=1, stride=1, dilation=1, padding:int|tuple[int, ...]=0,
    ...
    if not all(x == 3 for x in HW) or stride != 1 or dilation != 1 or not WINO_OLD:
      # normal conv
