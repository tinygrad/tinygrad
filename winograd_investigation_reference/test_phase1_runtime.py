...
print("="*80)
print("PHASE 1: XHAT Only - Runtime Investigation")
print("="*80)
...
c_new, r_new = bench_runtime(x, w, 1, {'WINO': 1, 'WINO_OLD': 0}, "PHASE 1 NEW (XHAT only)")
...
print(f"NEW Phase 1 runtime: {r_new:.3f}ms ({r_new/r_base:.2f}× vs baseline)")
print(f"\nNEW vs OLD: {r_new:.3f}ms vs {r_old:.3f}ms")
if r_new > r_old:
    print(f"❌ NEW Phase 1 is {(r_new-r_old)/r_old*100:.1f}% SLOWER than complete OLD ({r_new/r_old:.2f}×)")
else:
    print(f"✓ NEW Phase 1 is {(r_old-r_new)/r_new*100:.1f}% FASTER than complete OLD ({r_old/r_new:.2f}×)")
