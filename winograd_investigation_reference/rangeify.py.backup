...
from tinygrad.helpers import argsort, prod, all_same, pluralize, getenv, flatten, dedup, all_int, DEBUG, SPLIT_REDUCEOP, REAL_SUBSTITUTE, Metadata, WINO
...
def remove_bufferize(src:UOp, buf:UOp, idx:UOp):
  ...
  if REAL_SUBSTITUTE:
    ...
    #print("REPLACES", *[f"\n{k}" for k in replaces])
    # rR = UOp(Ops.RANGE, dtypes.index, arg=(8, AxisType.LOOP), src=(UOp(Ops.CONST, dtypes.index, arg=2)))
    # if [r.arg for r in replaces if r.op is Ops.RANGE and r.arg[0] == 8]:
    #   print("REPLACES", *[f"\n{k}" for k in replaces])
    #   print("SUBSTITUTE", UOp(Ops.SUBSTITUTE, dtype=src.dtype, src=(src, UOp(Ops.NOOP, src=tuple(replaces[0::2])), UOp(Ops.NOOP, src=tuple(replaces[1::2])))))
