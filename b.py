...
def helper_tc_allclose(n:int, m:int, k:int, dtype_in:DType, dtype_out:DType, axis:int=0, tc_opt:int=0):
  ...
  np_c = np_a @ np_b
  if dtype_out == dtypes.half: tc_rtol = 1e-2, 1e-3
  elif dtype_in == dtypes.bfloat16: tc_rtol = 1e-2, 3e-3
  else: tc_rtol = 5e-1
  np.testing.assert_allclose(np_c, out, rtol=tc_rtol)

#helper_tc_allclose(8, 16, 16, dtypes.half, dtypes.float32)
#helper_tc_allclose(8, 16, 32, dtypes.f8e4m3, dtypes.float32)
helper_tc_allclose(8192, 8192, 8192, dtypes.f8e4m3, dtypes.float32)