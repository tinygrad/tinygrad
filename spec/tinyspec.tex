...
\op{Alloc} & () & \texttt{ParamArg} &
  Unbound storage of $\mathrm{size}$ elements. \\
...
\smallskip
\texttt{ParamArg} contains slot, dtype, concrete size (or \textsc{null} for a scalar), value bounds, alignment, name, addrspace, device, volatility, optional image shape, and an optional bound device buffer (absent for \op{Alloc}s). \textbf{addrspace} is \texttt{GLOBAL}, \texttt{LOCAL}, or \texttt{REG}.
...
\smallskip
A value \op{Call} is void: its \op{Sink} body stores to output \op{Param}s bound positionally to the call's \op{Alloc} arguments; output $a_k$ is \op{After}$(a_k, \op{Call})$. \op{Alloc}s are scoped to their \op{Call}: they are never implicit inputs of the enclosing graph.
...
\op{End} & (body, $range_0$, $range_1$, \ldots) & --- & Close one or more \op{Range} loop. \\
\op{Backedge} & (body, range, condition) & --- & Conditional loop. \\
...
\midrule
\op{Buffer}, \op{Alloc} & from arg & from arg ($\mathrm{size}$) & from arg & dtype range \\
\op{Param} & from arg & from arg ($\mathrm{size}$) & from arg & from src or dtype range \\
\op{Const} & from arg & $()$ & \textsc{null} & $[v, v]$ \\[3pt]
...
{\color{axcyan}\texttt{LOCAL}} & \texttt{l} & g, L, R & inner/outer & Workgroup local dimension (shared memory). \\
...
{\color{axmagenta}\texttt{UNROLL}} & \texttt{r} & R, l & inner & Fully unrolled loop. \\
\bottomrule
\end{tabular}
...
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  ...
  \item \textbf{Parameterizes outputs}: for each returned value $v_i$, creates an output \op{Param}$(m+i)$ and a matching \op{Alloc} $b_i$ (unique symbolic storage identity), where $m$ is the number of inputs.
...
The result is a reusable graph fragment: the body's storage is represented by \op{Param} references and local \op{Alloc}s, not concrete buffers, and a single call can return any number of values. At schedule time, an ordinary value-producing \op{Call} is inlined by positional \op{Param} substitution and each output \op{After} resolves to the value stored in the body, preserving required output writes. A precompiled call keeps the body opaque and binds its \op{Alloc}s to concrete buffers for execution.
