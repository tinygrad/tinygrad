...
\op{Unshard} & $(T, F_0, F_1, \ldots)$ & --- & $F_k$ is axis $k$'s factor list; owner factors (with \op{Range}s) shard. \\
\bottomrule
\end{tabular}
...
\op{Unshard} & $\mathrm{src}[0].\mathrm{dtype}$ & per-axis $\prod$ of factor spans & $\mathrm{src}[0].\mathrm{device}$ & $\mathrm{src}[0]$ \\
...
\medskip
\textbf{sharding} tracks multi-device sharding as a set of (axis, owner-factor) pairs, recursive through ops. \op{Unshard} defines it: $\mathrm{src}[1:]$ is one factor list $F_k$ per axis (a UOp, or \op{Stack} of UOps, major$\to$minor). A factor is \emph{local} (a plain const; its span is its value) or \emph{owner} (an int expr containing \op{Range}s; its span is $\mathrm{vmax}{+}1$). Owner factors mark the sharded axes; one axis may carry several (e.g.\ two ranges merged by a reshape). The full shape is the per-axis product of spans, the shard shape ($\mathrm{src}[0]$) is the per-axis product of local spans, and the interleaving of owner and local factors is the layout: $[\mathrm{rng}, L]$ contiguous, $[L, \mathrm{rng}]$ strided, $[L, \mathrm{rng}, L]$ a middle insert. \op{Buffer} with $n$-tuple device: sharded on axis $0$ (device dim). The single-axis convenience \textbf{axis} is \textsc{null} unless exactly one axis is sharded.
\op{Reshape} splits and merges the factor lists (owner factors split as $e{//}d,\; e\%d$); it raises if a factor is cut asymmetrically (items moved between shards). \op{Permute} reorders the factor lists with their axes. \op{Expand} prepends pure-local factor lists. \op{Pad} grows a local factor's span; \op{Flip} is local to the shard.
\op{Reduce} on a sharded axis drops it. \op{Copy} $\to$ \textsc{null}. ALU and \op{Stack} take the union of source shardings. Default: \textsc{null}.
...
\textbf{Sharding} splits a tensor along an axis across $n$ devices. It opens a \op{Range} of type \texttt{DEVICE} (a symbolic per-device index $d$), shrinks each device's view to its piece, then closes the range with \op{Unshard}. The result is a logical tensor whose shape along that axis is the full size; each device holds $1/n$ of it. \op{Unshard} is the inverse of sharding --- it marks the boundary between per-device computation and the logical multi-device tensor. The range need not be \texttt{DEVICE}; e.g.\ a \texttt{WARP} or \texttt{LOCAL} range closes the same way, concatenating per-lane shards. The canonical layout is $[\mathrm{rng}, L]$ (contiguous); reshapes produce strided $[L, \mathrm{rng}]$ or middle-insert $[L, \mathrm{rng}, L]$ layouts, and a tensor may be sharded along several axes at once --- \op{Unshard} carries one factor list per axis, with owner factors marking the sharded ones.
