...
\op{Expand} & $(T, \mathbf{s'})$ & --- & Prepend axes $\mathbf{s'}$ on the left. Output shape is $\mathbf{s'} + T.\mathrm{shape}$. \\
...
\op{Reshape} remaps axis to preserve the shard boundary. \op{Permute} follows the permutation. \op{Expand} shifts axis right by $|\mathbf{s'}|$.
