...
\textbf{Op} & \textbf{src} & \textbf{arg} & \textbf{Semantics} \\
\midrule
\op{Call} & (body, $a_0$, $a_1$, \ldots) & --- & A \op{Tuple} body substitutes each \op{Param} $k$ with $a_k$ and produces values (gradient-able); any other body is an opaque invocation of a compiled kernel or custom function. \\
