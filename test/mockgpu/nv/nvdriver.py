...
class NVCtlFileDesc(VirtFileDesc):
    ...
    def ioctl(self, fd, request, argp):
        return self.driver.ctl_ioctl(request, argp)
...
class NVUVMFileDesc(VirtFileDesc):
    ...
    def ioctl(self, fd, request, argp):
        return self.driver.uvm_ioctl(request, argp)
...
class NVDevFileDesc(VirtFileDesc):
    ...
    def ioctl(self, fd, request, argp):
        return self.driver.dev_ioctl(self.gpu, request, argp)

    def mmap(self, start, sz, prot, flags, fd, offset):
        start = libc.mmap(start, sz, prot, flags | mmap.MAP_ANONYMOUS, -1, 0)
        if self._mapping_userland:
            self.driver.track_address(
                start,
                start + sz,
                lambda mv, off: None,
                lambda mv, off: self.driver._gpu_mmio_write(mv, off, self.gpu),
            )
        return start
...
class NVDriver(VirtDriver):
    def __init__(self, gpus=6):
        ...
        self.tracked_files += [
            VirtFile("/dev/nvidiactl", functools.partial(NVCtlFileDesc, driver=self)),
    ...
    def _prepare_gpu(self, gpu_id):
        ...
        self.tracked_files += [
            VirtFile(
                f"/dev/nvidia{gpu_id}",
                functools.partial(NVDevFileDesc, driver=self, gpu=self.gpus[gpu_id]),
            )
        ]
    ...
    def rm_alloc(self, argp):
        ...
        if struct.hClass == nv_gpu.NV01_ROOT_CLIENT:
            self.root_handle = struct.hObjectNew = self._alloc_handle()
            ...
            assert struct.hObjectParent in self.object_by_handle and isinstance(
                self.object_by_handle[struct.hObjectParent], NVGPU
            )
            struct.hObjectNew = self._alloc_handle()
            self.object_by_handle[struct.hObjectNew] = NVSubDevice(
                self.object_by_handle[struct.hObjectParent]
            )
        elif struct.hClass == nv_gpu.TURING_USERMODE_A:
            assert struct.hObjectParent in self.object_by_handle and isinstance(
                self.object_by_handle[struct.hObjectParent], NVSubDevice
            )
            struct.hObjectNew = self._alloc_handle()
            self.object_by_handle[struct.hObjectNew] = NVUserMode(
                self.object_by_handle[struct.hObjectParent]
            )
            ...
            struct.hObjectNew = self._alloc_handle()
            self.object_by_handle[struct.hObjectNew] = NVVASpace(
                self.object_by_handle[struct.hObjectParent]
            )
        elif (
            struct.hClass == nv_gpu.NV1_MEMORY_SYSTEM
            or struct.hClass == nv_gpu.NV1_MEMORY_USER
        ):
            assert struct.hObjectParent in self.object_by_handle and isinstance(
                self.object_by_handle[struct.hObjectParent], NVGPU
            )
            ...
            struct.hObjectNew = self._alloc_handle()
            self.object_by_handle[struct.hObjectNew] = NVAllocation(
                self.object_by_handle[struct.hObjectParent], params.size
            )
            ...
            struct.hObjectNew = self._alloc_handle()
            self.object_by_handle[struct.hObjectNew] = NVChannelGroup(
                self.object_by_handle[struct.hObjectParent]
            )
        elif struct.hClass == nv_gpu.FERMI_CONTEXT_SHARE_A:
            assert struct.hObjectParent in self.object_by_handle and isinstance(
                self.object_by_handle[struct.hObjectParent], NVChannelGroup
            )
            struct.hObjectNew = self._alloc_handle()
            self.object_by_handle[struct.hObjectNew] = NVContextShare(
                self.object_by_handle[struct.hObjectParent]
            )
            ...
            struct.hObjectNew = self._alloc_handle()
            params = nv_gpu.NV_CHANNELGPFIFO_ALLOCATION_PARAMETERS.from_address(
                params_ptr
            )
            ...
            struct.hClass == nv_gpu.AMPERE_DMA_COPY_B
            or struct.hClass == nv_gpu.ADA_COMPUTE_A
        ):
            assert struct.hObjectParent in self.object_by_handle and isinstance(
                self.object_by_handle[struct.hObjectParent], NVGPFIFO
            )
            struct.hObjectNew = self._alloc_handle()
        elif struct.hClass == nv_gpu.GT200_DEBUGGER:
            struct.hObjectNew = self._alloc_handle()
        else:
            raise RuntimeError(f"Unknown {struct.hClass} to rm_alloc")
        return 0
    ...
    def rm_control(self, argp):
        ...
        if struct.cmd == nv_gpu.NV0000_CTRL_CMD_GPU_GET_ID_INFO_V2:
            params: Any = nv_gpu.NV0000_CTRL_GPU_GET_ID_INFO_V2_PARAMS.from_address(
                params_ptr
            )
            params.deviceInstance = params.gpuId  # emulate them to be the same
        elif (
            struct.cmd == nv_gpu.NV0080_CTRL_CMD_GPU_GET_CLASSLIST_V2
            or struct.cmd == nv_gpu.NV0080_CTRL_CMD_GPU_GET_CLASSLIST
        ):
            if struct.cmd == nv_gpu.NV0080_CTRL_CMD_GPU_GET_CLASSLIST:
                params = nv_gpu.NV0080_CTRL_GPU_GET_CLASSLIST_PARAMS.from_address(
                    params_ptr
                )
            else:
                params = nv_gpu.NV0080_CTRL_GPU_GET_CLASSLIST_V2_PARAMS.from_address(
                    params_ptr
                )
            ...
            if struct.cmd == nv_gpu.NV0080_CTRL_CMD_GPU_GET_CLASSLIST:
                ...
                for i, c in enumerate(classes):
                    params.classList[i] = c
        elif struct.cmd == nv_gpu.NV2080_CTRL_CMD_GR_GET_INFO:
            info = {
                nv_gpu.NV2080_CTRL_GR_INFO_INDEX_SM_VERSION: nv_gpu.NV2080_CTRL_GR_INFO_SM_VERSION_3_5,
            ...
            reqlist = (nv_gpu.NV2080_CTRL_GR_INFO * params.grInfoListSize).from_address(
                params.grInfoList
            )
            for i in range(params.grInfoListSize):
                reqlist[i].data = info[reqlist[i].index]
        elif struct.cmd == nv_gpu.NV2080_CTRL_CMD_GPU_GET_GID_INFO:
            assert struct.hObject in self.object_by_handle and isinstance(
                self.object_by_handle[struct.hObject], NVSubDevice
            )
            gpu = self.object_by_handle[struct.hObject].device
            params = nv_gpu.NV2080_CTRL_GPU_GET_GID_INFO_PARAMS.from_address(params_ptr)
            if params.flags != nv_gpu.NV2080_GPU_CMD_GPU_GET_GID_FLAGS_FORMAT_BINARY:
            ...
            assert struct.hObject in self.object_by_handle and isinstance(
                self.object_by_handle[struct.hObject], NVGPFIFO
            )
            params = (
                nv_gpu.NVC36F_CTRL_CMD_GPFIFO_GET_WORK_SUBMIT_TOKEN_PARAMS.from_address(
                    params_ptr
                )
            )
            gpu_fifo = self.object_by_handle[struct.hObject]
            params.workSubmitToken = gpu_fifo.token
        elif struct.cmd == nv_gpu.NVA06C_CTRL_CMD_GPFIFO_SCHEDULE:
            pass
        elif struct.cmd == nv_gpu.NV2080_CTRL_CMD_PERF_BOOST:
            pass
        elif struct.cmd == nv_gpu.NV2080_CTRL_CMD_FB_FLUSH_GPU_CACHE:
            pass
        elif struct.cmd == nv_gpu.NV83DE_CTRL_CMD_DEBUG_READ_ALL_SM_ERROR_STATES:
            params = (
                nv_gpu.NV83DE_CTRL_DEBUG_READ_ALL_SM_ERROR_STATES_PARAMS.from_address(
                    params_ptr
                )
            )
            ...
            params = (
                nv_gpu.struct_NV83DE_CTRL_DEBUG_READ_MMU_FAULT_INFO_PARAMS.from_address(
                    params_ptr
                )
            )
            params.count = 1
            params.mmuFaultInfoList[0].faultAddress = int(
                os.environ["MOCKGPU_EMU_FAULTADDR"], base=16
            )
            params.mmuFaultInfoList[0].faultType = 1
            params.mmuFaultInfoList[0].accessType = 1
        else:
            raise RuntimeError(f"Unknown {struct.cmd} to rm_control")
        return 0

    def ctl_ioctl(self, req, argp):
        nr = req & 0xFF
        if nr == nv_gpu.NV_ESC_RM_ALLOC:
            return self.rm_alloc(argp)
        elif nr == nv_gpu.NV_ESC_RM_ALLOC_MEMORY:
            pass
        elif nr == nv_gpu.NV_ESC_RM_CONTROL:
            return self.rm_control(argp)
            ...
            for i, gpu in enumerate(self.gpus.values()):
                st = nv_gpu.nv_ioctl_card_info_t.from_address(
                    argp + i * ctypes.sizeof(nv_gpu.nv_ioctl_card_info_t)
                )
                st.gpu_id = gpu.gpuid
                st.pci_info.device_id = 0x2684
                st.valid = True
        else:
            raise RuntimeError(f"Unknown {nr} to nvidiactl")
        return 0

    def uvm_ioctl(self, nr, argp):
        if nr == nv_gpu.UVM_INITIALIZE:
            pass
        elif nr == nv_gpu.UVM_MM_INITIALIZE:
            pass
        elif nr == nv_gpu.UVM_REGISTER_GPU:
            st: Any = nv_gpu.UVM_REGISTER_GPU_PARAMS.from_address(argp)
            assert any(
                all(st.gpu_uuid.uuid[i] == gpu.gpu_uuid()[i] for i in range(16))
                for gpu in self.gpus.values()
            )
        elif nr == nv_gpu.UVM_REGISTER_GPU_VASPACE:
            pass
        elif nr == nv_gpu.UVM_ENABLE_PEER_ACCESS:
            pass  # uvm and shared spaced are setup already, no emulation for now
            ...
            libc.mmap(
                ...
                MAP_FIXED | mmap.MAP_SHARED | mmap.MAP_ANONYMOUS,
                -1,
                0,
            )
            ...
            for gpu_attr_id in range(st.gpuAttributesCount):
                ...
                for _gpu in self.gpus.values():
                    if all(
                        st.perGpuAttributes[gpu_attr_id].gpuUuid.uuid[i]
                        == _gpu.gpu_uuid()[i]
                        for i in range(16)
                    ):
                        gpu = _gpu
