...
class SchedResult(Enum):
    CONT = auto()
    YIELD = auto()  # noqa: E702
...
class GPFIFO:
    def __init__(self, token, base, entries_cnt):
        ...
        self.ctrl = nv_gpu.AmpereAControlGPFifo.from_address(
            self.base + self.entries_cnt * 8
        )
        self.state = {}
    ...
    def _state64(self, reg):
        return (self.state[reg] << 32) + self.state[reg + 4]

    def _state64_le(self, reg):
        return (self.state[reg + 4] << 32) + self.state[reg]

    def _reset_buf_state(self):
        self.buf, self.buf_ptr = None, 0
    ...
    def execute_qmd(self, qmd_addr):
        ...
        const0 = to_mv(
            qmd.constant_buffer_addr_lower_0 + (qmd.constant_buffer_addr_upper_0 << 32),
            0x160,
        ).cast("I")
        args_cnt, vals_cnt = const0[80], const0[81]
        args_addr = (
            qmd.constant_buffer_addr_lower_0
            + (qmd.constant_buffer_addr_upper_0 << 32)
            + 0x160
        )
        args = to_mv(args_addr, args_cnt * 8).cast("Q")
        vals = to_mv(args_addr + args_cnt * 8, vals_cnt * 4).cast("I")
        cargs = [ctypes.cast(args[i], ctypes.c_void_p) for i in range(args_cnt)] + [
            ctypes.cast(vals[i], ctypes.c_void_p) for i in range(vals_cnt)
        ]
        gx, gy, gz = qmd.cta_raster_width, qmd.cta_raster_height, qmd.cta_raster_depth
        lx, ly, lz = (
            qmd.cta_thread_dimension0,
            qmd.cta_thread_dimension1,
            qmd.cta_thread_dimension2,
        )
        gpuocelot_lib.ptx_run(
            ctypes.cast(prg_addr, ctypes.c_char_p),
            args_cnt + vals_cnt,
            (ctypes.c_void_p * len(cargs))(*cargs),
            lx,
            ly,
            lz,
            gx,
        ...
        if qmd.release0_enable:
            rel0 = to_mv(
                qmd.release0_address_lower + (qmd.release0_address_upper << 32), 0x10
            ).cast("Q")
            rel0[0] = qmd.release0_payload_lower + (qmd.release0_payload_upper << 32)
            rel0[1] = int(time.perf_counter() * 1e9)
        if qmd.release1_enable:
            rel1 = to_mv(
                qmd.release1_address_lower + (qmd.release1_address_upper << 32), 0x10
            ).cast("Q")
        ...
        if qmd.dependent_qmd0_enable:
            if qmd.dependent_qmd0_action == 1:
                self.execute_qmd(qmd.dependent_qmd0_pointer << 8)
            else:
                raise RuntimeError("unsupported dependent qmd action")

    def execute_cmd(self, cmd) -> SchedResult:
        if cmd == nv_gpu.NVC56F_SEM_EXECUTE:
            return self._exec_signal()
        elif cmd == nv_gpu.NVC6C0_LAUNCH_DMA:
            return self._exec_nvc6c0_dma()
        elif cmd == nv_gpu.NVC6B5_LAUNCH_DMA:
            return self._exec_nvc6b5_dma()
        elif cmd == nv_gpu.NVC6C0_SEND_SIGNALING_PCAS2_B:
            return self._exec_pcas2()
        elif cmd == 0x0320:
            return self._exec_load_inline_qmd()  # NVC6C0_LOAD_INLINE_QMD_DATA
        else:
    ...
    def _exec_signal(self) -> SchedResult:
        ...
        if typ == 1:
            ...
            raise RuntimeError(f"Unsupported type={typ} in exec wait/signal")
        return SchedResult.CONT
    ...
    def _exec_nvc6c0_dma(self):
        ...
        assert (
            typ == 6 and mthd == nv_gpu.NVC6C0_LOAD_INLINE_DATA
        ), f"Expected inline data not found after nvc6c0_dma, {typ=} {mthd=}"
        copy_data = [self._next_dword() for _ in range(dsize)]
        assert (
            len(copy_data) * 4 == sz
        ), f"different copy sizes in _exec_nvc6c0_dma: {len(copy_data) * 4} != {sz}"
    ...
    def _exec_nvc6b5_dma(self):
        ...
        if (flags & 0b11) != 0:
            ...
            raise RuntimeError("unknown nvc6b5_dma flags")
...
class NVGPU(VirtGPU):
    ...
    def gpu_uuid(self, sz=16):
        return self.gpuid.to_bytes(sz, byteorder="big", signed=False)
