# ir3 wave emulator
# A630 64-wide lockstep.
# Cat0 (br/jump/park) stays in Python
# ALU/mem between cat0 is one clang kernel (RANGE per ISA insn), split at (jp)
# and branch targets so reconverge still wake()s in Python.
# pointer-only (pc/act/pmode/pmask in the wave mmap).
#
# Buffers (ctypes args):
...
# https://gitlab.freedesktop.org/mesa/mesa/-/blob/main/src/freedreno/common/freedreno_devices.py
# a630 fd_dev_info: threadsize_base=64, cs_shared_mem_size=32*1024
...
_kind_memo: dict[int, object] = {}
def _kind(raw: dict):
  if (hit:=_kind_memo.get(i:=id(raw))) is not None: return hit
...
_pack_memo: dict[int, dict[str, int]] = {}
def _pack_named(raw: dict) -> dict[str, int]:
  if (hit:=_pack_memo.get(i:=id(raw))) is not None: return hit
  name = _iname(raw) or ""
  _pack_memo[i] = ret = {k: _pack_one(raw, name, k) for k in _layout_keys(raw)}
  return ret

### CTX — UOp src/wr/exec per encoding
...
class _Ctx:
  def __init__(self, nt: int):
    ...
    self.caddr = self._caddr = UOp.param(5, dtypes.uint64, 1)
    self.wst = UOp.param(6, dtypes.uint32, 64)
    self._pc = self.wst.index(0).load()
    self.tid, self.active = None, None  # set in begin_lane: one RANGE per ISA insn

  def begin_lane(self, axis: int, prev: UOp|None=None):
    self.tid = UOp.range(self.nt, axis, dtype=dtypes.int)
    if prev is not None:
      self.gpr, self.h = self.gpr.after(prev), self.h.after(prev)
      self.lds, self.pvt, self.vmem = self.lds.after(prev), self.pvt.after(prev), self.vmem.after(prev)
    # pin loads to this RANGE so RANGE-independent addrs do not CSE into an earlier loop
    self.vmem, self.lds, self.pvt = self.vmem.after(self.tid), self.lds.after(self.tid), self.pvt.after(self.tid)
    self.caddr = self._caddr.after(self.tid)
    wst = self.wst.after(self.tid)
    def u64(lo: int) -> UOp:
      return (wst.index(_c(lo)).load().cast(dtypes.uint64) |
              (wst.index(_c(lo + 1)).load().cast(dtypes.uint64) << _c(32, dtypes.uint64)))
    act, pmode, pmask = u64(2), wst.index(4).load(), u64(6)
  ...
  def bind(self, raw: dict):
    self.raw = raw
    packed = _pack_named(raw)
    for k in _layout_keys(raw): self.pval[k] = _c(packed[k])
...
### COMPILE — one RANGE per ISA insn; REPEAT stays inside that RANGE

def _compile(raws: list[dict], nt: int) -> UOp:
  ctx, ended, prev = _Ctx(nt), [], None
  for i, raw in enumerate(raws):
    name = _iname(raw)
    if name in _CTRL or name in ("sct", "gat") or name == "getone":
    ...
    prev = UOp.group(*ctx.stores).end(ctx.tid)
    ended.append(prev)
  kn = "ir3w_" + hashlib.sha1(repr((tuple((_kind(r), _bake(r)) for r in raws), nt)).encode()).hexdigest()[:12]
  pcst = ctx.wst.after(ended[-1]).index(0).store(ctx._pc + _c(len(raws)))
  return UOp.sink(*ended, pcst, arg=KernelInfo(name=kn), tag=1)

def _bake(raw: dict) -> tuple:
  packed = _pack_named(raw)
  return tuple(packed.get(k) for k in _ENC_KEYS)

_runners: dict[tuple, tuple] = {}
_runners_id: dict[tuple, tuple] = {}
def _get_runner(raws: list[dict], nt: int):
  if (hit:=_runners_id.get(ik:=(tuple(id(r) for r in raws), nt))) is not None: return hit
  key = (tuple((_kind(r), _bake(r)) for r in raws), nt)
  if key not in _runners:
    try:
      sink = _compile(raws, nt)
      ...
      names = [_iname(r) for r in raws]
      raise RuntimeError(f"ir3 uop compile {names}: {type(e).__name__}: {e}") from e
    _runners[key] = (prg, runtime)
  _runners_id[ik] = _runners[key]
  return _runners[key]
...
_wave_call: dict[tuple, tuple] = {}
def _run_wave_op(raws: list[dict], nt: int, c_bufs: list):
  if (hit:=_wave_call.get(ik:=(tuple(id(r) for r in raws), nt))) is None:
    prg, runtime = _get_runner(raws, nt)
    gl = tuple(prg.arg.globals)
    hit = _wave_call[ik] = (runtime.fxn, gl, gl == tuple(range(len(c_bufs))))
  fxn, gl, contig = hit
...
def _is_compiled(name: str|None) -> bool:
  return name not in _CTRL and name not in ("sct", "gat") and name != "getone"

def _alu_ends(instrs: list[dict], wake_at: list[bool]) -> list[int]:
  n, leaders = len(instrs), {pc for pc, w in enumerate(wake_at) if w}
  out = list(range(n))
  for pc in range(n):
    if not _is_compiled(_iname(instrs[pc])): continue
    j = pc + 1
    while j < n and j not in leaders and _is_compiled(_iname(instrs[j])): j += 1
    out[pc] = j
  return out

def _wake_at(instrs: list[dict]) -> list[bool]:
  n, at = len(instrs), [False] * len(instrs)
  for pc, raw in enumerate(instrs):
    if raw.get("JP"): at[pc] = True
    name = _iname(raw)
    if name in ("jump", "br", "brao", "braa", "getone") and "IMMED" in raw:
      tgt = (pc + _s32(raw["IMMED"])) & 0xffffffff
      if tgt < n: at[tgt] = True
    if name in ("br", "brao", "braa", "getone") and pc + 1 < n: at[pc + 1] = True
  return at

def _step_wave(wave: _Wave, instrs: list[dict], wg, nins: int, wake_at: list[bool]|None=None, alu_end: list[int]|None=None):
  if wake_at is None: wake_at = _wake_at(instrs)
  if alu_end is None: alu_end = _alu_ends(instrs, wake_at)
  for _ in range(1_000_000):
    ...
    if pc < 0 or pc >= nins:
      ...
      return
    raw = instrs[pc]
    if wake_at[pc] and int(wave.sp_mv[0]): wave.wake(pc, jp=bool(raw.get("JP")))
    name = _iname(raw)
    if _is_cat0(raw, name):
      if _exec_cat0(wave, wg, raw, name) == "bar": return
    elif name in ("sct", "gat"):
      raise RuntimeError(f"unhandled ir3 {name} {raw}")
    else:
      _run_wave_op(instrs[pc:alu_end[pc]], wave.nlanes, unwrap(wave.c_bufs))
  raise RuntimeError("ir3 wave exceeded 1M instructions")

def _run_waves(waves: list[_Wave], instrs: list[dict], wg, wake_at: list[bool]|None=None, alu_end: list[int]|None=None):
  nins = len(instrs)
  if wake_at is None: wake_at = _wake_at(instrs)
  if alu_end is None: alu_end = _alu_ends(instrs, wake_at)
  for _ in range(10_000_000):
    ...
    for w in wait: _step_wave(w, instrs, wg, nins, wake_at, alu_end)
...
def run_shader(base: int, sz: int, const_addr: int, regs: dict[int, int], branchstack: int=8):
  ...
  wake_at = _wake_at(instrs)
  alu_end = _alu_ends(instrs, wake_at)
  ...
  for gz, gy, gx in itertools.product(range(ng[2]), range(ng[1]), range(ng[0])):
    ...
    waves = _waves(wg)
    for w in waves:
      w.c_bufs, w.park_limit = wg.c_bufs(w.base, w.addr), park
    _run_waves(waves, instrs, wg, wake_at, alu_end)
