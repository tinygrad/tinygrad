...
from test.mockgpu.qcom.emu import A630Emu
...
class A630GPU:
  ...
  def _execute(self, dat:bytes) -> bytes:
    ...
    while ptr + 4 <= len(dat):
      ...
      if (cmd >> 24) == 0x70:
        ...
        if not self._pkt7(opcode, vals): return dat[ptr:]
        ptr += 4 + 4 * n
      elif (cmd >> 28) == 0x4:
  ...
  def _pkt7(self, opcode:int, vals:tuple[int, ...]) -> bool:
    if opcode in (mesa.CP_WAIT_FOR_IDLE, mesa.CP_WAIT_MEM_WRITES, mesa.CP_SET_MARKER, mesa.CP_NOP): return True

    if opcode == mesa.CP_EVENT_WRITE and len(vals) >= 1:
      event = vals[0] & 0xFF
      if event == mesa.CACHE_FLUSH_TS and len(vals) >= 4:
        addr = self._u64(vals[1], vals[2])
        to_mv(addr, 4).cast('I')[0] = vals[3]
    ...
    if opcode == mesa.CP_REG_TO_MEM and len(vals) >= 3:
      ...
      to_mv(dest, 8).cast('Q')[0] = self.always_on
      return True
