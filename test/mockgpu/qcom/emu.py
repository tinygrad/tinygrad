      ...
      for component in range(inst.repeat + 1):
        ...
        for raw in zip(*values, strict=True):
          if inst.name.startswith('mad.f'):
            val = math.fma(_mod_float(raw[0], inst.src_mods[0], half), _mod_float(raw[1], inst.src_mods[1], half),
...
def write_u32(addr:int, val:int) -> None: to_mv(addr, 4).cast('I')[0] = val & 0xFFFFFFFF
def write_u64(addr:int, val:int) -> None: to_mv(addr, 8).cast('Q')[0] = val & 0xFFFFFFFFFFFFFFFF
def read_u32(addr:int) -> int: return int(to_mv(addr, 4).cast('I')[0])
def memcpy_u8(dst:int, src:int, n:int) -> None: ctypes.memmove(dst, src, n)
