...
from tinygrad.helpers import DEV, IMAGE, Target
...
class TestA630CommandProcessor(unittest.TestCase):
  def test_raw_packet_header_and_control_goldens(self):
    self.assertEqual(pkt4_hdr(mesa.REG_A6XX_SP_CS_NDRANGE_0, 2), 0x40b99002)
    self.assertEqual(pkt7_hdr(mesa.CP_MEMCPY, 5), 0x70758005)
    self.assertEqual(qreg.cp_wait_reg_mem_0(function=mesa.WRITE_GE, poll=mesa.POLL_MEMORY), 0x15)

  def test_ir3_mock_trig_support_is_target_local(self):
    mock = IR3Renderer(Target.parse('MOCK+QCOM:IR3:a630'))
    real = IR3Renderer(Target.parse('QCOM:IR3:a630'))
    self.assertNotIn(Ops.SIN, mock.code_for_op)
    self.assertIn(Ops.SIN, real.code_for_op)
  ...
  def test_reserved_packet_bits_do_not_mutate_state(self):
    gpu = A630GPU(0)
    register = mesa.REG_A6XX_SP_CS_NDRANGE_0
    packets = [
      [pkt4_hdr(register, 1) | (1 << 26), 0x1234],
      [pkt7_hdr(mesa.CP_WAIT_FOR_IDLE, 0) | (1 << 14)],
      [pkt7_hdr(mesa.CP_WAIT_FOR_IDLE, 0) | (1 << 24)],
    ]
    for packet in packets:
      with self.subTest(header=packet[0]), self.assertRaisesRegex(ValueError, "reserved bits"):
        gpu.execute_words(packet)
      self.assertEqual(gpu.regs, {})

  def test_mapping_rejects_wrapped_range(self):
    gpu = A630GPU(0)
    with self.assertRaisesRegex(ValueError, "overflows the 64-bit address space"):
      gpu.map_range((1 << 64) - 1, 2)
    self.assertEqual(gpu.mapped_ranges, {})

  def test_kernel_seeds_half_constants_from_low_bits(self):
    gpu = A630GPU(0)
    shader, constants = ctypes.create_string_buffer(128), (ctypes.c_uint32 * 4)(0x00008000, 0x00007fff, 3, 4)
    shader_addr, constants_addr = ctypes.addressof(shader), ctypes.addressof(constants)
    gpu.map_range(shader_addr, ctypes.sizeof(shader))
    gpu.map_range(constants_addr, ctypes.sizeof(constants))
    gpu.states[(mesa.SB6_CS_SHADER, mesa.ST_CONSTANTS)] = (constants_addr, 1)
    gpu.regs.update({
      mesa.REG_A6XX_SP_CS_BASE: shader_addr & 0xffffffff,
      mesa.REG_A6XX_SP_CS_BASE + 1: shader_addr >> 32,
      mesa.REG_A6XX_SP_CS_INSTR_SIZE: 1,
      mesa.REG_A6XX_SP_CS_NDRANGE_0: qreg.a6xx_sp_cs_ndrange_0(kerneldim=3, localsizex=0, localsizey=0, localsizez=0),
      mesa.REG_A6XX_SP_CS_CONST_CONFIG_0: qreg.a6xx_sp_cs_const_config_0(localidregid=0xfc, wgidconstid=0xfc),
    })
  ...
  def test_kernel_only_seeds_mapped_constant_tail(self):
    gpu = A630GPU(0)
    shader, constants = ctypes.create_string_buffer(128), (ctypes.c_uint32 * 4)(1, 2, 3, 4)
    shader_addr, constants_addr = ctypes.addressof(shader), ctypes.addressof(constants)
    gpu.map_range(shader_addr, ctypes.sizeof(shader))
    gpu.map_range(constants_addr, ctypes.sizeof(constants))
    gpu.states[(mesa.SB6_CS_SHADER, mesa.ST_CONSTANTS)] = (constants_addr, 256)
    gpu.regs.update({
      mesa.REG_A6XX_SP_CS_BASE: shader_addr & 0xffffffff,
      mesa.REG_A6XX_SP_CS_BASE + 1: shader_addr >> 32,
      mesa.REG_A6XX_SP_CS_INSTR_SIZE: 1,
  ...
  def test_memcpy_packet(self):
    gpu = A630GPU(0)
    source, destination = (ctypes.c_uint32 * 2)(0x12345678, 0xabcdef01), (ctypes.c_uint32 * 2)()
    for obj in (source, destination): gpu.map_range(ctypes.addressof(obj), ctypes.sizeof(obj))
    gpu.execute_words([pkt7_hdr(mesa.CP_MEMCPY, 5), 2, ctypes.addressof(source) & 0xffffffff,
                       ctypes.addressof(source) >> 32, ctypes.addressof(destination) & 0xffffffff,
                       ctypes.addressof(destination) >> 32])
    self.assertEqual(list(destination), list(source))

    byte_source, byte_destination = ctypes.c_uint8(0xa5), ctypes.c_uint8(0)
    for byte_obj in (byte_source, byte_destination): gpu.map_range(ctypes.addressof(byte_obj), ctypes.sizeof(byte_obj))
    gpu.execute_words([pkt7_hdr(mesa.CP_MEMCPY, 5), (1 << 31) | 1, ctypes.addressof(byte_source) & 0xffffffff,
                       ctypes.addressof(byte_source) >> 32, ctypes.addressof(byte_destination) & 0xffffffff,
                       ctypes.addressof(byte_destination) >> 32])
    self.assertEqual(byte_destination.value, byte_source.value)

  def test_copy_engine_can_release_compute_wait_in_same_context(self):
    gpu = A630GPU(0)
    source, signal = ctypes.c_uint32(1), ctypes.c_uint32(0)
    for obj in (source, signal): gpu.map_range(ctypes.addressof(obj), ctypes.sizeof(obj))
    wait = [pkt7_hdr(mesa.CP_WAIT_REG_MEM, 6), qreg.cp_wait_reg_mem_0(function=mesa.WRITE_GE, poll=mesa.POLL_MEMORY),
            ctypes.addressof(signal) & 0xffffffff, ctypes.addressof(signal) >> 32, 1, 0xffffffff, 32]
    copy = [pkt7_hdr(mesa.CP_MEMCPY, 5), 1, ctypes.addressof(source) & 0xffffffff, ctypes.addressof(source) >> 32,
            ctypes.addressof(signal) & 0xffffffff, ctypes.addressof(signal) >> 32]
    gpu.submit(wait, 1, 7)
    gpu.submit(copy, 2, 7)
    gpu.progress()
    self.assertEqual((signal.value, gpu.completed_timestamps[7], gpu.pending), (1, 2, []))
...
class TestQCOMDriverLifecycle(unittest.TestCase):
  ...
  def test_waittimestamp_is_context_local_when_a_wait_is_blocked(self):
    driver = QCOMDriver()
    contexts = [kgsl.struct_kgsl_drawctxt_create() for _ in range(2)]
    for context in contexts: driver.ioctl(0x13, ctypes.addressof(context))
    ...
    signal = ctypes.c_uint32()
    signal_addr = ctypes.addressof(signal)
    wait_words = (ctypes.c_uint32 * 7)(pkt7_hdr(mesa.CP_WAIT_REG_MEM, 6),
      qreg.cp_wait_reg_mem_0(function=mesa.WRITE_GE, poll=mesa.POLL_MEMORY),
      signal_addr & 0xffffffff, signal_addr >> 32, 1, 0xffffffff, 32)
    complete_words = (ctypes.c_uint32 * 2)(pkt4_hdr(mesa.REG_A6XX_SP_CS_NDRANGE_0, 1), 0x1234)
    for obj in (signal, wait_words, complete_words):
      mapping = kgsl.struct_kgsl_map_user_mem(hostptr=ctypes.addressof(obj), len=ctypes.sizeof(obj))
      driver.ioctl(0x15, ctypes.addressof(mapping))
    for context, words in zip(contexts, (wait_words, complete_words)):
      command = kgsl.struct_kgsl_command_object(gpuaddr=ctypes.addressof(words), size=ctypes.sizeof(words))
      submission = kgsl.struct_kgsl_gpu_command(cmdlist=ctypes.addressof(command), numcmds=1, context_id=context.drawctxt_id,
                                                 cmdsize=ctypes.sizeof(command))
      driver.ioctl(0x4a, ctypes.addressof(submission))

    first_wait = kgsl.struct_kgsl_device_waittimestamp_ctxtid(context_id=contexts[0].drawctxt_id, timestamp=1)
    with self.assertRaisesRegex(TimeoutError, "blocked before timestamp 1"):
      driver.ioctl(0x07, ctypes.addressof(first_wait))
    second_wait = kgsl.struct_kgsl_device_waittimestamp_ctxtid(context_id=contexts[1].drawctxt_id, timestamp=1)
    driver.ioctl(0x07, ctypes.addressof(second_wait))
    self.assertNotIn(contexts[0].drawctxt_id, driver.gpu.completed_timestamps)
    self.assertEqual(driver.gpu.completed_timestamps[contexts[1].drawctxt_id], 1)
  ...
  def test_execution_error_rolls_back_pm4_state_and_writes(self):
    driver = QCOMDriver()
    context = kgsl.struct_kgsl_drawctxt_create()
    driver.ioctl(0x13, ctypes.addressof(context))
    signal = ctypes.c_uint32()
    ...
    words = (ctypes.c_uint32 * 10)(
      pkt7_hdr(mesa.CP_EVENT_WRITE, 4), qreg.cp_event_write_0(event=mesa.CACHE_FLUSH_TS),
      signal_addr & 0xffffffff, signal_addr >> 32, 0x4567,
      pkt7_hdr(mesa.CP_EXEC_CS, 4), 0, 1, 1, 1,
    )
    for obj in (signal, words):
      mapping = kgsl.struct_kgsl_map_user_mem(hostptr=ctypes.addressof(obj), len=ctypes.sizeof(obj))
      driver.ioctl(0x15, ctypes.addressof(mapping))
  ...
  def test_command_list_overflow_and_timestamp_exhaustion_are_transactional(self):
    driver = QCOMDriver()
    context = kgsl.struct_kgsl_drawctxt_create()
    driver.ioctl(0x13, ctypes.addressof(context))
    submission = kgsl.struct_kgsl_gpu_command(cmdlist=(1 << 64) - 16, numcmds=1, context_id=context.drawctxt_id,
                                               cmdsize=ctypes.sizeof(kgsl.struct_kgsl_command_object))
    with self.assertRaisesRegex(ValueError, "command list overflows the 64-bit address space"):
      driver.ioctl(0x4a, ctypes.addressof(submission))
    self.assertEqual(submission.timestamp, 0)
    self.assertEqual(driver.submitted_timestamps[context.drawctxt_id], 0)
    self.assertEqual(driver.gpu.pending, [])

    words = (ctypes.c_uint32 * 2)(pkt4_hdr(mesa.REG_A6XX_SP_CS_NDRANGE_0, 1), 0x1234)
    mapping = kgsl.struct_kgsl_map_user_mem(hostptr=ctypes.addressof(words), len=ctypes.sizeof(words))
    driver.ioctl(0x15, ctypes.addressof(mapping))
    command = kgsl.struct_kgsl_command_object(gpuaddr=ctypes.addressof(words), size=ctypes.sizeof(words))
    submission = kgsl.struct_kgsl_gpu_command(cmdlist=ctypes.addressof(command), numcmds=1, context_id=context.drawctxt_id,
                                               cmdsize=ctypes.sizeof(command))
    driver.submitted_timestamps[context.drawctxt_id] = (1 << 32) - 1
    with self.assertRaisesRegex(RuntimeError, "timestamps are exhausted"):
      driver.ioctl(0x4a, ctypes.addressof(submission))
    self.assertEqual(submission.timestamp, 0)
    self.assertEqual(driver.gpu.pending, [])
