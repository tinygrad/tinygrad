...
from tinygrad.uop.ops import Ops, UOp, python_alu
from test.mockgpu.qcom.emu import CapturedProgram, _normalize_python_uops, _uses_ir3_layout, bind_program, find_program, unbind_program
...
class TestA630CommandProcessor(unittest.TestCase):
  def test_renderer_fallback_layout_detection(self):
    from tinygrad.device import TinyELF
    from tinygrad.helpers import Target
    sink, linear = UOp(Ops.SINK), UOp(Ops.LINEAR)
    obj = TinyELF(b"", "test", Target(device="QCOM"), ())
    self.assertTrue(_uses_ir3_layout(obj, UOp(Ops.PROGRAM, src=(sink, linear, UOp(Ops.SOURCE, arg="bmly")))))
    self.assertFalse(_uses_ir3_layout(obj, UOp(Ops.PROGRAM, src=(sink, linear, UOp(Ops.SOURCE, arg="__kernel void test() {}")))))

  def test_program_rejects_oversized_global_dimensions(self):
    program = object.__new__(QCOMProgram)
    program.max_threads = 1024
    for axis in range(3):
      global_size = tuple(65537 if index == axis else 1 for index in range(3))
  ...
  def test_import_does_not_change_python_alu(self):
    self.assertNotIn(Ops.FDIV, python_alu)

  def test_python_uops_expand_nested_dependencies(self):
    base = UOp.const(3, dtypes.uint)
    nested = UOp(Ops.ADD, dtypes.uint, (base, UOp.const(2, dtypes.uint)))
    result = UOp(Ops.SHL, dtypes.uint, (nested, UOp.const(1, dtypes.ushort)))
    normalized = _normalize_python_uops([result])
    seen:set[UOp] = set()
    for uop in normalized:
      self.assertTrue(all(source.dtype is dtypes.void or source in seen for source in uop.src))
      seen.add(uop)
    self.assertIs(normalized[-1].op, Ops.SHL)
    self.assertIs(normalized[-1].src[1].dtype, dtypes.uint)
  ...
  def test_identical_shader_images_keep_program_identity(self):
    image = b"\x01\x02\x03\x04"
    shader_a, shader_b = ctypes.create_string_buffer(image), ctypes.create_string_buffer(image)
    program_a = CapturedProgram(image, cast(PythonProgram, None), (), (), ())
    program_b = CapturedProgram(image, cast(PythonProgram, None), (), (), ())
    address_a, address_b = ctypes.addressof(shader_a), ctypes.addressof(shader_b)
    try:
      bind_program(address_a, program_a)
      bind_program(address_b, program_b)
      self.assertIs(find_program(address_a, len(image)), program_a)
      self.assertIs(find_program(address_b, len(image)), program_b)
    finally:
      unbind_program(address_a)
      unbind_program(address_b)
