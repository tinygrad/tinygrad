...
class PythonRemu:
  ...
  rsrc3: int | None = None  # CDNA ACCUM_OFFSET (kernel-specific AccVGPR base)
  ...
  def run_asm(self, lib: int, lib_sz: int, gx: int, gy: int, gz: int, lx: int, ly: int, lz: int, args_ptr: int) -> int:
    ...
    return run_asm(lib, lib_sz, gx, gy, gz, lx, ly, lz, args_ptr, self.rsrc2, self.scratch_size, self.arch, self.user_data, self.rsrc3)
