...
from test.mockgpu.driver import (
    VirtDriver,
    VirtFileDesc,
    TextFileDesc,
    DirFileDesc,
...
def ioctls_from_header():
    ...
    return type(
        "KFD_IOCTLS", (object,), {name: int(nr, 0x10) for name, _, nr, _ in matches}
    ), {
        int(nr, 0x10): getattr(kfd, "struct_" + sname)
...
class KFDFileDesc(VirtFileDesc):
    ...
    def ioctl(self, fd, request, argp):
        return self.driver.kfd_ioctl(request, argp)
...
class AMDDriver(VirtDriver):
    def __init__(self, gpus=6):
        ...
        self.tracked_files += [
            VirtFile("/dev/kfd", functools.partial(KFDFileDesc, driver=self))
        ] + [
            VirtFile(
                "/sys/devices/virtual/kfd/kfd/topology/nodes",
                functools.partial(
                    DirFileDesc, child_names=[str(i) for i in range(gpus)]
                ),
            )
        ]
    ...
    def _alloc_doorbell(self, gpu_id):
        x = (
            ctypes.addressof(from_mv(self.doorbells[gpu_id]))
            + self.next_doorbell[gpu_id] * 8
        )
    ...
    def _prepare_gpu(self, gpu_id):
        ...
        self.tracked_files += [
            ...
            VirtFile(
                f"/sys/devices/virtual/kfd/kfd/topology/nodes/{gpu_id}",
                functools.partial(DirFileDesc, child_names=["gpu_id", "properties"]),
            ),
            VirtFile(
                f"/sys/devices/virtual/kfd/kfd/topology/nodes/{gpu_id}/gpu_id",
                functools.partial(TextFileDesc, text=f"{gpu_id}"),
            ),
            VirtFile(
                f"/sys/devices/virtual/kfd/kfd/topology/nodes/{gpu_id}/properties",
                functools.partial(
                    TextFileDesc, text=gpu_props.format(drm_render_minor=gpu_id)
                ),
            ),
            VirtFile(
                f"/dev/dri/renderD{gpu_id}",
                functools.partial(DRMFileDesc, driver=self, gpu=f"{self.gpus[gpu_id]}"),
            ),
        ]
    ...
    def kfd_ioctl(self, req, argp):
        ...
        if nr == kfd_ioctls.AMDKFD_IOC_ACQUIRE_VM:
            pass
        elif nr == kfd_ioctls.AMDKFD_IOC_ALLOC_MEMORY_OF_GPU:
            if struct.gpu_id not in self.gpus:
                return -1
            struct.handle = self._alloc_handle()
            self.object_by_handle[struct.handle] = copy.deepcopy(
                struct
            )  # save memory struct to know what mem it is
            ...
            dev_ids = (ctypes.c_int32 * struct.n_devices).from_address(
                struct.device_ids_array_ptr
            )
            ...
            if struct.event_type == kfd.KFD_IOC_EVENT_MEMORY:
                self.mmu_event_ids.append(struct.event_id)
        elif nr == kfd_ioctls.AMDKFD_IOC_CREATE_QUEUE:
            gpu = self.gpus[struct.gpu_id]
            if struct.queue_type == kfd.KFD_IOC_QUEUE_TYPE_SDMA:
                gpu.add_sdma_queue(
                    struct.ring_base_address,
                    struct.ring_size,
                    struct.read_pointer_address,
                    struct.write_pointer_address,
                )
                ...
                raise RuntimeError("Unsuported, queue")

            # Track writes to doorbell, calling callback
            struct.doorbell_offset = self._alloc_doorbell(struct.gpu_id)
            self.track_address(
                struct.doorbell_offset,
                struct.doorbell_offset + 8,
                lambda mv, off: None,
                lambda mv, off: self._emulate_execute(),
            )
        elif nr == kfd_ioctls.AMDKFD_IOC_WAIT_EVENTS:
            evs = (kfd.struct_kfd_event_data * struct.num_events).from_address(
                struct.events_ptr
            )
            for ev in evs:
                if (
                    ev.event_id in self.mmu_event_ids
                    and "MOCKGPU_EMU_FAULTADDR" in os.environ
                ):
                    ev.memory_exception_data.gpu_id = 1
                    ev.memory_exception_data.va = int(
                        os.environ["MOCKGPU_EMU_FAULTADDR"], 16
                    )
