...
REMU_PATHS = [
    "libremu.so",
    "/usr/local/lib/libremu.so",
    "libremu.dylib",
    "/usr/local/lib/libremu.dylib",
    "/opt/homebrew/lib/libremu.dylib",
]
...
def create_sdma_packets():
    ...
    for name, pkt in [
        (name, s)
        for name, s in amd_gpu.__dict__.items()
        ...
        assert ctypes.sizeof(structs[new_name]) == ctypes.sizeof(
            pkt
        ), f"{ctypes.sizeof(structs[new_name])} != {ctypes.sizeof(pkt)}"
...
class PM4Executor(AMDQueue):
    ...
    def _exec_dispatch_direct(self, n):
        ...
        prg_addr = (
            self.gpu.regs[regCOMPUTE_PGM_LO]
            + (self.gpu.regs[regCOMPUTE_PGM_LO + 1] << 32)
        ) << 8
        args_addr = self.gpu.regs[regCOMPUTE_USER_DATA_0] + (
            self.gpu.regs[regCOMPUTE_USER_DATA_0 + 1] << 32
        )
        lc = [
            self.gpu.regs[i]
            for i in range(regCOMPUTE_NUM_THREAD_X, regCOMPUTE_NUM_THREAD_X + 3)
        ]
        ...
        if err != 0:
            raise RuntimeError(
                "remu does not support the new instruction introduced in this kernel"
            )
...
class SDMAExecutor(AMDQueue):
    ...
    def _execute_poll_regmem(self):
        struct = sdma_pkts.poll_regmem.from_address(
            self.base + self.rptr[0] % self.size
        )
    ...
    def _execute_copy(self):
        struct = sdma_pkts.copy_linear.from_address(
            self.base + self.rptr[0] % self.size
        )
