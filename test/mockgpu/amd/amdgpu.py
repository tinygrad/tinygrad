...
class AMDQueue:
  ...
  @property
  def executing(self):
    return self.rptr[0] < self.wptr[0]
...
class PM4Executor(AMDQueue):
  ...
  @property
  def executing(self):
    return self.rptr[0] < self.wptr[0] or self.ib_executor is not None
  ...
  def execute(self):
    ...
    while self.executing and cont:
      ...
      if op == amd_gpu.PACKET3_SET_SH_REG:
        self._exec_set_reg(n, pm4.PACKET3_SET_SH_REG_START)
      elif op == amd_gpu.PACKET3_SET_UCONFIG_REG:
        self._exec_set_reg(n, pm4.PACKET3_SET_UCONFIG_REG_START)
      elif op == amd_gpu.PACKET3_ACQUIRE_MEM:
        self._exec_acquire_mem(n)
      elif op == amd_gpu.PACKET3_RELEASE_MEM:
        self._exec_release_mem(n)
      elif op == amd_gpu.PACKET3_COPY_DATA:
        self._exec_copy_data(n)
      elif op == amd_gpu.PACKET3_WAIT_REG_MEM:
        cont = self._exec_wait_reg_mem(n)
      elif op == amd_gpu.PACKET3_DISPATCH_DIRECT:
        self._exec_dispatch_direct(n)
      elif op == amd_gpu.PACKET3_INDIRECT_BUFFER:
        self._exec_indirect_buffer(n)
      elif op == amd_gpu.PACKET3_EVENT_WRITE:
        self._exec_event_write(n)
      else:
        raise RuntimeError(f"PM4: Unknown opcode: {op}")
    return (self.rptr[0] - prev_rptr) + executed_in_ib

  def _exec_acquire_mem(self, n):
    assert n == 6
    for _ in range(7):
      self._next_dword()  # TODO: implement
  ...
  def _exec_release_mem(self, n):
    ...
    if mem_data_sel == 1 or mem_data_sel == 2:
      ptr.cast("Q")[0] = val
    elif mem_data_sel == 3:
      if mem_event_type == CACHE_FLUSH_AND_INV_TS_EVENT:
        ptr.cast("Q")[0] = int(time.perf_counter() * 1e8)
      else:
        raise RuntimeError(f"Unknown {mem_data_sel=} {mem_event_type=}")
    else:
      raise RuntimeError(f"Unknown {mem_data_sel=}")
  ...
  def _exec_wait_reg_mem(self, n):
    ...
    if mem_space == 0 and mem_op == 1:
      mval = val  # hack for memory barrier, should properly handle (req_req, reg_done)
    elif mem_space == 0:
      mval = self.gpu.regs[addr_hi << 32 | addr_lo]
    elif mem_space == 1:
      mval = to_mv(addr_lo + (addr_hi << 32), 4).cast("I")[0]

    mval &= mask

    if mem_function == WAIT_REG_MEM_FUNCTION_GEQ:
      can_cont = bool(mval >= val)
    elif mem_function == WAIT_REG_MEM_FUNCTION_NEQ:
      can_cont = bool(mval != val)
    elif mem_function == WAIT_REG_MEM_FUNCTION_EQ:
      can_cont = bool(mval == val)
    else:
      raise RuntimeError(f"Do not support {mem_function=}")

    if not can_cont:
      self.rptr[0] = self.rptr[0] - 7  # revert this packet, need to wait again
    return can_cont
  ...
  def _exec_dispatch_direct(self, n):
    ...
    if err != 0:
      raise RuntimeError("remu does not support the new instruction introduced in this kernel")
  ...
  def _exec_event_write(self, n):
    ...
    match event_dw & 0xFF:  # event type
      ...
      case _:
        pass  # NOTE: for now most events aren't emulated
...
class SDMAExecutor(AMDQueue):
  ...
  def execute(self):
    ...
    while self.executing and cont:
      ...
      if op == 0:
        self.rptr[0] += 4
      elif op == amd_gpu.SDMA_OP_FENCE:
        self._execute_fence()
      elif op == amd_gpu.SDMA_OP_TRAP:
        self._execute_trap()
      elif op == amd_gpu.SDMA_OP_POLL_REGMEM:
        cont = self._execute_poll_regmem()
      elif op == amd_gpu.SDMA_OP_GCR:
        self._execute_gcr()
      elif op == amd_gpu.SDMA_OP_COPY:
        self._execute_copy()
      elif op == amd_gpu.SDMA_OP_TIMESTAMP:
        self._execute_timestamp()
      else:
        raise RuntimeError(f"Unknown SDMA op {op}")
  ...
  def _execute_poll_regmem(self):
    ...
    if struct.mem_poll == 0:
      mval = struct.value & struct.mask
    elif struct.mem_poll == 1:
      mval = to_mv(struct.addr, 4).cast("I")[0] & struct.mask

    if struct.func == WAIT_REG_MEM_FUNCTION_GEQ:
      can_cont = bool(mval >= struct.value)
    elif struct.func == WAIT_REG_MEM_FUNCTION_EQ:
      can_cont = bool(mval == struct.value)
    elif struct.func == WAIT_REG_MEM_FUNCTION_ALWAYS:
      can_cont = True
    else:
      raise RuntimeError(f"Do not support {struct.func=}")
...
class AMDGPURegisters:
  ...
  def __setitem__(self, addr: int, val: int):
    ...
    if getbits(self.grbm_index, 31, 31):
      for se in range(self.n_se):
        self.regs[(addr, se)] = val
    else:
...
class AMDGPU(VirtGPU):
  ...
  def map_range(self, vaddr, size):
    self.mapped_ranges.add((vaddr, size))

  def unmap_range(self, vaddr, size):
    self.mapped_ranges.remove((vaddr, size))
