...
# Exec mask type depends on wave size: 32-bit for RDNA (32 lanes), 64-bit for CDNA4 (64 lanes)
EXEC_MASK_DTYPE = dtypes.uint64 if WAVE_SIZE == 64 else dtypes.uint32
EXEC_ALL_ONES = (1 << WAVE_SIZE) - 1
...
def _lane_active(exec_mask: UOp, lane: UOp) -> UOp:
  if WAVE_SIZE == 64:
    return ((exec_mask.cast(dtypes.uint64) >> lane.cast(dtypes.uint64)) & UOp.const(dtypes.uint64, 1)).ne(UOp.const(dtypes.uint64, 0))
...
def _set_lane_bit(old: UOp, lane: UOp, val: UOp, exec_mask: UOp) -> UOp:
  ...
  if WAVE_SIZE == 64:
    mask = UOp.const(dtypes.uint64, 1) << lane.cast(dtypes.uint64)
    new_bit = _to_u32(val).cast(dtypes.uint64) << lane.cast(dtypes.uint64)
    cleared = old.cast(dtypes.uint64) & (mask ^ UOp.const(dtypes.uint64, (1 << 64) - 1))
...
class _Ctx:
  ...
  def unroll_lanes(self, get_lane_bit, exec_mask: UOp, apply_exec: bool = True) -> UOp:
    ...
    return result & exec_mask.cast(dt) if apply_exec else result

  def rexec(self) -> UOp:
    """Read the EXEC mask as the correct width (32-bit for RDNA, 64-bit for CDNA4)."""
    if WAVE_SIZE == 64:
      from tinygrad.renderer.amd.dsl import EXEC_HI
      lo = self.rsgpr_dyn(_c(EXEC_LO.offset)).cast(dtypes.uint64)
      hi = self.rsgpr_dyn(_c(EXEC_HI.offset)).cast(dtypes.uint64)
      return lo | (hi << UOp.const(dtypes.uint64, 32))
    return self.rsgpr_dyn(_c(EXEC_LO.offset))

  def wexec(self, val: UOp) -> list[UOp]:
    """Write the EXEC mask, splitting into LO/HI for 64-bit."""
    if WAVE_SIZE == 64:
      from tinygrad.renderer.amd.dsl import EXEC_HI
      lo = val.cast(dtypes.uint32)
      hi = (val.cast(dtypes.uint64) >> UOp.const(dtypes.uint64, 32)).cast(dtypes.uint32)
  ...
  def scalar_stores(self, assigns: list[tuple[str, UOp]], sdst_reg: UOp, sdst_size: int = 1) -> list[UOp]:
    ...
    for dest, val in assigns:
      ...
      elif dest.startswith('EXEC'): stores.extend(self.wexec(val))
  ...
  def compile_sop_pcode(self, op, srcs: dict[str, UOp], sdst_reg: UOp, sdst_size: int) -> UOp:
    ...
    pcode = get_pcode(op)
    srcs.update({'VCC': self.rsgpr_dyn(_c(VCC_LO.offset)), 'EXEC': self.rexec(), 'SCC': self.rsgpr_dyn(_c(SCC.offset))})
  ...
  def compile_lane_pcode(self, op, inst) -> UOp:
    ...
    exec_lo = self.rexec()
    exec_lo_32 = exec_lo.cast(dtypes.uint32) if WAVE_SIZE == 64 else exec_lo
    srcs = {
      'SRC0': src0_reg, 'VDST': vdst_off, 'EXEC_LO': exec_lo_32, 'EXEC': exec_lo.cast(dtypes.uint64), '_vgpr': self.vgpr,
  ...
  def compile_vop_pcode(self, op, srcs: dict[str, UOp], lane: UOp, vdst_reg: UOp, exec_mask: UOp,
    ...
    for mask_val, reg in [(vcc_val, vcc_reg), (exec_val, EXEC_LO.offset)]:
      ...
      if reg == EXEC_LO.offset:
        stores.extend(self.wexec(new_mask))
      else:
        stores.append(self.wsgpr_dyn(_c(reg), new_mask.cast(dtypes.uint32)))
...
def _compile_sopp(inst: ir3.SOPP | ir4.SOPP, ctx: _Ctx) -> UOp:
  ...
  if inst.op in _get_pcode_dict(inst.op):
    ...
    vcc, exec_lo = ctx.rsgpr_dyn(_c(VCC_LO.offset)), ctx.rexec()
...
def _compile_vopc(inst: ir3.VOPC|ir3.VOP3|ir4.VOPC|ir4.VOP3|irc.VOPC|irc.VOP3, ctx: _Ctx,
  ...
  if is_cmpx:
    stores = list(ctx.wexec(new_result))
...
class WaveState:
  ...
  def __init__(self, n_lanes: int = WAVE_SIZE):
    ...
    if WAVE_SIZE == 64:
      self._write_sgpr(EXEC_LO.offset, MASK32)  # EXEC_LO: all 32 low lanes active
      from tinygrad.renderer.amd.dsl import EXEC_HI
      self._write_sgpr(EXEC_HI.offset, (1 << (n_lanes - 32)) - 1 if n_lanes > 32 else 0)  # EXEC_HI: remaining lanes
    else:
