...
# an emulated machine immediate takes its width from the instruction encoding, so the const states it
def _c(val, dtype=dtypes.uint32): return UOp(Ops.CONST, dtype, arg=dtype.const(val))
...
def _split64(val: UOp) -> tuple[UOp, UOp]:
  ...
  return v64.cast(dtypes.uint32), (v64 >> _c(32, dtypes.uint64)).cast(dtypes.uint32)
...
def _apply_src_mods(val: UOp, mod_bit: int, abs_bits: int, neg_bits: int, bits: int = 32) -> UOp:
  ...
  if abs_bits & (1 << mod_bit): fv = (fv.bitcast(ut) & _c(mask, ut)).bitcast(ft)
...
def _lane_active(exec_mask: UOp, lane: UOp) -> UOp:
  if exec_mask.dtype == dtypes.uint64: return ((exec_mask >> lane.cast(dtypes.uint64)) & _c(1, dtypes.uint64)).ne(_c(0, dtypes.uint64))
...
def _set_lane_bit(old: UOp, lane: UOp, val: UOp, exec_mask: UOp) -> UOp:
  ...
  if old.dtype in (dtypes.uint64, dtypes.int64):
    dt = dtypes.uint64
    mask = _c(1, dt) << lane.cast(dt)
    new_bit = _to_u32(val).cast(dt) << lane.cast(dt)
    cleared = old.cast(dt) & (mask ^ _c(0xFFFFFFFFFFFFFFFF, dt))
...
def _write_64bit(val: UOp, wfn, reg_or_addr, is_mem: bool, *args) -> list[UOp]:
  ...
  return [wfn(reg_or_addr, lo, *args), wfn(reg_or_addr + (_c(incr, reg_or_addr.dtype) if isinstance(reg_or_addr, UOp) else incr), hi, *args)]
...
def _mem_store(mem: UOp, addr: UOp, val: UOp, active: UOp, addr_bits: int = 32, data_bits: int = 32) -> list[UOp]:
  ...
  next_idx = mem.index((word_addr + _c(1, adt)).valid(active & is_cross))
...
def _mem_store_bytes(mem: UOp, addr: UOp, val: UOp, active: UOp, data_bits: int = 32) -> list[UOp]:
  ...
  for i in range(data_bits // 8):
    byte_val = (val_u32 >> _c(i * 8, dtypes.uint32)) & _c(0xFF, dtypes.uint32)
    stores.append(mem.index((addr + _c(i, dtypes.uint64)).valid(active)).store(byte_val.cast(dtypes.uint8)))
  return stores
...
class _Ctx:
  ...
  def inst_word(self, dword_idx: int) -> UOp:
    ...
    pc = self.rpc()
    addr = pc if dword_idx == 0 else pc + _c(dword_idx * 4, dtypes.uint64)
    return self.vmem.index(addr >> _c(2, dtypes.uint64)).load()
  ...
  def inst_field(self, field) -> UOp:
    ...
    if lo // 32 == hi // 32:  # Same dword
      mask = (1 << (hi - lo + 1)) - 1
      shifted = word if lo_in_dword == 0 else word >> _c(lo_in_dword, dtypes.uint32)
      return shifted & _c(mask, dtypes.uint32)
    else:  # Spans two dwords
      lo_bits = 32 - lo_in_dword
      lo_mask = (1 << lo_bits) - 1
      hi_mask = (1 << (hi_in_dword + 1)) - 1
      lo_part = (word >> _c(lo_in_dword, dtypes.uint32)) & _c(lo_mask, dtypes.uint32)
      hi_part = self.inst_word(dword_idx + 1) & _c(hi_mask, dtypes.uint32)
      return lo_part | (hi_part << _c(lo_bits, dtypes.uint32))
  ...
  def rsrc_dyn(self, off: UOp, lane: UOp | None, bits: int = 32, literal: UOp | None = None, is_f64: bool = False, do_cast: bool = True) -> UOp:
    ...
    if bits == 64:
      ...
      if literal is not None:
        lit_val = literal.cast(dtypes.uint64) << _c(32, dtypes.uint64) if is_f64 else literal.cast(dtypes.uint64)
  ...
  def inc_pc(self) -> list[UOp]:
    ...
    new_pc = self.rpc() + _c(self.inst_size, dtypes.uint64)
  ...
  def compile_vop_pcode(self, op, srcs: dict[str, UOp | int], lane: UOp, vdst_reg: UOp, exec_mask: UOp,
    ...
    srcs.update({'EXEC': exec_mask, 'SCC': self.rsgpr_dyn(_c(SCC.offset)), 'laneId': lane, 'VDST': vdst_reg,
                  ...
                  'MAX_FLOAT_F32': _c(3.4028234663852886e38, dtypes.float32),
    ...
    for dest, val in assigns:
      ...
      if 'D0' in dest and '[laneId]' in dest:
        ...
        if (slice_match := re.match(r'D0\[(\d+)\s*:\s*(\d+)\]', dest)):
          ...
          if d0_hi_bit != 31 or d0_lo_bit != 0:
            ...
            val_bits = val.bitcast(dtypes.uint16).cast(dtypes.uint32) if val.dtype == dtypes.half else \
                       ...
                       val.cast(dtypes.uint32) & _c(slice_mask, dtypes.uint32)
        ...
        if int_saturate is not None: val = int_saturate
          ...
          clamped = val.maximum(_c(0.0, val.dtype)).minimum(_c(1.0, val.dtype))
          val = _FUNCS['isNAN'](val).where(_c(0.0, val.dtype), clamped)
        if val.dtype in (dtypes.uint64, dtypes.int64, dtypes.float64):
          ...
          hi_result = (old_val & _c(0xFFFF, dtypes.uint32)) | (result << _c(16, dtypes.uint32))
          # GFX9/CDNA zeroes upper 16 bits on lo-half write; RDNA preserves them
          lo_result = (result & _c(0xFFFF, dtypes.uint32)) if self.wave_size == 64 else \
                      (old_val & _c(0xFFFF0000, dtypes.uint32)) | (result & _c(0xFFFF, dtypes.uint32))
    ...
    if slice_stores:
      ...
      for lo_bit, width, val_bits in slice_stores:
        mask = _c(((1 << width) - 1) << lo_bit, dtypes.uint32)
        result = (result & (mask ^ _c(0xFFFFFFFF, dtypes.uint32))) | (val_bits << _c(lo_bit, dtypes.uint32))
...
def _compile_sopp(inst: ir3.SOPP | ir4.SOPP, ctx: _Ctx) -> UOp:
  ...
  if inst.op in (ir3.SOPPOp.S_ENDPGM, ir4.SOPPOp.S_ENDPGM, irc.SOPPOp.S_ENDPGM):
    return UOp.sink(ctx.wsgpr_dyn(_c(PC_LO_IDX), _c(0xFFFFFFFF, dtypes.uint32)),
                          ctx.wsgpr_dyn(_c(PC_HI_IDX), _c(0xFFFFFFFF, dtypes.uint32)))
  ...
  if inst.op in _get_pcode_dict(inst.op):
    ...
    srcs: dict[str, UOp|int] = {'PC': pc_bytes.cast(dtypes.int64), 'SIMM16': simm16, 'SCC': ctx.rsgpr_dyn(_c(SCC.offset)), 'VCC': vcc,
            'VCCZ': vcc.eq(_c(0, vcc.dtype)).cast(dtypes.uint32),
            'EXECZ': exec_val.eq(_c(0, exec_val.dtype)).cast(dtypes.uint32)}
...
def _compile_smem(inst: ir3.SMEM | ir4.SMEM, ctx: _Ctx) -> UOp:
  ...
  dword_base = addr >> _c(2, dtypes.uint64)
  vals = [ctx.vmem.index(dword_base + _c(i, dtypes.uint64)) for i in range(ndwords)]
  if abs(nval) < 1:
    nbits = int(abs(nval) * 32)
    byte_off = (addr & _c(3, dtypes.uint64)).cast(dtypes.uint32) * _c(8, dtypes.uint32)
    extracted = (vals[0] >> byte_off) & _c((1 << nbits) - 1, dtypes.uint32)
...
def _load_dpp16_src0(ctx: _Ctx, inst, lane: UOp, fallback: UOp) -> UOp:
  ...
  invalid = _c(0, fallback.dtype) if getattr(inst, 'bc', 0) else fallback
...
def _compile_vopc(inst: ir3.VOPC|ir3.VOPC_DPP16|ir3.VOP3|ir4.VOPC|ir4.VOPC_DPP16|ir4.VOP3|irc.VOPC|irc.VOP3, ctx: _Ctx,
  ...
  def get_cmp_bit(lane) -> UOp:
    ...
    for dest, val in parse_pcode(pcode, {'S0': s0, 'S1': s1, 'laneId': lc, 'D0': _c(0, dtypes.uint64)})[1]:
      if '[laneId]' in dest and ('D0' in dest or 'EXEC' in dest): return val.cast(dtypes.uint32)
    return _c(0)
...
def _compile_bitop3(inst, ctx: _Ctx, exec_mask: UOp, bits: dict, op_name: str) -> UOp:
  ...
  def bnot(v): return v ^ _c(mask, dt)
  result = _c(0, dt)
  for i in range(8):
...
def _compile_vop3(inst: ir3.VOP3 | ir4.VOP3 | irc.VOP3, ctx: _Ctx) -> UOp:
  ...
  srcs = {'S0': src0, 'S1': src1, 'S2': src2, 'OPSEL': _c(opsel, dtypes.uint32)}
...
def _compile_mfma(inst: irc.VOP3P, ctx: _Ctx) -> UOp:
  ...
  def cvt_elem(raw: UOp, sub_idx: int) -> UOp:
    if is_i8:
      ...
      byte_val = (raw >> _c(sub_idx * 8, dtypes.uint32)) & _c(0xFF, dtypes.uint32)
      return (byte_val.cast(dtypes.int32) ^ _c(0x80, dtypes.int32)) - _c(0x80, dtypes.int32)
    elif is_f32_src:
      return raw  # already uint32 (f32 bit pattern)
    elif is_fp8:
      return ((raw >> _c(sub_idx * 8, dtypes.uint32)) & _c(0xFF, dtypes.uint32)).cast(dtypes.uint32)
    elif is_bf16:
      # bf16→f32 bits: just shift left by 16 (bf16 is upper 16 bits of f32)
      return ((raw >> _c(sub_idx * 16, dtypes.uint32)) & _c(0xFFFF, dtypes.uint32)) << _c(16, dtypes.uint32)
    else:
      ...
      h = (raw >> _c(sub_idx * 16, dtypes.uint32)) & _c(0xFFFF, dtypes.uint32)
      sign = (h >> _c(15, dtypes.uint32)) & _c(1, dtypes.uint32)
      exp = (h >> _c(10, dtypes.uint32)) & _c(0x1F, dtypes.uint32)
      mant = h & _c(0x3FF, dtypes.uint32)
      ...
      f32_bits = (sign << _c(31, dtypes.uint32)) | \
                 ((exp + _c(112, dtypes.uint32)) << _c(23, dtypes.uint32)) | \
                 (mant << _c(13, dtypes.uint32))
      is_zero = exp.eq(_c(0, dtypes.uint32))
      # Return uint32 (f32 bit pattern) — stored directly to uint32 temp array, bitcast to float on read
      return is_zero.where(_c(0, dtypes.uint32), f32_bits)
  ...
  lane_in_grp = read_lane % _c(grp_size, dtypes.int)
  grp_idx = read_lane // _c(grp_size, dtypes.int)

  if M == 32:
    # 32x32: lane_in_grp%16 = sub-row/col (0-15), lane_in_grp//16 = block (0=rows 0-15, 1=rows 16-31)
    sub_mn = lane_in_grp % _c(16, dtypes.int)
    block_mn = lane_in_grp // _c(16, dtypes.int)
    mn_idx = block_mn * _c(16, dtypes.int) + sub_mn  # actual M/N index (0-31)
  else:
  ...
  for kl in range(k_per_grp):
    ...
    if M == 4:
      a_idx = grp_idx * _c(M * K, dtypes.int) + mn_idx * _c(K, dtypes.int) + _c(kl, dtypes.int)
    else:
      a_idx = mn_idx * _c(K, dtypes.int) + grp_idx * _c(k_per_grp, dtypes.int) + _c(kl, dtypes.int)
    ...
    if M == 4:
      b_idx = b_off + grp_idx * _c(N * K, dtypes.int) + mn_idx * _c(K, dtypes.int) + _c(kl, dtypes.int)
    else:
      b_idx = b_off + mn_idx * _c(K, dtypes.int) + grp_idx * _c(k_per_grp, dtypes.int) + _c(kl, dtypes.int)
  ...
  if M == 32 and N == 32:
    ...
    c_lane_in_32 = compute_lane % _c(32, dtypes.int)
    c_sub = c_lane_in_32 % _c(16, dtypes.int)
    c_block = c_lane_in_32 // _c(16, dtypes.int)
    n_idx = c_block * _c(16, dtypes.int) + c_sub
    c_half = compute_lane // _c(32, dtypes.int)  # 0 or 1
    ...
    for out_reg in range(16):
      ...
      m_base = c_half * _c(16, dtypes.int) + _c((out_reg // 4) * 4 + (out_reg % 4), dtypes.int)
      ...
      for k in range(K):
        a_val = tmp2.index(m_base * _c(K, dtypes.int) + _c(k, dtypes.int)).bitcast(acc_dt)
        b_val = tmp2.index(b_off + n_idx * _c(K, dtypes.int) + _c(k, dtypes.int)).bitcast(acc_dt)
        acc = acc + a_val * b_val
    ...
    n_idx = compute_lane % _c(grp_sub, dtypes.int)
    c_grp = compute_lane // _c(grp_sub, dtypes.int)
    ...
    for out_reg in range(out_per_lane):
      ...
      if M == 4:
        ...
        m_base = c_grp * _c(M * K, dtypes.int) + _c(out_reg * K, dtypes.int)
        for k in range(K):
          a_val = tmp2.index(m_base + _c(k, dtypes.int)).bitcast(acc_dt)
          b_val = tmp2.index(b_off + c_grp * _c(N*K, dtypes.int) + n_idx * _c(K, dtypes.int)+_c(k, dtypes.int)).bitcast(acc_dt)
          acc = acc + a_val * b_val
      else:
        # 16x16: K is split across groups. Shared MxK/NxK arrays.
        m_base = c_grp * _c(out_per_lane, dtypes.int) + _c(out_reg, dtypes.int)
        for k in range(K):
          a_val = tmp2.index(m_base * _c(K, dtypes.int) + _c(k, dtypes.int)).bitcast(acc_dt)
          b_val = tmp2.index(b_off + n_idx * _c(K, dtypes.int) + _c(k, dtypes.int)).bitcast(acc_dt)
          acc = acc + a_val * b_val
...
def _compile_wmma(inst: ir3.VOP3P | ir4.VOP3P | irc.VOP3P, ctx: _Ctx) -> UOp:
  ...
  def read_f16_val(src, lane, vgpr, half):
    v = ctx.rvgpr_dyn(src + _c(vgpr), _c(lane, dtypes.int))
    return cvt((v >> _c(16, dtypes.uint32)) if half else (v & _c(0xFFFF, dtypes.uint32)))
  ...
  if is_f16_output:
    ...
    def f32_to_bf16_bits(v: UOp) -> UOp: return (v.bitcast(dtypes.uint32) >> _c(16, dtypes.uint32)) & _c(0xFFFF, dtypes.uint32)
    ...
    if is_rdna4:  # pack 2 f16 per VGPR: adjacent m values share (lane, vgpr) since vgpr=m&7, half=m&1
      stores = [ctx.wvgpr_dyn(vdst_reg + _c(d_map(m, n)[1] // 2), _c(d_map(m, n)[0], dtypes.int),
                out_cvt(mat_d[m*16+n]) | (out_cvt(mat_d[(m+1)*16+n]) << _c(16, dtypes.uint32)), exec_mask)
                for n in range(16) for m in range(0, 16, 2)]
    else:  # (rdna3) 1 f16 per VGPR (lo half only)
      stores = [ctx.wvgpr_dyn(vdst_reg + _c(d_map(m, n)[1]), _c(d_map(m, n)[0], dtypes.int), out_cvt(mat_d[m*16+n]), exec_mask)
                for m in range(16) for n in range(16)]
  else: # f32
    mat_c = [ctx.rvgpr_dyn(src2_r + _c(d_map(m, n)[1]), _c(d_map(m, n)[0], dtypes.int)).bitcast(dtypes.float32)
             for m in range(16) for n in range(16)]
    mat_d = [sum(mat_a[r*16+k] * mat_b[c*16+k] for k in range(16)) + mat_c[r*16+c] for r in range(16) for c in range(16)]
    stores = [ctx.wvgpr_dyn(vdst_reg + _c(d_map(m, n)[1]), _c(d_map(m, n)[0], dtypes.int), mat_d[m*16+n].bitcast(dtypes.uint32), exec_mask)
              for m in range(16) for n in range(16)]
  return UOp.sink(*stores, *ctx.inc_pc())
...
def _compile_vop3p(inst: ir3.VOP3P | ir4.VOP3P | irc.VOP3P, ctx: _Ctx) -> UOp:
  ...
  if is_pk_f32:
    ...
    def build_pk_f32(src_lo: UOp, src_off: UOp, opsel_lo: int, opsel_hi_bit: int, neg_lo: int, neg_hi_bit: int) -> UOp:
      ...
      if neg_lo: lo = lo ^ _c(0x80000000, dtypes.uint32)
      if neg_hi_bit: hi = hi ^ _c(0x80000000, dtypes.uint32)
      return _u64(lo, hi)
    ...
    def apply_abs(v, bit, opsel_hi_bit, opsel_bit):
      ...
      if not (combined_opsel_hi & opsel_hi_bit): return v & _c(0x7FFFFFFF, dtypes.uint32)  # f32 abs
      if opsel & opsel_bit: return v & _c(0x7FFF0000, dtypes.uint32)  # f16 hi abs (preserve lo)
      return v & _c(0xFFFF7FFF, dtypes.uint32)  # f16 lo abs (preserve hi)
    def apply_neg_mix(v, bit, opsel_hi_bit, opsel_bit):
      if not (neg & bit): return v
      if not (combined_opsel_hi & opsel_hi_bit): return v ^ _c(0x80000000, dtypes.uint32)  # f32 neg
      if opsel & opsel_bit: return v ^ _c(0x80000000, dtypes.uint32)  # f16 hi neg
      return v ^ _c(0x00008000, dtypes.uint32)  # f16 lo neg
    ...
    srcs = {'S@0': s0_mod, 'S@1': s1_mod, 'S@2': s2_mod,
            'OPSEL_HI': _c(combined_opsel_hi, dtypes.uint32), 'OPSEL': _c(opsel, dtypes.uint32)}
  else:
    def get_half_bits(val: UOp, use_hi: bool, apply_neg: bool = False) -> UOp:
      bits = ((val >> _c(16, dtypes.uint32)) if use_hi else val) & _c(0xFFFF, dtypes.uint32)
    ...
    def build_remapped_src(src: UOp, opsel_lo_bit: int, opsel_hi_bit: int, neg_lo_bit: int, neg_hi_bit: int) -> UOp:
      ...
      return lo | (hi << _c(16, dtypes.uint32))
    ...
    if is_dot_iu: srcs['NEG'] = _c(neg, dtypes.uint32)
  return ctx.compile_vop_pcode(inst.op, srcs, lane, vdst_reg, exec_mask)
...
def _compile_mem_op(inst: ir3.DS|ir3.FLAT|ir3.GLOBAL|ir3.SCRATCH|ir4.DS|ir4.VFLAT|ir4.VGLOBAL|ir4.VSCRATCH
  ...
  addr_shift = _c(2, dtypes.uint32 if is_lds else dtypes.uint64)
  ...
  def make_addr(lane: UOp) -> UOp:
    ...
    if is_scratch:
      ...
      addr_offset = vaddr if sve == 1 else _c(0, dtypes.uint64)
      # Add saddr value only if use_saddr is true (saddr < 124)
      saddr_contrib = use_saddr.where(ctx.rsgpr_dyn(saddr_reg).cast(dtypes.uint64), _c(0, dtypes.uint64)) \
        if saddr_reg is not None else _c(0, dtypes.uint64)
    ...
    saddr_base = _u64(ctx.rsgpr_dyn(saddr_reg), ctx.rsgpr_dyn(saddr_reg + _c(1))) if saddr_reg is not None else _c(0, dtypes.uint64)
  ...
  def make_srcs(lane: UOp) -> dict:
    ...
    if is_lds:
      if data_bits_mem == 128:
        ...
        data = {'DATA': ctx.rvgpr_dyn(vdata_reg, lane), 'DATA2': ctx.rvgpr_dyn(data1_reg, lane) if has_data1 else _c(0, dtypes.uint32)}
      else:
        data = {'DATA': _u64(ctx.rvgpr_dyn(vdata_reg, lane), ctx.rvgpr_dyn(vdata_reg + _c(1), lane)),
                'DATA2': _u64(ctx.rvgpr_dyn(data1_reg, lane), ctx.rvgpr_dyn(data1_reg + _c(1), lane)) if has_data1 else _c(0, dtypes.uint64)}
    ...
    use_saddr = (saddr_reg < _c(124)) if saddr_reg is not None else UOp.const(False)
    saddr_raw = _u64(ctx.rsgpr_dyn(saddr_reg), ctx.rsgpr_dyn(saddr_reg + _c(1))) if saddr_reg is not None else _c(0, dtypes.uint64)
    saddr_base = use_saddr.where(saddr_raw, _c(0, dtypes.uint64))
    ...
    vdata = _rvdata(vdata_reg, lane).cast(dtypes.uint64) if 'STORE' in op_name \
      else _rvdata(vdst_reg, lane) if 'D16' in op_name else _c(0, dtypes.uint32)
    if 'STORE' in op_name and data_bits_mem >= 64:
      vdata = vdata | (_rvdata(vdata_reg + _c(1), lane).cast(dtypes.uint64) << _c(32, dtypes.uint64))
    ...
    for i in range(data_bits_mem // 32):
      srcs[f'VDATA{i}'] = _rvdata(vdata_reg + _c(i), lane) if 'STORE' in op_name else _c(0, dtypes.uint32)
    return srcs
...
def _compile_mubuf(inst: irc.MUBUF, ctx: _Ctx) -> UOp:
  ...
  base = _u64(ctx.rsgpr_dyn(srsrc), ctx.rsgpr_dyn(srsrc + _c(1))) & _c(0xFFFFFFFFFFFF, dtypes.uint64)
  ...
  addr = in_bounds.where(addr, _c(0, dtypes.uint64))  # safe address when OOB
  mem = ctx.vmem
  ...
  if is_lds and not is_store:
    ...
    for i in range(n_dwords):
      word_addr = (addr + _c(i * 4, dtypes.uint64)) >> _c(2, dtypes.uint64)
