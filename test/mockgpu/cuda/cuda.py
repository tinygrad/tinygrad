...
def cuLaunchKernel(
    ...
    cargs = [
        ctypes.cast(getattr(extra, field[0]), ctypes.c_void_p)
        for field in extra._fields_
    ]
    gpuocelot_lib.ptx_run(
        ctypes.cast(f.value, ctypes.c_char_p),
        len(cargs),
...
def cuMemFreeHost(p: ctypes.c_void_p) -> int:
    return cuMemFree_v2(p)
...
def cuStreamWaitEvent(stream: Any, event, flags: int) -> int:
    return orig_cuda.CUDA_SUCCESS
...
def cuGetErrorString(error: int, pStr) -> int:
    error_str = orig_cuda.cudaError_enum__enumvalues.get(
