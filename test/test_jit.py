...
def _simple_test(add, extract=lambda x: x, N=10):
    for _ in range(5):
        ...
        np.testing.assert_allclose(
            extract(c).numpy(), a.numpy() + b.numpy(), atol=1e-4, rtol=1e-5
        )
...
class TestJit(unittest.TestCase):
    ...
    @unittest.skipUnless(
        Device.DEFAULT in ["LLVM", "CLANG"], f"no support on {Device.DEFAULT}"
    )
    ...
    def test_jit_copyin(self):
        ...
        for _ in range(5):
            ...
            c = f(b)
            np.testing.assert_allclose(
                c.numpy(), b.numpy() + [1, 2, 3], atol=1e-4, rtol=1e-5
            )
    ...
    @unittest.skipIf(
        CI and Device.DEFAULT in {"GPU", "CUDA", "METAL", "NV", "AMD"}, "no GPU CI"
    )
    ...
    @unittest.skipIf(
        CI and Device.DEFAULT in {"GPU", "CUDA", "METAL"},
        "no GPU/CUDA/METAL in CI, fine to run on AMD/NV",
    )
    def test_jitted_view(self):
        ...
        for _ in range(5):
            ...
            xc = jf(a)
            np.testing.assert_allclose(
                (a.numpy().sum(axis=(1,)) + 5).view(np.int32),
                xc.numpy(),
                atol=1e-4,
                rtol=5e-5,
            )
...
class TestMultioutputJit(unittest.TestCase):
    def _test(self, f):
        for _ in range(5):
            ...
            np.testing.assert_allclose(
                out0.numpy(), a.numpy() + b.numpy(), atol=1e-4, rtol=1e-5
            )
            np.testing.assert_allclose(
                out1.numpy(), a.numpy() - b.numpy(), atol=1e-4, rtol=1e-5
            )
            np.testing.assert_allclose(
                out2.numpy(), a.numpy() * b.numpy(), atol=1e-4, rtol=1e-5
            )
...
class TestJitInsideJit(unittest.TestCase):
    def test_jit_jit_error(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "having TinyJit inside another TinyJit is not supported"
        ):
            g(Tensor([1])).realize()
...
class TestCopyInsideJit(unittest.TestCase):
    def test_copy_inside_jit(self):
        ...
        for _ in range(5):
            ...
            np.testing.assert_allclose(
                out.flatten().tolist(),
                [x + y for x, y in zip(a.flatten().tolist(), b.flatten().tolist())],
            )
...
class TestJitPrune(unittest.TestCase):
    def test_simple_prune(self):
        ...
        for _ in range(3):
            ...
            out = w2_noprune(a)
            np.testing.assert_allclose(
                out.tolist(), [x * 2 + y for x, y in zip(weights.tolist(), a.tolist())]
            )
