...
def equal_distribution(
    tiny_func, torch_func=None, numpy_func=None, shape=(40, 43), alpha=0.04
):
    ...
    return (
        numpy_func is None or (kstest(x1, y) >= alpha and kstest(x2, y) >= alpha)
    ) and (torch_func is None or (kstest(x1, z) >= alpha and kstest(x2, z) >= alpha))
...
class TestRandomness(unittest.TestCase):
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.float16) and is_dtype_supported(dtypes.ulong),
        "need float16 and ulong support",
    )
    def test_rand_float16(self):
        ...
        equal_distribution(
            lambda *x: Tensor.rand(*x, dtype=dtypes.float16),
            torch.rand,
            lambda x: np.random.rand(*x),
            shape=(2, N, N),
        )

    @unittest.skipIf(
        CI and Device.DEFAULT in {"NV", "CUDA"},
        "gpuocelot doesn't support certain ops needed for threefry",
    )
    ...
    """
        ...
        r = Tensor._threefry_random_bits(
            Tensor([0, 1337], dtype="uint32"), counts0, counts1
        ).numpy()
        ...
        for ei in lower_schedule(Tensor.rand(20).schedule()):
            if isinstance(ei.prg, CompiledRunner):
                for u in ei.prg.p.uops:
                    self.assertNotIn(
                        u.dtype,
                        {dtypes.long, dtypes.ulong},
                        msg=f"long found in {ei.prg.p.name}",
                    )
    ...
    @unittest.skipIf(
        CI and Device.DEFAULT in ("GPU", "CUDA", "METAL", "NV"), "no GPU CI"
    )
    ...
    def test_rand_bfloat16(self):
        ...
        assert nx[nx == 0].size > 0
        equal_distribution(
            lambda *x: Tensor.rand(*x, dtype=dtypes.bfloat16).float(),
            torch.rand,
            lambda x: np.random.rand(*x),
            shape=(2, N, N),
        )
    ...
    @unittest.skipIf(
        Device.DEFAULT in ["HSA", "AMD"], "bfloat16 local buffer broken in HSA"
    )
    ...
    def test_randint(self):
        ...
        self.assertTrue(
            equal_distribution(
                partial(Tensor.randint, low=-2, high=5),
                numpy_func=lambda x: np.random.randint(low=-2, high=5, size=x),
            )
        )
        self.assertTrue(
            equal_distribution(
                partial(Tensor.randint, low=-2, high=5, dtype="int32"),
                numpy_func=lambda x: np.random.randint(low=-2, high=5, size=x),
            )
        )
    ...
    def test_multinomial(self):
        ...
        if not (CI and Device.DEFAULT == "AMD"):
            ...
            self.assertTrue(
                equal_distribution(
                    lambda *_: Tensor(tiny_samples),
                    lambda _: torch.tensor(torch_samples),
                )
            )
