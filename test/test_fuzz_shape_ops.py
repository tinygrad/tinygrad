...
settings.register_profile(
    __file__,
    settings.default,
    max_examples=100 if CI else 250,
    deadline=None,
...
def st_shape(draw) -> tuple[int, ...]:
    s = draw(stn.array_shapes(min_dims=0, max_dims=6, min_side=0, max_side=128))
...
def apply(tor, ten, tor_fn, ten_fn=None):
    ...
    try:
        ...
        tor, ok = None, not ok  # noqa: E722
    try:
        ten = ten_fn(ten) if ten_fn is not None else tor_fn(ten)
    except:
        ten, ok = None, not ok  # noqa: E722
    return tor, ten, ok
...
class TestShapeOps(unittest.TestCase):
    ...
    def test_split(self, s: tuple[int, ...], dim: int, sizes: int | list[int]):
        ...
        assert ok
        if tor is None and ten is None:
            return

        assert len(tor) == len(ten)
        assert all(
            [np.array_equal(tor.numpy(), ten.numpy()) for (tor, ten) in zip(tor, ten)]
        )
    ...
    def test_chunk(self, s: tuple[int, ...], dim: int, num: int):
        ...
        assume(
            (
                0 <= (actual_dim := len(s) - dim if dim < 0 else dim) < len(s)
                and s[actual_dim] > 0
            )
            or (num < 16)
        )
