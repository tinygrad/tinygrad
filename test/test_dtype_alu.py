...
integer_binary_operations = binary_operations + [
    (Tensor.xor, np.bitwise_xor),
    (Tensor.bitwise_and, np.bitwise_and),
...
if (
    getenv("MOCKGPU") and Device.DEFAULT in {"NV", "CUDA"}
...
def universal_test_cast(a, in_dtype, dtype):
    ...
    numpy_value = np.array([a], dtype=_to_np_dtype(in_dtype)).astype(
        _to_np_dtype(dtype)
    )
...
def universal_test_midcast(a, b, c, op1, op2, d1: DType, d2: DType):
    ...
    an, bn, cn = (
        np.array([a]).astype(_to_np_dtype(d1)),
        np.array([b]).astype(_to_np_dtype(d1)),
        np.array([c]).astype(_to_np_dtype(d2)),
    )
    tensor_value = op2[0](op1[0](at, bt).cast(d2), ct).numpy()
    numpy_value = op2[1](op1[1](an, bn).astype(_to_np_dtype(d2)), cn)
    np.testing.assert_allclose(
        tensor_value, numpy_value, rtol=1e-6 if getenv("PTX") else 1e-7
    )
...
class TestDTypeALU(unittest.TestCase):
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.bfloat16, Device.DEFAULT),
        f"no bfloat16 on {Device.DEFAULT}",
    )
    @given(ht.bfloat16, ht.bfloat16, strat.sampled_from(binary_operations))
    def test_bfloat16(self, a, b, op):
        universal_test(a, b, dtypes.bfloat16, op)
    ...
    def test_bfloat16_unary(self, a, op):
        universal_test_unary(a, dtypes.bfloat16, op)
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.uint16, Device.DEFAULT),
        f"no uint16 on {Device.DEFAULT}",
    )
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.uint32, Device.DEFAULT),
        f"no uint32 on {Device.DEFAULT}",
    )
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.uint64, Device.DEFAULT),
        f"no uint64 on {Device.DEFAULT}",
    )
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.int64, Device.DEFAULT),
        f"no int64 on {Device.DEFAULT}",
    )
    ...
    @given(
        ...
        ht.int32,
        ht.float32,
        strat.sampled_from(integer_binary_operations),
        strat.sampled_from(binary_operations),
    )
    ...
    @given(
        (
            strat.floats(width=32, min_value=0, max_value=10.0)
            if skip_overflow
            else ht.float32
        ),
        (
    ...
    @unittest.skipIf(
        Device.DEFAULT == "PYTHON", "TODO: fix cast inf to int32 in PYTHON"
    )
    ...
    @given(
        strat.data(),
        strat.sampled_from(dtypes_float),
        strat.sampled_from((dtypes.uint8, dtypes.uint16)),
    )
    def test_float_cast_to_unsigned(self, a, float_dtype, unsigned_dtype):
        if not is_dtype_supported(float_dtype, Device.DEFAULT):
            float_dtype = dtypes.float32
        float_strat = {
            dtypes.float16: ht.float16,
            dtypes.float32: ht.float32,
            dtypes.float64: ht.float64,
        }[float_dtype]
    ...
    def test_float_cast_to_unsigned_overflow(self, a, float_dtype, unsigned_dtype):
        ...
        overflow_strat = float_strat.filter(
            lambda x: x > dtypes.max(unsigned_dtype) and x <= dtypes.max(dtypes.int32)
        )
    ...
    def test_float_cast_to_unsigned_underflow(self, a, float_dtype, unsigned_dtype):
        ...
        underflow_strat = float_strat.filter(
            lambda x: x < 0 and x >= dtypes.min(dtypes.int32)
        )
