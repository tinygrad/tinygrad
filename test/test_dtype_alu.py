...
def universal_test_unary(a, dtype, op):
  ...
  if op[0] == Tensor.cos and abs(a) > 100:
    return
  out: Tensor = op[0](ta)
...
class TestDTypeALU(unittest.TestCase):
  ...
  def test_bfloat16_unary(self, a, op):
    universal_test_unary(from_storage_scalar(a, dtypes.bfloat16), dtypes.bfloat16, op)
  ...
  def test_float_cast_to_unsigned(self, a, float_dtype, unsigned_dtype):
    if not is_dtype_supported(float_dtype):
