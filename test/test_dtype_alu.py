...
def universal_test_unary(a, dtype, op):
  ...
  if dtype in dtypes_float: np.testing.assert_allclose(tensor_value, numpy_value, atol=5 if (Device.DEFAULT == "METAL" or getenv("PTX")) and op[0] == Tensor.sin else 1e-3, rtol=2 if Device.DEFAULT == "METAL" and op[0] == Tensor.sin else 1e-4 if dtype == dtypes.float32 else 1e-2)  # exp and log and sin are approximations (in METAL, the default fast-math versions are less precise)
  else: np.testing.assert_equal(tensor_value, numpy_value)
