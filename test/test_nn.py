...
class TestNN(unittest.TestCase):
    def test_sparse_cat_cross_entropy(self):
        # create in tinygrad
        input_tensor = Tensor.randn(
            6, 5
        )  # not square to test that mean scaling uses the correct dimension
        ...
        for smoothing in [0.0, 0.1, 0.5, 1.0]:
            for ignore_index in [-1, 0, 2]:
                for reduction in ["none", "sum", "mean"]:
                    loss = input_tensor.sparse_categorical_crossentropy(
                        target,
                    ...
                    torch_loss = torch.nn.CrossEntropyLoss(
                        ...
                        label_smoothing=smoothing,
                        ignore_index=ignore_index,
                    )(torch_input, torch_target)
                    np.testing.assert_allclose(
                        loss.numpy(), torch_loss.detach().numpy(), atol=1e-5, rtol=1e-6
                    )
                    ...
                    torch_loss = torch.nn.CrossEntropyLoss(
                        ...
                        torch_input.unsqueeze(0).permute(0, 2, 1),
                        torch_target.unsqueeze(0),
                    )
                    np.testing.assert_allclose(
                        loss.numpy(), torch_loss.detach().numpy(), atol=1e-5, rtol=1e-6
                    )
                ...
                with torch.no_grad():
                    if threed:
                        tbn = torch.nn.BatchNorm3d(
                            sz, track_running_stats=track_running_stats
                        ).eval()
                    else:
    ...
    def test_batchnorm_axis(self):
        ...
        a = (
            x.batchnorm(weight, bias, mean, invstd, axis=(0, 2))
            .permute(1, 0, 2, 3, 4)
            .reshape(4, 6, 2, 2)
        )
        b = (
            ...
            .reshape(4, 6, 2, 2)
            .batchnorm(
                weight.flatten(), bias.flatten(), mean.flatten(), invstd.flatten()
            )
        )
        t_x = torch.tensor(x.permute(1, 0, 2, 3, 4).reshape(4, 6, 2, 2).numpy())
        t_weight, t_bias = torch.tensor(weight.flatten().numpy()), torch.tensor(
            bias.flatten().numpy()
        )
        t_mean, t_invstd = torch.tensor(mean.flatten().numpy()), torch.tensor(
            invstd.flatten().numpy()
        )
    ...
    def test_linear(self):
        ...
        _test_linear(
            Tensor.randn(BS, T, in_dim), in_dim, out_dim
        )  # test with more dims
    ...
    def _run_conv2d_same_padding_test(
        self, BS, C1, C2, H, W, K, S, padding="same", D=1
    ):
        # create in tinygrad
        ...
        with torch.no_grad():
            torch_layer = torch.nn.Conv2d(
                C1, C2, kernel_size=K, stride=S, padding=padding, dilation=D
            ).eval()
            torch_layer.weight[:] = torch.tensor(
                layer.weight.numpy(), dtype=torch.float32
            )
    ...
    def test_conv2d_same_padding_invalid_stride(self):
        ...
        self.assertRaises(
            ValueError, Conv2d, C1, C2, kernel_size=K, stride=S, padding=P
        )
    ...
    def test_groupnorm(self):
        ...
        for _ in range(10):
            ...
            np.testing.assert_allclose(
                z.numpy(), torch_z.detach().numpy(), atol=5e-6, rtol=5e-6
            )
            np.testing.assert_allclose(
                x.grad.numpy(), torch_x.grad.detach().numpy(), atol=5e-4, rtol=5e-4
            )
            np.testing.assert_allclose(
    ...
    def test_instancenorm_2d(self):
        ...
        for _ in range(10):
            ...
            np.testing.assert_allclose(
                x.grad.numpy(), torch_x.grad.detach().numpy(), atol=1e-3, rtol=1e-3
            )
            np.testing.assert_allclose(
                layer.weight.grad.numpy(),
                torch_layer.weight.grad.detach().numpy(),
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.long), f"no long on {Device.DEFAULT}"
    )
    ...
    def test_embedding_one_kernel(self):
        ...
        self.assertEqual(
            3,
            len([item for item in schedule if item.ast.op is Ops.SINK]),
    ...
    def test_load_state_dict_shape_mismatch(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            r"Shape mismatch in layer `weight`: Expected shape \(2, 2\), but found \(4, 4\) in state dict.",
        ):
            load_state_dict(layer, state_dict)
    ...
    def test_lstm_cell(self):
        ...
        with torch.no_grad():
            ...
            torch_out_h, torch_out_c = torch_layer(
                torch.tensor(inp.numpy()), (torch_out_h, torch_out_c)
            )
            np.testing.assert_allclose(out_h.numpy(), torch_out_h.numpy(), atol=1e-6)
            np.testing.assert_allclose(out_c.numpy(), torch_out_c.numpy(), atol=1e-6)
