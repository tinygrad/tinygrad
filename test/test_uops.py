...
def _uops_to_prg(uops, typesig):
  ...
  return CompiledASTRunner(None, "test", src,
                           ...
                           runtime_args=runtime_args, typesig=typesig).build(Device[Device.DEFAULT].compiler, Device[Device.DEFAULT].runtime)
...
def _test_single_value(vals, op, dtype):
  ...
  prg = _uops_to_prg(uops, [dtype]*(1+len(buf2)))
  prg.exec([buf]+buf2)
  return buf.toCPU()[0]
