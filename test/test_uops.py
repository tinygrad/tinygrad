...
from tinygrad.ops import Ops, UOp, UPat, UnaryOps, TernaryOps, KernelInfo, exec_alu, spec # noqa F401
...
class TestExecALU(TestUOps):
  ...
  def test_div(self):
    ...
    np.testing.assert_allclose(exec_alu(Ops.MUL, dtypes.float32, (7.0, exec_alu(UnaryOps.RECIP, dtypes.float32, (3.0,)))), 2+(1.0/3.0))
    np.testing.assert_allclose(exec_alu(Ops.MUL, dtypes.float32, (7.0, exec_alu(UnaryOps.RECIP, dtypes.float32, (-3.0,)))), -2-(1.0/3.0))
