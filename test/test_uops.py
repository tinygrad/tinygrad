...
def _test_single_value(vals, op, dts):
  ...
  loads = (uop(uops, UOps.LOAD, dtype, [buf_loads[i].index(uop(uops, UOps.CONST, dtypes.int32, (), 0))]) for i, dtype in enumerate(dts))
  alu = uop(uops, UOps.ALU, output_dtype, loads, op)
  out = uop(uops, UOps.STORE, dtypes.void, (buf_store.index(uop(uops, UOps.CONST, dtypes.int32, (), 0)), alu))
...
def _test_uops_result(output_dtype, uops, res):
  ...
  # res = output_fn(uops)
  out = uop(uops, UOps.STORE, dtypes.void, (buf_store.index(uop(uops, UOps.CONST, dtypes.int32, (), 0)), res))
  buf = Buffer(Device.DEFAULT, 1, output_dtype).allocate()
  prg = _uops_to_prg([out])
  prg.exec([buf])
  ret = np.empty(1, _to_np_dtype(output_dtype))
...
class TestLocalAccess(unittest.TestCase):
  ...
  def test_local_basic(self):
    ...
    st = uop(uops, UOps.STORE, dtypes.void, (smem.index(uop(uops, UOps.CONST, dtypes.int32, (), 0)), uop(uops, UOps.CONST, dtypes.float32, (), 42.0)))
    barr = uop(uops, UOps.BARRIER, dtypes.void, (st,))
    sres = uop(uops, UOps.LOAD, dtypes.float32, (smem.index(uop(uops, UOps.CONST, dtypes.int32, (), 0)), barr))
  ...
  def test_local_indirect(self):
    ...
    st1 = uop(uops, UOps.STORE, dtypes.void, (smem.index(uop(uops, UOps.CONST, dtypes.int32, (), 1)), uop(uops, UOps.CONST, dtypes.int32, (), 2)))
    st2 = uop(uops, UOps.STORE, dtypes.void, (smem.index(uop(uops, UOps.CONST, dtypes.int32, (), 2)), uop(uops, UOps.CONST, dtypes.int32, (), 42)))
    barr = uop(uops, UOps.BARRIER, dtypes.void, (st1,st2))
    ofs = uop(uops, UOps.LOAD, dtypes.int32, (smem.index(uop(uops, UOps.CONST, dtypes.int32, (), 1)), barr))
    sres = uop(uops, UOps.LOAD, dtypes.int32, (smem.index(ofs),))
...
class TestAssembly(unittest.TestCase):
  def test_bitshift_left(self):
    ...
    l1 = UOp(UOps.LOAD, dtypes.int, (g1.index(c1),))
