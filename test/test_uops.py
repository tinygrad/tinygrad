...
from tinygrad.device import CompiledAST, CompiledASTRunner, Compiled
...
def _uops_to_prg(uops):
  ...
  return CompiledASTRunner(CompiledAST(None, "test", src, lib, [1] if Device[Device.DEFAULT].linearizer_opts.has_local else None,
...
def _test_single_value(vals, op, dts):
  ...
  buf_store = uop(uops, UOps.DEFINE_GLOBAL, PtrDType(output_dtype), (), 'data0')
  buf_loads = [uop(uops, UOps.DEFINE_GLOBAL, PtrDType(dtype), (), f'data{i+1}') for i,dtype in enumerate(dts)]
