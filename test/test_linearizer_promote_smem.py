...
def helper_lds_allclose(r:Tensor, desired:np.ndarray, opts:list[Opt]|None=None, desired_bufs_sizes:list[tuple[int, int]]|None=None,
  ...
  for i,(buf, sz) in enumerate(desired_bufs_sizes):
    assert local_bufs[i].arg == f"smem{buf}", f"Expected buffer argument index lds{buf}, got {local_bufs[i].arg}"
...
class TestLDS(unittest.TestCase):
  ...
  def test_lds_basic(self):
    # output
    helper_lds_matmul(opts=[Opt(OptOps.PROMOTE_SMEM, 0, None)], desired_bufs_sizes=[(0,1)], apply_lds=False)
    # input
    helper_lds_matmul(opts=[Opt(OptOps.LOCAL, 0, 4), Opt(OptOps.PROMOTE_SMEM, 1, None)], desired_bufs_sizes=[(1,4)], apply_lds=False)
    helper_lds_matmul(opts=[Opt(OptOps.UNROLL, 0, 4),Opt(OptOps.PROMOTE_SMEM, 2, None)], desired_bufs_sizes=[(2,4)], apply_lds=False)
    # multi
    helper_lds_matmul(opts=[Opt(OptOps.PROMOTE_SMEM, 0, None), Opt(OptOps.PROMOTE_SMEM, 1, None)], desired_bufs_sizes=[(0,1),(1,1)], apply_lds=False)
