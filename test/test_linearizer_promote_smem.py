...
def helper_promote_smem_allclose(r:Tensor, desired:np.ndarray, opts:list[Opt]|None=None, desired_bufs_sizes:list[tuple[int, int]]|None=None,
                                 rtol:float=1e-5, atol:float=1e-5, apply_promote_smem=True):
  realized_ast, bufs = helper_realized_ast(r)

  if opts is None: opts = []
  if desired_bufs_sizes is None: desired_bufs_sizes = [(i, 1) for i in range(len(bufs))]
  if apply_promote_smem: opts += [Opt(OptOps.PROMOTE_SMEM, i, None) for i in range(len(bufs))]
...
def helper_promote_smem_matmul(opts:list[Opt], desired_bufs_sizes, N=32, M=64, K=16, dtype_in=dtypes.float, acc_dtype=dtypes.float,
...
class TestPromoteSMEM(unittest.TestCase):
  ...
  def test_promote_smem_args(self):
    ...
    valid_opts = [Opt(OptOps.PROMOTE_SMEM, 0, None),
                  Opt(OptOps.PROMOTE_SMEM, 1, None),
                  Opt(OptOps.PROMOTE_SMEM, 2, None)]
    for opt in valid_opts:
      k = Kernel(realized_ast)
      k.apply_opt(opt)

    invalid_opts = [Opt(OptOps.PROMOTE_SMEM, -1, None),
                    Opt(OptOps.PROMOTE_SMEM, 3, None)]
  ...
  def test_invalid_promote_smem_pad(self):
    ...
    k = Kernel(realized_ast)
    with self.assertRaises(KernelOptError): k.apply_opt(Opt(OptOps.PROMOTE_SMEM, 1, None))
  ...
  def test_promote_smem_basic(self):
    ...
    # input
    helper_promote_smem_matmul(opts=[Opt(OptOps.LOCAL, 0, 4), Opt(OptOps.PROMOTE_SMEM, 1, None)], desired_bufs_sizes=[(1,4)], apply_promote_smem=False)
    helper_promote_smem_matmul(opts=[Opt(OptOps.UNROLL, 0, 4),Opt(OptOps.PROMOTE_SMEM, 2, None)], desired_bufs_sizes=[(2,4)], apply_promote_smem=False)
    # multi
    helper_promote_smem_matmul(opts=[Opt(OptOps.PROMOTE_SMEM, 0, None), Opt(OptOps.PROMOTE_SMEM, 1, None)], desired_bufs_sizes=[(0,1),(1,1)], apply_promote_smem=False)
  ...
  def test_promote_smem_tc(self):
    for i, tc in enumerate(Device[Device.DEFAULT].renderer.tensor_cores):
      ...
      helper_promote_smem_matmul(opts, desired_bufs_sizes=[(0,M*N),(1,M*K),(2,K*N)], N=N, M=M, K=K*2, dtype_in=tc.dtype_in, acc_dtype=tc.dtype_out)

      opts = [Opt(OptOps.TC, 0, (i, 0, 1)),
              Opt(OptOps.UNROLL, 0, 2)]
      helper_promote_smem_matmul(opts, desired_bufs_sizes=[(0,M*N),(1,M*K*2),(2,K*N*2)], N=sz, M=sz, K=sz, dtype_in=tc.dtype_in, acc_dtype=tc.dtype_out)

      opts = [Opt(OptOps.TC, 0, (i, 0, 1)),
              Opt(OptOps.UNROLL, 0, 2),
              Opt(OptOps.UPCAST, 1, 2)]
      helper_promote_smem_matmul(opts, desired_bufs_sizes=[(0,M*N*2),(1,M*K*2),(2,K*N*4)], N=sz, M=sz, K=sz, dtype_in=tc.dtype_in, acc_dtype=tc.dtype_out)
  ...
  def test_promote_smem_tc_local(self):
    for i, tc in enumerate(Device[Device.DEFAULT].renderer.tensor_cores):
      ...
      opts = [Opt(OptOps.TC, 0, (i, 0, 1)),
              Opt(OptOps.LOCAL, 0, 2),
              Opt(OptOps.UPCAST, 1, 2)]
      helper_promote_smem_matmul(opts, desired_bufs_sizes=[(0,M*N*4),(1,M*K*2),(2,K*N*2)], N=sz, M=sz, K=sz, dtype_in=tc.dtype_in, acc_dtype=tc.dtype_out)
