...
def helper_exec_op(device, outbuf, inbufs):
    if (device, len(inbufs)) not in cached_prgs:
        with Context(DEBUG=0):
            fst = [
                Tensor.randn(BUF_SIZE, dtype=dtypes.int).realize()
                for i in range(len(inbufs))
            ]
            s = fst[0]
    ...
    if fill:
        with Context(DEBUG=0):
            data = np.random.randint(
                -10000, 10000, size=rawbuf.size, dtype=_to_np_dtype(rawbuf.dtype)
            )
...
def helper_test_graphs(graph_impl, graphs, runs=RUN_CNT):
        ...
        for ji in graph:
            out_buffers.update(
                [
                    ji.bufs[i]
                    for i in (
                        ji.prg.p.outs if isinstance(ji.prg, CompiledRunner) else [0]
                    )
                ]
            )
            bufs += ji.bufs
    ...
    ground_truth_np = [
        np.frombuffer(x, _to_np_dtype(bufs[i].dtype))
        for i, x in enumerate(ground_thruth_bufs)
    ]
    ...
    for _ in range(runs):
        ...
        test_bufs_np = [
            np.frombuffer(x, _to_np_dtype(bufs[i].dtype))
            for i, x in enumerate(test_bufs)
        ]
...
class TestGraph(unittest.TestCase):
    def test_order_2_writes_to_same_buf(self):
        ...
        graphs = [
            [
                helper_exec_op(d0, b0[0], [b0[1], b0[2]]),
                helper_exec_op(d0, b0[0], [b0[3], b0[4]]),
            ]
        ]
    ...
    def test_order_read_write_same_buf(self):
        ...
        graphs = [
            [
                helper_exec_op(d0, b0[0], [b0[1], b0[2]]),
                helper_exec_op(d0, b0[1], [b0[3], b0[4]]),
            ]
        ]
    ...
    def test_order_write_read_same_buf(self):
        ...
        graphs = [
            [
                helper_exec_op(d0, b0[0], [b0[1], b0[2]]),
                helper_exec_op(d0, b0[1], [b0[0], b0[4]]),
            ]
        ]
    ...
    @unittest.skipUnless(
        Device.DEFAULT in {"CUDA", "NV", "AMD"}, "mutidevice graph required"
    )
    ...
    def test_read_write_several_graphs(self):
        ...
        graphs = [
            [
                helper_exec_op(d0, b0[3], [b0[1], b0[2]]),
                helper_exec_op(d0, b0[4], [b0[1], b0[2]]),
                helper_exec_op(d0, b0[5], [b0[1], b0[2]]),
            ],
