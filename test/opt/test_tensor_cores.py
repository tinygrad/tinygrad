...
def helper_tc_ensure_uops_and_opts_count(N: int, M:int, K:int, dtype_in:DType, dtype_out:DType, axis:int=0, tc_select:int=-1, tc_opt:int=0,
  ...
  if ensure_triggered:
    program = get_program(realized_ast, Device[Device.DEFAULT].renderer, device=Device.DEFAULT, opts=opts_to_apply)
...
def helper_tc_allclose(N:int, M:int, K:int, dtype_in:DType, dtype_out:DType, axis:int=0, tc_select:int=-1, tc_opt:int=0, use_tensor_cores:int=1):
  ...
  prg = CompiledRunner(replace(get_program(realized_ast, Device[Device.DEFAULT].renderer, device=Device.DEFAULT, opts=opts), device=Device.DEFAULT))
