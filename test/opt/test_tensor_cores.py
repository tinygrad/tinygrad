...
def helper_tc_ensure_uops_and_opts_count(N: int, M:int, K:int, dtype_in:DType, dtype_out:DType, axis:int=0, tc_select:int=-1, tc_opt:int=0,
  ...
  if ensure_triggered:
    program = get_program(realized_ast, Device.DEFAULT, opts=opts_to_apply)
...
def helper_tc_allclose(N:int, M:int, K:int, dtype_in:DType, dtype_out:DType, axis:int=0, tc_select:int=-1, tc_opt:int=0, use_tensor_cores:int=1):
  ...
  prg = CompiledRunner(replace(get_program(realized_ast, Device.DEFAULT, opts=opts), device=Device.DEFAULT))
...
class TestTensorCores(unittest.TestCase):
  ...
  def test_tensor_cores_codegen(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      ...
      prg = get_program(r.schedule()[-1].ast, Device.DEFAULT, opts=[Opt(op=OptOps.TC, axis=0, arg=(-1, 2, 1))])
  ...
  @Context(ALLOW_TF32=1)
  @unittest.skipIf((Device.DEFAULT == "AMD") or (Device.DEFAULT == "PYTHON" and Device.default.renderer.tc_dev == "AMD"), "broken for AMD")
  @unittest.skipUnless(Device[Device.DEFAULT].renderer.tensor_cores, "test requires tensor cores")
  ...
  @unittest.skipUnless((Device.DEFAULT == "AMD") or (Device.DEFAULT == "PYTHON" and Device.default.renderer.tc_dev == "AMD"), "test for AMD's tc")
  @unittest.skipUnless(Device[Device.DEFAULT].renderer.tensor_cores, "test requires tensor cores")
  ...
  def test_tensor_cores_multi_reduce(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      ...
      for axis in range(9):
        ...
        program = get_program(realized_ast, Device.DEFAULT, opts=[Opt(OptOps.TC, axis, (-1, 2, 1))])
  ...
  def test_tensor_cores_unroll_phi(self):
    ...
    for u in get_program(ast, Device.DEFAULT, opts=opts).uops:
      if u.op is Ops.WMMA:
