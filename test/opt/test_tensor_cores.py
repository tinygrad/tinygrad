  ...
  def test_tensor_cores_multi_reduce(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      ...
      for axis in range(9):
        ...
        real_bufs[0].copy_from(Buffer("PYTHON", real_bufs[0].size, real_bufs[0].dtype, opaque=memoryview(bytes(real_bufs[0].nbytes)))) # Zero to check that all values are filled
