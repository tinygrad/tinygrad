...
class TestRendererFailures(unittest.TestCase):
  ...
  def test_gated_store_with_alu(self):
    ...
    gated_alu_store = UOp(Ops.STORE, src=(a.index(lidx0.valid(gate_alu)), UOp.const(1, dtypes.int)))
  ...
  def test_gated_store_with_alu_2d(self):
    ...
    gated_alu_store = UOp(Ops.STORE, src=(a.index((lidx0+lidx1*4).valid(gate_alu_0&gate_alu_1)), UOp.const(1, dtypes.int)))
    sink = UOp(Ops.SINK, src=(gated_alu_store,), arg=KernelInfo())
...
class TestCStyleFailures(unittest.TestCase):
  def test_inline_const_alu(self):
    ...
    ret = _setup_and_test_alu(Ops.MAX, 1, UOp.const(dtypes.int.min+1, dtypes.int))
...
class TestWGSLFailures(unittest.TestCase):
  def test_multiply_infinity(self):
    ...
    ret = _setup_and_test_alu(Ops.MUL, 5.0, UOp.const(float("inf"), dtypes.float32))
