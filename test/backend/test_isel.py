...
def lane(y:UOp, i:int) -> UOp: return y.index(UOp.const(i), dtype=y.dtype.scalar())
...
class TestIselX86(unittest.TestCase):
  ...
  def test_complex_address(self):
    ...
    # displacement is the constant in "a" scaled to the buffer element size, a widthless value the encoding sizes
    self.assertTrue(n.src[2].op is Ops.CONST and n.src[2].arg == 4)
