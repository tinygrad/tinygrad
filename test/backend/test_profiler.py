...
class TestProfiler(unittest.TestCase):
  ...
  def test_profile_copyin(self):
    ...
    with helper_collect_profile(TestProfiler.d0) as profile:
      buf1.copy_from(Buffer("PYTHON", 2, dtypes.float, initial_value=struct.pack("ff", 0, 1)))
  ...
  def test_profile_multidev(self):
    ...
    with helper_collect_profile(TestProfiler.d0, d1) as profile:
      buf1.copy_from(Buffer("PYTHON", 2, dtypes.float, initial_value=struct.pack("ff", 0, 1)))
      buf2.copy_from(Buffer("PYTHON", 2, dtypes.float, initial_value=struct.pack("ff", 0, 1)))
