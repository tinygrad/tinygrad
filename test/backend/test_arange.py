...
from test.helpers import needs_second_gpu, check_schedule, assert_kernel_count, KernelCountException, stages_host_copies
...
class TestIndexing(unittest.TestCase):
  ...
  def test_llama_embedding(self, noopt=1, op_limit=65536):
    ...
    with Context(NOOPT=noopt):
      ...
      assert_kernel_count(3 if stages_host_copies() else 2)
    if getenv("CHECK", 1):
      import torch
      with torch.no_grad():
