...
class TestEncodingsX86(unittest.TestCase):
  ...
  def test_base_address(self):
    load = ins(X86Ops.MOV, dtypes.int32, (def_reg(dtypes.uint64, RDI), UOp(Ops.NOOP), imm(0), imm(4)), RDI)
  ...
  def test_rsp_base_address(self):
    load = ins(X86Ops.MOV, dtypes.int32, (def_reg(dtypes.uint64, RSP), UOp(Ops.NOOP), imm(0), imm(4)), RSP)
  ...
  def test_rbp_base_address(self):
    load = ins(X86Ops.MOV, dtypes.int32, (def_reg(dtypes.uint64, RBP), UOp(Ops.NOOP), imm(0), imm(4)), RBP)
  ...
  def test_base_index_address(self):
    load = ins(X86Ops.MOV, dtypes.int32, (def_reg(dtypes.uint64, RAX), def_reg(dtypes.int32, RDX), imm(0), imm(4)), RAX)
  ...
  def test_rsp_index_address(self):
    load = ins(X86Ops.MOV, dtypes.int32, (def_reg(dtypes.uint64, RAX), def_reg(dtypes.int32, RSP), imm(0), imm(4)), RAX)
  ...
  def test_r12_index_address(self):
    load = ins(X86Ops.MOV, dtypes.int32,
               (def_reg(dtypes.uint64, RAX), def_reg(dtypes.int32, GPR[12]), imm(0), imm(4)), RAX)
  ...
  def test_complex_address_8bit_disp(self):
    load = ins(X86Ops.MOV, dtypes.int32, (def_reg(dtypes.uint64, RDI), def_reg(dtypes.int32, RSI), imm(10), imm(4)), RDI)
  ...
  def test_complex_address_32bit_disp(self):
    load = ins(X86Ops.MOV, dtypes.int32,
               (def_reg(dtypes.uint64, RDI), def_reg(dtypes.int32, RSI), imm(10000), imm(4)), RDI)
  ...
  def test_write_mem(self):
    address = (def_reg(dtypes.uint64, RDI), def_reg(dtypes.int32, RSI), imm(10), imm(4))
    xmm0 = def_reg(dtypes.float32, XMM[0])
    extr = ins(X86Ops.VPEXTRD, dtypes.void, address + (xmm0, imm(0)))
  ...
  def test_disp_imm_same_value(self):
    address = (def_reg(dtypes.uint64, RDI), def_reg(dtypes.int8, RSI), imm(10), imm(1))
    mov = ins(X86Ops.MOVi, dtypes.void, address + (imm(10),))
    # mov byte ptr [rdi + rsi + 0xa], 0xa
    self.assertEqual(bytes.fromhex(self.encode(mov)), bytes.fromhex("40 C6 44 37 0A 0A"))

    address = (def_reg(dtypes.uint64, RDI), def_reg(dtypes.int32, RSI), imm(10), imm(4))
    imul = ins(X86Ops.IMULi, dtypes.int32, address + (imm(10),), RDI)
