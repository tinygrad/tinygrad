...
class TestArgOrder(unittest.TestCase):
  ...
  def test_mixed_scalar_widths(self):
    scalar_spec = [(name, dt, val) for name,(dt,val) in zip("abcd", ((dtypes.int8, -3), (dtypes.int16, 2000),
                   (dtypes.int32, -100000), (dtypes.int64, 2**35))) if dt in Device[Device.DEFAULT].renderer.supported_dtypes()]
    out_dtype = dtypes.int64 if dtypes.int64 in Device[Device.DEFAULT].renderer.supported_dtypes() else dtypes.int32
    spec = scalar_spec[:1] + [("out", out_dtype, 0)] + scalar_spec[1:2] + [("inp", out_dtype, 0)] + scalar_spec[2:]
    params = {name:UOp.param(slot, dt, (1,) if name in ("out", "inp") else (), name=None if name in ("out", "inp") else name,
                             addrspace=AddrSpace.GLOBAL if name in ("out", "inp") else AddrSpace.ALU)
              for slot,(name,dt,_) in enumerate(spec)}
    value = params["inp"][0].load()
    for name,_,_ in scalar_spec: value += params[name].cast(out_dtype)
    sink = params["out"][0].store(value).sink(arg=KernelInfo("arg_widths"))
    vals = {name:val for name,_,val in scalar_spec}
    out = Buffer(Device.DEFAULT, 1, out_dtype, initial_value=bytes(out_dtype.itemsize))
    inp = Buffer(Device.DEFAULT, 1, out_dtype, initial_value=struct.pack(out_dtype.fmt, 17))
    _run(sink, [out, inp], vals)
    self.assertEqual(struct.unpack(out_dtype.fmt, out.as_memoryview())[0], 17 + sum(vals.values()))

  def test_many_args(self):
    params = [UOp.param(slot, dtypes.int, () if slot == 4 else (1,), name="val" if slot == 4 else None,
                        addrspace=AddrSpace.ALU if slot == 4 else AddrSpace.GLOBAL) for slot in range(9)]
    sink = params[0][0].store(sum((x[0].load() for x in params[1:] if x.addrspace is AddrSpace.GLOBAL), params[4])) \
             .sink(arg=KernelInfo("many_args"))
    bufs = [Buffer(Device.DEFAULT, 1, dtypes.int, initial_value=struct.pack("i", i)) for i in range(8)]
    _run(sink, bufs, {"val":8})
    self.assertEqual(_read_int(bufs[0]), sum(range(1, 9)))
  ...
  def test_repeated_buffer_slot(self):
    signature = ((ParamArg(0, dtypes.int, name="val", addrspace=AddrSpace.ALU), ()),
                 (ParamArg(1, dtypes.float), (32,)), (ParamArg(1, dtypes.float), (2, 4, 4)))
    self.assertEqual([(arg.slot, shape, idx) for _,arg,shape,idx in TinyELF.iter_sig(signature)],
                     [(0, (), 0), (1, (32,), 0), (1, (2, 4, 4), 0)])
  ...
  def test_manual_program_signature(self):
    ...
    for linear in (UOp(Ops.LINEAR), UOp(Ops.LINEAR, src=(out, inp))):
      with self.subTest(linear_params=len(linear.src)):
        prg = UOp(Ops.PROGRAM, src=(sink, linear, UOp(Ops.SOURCE, arg=""), UOp(Ops.BINARY, arg=b"")), arg=ProgramInfo.from_sink(sink))
