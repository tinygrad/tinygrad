...
def uop(uops:list[UOp], op:Ops, dtype:Optional[DType], src:tuple[UOp, ...], arg:Any=None) -> UOp:
  if op is Ops.CONST: uops.append(UOp.const(arg, dtype))
...
def _test_single_value_const(vals, op, dts):
  ...
  alu = uop(uops, op, output_dtype, loads)
  out = buf_store[UOp.const(0, dtypes.int32)].store(alu)
  buf = Buffer(Device.DEFAULT, 1, output_dtype).allocate()
  run_uops([out], [buf])
  return np.frombuffer(buf.as_memoryview(), _to_np_dtype(output_dtype))[0]
...
class TestAssembly(unittest.TestCase):
  def test_bitshift_left(self):
    ...
    c1 = UOp.const(2, dtypes.int)
    c2 = UOp.const(3, dtypes.int)
    l1 = g1.index(c1)
    a1 = UOp(Ops.MUL, src=(l1, c1))
    a2 = UOp(Ops.MUL, src=(l1, c2))
    uops = to_uops_list([out.index(UOp.const(0, dtypes.int)).store(a1), out.index(UOp.const(1, dtypes.int)).store(a2)],
  ...
  def test_mulacc_shl(self):
    ...
    c1 = UOp.const(0, dtypes.int)
    c2 = UOp.const(1, dtypes.int)
    expr = g1.index(c1) * UOp.const(4096, dtypes.int) + g1.index(c2)
