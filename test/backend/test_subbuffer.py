...
class TestSubBuffer(unittest.TestCase):
  ...
  def test_subbuffer_deallocate(self):
    with Context(LRU=0):
      ...
      self.buf.copy_from(Buffer("PYTHON", 10, dtypes.uint8, initial_value=bytes(range(10, 20))))
  ...
  def test_subbuffer_copy_in_out(self):
    ...
    sub_buf.copy_from(Buffer("PYTHON", 3, dtypes.uint8, initial_value=bytes(range(3))))
  ...
  def test_subbuffer_copy_in_out_view_of_view(self):
    ...
    view2.copy_from(Buffer("PYTHON", 3, view2.dtype, initial_value=data_in))
  ...
  def test_subbuffer_alloc(self):
    ...
    sub_buf.allocate()
    sub_buf.copy_from(Buffer("PYTHON", 4, dtypes.int8, initial_value=bytes(range(10, 14))))
