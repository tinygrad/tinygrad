...
def apply_rewrite_values(expr):
  ...
  if len(srcs) == 1:
    if srcs[0].is_const: return (v,) if not isinstance(v:=srcs[0].const_value, tuple) else v
    if srcs[0].op is Ops.STACK: return tuple(s.const_value for s in srcs[0].src)
  return tuple(s.const_value for s in srcs)

def evaluate_uop(uop, variables):
  if uop.is_const:
    return uop.const_value
...
class TestArithmeticSimplifications(unittest.TestCase):
  def test_full_graph_rewrite_division_by_zero(self):
    ...
    self.assertTrue(math.isinf(optimized_div_uop.const_value) or math.isnan(optimized_div_uop.const_value))
  ...
  def test_full_graph_rewrite_redundant_operations(self):
    ...
    self.assertEqual(optimized_uop.const_value, 10.0)
...
class TestFoldingAndReduction(unittest.TestCase):
  ...
  def test_full_graph_rewrite_constant_reduction_folding(self):
    ...
    self.assertEqual(optimized_sink.const_value, expected_sum)
  ...
  def test_full_graph_rewrite_nested_loop_collapse(self):
    ...
    self.assertEqual(optimized_reduce_uop.const_value, sum((i * 10) + j for i in range(8) for j in range(4)))
...
class TestModuloAndDivisionFolding(unittest.TestCase):
  def test_full_graph_rewrite_modulo_folding_with_define_var(self):
    ...
    self.assertEqual(optimized_mod_uop.const_value, 2)
  ...
  def test_full_graph_rewrite_division_folding_with_define_var(self):
    ...
    self.assertEqual(optimized_div_uop.src[1].const_value, 2)
  ...
  def test_full_graph_rewrite_complex_mod_div_folding(self):
    ...
    self.assertTrue(optimized_div_uop.is_const)
    self.assertEqual(optimized_div_uop.const_value, 1)
...
class TestEdgeCasesAndSpecialOperations(unittest.TestCase):
  def test_full_graph_rewrite_transcendental_edge_cases(self):
    ...
    self.assertTrue(math.isnan(optimized_log2_neg.const_value), f"Expected NaN for log2(-1.0), got {optimized_log2_neg.const_value}")
    self.assertTrue(math.isinf(optimized_recip_zero.const_value) and optimized_recip_zero.const_value > 0,
                    f"Expected +inf for reciprocal(0.0), got {optimized_recip_zero.const_value}")
...
class TestGEPAndVectorizeRewrite(unittest.TestCase):
  def test_gep_single_element_extraction(self):
    ...
    self.assertEqual((apply_rewrite(base_vector.index(2))).const_value, 3.0)
  ...
  def test_gep_on_const_stack(self):
    ...
    self.assertEqual((apply_rewrite(const_stack.index(2))).const_value, 3.0)
