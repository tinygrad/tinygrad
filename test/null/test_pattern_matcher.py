...
class TestPatternMatcher(unittest.TestCase):
  ...
  def test_arg(self):
    ...
    c1 = UOp.const(0.0, dtypes.float)
    c2 = UOp.const(False)
    c3 = UOp(Ops.MAX, src=(c1, c1))
    c4 = UOp(Ops.MUL, src=(c1, c1))
    c5 = UOp.const(-1, dtypes.int)
    self.assertEqual(matcher.rewrite(c1), c1.rtag())
    self.assertEqual(matcher.rewrite(c2), c2.rtag())
  ...
  def test_filter_arg(self):
    ...
    y1 = UOp.const(1, dtypes.int)
    y2 = UOp.const(2, dtypes.int)
    y3 = UOp.const(-1, dtypes.int)
  ...
  def test_dup_name(self):
    ...
    y1 = UOp.const(1.0, dtypes.float)
    y2 = UOp.const(1.0, dtypes.float)
  ...
  def test_dtype(self):
    ...
    c1 = UOp.const(1.0, dtypes.float)
    c2 = UOp.const(1.0, dtypes.float64)
    self.assertEqual(matcher.rewrite(c1), c1.rtag())
    self.assertEqual(matcher.rewrite(c2), None)
  ...
  def test_dtype_set(self):
    ...
    c1 = UOp.const(1.0, dtypes.float)
    c2 = UOp.const(1.0, dtypes.float64)
    c3 = UOp.const(1.0, dtypes.float16)
    c4 = UOp.const(1, dtypes.int)
    self.assertEqual(matcher.rewrite(c1), c1.rtag())
    self.assertEqual(matcher.rewrite(c2), c2.rtag())
