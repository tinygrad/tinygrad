...
from test.helpers import to_uops_list, const_value

simple_pm = PatternMatcher([
  (UPat.cvar('x', dtypes.weakint), lambda x: UOp.const(1.0) + UOp.const(2.0)),
  (UPat.cvar('x') + UPat.cvar('y'), lambda x,y: UOp.const(x.arg+y.arg)),
  (UPat.cvar('x') * UPat.cvar('y') * UPat.cvar('z'), lambda x,y,z: UOp.const(x.arg*y.arg*z.arg)),
...
def const_values(u:UOp):
  ...
  if u.op is Ops.STACK: return tuple(const_value(x) for x in u.src)
...
class TestUOpGraph(unittest.TestCase):
  def test_add_constant_fold(self):
    ...
    self.assertEqual(const_value(uops[-2]), 3.0)
  ...
  def test_where_same_fold(self):
    ...
    uops = to_uops_list([out])
    self.assertEqual(len(uops), 3)  # the folded const is the pair CAST(float, CONST), +1 for SINK
    self.assertEqual(const_value(uops[-2]), 1.0)
  ...
  def test_where_const_fold(self):
    ...
    uops = to_uops_list([out])
    self.assertEqual(len(uops), 3)  # the folded const is the pair CAST(float, CONST), +1 for SINK
    self.assertEqual(const_value(uops[-2]), 2.0)
  ...
  def test_const_bitcast(self):
    ...
    self.assertEqual(const_value(uops[-2]), 0x3F800000)
  ...
  def test_gep_vec_const_fold(self):
    for vec_size in [2, 4, 8]:
      ...
      with Context(SPEC=0):
        ...
        self.assertEqual([const_value(u) for u in uops if u.op is Ops.CAST and u.is_const], [c.arg for c in consts])
  ...
  def test_depth_2_const_fold(self):
    ...
    self.assertEqual(const_value(out.src[1]), 6)
  ...
  def test_where_on_gated_load_fold(self):
    ...
    for u in uops:
      ...
      if u.op is Ops.LOAD and u.src[0].src[0].op is Ops.PARAM: assert const_value(u.src[1])==5
  ...
  def test_where_on_gated_load_folds_swapped_branches(self):
    ...
    for u in uops:
      assert u.op is not Ops.WHERE
      if u.op is Ops.LOAD: assert const_value(u.src[1])==5
  ...
  def test_where_in_store_becomes_gate(self):
    ...
    for u in uops:
      assert u.op is not Ops.WHERE
      if u.op is Ops.STORE: assert const_value(u.src[1])==5
...
class TestConstBufferize(unittest.TestCase):
  def test_const_bufferize_with_ranges(self):
    ...
    const_vals = [u.arg for u in result.toposort() if u.op is Ops.CONST and u.dtype is dtypes.weakfloat]
