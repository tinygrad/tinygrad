...
class TestLinearizerFailures(unittest.TestCase):
  def test_fail_1(self):
    ...
    c8 = c7.index(c3)
    c9 = ((((c6+(c8*UOp.const(-1.0, dtypes.float)))*(c6+(c8*UOp.const(-1.0, dtypes.float)))).reduce(c5, arg=Ops.ADD)*UOp.const(0.000390625, dtypes.float))+UOp.const(1e-05, dtypes.float)).sqrt().reciprocal()
