...
from tinygrad.engine.search import time_linearizer, bufs_from_lin, actions, beam_search, _try_compile_linearized_w_idx
...
class TestTimeProgram(unittest.TestCase):
  def setUp(self):
    ast = get_ast_helper()
    k = Kernel(ast)
    dev = Device[k.opts.device]
    _, proc = _try_compile_linearized_w_idx((0, k), dev.compiler)

    self.p, self.lib, self.compile_et = proc
