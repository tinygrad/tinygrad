...
class TestTimeLinearizer(unittest.TestCase):
    @unittest.skipIf(
        Device.DEFAULT == "WEBGPU", "WebGPU timestamps are low precision, tm is 0"
    )
    def test_reasonable_time(self):
        ...
        memops = {
            x.src[0].arg: x.src[-1].arg.real_size()
            for x in si.ast.toposort
            if x.op is Ops.LOAD
        }
        rawbufs = [out] + [
            Buffer(Device.DEFAULT, memops[i], x.dtype).allocate()
            for i, x in enumerate(si.inputs, start=len(si.outputs))
        ]
        tm = time_linearizer(
            Kernel(si.ast), rawbufs, allow_test_size=False, cnt=10, disable_cache=True
        )
    ...
    def test_kernel_count(self):
        ...
        time_linearizer(
            lin, bufs, allow_test_size=False, cnt=2, disable_cache=True, clear_l2=True
        )
        assert (
            GlobalCounters.kernel_count == kernel_count
        ), "kernel count was incremented by time_linearizer"
...
class TestBEAM(unittest.TestCase):
    def test_dynamic_beam(self):
        ...
        kernel_count = GlobalCounters.kernel_count
        with Context(BEAM=1):
            Tensor.zeros(16).contiguous().realize()
            ...
            assert (
                len(
                    [
                        x
                        for x in lins
                        if x.applied_opts[0] == Opt(OptOps.UPCAST, axis=0, arg=4)
                    ]
                )
                == 0
            ), "did not de-dup UPCAST"
            ...
            assert (
                len(
                    [
                        x
                        for x in lins
                        if x.applied_opts[0] == Opt(OptOps.LOCAL, axis=0, arg=4)
                    ]
                )
                == 0
            ), "did not de-dup LOCAL"
            ...
            assert (
                len(
                    [
                        x
                        for x in lins
                        if x.applied_opts[0] == Opt(OptOps.UNROLL, axis=0, arg=3)
                    ]
                )
                == 0
            ), "did not de-dup UNROLL"
            ...
            assert (
                len(
                    [
                        x
                        for x in lins
                        if x.applied_opts[0] == Opt(OptOps.GROUP, axis=0, arg=3)
                    ]
                )
                == 0
            ), "did not de-dup GROUP"
            ...
            assert (
                len(
                    [
                        x
                        for x in lins
                        if x.applied_opts[0] == Opt(OptOps.GROUPTOP, axis=0, arg=3)
                    ]
                )
                == 0
            ), "did not de-dup GROUPTOP"
    ...
    def test_search_over_shape(self):
        ...
        dtype_pairs = [
            (tc.dtype_in, tc.dtype_out)
            for tc in Device[Device.DEFAULT].renderer.tensor_cores
        ]
        multi_shape_dtype_pairs = [
            dts for dts in dtype_pairs if dtype_pairs.count(dts) > 1
        ]

        if len(multi_shape_dtype_pairs) == 0:
            raise unittest.SkipTest(
                "only one tc available per dtype pair to search over"
            )
        ...
        for dtype_in, dtype_out in multi_shape_dtype_pairs:
            ...
            assert (
                len(
                    set(
                        lin.tensor_core.dims
                        for lin in lins
                        if lin.tensor_core is not None
                    )
                )
                > 1
            )
    ...
    def test_filter_global_buffer(self):
        ...
        tm = time_linearizer(
            best_lin, bufs, allow_test_size=False, cnt=2, disable_cache=True
        )
        assert tm
