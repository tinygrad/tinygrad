...
def step_tinygrad(optim, kwargs={}, gpu=False):
  ...
  if gpu is True:
    [x.cuda_() for x in get_parameters([net, optim])]
  out = net.forward()
  out.backward()
  optim.step()
...
class TestOptim(unittest.TestCase):
  ...
  def test_adam(self):
    for x, y in zip(step_tinygrad(Adam, gpu=self.gpu), step_pytorch(torch.optim.Adam)):
      np.testing.assert_allclose(x, y, atol=1e-4)

  def test_sgd(self):
    for x, y in zip(
      step_tinygrad(SGD, kwargs={"lr": 0.001}, gpu=self.gpu),
  ...
  def test_rmsprop(self):
    for x, y in zip(
      step_tinygrad(RMSprop, kwargs={"lr": 0.001, "decay": 0.99}, gpu=self.gpu),
