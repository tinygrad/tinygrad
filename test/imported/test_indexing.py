...
def set_(reference: Tensor, shape, strides, offset):
    ...
    if reference.lazydata.base.realized is None:
        reference.realize()
    assert (
        reference.lazydata.base.realized
    ), "base has to be realized before setting it to strided's base"
    strided = Tensor(
        reference.lazydata.view(
            ShapeTracker((View.create(shape=shape, strides=strides, offset=offset),))
        )
    )
    assert (
        strided.lazydata.st.real_strides() == strides
    ), "real_strides should equal strides for strided"
    return strided
...
def diagonal(tensor: Tensor) -> Tensor:
    assert tensor.ndim == 2 and all_same(
        tensor.shape
    ), "only support 2 ndim square tensors"
...
def make_tensor(shape, dtype: dtypes, noncontiguous) -> Tensor:
    ...
    if dtype == dtypes.bool:
        return Tensor.randint(shape=shape, low=0, high=2, contiguous=contiguous).cast(
            dtypes.bool
        )
    elif dtype.is_unsigned():
        return Tensor.randint(shape=shape, low=0, high=10, contiguous=contiguous).cast(
            dtype
        )
    elif dtype.is_int():
        return Tensor.randint(shape=shape, low=-9, high=10, contiguous=contiguous).cast(
            dtype
        )  # signed int
    elif dtype.is_float():
        return Tensor.rand(
            shape=shape, low=-9, high=9, dtype=dtype, contiguous=contiguous
        )
    else:
...
class TestIndexing(unittest.TestCase):
    def test_index(self):
        ...
        numpy_testing_assert_equal_helper(
            reference[..., 2],
            np.array([[3.0, 6.0, 9.0], [12.0, 15.0, 18.0], [21.0, 24.0, 27.0]]),
        )
        numpy_testing_assert_equal_helper(
            reference[0, ..., 2], np.array([3.0, 6.0, 9.0])
        )
        ...
        numpy_testing_assert_equal_helper(
            reference_5d[..., 1, 0], reference_5d[:, :, :, 1, 0]
        )
        numpy_testing_assert_equal_helper(
            reference_5d[2, ..., 1, 0], reference_5d[2, :, :, 1, 0]
        )
        numpy_testing_assert_equal_helper(
            reference_5d[2, 1, 0, ..., 1], reference_5d[2, 1, 0, :, 1]
        )
        ...
        numpy_testing_assert_equal_helper(
            reference[2:4, None], reference[2:4].unsqueeze(1)
        )
        numpy_testing_assert_equal_helper(
        ...
        numpy_testing_assert_equal_helper(
            reference[1:5:2], Tensor.stack(reference[1], reference[3], dim=0)
        )
        numpy_testing_assert_equal_helper(
            reference[1:6:2],
            Tensor.stack(reference[1], reference[3], reference[5], dim=0),
        )
        numpy_testing_assert_equal_helper(
            reference[1:9:4], Tensor.stack(reference[1], reference[5], dim=0)
        )
        numpy_testing_assert_equal_helper(
            reference[2:4, 1:5:2],
            Tensor.stack(reference[2:4, 1], reference[2:4, 3], dim=1),
        )
        numpy_testing_assert_equal_helper(
            reference[3, 1:6:2],
            Tensor.stack(reference[3, 1], reference[3, 3], reference[3, 5], dim=0),
        )
        numpy_testing_assert_equal_helper(
            reference[None, 2, 1:9:4],
            Tensor.stack(reference[2, 1], reference[2, 5], dim=0).unsqueeze(0),
        )
        numpy_testing_assert_equal_helper(
            reference[:, 2, 1:6:2],
            Tensor.stack(
                reference[:, 2, 1], reference[:, 2, 3], reference[:, 2, 5], dim=1
            ),
        )
    ...
    @unittest.skipIf(
        CI and Device.DEFAULT in ["CLANG", "GPU", "METAL", "NV", "AMD"], "slow"
    )
    def test_advancedindex(self):
        ...
        numpy_testing_assert_equal_helper(
            reference[ri([0, 1, 2]), ri([0])], np.array([1, 3, 5])
        )
        numpy_testing_assert_equal_helper(
            reference[ri([0, 1, 2]), ri([1])], np.array([2, 4, 6])
        )
        ...
        numpy_testing_assert_equal_helper(
            reference[[ri([0, 0]), ri([0, 1])]], np.array([1, 2])
        )
        numpy_testing_assert_equal_helper(
            reference[[ri([0, 1, 1, 0, 2]), ri([1])]], np.array([2, 4, 4, 2, 6])
        )
        numpy_testing_assert_equal_helper(
            reference[[ri([0, 0, 1, 1]), ri([0, 1, 0, 0])]], np.array([1, 2, 3, 3])
        )

        rows = ri([[0, 0], [1, 2]])
        columns = ([0],)
        numpy_testing_assert_equal_helper(
            reference[rows, columns], np.array([[1, 1], [3, 5]])
        )

        rows = ri([[0, 0], [1, 2]])
        columns = ri([1, 0])
        numpy_testing_assert_equal_helper(
            reference[rows, columns], np.array([[2, 1], [4, 5]])
        )
        rows = ri([[0, 0], [1, 2]])
        columns = ri([[0, 1], [1, 0]])
        numpy_testing_assert_equal_helper(
            reference[rows, columns], np.array([[1, 2], [4, 5]])
        )
        ...
        numpy_testing_assert_equal_helper(
            reference[ri([0, 1, 2]), ri([0])], np.array([0, 1, 2])
        )
        numpy_testing_assert_equal_helper(
            reference[ri([0, 1, 2]), ri([1])], np.array([4, 5, 6])
        )
        ...
        numpy_testing_assert_equal_helper(
            reference[[ri([0, 0]), ri([0, 1])]], np.array([0, 4])
        )
        numpy_testing_assert_equal_helper(
            reference[[ri([0, 1, 1, 0, 3]), ri([1])]], np.array([4, 5, 5, 4, 7])
        )
        numpy_testing_assert_equal_helper(
            reference[[ri([0, 0, 1, 1]), ri([0, 1, 0, 0])]], np.array([0, 4, 1, 1])
        )

        rows = ri([[0, 0], [1, 2]])
        columns = ([0],)
        numpy_testing_assert_equal_helper(
            reference[rows, columns], np.array([[0, 0], [1, 2]])
        )

        rows = ri([[0, 0], [1, 2]])
        columns = ri([1, 0])
        numpy_testing_assert_equal_helper(
            reference[rows, columns], np.array([[4, 0], [5, 2]])
        )
        rows = ri([[0, 0], [1, 3]])
        columns = ri([[0, 1], [1, 2]])
        numpy_testing_assert_equal_helper(
            reference[rows, columns], np.array([[0, 4], [5, 11]])
        )
    ...
    """
        ...
        reference = consec((3, 2))
        numpy_testing_assert_equal_helper(
            reference[ri([0, 2]),], np.array([[1, 2], [5, 6]])
        )
        numpy_testing_assert_equal_helper(reference[ri([1]), ...], np.array([[3, 4]]))
        numpy_testing_assert_equal_helper(
            reference[..., ri([1])], np.array([[2], [4], [6]])
        )
      ...
      """
        ...
        def tensor_indices_to_np(tensor: Tensor, indices):
            ...
            idxs = tuple(
                (
                    i.numpy().tolist()
                    if isinstance(i, Tensor) and i.dtype == dtypes.int64
                    else i
                )
                for i in indices
            )
        ...
        def assert_get_eq(tensor, indexer):
            numpy_testing_assert_equal_helper(
                tensor[indexer], get_numpy(tensor, indexer)
            )
        ...
        def get_set_tensor(indexed: Tensor, indexer):
            ...
            set_tensor = Tensor.randint(set_count, high=set_count).reshape(
                set_size
            )  # .cast(dtypes.float64)
            return set_tensor
  ...
  """
    ...
    def test_bool_indices(self):
        ...
        numpy_testing_assert_equal_helper(
            v[boolIndices], Tensor.stack([v[0], v[2], v[3]])
        )
  ...
  """
    ...
    def test_jit_indexing(self):
        ...
        ref = Tensor(
            np.concatenate((np.ones(50), np.arange(50, 100))), dtype=dtypes.float
        )
  ...
  """
    ...
    def test_int_indices2d(self):
        ...
        numpy_testing_assert_equal_helper(
            x[rows, columns].numpy().tolist(), [[0, 2], [9, 11]]
        )
    ...
    def test_empty_ndim_index(self):
        x = Tensor.randn(5)
        numpy_testing_assert_equal_helper(
            Tensor.empty(0, 2), x[Tensor.empty(0, 2, dtype=dtypes.int64)]
        )

        x = Tensor.randn(2, 3, 4, 5)
        numpy_testing_assert_equal_helper(
            Tensor.empty(2, 0, 6, 4, 5), x[:, Tensor.empty(0, 6, dtype=dtypes.int64)]
        )
  ...
  """
    ...
    @unittest.skip(
        "getitem expects elementwise ops folding to be instant, but it happens late in the scheduler"
    )
  ...
  """
    ...
    def test_ellipsis_tensor(self):
        ...
        idx = Tensor([0, 2])
        numpy_testing_assert_equal_helper(
            x[..., idx].numpy().tolist(), [[0, 2], [3, 5], [6, 8]]
        )
        numpy_testing_assert_equal_helper(
            x[idx, ...].numpy().tolist(), [[0, 1, 2], [6, 7, 8]]
        )
...
class TestNumpy(unittest.TestCase):
    ...
    def test_boolean_indexing_weirdness(self):
        ...
        numpy_testing_assert_equal_helper(
            Tensor.ones(1, 2), a[True, [0, 1], True, True, [1], [[2]]]
        )
    ...
    def test_broaderrors_indexing(self):
        ...
        self.assertRaises(
            IndexError, a.contiguous().__setitem__, ([0, 1], [0, 1, 2]), 0
        )
