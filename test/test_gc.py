...
class TestGC(unittest.TestCase):
    ...
    def test_gc(self):
        ...
        del a, b
        assert (
            tensors_allocated() - base == 2
        )  # one for Tensor._device_rng_counters, and one for Tensor._device_seeds
        Tensor.manual_seed(0)
    ...
    def test_toposort_blocks_gc(self):
        ...
        if bufs_allocated() - init != 0:
            ...
            raise AssertionError(
                f"never gced {[x for x in gc.get_objects() if isinstance(x, Buffer)]}"
            )
