...
class TestFocalLoss(unittest.TestCase):
  def test_simple(self):
    ...
    self.assertAlmostEqual(self._torch_sigmoid_focal_loss(a, b), self._tinygrad_sigmoid_focal_loss(a, b))
  ...
  def _tinygrad_sigmoid_focal_loss(self, a: np.ndarray, b: np.ndarray, alpha=0.1, gamma=1) -> float:
    return focal_loss(Tensor(a).sigmoid(), Tensor(b), alpha=alpha, gamma=gamma).numpy()[0]
