...
def verify_ast(sink: UOp):
    return type_verify(list(sink.toposort), shape_spec)
...
def check_schedule(
    t: Union[Tensor, List[Tensor], UOp],
    allowed: int,
    ...
    if isinstance(t, Tensor):
        sched = t.schedule()
    elif isinstance(t, List) and isinstance(t[0], Tensor):
        sched = Tensor.schedule(*t)
    else:
    ...
    if filter_sink:
        sched = [
            s for s, ei in zip(sched, lowered) if isinstance(ei.prg, CompiledRunner)
        ]
...
def _realize_weights(m):
    for p in nn.state.get_parameters(m):
        p.realize()
...
def _test_conv2d(allowed: int, dtype: DType = dtypes.float, **kwargs):
    ...
    if getenv("CHECK", 1):
        ...
        assert (
            ref_img.grad is not None
            and ref_w.grad is not None
            and img.grad is not None
            and w.grad is not None
        )
        np.testing.assert_allclose(
            img.grad.numpy(),
            ref_img.grad.detach().numpy(),
            atol=1e-6 if dtype == dtypes.float else 1e-2,
        )
        np.testing.assert_allclose(
            w.grad.numpy(),
            ref_w.grad.detach().numpy(),
            atol=1e-6 if dtype == dtypes.float else 1e-2,
        )


@track_rewrites(named=True)
def schedule_graph_rewrite(big_sink: UOp):
    return graph_rewrite(big_sink, remove_movement_ops + sym, {})
...
class TestSchedule(unittest.TestCase):
    ...
    def test_reduce_permute_binop_fusion(self):
        ...
        with self.assertRaises(KernelCountException):
            check_schedule(c, 1)
    ...
    def test_cache_reduce_parent(self):
        ...
        reduceops = [
            x for si in schedule for x in si.ast.toposort if x.op is Ops.REDUCE_AXIS
        ]
    ...
    def test_div_collapse_buffer(self):
        ...
        )  # the scheduler can fold divs now!
    ...
    def test_dedup_assign(self):
        ...
        b = Tensor.full((4,), 2.0).contiguous()
        first = a.assign(b)
        second = a.assign(b)
    ...
    def test_dedup_outputs(self):
        a = Tensor.full((4, 4), 1.0).contiguous().realize()
        b = Tensor.full((4, 4), 1.0).contiguous().realize()
    ...
    def test_fold_conv_relu_alt(self):
        ...
        check_schedule(
            img_conv,
            2,
            [*nn.state.get_parameters(c1), *nn.state.get_parameters(c2), img],
        )
    ...
    # multireduce spec
    @unittest.skipUnless(
        SPLIT_REDUCEOP, "Testing split reducop requires SPLIT_REDUCEOP"
    )
    def test_preserve_multistage_reduce(self):
        ...
        np.testing.assert_allclose(
            out.numpy(), (x.numpy() - x.numpy().max(keepdims=True)).max()
        )
    ...
    def test_multireduce_shrink(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            a.numpy().sum(axis=1)[:16] + b.numpy().sum(axis=1)[:16] + c.numpy(),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_reduce_same_size(self):
        ...
        np.testing.assert_allclose(
            out0.numpy(), out0_np := a.numpy().sum() + 2, atol=1e-4, rtol=1e-6
        )
        np.testing.assert_allclose(
            out1.numpy(), out1_np := a.numpy().sum() + 4, atol=1e-4, rtol=1e-6
        )
        np.testing.assert_allclose(
            out2.numpy(), out0_np * out1_np, atol=1e-4, rtol=1e-6
        )
    ...
    def test_reduce_multiple_paths(self):
        ...
        np.testing.assert_allclose(
            out0.numpy(), out0_np := np.exp2(a.numpy().sum()), atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            out1.numpy(), a.numpy().sum() + out0_np, atol=1e-4, rtol=1e-6
        )
    ...
    def test_multireduce_reduce_multiple_paths(self):
        ...
        np.testing.assert_allclose(
            out0.numpy(), np_out0 := np.exp2(a.numpy().sum()), atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            out1.numpy(), np_out1 := a.numpy().sum() + np_out0, atol=1e-4, rtol=1e-4
        )
        np_b = a.numpy() + np_out0 + np_out1
        np.testing.assert_allclose(
            out2.numpy(), np_out2 := np.exp2(np_b.sum()), atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            out3.numpy(), np_b.sum() + np_out2, atol=1e-4, rtol=1e-4
        )
    ...
    def test_reduce_ext_reduce_child(self):
        ...
        np.testing.assert_allclose(
            out0.numpy(), a.numpy().sum() + b.numpy().sum() + 2, atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            out1.numpy(), a.numpy().sum() + b.numpy().sum() + 4, atol=1e-4, rtol=1e-4
        )
    ...
    def test_reduce_multiple_paths_midreduce(self):
        ...
        np.testing.assert_allclose(
            r.numpy(), r_np := a.numpy().sum(), atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            out0.numpy(), out0_np := np.exp2(r_np), atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            out1.numpy(), out1_np := (a.numpy() - out0_np).max(), atol=1e-4, rtol=1e-4
        )
    ...
    def test_reduce_multiple_paths_midreduce_fused(self):
        ...
        np.testing.assert_allclose(
            out0.numpy(), out0_np := a.numpy().sum() + 4, atol=1e-4, rtol=1e-6
        )
        np.testing.assert_allclose(
            out1.numpy(), out1_np := b.numpy().max() + out0_np * 2, atol=1e-4, rtol=1e-6
        )
        np.testing.assert_allclose(
            out2.numpy(), a.numpy().sum() + out1_np, atol=1e-4, rtol=1e-6
        )
    ...
    def test_reduce_multiple_paths_midexpand(self):
        ...
        np.testing.assert_allclose(
            e.numpy(), e_np := b.numpy() + out0_np, atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            out1.numpy(), r_np + e_np[0][0][0], atol=1e-4, rtol=1e-4
        )
    ...
    def test_reduce_expand_child(self):
        ...
        run_schedule(check_schedule([out0, out1], 4))
        np.testing.assert_allclose(
            out0.numpy(), a.numpy().sum() + 2, atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            out1.numpy(), a.numpy().sum() + b.numpy(), atol=1e-4, rtol=1e-4
        )
    ...
    def test_std_multireduce_fusion(self):
        ...
        run_schedule(check_schedule(out, 2))
        np.testing.assert_allclose(
            out.numpy(), x.numpy().std(axis=-1, ddof=1), atol=1e-4, rtol=1e-4
        )
    ...
    def test_scaled_dot_product_attention_multireduce_fusion(self):
        ...
        if getenv("CHECK", 1):
            ...
            compare = torch.nn.functional.scaled_dot_product_attention(
                torch.tensor(q.numpy()),
                torch.tensor(k.numpy()),
                torch.tensor(v.numpy()),
            )
            np.testing.assert_allclose(
                out.numpy(), compare.numpy(), atol=1e-6, rtol=1e-3
            )
    ...
    def test_ugly_reduceop_pairing(self):
        ...
        out = (c * a.sum(-1, keepdim=True)).sum(-1) + (b * a.sum(-1, keepdim=True)).sum(
            -1
        )  # a.sum has >1 children but should still fuse
        # run_schedule(check_schedule(out, 1))
        run_schedule(check_schedule(out, 3))
        np.testing.assert_allclose(
            out.numpy(),
            (c.numpy() * a.numpy().sum(axis=-1, keepdims=True)).sum(-1)
            + (b.numpy() * a.numpy().sum(axis=-1, keepdims=True)).sum(-1),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_reduce_expand_reduce_fusion(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            (a.numpy() + a.numpy().sum(axis=-1, keepdims=True)).sum(axis=-1),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_reduce_expand_reduce_expand_fusion(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            a.numpy()
            + (a.numpy() + a.numpy().sum(axis=-1, keepdims=True)).sum(
    ...
    def test_branching_reduces_and_expands_fusion(self):
        ...
        np.testing.assert_allclose(
            out0.numpy(),
            a.numpy() + a.numpy().sum(axis=-1, keepdims=True),
            atol=1e-4,
            rtol=1e-4,
        )
        np.testing.assert_allclose(
    ...
    def test_multireduce_fusion_simple_sequential(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            (y.numpy() + x.numpy().sum(axis=-1, keepdims=True)).sum(axis=-1),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_multireduce_fusion_simple_parallel(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            y.numpy().sum(axis=-1) + x.numpy().sum(axis=-1),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_multireduce_fusion_parallel(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            x.numpy().std(axis=-1, ddof=1) + y.numpy().std(axis=-1, ddof=1),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_multireduce_diffops_sequential(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            (x.numpy() - x.numpy().max(axis=-1, keepdims=True)).sum(axis=-1),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_multireduce_fusion_diffops_parallel(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            x.numpy().sum(axis=-1) + y.numpy().max(axis=-1),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_multireduce_fusion_sequential_and_parallel(self):
        ...
        x = Tensor.randn(4, 32).realize()
        y = Tensor.randn(4, 32).realize()
        mu = (x - x.max(axis=-1, keepdim=True)).mean(axis=-1, keepdim=True) + (
            y - y.max(axis=-1, keepdim=True)
        ).mean(axis=-1, keepdim=True)
        out = [
            ((x - mu).square().sum(-1) / x.shape[-1]).sqrt(),
            ((y - mu).square().sum(-1) / y.shape[-1]).sqrt(),
        ]
        np_mu = (x.numpy() - x.numpy().max(axis=-1, keepdims=True)).mean(
            axis=-1, keepdims=True
        ) + (y.numpy() - y.numpy().max(axis=-1, keepdims=True)).mean(
            axis=-1, keepdims=True
        )
        # run_schedule(check_schedule(out, 1))
        ...
        np.testing.assert_allclose(
            out[0].numpy(),
            np.sqrt(np.square(x.numpy() - np_mu).sum(-1) / x.shape[-1]),
            atol=1e-4,
            rtol=1e-4,
        )
        np.testing.assert_allclose(
            out[1].numpy(),
            np.sqrt(np.square(y.numpy() - np_mu).sum(-1) / y.shape[-1]),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_softmax_fusion(self):
        ...
        run_schedule(check_schedule(out, 3))
        expected = (
            x_exp := np.exp(x.numpy() - x.numpy().max(-1, keepdims=True))
        ) / x_exp.sum(-1, keepdims=True)
    ...
    def test_layernorm_onelayer_fusion(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            expected * layer.weight.numpy() + layer.bias.numpy(),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_prefer_half_buffer(self):
        ...
        for si in sched[:-2]:
            assert all(out.dtype == dtypes.half for out in si.outputs)
    ...
    def test_multireduce_push_permute_chase(self):
        ...
        run_schedule(schedule)
        np.testing.assert_allclose(
            d.numpy(), (a.numpy().sum(2) + b.numpy()).T * 4, atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            e.numpy(),
            (a.numpy().sum(2) + b.numpy()) * (d.numpy() + a.numpy()).sum(2),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_multireduce_push_shrink_chase(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            (a.numpy().sum(1) + c.numpy())[:4] * b.numpy() + d.numpy().sum(1)[:4],
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_partial_fuse1(self):
        ...
        np.testing.assert_allclose(
            d.numpy(), (a.numpy().sum() - b.numpy().sum()) * 4, atol=1e-4, rtol=1e-4
        )
    ...
    def test_partial_fuse2(self):
        ...
        np.testing.assert_allclose(c.numpy(), a.numpy().sum() + 2, atol=1e-4, rtol=1e-4)
        np.testing.assert_allclose(
            d.numpy(), b.numpy().sum() - (a.numpy().sum() + 2), atol=1e-4, rtol=1e-4
        )
    ...
    def test_partial_fuse3(self):
        ...
        np.testing.assert_allclose(
            c.numpy(), c_np := a.numpy().sum() + 2, atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(
            d.numpy(), d_np := a.numpy().sum() * 2, atol=1e-4, rtol=1e-4
        )
        np.testing.assert_allclose(e.numpy(), e_np := c_np * d_np, atol=1e-4, rtol=1e-4)
        np.testing.assert_allclose(
            f.numpy(), b.numpy().sum() - e_np, atol=1e-4, rtol=1e-4
        )
    ...
    def test_partial_fuse4(self):
        ...
        np.testing.assert_allclose(e.numpy(), e_np := c_np * d_np, atol=1e-4, rtol=1e-4)
        np.testing.assert_allclose(
            f.numpy(), (b.numpy() - d_np).sum() - e_np, atol=1e-4, rtol=1e-4
        )
    ...
    def test_pad_reduce_safe(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            np.pad(
                a.numpy() + b.numpy(), ((0, 1), (0, 1), (0, 1)), constant_values=1.0
            ).sum(),
            atol=1e-5,
            rtol=1e-6,
        )
    ...
    def test_multireduce_pad_reduce_safe(self):
        ...
        out = (
            a.pad(((0, 1), (0, 1), (0, 1)), value=1.0).sum(keepdim=True)
            + b.pad(((0, 1), (0, 1), (0, 1)), value=1.0).sum()
        ).contiguous()
        # run_schedule(check_schedule(out, 1))
        run_schedule(check_schedule(out, 2))
        np.testing.assert_allclose(
            out.numpy(),
            np.pad(a.numpy(), ((0, 1), (0, 1), (0, 1)), constant_values=1.0).sum(
                keepdims=True
            )
            + np.pad(b.numpy(), ((0, 1), (0, 1), (0, 1)), constant_values=1.0).sum(),
            atol=1e-4,
            rtol=1e-4,
        )
    ...
    def test_pad_reduce_unsafe(self):
        ...
        np.testing.assert_allclose(
            out.numpy(),
            np.pad(
                np.log2(a.numpy()), ((0, 1), (0, 1), (0, 1)), constant_values=1.0
            ).sum(),
            atol=1e-5,
            rtol=1e-6,
        )
    ...
    def test_multireduce_pad_reduce_unsafe(self):
        ...
        out = (
            (a.log2().pad(((0, 1), (0, 1), (0, 1)), value=1.0).sum() + b)
            .abs()
            .log2()
            .pad(((0, 1), (0, 1), (0, 1)), value=1.0)
            .sum()
            .contiguous()
        )
        # run_schedule(check_schedule(out, 1))
    ...
    def test_base_change_shrink_pad(self):
        ...
        np.testing.assert_equal(
            d.numpy(), np.pad(np.exp2(a.numpy())[:-1, :-1], ((0, 1), (0, 1))) * 2
        )
    ...
    def test_base_change_expand_pad(self):
        ...
        np.testing.assert_equal(
            d.numpy(),
            np.pad(np.exp2(a.numpy())[:, None, :], ((0, 0), (1, 1), (0, 0))) * 2,
        )
    ...
    def test_base_change_pad_expand(self):
        a = Tensor.full((4, 4), 1.0).contiguous().realize()
        b = Tensor.full((4, 4), 2.0).contiguous().realize()
        c = (a + b).pad(((1, 1), (1, 1)))
        ...
        np.testing.assert_equal(
            d.numpy(), np.broadcast_to(c_np.astype(np.half), (2, *c_np.shape)) * 4
        )
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.half) and is_dtype_supported(dtypes.ulong),
        "need half and ulong",
    )
    def test_conv2d_half(self):
        _test_conv2d(7, dtype=dtypes.half)

    @unittest.skipUnless(is_dtype_supported(dtypes.half), "need half")
    @unittest.skipIf(Device.DEFAULT == "WEBGPU", "Causes other tests to fail")
    @unittest.expectedFailure
    def test_conv2d_fused_half(self):
        _test_conv2d(5, dtype=dtypes.half)

    @unittest.skip(
        "splitting kernels exceeding device buffer count is not yet supported"
    )
    def _test_buf_cnt(self, cnt: int, allowed: int):
        ...
        alu = functools.reduce(
            lambda x, y: x + y,
            [Tensor.ones((1, 1)).contiguous().realize() for _ in range(cnt - 1)],
        )
        s = alu.schedule()
        assert len(s) == allowed
        run_schedule(s)
        expected = functools.reduce(
            lambda x, y: x + y, [np.ones((1, 1)) for _ in range(cnt - 1)]
        )
        np.testing.assert_equal(alu.numpy(), expected)

    def test_buf_cnt_at_limit(self):
        self._test_buf_cnt(31, allowed=1)

    @unittest.expectedFailure
    def test_buf_cnt_over_limit(self):
        self._test_buf_cnt(32, allowed=2)

    @unittest.expectedFailure
    def test_buf_cnt_over_limit_alt(self):
        self._test_buf_cnt(63, allowed=3)
    ...
    def test_image_matmul(self):
        with Context(IMAGE=2):
            ...
            run_schedule(check_schedule(out, 3))
            np.testing.assert_allclose(
                out.numpy(), x.numpy() @ y.numpy(), atol=1e-4, rtol=1e-4
            )
    ...
    def _test_fusion(self, shapes, f, cnt):
        with Context(DEBUG=0, TRACK_MATCH_STATS=0):
            args = [Tensor.randn(s).realize() for s in shapes]
        ...
        if getenv("COMPARE", 1):
            ...
            np.testing.assert_allclose(
                compare.numpy(), good.numpy(), atol=1e-4, rtol=1e-4
            )
    ...
    def test_late_fusion_post_reshape(self):
        self._test_fusion(
            [(4, 4), (1, 4)], lambda a, b: a.sum(1).reshape(b.shape) + b, 1
        )

    def test_late_fusion_post_permute(self):
        self._test_fusion(
            [(4, 6, 4), (4, 4, 1)],
            lambda a, b: a.sum(1, keepdim=True).permute((2, 0, 1)) + b,
            2,
        )
    ...
    def test_late_fusion_double_transpose(self):
        self._test_fusion(
            [(32, 16, 1)],
            lambda a: (
                a.expand(32, 16, 16).sum((2,), keepdim=True).permute((1, 0, 2)) + 2
            )
            .permute((1, 0, 2))
            .contiguous(),
            1,
    ...
    def test_cast_padded_view(self):
        ...
        self.assertListEqual(
            realized_view.tolist(), [[0.0, 1.0, 2.0, 3.0], [0.0, 0.0, 0.0, 0.0]]
        )
...
class TestIndexing(unittest.TestCase):
    def check_schedule(self, xt: Union[Tensor, List[Tensor]], cnt: int):
        with Context(FUSE_ARANGE=getenv("FUSE_ARANGE", 1)):
            ...
            if FUSE_ARANGE:
                self.assertEqual(len(kernels), cnt)
            for ei in lowered:
                ei.run(do_update_stats=True)
        return s
    ...
    def test_simple_indexing_alt(self):
        ...
        np.testing.assert_equal(
            xt.numpy(), (np.arange(16).reshape(4, 4))[[1, 2], [1, 2]]
        )
    ...
    def test_arange_index(self):
        ...
        self.check_schedule(out, 2)
        np.testing.assert_allclose(
            out.numpy(), (x.numpy() + np.arange(10)[2]).sum(), atol=1e-5, rtol=1e-6
        )
    ...
    def test_arange_group_childless_view(self):
        ...
        a = Tensor.arange(4) + x
        a[0] = 6
        np.testing.assert_equal(a.numpy(), [6.0, 2.0, 3.0, 4.0])
    ...
    def test_precompute_freqs_cis(self):
        args = {
            "dim": 32 if CI else 128,
            "end": 2048 if CI else 8192,
            "theta": 10000,
            "dtype": dtypes.half,
        }
    ...
    def test_fuse_assign_contiguous(self):
        ...
        np.testing.assert_equal(
            x.numpy(), [[0, 1, 0, 0], [2, 3, 0, 0], [4, 5, 0, 0], [6, 7, 0, 0]]
        )
    ...
    def test_mnist_val(self):
        ...
        samples = Tensor.randint(
            BS := getenv("BS", 512), high=cast(int, Y_train.shape[-1])
        ).realize()
        ...
        loss_ref = torch.nn.CrossEntropyLoss()(
            torch.tensor(yt.numpy()),
            torch.tensor(Y_train.numpy())[torch.tensor(samples.numpy())],
        )
    ...
    def test_fold_arange_view(self):
        ...
        np.testing.assert_allclose(
            r.numpy(), (X.numpy() + np.arange(16).reshape(4, 4)).sum(1, keepdims=True)
        )
    ...
    def test_no_rewrite_elementwise(self):
        ...
        ld1 = UOp(
            Ops.LOAD, dtypes.int, (bufs[1], ShapeTracker.from_shape((32, 32)).to_uop())
        )
        ld2 = UOp(
            Ops.LOAD, dtypes.int, (bufs[2], ShapeTracker.from_shape((32, 32)).to_uop())
        )
        sink = UOp(
            Ops.SINK,
            dtypes.void,
            (
                UOp(
                    Ops.STORE,
                    dtypes.void,
                    (bufs[0], ShapeTracker.from_shape((32, 32)).to_uop(), ld1 + ld2),
                ),
            ),
        )
        ...
        sink = UOp(
            ...
            (
                UOp(
                    Ops.STORE,
                    dtypes.void,
                    (bufs[0], ShapeTracker.from_shape(()).to_uop(), r),
                ),
            ),
        )
        rsink = graph_rewrite(sink, view_right)
        ...
        sink = UOp(
            ...
            (
                UOp(
                    Ops.STORE,
                    dtypes.void,
                    (bufs[0], ShapeTracker.from_shape((1, 1)).to_uop(), r),
                ),
            ),
        )
        rsink = graph_rewrite(sink, view_right)
...
@track_rewrites(named=True)
def swizzle_rewrite(u: UOp) -> UOp:
    return graph_rewrite(graph_rewrite(u, view_left), view_right)


def swizzle_cnt(u: UOp) -> int:
    return len([x for x in u.toposort if x.op is Ops.VIEW and len(x.src) != 0])
...
def _load_buffer(ctx: list[UOp], buf: UOp):
    ...
    ctx.append(buf)
    return UOp(
        Ops.LOAD, buf.dtype, (glbl, ShapeTracker.from_shape((buf.size,)).to_uop())
    )
...
def run_tensor_ast(r: Tensor):
    ...
    sink = UOp(
        ...
        src=(
            glbl,
            ShapeTracker.from_shape(r.lazydata.base.shape).to_uop(),
            r.lazydata.base,
        ),
    ).sink()
    sink = graph_rewrite(
        sink,
        remove_movement_ops + ops_folding + load_buffers + view_left,
        bufs := [output],
    )
...
class TestSwizzle(unittest.TestCase):
        ...
        swizzle = UOp(
            ...
            arg=ShapeTracker(
                views=(
                    View(
                        shape=(2, 3, 3, 65, 3, 65),
                        strides=(103788, 34596, 3, 558, 1, 9),
                        offset=0,
                        mask=((0, 2), (0, 3), (0, 3), (0, 62), (0, 3), (0, 62)),
                        contiguous=False,
                    ),
                    View(
                        shape=(2, 3, 256, 256),
                        strides=(114075, 38025, 195, 1),
                        offset=0,
                        mask=((0, 2), (0, 3), (0, 195), (0, 195)),
                        contiguous=False,
                    ),
                    View(
                        shape=(1, 2, 1, 3, 4, 64, 4, 64),
                        strides=(0, 196608, 0, 65536, 16384, 256, 64, 1),
                        offset=0,
                        mask=None,
                        contiguous=True,
                    ),
                ...
                UOp(
                    ...
                    src=(
                        UOp(
                            ...
                            src=(
                                UOp(
                                    Ops.DEFINE_GLOBAL, dtypes.float.ptr(), arg=1, src=()
                                ),
                                UOp(
                                    Ops.VIEW,
                                    dtypes.void,
                                    arg=(
                                        ld_st := ShapeTracker(
                                            views=(
                                                View(
                                                    shape=(2, 1, 3, 16, 62, 62, 3, 3),
                                                    strides=(0, 0, 9, 27, 0, 0, 3, 1),
                                                    offset=0,
                                                    mask=None,
                                                    contiguous=False,
                                                ),
    ...
    def test_permute_rewrite(self):
        ...
        t_np = (x.numpy() * y.numpy()).sum(axis=(0, 2)).reshape(1, 4, 1).transpose(
            0, 2, 1
        ) + z.numpy()
    ...
    def test_non_contiguous_view_simplify(self):
        ...
        a = UOp(
            Ops.LOAD,
            dtypes.char,
            (UOp.new_buffer(Device.DEFAULT, 4194304, dtypes.char), st.to_uop()),
        )
    ...
    def test_contiguous_view_simplify(self):
        ...
        a = UOp(
            Ops.LOAD,
            dtypes.char,
            (UOp.new_buffer(Device.DEFAULT, base.size, dtypes.char), base.to_uop()),
        )
...
def store_val(si: ScheduleItem):
    return si.ast.src[0].src[2]
...
def tensor_rewrite(t) -> UOp:
    return graph_rewrite(t.lazydata.base, remove_movement_ops + symbolic_simple)
...
class TestBigGraph(unittest.TestCase):
    ...
    def test_masked_const_elementwise(self):
        ...
        sink = tensor_rewrite(a)
        assert UPat(
            Ops.REDUCE_AXIS, src=(UPat.cvar().view() * UPat.cvar().view(),)
        ).match(sink, {})
    ...
    def test_const_folding_mul(self):
        a = Tensor([1])
        sink = tensor_rewrite(a * 0)
        assert UPat(Ops.CONST, arg=0).match(
            sink, {}
        ), f"expected {sink} to collapse to a const 0"
        assert sink.shape == a.shape

    def test_const_folding_ne(self):
        a = Tensor([1])
        sink = tensor_rewrite(a != a)
        assert UPat(Ops.CONST, arg=False).match(
            sink, {}
        ), f"expected {sink} to collapse to a const False"
        assert sink.shape == a.shape
    ...
    [
        ...
        (
            UPat(
                Ops.BIND,
                src=(
                    UPat(Ops.DEFINE_VAR, src=(UPat(Ops.VIEW, src=(UPat(Ops.DEVICE),)))),
                    UPat(Ops.CONST),
                ),
            ),
            lambda: True,
...
class TestConst(unittest.TestCase):
        ...
        const_ast_pattern = UPat(
            ...
            src=(
                UPat.store(
                    ...
                    UPat(
                        Ops.WHERE,
                        src=(UPat(Ops.VALID), UPat.cvar("x"), UPat(Ops.CONST, arg=0)),
                    ),
                ),
            ),
        )
    ...
    def test_unmasked_const_ast(self):
        ...
        print(sched[0].ast)
        const_ast_pattern = UPat(
            Ops.SINK, src=(UPat.store(UPat(), UPat(), UPat(Ops.CONST)),)
        )
        self.assertEqual(len(const_ast_pattern.match(sched[0].ast, {})), 1)
        run_schedule(sched)
...
class TestCopyFolding(unittest.TestCase):
    ...
    def test_alu_after_copy(self):
        ...
        assert all_same(
            [x.device for x in add.src]
        ), f"ALU has different devices! {[x.device for x in add.src]}"
    ...
    def test_permute_on_disk(self):
        with open(temp("dt_arange_4_permute"), "wb") as f:
            f.write(Tensor.arange(4).realize().lazydata.base.buffer.as_buffer())
        a = Tensor.empty(
            4, dtype=dtypes.int32, device=f"disk:{temp('dt_arange_4_permute')}"
        )
    ...
    def test_permute_after_shrink_on_disk(self):
        with open(temp("dt_arange_5_permute"), "wb") as f:
            f.write(Tensor.arange(5).realize().lazydata.base.buffer.as_buffer())
        a = Tensor.empty(
            5, dtype=dtypes.int32, device=f"disk:{temp('dt_arange_5_permute')}"
        )
        b = a.shrink(((0, 4),)).reshape(2, 2).permute(1, 0).to("CLANG")
        b.realize()
        self.assertListEqual(b.tolist(), [[0, 2], [1, 3]])
...
class TestTensorUOpSpec(unittest.TestCase):
    def test_const_must_be_unmasked(self):
        ...
        unsafe_push_views = PatternMatcher(
            [
                (
                    UPat.cvar("root").view(name="view"),
                    lambda root, view: root.replace(
                        src=tuple(x.view(view.st) for x in root.src)
                    ),
                ),
            ]
        )
        a.lazydata = graph_rewrite(
            a.lazydata.sink(), remove_movement_ops + merge_views + unsafe_push_views
        )
    ...
    def test_symbolic_shape_ok(self):
        ...
        a.lazydata = graph_rewrite(
            a.reshape(vi).sum().lazydata, remove_movement_ops + merge_views
        )
        a.schedule()
...
class TestBufferUOp(unittest.TestCase):
    ...
    def test_buffer_view_not_allowed(self):
        ...
        with self.assertRaisesRegex(
            AssertionError, "VIEW only works here if it's contiguous"
        ):
...
class TestUOpBecome(unittest.TestCase):
    ...
    def test_new_buffer_mops(self):
        ...
        assert UPat(Ops.EXPAND, src=(UPat(Ops.RESHAPE),)).match(
            b.lazydata, {}
        ), f"{b.lazydata}"
    ...
    def test_become_existing_buffer(self):
        ...
        check_schedule(b, 0)
        assert UPat(Ops.RESHAPE, src=(UPat(Ops.BUFFER))).match(
            b.lazydata, {}
        )  # scheduling backtracks to the movement op if the realized tensor becomes a view
    ...
    def test_become_const_in_base(self):
        ...
        assert UPat(Ops.CONST, arg=0).match(
            b.lazydata.base, {}
        )  # scheduling replaces the tensor lazydata with a VIEW(BUFFER)
    ...
    def test_become_multiple_choices(self):
        a = Tensor.empty(16)
        b = (a.reshape(1, 1, 4, 1, 4) + 0).reshape(1, 1, 4, 4).shrink(
            ((0, 1), (0, 1), (0, 3), (0, 3))
        ) + 0
        ...
        assert UPat(
            Ops.SHRINK,
            src=(UPat(Ops.RESHAPE, src=(UPat(Ops.RESHAPE, src=(UPat(Ops.BUFFER),)),)),),
        ).match(b.lazydata, {})
        assert UPat(
            Ops.SHRINK, src=(UPat(Ops.RESHAPE, src=(UPat(Ops.BUFFER),)),)
        ).match(c.lazydata, {})
