...
from tinygrad.helpers import CI, getenv, prod, Contex
from tinygrad.nn.state import get_parameters, get_state_dic
...
class TestMultiTensor(unittest.TestCase):
    ...
    def test_sharded_memory(self):
        ...
        for x in (d0, d1, d2, d3, d4):
            Device[x].synchronize()
        mem_base = GlobalCounters.mem_used

        X = Tensor.ones(256).contiguous().realize()
        assert GlobalCounters.mem_used - mem_base == X.dtype.itemsize * 256, (
            GlobalCounters.mem_used - mem_base
        )
        X.shard_(devices_4).realize()
        for x in (d0, d1, d2, d3, d4):
            Device[x].synchronize()
        assert GlobalCounters.mem_used - mem_base == X.dtype.itemsize * 256 * 4, (
            GlobalCounters.mem_used - mem_base
        )
    ...
    def test_simple_add(self):
        return self._test_simple_add_axis(None, None)

    def test_simple_add_X(self):
        return self._test_simple_add_axis(0, None)

    def test_simple_add_W(self):
        return self._test_simple_add_axis(None, 0)

    def test_simple_add_XW(self):
        return self._test_simple_add_axis(0, 0)
    ...
    @given(
        strat.sampled_from((4, 5)),
        strat.sampled_from((devices_2, devices_3)),
        strat.sampled_from((Ops.ADD, Ops.MUL, Ops.MAX)),
        strat.sampled_from((None, 0, 1)),
        strat.sampled_from((None, 0, 1)),
        strat.sampled_from((1, 0, -1)),
    )
    def test_simple_reduce(self, N, devices, rop, shard_axis, reduce_axis, sign):
        ...
        f = {
            Ops.ADD: lambda x: x.sum(reduce_axis),
            Ops.MUL: lambda x: x.prod(reduce_axis),
            Ops.MAX: lambda x: x.max(reduce_axis),
        }[rop]
        fX = f(X)
        fn = f(n)
    ...
    def test_copy_jit(self):
        @TinyJi
        def copy_tensor(x: Tensor):
            return x.to(f"{x.device.split(':')[0]}:1") + 1
    ...
    def test_fuzz_allreduce(self):
        ...
        for it in range(100):
            for n in range(2, 4 + 1):
                shape = tuple(
                    [
                        (n if i == 0 else 1) * random.randint(1, 10)
    ...
    def _test_matmul_shard_axis(self, shard_x, shard_w, device):
        ...
        np.testing.assert_allclose(
            X.numpy() @ W.numpy(), O.to(Device.DEFAULT).numpy(), atol=1e-5
        )
    ...
    def _test_double_matmul_shard_axis(self, shard_x, shard_w, device):
        ...
        O = (Xs @ W1s) @ W2s
        np.testing.assert_allclose(
            (X.numpy() @ W1.numpy()) @ W2.numpy(),
            O.to(Device.DEFAULT).numpy(),
            atol=1e-5,
        )

    def test_matmul_shard_none(self):
        return self._test_matmul_shard_axis(None, None, devices_2)

    def test_matmul_shard_X_0(self):
        return self._test_matmul_shard_axis(0, None, devices_2)

    def test_matmul_shard_X_1(self):
        return self._test_matmul_shard_axis(1, None, devices_2)

    def test_matmul_shard_W_0(self):
        return self._test_matmul_shard_axis(None, 0, devices_2)

    def test_matmul_shard_W_1(self):
        return self._test_matmul_shard_axis(None, 1, devices_2)

    def test_matmul_shard_0_0(self):
        return self._test_matmul_shard_axis(0, 0, devices_2)

    def test_matmul_shard_0_1(self):
        return self._test_matmul_shard_axis(0, 1, devices_2)

    def test_matmul_shard_1_0(self):
        return self._test_matmul_shard_axis(1, 0, devices_2)

    def test_matmul_shard_1_1(self):
        return self._test_matmul_shard_axis(1, 1, devices_2)

    def test_double_matmul_shard_X_0(self):
        return self._test_double_matmul_shard_axis(0, None, devices_2)

    def test_double_matmul_shard_X_1(self):
        return self._test_double_matmul_shard_axis(1, None, devices_2)

    def test_double_matmul_shard_W_0(self):
        return self._test_double_matmul_shard_axis(None, 0, devices_2)

    def test_double_matmul_shard_W_1(self):
        return self._test_double_matmul_shard_axis(None, 1, devices_2)
    ...
    def test_backprop_conv_wino(self):
        with Context(WINO=1):
            self.test_backprop_conv()

    def test_backward_sum(self):
        x = Tensor([[1.0, 2, 3, 4], [5, 6, 7, 8]]).shard(devices_2, axis=0)
        w = Tensor([1.0, 2, 3, 4], requires_grad=True).shard(devices_2)
        out = x * w
        out.mean().backward()
        tst = w.grad.numpy()
        np.testing.assert_allclose(tst, [0.75, 1.0, 1.25, 1.5])
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.long), f"long dtype not supported on {Device.DEFAULT}"
    )
    ...
    @unittest.skipIf(
        CI and Device.DEFAULT in ("CUDA", "NV", "LLVM"), "slow, and flaky on LLVM"
    )
    @unittest.skipIf(
        Device.DEFAULT == "WEBGPU", "WEBGPU can only run kernels with up to 10 buffers"
    )
    def test_data_parallel_resnet_train_step(self):
        ...
        if isinstance(fake_image_sharded, list):
            for i, shard in enumerate(fake_image_sharded):
                print(
                    f"Shard for device {devices_2[i]}: shape {shard.shape}, dtype {shard.dtype}"
                )
        else:
        ...
        if isinstance(labels_sharded, list):
            for i, shard in enumerate(labels_sharded):
                print(
                    f"Label shard for device {devices_2[i]}: shape {shard.shape}, dtype {shard.dtype}"
                )
        else:
        ...
        optimizer.zero_grad()
        m.load_from_pretrained()
        output = m(fake_image).sparse_categorical_crossentropy(
        ...
        GlobalCounters.reset()
        optimizer.zero_grad()
        shard_output = m(fake_image_sharded).sparse_categorical_crossentropy(
            labels_sharded, label_smoothing=0.1
        )
        ...
        class Attn:
            ...
            def __call__(self, xk: Tensor, start_pos: UOp):
                ...
                if not hasattr(self, "cache_k"):
                    self.cache_k = (
                        Tensor.zeros(bsz, max_context, 1, 1)
                        .shard(devices_2)
                        .contiguous()
                        .realize()
                    )
                keys = (
                    self.cache_k.shrink((None, (0, start_pos), None, None))
                    .cat(xk, dim=1)
                    .contiguous()
                    if start_pos > 0
                    else xk
    ...
    def test_multi_tensor_jit_body(self):
        ...
        for _ in range(5):
            r = jf()
            np.testing.assert_allclose(
                r.numpy(), np.ones(256) + np.ones(256), atol=1e-4, rtol=1e-5
            )
        assert len(jf.jit_cache) > 0
    ...
    def test_multi_device_jit_graph(self):
        if Device[d0].graph is None or Device[d1].graph is None:
        ...
        graph_d0 = (
            Device[d0].graph.func
            if isinstance(Device[d0].graph, functools.partial)
            else Device[d0].graph
        )
        graph_d1 = (
            Device[d1].graph.func
            if isinstance(Device[d1].graph, functools.partial)
            else Device[d1].graph
        )
    ...
    def test_uneven_shard(self):
        for N in range(1, 6):
            ...
            np.testing.assert_equal(
                X.reshape(2, 2, 257).numpy(), n.reshape((2, 2, 257))
            )
            np.testing.assert_equal(
                X.shrink(((0, 2), (0, 1), (0, 257))).numpy(), n[0:2, 0:1, 0:257]
            )
            np.testing.assert_equal(
                X.expand((4, 4, 257)).numpy(), np.tile(n, (1, 4, 1))
            )
            np.testing.assert_equal(
                X.permute((0, 2, 1)).numpy(), np.transpose(n, (0, 2, 1))
            )
    ...
    def test_uneven_shard_with_empty(self):
        ...
        np.testing.assert_equal(
            X.shard(devices, 0).reshape(8, 1, 6).numpy(), np_x.reshape(8, 1, 6)
        )
    ...
    def test_bn_ast_on_devices(self):
        ...
        for p in get_parameters(bn):
            p.shard_(devices_4).realize()

        out = bn(t)
        scheds = [
            sched
            for sched in out.schedule()
            if sched.outputs[0].device in devices_4 and sched.ast.op is not Ops.COPY
        ]
        assert set(out.device for sched in scheds for out in sched.outputs) == set(
            devices_4
        ), "should have ast on each shard device"
    ...
    def test_reshape_on_axis(self):
        ...
        # test split and rejoin to the righ
        ...
        # test split and rejoin to the right and reshape to the lef
    ...
    def test_reshape_on_axis_uneven(self):
        ...
        # split off and merge to the righ
    ...
    def test_rand_like_uneven_shard(self):
        ...
        self.assertEqual(t.lazydata.axis, t2.lazydata.axis)
        assert all(
            tlb.shape == t2lb.shape
            for tlb, t2lb in zip(t.lazydata.src, t2.lazydata.src)
        )
    ...
    def test_broadcast_const(self):
        for axis in (None, 0, 1):
            ...
            for si in t.schedule():
                ...
                assert (
                    ast.src[2].src[1].src[1].op is Ops.CONST
                    and ast.src[2].src[1].src[1].arg == 1
                )
            t = 2 * t
            for si in t.schedule():
                ast = si.ast.src[0]
                ...
                assert (
                    ast.src[2].src[0].src[1].op is Ops.CONST
                    and ast.src[2].src[0].src[1].arg == 2
                )
            ...
            for si in t.schedule():
                ...
                assert (
                    ast.src[2].src[1].src[1].op is Ops.CONST
                    and ast.src[2].src[1].src[1].arg == 3
                )
    ...
    def test_shard_memory(self):
        ...
        assert all(
            [lb is lb.base and lb.realized.base.size == 4 * 16 for lb in t.lazydata.src]
        )
    ...
    def test_multi_const_folding(self):
        ...
        self.assertEqual(
            len([x for x in sched if any(u.op is Ops.COPY for u in x.ast.toposort)]), 2
        )
        run_schedule(sched)
...
class TestHandleData(unittest.TestCase):
    def test_copied_to_device(self):
        ...
        assert len(sched) == 1
        # setup again because create_schedule has side effec
        t = Tensor([1, 2, 3, 4]).shard(device).realize()
        not_covered = t.to(d5)
...
class TestShrinkMultiTensorShardedAxis(unittest.TestCase):
    ...
    def test_ops(self, dtype):
        ...
        for i in range(4):
            ...
            # elementwise
            np.testing.assert_allclose(
                a.exp().numpy(), b.exp().numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.reciprocal().numpy(), b.reciprocal().numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.pow(-0.5).numpy(), b.pow(-0.5).numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                (a + a).numpy(), (b + b).numpy(), rtol=1e-7, atol=1e-3
            )
            ...
            np.testing.assert_allclose(
                (a.where(1, 0)).numpy(), (b.where(1, 0)).numpy(), rtol=1e-7, atol=1e-3
            )

            # reduce
            np.testing.assert_allclose(
                a.max().numpy(), b.max().numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.sum().numpy(), b.sum().numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.mean().numpy(), b.mean().numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.max(0).numpy(), b.max(0).numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.sum(0).numpy(), b.sum(0).numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.mean(0).numpy(), b.mean(0).numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.max(1).numpy(), b.max(1).numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.sum(1).numpy(), b.sum(1).numpy(), rtol=1e-7, atol=1e-3
            )
            np.testing.assert_allclose(
                a.mean(1).numpy(), b.mean(1).numpy(), rtol=1e-7, atol=1e-3
            )

            # pad it back
            np.testing.assert_allclose(
                a.pad(((2 * i, 2 * (4 - i - 1)), None)).numpy(),
                b.pad(((2 * i, 2 * (4 - i - 1)), None)).numpy(),
                rtol=1e-7,
                atol=1e-3,
            )
            ...
            # other movemen
            np.testing.assert_allclose(
                a.pad((None, (1, 1))).numpy(),
                b.pad((None, (1, 1))).numpy(),
                rtol=1e-7,
                atol=1e-3,
            )
            np.testing.assert_allclose(
                a.shrink((None, (1, 3))).numpy(),
                b.shrink((None, (1, 3))).numpy(),
                rtol=1e-7,
                atol=1e-3,
            )
            np.testing.assert_allclose(
            ...
            np.testing.assert_allclose(
                a.reshape((2, 2, 4)).numpy(),
                b.reshape((2, 2, 4)).numpy(),
                rtol=1e-7,
                atol=1e-3,
            )
            np.testing.assert_allclose(
                a.reshape((2, 1, 8)).expand((2, 5, 8)).numpy(),
                b.reshape((2, 1, 8)).expand((2, 5, 8)).numpy(),
                rtol=1e-7,
                atol=1e-3,
            )
            np.testing.assert_allclose(
                a.flip(-1).numpy(), b.flip(-1).numpy(), rtol=1e-7, atol=1e-3
            )
    ...
    def test_add_two_partitions(self):
        ...
        expected = np.concatenate(
            [np.zeros_like(t.numpy()[0:2]), na, np.zeros_like(t.numpy()[4:6]), nb]
        )
    ...
    def test_add_different_tensors(self):
        ...
        expected = (
            ...
            + np.array([[0, 0, 1, 1, 2, 2, 3, 3] for _ in range(8)]).T
        )
...
def helper_test_shard_op(shps, fxn, atol=1e-6, rtol=1e-3):
    for shp in shps:
        ...
        try:
            assert (
                single_out.shape == multi_out.shape
            ), f"shape mismatch: single={single_out.shape} | multi={multi_out.shape}"
            assert (
                single_out.dtype == multi_out.dtype
            ), f"dtype mismatch: single={single_out.dtype} | multi={multi_out.dtype}"
...
class TestTensorOps(unittest.TestCase):
    def test_interpolate(self):
        helper_test_shard_op(
            [(4, 16, 16), (4, 24, 24)], lambda x: Tensor.interpolate(x, (19, 19))
        )
