...
class TestArange(unittest.TestCase):
    ...
    def test_complexity(self, opts=None, limit=None):
        ...
        print(f"{f1=}, {f2=}")
        assert (f1 < 5000 and f2 < 5000) or (
            f2 / f1 < 15
        ), f"bad complexity, flops {f2/f1:.1f}X while inputs 10X"
    ...
    def test_complexity_w_upcast(self):
        return self.test_complexity([Opt(OptOps.UPCAST, 0, 4)], limit=1)

    def test_complexity_w_unroll2(self):
        return self.test_complexity([Opt(OptOps.UNROLL, 0, 2)], limit=1)

    def test_complexity_w_unroll4(self):
        return self.test_complexity([Opt(OptOps.UNROLL, 0, 4)], limit=1)

    def test_complexity_w_unroll8(self):
        return self.test_complexity([Opt(OptOps.UNROLL, 0, 8)], limit=1)

    def test_complexity_w_upcast_and_unroll(self):
        return self.test_complexity(
            [Opt(OptOps.UPCAST, 0, 4), Opt(OptOps.UNROLL, 0, 4)], limit=1
        )

    @unittest.skip("doesn't work yet")
    def test_complexity_w_local_and_padto(self):
        return self.test_complexity(
            [Opt(OptOps.LOCAL, 0, 16), Opt(op=OptOps.PADTO, axis=1, arg=32)]
        )
    ...
    def test_all_opts(self, opts=None, exclude=None):
        ...
        if opts is not None:
            for o in opts:
                k.apply_opt(o)
        all_opts_256 = [
            kk.applied_opts for kk in get_kernel_actions(k, include_0=False).values()
        ]
    ...
    def test_all_opts_w_unroll(self):
        return self.test_all_opts(
            [Opt(OptOps.UNROLL, 0, 4)], [Opt(op=OptOps.GROUP, axis=0, arg=0)]
        )
...
class TestIndexing(unittest.TestCase):
    ...
    def test_manual_index(self):
        ...
        with Context(NOOPT=1, FUSE_ARANGE=1):
            GlobalCounters.reset()
            rng = (
                Tensor.ones(4, 256, 16384, dtype=dtypes.int)
                ._cumalu(axis=-1, op=Ops.ADD, _include_initial=True)
                .reshape(4, 256, 16384, 1)
            )
            idxs = idxs.reshape(4, 1, 1, 1).expand(4, 256, 16384, 1)
            reshape_dataset = dataset.T.reshape(1, 256, 16384, 1).expand(
                4, 256, 16384, 1
            )
            ...
            run_schedule(sched)
            assert (
                GlobalCounters.global_ops < 4 * 16384
            ), f"too many ops {GlobalCounters.global_ops}"
    ...
    def test_index_fused(self, noopt=1):
        ...
        with Context(NOOPT=noopt, FUSE_ARANGE=1):
            ...
            assert (
                GlobalCounters.global_ops < 4 * 16384
            ), f"too many ops {GlobalCounters.global_ops} != {4*16384}"
        np.testing.assert_allclose(real_index, X.numpy())
    ...
    def test_index_mnist(self, noopt=1, op_limit=512 * 784 * 13):
        ...
        with Context(NOOPT=noopt, FUSE_ARANGE=1, SPLIT_REDUCEOP=0):
            ...
            assert (
                GlobalCounters.global_ops < op_limit
            ), f"too many ops {GlobalCounters.global_ops} != {op_limit}"
    ...
    @unittest.skipIf(
        getenv("PTX") or Device.DEFAULT == "WEBGPU",
        "broken on ptx and WebGPU for some reason",
    )
