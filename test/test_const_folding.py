    ...
    def test_cast_padded(self):
        ...
        if is_dtype_supported(dtypes.int16):
            ...
            np.testing.assert_equal(
                Tensor.ones(4).pad(((1, 1),)).cast(dtypes.int16).numpy(),
                [0, 1, 1, 1, 1, 0],
            )
        if is_dtype_supported(dtypes.uint16):
            _check_ast_count(
                1, Tensor.full(4, fill_value=-1).pad(((1, 1),)).cast(dtypes.uint16)
            )
            np.testing.assert_equal(
                Tensor.full(4, fill_value=-1)
                .pad(((1, 1),))
                .cast(dtypes.uint16)
                .numpy(),
        ...
        if is_dtype_supported(dtypes.int64):
            ...
            np.testing.assert_equal(
                Tensor.ones(4).pad(((1, 1),)).cast(dtypes.int64).numpy(),
                [0, 1, 1, 1, 1, 0],
            )
...
class TestReduceOpsConstFolding(unittest.TestCase):
    def test_const_sum(self):
        ...
        np.testing.assert_equal(
            Tensor.ones(4, 5, 6).sum(axis=0).numpy(), np.full((5, 6), 4)
        )
    ...
    def test_padded_const_sum(self):
        ...
        np.testing.assert_allclose(
            Tensor.ones(4).pad(((1, 1),)).exp().sum().numpy(), 4 * math.e + 2
        )

    def test_bool_zero_max(self):
        _check_ast_count(
            0, Tensor.full((1, 2), True).shrink(((0, 1), (0, 0))).max((1, 0))
        )
        np.testing.assert_equal(
            Tensor.full((1, 2), True).shrink(((0, 1), (0, 0))).max((1, 0)).numpy(),
            False,
        )
    ...
    def test_zero_size_ops(self):
        for reduceop in [
            lambda x: x.prod(),
            lambda x: x.sum(),
        ]:  # lambda x:x.max() NOTE: numpy gives "reduction operation maximum which has no identity"
            _check_ast_count(0, reduceop(Tensor.empty(1, 0)))
            np.testing.assert_equal(
                reduceop(Tensor.empty(shape := (1, 0))).numpy(),
                reduceop(np.empty(shape)),
            )

    def test_zero_size_ops_view(self):
        for reduceop in [lambda x: x.prod(), lambda x: x.sum()]:
            _check_ast_count(
                0, reduceop(Tensor.empty(1, 0, 4).permute((1, 2, 0)).contiguous())
            )
            np.testing.assert_equal(
                reduceop(Tensor.empty(shape := (1, 0))).numpy(),
                reduceop(np.empty((shape))),
    ...
    def test_zero_size_ops_realized(self):
        for reduceop in [lambda x: x.prod(), lambda x: x.sum()]:
            ...
            np.testing.assert_equal(
                reduceop((Tensor.randn(shape := (0, 1)) + 1).realize()).numpy(),
                reduceop(np.empty(shape)),
            )
    ...
    def test_const_prod(self):
        ...
        np.testing.assert_equal(
            Tensor.full((2, 3), fill_value=2).prod().numpy(), 2 ** (2 * 3)
        )
        _check_ast_count(0, Tensor.full((4, 5, 6), fill_value=2).prod(axis=0))
        np.testing.assert_equal(
            Tensor.full((4, 5, 6), fill_value=2).prod(axis=0).numpy(),
            np.full((5, 6), 2**4),
        )
