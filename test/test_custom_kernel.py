...
def _kernel(fxn:Callable, tensors:list[Tensor]) -> list[Tensor]:
  ...
  return [Tensor(t.uop.after(kernel).contiguous()) for t in tensors]
...
class TestCustomKernel(unittest.TestCase):
  def test_simple(self):
    ...
    c = _kernel(custom_elementwise_add_kernel, [c,a,b])[0]
  ...
  def test_multioutput(self):
    ...
    c,d = _kernel(custom_elementwise_addmul_kernel, [c,d,a,b])[:2]
    Tensor.realize(c,d)
