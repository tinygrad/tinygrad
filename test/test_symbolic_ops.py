...
class TestSymbolicOps(unittest.TestCase):
    ...
    def test_attention(self, dropout_p=0.0):
        def f(q, k, v):
            return Tensor.scaled_dot_product_attention(
                q.transpose(1, 2),
                k.transpose(1, 2),
                v.transpose(1, 2),
                dropout_p=dropout_p,
        ...
        for i in range(1, 5):
            ...
            symbolic = (
                f(q, k.reshape(2, vi, 4, 8), v.reshape(2, vi, 4, 8))
                .reshape(2, 4, 1, 8)
                .numpy()
            )
    ...
    def test_attention_pos_0_sz_0(self):
        Attention(128, 8)(
            Tensor.ones(1, 0, 128), Variable("start_pos", 0, 128).bind(0), None
        )

    def test_attention_pos_0_sz_1(self):
        Attention(128, 8)(
            Tensor.ones(1, 1, 128), Variable("start_pos", 0, 128).bind(0), None
        )

    def test_attention_pos_0_sz_2(self):
        Attention(128, 8)(
            Tensor.ones(1, 2, 128), Variable("start_pos", 0, 128).bind(0), None
        )
    ...
    def test_cat_dim0_two_vars(self):
        ...
        for i in range(1, 5):
            for j in range(1, 5):
                ...
                b = Tensor.rand(j, 3)
                symbolic = (
                    f(a.reshape(vi, 3), b.reshape(vj, 3)).reshape(i + j, 3).numpy()
                )
                expected = f(a, b).numpy()
                np.testing.assert_allclose(symbolic, expected, atol=1e-6, rtol=1e-6)
    ...
    def test_cat_dim1_two_vars(self):
        ...
        for i in range(1, 5):
            for j in range(1, 5):
                ...
                b = Tensor.rand(3, j)
                symbolic = (
                    f(a.reshape(3, vi), b.reshape(3, vj)).reshape(3, i + j).numpy()
                )
                expected = f(a, b).numpy()
                np.testing.assert_allclose(symbolic, expected, atol=1e-6, rtol=1e-6)
