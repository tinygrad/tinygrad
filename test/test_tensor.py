...
settings.register_profile(
    ...
    max_examples=200,
    deadline=None,
    derandomize=getenv("DERANDOMIZE_CI", False),
)
...
class TestTinygrad(unittest.TestCase):
    ...
    def test_accumulate_gradients(self):
        ...
        np.testing.assert_allclose(xgrad3.numpy(), xgrad.numpy() * 3.0, atol=1e-6)
        np.testing.assert_allclose(wgrad3.numpy(), wgrad.numpy() * 3.0, atol=1e-6)
        np.testing.assert_allclose(xgrad2.numpy(), xgrad.numpy() * 2.0, atol=1e-6)
        np.testing.assert_allclose(wgrad2.numpy(), wgrad.numpy() * 2.0, atol=1e-6)
    ...
    def test_second_order_backward_pass(self):
        def test_pytorch():
            ...
            first_derivative = torch.autograd.grad(
                outputs=f, inputs=x_val, create_graph=True
            )[0]
            second_derivative = torch.autograd.grad(
                outputs=first_derivative, inputs=x_val
            )[0]
            # d^2f/dx^2 = 6x = 6*2 = 12
    ...
    def test_tensor_mixed_list_tuple(self):
        def _list_or_tuple():
            return list if random.random() < 0.5 else tuple

        def _generate_data(depth):
            if depth == 0:
                return _list_or_tuple()()
            if depth == 1:
                return _list_or_tuple()([random.random(), random.random()])
            return _list_or_tuple()(
                [_generate_data(depth - 1), _generate_data(depth - 1)]
            )
    ...
    def test_tensor_list_special_values(self):
        if is_dtype_supported(dtypes.float16):
            ...
            with np.errstate(over="ignore"):
                np.testing.assert_allclose(
                    Tensor(data, dtype=dtypes.float16).numpy(),
                    np.array(data).astype(np.float16),
                )
        ...
        data = data + [-x for x in data]
        np.testing.assert_allclose(
            Tensor(data, dtype=dtypes.uint32).numpy(), np.array(data).astype(np.uint32)
        )
        ...
        data = data + [-x for x in data]
        np.testing.assert_allclose(
            Tensor(data, dtype=dtypes.int32).numpy(), np.array(data).astype(np.int32)
        )
    ...
    def test_env_overwrite_default_device(self):
        subprocess.run(
            [
                'DISK=1 python3 -c "from tinygrad import Device; assert Device.DEFAULT != \\"DISK\\""'
            ],
            shell=True,
            check=True,
        )
        subprocess.run(
            [
                'NPY=1 python3 -c "from tinygrad import Device; assert Device.DEFAULT != \\"NPY\\""'
            ],
            shell=True,
            check=True,
        )
        subprocess.run(
            [
                f'{Device.DEFAULT}=1 python3 -c "from tinygrad import Device; assert Device.DEFAULT == \\"{Device.DEFAULT}\\""'
            ],
            shell=True,
            check=True,
        )
        subprocess.run(
            [
                f'DISK=1 {Device.DEFAULT}=1 python3 -c "from tinygrad import Device; assert Device.DEFAULT == \\"{Device.DEFAULT}\\""'
            ],
            shell=True,
            check=True,
        )
        subprocess.run(
            [
                f'NPY=1 {Device.DEFAULT}=1 python3 -c "from tinygrad import Device; assert Device.DEFAULT == \\"{Device.DEFAULT}\\""'
            ],
            shell=True,
            check=True,
        )
...
class TestMoveTensor(unittest.TestCase):
    ...
    @given(
        strat.sampled_from([d0, d1]),
        strat.sampled_from([d0, d1]),
        strat.sampled_from([dtypes.float16, dtypes.float32]),
        strat.sampled_from([True, False, None]),
    )
...
class TestTensorMetadata(unittest.TestCase):
    ...
    def test_complex(self):
        ...
        self.assertEqual(
            set(m.name for m in si.metadata), {"relu", "sigmoid", "__mul__"}
        )
    ...
    def test_complex_backward(self):
        ...
        self.assertEqual(
            set(m.name for m in si.metadata), {"sigmoid", "sigmoid", "relu"}
        )
...
class TestIdxUpcast(unittest.TestCase):
    def _find_op(self, ast: UOp, op: Ops):
        if ast.op is op:
            return ast
        for src in ast.src:
            if (ret := self._find_op(src, op)) is not None:
                return ret
    ...
    def _schedule_render(self, a: Tensor):
        ...
        for s in schedule:
            if s.ast.op is Ops.SINK:
                ...
                uops = linearize_uop(
                    full_graph_rewrite(
                        rewrite_shapetracker_with_index(s.ast, renderer), renderer
                    )
                )
    ...
    def _assert(self, dtype: DType, a: Tensor):
        ...
        if (
            idx is not None
        ):  # PTX turns Ops.INDEX into pointer arithmetic earlier than cstyle, plus it's already cast to int64
    ...
    def do_op_then_assert(self, dtype: DType, dim1, dim2, dim3):
        self._assert(
            dtype, Tensor.empty(dim1, dim2, 1).expand(-1, -1, dim3).contiguous()
        )
    ...
    def test_overflow_sym(self):
        self.do_op_then_assert(
            dtypes.long, 2048, 2048, UOp.variable("dim3", 0, 2048).bind(32)
        )
    ...
    def test_regular_sym(self):
        self.do_op_then_assert(
            dtypes.int, 2048, 2048, UOp.variable("dim3", 0, 64).bind(32)
        )
    ...
    def test_overflow_kernel_run(self):
        ...
        a = (
            Tensor.empty(2**11, 2**11, 1, dtype=dtypes.int8)
            .permute((2, 0, 1))
            .expand((2**9 + 10, -1, -1))
            .contiguous()
        )
