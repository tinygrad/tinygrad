...
class TestIndexPut(unittest.TestCase):
  ...
  def test_index_put_fill(self):
    tgt = Tensor.zeros((2, 2))
    tgt = tgt.index_put(Tensor([[0, 0], [0, 1], [1, 0], [1, 1]]), Tensor([1.0, 2.0, 3.0, 4.0]))
    np.testing.assert_equal(tgt.numpy(), [[1, 2], [3, 4]])
  ...
  def test_index_put_oob(self):
    tgt = Tensor.zeros((3, 3))
    try:
      tgt = tgt.index_put(Tensor([[0, 0], [3, 1]]), Tensor([1.0, 2.0]))
      assert False
    except AssertionError as e:
      assert "indices out of bounds" in str(e)
