...
class TestLinearizerDumb(unittest.TestCase):
    ...
    def test_unmerged_ifs(self):
                    ...
                    src=(
                        ...
                        UOp(
                            ...
                            arg=ShapeTracker(
                                views=(
                                    View(
                                        shape=(64, 1, 512, 7, 7, 1, 1, 1),
                                        strides=(25088, 0, 49, 7, 1, 0, 0, 0),
                                        offset=0,
                                        mask=None,
                                        contiguous=True,
                                    ),
        ...
        opts = [
            Opt(op=OptOps.TC, axis=2, arg=(-1, 2)),
            Opt(op=OptOps.UPCAST, axis=2, arg=0),
            Opt(op=OptOps.UNROLL, axis=1, arg=0),
        ]
    ...
    def test_max_simplify_and_cancel(self):
        ...
        prg = k.to_program()
        print(prg.src)
        assert prg.uops is not None and not any(
            uop.op is Ops.MAX for uop in prg.uops
        ), "leftover MAX"
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.half), f"half dtype not supported on {Device.DEFAULT}"
    )
    ...
    def test_unaligns_idxs(self):
        ...
        opts = [
            ...
            Opt(op=OptOps.LOCAL, axis=0, arg=3),
        ]
        k = Kernel(ast, opts=Device[Device.DEFAULT].renderer)
        for opt in opts:
            k.apply_opt(opt)
        ...
        assert (
            load_idxs[0] < load_idxs[1]
        ), f"first loaded idx {load_idxs[0].arg} then {load_idxs[1].arg}!"
    ...
    def test_upcasted_stores_out_of_order(self):
                    ...
                    src=(
                        ...
                        UOp(
                            ...
                            arg=ShapeTracker(
                                views=(
                                    View(
                                        shape=(4, 5, 13, 1, 1, 1, 1, 1, 4, 3, 3),
                                        strides=(2340, 468, 36, 0, 0, 0, 0, 0, 9, 3, 1),
                                        offset=0,
                                        mask=None,
                                        contiguous=True,
                                    ),
        ...
        for i in range(len(store_idxs) - 1):
            ...
            assert (
                first_bounds < next_bounds
            ), f"first stored (max) idx {first_bounds} then {next_bounds}!"
