...
class TestLinearizerDumb(unittest.TestCase):
  ...
  def test_unmerged_ifs(self):
    ...
    assert gate_count == 14, f"must have only one gate {gate_count} != 1"
    assert len([u for u in k.uops if u.op is UOps.IF]) == 14, f"must have a single IF, got {len([u for u in k.uops if u.op is UOps.IF])}"
