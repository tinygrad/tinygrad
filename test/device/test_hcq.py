...
class TestHCQ(unittest.TestCase):
  ...
  @unittest.skipIf(Device.DEFAULT in {"CPU"} or getenv("AMD_IFACE", "").startswith("PCI"), "Can't handle async update on CPU/MOCKAM device")
  ...
  @unittest.skipUnless(MOCKGPU and not getenv("AMD_IFACE", "").startswith("PCI"), "Emulate this on MOCKGPU to check the path in CI")
