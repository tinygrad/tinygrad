...
from test.external.fuzz_shapetracker import (
    do_permute,
...
def fuzz_plus() -> tuple[ShapeTracker, ShapeTracker]:
    ...
    for _ in range(4):
        random.choice(shapetracker_ops)(m)
    backup = m.sts[0]
...
def fuzz_invert() -> tuple[ShapeTracker, ShapeTracker]:
    start = ShapeTracker.from_shape(
        (random.randint(1, 10), random.randint(1, 10), random.randint(1, 10))
    )
    m = MultiShapeTracker([start])
    for _ in range(8):
        random.choice(invertible_shapetracker_ops)(m)
...
if __name__ == "__main__":
    ...
    for fuzz in [
        globals()[f"fuzz_{x}"] for x in getenv("FUZZ", "invert,plus").split(",")
    ]:
        same_but_neq = 0
        ...
        if getenv("CHECK_NEQ"):
            print(
                f"same but unequal {same_but_neq}/{total} = {(same_but_neq/total)*100:.2f}%"
            )
