...
class TestOnnxOps(unittest.TestCase):
  ...
  def helper_build_model(self, op: str, inps: dict[str, np.ndarray], opts: dict[str, Any], outs: list[str]):
    ...
    # TODO: remove this once ORT supports 1.18.0
    opset_id = None
    if type(self).__name__ == "TestMainOnnxOps":
      opset_id = ai_onnx
    if type(self).__name__ == "TestTrainingOnnxOps":
      opset_id = ai_onnx_training
    if type(self).__name__ == "TestContribOnnxOps":
      opset_id = 1
    model = onnx.helper.make_model(
      graph, producer_name=f"test_{op.lower()}", ir_version=IR_VERSION, opset_imports=[onnx.helper.make_opsetid(self.DOMAIN, opset_id)]
    )
    return model
...
class TestMainOnnxOps(TestOnnxOps):
  ...
  def test_qlinearmatmul_2D_int8_float16(self):
    self._run_qlinearmatmul_test(np.int8, np.float16, 2)

  def test_qlinearmatmul_3D_int8_float16(self):
    self._run_qlinearmatmul_test(np.int8, np.float16, 3)

  def test_qlinearmatmul_2D_int8_float32(self):
    self._run_qlinearmatmul_test(np.int8, np.float32, 2)

  def test_qlinearmatmul_3D_int8_float32(self):
    self._run_qlinearmatmul_test(np.int8, np.float32, 3)
...
class TestContribOnnxOps(TestOnnxOps):
    ...
    for has_beta in [True, False]:
      for has_bias in [True, False]:
        with self.subTest(has_beta=has_beta, has_bias=has_bias):
          ...
          if has_beta:
            inputs["beta"] = np.random.randn(hidden_size).astype(np.float32)
          if has_bias:
            inputs["bias"] = np.random.randn(hidden_size).astype(np.float32)
  ...
  def test_bias_gelu(self):
    shape = (2, 3, 4)
    inputs = {"A": np.random.randn(*shape).astype(np.float32), "B": np.random.randn(shape[-1]).astype(np.float32)}
    attributes = {}
    outputs = ["C"]
