...
class CLCache:
    def __init__(self, allowed=None, strict=False, preclear=True, var_vals=None):
        self.allowed, self.strict, self.preclear, self.var_vals = (
            allowed,
            strict,
            preclear,
            var_vals if var_vals is not None else {},
        )
        self.count = 0
    ...
    def __exit__(self, _type, value, traceback):
        capturing.clear()
        print(
            f"cache: exiting with size {self.count}",
            f"allowed {self.allowed}" if self.allowed is not None else "",
        )
...
class TestInferenceMinKernels(unittest.TestCase):
    ...
    def test_convnext(self):
        ...
        for p in get_parameters(model):
            p.assign(np.zeros(p.shape, dtype=_to_np_dtype(p.dtype)))
    ...
    def _test_no_binop_rerun(self, f1, f2=None, allowed=1):
        ...
        if f2 is not None:
            np.testing.assert_allclose(
                c.numpy().ravel(), d.numpy().ravel(), rtol=1e-3, atol=1e-5
            )

    def test_no_binop_rerun(self):
        return self._test_no_binop_rerun(
            lambda a, b: a * b, lambda a, b: (a * b).reshape(16, 16, 1)
        )

    def test_no_binop_rerun_alt(self):
        return self._test_no_binop_rerun(
            lambda a, b: (a * b).reshape(16, 16, 1), lambda a, b: a * b
        )

    def test_no_binop_rerun_reduce_broadcast(self):
        return self._test_no_binop_rerun(
            lambda a, b: a.sum() + b, lambda a, b: a.sum().reshape(1, 1) + b, allowed=2
        )

    @unittest.skip(
        "this test started failing with the new change, based movementop issue"
    )
    def test_no_binop_rerun_transposed(self):
        return self._test_no_binop_rerun(lambda a, b: (a.T * b.T).T, lambda a, b: a * b)

    def test_no_binop_rerun_mid_reshape(self):
        return self._test_no_binop_rerun(
            lambda a, b: (a * b).reshape(256) + a.reshape(256)
        )
...
class TestOpt(unittest.TestCase):
    ...
    def test_permute_was_pushed(self):
        ...
        with CLCache(2):
            ...
            d.realize()
        np.testing.assert_allclose(
            a.numpy().sum(2).transpose(1, 0), d.numpy(), rtol=1e-3, atol=1e-5
        )
    ...
    def test_permute_was_pushed_through_contract_reshape(self):
        ...
        with CLCache(2):
            ...
            d.realize()
        np.testing.assert_allclose(
            a.numpy().sum(-1).reshape(16, 16).transpose(1, 0),
            d.numpy(),
            rtol=1e-3,
            atol=1e-5,
        )
    ...
    def test_permute_was_pushed_through_contractw1s_reshape(self):
        ...
        with CLCache(2):
            ...
            d.realize()
        np.testing.assert_allclose(
            a.numpy().sum(-1).reshape(16, 1, 16).transpose(2, 1, 0),
            d.numpy(),
            rtol=1e-3,
            atol=1e-5,
        )
    ...
    def test_permute_was_pushed_through_expand_reshape(self):
        ...
        with CLCache(2):
            ...
            d.realize()
        np.testing.assert_allclose(
            a.numpy().sum(2).transpose(1, 0).reshape(4, 4, 4, 4),
            d.numpy(),
            rtol=1e-3,
            atol=1e-5,
        )
    ...
    def test_no_reduceop_rerun(self):
        ...
        np.testing.assert_allclose(
            c.numpy().transpose(1, 0), d.numpy(), rtol=1e-3, atol=1e-5
        )
    ...
    def test_no_reduceop_rerun_alt(self):
        ...
        with CLCache() as cache:
            ...
            cache_len = cache.count
        np.testing.assert_allclose(
            c.numpy(), d.numpy().transpose(1, 0), rtol=1e-3, atol=1e-5
        )
        assert cache_len == 1, "reduceop was rerun!"
            ...
            for axis in [0, 1]:
                for n in [4, 8, 16]:
                    ...
                    with CLCache(allowed=2):
                        ...
                        a.realize()
                    np.testing.assert_allclose(
                        a.numpy(), b.numpy(), rtol=1e-3, atol=1e-5
                    )
