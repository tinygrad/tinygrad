...
def define_lars_flags():
    ...
    flags.DEFINE_float(
        "end_learning_rate", default=None, help=("Polynomial decay end learning rate.")
    )
...
class PolynomialDecayWithWarmup(learning_rate_schedule.LearningRateSchedule):
    ...
    def _get_learning_rate(self, step):
        with ops.name_scope_v2(self.name or "PolynomialDecayWithWarmup") as name:
            ...
            warmup_steps = ops.convert_to_tensor(self.warmup_steps, name="warmup_steps")
            ...
            decay_rate = tf.where(
                step <= warmup_steps, warmup_rate, poly_rate, name=name
            )
            return decay_rate
