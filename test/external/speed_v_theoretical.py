...
class TestKernelSpeed(unittest.TestCase):
    ...
    def _test_matmul(
        ...
        ops = 2 * M * N * K
        mems = (
            a.dtype.itemsize * M * K
            + b.dtype.itemsize * K * N
            + c.dtype.itemsize * M * N
        )
        tm = min(tms)
        tflops = ops / tm / 1e12
        gbs = mems / tm / 1e9
    ...
    def test_conv_3x3_256_32_32_256_256(self):
        self._test_conv_3x3(256, 32, 32, 256, 256, nv_tflops=27, amd_tflops=20)

    # theoretical is nv_tflops=165, amd_tflops=123
    def test_gemm_4096(self):
        self._test_matmul(4096, nv_tflops=115, amd_tflops=80)

    def test_gemm_8192(self):
        self._test_matmul(8192, nv_tflops=125, amd_tflops=73)

    # theoretical is nv_gbs=1008, amd_gbs=960
    def test_gemv_16384_4096(self):
        self._test_matmul(16384, 4096, 1, nv_gbs=840, amd_gbs=750)

    def test_gemv_4096_16384(self):
        self._test_matmul(4096, 16384, 1, nv_gbs=830, amd_gbs=760)
