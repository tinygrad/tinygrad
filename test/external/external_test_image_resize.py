...
def compare_resize(image:np.ndarray, size, mode):
  ...
  tiny_image = tiny_image.interpolate(size, "linear" if mode == "bilinear" else mode).permute(0,2,3,1).squeeze(0).numpy()
  # Image.resize expects (width, height).
...
if __name__ == "__main__":
  for _ in range(100):
    ...
    mode = np.random.choice(['nearest-exact', 'bilinear'])
    print(mode, new_size)
