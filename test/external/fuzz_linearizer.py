...
if getenv("VALIDATE_HCQ"):
    if Device.DEFAULT == "NV":
        ...
        on_linearizer_did_run = (
            extra.qcom_gpu_driver.opencl_ioctl.collect_last_launch_state
        )
...
def tuplize_uops(uops: list[UOp]) -> tuple:
    return tuple(
        [(x.op, x.dtype, tuple(uops.index(x) for x in x.src), x.arg) for x in uops]
    )
...
def get_fuzz_rawbufs(lin):
    ...
    rawbufs[0] = get_fuzz_rawbuf_like(
        rawbufs[0], zero=True, size=rawbufs[0].size + RED_AREA_SIZE
    )
    # setting inputs
        ...
        for rawbuf in rawbufs[1:]:
            if dtypes.is_unsigned(rawbuf.dtype):
                data = np.random.randint(
                    0, 100, size=rawbuf.size, dtype=_to_np_dtype(rawbuf.dtype)
                )
            elif dtypes.is_int(rawbuf.dtype):
                data = np.random.randint(
                    -100, 100, size=rawbuf.size, dtype=_to_np_dtype(rawbuf.dtype)
                )
            elif rawbuf.dtype == dtypes.bool:
                data = np.random.choice([True, False], size=rawbuf.size)
            elif rawbuf.dtype == dtypes.half:
                data = np.random.uniform(-1, 1, size=rawbuf.size).astype(
                    dtype=_to_np_dtype(rawbuf.dtype)
                )
            else:
                data = np.random.uniform(-10, 10, size=rawbuf.size).astype(
                    dtype=_to_np_dtype(rawbuf.dtype)
                )
            rawbuf.copyin(
                Tensor(data, device=lin.opts.device)
                .realize()
                .lazydata.base.realized.as_buffer()
            )
    return rawbufs


def get_fuzz_rawbuf_like(
    old_rawbuf, zero=False, copy=False, size=None, force_device=None
):
    rawbuf = type(old_rawbuf)(
        force_device or old_rawbuf.device,
        old_rawbuf.size if size is None else size,
        old_rawbuf.dtype,
    ).allocate()
    if copy:
        with Context(DEBUG=0):
...
def run_linearizer(
    lin: Kernel, rawbufs=None, var_vals=None
) -> tuple[str, Any]:  # (error msg, run state)
    if rawbufs is None:
        rawbufs = bufs_from_lin(lin)
    if var_vals is None:
...
def compare_linearizer(
    lin: Kernel, rawbufs=None, var_vals=None, ground_truth=None, rtol=1e-2, atol=1e-2
):
    ...
    if ground_truth is None and not has_bf16:
        ...
        ground_truth = np.frombuffer(
            rawbufs[0].as_buffer(), _to_np_dtype(rawbufs[0].dtype)
        ).copy()
        ...
        if DEBUG >= 2:
            ...
            if getenv("DEBUG_VALUES") > 0:
                mismatch_indices = np.where(
                    ~np.isclose(result, ground_truth, rtol=rtol, atol=atol)
                )
                ...
                for i, idx in enumerate(mismatch_indices[0]):
                    print(
                        f"mismatch at {idx=}: result={mismatched_result[i]} <> ground_truth={mismatched_ground_truth[i]}"
                    )
...
def fuzz_linearizer(lin: Kernel, rtol=1e-2, atol=1e-2, opts_list=None):
    ...
    seen_uops = {}
    last_lins = [lin]
    failures: defaultdict[str, list[tuple[tuple[UOp, ...], list[Opt]]]] = defaultdict(
        list
    )
    ...
    test_depth = (
        1 if opts_list is not None else getenv("DEPTH", 1 if FUZZ_ALL_ACTIONS else 10)
    )
    for depth in range(test_depth):
        ...
        for lin in last_lins:
            ...
            if depth == 0 and getenv("FUZZ_REQUIRE_TC", 0):
                tc_acts = {
                    i: k for k in actions.values() if k.applied_opts[0].op == OptOps.TC
                }
                if len(tc_acts) == 0:
                    return failures
                else:
            ...
            for test_lin in test_lins:
                ...
                try:
                    ...
                    print(e)
                    failures["LINEARIZE_ERROR"].append(
                        (test_lin.ast, test_lin.applied_opts)
                    )
                    continue
                ...
                (msg, rawbufs, var_vals, ground_truth, state1) = compare_linearizer(
                    test_lin, rawbufs, var_vals, ground_truth, rtol=rtol, atol=atol
                )
                if state1 is not None and validate_device is not None:
                    ...
                    if validate_rawbufs is None:
                        validate_rawbufs = [
                            get_fuzz_rawbuf_like(
                                x, copy=True, force_device=validate_device.device
                            )
                            for x in rawbufs
                        ]
                    ...
                    if _msg != "PASS":
                        failures[f"VALIDATE_DEV_{_msg}"].append(
                            (validate_lin.ast, validate_lin.applied_opts)
                        )

                    ok, err_msg = compare_states(state1, state2)
                    if not ok:
                        failures["HCQ_COMPARE_FAILURE"].append(
                            (
                                err_msg,
                                test_lin.ast,
                                test_lin.applied_opts,
                                state1,
                                state2,
                            )
                        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run a fuzz testing on one or more kernels",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="replay asts and opts got from beam with CAPTURE_BEAM",
    )
    parser.add_argument(
        "--logfile",
        type=str,
    ...
    if args.ast is not None:
        ...
        ast_strs, opts_list = [
            x.split(" :: ")[0] for x in fdata if not x.startswith("#")
        ], [x.split(" :: ")[1] for x in fdata if not x.startswith("#")]
    ...
    try:
        for i, ast in enumerate(ast_strs[: getenv("FUZZ_N", len(ast_strs))]):
            ...
            with Timing(f"tested ast {i}: "):
                tested += 1
                fuzz_failures = fuzz_linearizer(
                    lin,
                    rtol=args.rtol,
                    atol=args.atol,
                    opts_list=(opts_list[i] if opts_list else None),
                )
                if fuzz_failures:
                    failed_ids.append(i)
    ...
    for msg, errors in failures.items():
        for i, payload in enumerate(errors):
            print(
                f"{msg} {i} kernel: {payload}"
            )  # easier to use with output with verify_kernel.py
