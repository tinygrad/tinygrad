...
def benchmark_model(m, devices, validate_outs=False):
    ...
    input_shapes = {
        inp.name: tuple(
            x.dim_value if x.dim_value != 0 else 1
    ...
    np_inputs = {
        k: torch.randn(shp).numpy().astype(input_types[k])
    ...
    # print input names
    if DEBUG >= 2:
        print([inp.name for inp in onnx_model.graph.input if inp.name not in excluded])
    for device in devices:
        try:
            ...
            benchmark(
                m,
                f"tinygrad_{device.lower()}_jitless",
                lambda: {k: v.numpy() for k, v in tinygrad_model(inputs).items()},
            )

            from tinygrad.engine.jit import TinyJit

            tinygrad_jitted_model = TinyJit(
                lambda **kwargs: {
                    k: v.realize() for k, v in tinygrad_model(kwargs).items()
                }
            )
            for _ in range(3):
                {k: v.numpy() for k, v in tinygrad_jitted_model(**inputs).items()}
            benchmark(
                m,
                f"tinygrad_{device.lower()}_jit",
                lambda: {
                    k: v.numpy() for k, v in tinygrad_jitted_model(**inputs).items()
                },
            )  # noqa: F821
    ...
    try:
        ...
        try:
            ...
            print(f"{m:16s}torch_cpu {type(e).__name__:>25}")
        ...
        try:
            benchmark(
                m, f"torch_{torch_device}", lambda: torch_mps_model(*torch_mps_inputs)
            )
        except Exception as e:
    ...
    for backend in [
        ...
        try:
            benchmark(
                m,
                f"onnxruntime_{backend.lower()}",
                lambda: ort_sess.run(output_names, np_inputs),
            )
        except Exception as e:
            print(f"{m:16s}onnxruntime_{backend.lower()} {type(e).__name__:>25}")
        del ort_sess
    ...
    if validate_outs:
        for device in devices:
            ...
            ort_sess = ort.InferenceSession(
                str(fn), ort_options, ["CPUExecutionProvider"]
            )
            ...
            print(
                f"{m:16s}outputs validated on {device=} with rtol={rtol:.1e}, atol={atol:.1e}"
            )

    if open_csv is None:
        open_csv = csv.DictWriter(
            open("onnx_inference_speed.csv", "w", newline=""),
            fieldnames=list(CSV.keys()),
        )
    ...
    for k in tiny_out.keys():
        ...
        if tiny_v is None:
            ...
            np.testing.assert_allclose(
                ...
                rtol=rtol,
                atol=atol,
                err_msg=f"For tensor '{k}' in {tiny_out.keys()}",
            )
...
if __name__ == "__main__":
    ...
    if getenv("MODEL", "") != "":
        benchmark_model(getenv("MODEL", ""), devices, True)
    else:
        for m in MODELS:
            benchmark_model(m, devices, True)
