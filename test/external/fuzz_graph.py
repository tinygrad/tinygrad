...
def gen_prg(device, inputs_cnt):
    ...
    with Context(DEBUG=0):
        fst = [
            Tensor.randn(BUF_LEN, dtype=dtypes.int).realize() for i in range(inputs_cnt)
        ]
        s = fst[0]
...
def gen_graph():
    ...
    for i in range(kernel_count):
        ...
        if is_copy:
            deps_pool = [
                buf for buf in all_buffers[-last_n_deps:] if buf.device != target_device
            ]
            if len(deps_pool) == 0:
                deps = []
            else:
            ...
            deps_count = random.randint(
                0, min(getenv("MAX_DEPS_COUNT", 6), len(deps_pool))
            )
            if deps_count == 0:
                deps = []
            else:
...
def fuzz_graph(jis, all_buffers, input_buffers):
    ...
    ground_truth_np = [
        np.frombuffer(x, _to_np_dtype(all_buffers[i].dtype))
        for i, x in enumerate(ground_thruth_bufs)
    ]

    for _ in range(getenv("FUZZ_GRAPH_SPLIT_RUNS", 64)):
        max_split_points = len(jis) // 3
        split_points = random.randint(
            0, min(max_split_points, getenv("FUZZ_GRAPH_MAX_SPLITS", 8))
        )
        split = [0]
        for i in range(split_points - 1):
            split.append(
                random.randint(split[-1] + 2, len(jis) - 2 * (max_split_points - i))
            )
