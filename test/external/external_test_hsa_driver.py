...
class TestHSADriver(unittest.TestCase):
    def test_hsa_simple_enqueue(self):
        ...
        assert (
            test_buf.as_buffer().cast("I")[0] == 1
        ), f"{test_buf.as_buffer().cast('I')[0]} != 1, all packets executed?"
        del queue
    ...
    def test_hsa_ring_enqueue(self):
        ...
        assert (
            test_buf.as_buffer().cast("I")[0] == expected
        ), f"{test_buf.as_buffer().cast('I')[0]} != {expected}, all packets executed?"
        del queue
    ...
    def test_hsa_copies_sync(self):
        ...
        jit_cache = [
            ExecItem(BufferXfer(), [test_buf0, test_buf2]),
            ExecItem(BufferXfer(), [test_buf2, test_buf1]),
        ]
        ...
        for i in range(10000):
            test_buf0.copyin(memoryview(bytearray(1 * 4)))
            test_buf2.copyin(
                memoryview(bytearray(int.to_bytes(4, length=1 * 4, byteorder="little")))
            )
            graph([], {})
