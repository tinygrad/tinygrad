...
if __name__ == "__main__":
    ...
    for num, ast in enumerate(ast_strs):
        ...
        print(
            f"{average_tm_ptx/average_tm_cuda:5.2f}x -- {num:4d} {colorize_float(ratio)}  {min(tm_ptx)*1e6:7.2f} us",
            lin.name,
        )
        if ratio > 1.5:
            ...
            if single != -1:
                for ln, (l1, l2) in enumerate(
                    itertools.zip_longest(ll1, ll2, fillvalue="")
                ):
