...
def fuzz_uops(uops: List[UOp]) -> List[Tuple[UOp, ...]]:
    ...
    for u in uops:
        if u.op in END_FOR_UOP:
            blocks.append([u])
        elif u.op in {x[1] for x in END_FOR_UOP.values()}:
            blocks.extend([[u], []])
        else:
            blocks[-1].append(u)
    ...
    for up in itertools.product(*paths_for_block.values()):
        ...
        if len(paths) >= FUZZ_SCHEDULE_MAX_PATHS:
            break
    return list(paths)
...
class UOpsFuzzerRunner(CompiledRunner):
    def __call__(
        ...
        init_rawbufs, init_name = {
            x: x.as_buffer() for x in rawbufs
        }, self.p.function_name
        init_globals = dict(zip(self.p.globals, rawbufs))
        if DEBUG >= 1:
            print(
                colored(
                    f"fuzzing {len(fuzz_paths)} uop permutations for {init_name}",
                    "yellow",
                )
            )
        ...
        ground_truth = {
            x: np.frombuffer(x.as_buffer(), _to_np_dtype(x.dtype)) for x in rawbufs
        }
        ...
        for i, path in enumerate(fuzz_paths):
            ...
            self.p = replace(
                self.p,
                name=(name := f"{init_name}fuzz{i}"),
                src=Device[self.p.device].renderer.render(name, uops),
                uops=uops,
            )
            if DEBUG >= 4:
                print(self.p.src)
            ...
            for x in (rawbufs := [init_globals[i] for i in self.p.globals]):
                x.copyin(init_rawbufs[x])
            # verify
            super().__call__(rawbufs, var_vals, wait)
            for i, x in enumerate(rawbufs):
                try:
                    np.testing.assert_allclose(
                        np.frombuffer(x.as_buffer(), _to_np_dtype(x.dtype)),
                        ground_truth[x],
                        atol=1e-6,
                        rtol=1e-6,
                    )
