...
def test(devs: list[str], N: int, iters: int = 10):
    ...
    for i in range(-3, iters):
        ...
        if i < 0:
            continue  # warm up jit
        i_secs = GlobalCounters.time_sum_s
        i_gflops = GlobalCounters.global_ops / i_secs / 10**9
        i_gbs = (N * 4) / i_secs / 10**9
        print(
            f"{'ring_allreduce' if RING >= 2 else 'naive_allreduce'} iter {i+1}/{iters}: {i_secs:.6f} sec {i_gflops:.2f} GFLOP/s {i_gbs:.2f} GB/s"
        )
        secs += i_secs
        gflops += i_gflops
        gbs += i_gbs
...
def run(sz, n_gpus=6, iters=10, use_ring=False):
    ...
    with Context(RING=(2 if use_ring else 0), DEBUG=max(DEBUG.value, 2)):
        return test(devs, N, iters=iters)
...
def main():
    ...
    if getenv("BENCHMARK_SPLIT"):
        ...
        while r - l > 1:
            m = (l + r) // 2
            (ring_gflops, ring_gbs, ring_secs) = run(
                m * 1024 * 4, n_gpus=n_gpus, iters=100, use_ring=True
            )
            (naive_gflops, naive_gbs, naive_secs) = run(
                m * 1024 * 4, n_gpus=n_gpus, iters=100, use_ring=False
            )
            if ring_secs > naive_secs:
                l = m
            else:
                r = m
        ...
        print(
            f"Using {sz/10**9:.2f} GB of numbers on each of {n_gpus} GPUs, {n_gpus*sz/10**9:.2f} GB total."
        )
