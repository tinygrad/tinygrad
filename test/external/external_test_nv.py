...
class TestNV(unittest.TestCase):
  ...
  def test_error_on_huge_dims(self):
    ast = LazyOp(
                  ...
                  src=(
                    LazyOp(
                      ...
                      arg=MemBuffer(
                        idx=1,
                        dtype=dtypes.half,
                        st=ShapeTracker(views=(View(shape=(1, 1, 1024, 683), strides=(0, 0, 0, 1), offset=0, mask=None, contiguous=False),)),
                      ),
                    ),
                    LazyOp(
                      op=BufferOps.LOAD,
                      ...
                      arg=MemBuffer(
                        idx=2,
                        dtype=dtypes.half,
                        st=ShapeTracker(views=(View(shape=(1, 1, 1024, 683), strides=(0, 0, 683, 1), offset=0, mask=None, contiguous=True),)),
                      ),
                    ),
                  ),
                  arg=None,
      ...
      arg=MemBuffer(
        idx=0, dtype=dtypes.float, st=ShapeTracker(views=(View(shape=(1, 1, 1024, 1), strides=(0, 0, 1, 0), offset=0, mask=None, contiguous=True),))
      ),
    )  # noqa: E501
    opts = [
  ...
  def test_buf4_usage(self):
    ...
    ast = LazyOp(
            ...
            LazyOp(
              ...
              src=(
                LazyOp(
                  ...
                  src=(),
                  arg=MemBuffer(
                    idx=1, dtype=dtypes.ulong, st=ShapeTracker(views=(View(shape=(3,), strides=(1,), offset=0, mask=None, contiguous=True),))
                  ),
                ),
              ),
              arg=dtypes.float,
      ...
      arg=MemBuffer(idx=0, dtype=dtypes.float, st=ShapeTracker(views=(View(shape=(3,), strides=(1,), offset=0, mask=None, contiguous=True),))),
    )  # noqa: E501
