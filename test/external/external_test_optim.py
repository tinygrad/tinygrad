...
def step(optim, steps=1, kwargs={}, scheduler=None, schedopts=None, do_optim=True):
  ...
  if scheduler is not None:
    scheduler = scheduler(optim, **schedopts)
  lrs = []
...
def step_tf(optim, steps=1, kwargs={}, scheduler=None, schedopts=None, do_optim=True):
  ...
  for _ in range(steps):
    ...
    if callable(lr_t):
      lr_t = lr_t(math_ops.cast(optim.iterations, tf.float32))
    lrs.append(lr_t)
...
def create_tiny_lars(params, lr, skip_list=False):
  if skip_list:
    return OptimizerGroup(LARS([params[0]], lr), SGD([params[1]], lr, classic=True, weight_decay=0.0, momentum=0.9))
  return LARS(params, lr)


def create_tf_lars(lr, skip_list=False):
  return LARSOptimizer(lr, skip_list=["W"] if skip_list else None)


def create_tiny_polylr(optim, initial_lr, end_lr, train_steps, warmup, power=2, skip_list=False):
  assert power == 2
  if skip_list:
    return LRSchedulerGroup(
...
def create_tf_polylr(initial_lr, end_lr, train_steps, warmup, power=2, skip_list=False):
  assert power == 2
  return PolynomialDecayWithWarmup_tf(1, 1, train_steps, initial_learning_rate=initial_lr, end_learning_rate=end_lr, warmup_epochs=warmup)
...
class ExternalTestOptim(unittest.TestCase):
  ...
  def _test_optim(self, tinygrad_optim, tensorflow_optim, steps, opts, atol, rtol, tiny_sched=None, tf_sched=None, schedopts=None, do_optim=True):
    for x, y in zip(
      step(tinygrad_optim, steps=steps, kwargs=opts, scheduler=tiny_sched, schedopts=schedopts, do_optim=do_optim),
      step_tf(tensorflow_optim, steps=steps, kwargs=opts, scheduler=tf_sched, schedopts=schedopts, do_optim=do_optim),
    ):
      np.testing.assert_allclose(x, y, atol=atol, rtol=rtol)

  def _test_lamb(self, steps, opts, atol, rtol):
    self._test_optim(LAMB, tfa.optimizers.LAMB, steps, opts, atol, rtol)

  def _test_lars(self, steps, opts, atol, rtol):
  ...
  def _test_lars_polylr(self, steps, opts, schedopts, atol, rtol, do_optim=True):
    self._test_optim(
      ...
      atol,
      rtol,
      tiny_sched=create_tiny_polylr,
      tf_sched=create_tf_polylr,
      schedopts=schedopts,
      do_optim=do_optim,
    )

  def test_lamb(self):
    self._test_lamb(1, {"lr": 0.001}, 1e-5, 0)

  def test_lamb_high_lr(self):
    self._test_lamb(1, {"lr": 10}, 1e-5, 1e-5)

  def test_multistep_lamb(self):
    self._test_lamb(10, {"lr": 0.001}, 1e-5, 0)

  def test_multistep_lamb_high_lr(self):
    self._test_lamb(10, {"lr": 10}, 1e-5, 3e-4)

  def test_lars(self):
    self._test_lars(1, {"lr": 0.01}, 1e-5, 0)

  def test_lars_high_lr(self):
    self._test_lars(1, {"lr": 10}, 1e-5, 1e-5)

  def test_multistep_lars(self):
    self._test_lars(10, {"lr": 0.001}, 1e-5, 0)

  def test_multistep_lars_high_lr(self):
    self._test_lars(10, {"lr": 10}, 1e-5, 3e-4)

  def test_lars_skip(self):
    self._test_lars(10, {"lr": 10, "skip_list": True}, 1e-5, 3e-4)

  def test_lars_skip_high_lr(self):
    self._test_lars(1, {"lr": 10, "skip_list": True}, 1e-5, 1e-5)

  def test_lars_skip_multistep(self):
    self._test_lars(10, {"lr": 0.001, "skip_list": True}, 1e-5, 0)

  def test_lars_skip_multistep_high_lr(self):
    self._test_lars(10, {"lr": 10, "skip_list": True}, 1e-5, 3e-4)

  def test_lars_polylr(self):
    self._test_lars_polylr(10, {"lr": 1.0}, {"initial_lr": 1.0, "end_lr": 1e-4, "train_steps": 10, "warmup": 3}, 1e-5, 1e-5)

  def test_lars_polylr_large(self):
    self._test_lars_polylr(100, {"lr": 10.0}, {"initial_lr": 10.0, "end_lr": 1e-5, "train_steps": 100, "warmup": 43}, 1e-5, 1e-5, do_optim=False)

  def test_lars_polylr_skip(self):
    self._test_lars_polylr(
      10, {"lr": 1.0, "skip_list": True}, {"initial_lr": 1.0, "end_lr": 1e-4, "train_steps": 10, "warmup": 3, "skip_list": True}, 1e-5, 1e-5
    )
...
class TestCosineAnnealingLRWithWarmup(unittest.TestCase):
  ...
  def _test_lr(self, base_lr, end_lr, warmup_steps, decay_steps):
    ...
    for i in range(warmup_steps):
      expected.append((i + 1) / warmup_steps * base_lr)
    for i in range(decay_steps):
      expected.append(end_lr + (base_lr - end_lr) * (1 + math.cos((i + 1) / decay_steps * math.pi)) / 2)
    np.testing.assert_allclose(lr, expected, rtol=1e-5)

  def test_lr_0(self):
    self._test_lr(3e-4, 8e-5, 3, 5)

  def test_lr_1(self):
    self._test_lr(3e-4, 8e-5, 10, 20)

  def test_lr_llama3(self):
    self._test_lr(8e-5, 8e-7, 20, 100)
