...
def to_program(self, *args, **kwargs):
  k = Kernel(self.ast, opts=self.opts)
  for opt in self.applied_opts: k.apply_opt(opt)
  # NOTE: replay with the captured renderer, not the one in master
  return k.opts.render((p:=k.to_program(name_override=self.name)).function_name, cast(List,p.uops))
...
def diff(offset:int) -> Union[Tuple[int, int], bool]:
  ...
  for row in cur.fetchall():
    ...
    # try recreate
    try:
      fxn = to_program if isinstance(args[0], Kernel) else full_ast_rewrite
      with Context(**{k:v for k,v in ctx.items() if k in ContextVar._cache and k != "DEBUG"}): good = fxn(*args, **kwargs)
    except Exception as e:
      logging.warning(f"FAILED TO RECREATE KERNEL {e}")
      for x in args: logging.info(x)
      if len(kwargs) != 0: logging.info(kwargs)
      if ASSERT_DIFF: return True
      continue
    # diff
    try:
      # NOTE: DEFINE_ACC arg depends on acc_number so we only diff the final src and not the uops
      if isinstance(args[0], Kernel): compare, good = compare.src, good.src
      assert compare == good
    except AssertionError:
      logging.info("PROCESS REPLAY DETECTED CHANGE")
      if isinstance(args[0], Kernel):
        # TODO: should Kernel have __repr__?
        logging.info(args[0].ast)
        logging.info(args[0].applied_opts)
      else: logging.info(args)
      if len(kwargs) != 0: logging.info(kwargs)
      print_diff(good, compare)
      changes = list(difflib.unified_diff(str(good).splitlines(), str(compare).splitlines()))
      ...
      if changed > MAX_DIFF_PCT:
        logging.warning(f"detected changes in over {MAX_DIFF_PCT}%. skipping further diff generation.")
        early_stop.set()
        break
  conn.commit()
  cur.close()
...
def main(maxtasksperchild:int=16) -> None:
  conn = db_connection()
  cur = conn.cursor()
  ...
  with multiprocessing.get_context("spawn").Pool(multiprocessing.cpu_count(), maxtasksperchild=maxtasksperchild) as pool:
    ...
    ret: List[Union[bool, Tuple[int, int]]] = list(tqdm(pool.imap_unordered(diff, inputs), total=len(inputs)))
    pool.close()
    pool.join()
    pool.terminate()
