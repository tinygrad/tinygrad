...
def diff(offset:int, fxns:dict[str, Callable[..., tuple|None]]) -> None:
  ...
  for row in cur.fetchall():
    ...
    try:
      ...
      ctx_vars = {k:v.value for k,v in ctx_vals.items() if k not in ("DEBUG", "CAPTURE_PROCESS_REPLAY") and (var:=ContextVar._cache.get(k)) is not None and var.value != v.value}
