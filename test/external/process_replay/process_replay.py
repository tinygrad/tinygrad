...
import os, multiprocessing, logging, pickle, sqlite3, difflib, warnings, itertools, base64, codecs
...
def replay_get_program(src:str, name:str, ast:UOp, renderer:Renderer, opts:list[Opt]|None=None) -> tuple[str, str, tuple[Any, ...]]:
  sink_arg = ast.arg or KernelInfo(opts_to_apply=None if opts is None else tuple(opts))
  input_ast = ast.replace(arg=replace(sink_arg, name=name))
...
def diff(offset:int) -> None:
  ...
  for row in cur.fetchall():
    ...
    name, loc = "", ""
    try:
      ast, name, renderer, opts, src, ctx_vals = pickle.loads(row[0])
      ...
      with Context(**ctx_vars):
        ret = replay_get_program(src, name, ast, renderer, opts)
...
def _pmap() -> None:
  ...
  with multiprocessing.get_context("spawn").Pool(multiprocessing.cpu_count()) as pool:
    ...
    for _ in pool.imap_unordered(diff, range(0, row_count, s:=min(PAGE_SIZE, row_count))): bar.update(s)
    pool.close()
    pool.join()
    pool.terminate()
