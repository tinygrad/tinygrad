...
def trunc_log(x):
    if len(lines := repr(x).splitlines()) > MAX_LINES:
...
# user config
ASSERT_DIFF = int(
    (flag := "[pr]") in os.getenv("COMMIT_MESSAGE", flag)
    or flag in os.getenv("PR_TITLE", flag)
)
if not getenv("ASSERT_PROCESS_REPLAY", 1):
    ASSERT_DIFF = 0
SKIP_PROCESS_REPLAY = (k := "[skip_process_replay]") in os.getenv(
    "COMMIT_MESSAGE", ""
) or k in os.getenv("PR_TITLE", "")
if REF == "master":
    SKIP_PROCESS_REPLAY = True


class ProcessReplayWarning(Warning):
    pass
...
def recreate_kernel(
    ast: UOp, opts: Renderer, applied_opts: list[Opt], name: str
) -> str:
...
def diff(offset: int, name: str, fxn: Callable) -> None:
    ...
    if ASSERT_DIFF and name != "schedule":
    ...
    cur = conn.cursor()
    cur.execute(
        f"SELECT val FROM '{name}_{TABLE_NAME}' LIMIT ? OFFSET ?", (PAGE_SIZE, offset)
    )
    changed = 0
    for row in cur.fetchall():
        if changed > MAX_DIFF_PCT:
            warnings.warn(
                f"detected changes in over {MAX_DIFF_PCT}% of {name}s. skipping further diff generation."
            )
            early_stop.set()
            break
        # try unpickle
        try:
            args = pickle.loads(row[0])
        ...
        try:
            ctx_vars = {
                ...
                if k != "DEBUG"
                and (var := ContextVar._cache.get(k)) is not None
                and var.value != v.value
            }
            ...
            if ctx_vars:
                logging.info(ctx_vars)
            for x in args[:-2]:
                trunc_log(x)
            continue
        # diff kernels
        try:
            assert str(args[-1]) == str(good)
            ...
            for x in args[:-2]:
                trunc_log(x)
            changes = list(
                difflib.unified_diff(str(good).splitlines(), str(args[-1]).splitlines())
            )
            logging.info(
                "\n".join(
                    colored(
                        ...
                        (
                            "red"
                            if line.startswith("-")
                            else "green" if line.startswith("+") else None
                        ),
                    )
                    for line in changes
                )
...
def _pmap(name: str, fxn: Callable, maxtasksperchild: int = 16) -> None:
    ...
    try:
        row_count = cur.execute(
            f"select count(*) from '{name}_{TABLE_NAME}'"
        ).fetchone()[0]
    except sqlite3.OperationalError:
        warnings.warn(
            f"{name}_{TABLE_NAME} isn't accessible in master, did DB_VERSION change?",
            ProcessReplayWarning,
        )
        return None
    conn.commit()
    cur.close()
    with multiprocessing.get_context("spawn").Pool(
        multiprocessing.cpu_count(), maxtasksperchild=maxtasksperchild
    ) as pool:
        inputs = list(range(0, row_count, PAGE_SIZE))
        list(
            tqdm(
                pool.imap_unordered(
                    functools.partial(diff, name=name, fxn=fxn), inputs
                ),
                total=len(inputs),
            )
        )
