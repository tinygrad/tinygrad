...
def print_si_diff(si0:ScheduleItem, si1:ScheduleItem):
  ...
  execs0 = lower_schedule_item(si0)
  execs1 = lower_schedule_item(si1)
  for ei0, ei1 in zip(execs0, execs1):
    assert isinstance(ei0.prg, CompiledRunner)
    assert isinstance(ei1.prg, CompiledRunner)
