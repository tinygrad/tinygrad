...
OPENPILOT_MODEL = (
    sys.argv[1]
    if len(sys.argv) > 1
    else "https://github.com/commaai/openpilot/raw/v0.9.4/selfdrive/modeld/models/supercombo.onnx"
)
...
if __name__ == "__main__":
    ...
    input_types = {
        inp.name: tensor_dtype_to_np_dtype(inp.type.tensor_type.elem_type)
        for inp in onnx_model.graph.input
    }
    new_inputs = {
        k: Tensor.randn(*shp, dtype=_from_np_dtype(input_types[k])).mul(8).realize()
    ...
    for _ in range(5):
        ...
        ret = (
            next(iter(run_onnx(new_inputs_junk).values())).cast(dtypes.float32).numpy()
        )
    ...
    for _ in range(20):
        ...
        inputs = {
            **{k: v for k, v in new_inputs_junk.items() if "img" in k},
            **{
    ...
    suffix = ""
    if IMAGE.value < 2:
        suffix += f"_image{IMAGE.value}"  # image=2 has no suffix for compatibility
    if getenv("FLOAT16") == 1:
        suffix += "_float16"
    path = (
        Path(__file__).parent
        / "openpilot"
        / f"{hashlib.md5(OPENPILOT_MODEL.encode()).hexdigest()}{suffix}.npy"
    )

    # validate if we have records
    tinygrad_out = (
        next(iter(run_onnx_jit(**new_inputs).values())).cast(dtypes.float32).numpy()
    )
    if getenv("SAVE_OUTPUT"):
        ...
        for _ in trange(1000):
            ret = (
                next(iter(run_onnx_jit(**new_inputs).values()))
                .cast(dtypes.float32)
                .numpy()
            )
