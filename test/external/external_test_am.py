import unittest
from tinygrad.runtime.support.am.amdev import (
    AMMemoryManager,
    AMPageTableTraverseContext,
)
...
class FakeGMC:
    ...
    def flush_tlb(self, *args, **kwargs):
        pass


class FakePCIDev:
    def __init__(self):
        self.regions = [(0xC12300000000, 0xC12400000000, 0x0)]
...
class FakeAM:
    ...
    def paddr2cpu(self, paddr: int) -> int:
        return paddr + mv_address(self.vram)
...
def helper_read_entry_components(entry_val):
    return {
        "paddr": entry_val & 0x0000FFFFFFFFF000,
        "fragment": (entry_val >> 7) & 0x1F,
        "valid": entry_val & 0x1,
        "read": (entry_val >> 5) & 0x1,
        "write": (entry_val >> 6) & 0x1,
        "exec": (entry_val >> 4) & 0x1,
        "mtype": (entry_val >> 48) & 0x7,
        "T": (entry_val >> 51) & 0x1,
        "L": (entry_val >> 55) & 0x1,
        "F": (entry_val >> 56) & 0x1,
    }
...
class TestAMPageTable(unittest.TestCase):
    ...
    def test_page_table_walkers(self):
        ...
        for va, sz in [
            ...
            assert (
                total_covered == sz
            ), f"Expected total coverage {total_covered} to be {sz}"
            ...
            for tup in results:
                ...
                for i in range(_n_ptes):
                    ...
                    assert (
                        pte["paddr"] == va + _offset + i * _pte_covers
                    ), f"Expected paddr {pte['paddr']:#x} to be {va + _offset + i * _pte_covers:#x}"
                    assert pte["valid"] == 1
    ...
    def test_double_map(self):
        ...
        for va, sz in [(0x10000, 0x3000), (0x1000000, 0x1000000), (0x12000, 0x4000)]:
            ...
            with self.assertRaises(AssertionError):
                mm0.map_range(
                    vaddr=exteranl_va + 0x1000, size=0x1000, paddrs=[(va, sz)]
                )

            with self.assertRaises(AssertionError):
                mm0.map_range(
                    vaddr=exteranl_va + 0x2000, size=0x100000, paddrs=[(va, sz)]
                )
            ...
            mm0.map_range(
                vaddr=exteranl_va + 0x2000,
                size=0x100000,
                paddrs=[(0xDEAD0000, 0x1000), (0xDEAD1000, 0xFF000)],
            )

            ctx = AMPageTableTraverseContext(
                self.d[0], mm0.root_page_table, exteranl_va + 0x2000
            )
            for tup in ctx.next(0x100000):
                ...
                for i in range(_n_ptes):
                    pte = helper_read_entry_components(_pt.entries[_pte_idx + i])
                    assert (
                        pte["paddr"] == 0xDEAD0000 + _offset + i * _pte_covers
                    ), f"paddr {pte['paddr']:#x} not {0xdead0000 + _offset + i * _pte_covers:#x}"
                    assert pte["valid"] == 1
    ...
    def test_free_pt(self):
        ...
        for off in [0, 0x3000, 0x10000]:
            mm0.map_range(
                0x1000000 + off,
                (2 << 20) - off,
                paddrs=[(0x10000, 0x1000)] * (512 - off // 0x1000),
            )
