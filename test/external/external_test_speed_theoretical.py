...
def _test(tcount, fxn, dtype=dtypes.float):
    ...
    for sz in range(SZMIN, SZMAX + 1):
        jfxn = TinyJit(fxn)
        ts = [
            Tensor.zeros((2**sz) * 1024 * 1024, dtype=dtype).contiguous().realize()
        ...
        print(
            f"{ts[0].nbytes()/(1024*1024):10.0f} MB, {min(tms)*1e3:6.2f} ms {gbs:10.2f} GB/s {gflops:10.2f} GFLOPS {str(ts[0].shape):20s}"
        )
        allgbs.append(gbs)
    return max(allgbs)
...
class TestRamBandwidth(unittest.TestCase):
    def test_add(self):
        self.assertGreater(_test(2, Tensor.add), MEMBW)

    def test_exp(self):
        self.assertGreater(_test(1, Tensor.exp), MEMBW)

    def test_sum(self):
        self.assertGreater(_test(1, Tensor.sum), MEMBW)
...
class TestFlops(unittest.TestCase):
    def test_flops_int8(self):
        _test(1, flopsmax, dtypes.int8)

    def test_flops_fp16(self):
        _test(1, flopsmax, dtypes.half)

    def test_flops_fp32(self):
        _test(1, flopsmax)
