...
if __name__ == "__main__":
    ...
    for num, ast in enumerate(ast_strs):
        ...
        if not failed and not has_bf16:
            curesult = np.frombuffer(
                test_cubufs[0].as_buffer(), _to_np_dtype(test_cubufs[0].dtype)
            )
            nvresult = np.frombuffer(
                test_nvbufs[0].as_buffer(), _to_np_dtype(test_nvbufs[0].dtype)
            )
        ...
        print(
            f"{average_tm_nv/average_tm_cuda:5.2f}x -- {num:4d} {colorize_float(ratio)}  {min(tm_nv)*1e6:7.2f} us",
            nvlin.name,
        )
        if ratio > 1.04:
