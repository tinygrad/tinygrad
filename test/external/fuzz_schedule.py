...
def fuzz_schedule(outs: List[UOp]):
    ...
    for combination in itertools.product(*ctx_vars.values()):
        for var, val in zip(ctx_vars, combination):
            var.value = val
        ...
        for ts in find_all_toposorts(graph, in_degree):
            unique_ts[ts] = ctx_var_values
    toposorts = list(unique_ts.items())
    if DEBUG >= 1:
        print(colored(f"fuzzing {len(toposorts)} schedule permutations", "yellow"))
    ...
    for lsi in ts:
        ...
        for x in lsi.inputs:
            if x not in ground_truth and x.device != "NPY":
                prerealized[x] = x.buffer.as_buffer()
        si = ScheduleItem(
            lsi.ast,
            tuple(x.buffer for x in lsi.outputs + lsi.inputs if x.size != 0),
            lsi.metadata,
        )
        _exec_si(si, seed)
        for out in lsi.outputs:
    ...
    for i, (ts, ctx) in enumerate(toposorts[1:]):
        if DEBUG >= 1:
            print(colored(f"testing permutation {i} {ctx}", "yellow"))
        rawbufs: Dict[UOp, Buffer] = {}
        for lsi in ts:
            for out in lsi.outputs:
                base = (
                    rawbufs[lsi.inputs[0]].base if out.op is Ops.BUFFER_VIEW else None
                )
                rawbufs[out] = Buffer(
                    out.buffer.device, out.buffer.size, out.buffer.dtype, base=base
                )
                if out.op is Ops.ASSIGN:
                    rawbufs[out].ensure_allocated().copyin(prerealized[out])
            for x in lsi.inputs:
                if x not in rawbufs:
                    # override the assign_target after ASSIGN
                    if x in assign_targets and assign_targets[x] in rawbufs:
                        rawbufs[x] = rawbufs[assign_targets[x]]
                    elif x.device == "NPY":
                        rawbufs[x] = x.buffer
                    # copy the pre realized input
                        ...
                        rawbufs[x] = Buffer(
                            ...
                            x.buffer.size,
                            x.buffer.dtype,
                            initial_value=bytes(prerealized[x]),
                        )
            si = ScheduleItem(
                lsi.ast,
                tuple(rawbufs[x] for x in lsi.bufs if x.size != 0),
                lsi.metadata,
            )
            _exec_si(si, seed)
            for out in lsi.outputs:
                outbuf = np.frombuffer(
                    rawbufs[out].as_buffer(), _to_np_dtype(out.dtype)
                )
                try:
                    np.testing.assert_allclose(
                        outbuf,
                        np.frombuffer(ground_truth[out], _to_np_dtype(out.dtype)),
                        atol=1e-2,
                        rtol=1e-2,
                    )
...
def find_all_toposorts(
    graph: DefaultDict[T, List[T]], in_degree: Union[DefaultDict[T, int], Dict[T, int]]
) -> List[Tuple[T, ...]]:
    visited: Set[T] = set()
    ret: List[Tuple[T, ...]] = []
    path: List[T] = []
    ...
    def recurse_paths(path: List[T]):
        for v, d in in_degree.items():
            ...
            visited.add(v)
            recurse_paths(path)
            if len(ret) >= FUZZ_SCHEDULE_MAX_PATHS:
                return
            # backtrack
            for u in graph[v]:
                in_degree[u] += 1
    ...
    if len(ret) == 0:
        raise RuntimeError("detected cycle in the graph")
