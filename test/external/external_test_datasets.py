...
class ExternalTestDatasets(unittest.TestCase):
    ...
    def _create_samples(self, val, num_samples=2):
        ...
        img, lbl = np.random.rand(190, 392, 392).astype(np.float32), np.random.randint(
            0, 100, size=(190, 392, 392)
        ).astype(np.uint8)
        ...
        for i in range(num_samples):
            ...
            preproc_img, preproc_lbl = preprocess(
                Path(tempfile.gettempdir()) / f"case_000{i}"
            )
            preproc_img_pth, preproc_lbl_pth = temp(
                f"{dataset}/case_000{i}_x.npy"
            ), temp(f"{dataset}/case_000{i}_y.npy")
        ...
        return (
            preproc_pth,
            list(preproc_pth.glob("*_x.npy")),
            list(preproc_pth.glob("*_y.npy")),
        )
    ...
    def _create_kits19_tinygrad_dataloader(
        self,
        preproc_pth,
        val,
        batch_size=1,
        ...
        if use_old_dataloader:
            dataset = iterate(
                list(Path(tempfile.gettempdir()).glob("case_*")),
                preprocessed_dir=preproc_pth,
                val=val,
                shuffle=shuffle,
                bs=batch_size,
            )
    ...
    def test_kits19_training_set(self):
        ...
        ref_dataset = self._create_kits19_ref_dataloader(
            preproc_img_pths, preproc_lbl_pths, False
        )
    ...
    def test_kits19_validation_set(self):
        ...
        ref_dataset = self._create_kits19_ref_dataloader(
            preproc_img_pths, preproc_lbl_pths, True
        )
        tinygrad_dataset = self._create_kits19_tinygrad_dataloader(
            preproc_pth, True, use_old_dataloader=True
        )
