...
def _resize_image_and_masks(
  ...
  image = torch.nn.functional.interpolate(
    image[None], size=image_size, scale_factor=None, mode="bilinear", recompute_scale_factor=None, align_corners=False
  )[0]
  ...
  if "masks" in target:
    mask = target["masks"]
    mask = torch.nn.functional.interpolate(mask[:, None].float(), size=image_size, scale_factor=None, recompute_scale_factor=None)[:, 0].byte()
    target["masks"] = mask
  return image, target
...
class GeneralizedRCNNTransform(nn.Module):
  ...
  def postprocess(
    self, result: List[Dict[str, Tensor]], image_shapes: List[Tuple[int, int]], original_image_sizes: List[Tuple[int, int]]
  ...
  def __repr__(self) -> str:
    ...
    format_string += "{0}Resize(height={1}, width={2}, mode='bilinear')".format(_indent, self.image_size[0], self.image_size[1])
    format_string += "\n)"
