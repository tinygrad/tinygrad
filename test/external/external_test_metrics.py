...
class ExternalTestMetrics(unittest.TestCase):
  ...
  def test_log_perplexity(self):
    ...
    np.testing.assert_allclose(log_perplexity(Tensor([[[10000.0, 0, 0, 0]]]), Tensor([[0]])).numpy(), 0)
    ...
    for i in range(x.numel() - 1):
      self.assertLess(log_perplexity(x, Tensor([[i]])).item(), log_perplexity(x, Tensor([[i + 1]])).item())
    # torch eval examples
    np.testing.assert_allclose(
      log_perplexity(Tensor([[[0.3659, 0.7025, 0.3104], [0.0097, 0.6577, 0.1947]]]), Tensor([[2, 1]])).exp().numpy(), 2.7593, rtol=1e-5
    )
    np.testing.assert_allclose(
      log_perplexity(
        Tensor([
          [[0.3, 0.7, 0.3, 0.1], [0.5, 0.4, 0.1, 0.4], [0.1, 0.1, 0.2, 0.5]],
          [[0.1, 0.6, 0.1, 0.5], [0.3, 0.7, 0.3, 0.4], [0.3, 0.7, 0.3, 0.4]],
        ]),
        Tensor([[2, 1, 3], [1, 0, 1]]),
      )
      .exp()
