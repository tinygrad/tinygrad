...
if __name__ == "__main__":
    ...
    for i in range(10):
        assert os.path.exists(
            os.path.join(BASEDIR, "eval", f"{i}.pkl")
    ...
    assert all(
        os.path.exists(os.path.join(INIT_CKPT_DIR, f)) for f in required_files
    ...
    assert (
        total_lm_accuracy >= 0.34
    ), "Checkpoint loaded incorrectly. Accuracy should be very close to 0.34085 as per MLPerf BERT README."
    print(
        f"Checkpoint loaded correctly. Accuracy of {total_lm_accuracy*100:.3f}% achieved. (Reference: 34.085%)"
    )
