...
class BenchmarkBertTrain(unittest.TestCase):
  ...
  def _test_layer(self, name, layer, input_shapes):
    ...
    with Context(TRACK_MATCH_STATS=0):
      Tensor.realize(*[t.assign(t.detach().contiguous()) for t in get_parameters(optim)])
    ...
    def step(inputs):
      ...
      y.sum().backward()
      if getenv("ASSIGN", 1):
        sched, _ = Tensor.schedule_with_vars(y, *list(inputs), *optim.schedule_step())
      else:
        sched, _ = Tensor.schedule_with_vars(y, *list(inputs), *[t.grad for t in optim.params])
    ...
    for _ in range(CNT):
      with Context(TRACK_MATCH_STATS=0):
        inputs = [Tensor.randn(*shape, requires_grad=False).realize() for shape in input_shapes]
  ...
  def test_attention_self_layer(self):
    self._est(*self._test_layer(*self._get_layer("attention_self")), 24)  # Assumes BERT-large
