...
#    Command: python3 python3 create_pretraining_data.py --input_file=/path/to/part-00XXX-of-00500 --vocab_file=/path/to/vocab.txt --output_file=/path/to/output.tfrecord
...
def load_dataset(file_path, max_seq_length=512, max_predictions_per_seq=76):
  ...
  parse_function = lambda proto: _parse_function(proto, max_seq_length, max_predictions_per_seq)
  return dataset.map(parse_function)

if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Verify the correctness of the preprocessing script for specific part", formatter_class=argparse.ArgumentDefaultsHelpFormatter)    # noqa: E501
  parser.add_argument("--preprocessed_part_dir", type=str, default=None, help="Path to dir with preprocessed samples from `wikipedia.py`")
  parser.add_argument("--tf_records", type=str, default=None, help="Path to TFRecords file from `create_pretraining_data.py` (Reference implementation)")
  ...
  for file_name in sorted(os.listdir(args.preprocessed_part_dir), key=lambda x: int(x.split("_")[1].split(".")[0])): # 0_3.pkl -> 3
  ...
  for i, (reference_example, preprocessed_sample) in tqdm(enumerate(zip(dataset, preprocessed_samples)), desc="Checking samples", total=len(preprocessed_samples)):
    ...
    for key in feature_keys:
        ...
        assert (reference_example_feature == preprocessed_sample[key]).all(), f"{key} are not equal at index {i}"
