...
def load_dataset(file_path, max_seq_length=512, max_predictions_per_seq=76):
    ...
    parse_function = lambda proto: _parse_function(
        proto, max_seq_length, max_predictions_per_seq
    )  # noqa: E731
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Verify the correctness of the preprocessing script for specific part",
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Max sequence length. For MLPerf keep it at 512",
    )
    parser.add_argument(
        "--max_predictions_per_seq",
        type=int,
        default=76,
        help="Max predictions per sequence. For MLPerf keep it at 76",
    )
    parser.add_argument(
        "--is_eval",
        type=bool,
        default=False,
        help="Whether to run eval or train preprocessing",
    )
    args = parser.parse_args()

    assert os.path.isfile(
        args.preprocessed_part
    ), f"The specified file {args.preprocessed_part} does not exist."
    assert os.path.isfile(
        args.tf_records
    ), f"The specified TFRecords file {args.tf_records} does not exist."

    with open(args.preprocessed_part, "rb") as f:
        preprocessed_samples = pickle.load(f)

    dataset = load_dataset(
        args.tf_records, args.max_seq_length, args.max_predictions_per_seq
    )
    tf_record_count = sum(1 for _ in dataset)
    assert tf_record_count == len(
        preprocessed_samples
    ), f"Samples in reference: {tf_record_count} != Preprocessed samples: {len(preprocessed_samples)}"
    print(f"Total samples in the part: {tf_record_count}")

    for i, (reference_example, preprocessed_sample) in tqdm(
        enumerate(zip(dataset, preprocessed_samples)),
        desc="Checking samples",
        total=len(preprocessed_samples),
    ):  # noqa: E501
        feature_keys = [
            "input_ids",
            "input_mask",
        ...
        for key in feature_keys:
            ...
            assert (
                reference_example_feature == preprocessed_sample[key]
            ).all(), f"{key} are not equal at index {i}\nReference: {reference_example_feature}\nPreprocessed: {preprocessed_sample[key]}"
