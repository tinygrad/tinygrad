...
class BenchmarkResnetTrain(unittest.TestCase):
    def _get_layer(self, layer_i, slice_i):
        ...
        xy = 112 >> layer_i
        xy >>= (
            1 if slice_i > 0 or layer_i == 0 else 0
        )  # layer 1 is preceded by maxpool2d
        name = f"layer{layer_i+1} slice{slice_i+1}"

        # get specific conv
        if conv:
            convs = [layer.conv1, layer.conv2, layer.conv3] + (
                [layer.downsample[0]] if layer.downsample else []
            )
            bns = [layer.bn1, layer.bn2, layer.bn3] + (
                [layer.downsample[1]] if layer.downsample else []
            )
            f = [convs[conv - 1]]
            if bn:
                f.append(bns[conv - 1])
            ...
            if conv == 3:
                xy //= convs[1].stride
            return (
                f"{name} conv{conv} x{str((bs, cin, xy, xy)):20s} k{str(f[0].weight.shape):20s}"
                + (" bn" if bn else ""),
                f,
                cin,
                xy,
    ...
    def _test_layer(self, name, layer, cin, xy):
        optim = SGD(
            get_parameters(layer), bs / 128 * 1.0
        )  # need sgd for some params but not consequential for benchmarking
        with Context(TRACK_MATCH_STATS=0):
            Tensor.realize(
                *[t.assign(t.detach().contiguous()) for t in get_parameters(optim)]
            )
        ...
        def step(x):
            ...
            if getenv("ASSIGN", 1):
                ...
                sched, _ = Tensor.schedule_with_vars(
                    y, x.grad, *[t.grad for t in optim.params]
                )
        ...
        for i in range(CNT):
            ...
            if best_tm is None or tm < best_tm:
                best_tm = tm
        print(
            f"\r{name:38s}: {best_tm * 1000:>9.2f} ms, {flops / 10**12 / best_tm:>6.2f} tflops, {mem / 10**9 / best_tm:>5.0f} GB/s, "
            f"{mem_used / 10**9: 6.2f} GB used, {kernels:>5d} kernels"
        )
        return best_tm, flops, mem, kernels

    def test_layer1_1(self):
        self._est(*self._test_layer(*self._get_layer(0, 0)), 1)

    def test_layer1_2(self):
        self._est(*self._test_layer(*self._get_layer(0, 1)), 2)

    def test_layer2_1(self):
        self._est(*self._test_layer(*self._get_layer(1, 0)), 1)

    def test_layer2_2(self):
        self._est(*self._test_layer(*self._get_layer(1, 1)), 3)

    def test_layer3_1(self):
        self._est(*self._test_layer(*self._get_layer(2, 0)), 1)

    def test_layer3_2(self):
        self._est(*self._test_layer(*self._get_layer(2, 1)), 5)

    def test_layer4_1(self):
        self._est(*self._test_layer(*self._get_layer(3, 0)), 1)

    def test_layer4_2(self):
        self._est(*self._test_layer(*self._get_layer(3, 1)), 2)
    ...
    def tearDownClass(cls):
        print(
            f"\restimated step tm: {cls.est_tm * 1000.0:.2f} ms, {cls.est_flops / 10 ** 12 / cls.est_tm:.3f} tflops, "
