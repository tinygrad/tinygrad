...
class AMPTFuzzer:
        ...
        for _offset, _pt, _pte_idx, _n_ptes, _pte_covers in pages:
            ...
            for i in range(_n_ptes):
                ...
                ctx = AMPageTableTraverseContext(
                    self.d, self.d.mm.root_page_table, start_vaddr
                )
                frags_l = list(ctx.next(contig_range))
                for f_offset, f_pt, f_pte_idx, f_n_ptes, f_pte_covers in frags_l:
                    for j in range(f_n_ptes):
                        f_pte = helper_read_entry_components(
                            f_pt.entries[f_pte_idx + j]
                        )
                        assert f_pte["valid"] == 1
                        assert (
                            f_pte["paddr"] == start_paddr + f_offset + j * f_pte_covers
                        ), f"paddr {f_pte['paddr']:#x} not {start_paddr+f_offset+j*f_pte_covers:#x}"
    ...
    def verify_memory(self, pages, pattern: int) -> bool:
        for _offset, _pt, _pte_idx, _n_ptes, _pte_covers in pages:
            for i in range(_n_ptes):
                pte = helper_read_entry_components(_pt.entries[_pte_idx + i])
                if self.d.vram[pte["paddr"]] != pattern:
                    return False
                if pte["valid"] == 0:
                    return False
    ...
    def random_alloc(self):
        ...
        size = random.randint(
            self.min_alloc_size,
            min(self.max_alloc_size, self.total_size - self.alloc_payload),
        )
        size = round_up(size, (2 << 20) if size > (4 << 20) else (4 << 10))

        try:
            ptr = self.d.mm.valloc(size)
        except MemoryError:
            print(
                f"Failed to allocate {size} bytes. Payload size is {self.alloc_payload}, so fragmenation is {(size / self.total_size)*100.0:.2f}%"
            )
            return None
    ...
    def random_free(self) -> bool:
        ...
        print(
            f"Freeing {size} bytes at {ptr.va_addr:x}, pattern verified: {pattern:02x}"
        )
        self.alloc_payload -= size
        self.d.mm.vfree(ptr)
