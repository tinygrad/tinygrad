...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Verify the correctness of one or more kernel",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="a string of a tuple of (ast, applied_opts,)",
    )
    parser.add_argument(
        "--file",
        type=str,
        default=None,
        help="a file containing a tuple of ast and applied_opts, one per line",
    )
    parser.add_argument(
        "--pkl",
        type=str,
        default=None,
        help="a pickle file containing a single tuple of ast and applied_opts",
    )
    parser.add_argument(
        "--rtol",
        type=float,
    ...
    parser.add_argument(
        "--timing", action="store_true", help="show final timing for the kernel"
    )
    parser.add_argument(
        "--expected-failures",
        type=int,
        default=0,
        help="the number of expected failed kernels",
    )
    ...
    if args.kernel is not None:
        ...
        raise RuntimeError(
            "no kernel specified; use --kernel, --file, or --pkl options"
        )
    ...
    for i, test_lin in enumerate(test_lins):
        ...
        (msg, rb, vv, gt) = compare_linearizer(
            test_lin, None, None, None, rtol=args.rtol, atol=args.atol
        )
        if msg != "PASS":
            failed_ids.append(i)
    ...
    if len(failed_ids) != args.expected_failures:
        raise RuntimeError(
            f"failed on {len(failed_ids)} kernels, expected {args.expected_failures}"
        )
    else:
