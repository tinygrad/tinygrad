...
for i in range(getenv("CNT", 2000)):
    ...
    mst = MultiShapeTracker(
        [ShapeTracker.from_shape((N,))]
    )  # st_ops don't mutate regular shapetrackers for some reason
    for j in range(20):
        random.choice(st_ops)(mst)
    assert (
        mst.sts[0].real_size() <= N
    ), f"{N=}, real_size={mst.sts[0].real_size()}, st={mst.sts[0]}"
