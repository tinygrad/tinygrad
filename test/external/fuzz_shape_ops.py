...
settings.register_profile(__file__, settings.default, max_examples=100 if CI else 250, deadline=None, derandomize=getenv("DERANDOMIZE_CI", False))
...
class TestShapeOps(unittest.TestCase):
  ...
  def test_chunk(self, s: tuple[int, ...], dim: int, num: int):
    ...
    assume((0 <= (actual_dim := len(s) - dim if dim < 0 else dim) < len(s) and s[actual_dim] > 0) or (num < 16))
