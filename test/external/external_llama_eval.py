...
class LLaMaAdaptor(BaseLM):
  def __init__(
    ...
    self.llama = LLaMa.build(Path(checkpoint_path), Path(tokenizer_path), model_gen, model_size, quantize, use_4bit)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--use_4bit', action='store_true', help="Quantize the weights to int4 in memory")
  ...
  # run eval and exit
  adaptor = LLaMaAdaptor(model_gen=args.gen, model_size=args.size, quantize=args.quantize, use_4bit=args.use_4bit,
