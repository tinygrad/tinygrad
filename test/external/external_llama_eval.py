...
class LLaMaAdaptor(BaseLM):
    def __init__(
        ...
        self.llama = LLaMa.build(
            checkpoint_path, tokenizer_path, model_gen, model_size, quantize
        )
    ...
    def greedy_until(self, requests):
        ...
        for request in requests:
            ...
            output = self.llama.greedy_until(
                prompt, until, max_length=128, temperature=0.0
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run LLaMA evals in tinygrad",
    ...
    parser.add_argument(
        ...
        type=str,
        default="7B",
        help="Size of model to use [7B, 13B, 30B, 65B] for Gen 1, [7B, 13B] for Gen 2",
    )
    parser.add_argument(
        "--gen", type=int, default="1", help="Generation of the model to use [1, 2]"
    )
    parser.add_argument(
        "--quantize", action="store_true", help="Quantize the weights to int8 in memory"
    )
    parser.add_argument(
        "--eval", type=str, default="arc_easy", help="Run in evaluation mode"
    )
    parser.add_argument("--limit", type=int, default=None, help="Limit tests in eval")
    parser.add_argument(
        "--weights",
        type=str,
        default="./weights/LLaMa/",
        help="Location of the weights",
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
        default="./weights/LLaMa/tokenizer.model",
    ...
    results = evaluator.evaluate(
        adaptor, tasks.get_task_dict(args.eval.split(",")), False, 0, args.limit
    )
