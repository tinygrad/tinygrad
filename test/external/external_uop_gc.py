...
def uops_allocated():
  return sum([isinstance(x, UOp) for x in gc.get_objects()])
...
def two_plus_two_schedule():
  (Tensor([2]) + Tensor([2])).schedule()
...
def two_plus_two_realize():
  (Tensor([2]) + Tensor([2])).realize()


def two_plus_two_item():
  (Tensor([2]) + Tensor([2])).item()
...
tests = [
  ...
  single_tensor,
  two_plus_two,
  two_plus_two_schedule,
  two_plus_two_kernel,
  two_plus_two_linearize,
  two_plus_two_realize,
  two_plus_two_item,
  gradient_test,
  realized_eye,
  realized_list,
