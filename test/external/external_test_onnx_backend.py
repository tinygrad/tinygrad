    ...
    def run(self, inputs: Any, **kwargs: Any) -> Tuple[Any, ...]:
        ...
        return tuple(
            (
                ...
                else [i.numpy() for i in x] if isinstance(x, list) else np.array(x)
            )
            for x in ret.values()
        )
...
backend_test.exclude(
    "test_resize_downsample_scales_cubic_*"
)  # unsure how to implement cubic
backend_test.exclude(
    "test_resize_downsample_sizes_cubic_*"
)  # unsure how to implement cubic
backend_test.exclude(
    "test_resize_upsample_scales_cubic_*"
)  # unsure how to implement cubic
backend_test.exclude(
    "test_resize_upsample_sizes_cubic_*"
)  # unsure how to implement cubic
backend_test.exclude(
    "test_ai_onnx_ml_tree_ensemble_*"
)  # https://github.com/onnx/onnx/blob/main/onnx/reference/ops/aionnxml/op_tree_ensemble.py#L121

# rest of the failing tests
backend_test.exclude(
    "test_resize_downsample_scales_linear_antialias_cpu"
)  # antialias not implemented
backend_test.exclude(
    "test_resize_downsample_sizes_linear_antialias_cpu"
)  # antialias not implemented
backend_test.exclude(
    "test_resize_tf_crop_and_resize_cpu"
)  # unsure about fill value after clip
backend_test.exclude(
    "test_ai_onnx_ml_label_encoder_tensor_value_only_mapping_cpu"
)  # bad data type string
backend_test.exclude(
    "test_ai_onnx_ml_label_encoder_tensor_mapping_cpu"
)  # bad data type string
backend_test.exclude(
    "test_group_normalization_*"
)  # numerical inaccuracy problem. Current Group Normalization OP fails test
