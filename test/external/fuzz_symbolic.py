...
unary_ops = [
  lambda a: a + random.randint(-4, 4),
  lambda a: a * random.randint(-4, 4),
  lambda a: a // random.randint(1, 9),
  lambda a: a % random.randint(1, 9),
  lambda a: a.maximum(random.randint(-10, 10)),
  lambda a: a.minimum(random.randint(-10, 10)),
]
...
if __name__ == "__main__":
  ...
  for i in range(10000):
    ...
    if check == z3.unknown and DEBUG >= 1:
      skipped += 1
      print(
        "Skipped due to timeout or interrupt:\n"
      ...
      with Context(CORRECT_DIVMOD_FOLDING=1):
        ...
        if num == rn:
          print("z3 found a mismatch but the expressions are equal!!")
      assert False, (
        f"mismatched {expr.render()} at v1={m[v1]}; v2={m[v2]}; v3={m[v3]} = {num} != {rn}\n"
        + "Reproduce with:\n"
        + f'v1=Variable("{u1.arg[0]}", {u1.arg[1]}, {u1.arg[2]})\n'
        + f'v2=Variable("{u2.arg[0]}", {u2.arg[1]}, {u2.arg[2]})\n'
        + f'v3=Variable("{u3.arg[0]}", {u3.arg[1]}, {u3.arg[2]})\n'
