...
if __name__ == "__main__":
    ...
    for _ in range(1000):
        ...
        if random.random() < 0.1:
            tape.append(random.choice([lt, le, gt, ge]))
        ...
        space = list(
            itertools.product(
                range(u1.vmin, u1.vmax + 1),
                range(u2.vmin, u2.vmax + 1),
                range(u3.vmin, u3.vmax + 1),
            )
        )
        volume = len(space)
        for v1, v2, v3 in random.sample(space, min(100, volume)):
            ...
            if num != rn:
                ...
                assert (
                    num == rn
                ), f"mismatched {expr.render()} at {v1=} {v2=} {v3=} = {num} != {rn}\n{expr.render(simplify=False)}"
            if DEBUG >= 1:
                print(f"matched {expr.render()} at {v1=} {v2=} {v3=} = {num} == {rn}")
