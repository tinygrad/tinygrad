...
from tinygrad.helpers import Profiling, Timing, getenv, NEW_LINEARIZE
  ...
  with Timing("all "):
    ...
    if not FORWARD_ONLY:
      ...
      if not SCHEDULE_ONLY:
        ...
        rewrites = get_rewrites_for_renderer(Device.default.renderer, linearizer=NEW_LINEARIZE)
        ...
        if LINEARIZE:
          with Timing("***** model linearize in "):
            ...
            for u in rewritten_uops:
              if NEW_LINEARIZE: uops_line.append(linearize(u))
              else: uops_line.append(apply_rewrites(u, rewrites_for_linearizer))
          with Timing("***** model verify in    "):
            for u in uops_line: type_verify(u if NEW_LINEARIZE else u.arg.lst)
          print(sum(len(u if NEW_LINEARIZE else u.arg.lst) for u in uops_line))
