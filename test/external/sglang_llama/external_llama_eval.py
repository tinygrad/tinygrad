...
class LLaMaAdaptor(LM):
  ...
  def generate_until(self, requests: list[Instance]) -> list[str]:
    ...
    for request in tqdm(requests):
      ...
      for _ in range(max_gen_toks):
        ...
        if next_tok in self.tokenizer.stop_tokens or next_tok in until:
          break
  ...
  def loglikelihood(self, requests: list[Instance]) -> list[tuple[float, bool]]:
    raise NotImplementedError()  # needs changes to extra/models/llama.py
...
if __name__ == "__main__":
  ...
  # run eval and exit
  adaptor = LLaMaAdaptor(model_size=args.size, quantize=args.quantize, checkpoint_path=args.model, max_length=args.ctx)
