...
class TestYOLOv8(unittest.TestCase):
  ...
  def test_forward_pass_torch_onnx(self):
    ...
    weights_location_pt = fetch(
      f"https://github.com/ultralytics/assets/releases/download/v0.0.0/yolov8{variant}.pt", name=f"yolov8{variant}.pt"
    ...
    image_location = [
      np.frombuffer(
        io.BytesIO(fetch("https://raw.githubusercontent.com/ultralytics/yolov5/master/data/images/bus.jpg").read_bytes()).read(), np.uint8
      )
    ]  # noqa: E501
