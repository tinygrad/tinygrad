...
class TestYOLOv8(unittest.TestCase):
    def test_all_load_weights(self):
        for variant in ["n", "s", "m", "l", "x"]:
            ...
            state_dict = safe_load(
                fetch(
                    f"https://gitlab.com/r3sist/yolov8_weights/-/raw/master/yolov8{variant}.safetensors"
                )
            )
    ...
    def test_predictions(self):
        ...
        for i in range(len(test_image_urls)):
            img = cv2.imdecode(
                np.frombuffer(fetch(test_image_urls[i]).read_bytes(), np.uint8), 1
            )
            test_image = preprocess([img])
            predictions = TinyYolov8(test_image)
            post_predictions = postprocess(
                preds=predictions, img=test_image, orig_imgs=[img]
            )
            labels = label_predictions(post_predictions)
            assert (
                labels == {5: 1, 0: 4, 11: 1}
                if i == 0
                else labels == {0: 13, 29: 1, 32: 1}
            )

    def test_forward_pass_torch_onnx(self):
        variant = "n"
        weights_location = fetch(
            f"https://gitlab.com/r3sist/yolov8_weights/-/raw/master/yolov8{variant}.safetensors"
        )
        weights_location_pt = fetch(
            f"https://github.com/ultralytics/assets/releases/download/v0.0.0/yolov8{variant}.pt",
            name=f"yolov8{variant}.pt",
        )  # it needs the pt extension  # noqa: E501
        ...
        onnx_output = onnx_session.run(
            [onnx_output_name], {onnx_input_name: input_image.numpy()}
        )
        ...
        np.testing.assert_allclose(
            onnx_output[0], tiny_output.numpy(), atol=5e-4, rtol=0.025
        )
