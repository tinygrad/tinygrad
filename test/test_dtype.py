...
def _test_to_np(a: Tensor, np_dtype, target):
  ...
  na = a.numpy()
  if DEBUG >= 2:
    print(na, na.dtype, a.uop.base.realized)
  try:
...
class TestDType(unittest.TestCase):
  ...
  def test_casts_to(self):
    list(map(lambda dtype: _test_cast(Tensor(self.DATA, dtype=dtype), self.DTYPE), get_available_cast_dtypes(self.DTYPE)))

  def test_casts_from(self):
    list(map(lambda dtype: _test_cast(Tensor(self.DATA, dtype=self.DTYPE), dtype), get_available_cast_dtypes(self.DTYPE)))
  ...
  def test_bitcast(self):
    ...
    list(
      map(
        lambda dtype: _test_bitcast(Tensor(self.DATA[:8], dtype=self.DTYPE), dtype) if dtype != dtypes.bool else None,
        get_available_cast_dtypes(self.DTYPE),
      )
    )
  ...
  def test_uint_overflow(self):
    if not dtypes.is_unsigned(self.DTYPE):
      raise unittest.SkipTest("only for unsigned")
  ...
  def test_finfo(self):
    if self.DTYPE not in [dtypes.float16, dtypes.bfloat16, dtypes.float32, dtypes.float64]:
      return
...
def _test_ops(a_dtype: DType, b_dtype: DType, target_dtype=None):
  ...
  if not is_dtype_supported(a_dtype) or not is_dtype_supported(b_dtype) or not is_dtype_supported(target_dtype):
    return
...
class TestFp8s(unittest.TestCase):
  def test_fp8e4m3_creation(self):
    assert Tensor([-1, 1, 2], dtype=dtypes.fp8e4m3).dtype == dtypes.fp8e4m3

  def test_fp8e5m2_creation(self):
    assert Tensor([-1, 1, 2], dtype=dtypes.fp8e5m2).dtype == dtypes.fp8e5m2
...
class TestFp8sConversions(unittest.TestCase):
  ...
  def test_float_to_fp8e4m3(self, x):
    np.testing.assert_equal(float_to_fp8(x, dtypes.fp8e4m3), ml_dtypes.float8_e4m3fn(x).tobytes()[0])
  ...
  def test_float_to_fp8e5m2(self, x):
    np.testing.assert_equal(float_to_fp8(x, dtypes.fp8e5m2), ml_dtypes.float8_e5m2(x).tobytes()[0])
  ...
  def test_fp8e4m3_to_float(self, x):
    np.testing.assert_equal(fp8_to_float(x, dtypes.fp8e4m3), np.uint8(x).view(ml_dtypes.float8_e4m3fn).item())

  @given(strat.integers(min_value=0, max_value=255))
  def test_fp8e5m2_to_float(self, x):
    np.testing.assert_equal(fp8_to_float(x, dtypes.fp8e5m2), np.uint8(x).view(ml_dtypes.float8_e5m2).item())
...
class TestFloatDType(TestDType):
  ...
  def test_float_to_uint(self):
    _test_op(lambda: Tensor([-0.9, -0.3, 1.2], dtype=dtypes.float32).cast(dtypes.uint32), dtypes.uint32, [0, 0, 1])
...
class TestDoubleDType(TestDType):
  ...
  def test_float64_to_float32_cast_inf(self):
    _test_op(lambda: Tensor([3.4e40, 3.4e38, 1, 0], dtype=dtypes.float64).cast(dtypes.float32), dtypes.float32, [float("inf"), 3.4e38, 1, 0])
...
class TestBFloat16Type(TestDType):
