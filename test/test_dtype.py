...
if Device.DEFAULT == "CPU":
    core_dtypes.remove(dtypes.bfloat16)  # NOTE: this is for teenygrad, don't remove
dtype_ints = [dt for dt in core_dtypes if dtypes.is_int(dt) and is_dtype_supported(dt)]
dtype_floats = [
    dt for dt in core_dtypes if dtypes.is_float(dt) and is_dtype_supported(dt)
]
...
def _test_to_np(a: Tensor, np_dtype, target):
    ...
    na = a.numpy()
    if DEBUG >= 2:
        print(na, na.dtype, a.lazydata.base.realized)
    try:
...
def _assert_eq(
    tensor: Tensor, target_dtype: DType, target, tol_target_dtype: float = 1e-7
):
    if DEBUG >= 2:
        print(tensor.numpy())
    try:
        ...
        np.testing.assert_allclose(
            tensor.numpy(),
            target,
            rtol={dtypes.float16: 1e-3, dtypes.bfloat16: 1e-2}.get(
                target_dtype, tol_target_dtype
            ),
        )
    except AssertionError as e:
        raise AssertionError(
...
def _test_cast(a: Tensor, target_dtype: DType):
    ...
    _test_op(
        ...
        target_dtype,
        list(a.numpy().astype(_to_np_dtype(target_dtype))),
    )


def _test_bitcast(a: Tensor, target_dtype: DType, target=None):
    if target_dtype == dtypes.bfloat16:
        raise unittest.SkipTest("no test for bf16 bitcast yet")
    if (
        getenv("PTX")
        and a.dtype == dtypes.int8
        and target_dtype.itemsize != a.dtype.itemsize
    ):
        raise unittest.SkipTest("shape changing bitcast of int8 broken on PTX")
    _test_op(
        lambda: a.bitcast(target_dtype),
        target_dtype,
        target or a.numpy().view(_to_np_dtype(target_dtype)).tolist(),
    )
...
class TestDType(unittest.TestCase):
    ...
    def test_to_np(self):
        _test_to_np(
            ...
            _to_np_dtype(self.DTYPE),
            np.array(self.DATA, dtype=_to_np_dtype(self.DTYPE)),
        )
    ...
    def test_dtypes_fields(self):
        ...
        self.assertTrue(
            all(
                issubclass(_to_np_dtype(value), np.generic)
                for value in fields.values()
                if _to_np_dtype(value) is not None
            )
        )
    ...
    def test_resulting_and_init_dtypes_match(self):
        ...
        data = [1.0, 2.0, 0.0, 0.5, -1.5, 5.25]
        for dt in dtypes:
            arr = np.asarray(data).astype(dt)
            tensor = Tensor(arr)
            if not is_dtype_supported(tensor.dtype):
                continue
            tin = tensor.numpy()
            tor = torch.as_tensor(arr).detach().numpy()
            assert (
                dt == tin.dtype == tor.dtype
            ), f"dtype mismatch: expected={dt} | tinygrad={tin.dtype} | torch={tor.dtype}"
...
def _test_ops(a_dtype: DType, b_dtype: DType, target_dtype=None):
    ...
    if (
        not is_dtype_supported(a_dtype)
        or not is_dtype_supported(b_dtype)
        or not is_dtype_supported(target_dtype)
    ):
        return
    ...
    _assert_eq(
        (
            Tensor([1], dtype=a_dtype).cast(b_dtype)
            + Tensor([1], dtype=a_dtype).cast(b_dtype)
        ).cast(a_dtype),
        a_dtype,
        [2],
...
class TestBFloat16DTypeCast(unittest.TestCase):
    def test_f16_to_bf16_conversion(self):
        ...
        np.testing.assert_allclose(
            back_to_float32.numpy(), original_to_float32.numpy(), rtol=1e-2, atol=1e-3
        )

    def test_f16_to_bf16_edge_cases(self):
        edge_cases = Tensor(
            [0.0, -0.0, float("inf"), float("-inf"), float("nan")], dtype=dtypes.float16
        )
        converted = edge_cases.cast(dtypes.bfloat16).cast(dtypes.float32)
        np.testing.assert_equal(
            converted.numpy(), edge_cases.cast(dtypes.float32).numpy()
        )

    def test_f16_to_bf16_range_precision(self):
        large_value = Tensor(
            [65504.0], dtype=dtypes.float16
        )  # Max representable in float16
        small_value = Tensor(
            [6.1035e-5], dtype=dtypes.float16
        ...
        np.testing.assert_allclose(
            large_converted.numpy(),
            large_value.cast(dtypes.float32).numpy(),
            rtol=1e-2,
            atol=1e-3,
        )
        np.testing.assert_equal(
            small_converted.numpy(), small_value.cast(dtypes.float32).numpy()
        )

    def test_f16_to_bf16_randomized(self):
        np.random.seed(42)  # For reproducibility
        random_values = Tensor(
            np.random.uniform(-65504, 65504, 1000), dtype=dtypes.float16
        )
        converted = random_values.cast(dtypes.bfloat16).cast(dtypes.float32)
        np.testing.assert_allclose(
            converted.numpy(),
            random_values.cast(dtypes.float32).numpy(),
            rtol=1e-2,
            atol=1e-3,
        )
...
class TestFloatDType(TestDType):
    ...
    def test_float_to_uint(self):
        _test_op(
            lambda: Tensor([-0.9, -0.3, 1.2], dtype=dtypes.float32).cast(dtypes.uint32),
            dtypes.uint32,
            [0, 0, 1],
        )
...
class TestDoubleDType(TestDType):
    ...
    @unittest.skipIf(
        (CI and Device.DEFAULT in {"CUDA", "NV"}) or getenv("PTX"),
        "conversion not supported on CI CUDA and PTX",
    )  # TODO: why not?
    def test_float64_increased_precision(self):
        for func in [
            ...
            a = [2, 3, 4]
            np.testing.assert_allclose(
                func(Tensor(a, dtype=self.DTYPE)).numpy(),
                func(torch.tensor(a, dtype=torch.float64)),
                rtol=1e-12,
                atol=1e-12,
            )
    ...
    def test_float64_to_float32_cast_inf(self):
        _test_op(
            lambda: Tensor([3.4e40, 3.4e38, 1, 0], dtype=dtypes.float64).cast(
                dtypes.float32
            ),
            dtypes.float32,
            [float("inf"), 3.4e38, 1, 0],
        )
...
class TestInt8DType(TestDType):
    ...
    def test_int8_to_uint16_negative(self):
        _test_op(
            lambda: Tensor([-1, -2, -3, -4], dtype=dtypes.int8).cast(dtypes.uint16),
            dtypes.uint16,
    ...
    def test_bitcast_alt(self):
        a = Tensor([72, -90, 27, 40, -53, 70, 96, 51], dtype=dtypes.int8).bitcast(
            dtypes.short
        )
...
class TestBitCast(unittest.TestCase):
    @given(
        strat.sampled_from(dtype_ints + dtype_floats),
        strat.sampled_from(dtype_ints + dtype_floats),
    )
    def test_shape_change_bitcast(self, dt1, dt2):
        # NOTE: this has to be assume to prevent hypothesis from skipping all samples
        assume(
            dt2 != dtypes.bfloat16 and dt1 != dtypes.bfloat16
        )  # no test for bf16 bitcast yet
        assume(
            not (getenv("PTX") and dt1 == dtypes.int8)
        )  # TODO: bitcasting int8 fails in PTX
        data = rand_for_dtype(dt1, 32).reshape(2, 2, 8)
        _test_op(
            lambda: Tensor(data, dtype=dt1).bitcast(dt2),
            dt2,
            data.view(_to_np_dtype(dt2)).tolist(),
        )
    ...
    def test_shape_change_bitcast_exceptions(self):
        ...
        with self.assertRaises(RuntimeError):
            ...
            Tensor.empty((4,), dtype=dtypes.int8, requires_grad=True).bitcast(
                dtypes.float16
            )
...
class TestUint16DType(TestDType):
    ...
    def test_uint16_to_int8_overflow(self):
        _test_op(
            lambda: Tensor([2**16 - 1, 2**16 - 2, 1, 0], dtype=dtypes.uint16).cast(
                dtypes.int8
            ),
            dtypes.int8,
            [-1, -2, 1, 0],
...
class TestHelpers(unittest.TestCase):
    ...
    @given(
        strat.sampled_from(signed_ints + uints),
        strat.integers(min_value=1, max_value=8),
    )
    ...
    @given(
        ...
        strat.integers(min_value=2, max_value=8),
    )
...
class TestTypeSpec(unittest.TestCase):
    def setUp(self):
        self.old_default_int, self.old_default_float = (
            dtypes.default_int,
            dtypes.default_float,
        )

    def tearDown(self):
        dtypes.default_int, dtypes.default_float = (
            self.old_default_int,
            self.old_default_float,
        )
    ...
    def test_env_set_default_float(self):
        ...
        subprocess.run(
            [
                'python3 -c "from tinygrad import dtypes; assert dtypes.default_float == dtypes.float"'
            ],
            shell=True,
            check=True,
        )
        ...
        subprocess.run(
            [
                'DEFAULT_FLOAT=HALF python3 -c "from tinygrad import dtypes; assert dtypes.default_float == dtypes.half"'
            ],
            shell=True,
            check=True,
        )
        ...
        with self.assertRaises(subprocess.CalledProcessError):
            subprocess.run(
                ['DEFAULT_FLOAT=INT32 python3 -c "from tinygrad import dtypes"'],
                shell=True,
                check=True,
            )
        ...
        with self.assertRaises(subprocess.CalledProcessError):
            subprocess.run(
                ['DEFAULT_FLOAT=TYPO python3 -c "from tinygrad import dtypes"'],
                shell=True,
                check=True,
            )

    @unittest.skipUnless(
        is_dtype_supported(dtypes.int8), f"no int8 on {Device.DEFAULT}"
    )
    def test_dtype_str_arg(self):
        ...
        for dtype_str, dtype in [
            ...
            np.testing.assert_equal(
                Tensor(n, dtype=dtype_str).numpy(), Tensor(n, dtype=dtype).numpy()
            )
            np.testing.assert_equal(
                Tensor(n).cast(dtype_str).numpy(), Tensor(n).cast(dtype).numpy()
            )
            if dtype.itemsize == 4:
                np.testing.assert_equal(
                    Tensor(n).bitcast(dtype_str).numpy(),
                    Tensor(n).bitcast(dtype).numpy(),
                )
                tested += 1
        assert tested == 3

        with self.assertRaises(AttributeError):
            Tensor([1, 2, 3], dtype="nonexistdtype")
        with self.assertRaises(AttributeError):
            Tensor([1, 2, 3], dtype="")

        np.testing.assert_equal(
            Tensor(n).sum(acc_dtype="int16").numpy(),
            Tensor(n).sum(acc_dtype=dtypes.int16).numpy(),
        )
    ...
    def test_full(self, default_int, default_float):
        ...
        if is_dtype_supported(dtypes.int64):
            _assert_eq(
                Tensor.zeros((2, 3), dtype=dtypes.int64), dtypes.int64, np.zeros((2, 3))
            )
        if is_dtype_supported(dtypes.float16):
            _assert_eq(
                Tensor.zeros((2, 3), dtype=dtypes.float16),
                dtypes.float16,
                np.zeros((2, 3)),
            )
        ...
        if is_dtype_supported(dtypes.int64):
            _assert_eq(
                Tensor.ones((2, 3), dtype=dtypes.int64), dtypes.int64, np.ones((2, 3))
            )
        if is_dtype_supported(dtypes.float16):
            _assert_eq(
                Tensor.ones((2, 3), dtype=dtypes.float16),
                dtypes.float16,
                np.ones((2, 3)),
            )
        ...
        if is_dtype_supported(dtypes.int64):
            _assert_eq(
                Tensor.full((2, 3), 3, dtype=dtypes.int64),
                dtypes.int64,
                np.full((2, 3), 3),
            )
            _assert_eq(
                Tensor.full((2, 3), 3.0, dtype=dtypes.int64),
                dtypes.int64,
                np.full((2, 3), 3),
            )
        if is_dtype_supported(dtypes.float16):
            _assert_eq(
                Tensor.full((2, 3), 3, dtype=dtypes.float16),
                dtypes.float16,
                np.full((2, 3), 3),
            )
            _assert_eq(
                Tensor.full((2, 3), 3.0, dtype=dtypes.float16),
                dtypes.float16,
                np.full((2, 3), 3),
            )
    ...
    def test_arange(self, default_int, default_float):
        ...
        if is_dtype_supported(dtypes.float16):
            _assert_eq(
                Tensor.arange(5, dtype=dtypes.float16), dtypes.float16, np.arange(5)
            )
        _assert_eq(
            Tensor.arange(3, 9, 0.7),
            dtypes.default_float,
            np.arange(3, 9, 0.7),
            1e-6 if Device.DEFAULT == "WEBGPU" else 1e-7,
        )
    ...
    def test_bool_ops(self, dtype, op):
        assert (
            op(Tensor.ones(4, 4, dtype=dtype), Tensor.ones(4, 4, dtype=dtype)).dtype
            == dtypes.bool
        )

    @given(
        strat.sampled_from(core_dtypes),
        strat.sampled_from(dtype_ints),
        strat.sampled_from(dtype_floats),
    )
    ...
    def test_attention_returns_same_dtype(self, data_dtype, default_float):
        ...
        assert (
            query.scaled_dot_product_attention(key, value, is_causal=True).dtype
            == data_dtype
        )
        assert (
            query.scaled_dot_product_attention(
                key, value, is_causal=True, dropout_p=0.3
            ).dtype
            == data_dtype
        )
        assert (
            query.scaled_dot_product_attention(key, value, is_causal=False).dtype
            == data_dtype
        )
        assert (
            query.scaled_dot_product_attention(key, value, attn_mask=mask).dtype
            == data_dtype
        )
...
class TestAutoCastType(unittest.TestCase):
    ...
    @given(
        strat.sampled_from(
            [d for d in core_dtypes if dtypes.is_int(d) and is_dtype_supported(d)]
        )
    )
    ...
    def test_broadcast_scalar(self, dt):
        assert (Tensor.ones(4, 4, dtype=dt) + 2.3).dtype == (
            dt if dtypes.is_float(dt) else dtypes.default_float
        )
        assert (Tensor.ones(4, 4, dtype=dt) + 2).dtype == (
            dt if dtypes.is_float(dt) or dtypes.is_int(dt) else dtypes.default_int
        )
    ...
    def test_where_no_scalar(self, dt1, dt2):
        self.check_where_alternate_input_other(
            Tensor(2, dtype=dt1), Tensor(3, dtype=dt2), least_upper_dtype(dt1, dt2)
        )
    ...
    def test_where_one_scalar(self, dt):
        t = Tensor(2, dtype=dt)
        self.check_where_alternate_input_other(
            t, 3.2, (dt if dtypes.is_float(dt) else dtypes.default_float)
        )
        self.check_where_alternate_input_other(
            t,
            3,
    ...
    def test_maximum(self, dt1, dt2):
        assert Tensor([0, 1, 2], dtype=dt1).maximum(
            Tensor([2, 0, 5], dtype=dt2)
        ).dtype == least_upper_dtype(dt1, dt2)

    @given(strat.sampled_from(core_dtypes))
    def test_maximum_const(self, dt):
        assert Tensor([1, 2], dtype=dt).maximum(3.1).dtype == (
            dt if dtypes.is_float(dt) else dtypes.default_float
        )
        assert Tensor([1, 2], dtype=dt).maximum(3).dtype == (
            dt if dtypes.is_float(dt) or dtypes.is_int(dt) else dtypes.default_int
        )
    ...
    def test_div(self):
        assert (
            Tensor([1, 2], dtype=dtypes.int32) / Tensor([2, 2], dtype=dtypes.int32)
        ).dtype == dtypes.default_float
        assert (
            Tensor([1, 2], dtype=dtypes.int16) / Tensor([2, 2], dtype=dtypes.int32)
        ).dtype == dtypes.default_float
        assert (
            Tensor([1, 2], dtype=dtypes.float32) / Tensor([2, 2], dtype=dtypes.float16)
        ).dtype == dtypes.float32
        assert (
            Tensor([1, 2], dtype=dtypes.int32) / Tensor([2, 2], dtype=dtypes.float16)
    ...
    def test_gradient_dtype(self):
        ...
        for default_dtype in [
            dtypes.float16,
            dtypes.bfloat16,
            dtypes.float32,
            dtypes.float64,
        ]:
            if not is_dtype_supported(default_dtype):
                continue
    ...
    def test_mean_half_precision_underflow(self):
        ...
        x = 0.001
        t = (
            Tensor([[x]], dtype=dtypes.half, requires_grad=True)
            .expand(N, N)
            .contiguous()
        )
        np.testing.assert_allclose(
            t.mean(axis=1).numpy(), np.array([x] * N, dtype=np.float16), rtol=1e-3
        )
    ...
    def test_mean_half_precision_overflow(self):
        ...
        np.testing.assert_allclose(
            t.grad.numpy().flatten(), [60000 * 2 / (N * N)] * N * N
        )
    ...
    def test_softmax_dtype(self):
        ...
        np.testing.assert_allclose(
            out.numpy(), tt.softmax(0, dtype=torch.float).numpy(), rtol=1e-3
        )
        out = t.log_softmax(0)
        self.assertEqual(out.dtype, dtypes.half)
        ...
        self.assertEqual(out.dtype, dtypes.float)
        np.testing.assert_allclose(
            out.numpy(), tt.log_softmax(0, dtype=torch.float).numpy(), rtol=1e-3
        )
...
class TestTensorMethod(unittest.TestCase):
    ...
    def test_abs_diff(self, dt):
        if dt == dtypes.bool or not is_dtype_supported(dt):
            return
