...
def _test_bitcast(a:Tensor, target_dtype:DType, target=None):
  ...
  _test_op(lambda: a.bitcast(target_dtype), target_dtype, target or a.numpy().view(_to_np_dtype(target_dtype)))
...
class TestDType(unittest.TestCase):
  ...
  def test_bitcast(self):
    ...
    list(map(
      lambda dtype:
        _test_bitcast(Tensor.rand((10, 40), dtype=self.DTYPE), dtype) if dtype != dtypes.bool else None,
     get_available_cast_dtypes(self.DTYPE)
    ))
