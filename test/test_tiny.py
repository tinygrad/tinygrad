...
class TestTiny(unittest.TestCase):
    ...
    def test_plus(self):
        out = Tensor([1.0, 2, 3]) + Tensor([4.0, 5, 6])
    ...
    def test_jit(self):
        ...
        def new_rand_list(ln=10):
            return [random.randint(0, 100000) for _ in range(ln)]
    ...
    @unittest.skipIf(
        IMAGE > 0 or (CI and Device.DEFAULT == "DSP"),
        "failing because of make things that can't be images not images",
    )
    def test_mnist_model(self):
        ...
        for p in nn.state.get_parameters(layers):
            p.replace(Tensor.ones_like(p).contiguous()).realize()
    ...
    def test_image(self):
        with Context(IMAGE=2):
            self.test_gemm(N=4, out_dtype=dtypes.imagef((4, 1, 4)))
