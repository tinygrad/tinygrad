...
from test.mockgpu.amd.emu import parse_pcode, _c
...
def _srcs():
  ...
  def u32(v=0): return _c(v)
  return {'S0': u32(), 'S1': u32(), 'S2': u32(), 'SCC': u32(), 'VCC': _c(0, dtypes.uint64), 'laneId': u32()}
...
class TestWithSources(unittest.TestCase):
  ...
  def test_v_add_f32_with_sources(self):
    ...
    s0 = _c(0x3f800000)  # 1.0f
    s1 = _c(0x40000000)  # 2.0f
  ...
  def test_v_mul_f32_with_sources(self):
    ...
    s0 = _c(0x40000000)  # 2.0f
    s1 = _c(0x40400000)  # 3.0f
...
class TestParseExpr(unittest.TestCase):
  ...
  def test_ternary(self):
    ...
    vrs = {'cond': UOp.const(True), 'a': _c(1), 'b': _c(0)}
...
class TestForLoopParsing(unittest.TestCase):
  ...
  def test_clz_parsing(self):
    ...
    S0 = _c(0xFFFFFFFF)  # All ones - CLZ should be 0
  ...
  def test_ctz_parsing(self):
    ...
    S0 = _c(1)  # LSB set - CTZ should be 0
    _vrs, assigns = parse_pcode(pcode, {'S0': S0})
    self.assertEqual(len(assigns), 1)
...
class TestDSPcodePatterns(unittest.TestCase):
  ...
  def test_mem_read_parsing(self):
    ...
    addr = _c(0)
    vrs = {'_lds': lds, 'ADDR': addr, 'OFFSET': _c(0)}
...
@unittest.skip("reads the width off a folded constant, and CAST(dt, CONST) no longer folds: the pcode `~` yields CAST(uint, CONST(bool))")
class TestConcatWidthParsing(unittest.TestCase):
  ...
  def test_permlanex16_altrow_concat(self):
    for row, expected in [(0, 1), (1, 0), (2, 3), (3, 2)]:
      parsed = parse_expr('{ row[1], ~row[0] }', {'row': _c(row)})
  ...
  def test_permlane64_altlane_concat(self):
    for lane, expected in [(0, 32), (1, 33), (31, 63), (32, 0), (63, 31)]:
      parsed = parse_expr('{ ~lane[5], lane[4:0] }', {'lane': _c(lane)})
      self.assertEqual(parsed.simplify().arg, expected)
  ...
  def test_permlane64_wave64_pcode_indices(self):
    ...
    srcs = {
      'SRC0': _c(0),
      'VDST': _c(1),
      'EXEC_LO': _c(0xFFFFFFFF),
      'EXEC': _c(0xFFFFFFFFFFFFFFFF, dtypes.uint64),
      '_vgpr': vgpr,
      '_wave_size': 64,
      'S0': _c(0),
      'S1': _c(0),
...
class TestAllPcode(unittest.TestCase):
  ...
  def _make_srcs(self):
    ...
    u32, u64 = lambda v=0: _c(v), lambda v=0: _c(v, dtypes.uint64)
    lds = UOp.param(3, dtypes.uint32, (16384,))
