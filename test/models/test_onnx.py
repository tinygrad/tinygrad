...
class TestOnnxModel(unittest.TestCase):
  ...
  def test_shufflenet(self):
    ...
    self._test_model(
      fetch("https://github.com/onnx/models/raw/main/validated/vision/classification/shufflenet/model/shufflenet-9.onnx"), input_name, input_new
    )
  ...
  def test_resnet(self):
    ...
    self._test_model(
      fetch("https://github.com/onnx/models/raw/main/validated/vision/classification/resnet/model/resnet18-v2-7.onnx"), input_name, input_new
    )
...
class TestHuggingFaceOnnxModels(unittest.TestCase):
  ...
  def _validate(self, repo_id, model_file, custom_inputs, rtol=1e-4, atol=1e-4):
    onnx_model_path = snapshot_download_with_retry(repo_id=repo_id, allow_patterns=["*.onnx", "*.onnx_data"], cache_dir=str(DOWNLOADS_DIR))
