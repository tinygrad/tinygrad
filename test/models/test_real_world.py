...
from examples.gpt2 import (
    Transformer as GPT2Transformer,
    MODEL_PARAMS as GPT2_MODEL_PARAMS,
)
...
def helper_test(
    nm, gen, model, max_memory_allowed, max_kernels_allowed, all_jitted=False
):
    with Context(JIT=2):
        tms = []
        for _ in range(4):
        ...
        print(
            f"{nm}: used {mem_used/1e9:.2f} GB and {kernels_used} kernels in {min(tms)/1e6:.2f} ms"
        )
        assert (
            mem_used / 1e9 < max_memory_allowed
        ), f"{nm} used more than {max_memory_allowed:.2f} GB - {mem_used/1e9:.2} GB used"
        assert (
            not kernels_used or kernels_used <= max_kernels_allowed
        ), f"{nm} used more than {max_kernels_allowed} kernels, it used {kernels_used}"
        if all_jitted:
            assert (
                kernels_used > 0
                and kernels_used == GlobalCounters.kernel_count
                or (
                    kernels_used <= GlobalCounters.kernel_count
                    and getattr(Device[Device.DEFAULT], "graph", None)
                )
...
class TestRealWorld(unittest.TestCase):
    ...
    def test_stable_diffusion(self):
        ...
        @TinyJit
        def test(t, t2):
            return model(t, Tensor([801]), t2).realize()

        helper_test(
            "test_sd",
            lambda: (
                Tensor.randn(1, 4, 64, 64),
                Tensor.randn(1, 77, params["ctx_dim"]),
            ),
            test,
            18.0,
            515,
    ...
    def test_unet_resblock(self):
        ...
        helper_test(
            "test_unet_resblock",
            lambda: (Tensor.empty(4, 16, 8, 8), Tensor.empty(1, 24)),
            test,
            0.01,
            37,
        )
    ...
    def test_train_mnist(self):
        ...
        with Tensor.train():
            ...
            helper_test(
                "train_mnist", lambda: (Tensor.randn(BS, 1, 28, 28),), train, 0.07, 92
            )
    ...
    def test_train_cifar_hyp(self):
        ...
        with Tensor.train():
            ...
            assert not np.isnan(
                lr_scheduler.min_lr
            ), "lr too small or initial_div_facotr too big for half"
