  ...
  if not forward_only:
    ...
    for t, tt in zip(ts, tst):
      np.testing.assert_allclose(
        t.grad, tt.grad.cpu().data, atol=grad_atol, rtol=grad_rtol
      )
...
class TestOps(unittest.TestCase):
  ...
  def test_dot(self):
    helper_test_op(
      [(45, 65), (65, 100)], lambda x, y: x.matmul(y), Tensor.dot, gpu=self.gpu
    )
  ...
  def test_broadcast_partial(self):
    for torch_op, tinygrad_op in [
      ...
      for shapes in [
        ...
        with self.subTest(op=torch_op.__name__, shapes=shapes):
          helper_test_op(
            shapes, torch_op, tinygrad_op, gpu=self.gpu, forward_only=self.gpu
          )
