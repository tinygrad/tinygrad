...
class TestOps(unittest.TestCase):
  ...
  def test_std(self, unbiased=True):
    helper_test_op([(45, 65, 85)], lambda x: torch.std(x, unbiased=unbiased), lambda x: Tensor.std(x))
  def test_std_axis(self, unbiased=True):
    helper_test_op([(45, 65, 85)], lambda x: torch.std(x, unbiased=unbiased, dim=0), lambda x: Tensor.std(x, axis=0))
