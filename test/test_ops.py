...
class TestOps(unittest.TestCase):
  ...
  def test_topk(self):
    ...
    helper_test_op([(4, 6)], lambda x: torch.stack(topk_int32(x, k=2, dim=0)), lambda x: Tensor.stack(*x.topk(k=2, dim=0)), forward_only=True)
    helper_test_op([(4, 6)], lambda x: torch.stack(topk_int32(x, k=2, dim=1)), lambda x: Tensor.stack(*x.topk(k=2, dim=1)), forward_only=True)
    helper_test_op([(4, 6)], lambda x: torch.stack(topk_int32(x, k=2, dim=-1)), lambda x: Tensor.stack(*x.topk(k=2, dim=-1)), forward_only=True)
    ...
    # NOTE: this fails on CI but passes locally, torch indices can be unstable
    # helper_test_op([(20,)], lambda x: torch.stack(topk_int32(x, k=5)), lambda x: Tensor.stack(*x.topk(k=5)),
