...
class TestOps(unittest.TestCase):
  ...
  def test_masked_select(self):
    helper_test_op([(10, 20, 30)], lambda x: x.masked_select(x>0.5), lambda x: x.masked_select(x>0.5), forward_only=True)
    helper_test_op([(10, 20, 30)], lambda x: x.masked_select(torch.tensor(True)), lambda x: x.masked_select(Tensor(True)), forward_only=True)
