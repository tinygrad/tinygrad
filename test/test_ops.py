...
class TestOps(unittest.TestCase):
  ...
  def test_masked_select(self):
    helper_test_op([()], lambda x: x.masked_select(x>0.5), lambda x: x.masked_select(x>0.5), forward_only=True)
    ...
    helper_test_op([(5000,3,32,32)], lambda x: x.masked_select(x>0.5), lambda x: x.masked_select(x>0.5), forward_only=True)
