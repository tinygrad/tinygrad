...
def helper_test_generic(name, f1, f1_args, f2, f2_args):
  ...
  print(
    ("\r" if not CI else "")
    + f"{name:42s} {et_torch:7.2f} ms ({flops / et_torch:9.2f} GFLOPS {mem / et_torch:7.2f} GB/s) in torch, {et_tinygrad:7.2f} ms ({flops / et_tinygrad:9.2f} GFLOPS {mem / et_tinygrad:7.2f} GB/s) in tinygrad, {colorize_float(et_tinygrad / et_torch)} {desc} {flops:10.2f} MOPS {mem:8.2f} MB"
  )  # noqa: E501
...
def helper_test_conv(bs, in_chans, out_chans, kernel_size, img_size_y, img_size_x):
  ...
  def f1(torch_dat):
    return torch_conv(torch_dat)

  def f2(tiny_dat):
    return tiny_conv(tiny_dat).realize()
...
class TestBigSpeed(unittest.TestCase):
  ...
  def test_large_conv_3x3(self):
    helper_test_conv(bs=4, in_chans=128, out_chans=128, kernel_size=3, img_size_y=130, img_size_x=130)

  def test_large_conv_5x5(self):
    helper_test_conv(bs=4, in_chans=128, out_chans=128, kernel_size=5, img_size_y=132, img_size_x=132)
...
class TestSpeed(unittest.TestCase):
  ...
  def test_partial_sum(self):
    ...
    def f(a, b):
      return a.reshape(int(4096 // R), int(4096 * R)).sum(axis=1)
  ...
  def test_array_packing(self):
    ...
    def f(a, b):
      return a.reshape(N, N // 32, 32).permute(1, 0, 2).contiguous()
  ...
  def test_permute(self):
    for N in [1024, 4096]:
      ...
      def f(a, b):
        return a.permute(1, 0).contiguous()
  ...
  def test_double_permute(self):
    ...
    def f(a):
      return a.permute(1, 0, 3, 2).contiguous()
  ...
  def test_gemm_unrolled(self):
    ...
    def f2(a, b):
      return (a.reshape(N, 1, N).expand(N, N, N) * b.reshape(1, N, N).expand(N, N, N)).sum(axis=2)
  ...
  def test_gemm_unrolled_permute_l(self):
    ...
    def f2(a, b):
      return (a.permute(1, 0).reshape(N, 1, N).expand(N, N, N) * b.reshape(1, N, N).expand(N, N, N)).sum(axis=2)
  ...
  def test_gemm_unrolled_permute_r(self):
    ...
    def f2(a, b):
      return (a.reshape(N, 1, N).expand(N, N, N) * b.permute(1, 0).reshape(1, N, N).expand(N, N, N)).sum(axis=2)
  ...
  def test_gemm_unrolled_permute_lr(self):
    ...
    def f2(a, b):
      return (a.permute(1, 0).reshape(N, 1, N).expand(N, N, N) * b.permute(1, 0).reshape(1, N, N).expand(N, N, N)).sum(axis=2)

    helper_test_generic_square("gemm_unrolled_permute_lr", N, f1, f2)

  def test_matvec_1024_1024(self):
    helper_test_matvec("matvec_1024_1024", 1024, 1024)

  def test_matvec_1024_4096(self):
    helper_test_matvec("matvec_1024_4096", 1024, 4096)

  def test_matvec_4096_1024(self):
    helper_test_matvec("matvec_4096_1024", 4096, 1024)

  def test_matvec_4096_4096(self):
    helper_test_matvec("matvec_4096_4096", 4096, 4096)
  ...
  def test_openpilot_conv2d(self):
    ...
    def f1(torch_dat):
      return torch_conv(torch_dat.permute(0, 3, 1, 2))

    def f2(tiny_dat):
      return tiny_conv(tiny_dat.permute(0, 3, 1, 2)).realize()
