...
def _test_uop_result(inputs: List[Tensor], stores: List[UOp], local_size=None):
  ...
  ei = CompiledRunner(
    ProgramSpec(uops[-1].arg.name if uops[-1].arg is not None else "test", src, Device.DEFAULT, uops[-1], uops=uops, local_size=local_size)
  )
...
class TestCStyleFailures(unittest.TestCase):
  ...
  def _test_src_strip_paren(self, op: Ops, should_strip_paren: bool = True):
    ...
    for _ in range(5):
      ret = python_alu[op](ret, Tensor.empty(1, dtype=dtype))
  ...
  def test_repeat_add(self):
    self._test_src_strip_paren(Ops.ADD)

  def test_repeat_mul(self):
    self._test_src_strip_paren(Ops.MUL)

  def test_repeat_xor(self):
    self._test_src_strip_paren(Ops.XOR)

  def test_repeat_or(self):
    self._test_src_strip_paren(Ops.OR)

  def test_repeat_and(self):
    self._test_src_strip_paren(Ops.AND)

  def test_repeat_sub(self):
    self._test_src_strip_paren(Ops.SUB, should_strip_paren=False)
