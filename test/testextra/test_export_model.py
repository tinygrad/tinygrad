...
class TextModelExport(unittest.TestCase):
  def test_multi_input_model_export(self):
    ...
    assert len(inputs) == len(prg["inputs"]) == len(inp_sizes), (
      f"Model and exported inputs don't match: mdl={len(inputs)}, prg={len(prg['inputs'])}, inp_sizes={len(inp_sizes)}"
    )  # noqa: E501
    ...
    for i, exported_input in enumerate(prg["inputs"]):
      assert inputs[i].dtype.name == exported_input["dtype"], (
        f"Model and exported input dtype don't match: mdl={inputs[i].dtype.name}, prg={exported_input['dtype']}"
      )  # noqa: E501
  ...
  def test_multi_output_model_export(self):
    ...
    assert len(outputs) == len(prg["outputs"]) == len(out_sizes), (
      f"Model and exported outputs don't match: mdl={len(outputs)}, prg={len(prg['outputs'])}, inp_sizes={len(out_sizes)}"
    )  # noqa: E501
    ...
    for i, exported_output in enumerate(prg["outputs"]):
      assert outputs[i].dtype.name == exported_output["dtype"], (
        f"Model and exported output dtype don't match: mdl={outputs[i].dtype.name}, prg={exported_output['dtype']}"
      )  # noqa: E501
