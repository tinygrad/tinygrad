...
class TestASMFP8FA(unittest.TestCase):
  ...
  def test_jit_backward(self):
    ...
    state = Tensor([1.,0.]).shard(devices,axis=None).contiguous().realize()
    nxt = Tensor.zeros(2).shard(devices,axis=None).contiguous().realize()
    def layer(x, freqs):
  ...
  def test_saved_operands_do_not_recompute_rope(self):
    ...
    state = Tensor.empty(2,device=device,dtype=dtypes.float32).shard(devices,axis=None)
    nxt = Tensor.empty(2,device=device,dtype=dtypes.float32).shard(devices,axis=None)
  ...
  def test_fused_rope_forward_backward(self):
    ...
    def run(fp8, write_bf16_qk=True):
      ...
      with Context(FP8_FA=1):
        out,_,lse = flash_attention(q,k,v,is_causal=True,fp8_qk=fp8,
                                    ...
                                    fa_bwd_amax=state if fp8 else None,next_fa_bwd_amax=nxt if fp8 else None)
        out.backward(do)
    ...
    for name,a,b,atol,rms_tol in zip(('output','LSE','gradient'),actual,reference,(0.025,0.005,0.003),(0.0005,0.001,0.0002)):
      ...
      if name != "gradient": np.testing.assert_allclose(aa,bb,atol=atol,rtol=0.0)
