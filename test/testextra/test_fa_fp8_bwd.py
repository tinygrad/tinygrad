...
class TestFP8BackwardReference(unittest.TestCase):
  def test_fused_state_update(self):
    ...
    model._fa_bwd_amax = [Tensor([1.,0.]).realize() for _ in range(2)]
    model._next_fa_bwd_amax = [Tensor([3.,4.]).realize(),Tensor([5.,6.]).realize()]
    loss = Tensor([17.]).realize()
    @TinyJit
    def step():
      snapshot = loss.clone()
      reset = model.update_fa_amax(reset=loss)
      Tensor.realize(snapshot,reset,*model._fa_bwd_amax)
      return snapshot
    for i in range(3):
      loss.assign(17.+i).realize()
      model._next_fa_bwd_amax[0].assign(Tensor([3.+i,4.+i])).realize()
      np.testing.assert_array_equal(step().numpy(),[17.+i])
      np.testing.assert_array_equal(loss.numpy(),[0.])
      np.testing.assert_array_equal(model._fa_bwd_amax[0].numpy(),[3.+i,4.+i])
      np.testing.assert_array_equal(model._fa_bwd_amax[1].numpy(),[5.,6.])
