...
class TestLrScheduler(unittest.TestCase):
  ...
  def _test_lr_scheduler(self, tinygrad_sched, torch_sched, epochs, opts, atol, rtol, adam=True):
    ...
    test_tensor = Tensor([0.0], requires_grad=True)  # NOTE: optimizers are broken on 0-dim tensors because it broadcasts to [lr]
    test_tensor.mean().backward()
    if adam:
      tinygrad_optim, torch_optim = Adam([test_tensor], lr=0.01), torch.optim.Adam([torch.tensor([0.0], requires_grad=True)], lr=0.01)
    else:
      tinygrad_optim, torch_optim = SGD([test_tensor], lr=0.01), torch.optim.SGD([torch.tensor([0.0], requires_grad=True)], lr=0.01)
  ...
  def test_multisteplr_gamma(self):
    self._test_multisteplr(10, {"milestones": [1, 2, 7], "gamma": 0.1337}, 1e-6, 1e-6)
  ...
  def test_onecyclelr(self):
    self._test_onecyclelr(
      100,
      {"pct_start": 0.3, "anneal_strategy": "linear", "cycle_momentum": False, "div_factor": 25.0, "final_div_factor": 10000.0, "max_lr": 1e-5},
      1e-6,
      1e-6,
    )
