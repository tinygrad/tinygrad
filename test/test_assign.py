...
class TestAssign(unittest.TestCase):
    ...
    def test_assign_other_jit(self):
        ...
        for i in range(1, 6):
            f(
                x, x.full_like(i).contiguous()
            )  # const would be implicitly folded without contiguous
            assert x.item() == i
    ...
    def test_assign_changes(self):
        ...
        old_a = a
        a.assign(Tensor.full((4,), 2.0).contiguous())
    ...
    def test_simple_assignment_multioutput(self):
        ...
        b = Tensor.full((32,), 1.0).contiguous().realize()
        c = Tensor.full((32,), 2.0).contiguous().realize()
        d = Tensor.full((32,), 3.0).contiguous().realize()
    ...
    def test_permuted_assignment_correct(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "contiguous"):
            ...
            np.testing.assert_equal(
                a.numpy(),
                np.arange(4 * 4).reshape(4, 4).transpose(1, 0)
                + np.arange(4 * 4).reshape(4, 4),
            )

    def test_permuted_reduceop_child_dual_use(self):
        a = Tensor.randn(32, 32, 32).realize()
        b = Tensor.full((32, 32), 1.0).contiguous().realize()
        with self.assertRaisesRegex(RuntimeError, "contiguous"):
            r = a.sum(axis=1)
    ...
    def test_permuted_reduceop_multioutput_dual_use(self):
        a = Tensor.randn(32, 32, 32).realize()
        b = Tensor.full((32, 32), 1.0).contiguous().realize()
        c = Tensor.full((32, 32), 2.0).contiguous().realize()
    ...
    def test_permuted_reduceop_multioutput_dual_use_possible(self):
        ...
        assert GlobalCounters.kernel_count - kc == 2
        np.testing.assert_equal(
            b.numpy(), a.numpy().sum(1) + np.arange(32 * 32).reshape(32, 32)
        )
        np.testing.assert_equal(
            c.numpy(),
            a.numpy().sum(1) + np.arange(32 * 32).reshape(32, 32).transpose(1, 0),
        )
    ...
    def test_permuted_assignment_masked_view_possible(self):
        ...
        np.testing.assert_equal(
            a.numpy(),
            np.ones((4, 4))
            + np.pad(np.ones((4, 4))[:, 0:2], ((0, 0), (0, 2)), constant_values=2),
        )
    ...
    def test_setitem_half(self):
        ...
        assign = a[:4].assign(b)
        assign.realize()
        np.testing.assert_allclose(a.numpy(), [2.0, 2.0, 2.0, 2.0, 1.0, 1.0, 1.0, 1.0])
    ...
    def test_disk_assignment(self):
        a = (
            Tensor.empty(5, device=f"disk:{temp('disk_assignment')}")
            .assign(Tensor.ones(5))
            .numpy()
        )
