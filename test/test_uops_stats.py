...
class TestUOpsStats(unittest.TestCase):
  ...
  def test_simple_matmul_8192(self):
    self.test_simple_matmul(8192, 8192, 8192)
...
class TestStatsOptimized(unittest.TestCase):
  ...
  def test_gemm_upcasted_locals(self):
    try:
      p = get_program(self.ast_gemm, opts=[Opt(OptOps.UPCAST, 0, 4), Opt(OptOps.UPCAST, 1, 4), Opt(OptOps.LOCAL, 0, 4), Opt(OptOps.LOCAL, 1, 4)])
    except KernelOptError:
