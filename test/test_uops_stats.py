...
class TestStatsOptimized(unittest.TestCase):
  ...
  def test_gemm(self):
    p = get_program(self.ast_gemm, device=Device.DEFAULT, opts=[])
    self.check_gemm(p)
    self.assertEqual(p.estimates.lds, 2*N*N*N*4 + 4*N*N)

  def test_gemm_tc_unroll(self):
    try:
      p = get_program(self.ast_gemm, device=Device.DEFAULT, opts=[Opt(OptOps.TC, 0, (-1, 0, 1)), Opt(OptOps.UNROLL, 0, 2)])
  ...
  def test_gemm_one_upcasted(self):
    p = get_program(self.ast_gemm, device=Device.DEFAULT, opts=[Opt(OptOps.UPCAST, 0, 4)])
    self.check_gemm(p)
    self.assertEqual(p.estimates.lds, N*N*N*4 + N*N*N*4//4 + 4*N*N)

  def test_gemm_upcasted(self):
    p = get_program(self.ast_gemm, device=Device.DEFAULT,
  ...
  def test_gemm_upcasted_locals(self):
    try:
      p = get_program(self.ast_gemm, device=Device.DEFAULT, opts=[Opt(OptOps.UPCAST, 0, 4), Opt(OptOps.UPCAST, 1, 4),
  ...
  def test_gemm_group(self):
    try:
      p = get_program(self.ast_gemm, device=Device.DEFAULT, opts=[Opt(OptOps.GROUP, 0, 4)])
    except KernelOptError:
      raise unittest.SkipTest("no locals")
    SZ = N*N*4
  ...
  def test_reduce(self):
    p = get_program(self.ast_reduce, device=Device.DEFAULT, opts=[])
    print(p.name, p.estimates.ops, p.estimates.mem, p.estimates.lds)
  ...
  def test_reduce_group(self):
    try:
      p = get_program(self.ast_reduce, device=Device.DEFAULT, opts=[Opt(OptOps.GROUP, 0, 50)])
    except KernelOptError:
      raise unittest.SkipTest("no locals")
