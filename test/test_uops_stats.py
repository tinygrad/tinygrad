...
class TestMemoryCount(unittest.TestCase):
    ...
    def test_expanded(self):
        ...
        _, mem = get_stats(a + b)
        self.assertEqual(
            mem, 1024 * 1024 * 2 + 1024
        )  # 1 full read + 1 lil read + 1 write
...
class TestUOpsStatsMatmulHalf(unittest.TestCase):
    def test_simple_matmul_half(self, N=16):
        ...
        a, b = Tensor.empty(N, N, dtype=dtypes.half), Tensor.empty(
            N, N, dtype=dtypes.half
        )
        c = a.matmul(b)
        c.realize()
        expected_ops = N**3 * 2
    ...
    def test_bigger_matmul_half(self):
        self.test_simple_matmul_half(64)

    def test_batched_matmul_half(self, N=16):
        GlobalCounters.reset()
        a, b = Tensor.empty(4, N, N, dtype=dtypes.half), Tensor.empty(
            1, N, N, dtype=dtypes.half
        )
        c = a.matmul(b)
        c.realize()
...
class TestStatsOptimized(unittest.TestCase):
    ...
    def test_gemm_one_upcasted(self):
        ...
        self.check_gemm(p)
        self.assertEqual(
            p.estimates.lds, N * N * N * 4 + N * N * N * 4 // 4 + 4 * N * N
        )
    ...
    def test_gemm_group(self):
        ...
        self.assertEqual(
            p.estimates.lds, 2 * N * N * N * 4 + SZ * 4 + (SZ * 4 + 4 * N * N) * 4
        )
