...
class TestPickle(unittest.TestCase):
  ...
  def test_pickle_numpy(self):
    t = Tensor(np.array([1, 2, 3, 4.0]), dtype=dtypes.float32)
    st = pickle.dumps(t)
    t2: Tensor = pickle.loads(st)
    np.testing.assert_equal(t.numpy(), t2.numpy())
  ...
  def test_pickle_jit(self):
    ...
    for _ in range(3):
      add(Tensor.rand(10, 10), Tensor.rand(10, 10))
    st = pickle.dumps(add)
    del add
...
class TestPickleJIT(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    for _ in range(3):
      add(Tensor.rand(N, N), Tensor.rand(N, N))
  ...
  def test_inspect(self):
    ...
    class InspectUnpickler(pickle.Unpickler):
      def find_class(self, module, name):
        return type("SpecializedFakeClass", (FakeClass,), {"name": name, "module": module})
