...
class TestVminVmaxProperties(unittest.TestCase):
    ...
    def test_vmin_vmax_variable_inside_special(self):
        uop = UOp(
            Ops.SPECIAL,
            dtypes.int,
            arg=("gidx0", UOp(Ops.DEFINE_VAR, dtypes.int, arg=("i", 1, 10))),
        )
    ...
    def test_vmin_vmax_multiplication_0_inf(self):
        ...
        y = UOp.load(
            UOp(Ops.DEFINE_GLOBAL, dtypes.float.ptr(), (), 0),
            UOp.const(dtypes.int, 0),
            dtype=dtypes.float,
        )
        uop = x * y
...
class TestConstFactor(unittest.TestCase):
    ...
    def test_const_factor_multiplication(self):
        ...
        self.assertEqual(
            uop.const_factor(), 5
        )  # For multiplication, it's one of the factors
    ...
    def test_const_factor_division(self):
        ...
        self.assertEqual(
            uop.const_factor(), 1
        )  # Division reduces the const_factor to 1
    ...
    def test_const_factor_multiplication_of_consts_and_vars(self):
        ...
        self.assertEqual(
            uop.const_factor(), 15
        )  # Constant multipliers are combined (3 * 5 = 15)
