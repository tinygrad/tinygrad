...
class TestArithmeticSimplifications(unittest.TestCase):
    def test_full_graph_rewrite_division_by_zero(self):
        optimized_div_uop = apply_rewrite(
            UOp.const(dtypes.float32, 10.0) / UOp.const(dtypes.float32, 0.0)
        )
        self.assertEqual(optimized_div_uop.op, Ops.CONST)
        self.assertTrue(
            math.isinf(optimized_div_uop.arg) or math.isnan(optimized_div_uop.arg)
        )

    def test_full_graph_rewrite_redundant_operations(self):
        optimized_uop = apply_rewrite(
            (UOp.const(dtypes.float32, 10.0) + UOp.const(dtypes.float32, 0.0))
    ...
    def test_full_graph_rewrite_division_by_one(self):
        optimized_uop = apply_rewrite(
            UOp.const(dtypes.float32, 42.0) / UOp.const(dtypes.float32, 1.0)
        )
        self.assertEqual(optimized_uop.op, Ops.CONST)
    ...
    def test_full_graph_rewrite_modulo_by_one(self):
        optimized_uop = apply_rewrite(
            UOp.const(dtypes.int32, 42) % UOp.const(dtypes.int32, 1)
        )
        self.assertEqual(optimized_uop.op, Ops.CONST)
...
class TestFoldingAndReduction(unittest.TestCase):
    ...
    def test_full_graph_rewrite_nested_loop_collapse(self):
        ...
        optimized_reduce_uop = apply_rewrite(
            expr.reduce(Ops.ADD, outer_range, inner_range)
        )
        self.assertEqual(optimized_reduce_uop.op, Ops.CONST)
        self.assertEqual(
            optimized_reduce_uop.arg,
            sum((i * 10) + j for i in range(8) for j in range(4)),
        )
        ...
        lhs = UOp(
            ...
            src=(
                UOp(
                    ...
                    dtypes.int.vec(4),
                    arg=None,
                    src=(UOp(Ops.SPECIAL, dtypes.int, arg=("lidx0", 32), src=()),) * 4,
                ),
...
class TestEdgeCasesAndSpecialOperations(unittest.TestCase):
    def test_full_graph_rewrite_transcendental_edge_cases(self):
        optimized_sink = full_graph_rewrite(
            UOp.const(dtypes.float32, -1.0)
            .log2()
            .sink(UOp.const(dtypes.float32, 0.0).reciprocal())
        )
        optimized_log2_neg, optimized_recip_zero = optimized_sink.src
        self.assertTrue(
            math.isnan(optimized_log2_neg.arg),
            f"Expected NaN for log2(-1.0), got {optimized_log2_neg.arg}",
        )
        self.assertTrue(
            math.isinf(optimized_recip_zero.arg) and optimized_recip_zero.arg > 0,
    ...
    def test_full_graph_rewrite_modulo_negative_dividend(self):
        ...
        for x_value in range(-5, 0):
            self.assertEqual(
                x_value % 3, evaluate_uop(optimized_sink.src[0], {"x": x_value})
            )
    ...
    def test_full_graph_rewrite_division_negative_divisor(self):
        ...
        for x_value in range(1, 6):
            self.assertEqual(
                x_value // -2, evaluate_uop(optimized_sink.src[0], {"x": x_value})
            )
...
class TestMulaccUnrolledAcc(unittest.TestCase):
    def test_unrolled2(self):
        ...
        acc = UOp(
            Ops.DEFINE_ACC, dtypes.int, (UOp.const(dtypes.int, 0),) + acc_range, (0,)
        )
        a = UOp.variable("a", 0, 10)
        b = UOp.variable("b", 0, 10)
    ...
    def test_unrolled4_float(self):
        ...
        acc = UOp(
            Ops.DEFINE_ACC,
            dtypes.float32,
            (UOp.const(dtypes.int, 0),) + acc_range,
            (0,),
        )

        a = [
            UOp.variable(f"a{i}", float("-inf"), float("inf"), dtype=dtypes.float32)
            for i in range(4)
        ]
        b = [
            UOp.variable(f"b{i}", float("-inf"), float("inf"), dtype=dtypes.float32)
            for i in range(4)
        ]
