...
from tinygrad.helpers import PackedStruct
...
class TestAutogen(unittest.TestCase):
  def test_packed_struct_sizeof(self):
    ...
    class Z(PackedStruct): _fields_ = layout
    ...
    class Baz(PackedStruct): _fields_ = layout
  ...
  def test_packed_struct_interop(self):
    class Baz(PackedStruct): _fields_ = [('a', ctypes.c_int, 30), ('b', ctypes.c_int, 30), ('c', ctypes.c_int, 2), ('d', ctypes.c_int, 2)]
    src = '''
