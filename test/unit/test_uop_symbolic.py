...
def render(self) -> tuple[str, ConstType, ConstType]:
    ...
    uops = linearize_uop(
        full_graph_rewrite(
            UOp(
                Ops.STORE, dtypes.void, (glbl.index(UOp.const(dtypes.int, 0)), self)
            ).sink()
        )
    )
...
def uconst(val):
    return UOp.const(dtypes.int, val)


def usum(ops):
    return functools.reduce(lambda x, y: x + y, ops)
...
class TestSymbolicPickle(unittest.TestCase):
    def _test_pickle_unpickle(self, x):
        self.assertEqual(x, pickle.loads(pickle.dumps(x)))

    def test_pickle_variable(self):
        self._test_pickle_unpickle(Variable("a", 3, 8))

    def test_pickle_variable_times_2(self):
        self._test_pickle_unpickle(Variable("a", 3, 8) * 2)
...
class TestSymbolic(unittest.TestCase):
    ...
    def test_lt(self):
        ...
        self.helper_test_variable(
            Variable("a", 3, 4) < Variable("b", 5, 6), 1, 1, "True"
        )
        self.helper_test_variable(
            Variable("a", 3, 5) < Variable("b", 5, 6), 0, 1, "(a<b)"
        )
        self.helper_test_variable(
            Variable("a", 5, 6) < Variable("b", 3, 5), 0, 0, "False"
        )
        self.helper_test_variable(
            Variable("a", 3, 4) < Variable("a", 3, 4), 0, 0, "False"
        )
    ...
    def test_lt_factors(self):
        ...
        self.helper_test_variable(
            ...
            0,
            1,
            ("(((idx1*4)+FLOAT4_INDEX)<512)", "((FLOAT4_INDEX+(idx1*4))<512)"),
        )
    ...
    def test_mul_neg_1(self):
        self.helper_test_variable(
            (Variable("a", 0, 2) * -1) // 3, -1, 0, "((((a*-1)+3)//3)+-1)"
        )
    ...
    def test_max_folds(self):
        self.helper_test_variable(
            Variable("a", 0, 20).maximum(10).maximum(11), 11, 20, "max(a, 11)"
        )

    def test_add_min_max(self):
        self.helper_test_variable(
            Variable("a", 0, 8) * 2 + 12, 12, 16 + 12, "((a*2)+12)"
        )
    ...
    def test_sum_div_remove(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7), Variable("b", 0, 3)]) // 20, 0, 0, "0"
        )

    def test_sum_div_min_max(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7), Variable("b", 0, 3)]) // 2, 0, 5, "((a+b)//2)"
        )

    def test_sum_div_mod_factor(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 4, Variable("b", 0, 3) * 4]) // 2,
            0,
            20,
            "((a*2)+(b*2))",
        )
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 4, Variable("b", 0, 3) * 4]) % 2, 0, 0, "0"
        )

    def test_sum_div_some_factor(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 5, Variable("b", 0, 3) * 4]) // 2,
            0,
            23,
            ("(((a*5)//2)+(b*2))", "((b*2)+((a*5)//2))"),
        )
    ...
    def test_sum_div_trim_const(self):
        self.helper_test_variable(
            (Variable("a", 0, 7) * 4 + Variable("b", 0, 3) * 4 + 7) // 16,
            0,
            2,
            "(((a+b)+1)//4)",
        )
    ...
    def test_sum_div_some_partial_factor(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 6, Variable("b", 0, 7) * 6]) // 16,
            0,
            5,
            "(((a*3)+(b*3))//8)",
        )
        self.helper_test_variable(
            usum([uconst(16), Variable("a", 0, 7) * 6, Variable("b", 0, 7) * 6]) // 16,
            1,
            6,
            "((((a*3)+(b*3))//8)+1)",
        )
        self.helper_test_variable(
            (Variable("a", 0, 7) * 30 + 20) // 20, 1, 11, "(((a*3)//2)+1)"
        )

    def test_sum_div_no_factor(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 5, Variable("b", 0, 3) * 5]) // 2,
            0,
            25,
            "(((a*5)+(b*5))//2)",
        )
    ...
    def test_mod_factor(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 100, Variable("b", 0, 3) * 50]) % 100,
            0,
            50,
            "((b%2)*50)",
        )
    ...
    def test_mod_congruence(self):
        ...
        self.helper_test_variable(
            (17 + 13 * Variable("a", 0, 3)) % 18, 2, 17, "((a*-5)+17)"
        )
        self.helper_test_variable(
            (2 + 9 * Variable("a", 0, 3)) % 18, 2, 11, "(((a%2)*9)+2)"
        )

    def test_mod_congruence_mul_add(self):
        self.helper_test_variable(
            (6 * (Variable("a", 0, 2) + 1)) % 9, 0, 6, "((a*-3)+6)"
        )

    def test_mod_congruence_multiple_vars(self):
        self.helper_test_variable(
            (9 + 9 * Variable("x", 0, 3) + 9 * Variable("y", 0, 3)) % 10,
            3,
            9,
            "(((x*-1)+(y*-1))+9)",
        )
        ...
        self.helper_test_variable(
            (10 + 12 * Variable("x", 0, 2) + Variable("y", 0, 4) % 3) % 13,
            8,
            12,
            "(((x*-1)+(y%3))+10)",
        )
    ...
    def test_div_congruence(self):
        ...
        self.helper_test_variable(
            (18 + 17 * Variable("a", 0, 2) + 17) // 18, 1, 3, "(a+1)"
        )

    def test_div_congruence_multiple_vars(self):
        self.helper_test_variable(
            (9 + (9 + 10) * Variable("x", 0, 3) + (8 + 10) * Variable("y", 0, 2)) // 10,
            0,
            10,
            "((x*2)+(y*2))",
        )
    ...
    def test_sum_div_const(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 4, uconst(3)]) // 4, 0, 7, "a"
        )

    def test_sum_div_const_big(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 4, uconst(3)]) // 16, 0, 1, "(a//4)"
        )

    def test_sum_lt_fold(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 4, Variable("b", 0, 3)]) < 16, 0, 1, "(a<4)"
        )
        self.helper_test_variable(
            usum([Variable("a", 0, 7) * 4, Variable("b", 0, 4)]) < 16,
            0,
            1,
            ("(((a*4)+b)<16)", "((b+(a*4))<16)"),
        )
        self.helper_test_variable(
            usum([Variable("uidx", 0, 3), Variable("a", 0, 1529) * 12]) < (4 * 67),
            0,
            1,
            "(a<23)",
        )
    ...
    def test_mod_mod(self):
        ...
        self.helper_test_variable(
            ((5 * Variable("a", 0, 31)) % 12) % 5, 0, 4, "(((a*5)%12)%5)"
        )
        self.helper_test_variable((Variable("a", 0, 31) % 4) % 12, 0, 3, "(a%4)")

    def test_mul_mul(self):
        self.helper_test_variable(
            (Variable("a", 0, 5) * 10) * 9, 0, 5 * 10 * 9, "(a*90)"
        )
    ...
    def test_distribute_mul(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 3), Variable("b", 0, 5)]) * 3, 0, 24, "((a*3)+(b*3))"
        )
        self.helper_test_variable(
            (1 + Variable("a", 0, 3)) * (-2) + 12, 4, 10, "((a*-2)+10)"
        )

    def test_mod_mul_sum(self):
        self.helper_test_variable(
            usum([Variable("b", 0, 2), Variable("a", 0, 5) * 10]) % 9,
            0,
            7,
            ("(b+a)", "(a+b)"),
        )
    ...
    def test_lt_simple_factor(self):
        self.helper_test_variable(
            (Variable("a", 0, 6) * 6 + Variable("b", 0, 6) * 6) < 8,
            0,
            1,
            "(((a*3)+(b*3))<4)",
        )
    ...
    def test_mod_factor_negative(self):
        self.helper_test_variable(
            usum([uconst(-29), Variable("a", 0, 10), Variable("b", 0, 10) * 28]) % 28,
            0,
            27,
            "((a+27)%28)",
        )
        self.helper_test_variable(
            usum([uconst(-29), Variable("a", 0, 100), Variable("b", 0, 10) * 28]) % 28,
            0,
            27,
            "((a+27)%28)",
        )
    ...
    def test_sum_combine_num(self):
        self.helper_test_variable(
            usum([uconst(29), Variable("a", 0, 10), uconst(-23)]), 6, 16, "(a+6)"
        )

    def test_sum_num_hoisted_and_factors_cancel_out(self):
        self.helper_test_variable(
            usum([Variable("a", 0, 1) * -4 + 1, Variable("a", 0, 1) * 4]), 1, 1, "1"
        )

    def test_div_cancel(self):
        self.helper_test_variable(
            usum([uconst(-40), Variable("a", 0, 10) * 2, Variable("b", 0, 10) * 40])
            // 40,
            -1,
            9,
            "(b+-1)",
    ...
    def test_add_div(self):
        ...
        self.helper_test_variable(
            (Variable("a", 0, 5) - 2) // 4, -1, 0, "(((a+2)//4)+-1)"
        )
        self.helper_test_variable(
            (Variable("a", 0, 5) - 1) // 4, -1, 1, "(((a+3)//4)+-1)"
        )
        ...
        self.helper_test_variable(
            (Variable("a", 0, 5) + 5) // 4, 1, 2, "(((a+1)//4)+1)"
        )
    ...
    def test_sum_div_partial_remove(self):
        self.helper_test_variable(
            usum([Variable("idx0", 0, 127) * 4, Variable("idx2", 0, 3)]) // 4,
            0,
            127,
            "idx0",
        )
    ...
    def test_div_numerator_negative(self):
        self.helper_test_variable(
            (Variable("idx", 0, 9) * -10) // 11, -9, 0, "((((idx*-10)+99)//11)+-9)"
        )

    def test_div_into_mod(self):
        self.helper_test_variable(
            (Variable("idx", 0, 16) * 4) % 8 // 4, 0, 1, "(idx%2)"
        )

    # TODO: simplify the expression
    def test_div_neg_cancel(self):
        self.helper_test_variable(
            (-Variable("idx", 0, 100) + 199) // -4 + 50,
            1,
            26,
            "((((idx*-1)+199)//-4)+50)",
        )
        self.helper_test_variable(
            (-Variable("idx", 0, 100) + 200) // -4 + 50,
            0,
            25,
            "((((idx*-1)+200)//-4)+50)",
        )
        self.helper_test_variable(
            (-Variable("idx", 0, 100) + 201) // -4 + 50,
            0,
            25,
            "((((idx*-1)+201)//-4)+50)",
        )
    ...
    def test_sum_div_complex2(self):
        ...
        self.helper_test_variable(
            (gidx0 * 4 + lidx2 * 2 + 1) // 10,
            0,
            3,
            ("(((gidx0*2)+lidx2)//5)", "((lidx2+(gidx0*2))//5)"),
        )
        self.helper_test_variable(
            (gidx0 * 4 + lidx2 * 2 + lidx3) // 10,
            0,
            3,
            ("(((gidx0*2)+lidx2)//5)", "((lidx2+(gidx0*2))//5)"),
        )
    ...
    def test_sum_div_complex3(self):
        ...
        lidx3 = Variable("lidx3", 0, 1)
        self.helper_test_variable(
            (gidx0 * 4 + lidx2 * 2 + lidx3) // 12,
            0,
            4,
            ("(((lidx2//2)+gidx0)//3)", "((gidx0+(lidx2//2))//3)"),
        )
        self.helper_test_variable(
            (lidx2 * 2 + gidx0 * 4 + lidx3) // 12,
            0,
            4,
            ("(((lidx2//2)+gidx0)//3)", "((gidx0+(lidx2//2))//3)"),
        )
    ...
    def test_sum_mul_distribute(self):
        ...
        lidx3 = Variable("lidx3", 0, 1)
        self.helper_test_variable(
            (gidx0 + lidx2 + lidx3) * 4, 0, 80, "(((gidx0*4)+(lidx2*4))+(lidx3*4))"
        )
    ...
    def test_div_neg_all_range(self):
        ...
        self.helper_test_variable(
            (-gidx * 8 - lidx + 999) // -4 + 250,
            1,
            250,
            "(((((gidx*-8)+(lidx*-1))+999)//-4)+250)",
        )
        self.helper_test_variable(
            (-gidx * 8 - lidx + 1000) // -4 + 250,
            0,
            250,
            "(((((gidx*-8)+(lidx*-1))+1000)//-4)+250)",
        )
        self.helper_test_variable(
            (-gidx * 8 - lidx + 1001) // -4 + 250,
            0,
            250,
            "(((((gidx*-8)+(lidx*-1))+1001)//-4)+250)",
        )
        self.helper_test_variable(
            (-gidx * 8 - lidx + 1002) // -4 + 250,
            0,
            250,
            "(((((gidx*-8)+(lidx*-1))+1002)//-4)+250)",
        )
    ...
    def test_div_mod_recombine_folded_mod(self):
        ...
        b = Variable("b", 0, 100)
        self.helper_test_variable(
            (31 * a + 1) % 30 + ((31 * a + 1) // 30) * 30, 1, 63, "((a*31)+1)"
        )
        with self.assertRaises(AssertionError):
            self.helper_test_variable(
                (31 * b + 1) % 18 + ((31 * b + 1) // 18) * 18, 1, 3101, "((b*31)+1)"
            )
    ...
    def test_div_mod_recombine_with_gcd(self):
        ...
        with self.assertRaises(AssertionError):
            self.helper_test_variable(
                (30 * b + 1) % 18 + ((30 * b + 1) // 18) * 18, 1, 3001, "((b*30)+1)"
            )
    ...
    def test_gated_load(self):
        ...
        self.helper_test_variable(
            (idx < 4).where(idx // 4, idx.const_like(-1)),
            -1,
            6,
            "((idx//4) if (idx<4) else -1)",
        )
    ...
    def test_simplex_lt(self):
        ...
        self.helper_test_variable(
            (a * 3 + b * 4 < 1).ne(True), 0, 1, "(((a+b)<1)!=True)"
        )
        self.helper_test_variable(
            (a * (-3) + b * 4 < 1).ne(True), 0, 1, "((((a*-3)+(b*4))<1)!=True)"
        )  # negative coeff, should not be simplified
        self.helper_test_variable(
            (a * 3 + d * 4 < 1).ne(True), 0, 1, "((((a*3)+(d*4))<1)!=True)"
        )  # var can be negative, should not be simplified
        self.helper_test_variable(
            (a + b + c * 2 < 1).ne(True),
            0,
            1,
            ("((((a+b)+c)<1)!=True)", "(((c+(a+b))<1)!=True)"),
        )
        self.helper_test_variable(
            (a + b * 2 + c * 4 < 1).ne(True),
            0,
    ...
    def test_where_combine(self):
        ...
        cc = cond.where(c, c + 1)
        self.helper_test_variable(
            bb + cc, 0, 7, "((b if (x<2) else 1)+(c if (x<2) else (c+1)))"
        )
        ...
        ba = cond.where(b, a)
        self.helper_test_variable(
            ab + ba, 0, 6, "((a if (x<2) else b)+(b if (x<2) else a))"
        )

        # not combining  # TODO: can combine if one is identity element const
        self.helper_test_variable(
            aa + ab, 0, 6, "((a if (x<2) else b)+(a if (x<2) else 0))"
        )
    ...
    def test_where_cast(self):
        ...
        uops = linearize_uop(
            full_graph_rewrite(
                UOp(
                    Ops.STORE, dtypes.void, (glbl.index(UOp.const(dtypes.int, 0)), expr)
                ).sink()
            )
        )
        rewritten_uop = [uop for uop in uops if uop.op is Ops.STORE][0].src[-1]
        ...
        self.assertEqual(
            rewritten_uop, cond.where(a.cast(dtypes.half), b.cast(dtypes.half))
        )
...
class TestSymbolicNumeric(unittest.TestCase):
    ...
    def test_mod_4(self):
        self.helper_test_numeric(lambda x: (x % 4))

    def test_div_4(self):
        self.helper_test_numeric(lambda x: (x // 4))

    def test_plus_1_div_2(self):
        self.helper_test_numeric(lambda x: (x + 1) // 2)

    def test_plus_1_mod_2(self):
        self.helper_test_numeric(lambda x: (x + 1) % 2)

    def test_times_2(self):
        self.helper_test_numeric(lambda x: x * 2)

    def test_times_2_plus_3(self):
        self.helper_test_numeric(lambda x: x * 2 + 3)

    def test_times_2_plus_3_mod_4(self):
        self.helper_test_numeric(lambda x: (x * 2 + 3) % 4)

    def test_times_2_plus_3_div_4(self):
        self.helper_test_numeric(lambda x: (x * 2 + 3) // 4)

    def test_times_2_plus_3_div_4_mod_4(self):
        self.helper_test_numeric(lambda x: ((x * 2 + 3) // 4) % 4)
...
class TestBounds(unittest.TestCase):
    def test_unrolled_arange(self):
        ...
        assert (((alu0 + 2559) // -4) * (-1)).vmin == 0 and (
            ((alu0 + 2559) // -4) * (-1)
        ).vmax == 639
