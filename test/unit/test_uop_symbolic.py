...
class TestSymbolic(unittest.TestCase):
  ...
  def test_mod_congruence_multiple_vars(self):
    ...
    self.helper_test_variable(
      (7 + 9 * Variable("x", 0, 2) + 9 * Variable("y", 0, 2) + Variable("z", 0, 2)) % 10, 3, 9, ("(((z+(x*-1))+(y*-1))+7)", "(((y*-1)+(z+(x*-1)))+7)")
    )
  ...
  def test_sum_lt_fold(self):
    ...
    self.helper_test_variable(usum([Variable("a", 0, 7) * 4, Variable("b", 0, 4)]) < 16, 0, 1, ("(((a*4)+b)<16)", "((b+(a*4))<16)"))
  ...
  def test_lt_sum_factor_rhs_partial(self):
    self.helper_test_variable(
      (Variable("a", 0, 6) * 6 + Variable("b", 0, 6) * 4 + Variable("c", 0, 6) * 8) < 4,
      0,
      1,
  ...
  def test_lt_sum_factor_rhs_all(self):
    self.helper_test_variable(
      (Variable("a", 0, 6) * 6 + Variable("b", 0, 6) * 4 + Variable("c", 0, 6) * 8) < 2,
      0,
      1,
  ...
  def test_sum_mul_distribute(self):
    ...
    lidx2 = Variable("lidx2", 0, 12)
    lidx3 = Variable("lidx3", 0, 1)
    self.helper_test_variable((gidx0 + lidx2 + lidx3) * 4, 0, 80, ("(((gidx0*4)+(lidx2*4))+(lidx3*4))", "((lidx3*4)+((gidx0*4)+(lidx2*4)))"))
...
class TestFuzzFailure(unittest.TestCase):
  def test_fuzz_failure1(self):
    ...
    v3 = Variable("v3", 0, 1)
    expr = (
      (((((((((((((((((((((0 // 4) % 2) // 8) + -2) + -4) + -3) + v1) + -4) + v2) + -2) + v3) + v2) // 3) % 7) * 1) // 2) + v2) * -1) + 2) + 1) + 0)
      + -3
    ) + v3
