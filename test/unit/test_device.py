...
class TestDevice(unittest.TestCase):
    def test_canonicalize(self):
        ...
        self.assertEqual(
            Device.canonicalize("disk:/dev/shm/test"), "DISK:/dev/shm/test"
        )
...
class TestCompiler(unittest.TestCase):
    def test_compile_cached(self):
        ...
        with patch.dict(os.environ, {"DISABLE_COMPILER_CACHE": "0"}, clear=True):
            self.assertEqual(
                MockCompiler("key").compile_cached("123"), str.encode("123")
            )
    ...
    def test_compile_cached_disabled(self):
        ...
        with patch.dict(os.environ, {"DISABLE_COMPILER_CACHE": "1"}, clear=True):
            self.assertEqual(
                MockCompiler("disabled_key").compile_cached("123"), str.encode("123")
            )
