...
class TestRealDoesntSimplify(unittest.TestCase):
  ...
  def test_1(self):
    self.st = ShapeTracker((View.create((8, 3, 1, 2, 11, 1), (33, 11, 0, 0, 1, 0), 0, None), View.create((8, 6, 11), (66, 11, 1), 0, None)))
    self.assertEqual(self.st.real_strides(), (33, None, 1))

  def test_2(self):
    self.st = ShapeTracker((View.create((2, 2, 4, 3, 3), (72, 9, 18, -3, -1), 8, None), View.create((4, 4, 3, 3), (36, 9, 3, 1), 0, None)))
...
class TestIndexExpressions2d(unittest.TestCase):
  def setUp(self):
    ...
    self.sts = [
      ShapeTracker.from_shape((prod(base_shape) + offset,)).shrink(((offset, offset + prod(base_shape)),)).reshape(base_shape)
  ...
  def test_permute_reshape_1(self):  # This tests multiple views
    ...
    for st, base_shape, offset in zip(self.sts, self.shapes, self.offset):
      ...
      self.idxs_exprs.append(
        lambda idxs, base_shape=base_shape, offset=offset: (idxs[0] * (base_shape[1] * 5) + idxs[2]) % base_shape[0] * base_shape[1]
  ...
  def test_permute_reshape_2(self):
    ...
    for st, base_shape, offset in zip(self.sts, self.shapes, self.offset):
      ...
      self.idxs_exprs.append(
        lambda idxs, base_shape=base_shape, offset=offset: (idxs[1] * (base_shape[1] * 5) + idxs[2]) % base_shape[0] * base_shape[1]
    ...
    st = ShapeTracker(
      views=(
        View(
          ...
          offset=-20,
          mask=((0, 1), (0, 2), (0, 1), (0, 4), (0, 4), (2, 11), (0, 4), (2, 11)),
          contiguous=False,
        ),
        View(shape=(2, 4, 11, 11, 4, 3, 3), strides=(10816, 0, 52, 1, 2704, 728, 14), offset=0, mask=None, contiguous=False),
      )
    )
  ...
  def test_size_variable(self):
    st = ShapeTracker(
      views=(
        ...
        View(shape=(1, 32, 1, (Variable("start_pos", 0, 8192) + 1), 128), strides=(0, 128, 0, 4096, 1), offset=0, mask=None, contiguous=False),
      )
    )
