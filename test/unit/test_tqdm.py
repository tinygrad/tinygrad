...
class TestProgressBar(unittest.TestCase):
    def _compare_bars(self, bar1, bar2):
        ...
        def parse_timer(timer):
            return sum(
                int(x) * y for x, y in zip(timer.split(":")[::-1], (1, 60, 3600))
            )
        ...
        if "?" not in suffix1 and "?" not in suffix2:
            ...
            timer1, rm1 = [
                parse_timer(timer)
                for timer in suffix1.split("[")[-1].split(",")[0].split("<")
            ]
            timer2, rm2 = [
                parse_timer(timer)
                for timer in suffix2.split("[")[-1].split(",")[0].split("<")
            ]
        ...
        diff = sum(
            [c1 != c2 for c1, c2 in zip(prog1, prog2)]
        )  # allow 1 char diff to be less flaky, but it should match
    ...
    def test_tqdm_output_iter(self, mock_terminal_size, mock_stderr):
        for _ in range(10):
            ...
            mock_terminal_size.return_value = namedtuple(
                field_names="columns", typename="terminal_size"
            )(ncols)
            ...
            for n in (bar := tinytqdm(range(total), desc="Test")):
                ...
                iters_per_sec = (
                    float(tinytqdm_output.split("it/s")[-2].split(" ")[-1])
                    if n > 0
                    else 0
                )
                elapsed = n / iters_per_sec if n > 0 else 0
                tqdm_output = tqdm.format_meter(
                    n=n, total=total, elapsed=elapsed, ncols=ncols, prefix="Test"
                )
            ...
            tinytqdm_output = mock_stderr.getvalue().split("\r")[-1].rstrip()
            iters_per_sec = (
                float(tinytqdm_output.split("it/s")[-2].split(" ")[-1]) if n > 0 else 0
            )
            elapsed = total / iters_per_sec if n > 0 else 0
            tqdm_output = tqdm.format_meter(
                n=total, total=total, elapsed=elapsed, ncols=ncols, prefix="Test"
            )
            self._compare_bars(tinytqdm_output, tqdm_output)
    ...
    def test_unit_scale(self, mock_terminal_size, mock_stderr):
        for unit_scale in [True, False]:
            ...
            for exponent in range(1, 22, 3):
                ...
                for _ in range(5):
                    total, ncols = random.randint(low, high), random.randint(
                        *NCOLS_RANGE
                    )
                    mock_terminal_size.return_value = namedtuple(
                        field_names="columns", typename="terminal_size"
                    ...
                    for n in tinytqdm(
                        ...
                        tinytqdm_output = (
                            mock_stderr.getvalue().split("\r")[-1].rstrip()
                        )

                        if n:
                            iters_per_sec = _get_iter_per_second(
                                tinytqdm_output.split("it/s")[-2].split(" ")[-1]
                            )
    ...
    def test_trange_output_iter(self, mock_terminal_size, mock_stderr):
        for _ in range(5):
            ...
            for n in (bar := tinytrange(total, desc="Test")):
                ...
                iters_per_sec = (
                    float(tiny_output.split("it/s")[-2].split(" ")[-1]) if n > 0 else 0
                )
                elapsed = n / iters_per_sec if n > 0 else 0
                tqdm_output = tqdm.format_meter(
                    n=n, total=total, elapsed=elapsed, ncols=ncols, prefix="Test"
    ...
    def test_tqdm_output_custom_0_total(self, mock_terminal_size, mock_stderr):
        for _ in range(10):
            ...
            while n < total:
                ...
                elapsed = n / iters_per_sec if n > 0 else 0
                tqdm_output = tqdm.format_meter(
                    n=n, total=0, elapsed=elapsed, ncols=ncols, prefix="Test"
                )
        ...
        for unit_scale in [True, False]:
            for _ in range(5):
                ...
                for n, g in enumerate(
                    tinytqdm(gen, desc="Test", unit_scale=unit_scale, rate=1e9)
                ):
                    assert g == n
                    tinytqdm_output = mock_stderr.getvalue().split("\r")[-1].rstrip()
                    if n:
    ...
    def test_tqdm_write(self, mock_terminal_size, mock_stderr):
        for _ in range(5):
            ...
            for i in tinytqdm(range(10)):
                ...
                self.assertEqual(
                    tinytqdm_out.split("\r\033[K")[-1], tqdm_out.split(f"{i-1}\n")[-1]
                )
