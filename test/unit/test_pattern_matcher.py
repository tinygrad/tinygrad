...
class TestPatternMatcher(unittest.TestCase):
    def test_simple_match(self):
        matcher = PatternMatcher(
            [(UPat(Ops.CONST, name="x", dtype=dtypes.float), lambda x: x)]
        )
    ...
    def test_upat_any(self):
        ...
        matcher = PatternMatcher(
            [
                (
                    UPat.var("a")
                    + UPat.any(UPat.var("x"), UPat.var("y"), UPat.var("z")),
                    test,
                ),
            ]
        )
        ...
        matcher = PatternMatcher(
            [
                (
                    UPat(
                        Ops.MUL,
                        src=[UPat(Ops.CONST, name="c"), UPat(Ops.CONST, arg=2)],
                        name="x",
                    ),
            ...
            [
                (
                    UPat(
                        GroupOp.ALU,
                        name="x",
                        src=(UPat(Ops.CONST, name="y"), UPat(Ops.CONST, name="y")),
                    ),
                    lambda x, y: x,
                )
            ]
    ...
    def test_dtype(self):
        matcher = PatternMatcher(
            [(UPat(Ops.CONST, name="x", dtype=dtypes.float32), lambda x: x)]
        )
        c1 = UOp(Ops.CONST, dtypes.float, arg=1.0)
    ...
    def test_dtype_set(self):
        matcher = PatternMatcher(
            [
                (
                    UPat(Ops.CONST, name="x", dtype={dtypes.float32, dtypes.float64}),
                    lambda x: x,
                )
            ]
        )
    ...
    def test_src_one(self):
        matcher = PatternMatcher(
            [
                (
                    UPat(GroupOp.ALU, name="x", src=(UPat(Ops.CONST), UPat(Ops.CONST))),
                    lambda x: x,
                )
            ]
        )
    ...
    def test_src_permutations(self):
        matcher = PatternMatcher(
            [
                (
                    UPat(
                        GroupOp.ALU, name="x", src=[UPat(Ops.CONST), UPat(GroupOp.ALU)]
                    ),
                    lambda x: x,
                )
            ]
    ...
    def test_src_repeat(self):
        matcher = PatternMatcher(
            [(UPat(GroupOp.ALU, name="x", src=UPat(Ops.CONST)), lambda x: x)]
        )
        c1 = UOp(Ops.CONST, dtypes.float, arg=1.0)
        c2 = UOp(Ops.CONST, dtypes.float, arg=2.0)
        c3 = UOp(Ops.ADD, dtypes.float, (c1, c2))
    ...
    def test_allow_len(self):
        matcher = PatternMatcher(
            [
                (
                    UPat(
                        Ops.MULACC, name="x", src=(UPat(Ops.CONST),), allow_any_len=True
                    ),
                    lambda x: x,
                )
            ]
            ...
            [
                (
                    UPat(
                        GroupOp.ALU,
                        src=[
                            UPat(GroupOp.ALU, src=[UPat(name="a"), UPat(name="b")]),
                            UPat(name="b"),
                        ],
                    ),
                    lambda a, b: b,
    ...
    def _assert_eq_upat(self, a: UPat, b: UPat):
        assert (
            sorted(map(str, a.op))
            if a.op
            else [] == (sorted(map(str, b.op)) if b.op else [])
        )
        assert (
            sorted(a.dtype) if a.dtype else [] == (sorted(b.dtype) if b.dtype else [])
        )
        ...
        def simple_src(u: UPat):
            if u.src is None:
                return []
            if isinstance(u.src, itertools.repeat):
                return next(u.src[0])
            return u.src[0]

        for a, b in zip(simple_src(a), simple_src(b)):
            self._assert_eq_upat(a, b)
