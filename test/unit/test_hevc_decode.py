...
def run_unit_tests():
  ...
  if result.failures:
    ...
    for test, traceback in result.failures:
      tb_parts = traceback.split('AssertionError: ')
      error_msg = tb_parts[-1].split('\n')[0] if 'AssertionError:' in traceback and tb_parts else 'Unknown failure'
      print(f"   {test}: {error_msg}")

  if result.errors:
    print(f"\n💥 Errors:")
    for test, traceback in result.errors:
      tb_parts = traceback.split('Exception: ')
      error_msg = tb_parts[-1].split('\n')[0] if 'Exception:' in traceback and tb_parts else 'Unknown error'
      print(f"   {test}: {error_msg}")
