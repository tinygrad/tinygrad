...
def get_viz_list():
  return get_metadata(traces)


class BaseTestViz(unittest.TestCase):
  def setUp(self):
    # clear the global context
    for lst in [tracked_keys, tracked_ctxs, active_rewrites, _name_cnt]:
      lst.clear()
...
class TestViz(BaseTestViz):
  ...
  def test_rewrite_location(self):
    def inner(sink):
      return graph_rewrite(sink, PatternMatcher([]))
  ...
  def test_default_name(self):
    ...
    @track_rewrites()
    def name_default():
      return graph_rewrite(a, PatternMatcher([]))
  ...
  def test_dyn_name_fxn(self):
    ...
    def name_from_fxn(s: UOp, arg: list | None = None):
      return graph_rewrite(s, PatternMatcher([]))
...
def leaf_rewrite(x: UOp):
  return x.rtag(1) if x.tag is None else None
...
root = TrackedPatternMatcher([
  (UPat(Ops.SINK, src=UPat(Ops.ADD), name="root"), root_rewrite),
])
...
class TestVizTree(BaseTestViz):
  ...
  def test_tree_view(self):
    ...
    @track_rewrites()
    def tree_rewrite():
      return graph_rewrite(sink, root, name="root")
  ...
  for _ in range(layout_len):
    ...
    if event_type == 0:
      ...
      for _ in range(event_count):
        alloc, ts, key = u("<BII")
        if alloc:
          v["events"].append({"event": "alloc", "ts": ts, "key": key, "arg": {"dtype": strings[u("<I")[0]], "sz": u("<Q")[0]}})
        else:
          v["events"].append({"event": "free", "ts": ts, "key": key})
...
class TestVizProfiler(unittest.TestCase):
  def test_perfetto_node(self):
    prof = [
      ProfileRangeEvent(device="NV", name="E_2", st=decimal.Decimal(1000), en=decimal.Decimal(1010), is_copy=False),
  ...
  def test_perfetto_copy_node(self):
    prof = [
      ProfileRangeEvent(device="NV", name="COPYxx", st=decimal.Decimal(1000), en=decimal.Decimal(1010), is_copy=True),
  ...
  def test_perfetto_graph(self):
    prof = [
      ...
      ProfileGraphEvent(
        ents=[
          ProfileGraphEntry(device="NV", name="E_25_4n2", st_id=0, en_id=1, is_copy=False),
