...
from tinygrad.ops import (
    tracked_ctxs as contexts,
    tracked_keys as keys,
    _name_cnt,
    _substitute,
)
...
class TestViz(unittest.TestCase):
    ...
    def test_track_rewrites(self):
        ...
        def do_rewrite(x: UOp):
            return graph_rewrite(x, symbolic)
    ...
    def test_bottom_up_rewrite(self):
        ...
        @track_rewrites(named=True)
        def fxn(sink):
            return graph_rewrite(sink, substitute, ctx={a + b: c}, bottom_up=True)
    ...
    def test_rewrite_without_context(self):
        def untracked_graph_rewrite(sink):
            return graph_rewrite(sink, symbolic)

        @track_rewrites(named=True)
        def tracked_graph_rewrite(sink):
            return graph_rewrite(sink, symbolic)
    ...
    def test_inner_rewrite_location(self):
        ...
        def inner_rewrite(sink):
            return graph_rewrite(sink, symbolic)

        @track_rewrites(named=True)
        def tracked_graph_rewrite(sink):
            return inner_rewrite(sink)
...
class TextVizProfiler(unittest.TestCase):
    def test_perfetto_node(self):
        prof = [
            ProfileRangeEvent(
                device="NV",
                name="E_2",
                st=decimal.Decimal(1000),
                en=decimal.Decimal(1010),
                is_copy=False,
            ),
            ProfileDeviceEvent(
                device="NV",
                comp_tdiff=decimal.Decimal(-1000),
                copy_tdiff=decimal.Decimal(-100),
            ),
        ]
    ...
    def test_perfetto_graph(self):
        prof = [
            ...
            ProfileDeviceEvent(
                device="NV:1",
                comp_tdiff=decimal.Decimal(-500),
                copy_tdiff=decimal.Decimal(-50),
            ),
            ProfileGraphEvent(
                ents=[
                    ProfileGraphEntry(
                        device="NV", name="E_25_4n2", st_id=0, en_id=1, is_copy=False
                    ),
                    ProfileGraphEntry(
                        device="NV:1", name="NV -> NV:1", st_id=2, en_id=3, is_copy=True
                    ),
                ],
                deps=[[], [0]],
                sigs=[
                    decimal.Decimal(1000),
                    decimal.Decimal(1002),
                    decimal.Decimal(1004),
                    decimal.Decimal(1008),
                ],
            ),
        ]
