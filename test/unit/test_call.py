...
class TestCallSchedule(unittest.TestCase):
  ...
  def test_precompile_schedule_cache_hit(self):
    ...
    b = Tensor.empty(4, 8)
    r0, r1 = f(a), f(b)
    c0 = next(u for u in r0.uop.toposort() if u.op is Ops.CALL and u.returned_outputs)
    c1 = next(u for u in r1.uop.toposort() if u.op is Ops.CALL and u.returned_outputs)
