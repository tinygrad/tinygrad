...
def _x86_address(base, idx, disp, disp_dt=dtypes.int8):
  return (UOp(X86Ops.DEFINE_REG, dtypes.int32.ptr(), arg=base), UOp(Ops.NOOP, arg=idx), UOp(X86Ops.IMM, disp_dt, arg=disp))

def x86_reg(dt:DType, reg:Register): return UOp(X86Ops.DEFINE_REG, dt, arg=reg)
...
class TestEncodingsX86(unittest.TestCase):
  ...
  def test_base_address(self):
    load = UOp(X86Ops.MOV, dtypes.int32, _x86_address(RDI, None, 0), RDI)
  ...
  def test_rsp_base_address(self):
    load = UOp(X86Ops.MOV, dtypes.int32, _x86_address(RSP, None, 0), RSP)
  ...
  # make sure that displacement is 8bit and not 32bit
  def test_rbp_base_address(self):
    load = UOp(X86Ops.MOV, dtypes.int32, _x86_address(RBP, None, 0), RBP)
  ...
  def test_base_index_address(self):
    load = UOp(X86Ops.MOV, dtypes.int32, _x86_address(RAX, RDX, 0), RAX)
  ...
  def test_rsp_index_address(self):
    load = UOp(X86Ops.MOV, dtypes.int32, _x86_address(RAX, RSP, 0), RAX)
  ...
  def test_r12_index_address(self):
    load = UOp(X86Ops.MOV, dtypes.int32, _x86_address(RAX, GPR[12], 0), RAX)
  ...
  def test_complex_address_8bit_disp(self):
    load = UOp(X86Ops.MOV, dtypes.int32, _x86_address(RDI, RSI, 10), RDI)
  ...
  def test_complex_address_32bit_disp(self):
    load = UOp(X86Ops.MOV, dtypes.int32, _x86_address(RDI, RSI, 10000, dtypes.int32), RDI)
  ...
  def test_8bit_legacy_encoding(self):
    cast = UOp(X86Ops.MOVSX, dtypes.int32, (x86_reg(dtypes.int8, RDX),), RAX)
    # movsx eax, dl
  ...
  def test_lower_8bits_reg(self):
    cast = UOp(X86Ops.MOVSX, dtypes.int32, (x86_reg(dtypes.int8, RDI),), RAX)
    # movsx eax, dil
  ...
  def test_16bit_legacy_encoding(self):
    cast = UOp(X86Ops.MOVSX, dtypes.int16, (x86_reg(dtypes.int8, RDX),), RAX)
    # movsx ax, dl
  ...
  def test_64bit_legacy_encoding(self):
    cast = UOp(X86Ops.MOVSX, dtypes.int64, (x86_reg(dtypes.int8, RDX),), RAX)
    # movsx rax, dl
  ...
  def test_compact_vex_encoding(self):
    xmm0, xmm1 = x86_reg(dtypes.float32, XMM[0]), x86_reg(dtypes.float32, XMM[1])
  ...
  def test_long_vex_encoding(self):
    xmm0, xmm8 = x86_reg(dtypes.float32, XMM[0]), x86_reg(dtypes.float32, XMM[8])
  ...
  def test_ymm_encoding(self):
    xmm0, xmm1 = x86_reg(dtypes.float32.vec(8), XMM[0]), x86_reg(dtypes.float32.vec(8), XMM[1])
  ...
  def test_reg_in_imm_field(self):
    xmm0, xmm1, xmm2 = x86_reg(dtypes.float32, XMM[0]), x86_reg(dtypes.float32, XMM[1]), x86_reg(dtypes.float32, XMM[2])
  ...
  def test_write_mem(self):
    base, index, disp = x86_reg(dtypes.int32.ptr(), RDI), x86_reg(dtypes.int32, RSI), imm(dtypes.int8, 10)
    xmm0 = x86_reg(dtypes.float32, XMM[0])
  ...
  def test_disp_imm_same_value(self):
    base, index, disp = x86_reg(dtypes.int8.ptr(), RDI), x86_reg(dtypes.int8, RSI), imm(dtypes.int8, 10)
    ...
    base, index, disp = x86_reg(dtypes.int32.ptr(), RDI), x86_reg(dtypes.int32, RSI), imm(dtypes.int32, 10)
  ...
  def test_cmove_ignore_cmp(self):
    cmove = UOp(X86Ops.CMOVE, dtypes.int32, (x86_reg(dtypes.int32, RAX), UOp(X86Ops.CMP)), RDX)
    # cmove edx, eax
