...
class TestTorchLoad(unittest.TestCase):
  ...
  def test_load_enet(self):
    compare_weights_both("https://github.com/lukemelas/EfficientNet-PyTorch/releases/download/1.0/efficientnet-b0-355c32eb.pth")

  # pytorch zip format
  def test_load_enet_alt(self):
    compare_weights_both("https://download.pytorch.org/models/efficientnet_b0_rwightman-3dd342df.pth")

  # pytorch zip format
  def test_load_convnext(self):
    compare_weights_both("https://dl.fbaipublicfiles.com/convnext/convnext_tiny_1k_224_ema.pth")

  @unittest.skipUnless(is_dtype_supported(dtypes.float16), "need float16 support")
  def test_load_llama2bfloat(self):
    compare_weights_both("https://huggingface.co/qazalin/bf16-lightweight/resolve/main/consolidated.00.pth?download=true")

  # pytorch tar format
  def test_load_resnet(self):
    compare_weights_both("https://download.pytorch.org/models/resnet50-19c8e357.pth")
...
class TestSafetensors(unittest.TestCase):
  ...
  def test_save_all_dtypes(self):
    for dtype in dtypes.fields().values():
      ...
      if dtype in [dtypes.double, *dtypes.fp8s] and Device.DEFAULT == "METAL":
...
class TestDiskTensor(unittest.TestCase):
  ...
  def test_copy_to_cpu_not_truncated(self):
    with open((fn := temp("dt_copy_to_cpu_not_truncated")), "wb") as f:
      f.write(b"\x01" * (size := int(2 * 1024**3)) + (test := b"test"))
