...
class TestGradient(unittest.TestCase):
    ...
    def _test_one_input_function(self, f: Callable, jf: Callable | None = None):
        ...
        for val in [-5.0, -2.0, 0.0, 2.0, 5.0]:
            tg_out, jax_out = (
                gx.substitute({x: x.const_like(val)}).ssimplify(),
                gf(val).item(),
            )
    ...
    def test_sin(self):
        self._test_one_input_function(lambda x: x.sin(), lambda x: jnp.sin(x))

    def test_sqrt(self):
        self._test_one_input_function(lambda x: x.sqrt(), lambda x: jnp.sqrt(x))

    def test_log2(self):
        self._test_one_input_function(lambda x: x.log2(), lambda x: jnp.log2(x))

    def test_exp2(self):
        self._test_one_input_function(lambda x: x.exp2(), lambda x: jnp.exp2(x))
    ...
    # chain rule
    def test_chain(self):
        self._test_one_input_function(
            lambda x: x.sin().sqrt(), lambda x: jnp.sqrt(jnp.sin(x))
        )
    ...
    def test_big_add_sin(self):
        self._test_two_input_function(
            lambda x, y: x.sin() + 3.0 / y, lambda x, y: jnp.sin(x) + 3.0 / y
        )
    ...
    def test_where(self):
        self._test_two_input_function(
            lambda x, y: (x < y).where(x, y), lambda x, y: jnp.where(x < y, x, y)
        )
...
class TestTensorGradient(unittest.TestCase):
    def test_example(self):
        ...
        self.assertListEqual(
            dx.tolist(), [[2.0, 2.0, 2.0], [0.0, 0.0, 0.0], [-2.0, -2.0, -2.0]]
        )
