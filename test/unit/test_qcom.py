...
class TestQcom(unittest.TestCase):
    def test_image_pitch(self):
        ...
        def __validate(imgdt, expected_pitch):
            img = dev.allocator.alloc(
                imgdt.shape[0] * imgdt.shape[1] * 16, options := BufferSpec(image=imgdt)
            )
            pitch = (img.descriptor[2] & 0x1FFFFF80) >> 7
            assert (
                pitch == expected_pitch
            ), f"Failed pitch for image: {imgdt}. Got 0x{pitch:X}, expected 0x{expected_pitch:X}"
