...
def is_pattern_uop(u: UOp, pat: UPat):
  assert pat.match(u, {}), f"{u}\nis not\n{pat}"


def is_pattern(ten: Tensor, pat: UPat):
  is_pattern_uop(ten.uop, pat)
...
class TestTensorMutates(unittest.TestCase):
  def test_mutate_add(self):
    ...
    for t in [a, b, ret]:
      is_pattern_uop(t.uop.base, realized_pattern)
...
class TestTensorUopRepresentation(unittest.TestCase):
  ...
  def test_empty_buf(self):
    ...
    is_pattern(
      ...
      UPat(
        Ops.RESHAPE,
        src=(UPat(Ops.SHRINK, src=(UPat(Ops.BUFFER),))),
      ),
    )
