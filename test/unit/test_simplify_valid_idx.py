...
from tinygrad.uop.symbolic import simplify_valid, simplify_mod_valid
...
class TestValidIdxSimplification(unittest.TestCase):
  ...
  def test_simplify_mod_valid_ge(self):
    # (r0*2+3)%4 >= 2: r0=0 -> 3>=2 ✓, r0=1 -> 1>=2 ✗ => r0 < 1
    r0 = Range(0, 2)
    valid = (((r0*2)+3)%4 < 2).ne(True)
    result = simplify_mod_valid(valid)
    self.assertIsNotNone(result)
    self.assertEqual(result.render(), "(r0<1)")

  def test_simplify_mod_valid_lt(self):
    # (r0*2+1)%4 < 2: r0=0 -> 1<2 ✓, r0=1 -> 3<2 ✗ => r0 < 1
    r0 = Range(0, 2)
    valid = ((r0*2)+1)%4 < 2
    result = simplify_mod_valid(valid)
    self.assertIsNotNone(result)
  ...
  def test_simplify_mod_valid_no_simplify(self):
    # range too large - can't simplify by checking only endpoints
    r0 = Range(0, 4)
    valid = ((r0*2)+3)%8 < 4
    self.assertIsNone(simplify_mod_valid(valid))
