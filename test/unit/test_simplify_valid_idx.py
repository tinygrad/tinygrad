...
def get_gated_load_uop(valid: UOp, idx: UOp):
  return UOp(Ops.LOAD, dtypes.float, (UOp(Ops.DEFINE_GLOBAL, dtypes.float.ptr(), arg=0).index(idx, valid), UOp.const(dtypes.float, 0.0)))
...
class TestValidIdxSimplification(unittest.TestCase):
  ...
  def test_simplify_within_valid1(self):
    ...
    self.check(load, "(((ridx0+ridx1)+ridx2)+ridx3)", "((((ridx0*3)+ridx1)<8)&((((ridx2*3)+ridx3)%4)<2))")
  ...
  def test_valid_becomes_const1(self):
    ...
    load = get_gated_load_uop(valid, idx)
    self.check(load, "(ridx0*1568)", "((ridx2<1)&(ridx1<6))")
  ...
  def test_valid_becomes_const2(self):
    ...
    load = get_gated_load_uop(valid, idx)
    self.check(load, "1", "((((ridx0+ridx1)<1)!=True)&(((ridx2+ridx3)<1)!=True))")
