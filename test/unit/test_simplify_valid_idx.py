...
def get_load_image_uop(image_shape: tuple[int, ...], valid: UOp, idx: tuple[UOp, UOp]):
    return UOp(
        ...
        dtypes.float.vec(4),
        (
            UOp(Ops.DEFINE_GLOBAL, dtypes.imagef(image_shape), arg=0).index(
                UOp(Ops.VECTORIZE, dtypes.int.vec(2), idx), valid
            ),
            UOp(
                Ops.VECTORIZE,
                dtypes.float.vec(4),
                src=(UOp.const(dtypes.float, 0.0),) * 4,
            ),
        ),
    )


def Special(expr, nmax):
    return UOp(Ops.SPECIAL, dtypes.int, (), (expr, nmax))


def Variable(expr, nmin, nmax):
    return UOp.variable(expr, nmin, nmax)
...
class TestValidIdxSimplification(unittest.TestCase):
    ...
    def test_cumsum(self):
        ...
        self.check(load, "0", "(((lidx0+(gidx0*4))<19)!=True)")
    ...
    def test_valid_order_matters2(self):
        ...
        for v in itertools.permutations([v0, v1, v2, v3]):
            self.assertEqual(
                simplify_valid(v[0] & v[1] & v[2] & v[3]).render(), "False"
            )
    ...
    def test_from_merge_views(self):
        ...
        load = get_gated_load_uop(valid, idx)
        self.check(load, "(((ridx0*2)+(ridx3*-1))+1)", "(ridx2<1)")
...
class TestImageSimplification(unittest.TestCase):
    def check(self, load, svalid, sidx0, sidx1):
        ...
        if svalid is not None:
            self.assertEqual(load.src[2].render(simplify=False), svalid)
    ...
    def test_openpilot_conv1(self):
        ...
        valid = ((((idx2 * 2) + (ridx1)) < 1).ne(True)) & (
            (((idx1 * 8) + (ridx2)) < 1).ne(True)
        )
        shape = (128, 1536, 4)
        idx = (
            (alu4 + 1530) % 1536,
            alu1 + ((idx1 + ((ridx2 + 7) // 8) + 31) // 32) + (-2),
        )

        load = get_load_image_uop(shape, valid, idx)
        self.check(
            load, None, "((((idx1*48)+(ridx2*6))+ridx0)+-6)", "(((idx2*2)+ridx1)+-1)"
        )
    ...
    def test_openpilot_conv2(self):
        ...
        self.check(
            load, None, "((((idx1*24)+(ridx2*3))+ridx0)+-3)", "(((idx2*2)+ridx1)+-1)"
        )
    ...
    def test_simplify4(self):
        ...
        load = get_load_image_uop(
            shape, alu9, (((alu8 + (alu2 * 8)) % 64), (alu2 // 8))
        )
        self.check(
            load, "(idx0<256)", "(((((idx0%8)*32)+(idx0//32))+8)%64)", "((idx0%8)//2)"
        )

        load = get_load_image_uop(
            shape, alu9, (((alu8 + (alu3 * 8)) % 64), (alu3 // 8))
        )
        self.check(
            load, "(idx0<256)", "(((((idx0%8)*32)+(idx0//32))+16)%64)", "((idx0%8)//2)"
        )

        load = get_load_image_uop(
            shape, alu9, (((alu8 + (alu4 * 8)) % 64), (alu4 // 8))
        )
        self.check(
            load, "(idx0<256)", "(((((idx0%8)*32)+(idx0//32))+24)%64)", "((idx0%8)//2)"
        )

        load = get_load_image_uop(
            shape, alu9, (((alu8 + (alu5 * 8)) % 64), (alu5 // 8))
        )
        self.check(
            load, "(idx0<256)", "((((idx0%8)*32)+(idx0//32))%64)", "((idx0%8)//2)"
        )
