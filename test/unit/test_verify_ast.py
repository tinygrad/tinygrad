...
def helper_test_verify_ast(*stores: UOp) -> Kernel:
    ...
    if DEBUG >= 3:
        for op in stores:
            print(op)
    try:
        type_verify(list(sink.toposort), shape_spec)
    except RuntimeError as e:
        raise InvalidASTException(e.args)
    k = Kernel(sink)
    k.linearize()
    if DEBUG >= 6:
        print_uops(k.uops)
...
class TestVerifyAST(unittest.TestCase):
    def test_tiny_add(self):
        ...
        store = UOp(
            Ops.STORE,
            dtypes.void,
            (buf_0, ShapeTracker.from_shape((32, 1)).to_uop(), a + b),
        )
    ...
    def test_exactly_one_full_shape(self):
        ...
        with self.assertRaises(InvalidASTException):
            helper_test_verify_ast(st0, st1)

    def test_no_implicit_broadcasting(self):
        bufs = [UOp(Ops.DEFINE_GLOBAL, dtypes.float.ptr(), (), i) for i in range(2)]
        a = UOp(
            Ops.LOAD, dtypes.float, (bufs[1], ShapeTracker.from_shape((4, 32)).to_uop())
        )
        b = a + UOp(Ops.REDUCE_AXIS, dtypes.float, (a,), (Ops.MAX, (1,)))
        st = UOp(
            Ops.STORE,
            dtypes.void,
            (bufs[0], ShapeTracker.from_shape((4, 32)).to_uop(), b),
        )
        with self.assertRaises(InvalidASTException):
    ...
    def test_reduce_store(self):
        bufs = [UOp(Ops.DEFINE_GLOBAL, dtypes.float.ptr(), (), i) for i in range(2)]
        a = UOp(
            Ops.LOAD, dtypes.float, (bufs[1], ShapeTracker.from_shape((32, 1)).to_uop())
        )
    ...
    def test_assert_swizzle(self):
        ...
        a = UOp(
            Ops.LOAD, dtypes.float, (buf, ShapeTracker.from_shape((32, 1)).to_uop())
        )
        r = UOp(Ops.REDUCE_AXIS, dtypes.float, (a,), (Ops.ADD, (0,)))
        st = UOp.store(
            buf,
            ShapeTracker.from_shape((32, 1)).to_uop(),
            r.view(r.st.expand((32, 1))) + a,
        )
        with self.assertRaisesRegex(InvalidASTException, "swizzle"):
            helper_test_verify_ast(st)

    def test_const_view_always_valid(self):
        buf = UOp(Ops.DEFINE_GLOBAL, dtypes.float.ptr(), (), 0)
        a = UOp.const(dtypes.int, 0).replace(
            src=(
                UOp(
                    Ops.VIEW,
                    dtypes.void,
                    (UOp(Ops.DEVICE, arg="CLANG"),),
