...
class MultiShapeTracker:
    def __init__(self, sts: list[ShapeTracker]):
        self.sts = sts
    ...
    def reshape(self, arg):
        self.sts = [x.reshape(arg) for x in self.sts]

    def permute(self, arg):
        self.sts = [x.permute(arg) for x in self.sts]

    def expand(self, arg):
        self.sts = [x.expand(arg) for x in self.sts]

    def shrink(self, arg):
        self.sts = [x.shrink(arg) for x in self.sts]

    def flip(self, arg):
        self.sts = [x.flip(arg) for x in self.sts]

    def pad(self, arg):
        self.sts = [x.pad(arg) for x in self.sts]
...
class TestShapeTrackerBasics(unittest.TestCase):
    ...
    def test_simplify_is_correct(self):
        multiv = ShapeTracker(
            views=(
                View(
                    shape=(15, 3), strides=(9, 1), offset=6, mask=None, contiguous=False
                ),
                View(
                    shape=(4, 3), strides=(12, 4), offset=0, mask=None, contiguous=False
                ),
            )
        )
...
class TestShapeTrackerAdd(unittest.TestCase):
    ...
    def test_off_by_one(self):
        ...
        st2 = ShapeTracker(
            views=(
                View(shape=(4,), strides=(1,), offset=0, mask=None, contiguous=True),
                View(shape=(5,), strides=(1,), offset=0, mask=None, contiguous=True),
            )
        )
...
class TestShapeTrackerAddVariable(unittest.TestCase):
    ...
    def test_merge_symbolic_views_2(self):
        ...
        vm1 = View(
            shape=(var_i, var_j), strides=(0, 0), offset=0, mask=None, contiguous=False
        )
        vm2 = View(
            shape=(var_i, var_j),
            strides=(var_j, 1),
