...
def is_toposorted(lst: list[UOp]):
  seen = set()
  for u in lst:
    if any(p not in seen for p in u.src):
      return False
    seen.add(u)
  return True
...
class TestBlockReorder(unittest.TestCase):
  ...
  def test_loads(self):
    ...
    self.assertListEqual(
      [g.src[0].src[1].render() for g in golden if g.op is Ops.LOAD],
      ["(gidx1*4)", "((gidx1*4)+1)", "((gidx1*4)+2)", "((gidx1*4)+3)", "(gidx0*27)", "((gidx0*27)+1)", "((gidx0*27)+2)", "((gidx0*27)+3)"],
    )
