...
from tinygrad.nn.state import RawTensorIO

class TestRawTensorIO(unittest.TestCase):
  def test_create(self):
    with self.assertRaises(ValueError):
      RawTensorIO(Tensor(b"Hello World").reshape(1, -1))
    with self.assertRaises(ValueError):
      RawTensorIO(Tensor([], dtype=dtypes.int64).reshape(1, -1))
