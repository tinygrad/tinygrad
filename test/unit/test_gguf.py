...
def ggml_tensor_to_numpy(tensor: ggml.ggml_tensor_p):
    ...
    ggml_type, n_dims, n_els = (
        tensor.contents.type,
        ggml.ggml_n_dims(tensor),
        ggml.ggml_nelements(tensor),
    )
    shape = tuple(reversed(tensor.contents.ne[:n_dims]))
    if ggml_type not in ggml_type_to_np_dtype:
        ctx = ggml.ggml_init(
            ggml.ggml_init_params(mem_size=n_els * 5 + 500, mem_buffer=None)
        )
        ntensor = ggml.ggml_new_tensor(
            ctx, ggml.GGML_TYPE_F32, n_dims, tensor.contents.ne
        )
        type_traits = ggml.ggml_internal_get_type_traits(ggml_type)
        type_traits.to_float(
            ggml.ggml_get_data(tensor), ggml.ggml_get_data_f32(ntensor), n_els
        )
        tensor, ggml_type = ntensor, ggml.GGML_TYPE_F32

    np_type = ggml_type_to_np_dtype[ggml_type]
    ctypes_type = np_dtype_to_ctype.get(np_type, None) or np.ctypeslib.as_ctypes_type(
        np_type
    )
...
@unittest.skipIf(
    any(not is_dtype_supported(t) for t in [dtypes.uint8, dtypes.half]),
    "Backend must support uint8 and half",
)
class TestGGUF(unittest.TestCase):
    ...
    def test_load_tinyllama_q8_0(self):
        self._test_gguf_load(
            "https://huggingface.co/ggml-org/models/resolve/main/tinyllamas/stories15M-q8_0.gguf?download=true"
        )

    def test_load_tinyllama_q4_0(self):
        self._test_gguf_load(
            "https://huggingface.co/ggml-org/models/resolve/main/tinyllamas/stories15M-q4_0.gguf?download=true"
        )

    def test_load_gpt2_q4_1(self):
        self._test_gguf_load(
            "https://huggingface.co/PrunaAI/gpt2-GGUF-smashed/resolve/main/gpt2.Q4_1.gguf?download=true"
        )

    def test_load_sample_q6_k(self):
        self._test_gguf_load(
            "https://huggingface.co/Isotr0py/test-gguf-sample/resolve/main/Quant_Q6_K_1024.gguf?download=true"
        )
    ...
    def _test_dequantization(self, ttype: int):
        ...
        n_el, n_bytes = (
            ggml_test_block_count * type_traits.blck_size,
            ggml_test_block_count * type_traits.type_size,
        )

        data_in = (
            np.random.random((n_el,)).astype(np.float32) * 100 - 50
        ...
        np.testing.assert_equal(
            dq_tensor.numpy(), np.frombuffer(c_dq_data, dtype=np.float32)
        )
    ...
    def _test_gguf_load(self, url: str):
        ...
        gguf_tensor = Tensor.empty(
            model_size, dtype=dtypes.uint8, device=f"disk:{fp}"
        ).to(Device.DEFAULT)
