...
class TestSignal(unittest.TestCase):
  def test_stft_reconstruction(self):
    ...
    y = stft(x)
    x_hat = stft(y, inverse=True)
    np.testing.assert_allclose(x[:, :x_hat.shape[1]].numpy(),
                               x_hat.numpy(),
                               atol=1e-3)  # TODO: improve precision

  def test_stft_spectrogram(self):
    x = Tensor.ones(2, 2000)
    stft = STFT(n_fft=512, window="hann", hop_length=160, win_length=320)
    spec = stft(x, return_spec=True)
    np.testing.assert_((spec.numpy() > 0).all())

  def test_log_mel_spectrogram(self):
    x = Tensor.ones(2, 2000)
    mel_spec_layer = MelSpectrogram(n_mels=80,
                                    n_fft=512,
                                    window="hann",
                                    hop_length=160,
                                    win_length=320,
