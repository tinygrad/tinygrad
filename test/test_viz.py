...
from tinygrad.runtime.support.hcq import ProfileEvent, ProfileDeviceEvent, ProfileRangeEvent, ProfileGraphEvent, ProfileGraphEntry
...
class TextVizProfiler(unittest.TestCase):
  def test_perfetto_node(self):
    ...
    self.assertEqual(j['traceEvents'][0]['pid'], 0)
    self.assertEqual(j['traceEvents'][0]['args']['name'], 'NV')

    self.assertEqual(j['traceEvents'][1]['name'], 'E_2')
    self.assertEqual(j['traceEvents'][1]['ts'], 0)
    self.assertEqual(j['traceEvents'][1]['dur'], 10)
  ...
  def test_perfetto_copy_node(self):
    ...
    self.assertEqual(j['traceEvents'][1]['name'], 'COPYxx')
    self.assertEqual(j['traceEvents'][1]['ts'], 900) # diff clock
    self.assertEqual(j['traceEvents'][1]['dur'], 10)
    self.assertEqual(j['traceEvents'][1]['ph'], 'X')
    self.assertEqual(j['traceEvents'][1]['pid'], 0)
    self.assertEqual(j['traceEvents'][1]['tid'], 1)
  ...
  def test_perfetto_graph(self):
    ...
    # Device regs always first
    self.assertEqual(j['traceEvents'][0]['args']['name'], 'NV')
    self.assertEqual(j['traceEvents'][1]['args']['name'], 'NV:1')

    self.assertEqual(j['traceEvents'][2]['name'], 'E_25_4n2')
    self.assertEqual(j['traceEvents'][2]['ts'], 0)
    self.assertEqual(j['traceEvents'][2]['dur'], 2)

    self.assertEqual(j['traceEvents'][3]['name'], 'NV -> NV:1')
    self.assertEqual(j['traceEvents'][3]['ts'], 954)
    self.assertEqual(j['traceEvents'][3]['dur'], 4)
