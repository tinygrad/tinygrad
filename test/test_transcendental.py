...
class TestTranscendentalMath(unittest.TestCase):
    @unittest.skipUnless(
        is_dtype_supported(dtypes.float64, Device.DEFAULT),
        f"no float64 on {Device.DEFAULT}",
    )
    @unittest.skipIf(getenv("MOCKGPU") and Device.DEFAULT in {"NV", "CUDA"}, "crashed")
    @given(
        ht.float64,
        strat.sampled_from(
            [(Tensor.exp, np.exp), (Tensor.log, np.log), (Tensor.sin, np.sin)]
        ),
    )
    def test_float64(self, x, op):
        ...
        with Context(TRANSCENDENTAL=2), np.errstate(all="ignore"):
            np.testing.assert_allclose(
                op[0](Tensor([x], dtype=dtypes.float64)).numpy(),
    ...
    @given(
        ht.float32,
        strat.sampled_from(
            [(Tensor.exp, np.exp), (Tensor.log, np.log)]
    ...
    def test_float32(self, x, op):
        ...
        if (
            (math.isnan(x) or (x < 0 and op[0] == Tensor.log))
            and CI
        ...
        with Context(TRANSCENDENTAL=2), np.errstate(all="ignore"):
            np.testing.assert_allclose(
                op[0](Tensor([x], dtype=dtypes.float32)).numpy(),
    ...
    @unittest.skipUnless(
        is_dtype_supported(dtypes.float16, Device.DEFAULT),
        f"no float16 on {Device.DEFAULT}",
    )
    @given(
        ht.float16,
        strat.sampled_from(
    ...
    def test_float16(self, x, op):
        ...
        with Context(TRANSCENDENTAL=2), np.errstate(all="ignore"):
            np.testing.assert_allclose(
                op[0](Tensor([x], dtype=dtypes.float16)).numpy(),
    ...
    @given(
        strat.sampled_from(
            [(dtypes.float64, 709.5), (dtypes.float32, 88.7), (dtypes.float16, 11)]
            if Device.DEFAULT != "WEBGPU"
            else [
                (dtypes.float64, 709.5),
                (dtypes.float32, 88.3),
                (dtypes.float16, 10.7),
            ]
        )
...
class TestFromFuzzer(unittest.TestCase):
    ...
    def test_sin(self, dtype):
        ...
        def _test_value(n: float, unit: float = 1.0):
            ...
            with Context(TRANSCENDENTAL=2):
                np.testing.assert_allclose(
                    Tensor([n], dtype=dtype).sin().numpy(),
                    np.sin(np.array([n], dtype=_to_np_dtype(dtype))),
                    atol=ulp,
                    rtol=1e-5,
                )
    ...
    @unittest.skipIf(
        ...
        "Nan location mismatch on Vulkan, Metal works",
    )
    def test_log2(self, dtype):
        ...
        def _test_value(n: float, unit: float = 1.0):
            ...
            with Context(TRANSCENDENTAL=2):
                np.testing.assert_allclose(
                    Tensor([n], dtype=dtype).log2().numpy(),
                    np.log2(np.array([n], dtype=_to_np_dtype(dtype))),
                    atol=ulp,
                    rtol=1e-5,
                )
