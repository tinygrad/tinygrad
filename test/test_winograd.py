...
class TestWinograd(unittest.TestCase):
    ...
    def test_speed(self):
        ...
        for i, s in enumerate(sched):
            ...
            if DEBUG >= 2:
                print(
                    f"{len(l.sts):4d} shapetrackers with max {max(len(x.views) for x in l.sts)} views"
                )
            for st in l.sts:
    ...
    def test_dtype(self):
        ...
        x, w = Tensor.empty(1, IC, Y, X, dtype=dtypes.half), Tensor.empty(
            OC, IC, 3, 3, dtype=dtypes.half
        )
