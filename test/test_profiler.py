...
def helper_collect_profile(*devs):
    ...
    Compiled.profile_events = [
        x
        for x in Compiled.profile_events
        if isinstance(x, ProfileDeviceEvent) and x.device.startswith("METAL")
    ]
...
def helper_profile_filter_device(profile, device: str):
    assert any(
        getattr(x, "device", None) == device and isinstance(x, ProfileDeviceEvent)
        for x in profile
    ), f"device {device} is not registred"
    dev_events = [
        x
        for x in profile
        if getattr(x, "device", None) == device and isinstance(x, ProfileDeviceEvent)
    ]
...
@unittest.skipUnless(
    issubclass(type(Device[Device.DEFAULT]), HCQCompiled)
    or Device.DEFAULT in {"METAL"},
    "HCQ device required to run",
)
class TestProfiler(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        TestProfiler.a = Tensor([0.0, 1.0], device=Device.DEFAULT).realize()
    ...
    def test_profile_kernel_run(self):
        ...
        with helper_collect_profile(TestProfiler.d0) as profile:
            TestProfiler.runner(
                [TestProfiler.b.lazydata.buffer, TestProfiler.a.lazydata.buffer],
                var_vals={},
            )
    ...
    def test_profile_copyin(self):
        buf1 = Buffer(
            Device.DEFAULT, 2, dtypes.float, options=BufferSpec(nolru=True)
        ).ensure_allocated()
    ...
    def test_profile_multidev(self):
        ...
        buf2 = Buffer(
            f"{Device.DEFAULT}:1", 2, dtypes.float, options=BufferSpec(nolru=True)
        ).ensure_allocated()
    ...
    @unittest.skipIf(
        Device.DEFAULT in "METAL" or (MOCKGPU and Device.DEFAULT == "AMD"),
        "AMD mockgpu does not support queue wait interrupts",
    )
    ...
    @unittest.skipIf(
        CI or not issubclass(type(Device[Device.DEFAULT]), HCQCompiled), "skip CI"
    )
    def test_dev_jitter_matrix(self):
        ...
        def _sync_d2d(d1: HCQCompiled, d2: HCQCompiled):
            d1.hw_compute_queue_t().signal(d1.timeline_signal, d1.timeline_value).wait(
                d2.timeline_signal, d2.timeline_value
            ).timestamp(d1.timeline_signal).signal(
                d1.timeline_signal, d1.timeline_value + 1
            ).submit(
                d1
            )
            d2.hw_compute_queue_t().signal(d2.timeline_signal, d2.timeline_value).wait(
                d1.timeline_signal, d1.timeline_value
            ).timestamp(d2.timeline_signal).signal(
                d2.timeline_signal, d2.timeline_value + 1
            ).submit(
                d2
            )
            d1.timeline_value += 2
        ...
        pairs = [
            (p1, p2) for p1 in enumerate(devs) for p2 in enumerate(devs) if p1 != p2
        ]
        for (i1, d1), (i2, d2) in pairs:
            cpu_diff = d1.gpu2cpu_compute_time_diff - d2.gpu2cpu_compute_time_diff
            jitter_matrix[i1][i2] = (
                statistics.median(
                    _sync_d2d(d1, d2) - _sync_d2d(d2, d1) for _ in range(20)
                )
                / 2
                - cpu_diff
            )
        ...
        print(
            "pairwise clock jitter matrix (us):\n"
            + "\n".join(
                [
                    "".join([f"{float(item):8.3f}" for item in row])
                    for row in jitter_matrix
                ]
            )
        )
