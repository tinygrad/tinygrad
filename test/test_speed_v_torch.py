...
from tinygrad.helpers import colored, getenv, DEBUG, print_ci
...
def helper_test_generic(name, f1, f1_args, f2, f2_args):
  ...
  flops = save_ops*1e-6
  mem = save_mem*1e-6
  print_ci(f"{prefix}{name:42s} {et_torch:7.2f} ms ({flops/et_torch:8.2f} GFLOPS {mem/et_torch:8.2f} GB/s) in torch, {et_tinygrad:7.2f} ms ({flops/et_tinygrad:8.2f} GFLOPS {mem/et_tinygrad:8.2f} GB/s) in tinygrad, {colorize_float(et_tinygrad/et_torch)} {desc} {flops:10.2f} MOPS {mem:8.2f} MB")
  prefix = " "
