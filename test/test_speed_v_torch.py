...
def helper_test_conv(bs, in_chans, out_chans, kernel_size, img_size_y, img_size_x):
  ...
  torch_conv = torch.nn.Conv2d(in_chans, out_chans, kernel_size, bias=None, dtype=torch.float if (str(torch_device) == 'cpu' and not HALF) else torch.float16).to(torch_device)
