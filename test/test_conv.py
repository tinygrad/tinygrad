...
class TestConv(unittest.TestCase):
  ...
  def test_two_binops_no_rerun(self):
    ...
    np.testing.assert_array_equal(r1.numpy(), np.maximum(out.numpy(), 0))
    np.testing.assert_array_equal(r2.numpy(), out.numpy() - 1)
    Tensor.no_grad = False
  ...
  def test_two_overlapping_binops_no_rerun(self):
    ...
    np.testing.assert_array_equal(r1.numpy(), np.maximum(out.numpy(), 0))
    np.testing.assert_array_equal(r2.numpy(), np.where(out.numpy() > 0, out.numpy(), (np.exp(out.numpy()) - 1)))
    Tensor.no_grad = False
