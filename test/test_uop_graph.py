...
class TestGraphRewrite(unittest.TestCase):
  def test_dedup(self):
    v1 = UOp(UOps.DEFINE_VAR, dtypes.float, arg=Variable('v', 0, 1))
    v2 = UOp(UOps.DEFINE_VAR, dtypes.float, arg=Variable('v', 0, 1))
  ...
  def test_depth_2_fold(self):
    v = UOp(UOps.DEFINE_VAR, dtypes.float, arg=Variable('v', 0, 1))
    c1 = UOp.const(dtypes.float, 1.0)
    c2 = UOp.const(dtypes.float, 2.0)
