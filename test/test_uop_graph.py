...
class TestUOpGraph(unittest.TestCase):
  ...
  def test_gep_vec_fold(self):
    ...
    def _test_vec(geps, count=4):
      ...
      out = UOp(UOps.STORE, dtypes.void, (d0.index(idx), vec))
      uops = to_uops_list([out])
      if DEBUG >= 4:
    ...
    # possible
    val = UOp(UOps.LOAD, dtypes.float.vec(4), (d1.index(idx),))
    ...
    # different_size
    val = UOp(UOps.LOAD, dtypes.float.vec(2), (d1.index(idx),))
    ...
    # different vals
    val1 = UOp(UOps.LOAD, dtypes.float.vec(2), (d1.index(idx),))
    val2 = UOp(UOps.LOAD, dtypes.float.vec(2), (d2.index(idx),))
  ...
  def test_cast_alu_fold(self):
    ...
    idx = UOp.const(dtypes.int, 0)
    ld = UOp(UOps.LOAD, dtypes.int, (d1.index(idx),))
    alu = ld.lt(1).cast(dtypes.bool)
    out = UOp(UOps.STORE, dtypes.void, (d0.index(idx), alu))
    uops = to_uops_list([out])
  ...
  def test_fold_gated_load(self):
    ...
    idx = UOp.const(dtypes.int, 0)
    ld0 = UOp(UOps.LOAD, dtypes.int, (glbl1.index(idx, UOp.const(dtypes.bool, False)),))
    ld1 = UOp(UOps.LOAD, dtypes.int, (glbl2.index(idx, UOp.const(dtypes.bool, True)),))
    uops = to_uops_list([UOp(UOps.STORE, dtypes.void, (glbl0.index(idx), ld1+ld0))])
    ld0 = uops[-1].src[-1]
  ...
  def test_fold_gated_load_local(self):
    ...
    st = UOp(UOps.STORE, dtypes.void, (smem.index(lidx), UOp.load(glbl0.index(lidx), dtype=dtypes.int)))
    barrier = UOp(UOps.BARRIER, dtypes.void, (st, ))
    ld0 = UOp(UOps.LOAD, dtypes.int, (smem.index(lidx+1, UOp.const(dtypes.bool, False)), barrier))
    ld1 = UOp(UOps.LOAD, dtypes.int, (smem.index(lidx+2, UOp.const(dtypes.bool, True)), barrier))
    uops = to_uops_list([UOp(UOps.STORE, dtypes.void, (glbl0.index(lidx), ld1+ld0))])
    ld0 = uops[-1].src[-1]
    # the gate and invalid value are deleted from ld1
  ...
  def test_fold_gated_store(self):
    ...
    st0 = UOp(UOps.STORE, dtypes.void, (glbl.index(idx0, UOp.const(dtypes.bool, False)), val))
    st1 = UOp(UOps.STORE, dtypes.void, (glbl.index(idx1, UOp.const(dtypes.bool, True)), val))
  ...
  def test_switched_range_order(self):
    ...
    store = UOp(UOps.STORE, dtypes.void, (glbl.index(alu), cf))
...
class TestLoadStoreFolder(unittest.TestCase):
  def test_simple_load_fold(self):
    ...
    load = [UOp(UOps.LOAD, dtypes.float, (buf.index(UOp.const(dtypes.int, i)),)) for i in range(4)]
  ...
  def test_two_load_fold(self):
    buf = UOp(UOps.DEFINE_GLOBAL, dtypes.float.ptr())
    load = [UOp(UOps.LOAD, dtypes.float, (buf.index(UOp.const(dtypes.int, i)),)) for i in range(8)]
    sink = UOp(UOps.VECTORIZE, dtypes.float.vec(len(load)), tuple(load))
    sink = float4_rewrite(sink.sink())
  ...
  def test_simple_load_fold_gated(self):
    ...
    load = [UOp(UOps.LOAD, dtypes.float, (buf.index(UOp.const(dtypes.int, i), gate),)) for i in range(4)]
    sink = UOp(UOps.VECTORIZE, dtypes.float.vec(len(load)), tuple(load))
    sink = float4_rewrite(sink.sink())
    assert len([x for x in sink.sparents if x.op is UOps.LOAD]) == 1
  ...
  def test_simple_load_dont_fold_different_gated(self):
    ...
    load = [UOp(UOps.LOAD, dtypes.float, (buf.index(UOp.const(dtypes.int, i), gate if i == 0 else gate2),
  ...
  def test_simple_store_fold(self):
    buf = UOp(UOps.DEFINE_GLOBAL, dtypes.float.ptr())
    load = [UOp(UOps.STORE, dtypes.float, (buf.index(UOp.const(dtypes.int, i)), UOp.const(dtypes.float, 0))) for i in range(4)]
  ...
  def test_simple_store_fold_gate(self):
    buf = UOp(UOps.DEFINE_GLOBAL, dtypes.float.ptr())
    gate = UOp.variable("g1", False, True, dtypes.bool)
    load = [UOp(UOps.STORE, dtypes.float, (buf.index(UOp.const(dtypes.int, i)), UOp.const(dtypes.float, 0), gate)) for i in range(4)]
    sink = UOp(UOps.SINK, dtypes.void, tuple(load))
    sink = float4_rewrite(sink)
    assert len([x for x in sink.sparents if x.op is UOps.STORE]) == 1
  ...
  def test_simple_store_dont_fold(self):
    ...
    gate = UOp.variable("g1", False, True, dtypes.bool)
    gate2 = UOp.variable("g2", False, True, dtypes.bool)
    load = [UOp(UOps.STORE, dtypes.float, (buf.index(UOp.const(dtypes.int, i), gate if i == 0 else gate2),
...
class TestIFUOps(unittest.TestCase):
  def test_create_ifs(self):
    ...
    idx = UOp.const(dtypes.int, 0)
    st = UOp(UOps.STORE, dtypes.void, (sbuf.index(idx), UOp.const(dtypes.float, 42)))
    barrier = UOp(UOps.BARRIER, dtypes.void, (st,))
    lbuf = UOp(UOps.LOAD, dtypes.float, (sbuf.index(UOp.const(dtypes.int, 0)), barrier))
    store = UOp(UOps.STORE, dtypes.void, (gbuf.index(UOp.const(dtypes.int, 0), gate), lbuf))
  ...
  def test_expand_ifs_one_gate(self):
    ...
    barrier = UOp(UOps.BARRIER, dtypes.void, (st,))
    lbufs = [UOp(UOps.LOAD, dtypes.float, (sbuf.index(UOp.const(dtypes.int, i)), barrier)) for i in range(4)]
    stores = [UOp(UOps.STORE, dtypes.void, (gbuf.index(UOp.const(dtypes.int, i), gate), lbufs[i])) for i in range(4)]
