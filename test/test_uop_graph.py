...
from tinygrad.codegen.uopgraph import linearize_uop, full_graph_rewrite, graph_rewrite, expander, reducer, bigsym, float4_folding
...
class TestGraphRewriteConst(unittest.TestCase):
  ...
  def test_add_const(self):
    ...
    ret = graph_rewrite(v1+v2, bigsym)
...
def expander_rewrite(sink):
  ...
  return graph_rewrite(sink, bigsym + reducer)
def float4_rewrite(sink): return graph_rewrite(sink, bigsym + expander + float4_folding)
...
def gate_rewrite(sink): return graph_rewrite(sink, bigsym + expander + reducer)
