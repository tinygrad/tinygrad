...
class TestLinearizer(unittest.TestCase):
  ...
  def test_multireduce_with_parallel(self):
    Tensor.manual_seed(0)
    x = Tensor.randn(32, dtype=dtypes.float).realize()
    x_p = Tensor.randn(32, dtype=dtypes.float).realize()
    first_x = LazyOp(BufferOps.LOAD, (), MemBuffer(1, dtypes.float, x.lazydata.st.reshape((1, 32)).expand((32, 32))))
    first_x_p = LazyOp(BufferOps.LOAD, (), MemBuffer(2, dtypes.float, x_p.lazydata.st.reshape((1, 32)).expand((32, 32))))
    first_reduce = LazyOp(ReduceOps.SUM, (first_x,), (1,))
    first_reduce_p = LazyOp(ReduceOps.SUM, (LazyOp(UnaryOps.EXP2, (first_x_p,)),), (1,))
    second_x = LazyOp(BufferOps.LOAD, (), MemBuffer(1, dtypes.float, x.lazydata.st.reshape((32, 1))))
    ...
    wanna_output = (x.numpy()-(x.numpy().sum(-1, keepdims=True)+np.exp2(x.numpy()).sum(-1, keepdims=True))).sum(-1).reshape(1,1)
    helper_linearizer_ast((store, ), [x], wanna_output=[wanna_output], opts=opts)
  ...
  def test_multiout_intermediate_multireduce(self):
    ...
    second_reduce = LazyOp(ReduceOps.SUM, (diff,), (1,))
    store0 = LazyOp(BufferOps.STORE, (second_reduce,), MemBuffer(0, dtypes.float, ShapeTracker.from_shape((27, 1, 1, 5))))
    store1 = LazyOp(BufferOps.STORE, (first_reduce,), MemBuffer(1, dtypes.float, ShapeTracker(views=(View(shape=(27,15,1,5), strides=(5,0,1,1), offset=0, mask=None, contiguous=False),))))
    ...
    np.testing.assert_allclose(np.frombuffer(outbufs[0].as_buffer(), _to_np_dtype(outbufs[0].dtype)).reshape(27,1,1,5), wanna_output0, atol=1e-6, rtol=1e-6)
    np.testing.assert_allclose(np.frombuffer(outbufs[1].as_buffer(), _to_np_dtype(outbufs[1].dtype))[:135].reshape(27,1,1,5), wanna_output1, atol=1e-6, rtol=1e-6)

  # multireduce common errors
  ...
  # multireduce specific functions
  ...
  def test_softmax_multireduce_multiout(self):
    ...
    # y = LazyOp(op=BinaryOps.MUL, src=(exp_x, LazyOp(op=UnaryOps.RECIP, src=(sum_exp_x,)))) # kernels cannot do a return to full shape
    recip_sum_exp_x = LazyOp(op=UnaryOps.RECIP, src=(sum_exp_x,))
    store0 = LazyOp(op=BufferOps.STORE, src=(recip_sum_exp_x,), arg=MemBuffer(idx=0, dtype=dtypes.float, st=ShapeTracker.from_shape((4,1,1))))
    store1 = LazyOp(op=BufferOps.STORE, src=(max_x,), arg=MemBuffer(idx=1, dtype=dtypes.float, st=ShapeTracker.from_shape((4,1,1))))
    expected1 = x.numpy().max(axis=-1, keepdims=True).reshape(4,1,1)
    expected0 = 1/expected1.sum(axis=-1, keepdims=True).reshape(4,1,1)
    helper_linearizer_ast((store0, store1,), [x], wanna_output=[expected0, expected1])
  ...
  def test_padto_where_multireduce(self):
    ...
    wanna_output = np.where(0.5*17 < (x.numpy()+np.where(0.75*17 < x.numpy().sum(axis=1,keepdims=True), a.numpy(), b.numpy())).sum(axis=1),0.0,1.0).reshape((N,1,1))
    ...
    wanna_output = np.where(0.5*17 < (x.numpy()+np.where(0.75*17 < x.numpy().sum(axis=0,keepdims=True), a.numpy(), b.numpy())).sum(axis=0),0.0,1.0).reshape(1,1,N)
    ...
    # wanna_output = np.where(0.5*17 < (x.numpy()+np.where(0.75*17 < x.numpy().sum(keepdims=True), a.numpy(), b.numpy())).sum(keepdims=True),0.0,1.0).reshape((1,1,1,1))
