...
def gen_mask(shape: tuple[int,...], indices):
    ...
    for dim,(dim_size, idx) in enumerate(zip(shape, indices)):
        if isinstance(idx, int):
            ...
            step = idx.step or 1
            mask = Tensor.arange(start, stop, step or 1).one_hot(dim_size).sum(0).reshape((1,)*dim + (dim_size,) + (1,)*(ndim-dim-1)).expand(shape)
        elif idx is None:
            continue
        else:
