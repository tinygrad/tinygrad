...
def get_schedule(onnx_model) -> Tuple[List[ScheduleItem], List[ScheduleItem]]:
  Tensor.no_grad = True
...
def thneed_test_onnx(output_fn, onnx_model):
...
if __name__ == "__main__":
  dat = fetch(sys.argv[1] if len(sys.argv) > 1 else OPENPILOT_MODEL)
  onnx_model = onnx.load(io.BytesIO(dat))

  schedule, schedule_independent, inputs = get_schedule(onnx_model)
  ...
  if FLOAT16 == 0:
    try:
      thneed_test_onnx(output_fn, onnx_model)
