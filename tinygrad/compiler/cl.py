...
class CLASTKernel(ASTKernel):
  ...
  def store(self, buf_index:int, value:List[Token]) -> None:
    ...
    should_upcast = hasattr(self.program, 'float4') and self.buftokens[buf_index].can_float4()
    ...
    for o,v in to_store.items():
      ...
      if should_upcast:
        ...
        v = Token(f"{self.program.float4}({','.join([to_store[o+j].tok for j in range(4)])})", Types.FLOAT4) 
      idxy, valid = self.sts[buf_index].expr_idxs(o)
      assert valid.min == 1, "store must always be valid"
      if hasattr(self.bufs[buf_index]._buf, "IMAGE"):
        ...
        self.kernel.append(f"(({self.program.buffer_prefix}float4*)data{buf_index})[{(idxy//4).render(render_cl)}] = {v.tok};\n")
      else:
  ...
  def load(self, buf_index:int) -> List[Token]:
    ...
    should_upcast = hasattr(self.program, 'float4') and const is None and self.buftokens[buf_index].can_float4()
    tokens = []
    for o in self.buftokens[buf_index].offsets():
      ...
      if (buf_index, o) not in self.loaded_keys:
        ...
        if const is not None:
          ...
          ldr = Token(f"(({self.program.buffer_prefix}float4*){self.buftokens[buf_index].tok})[{(idxy//4).render(render_cl)}]", Types.FLOAT4)
        else:
  ...
  def hand_coded_optimizations(self):
    ...
    # are we grouping?
    if hasattr(self.program, 'float4') and not self.buftokens[0].can_float4() and self.first_reduce <= 2 and self.first_reduce + 1 <= self.shape_len and prod(self.sts[0].shape[:self.first_reduce]) <= 2048:
  ...
  def codegen(self, opt=False) -> Callable:
    ...
    if self.group_for_reduce:
      ...
      for i,acc in enumerate(accumulators):
        self.kernel.append(self.program.smem_prefix + f"{acc.decltype()} {self.buftokens[-1].tok}{i}[{prod(self.group_for_reduce)}];")
        self.kernel.append(f"{self.buftokens[-1].tok}{i}[mid_idx] = {acc.tok};\n")
      self.kernel.append(self.program.barrier+"\n")
    ...
    if hasattr(self.program, "DISABLE_GLOBAL"): self.kernel += ["}"] * len(self.output_shape)
    self.kernel.append("\n}")

    # kernel function definition
    function_name = ("re_S" if self.reduceop else "ew_S") + '_'.join([str(x) for x in self.bufs[0].shape if x != 1])
    buftypes = [f"{'read_only' if i > 0 else 'write_only'} image2d_t" if hasattr(x._buf, "IMAGE") else self.program.buffer_prefix+self.buftokens[i].decltype() for i,x in enumerate(self.bufs)]
    self.kernel = list(self.prekernel) + [f"{self.program.kernel_prefix} void {function_name}(",] + \
      [', '.join([f'{t} data{i}' for i,t in enumerate(buftypes) if i not in self.bufs_to_delete] + self.program.extra_args)] + \
      [") {\n"] + self.kernel

    # compile kernel
    self.fxn = self.program(function_name, ' '.join(self.kernel), op_estimate=self.info.flops, mem_estimate=sum(prod(x._base_shape) for x in self.bufs))
