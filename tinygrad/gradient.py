...
def reduce_gradient(ctx: UOp, ret: UOp):
  ...
  if ret.arg[0] == Ops.MUL:
    return (to_inp_shape(ctx * ret) / ret.src[0],)
...
pm_gradient = PatternMatcher([
  ...
  (
    UPat(Ops.POW, name="ret", src=(UPat.var("b"), UPat.var("e"))),
    lambda ctx, ret, b, e: (
      ctx * (b.eq(0) & e.eq(0)).where(e, e * b.pow(e - 1)),
      ctx * b.eq(0).where((e < 0).where(ret.const_like(-math.inf), 0), ret * b.log2() * math.log(2.0)),
    ),
  ),
  (
    UPat(Ops.MAX, name="ret", src=(UPat.var("x"), UPat.var("y"))),
    lambda ctx, ret, x, y: ((x > y).where(ctx, (x.eq(y)).where(ctx * 0.5, 0)), (x < y).where(ctx, (x.eq(y)).where(ctx * 0.5, 0))),
  ),
  ...
  (
    ...
    lambda ctx, ret: (ctx.r(Ops.ADD, tuple(i for i, (si, so) in enumerate(zip(ret.src[0].shape, ret.arg)) if si != so)),),
  ),
  ...
  for t0 in reversed(_deepwalk(root, targets)):
    ...
    for k, v in zip(t0.src, lgrads):
      ...
      if len(forward_metadata := all_metadata.get(t0, ())):
        all_metadata[v] = tuple(dataclasses.replace(x, backward=True) for x in forward_metadata)
  return grads
