...
def _deepwalk(root:UOp, targets:set[UOp]) -> list[UOp]:
  ...
  return list(root._toposort_gated(lambda node: node.op not in {Ops.DETACH, Ops.ASSIGN} and in_target_path[node]))
  ...
  for t0 in reversed(_deepwalk(root, targets)):
    ...
    for k,v in zip(t0.src, lgrads):
      ...
      if len(forward_metadata:=all_metadata.get(t0, ())):
        ...
        for bw_uop in v._toposort_gated(lambda x: x not in (t0, *t0.src, grads[t0])):
