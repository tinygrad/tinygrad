...
pm_gradient = PatternMatcher([
  ...
  (UPat(Ops.EXPAND, name="ret"), lambda ctx, ret:
    ((ctx1:=ctx.cast(sum_acc_dtype(ctx.dtype))).r(Ops.ADD, axis:=tuple(i for i,(si,so) in enumerate(zip(ret.src[0].shape, ret.arg))
      if si!=so)).reshape(tuple([s if i not in axis else 1 for i,s in enumerate(ctx1.shape)])).cast(ctx.dtype),)),
