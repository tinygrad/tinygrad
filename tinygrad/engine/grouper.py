...
merge_views = PatternMatcher([
  ...
  (UPat(Ops.VIEW, src=(UPat(Ops.CONST, name="x"),), name="view"),
...
view_left = merge_views+PatternMatcher([
  ...
  (UPat(Ops.VIEW, src=(UPat({*GroupOp.ALU, Ops.CAST, Ops.BITCAST, Ops.BIND, Ops.LOAD, Ops.STORE, Ops.VALID}, name="e"),), name="view"),
   lambda e,view: e.replace(src=tuple(s.view(view.st) if s.st is not None else s for s in e.src))),
...
view_right = merge_views+PatternMatcher([
  ...
  (UPat(GroupOp.All-{Ops.SINK, Ops.BIND}, name="root"), elementwise_view_right),
