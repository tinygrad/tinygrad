...
def complete_create_schedule_with_vars(big_sink:UOp) -> tuple[dict[UOp, UOp], list[ExecItem], dict[str, int]]:
  ...
  if sc_ret is None:
    ...
    combined_sink = get_rangeify(big_sink_cache)
  ...
  big_sink = resolve_call(combined_sink.call(*call_args))
  assert big_sink is not None
  big_sink = graph_rewrite(big_sink, pm_post_sched_cache, ctx=input_buffers_inverse, name="unrewrite combined")
