...
def realize_before_view(ctx:ScheduleContext, view:UOp, src:UOp, b:UOp, **kwargs) -> None:
  ...
  if resolve(prod(src.shape) < prod(st.shape)) and not getenv("DONT_REALIZE_EXPAND"):
    if src.op is Ops.CAST and src.src[0].op is Ops.VIEW and src.src[0].src[0].op is Ops.BUFFER: return None
