...
class ScheduleItem:
  ...
  def output_idxs(self) -> Tuple[int, ...]: return tuple(x.buffer.arg for x in self.ast.src) if self.ast.op is Ops.SINK else (0,)
...
def full_ast_rewrite(pre:UOp, var_vals:Dict[Variable, int], assigned:Set[UOp]) -> Tuple[UOp, ScheduleItemContext]:
  ...
  sink = graph_rewrite(pre, lazy+multioutput if len(pre.src)>1 else lazy, {x.buffer:x.src[2] for x in pre.src})
  # assert cyclic dependency
  for b,ops in itertools.groupby((x for x in sink.sparents if x.op in {Ops.PRELOAD,Ops.LOAD} and x.buffer in assigned), key=lambda x:x.buffer):
  ...
  if len(assign_targets:=[x.buffer for x in sink.sparents if x.op is Ops.ASSIGN]) != 0:
    if not all((s:=x.st_arg).contiguous or (len(s.views) == 1 and (m:=s.views[0].mask) is not None \
        and ShapeTracker.from_shape(s.shape).shrink(m) == s.shrink(m)) for x in sink.sparents if x.op is Ops.PRELOAD and x.buffer in assign_targets):
...
do_realize = PatternMatcher([
  ...
  (UPatLoadStore(UPat(Ops.CAST, src=(UPat(Ops.LOAD, name="x"),), dtype=dtypes.float)).view(name="view"), lambda ctx,x,view,**kwargs: r.view(view.st)
   if (r:=ctx.get(b:=x.buffer)) is not None and r.op is Ops.STORE and isinstance(b.dtype, ImageDType) else None),
...
def create_schedule_with_vars(outs:List[LazyBuffer]) -> Tuple[List[ScheduleItem], Dict[Variable, int]]:
  ...
  for sink in sinks:
    metadata = tuple({mx for x in sink.sparents if x.op in GroupOp.Buffer and len(x.src) > 2 and (mx:=ctx.ubuf_metadata.get(x.buffer))})
