...
tensor_uop_spec = PatternMatcher([
  ...
  (UPat((Ops.DETACH, Ops.CONTIGUOUS), name="root", src=(UPat.var("x"),), arg=None), lambda root,x: root.dtype.base == x.dtype.base),
...
def create_schedule_with_vars(outs:list[UOp], skip_check:bool=not __debug__) -> tuple[list[ScheduleItem], dict[Variable, int]]:
  ...
  if not skip_check: type_verify(list(sink.toposort), extra_spec=schedule_uop_spec)
