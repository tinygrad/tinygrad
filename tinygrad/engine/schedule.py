...
def _append_store(stores:Dict[UOp, UOp], root:UOp, b:UOp, st:UOp, x:UOp) -> UOp:
  assert (dup:=stores.get(b)) is None, f"duplicate STORE for {b} {dup} {x}"
  stores[b] = x
...
def create_schedule_with_vars(outs:List[LazyBuffer]) -> Tuple[List[ScheduleItem], Dict[Variable, int]]:
  ...
  sink = UOp(UOps.SINK, dtypes.void, tuple(to_uop(out.base, buf_uops, cache) for out in outs))
  stores: Dict[UOp, UOp] = {}
  graph_rewrite(sink, append_stores, stores)
  prescheduled: List[ScheduleItem] = []
  for outputs in output_groups.values():
    sink = UOp(UOps.SINK, dtypes.void, tuple(stores[b] for b in outputs))
    bufs: List[UOp] = []
