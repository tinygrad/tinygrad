...
def _lower_lazybuffer(outs:List[LazyBuffer], realizes:Dict[LazyBuffer, None]):
  ...
  for i, out in enumerate(outs):
    ...
    # NOTE: this should reshape to match the reduceop st if we're using it
    output_view = out.arg[0].reshape(output_st.shape) if out.op is MetaOps.ASSIGN and out.arg else output_st
...
def _recursive_group(tr:LazyBuffer, st:ShapeTracker, r:LazyBuffer, children:DefaultDict[LazyBuffer, Dict[LazyBuffer, None]],
  ...
  if tr in realizes and tr is not r:
    ...
    group[tr] = st.contiguous and st.size == r.st.size and tr not in reduce_for_op and tr.device == r.device
    return
  for tr_next in children[tr]:
    # max one reduceop per kernel
    if tr_next.op in ReduceOps: return group.setdefault(r, False)
    # can only fuse contiguous
...
def _graph_schedule(outs:List[LazyBuffer], seen:Set[LazyBuffer]):
  ...
  for r in allbufs:
    if r.op not in ReduceOps or r.forced_realize: continue
    ...
    group = merge_dicts([group, descendants if all(descendants.values()) else {}])
    ...
    if not group:
      ...
      if can_chase:
        ...
        while len(children[tr]) == 1:
          ...
          if not st.contiguous or tr_next.op in ReduceOps or tr_next.device != r.device: break
          tr = tr_next
...
def create_schedule_with_vars(outs:List[LazyBuffer], seen:Optional[Set[LazyBuffer]]=None) -> Tuple[List[ScheduleItem], Dict[Variable, int]]:
  ...
  if SAVE_SCHEDULE:
    ...
    if SAVE_SCHEDULE.value > 1 and len(schedule) == SAVE_SCHEDULE.value:
      SCHEDULES.clear()
      SCHEDULES.append((graph, prescheduled))
      exit(0)
  ...
  if len(schedule) > getenv("EXIT_SCHEDULE", 1e18): exit(0)
