...
def create_schedule_with_vars(outs:List[LazyBuffer]) -> Tuple[List[ScheduleItem], Dict[Variable, int]]:
  ...
  sink = UOp(UOps.SINK, dtypes.void, tuple(to_uop(x, buf_uops, cache) for x in outs))
  stores: List[UOp] = []
  graph_rewrite(sink, append_stores, stores)
  schedule: List[ScheduleItem] = []
  for store in stores:
    bufs: List[UOp] = []
    ast = full_ast_rewrite(store.sink(), bufs, var_vals)
    schedule.append(si:=ScheduleItem(ast, tuple(uop_bufs[x] for x in bufs), ()))
    for b in si.outputs: del lazybufs[b].srcs
  return schedule, var_vals

def _append_store(stores:List[UOp], b:UOp, st:UOp, x:UOp, root:UOp) -> UOp:
  stores.append(x)
  return UOp(UOps.LOAD, root.dtype, (b, st))
append_stores = PatternMatcher([
  (UPat(UOps.LOAD, src=(UPat.var("b"), UPat.var("st"), UPat(UOps.STORE, src=(UPat.var("b"), UPat(), UPat()), name="x")), name="root"), _append_store),
])
