...
tensor_uop_spec = PatternMatcher([
  ...
  # COPY
  (UPat(Ops.COPY, name="copy", src=(UPat(Ops.DEVICE), UPat.var("copyin"),)),
   lambda copy,copyin: isinstance(copy.arg, bool) and copy.dtype == copyin.dtype),
...
ops_folding = symbolic_simple+PatternMatcher([
  ...
  (UPatScheduled(Ops.COPY, src=(UPat(), UPat(Ops.VIEW, name="copyin")), name="copy"),
   lambda base,b,copyin,copy: copyin if base.device == copyin.device and copy.arg is not True else None),
