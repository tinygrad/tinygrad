...
def create_schedule_with_vars(big_sink:UOp) -> tuple[list[ScheduleItem], dict[Variable, int], dict[UOp, UOp]]:
  ...
  ops_metadata = {v:k.metadata for k,v in tensor_map.items() if k.metadata is not None and v.base.op not in {Ops.CONST, Ops.BUFFER}}
  # create kernels
  ...
  for k,v in tensor_map.items():
    if (a:=kernel_map.get(v)) is not None and a.op is Ops.ASSIGN:
      becomes_map[k] = k.src[0] if k.op is Ops.ASSIGN else a.buf_uop.reshape(k.shape)
    elif v.base is k.base: continue
