...
def _graph_schedule(outs:List[LazyBuffer], seen:Set[LazyBuffer]) -> Tuple[DefaultDict[LazyBuffer, List[LazyBuffer]], DefaultDict[LazyBuffer, int],
  ...
  for r in realizes:
    ...
    while r_parents:
      if (p:=r_parents.pop()).realized or (p.op in LoadOps and p.op is not LoadOps.ASSIGN) or p in visited: continue
      visited.add(p)
  ...
  for key, lsi in prescheduled.items():
    ...
    for assign in parents_assigns:
      if assign in lsi.outputs: continue # the assign is fused
