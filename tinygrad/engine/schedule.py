...
def create_schedule(sched_sink:UOp) -> tuple[list[ExecItem], UOp]:
  with cpu_profile(TracingKey("linearize schedule")):
    # use linearize with schedule_mode for AFTER-based dependencies (single toposort)
    ordered = linearize(sched_sink, schedule_mode=True)
    ...
    for u in ordered:
      ...
      if k.op is Ops.RANGE:
        ...
        if u.op is Ops.END: schedule.append(u)  # append END after KERNEL tuple
      else:
