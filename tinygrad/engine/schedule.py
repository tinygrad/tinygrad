...
from tinygrad.helpers import Metadata, all_int, all_same, colored, diskcache_put, merge_dicts, prod, dedup, getenv, unwrap, flatten
from tinygrad.helpers import Context, ContextVar, FUSE_CONV_BW, FUSE_ARANGE, DEBUG, CAPTURE_PROCESS_REPLAY
...
remove_movement_ops = PatternMatcher([
  ...
  (UPat(Ops.VIEW, name="view", src=(UPat(Ops.MULTI, name="multi"),)),
   lambda view,multi: multi.replace(src=(tuple(x.view(view.st) for x in multi.src)))),
])
