...
from tinygrad.uop.ops import Ops, PatternMatcher, UOp, UPat, sym_infer, buffers, graph_rewrite, scoped_rewrite_cache
...
def compile_linear(linear:UOp, beam:int|None=None, validate=False, input_uops:list[UOp]|None=None, jit=False) -> UOp:
  ...
  with scoped_rewrite_cache(), scoped_type_verify_cache():
    linear = graph_rewrite(linear, pm_compile, ctx=True, name="precompile kernels", walk=True)
