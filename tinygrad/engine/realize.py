  ...
  def run(self, _var_vals:dict[str, int]|None=None, wait=False, jit=False, do_update_stats=True) -> float|None:
    ...
    if do_update_stats:
      ...
      if DEBUG >= 2:
        ...
        flops_str = colored(f"{flops*1e-6:7.0f} MFLOPS", 'yellow') if flops < 1e9 \
          else f"{flops*1e-9:7.0f} GFLOPS" if flops < 1e14 else colored(f"{flops*1e-12:7.0f} TFLOPS", 'green')
        mem_str = colored(f"{membw*1e-6:4.0f}|{ldsbw*1e-6:<6.0f} MB/s", 'yellow') if membw < 1e9 and ldsbw < 1e12  \
