...
class EncDec(Runner):
  def __init__(self, ctx, device):
    self.ctx = ctx
    name = f"encdec HEVC"
    super().__init__(colored(name, "yellow"), device, Estimates(lds=0, mem=0))
  def __call__(self, rawbufs:list[Buffer], var_vals:dict[str, int], wait=False):
    st = time.perf_counter()
...
si_lowerer = PatternMatcher([
  ...
  (UPat(Ops.ENCDEC, name="encdec"), lambda ctx,encdec: ((EncDec(encdec.arg[1], ctx[0].device)), list(ctx))),
])
