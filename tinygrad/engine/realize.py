...
from tinygrad.helpers import BEAM, HCQ2, size_to_str, time_to_str, VALIDATE_WITH_CPU, PROFILE, ProfilePointEvent, cpu_events, wait_cond
from tinygrad.uop.ops import Ops, PatternMatcher, UOp, UPat, AxisType, ProgramInfo, sym_infer, graph_rewrite
...
def exec_hcq(ctx:ExecContext, call:UOp, ast:UOp) -> float|None:
  tables:list[tuple[memoryview, int]] = []
  if (info:=call.arg.aux).inputs is not None:
    ...
    for j,dev in enumerate(call.arg.aux.device):
      ...
      wait_cond(lambda: bool(mv[0]), value=False, timeout_ms=ctx.timeout or getenv("HCQDEV_WAIT_TIMEOUT_MS", 30000), msg=f"{dev} hang detected")
      mv[:len(addrs)] = addrs
      tables.append((mv, len(addrs)))

  try: exec_kernel(replace(ctx, update_stats=DEBUG>=3), call, ast)
  except Exception:
    for mv,sz in tables: mv[:sz] = array.array('Q', [0] * sz)
    raise
...
if HCQ2: from tinygrad.runtime.support.hcq2 import hcq_compile, hcq_link # noqa: E402 # down here, hcq2 imports the helpers above
