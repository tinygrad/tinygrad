...
class ExecItem:
  ...
  def run(self, var_vals:Optional[Dict[Variable, int]]=None, wait=False, jit=False, do_update_stats=True) -> Optional[float]:
    ...
    if do_update_stats:
      ...
      if DEBUG >= 2:
        lds_estimate = sym_infer(self.prg.lds_estimate, var_vals)
        ...
        print(f"{colored(f'*** {self.prg.dname[:7]:7s} {GlobalCounters.kernel_count:4d}', 'magenta' if jit else ('green' if self.prg.first_run else None))} {self.prg.display_name+' '*(38-ansilen(self.prg.display_name))} mem {GlobalCounters.mem_used/1e9:5.2f} GB " +  # noqa: E501
              (str() if et is None else f"tm {ptm}/{GlobalCounters.time_sum_s*1e3:9.2f}ms ({op_estimate/((et or 1e-20)*1e9):8.2f} GFLOPS {mem_estimate/((et or 1e-20)*1e9):7.2f} GB/s x{int(round(lds_estimate/(mem_estimate+1e-10))):4d})" +  # noqa: E501
