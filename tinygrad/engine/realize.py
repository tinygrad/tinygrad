...
def get_program(ast: UOp, renderer: Renderer | None = None, opts: list[Opt] | None = None) -> ProgramSpec:
  ...
  return ProgramSpec(
    ...
    ast,
    uops,
    global_size=[1, 1, 1] if renderer.has_local else None,
...
class ExecItem:
  ...
  def run(self, _var_vals: dict[Variable, int] | None = None, wait=False, jit=False, do_update_stats=True) -> float | None:
    ...
    if PROFILE:
      cpu_events.append(ProfilePointEvent(self.prg.device, "exec", self.prg.display_name, {"metadata": self.metadata, "var_vals": var_vals}))
    ...
    if do_update_stats:
      ...
      if DEBUG >= 2:
        ...
        mem_str = (
          f"{membw * 1e-9:6.1f}|{ldsbw * 1e-9:<7.1f} GB/s" if membw < 1e13 else colored(f"{membw * 1e-12:6.1f}|{ldsbw * 1e-12:<7.1f} TB/s", "green")
        )
        print(
