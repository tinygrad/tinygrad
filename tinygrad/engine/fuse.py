...
def uval(u:UOp) -> UOp:
  assert u.op is Ops.LOAD and len(u.src) == 3 and u.src[2].op is Ops.STORE
  ret = u.src[2].src[2]
  return ret.src[0] if ret.op is Ops.CONTIGUOUS else ret

def get_st_childs(u:UOp, src:UOp) -> List[ShapeTracker]:
  assert all(x.base.op in {Ops.LOAD, Ops.PRELOAD, Ops.BUFFER, Ops.VIEW, Ops.WHERE} for x in u.src), f"invalid {u.op} src {[x.base.op for x in u.src]}"
  return dedup([unwrap(x.st) for x in u.src if x.base.op is Ops.LOAD and uval(x.base) is src])
...
def _recursive_group(tr:UOp, st:ShapeTracker, r:UOp, children:DefaultDict[UOp, Dict[UOp, None]], allbufs:Dict[UOp, UOp],
  ...
  parent_size = unwrap(uval(allbufs[r]).st).size
  cache.setdefault((tr, st))
  if tr in realizes and tr is not r:
    # can only fuse contiguous
    # max one reduceop per kernel
    if not st.contiguous or st.size != parent_size or tr in reduce_for_op: group.setdefault(r)
    return group.setdefault(tr)
  for tr_next in children[tr]:
    # max one reduceop per kernel
    if (tr_next_uop:=uval(allbufs[tr_next])).op is Ops.REDUCE_AXIS: return group.setdefault(r)
    # can only fuse contiguous
    if len(st_childs:=dedup(unwrap(x.st) for x in tr_next_uop.src if x.base.op is Ops.LOAD and x.base.buf_uop is tr)) > 1: return group.setdefault(r)
...
def _get_isolated_children(r:UOp, reduce_for_op:Dict[UOp, UOp], children:DefaultDict[UOp, Dict[UOp, None]], allbufs:Dict[UOp, UOp],
  ...
  while rc_parents:
    ...
    rc_parents.extend(x.base for x in p.src if x.base.op is Ops.LOAD and x.base.buf_uop is not r)
...
def get_realizes(children:DefaultDict[UOp, Dict[UOp, None]], allbufs:Dict[UOp, UOp], double_reduces:Dict[UOp, None],
  ...
  for r,r_uop in allbufs.items():
    r_uop = uval(r_uop)
    if r in realizes or r_uop.op is not Ops.REDUCE_AXIS: continue
    group: Dict[UOp, None] = {}
    ...
    if forced_realize or not group:
      ...
      if can_chase:
        ...
        if tr.op is Ops.CAST and tr.dtype.base.itemsize > tr.src[0].dtype.base.itemsize:
          tr = tr.src[0].base
      group = {tr: None}
      realizes[tr] = tr
  ...
  if FUSE_CONV_BW:
    for reduceop in double_reduces:
      top_reduce = reduceop.base.src[0].base
