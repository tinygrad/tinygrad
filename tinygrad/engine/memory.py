...
def _internal_memory_planner(buffers: list[list[Buffer]], noopt_buffers=None, ignore_checks=False, debug_prefix="") -> dict[Buffer, Buffer]:
  ...
  total_memory = (
    sum(round_up(buf.nbytes, min_block_size := 0x1000) for buf in first_appearance.keys()) * 2
  ...
  for (_, is_open_ev), buf in buffer_requests:
    ...
    if hasattr(Device[buf.device].allocator, "_offset") and not isinstance(buf.dtype, ImageDType):
      if is_open_ev:
        buffer_replace[buf] = (None, global_planner[buf.device][1].alloc(round_up(buf.nbytes, 0x1000)))
      else:
      ...
      if is_open_ev:
        buffer_replace[buf] = (reuse_buffers[key].pop(), None) if key in reuse_buffers and len(reuse_buffers[key]) > 0 else (buf, None)
      else:
  ...
  buffer_resolve: dict[Buffer, tuple[Buffer, int | None]] = {
    buf: (base or global_buffers[buf.device], off) for buf, (base, off) in buffer_replace.items()
  }
  ...
  if DEBUG >= 1:
    ak, av = (
      ...
      dedup(x for x in assigned.values() if x._base is None) + list(global_buffers.values()),
    )
    omem, nmem = sum([x.nbytes for x in ak]) / 1e6, sum([x.nbytes for x in av]) / 1e6
    if omem != nmem:
      print(f"{debug_prefix}memory reduced from {omem:.2f} MB -> {nmem:.2f} MB,", f"{len(ak)} -> {len(av)} bufs")
...
def memory_planner(schedule: list[ScheduleItem]) -> list[ScheduleItem]:
  ...
  assigned = _internal_memory_planner(
    [list(si.bufs) for si in schedule], noopt_buffers={b for si in schedule if si.ast.op is not Ops.SINK for b in si.bufs}
  )
