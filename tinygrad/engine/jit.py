...
class GraphRunner:
  def __init__(self, linear:UOp, input_uops:tuple[UOp, ...]=()):
    ...
    for call in self.linear.src:
      replace = [(p, param.arg.slot) for p, b in enumerate(get_call_arg_uops(call))
                 if (param:=b if b.op is Ops.PARAM else b.src[0] if b.op is Ops.MSELECT and b.src[0].op is Ops.PARAM else None) is not None]
