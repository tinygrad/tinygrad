...
def apply_graph_to_jit(jit_cache: list[ExecItem], input_rawbuffers: list[Buffer], var_vals: dict[Variable, int], max_batch_size=0) -> list[ExecItem]:
  ...
  def flush_batch():
    ...
    try:
      ...
      max_batch_size *= 2
      if DEBUG >= 2:
        print(f"JIT GRAPHing batch with {len(current_batch)} kernels on device {current_batch_devs[0]}")
    except GraphException as e:
      graphed_jit_cache.extend(current_batch)
      if DEBUG >= 2:
        print(f"JIT GRAPHing failed batch with {len(current_batch)} kernels on device {current_batch_devs[0]}: {e}")
    current_batch = []
    current_batch_devs = []
  ...
  for ji in jit_cache:
    match ji.prg:
      ...
      case BufferCopy():
        ji_graph_dev = next((Device[unwrap(b).device] for b in ji.bufs if unwrap(b).device not in {"CPU", "LLVM"}), None)
      case ViewOp():
        continue  # ViewOps are just ignored
      case _:
        ji_graph_dev = None  # Everything else is not graphed and flushes existing graph if it's being constructed
    ...
    can_share_graph = (
      can_be_graphed
      and len(current_batch_devs) > 0
      and graph_class(current_batch_devs[0]).supports_exec_item(dedup(current_batch_devs + [ji_graph_dev]), ji)
    )
...
class GraphRunner(Runner):
  def __init__(self, jit_cache: list[ExecItem], input_rawbuffers: list[Buffer], var_vals: dict[Variable, int]):
    ...
    self.symbolic_dims = dedup(
      [tuple(d) for ji in jit_cache if isinstance(ji.prg, CompiledRunner) and (d := ji.prg.p.local_size) and is_sym_dim(d)]
    ...
    def find_symbolic_dim(dim):
      return self.symbolic_dims.index(tuple(dim)) if dim is not None and tuple(dim) in self.symbolic_dims else None
    ...
    for j, ji in enumerate(jit_cache):
      ...
      if isinstance(ji.prg, CompiledRunner):
        if ji.prg.p.vars:
          self.var_vals_replace[j] = [(i, self.vars.index(v)) for i, v in enumerate(ji.prg.p.vars) if v not in ji.fixedvars]
  ...
  def supports_exec_item(devs: list[Compiled], ei: ExecItem) -> bool:
    return isinstance(ei.prg, CompiledRunner) and len(dedup(devs)) == 1
...
class MultiGraphRunner(GraphRunner):
  ...
  def supports_exec_item(devs: list[Compiled], ei: ExecItem) -> bool:
    ...
    return (
      isinstance(ei.prg, (CompiledRunner, BufferXfer)) and len(dedup([type(Device[b.device]) for b in ei.bufs if b] + [type(d) for d in devs])) == 1
    )


def get_out_buffers_for_ei(ei: ExecItem) -> list[Buffer]:
  if isinstance(ei.prg, CompiledRunner):
    return [cast(Buffer, ei.bufs[out]) for out in ei.prg.p.outs if out not in ei.prg.p.ins]
  if isinstance(ei.prg, (BufferCopy, BufferXfer)):
    return [cast(Buffer, ei.bufs[0])]
  return []
...
def update_depends(depends: set[Buffer | None], jit_cache: list[ExecItem]):
  for ei in jit_cache:
    if any(b in depends for b in ei.bufs):
      depends.update(get_out_buffers_for_ei(ei))
...
class CapturedJit(Generic[ReturnType]):
  ...
  def free_intermediates(self):
    ...
    for b in depends:
      if b is not None:
        if b.is_allocated():
          b.deallocate()
        if b._base is not None and b._base.allocated_views == 0 and b._base.is_allocated():
          b._base.deallocate()
  ...
  def replan_buffers_memory_layout(self):
    ...
    for old, new in asgn.items():
      if old.is_allocated():
        new.ensure_allocated().copyin(old.as_buffer())
    self.__post_init__()
...
def _prepare_jit_inputs(args, kwargs):
  ...
  input_buffers: list[Buffer] = flatten([
    rb.bufs if isinstance(rb := lb.base.realized, MultiBuffer) else [rb] for lb in lbs if lb.base.realized is not None
  ])
...
class TinyJit(Generic[ReturnType]):
  ...
  def add_buffer(self, b: Buffer) -> Buffer:
    if found := self._buffer_replace.get(b, None):
      return found
    if b.is_allocated() or b.uop_refcount > 0:
      return b
  ...
  def __call__(self, *args, **kwargs) -> ReturnType:
    ...
    if not JIT or self.cnt == 0:
      ...
      assert self.fxn is not None
      if capturing:
        raise RuntimeError(f"having TinyJit inside another TinyJit is not supported {len(capturing)=} {capturing=}")
      ...
      if DEBUG >= 1:
        print(f"JIT captured {len(jit_cache)} kernels with {len(input_buffers)} inputs")
      ...
      jit_cache = [
        ExecItem(item.prg, [assigned.get(b, b).ensure_allocated() for b in item.bufs if b is not None], item.metadata, item.fixedvars)
        for item in jit_cache
      ]
      ...
      if self.optimize:
        self.captured.replan_buffers_memory_layout()
    elif self.cnt >= 2:
      # jit exec
