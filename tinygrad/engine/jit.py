  ...
  for ji in jit_cache:
    match ji.prg:
      case CompiledRunner():
        ...
        can_share_graph = match_dev([ji_graph_dev, current_device], multi=graph_cls and graph_cls.graph_multi)
      case BufferXfer():
        ji_graph_dev, src_dev = Device[unwrap(ji.bufs[0]).device], Device[unwrap(ji.bufs[1]).device]
        can_be_graphed = ji_graph_dev.graph is not None and graph_class(ji_graph_dev).graph_multi and match_dev([ji_graph_dev, src_dev], multi=True)
        can_share_graph = can_be_graphed and match_dev([ji_graph_dev, src_dev, current_device], multi=True)
      case ViewOp(): continue
      case _: can_be_graphed = False
