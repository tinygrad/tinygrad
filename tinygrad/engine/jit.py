...
class GraphRunner:
  def __init__(self, linear:UOp, input_uops:tuple[UOp, ...]=()):
    ...
    self.vars = sorted({v.expr for _,p,dv in crs for v in p.vars if v.expr not in dv and v.expr not in p.runtimevars})
    ...
    for j,p,dv in crs:
      if (replace:=[(i, self.vars.index(v.expr)) for i, v in enumerate(p.vars) if v.expr not in dv and v.expr not in p.runtimevars]):
