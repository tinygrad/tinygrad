...
def apply_graph_to_jit(jit_cache: list[ExecItem], input_rawbuffers: list[Buffer], var_vals: dict[Variable, int], max_batch_size=0) -> list[ExecItem]:
  ...
  for ji in jit_cache:
    ...
    can_share_graph = can_be_graphed and len(current_batch_devs) > 0 and graph_class(current_batch_devs[0]).supports_exec_item(current_batch_devs, ji)
    ...
    if can_be_graphed: current_batch_devs = dedup(current_batch_devs + [Device[unwrap(b).device] for b in ji.bufs])
