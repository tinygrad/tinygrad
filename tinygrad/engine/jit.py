...
def _prepare_jit_inputs(args, kwargs):
  # jit maps realize(A)->B, and replays the kernel generated by the function, where A are the input tensors and B is the result tensor
  # A, realize(A), and B's types can be (unrealized, realized, const). input mapping can be any combination of these tensors
  # JIT records the mapping realize(A) only, expecting next realize(tensors) to be type(realize(A))
  # JIT requires that for a tensor A, realize(A)'s type is the same, etc realized -> realized and unrealized -> realized are valid
  ...
  def __call__(self, *args, **kwargs) -> ReturnType:
    ...
    if not JIT or self.cnt == 0:
      ...
      assert self.captured.rmap == rmap, \
        f"realize type violation: for realize({self.captured.rmap.keys()}), expected {rmap.values()}, got {self.captured.rmap.values()}"
