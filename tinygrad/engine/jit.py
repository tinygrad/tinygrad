  ...
  def __init__(self, jit_cache: list[ExecItem], input_buffers: list[Buffer], var_vals: dict[str, int]):
    ...
    for j,ji in enumerate(jit_cache):
      ...
      if isinstance(ji.prg, CompiledRunner):
        if ji.prg.p.vars:
          self.var_vals_replace[j] = [(i, self.vars.index(v.expr)) for i, v in enumerate(ji.prg.p.vars) if v.expr not in ji.fixedvars and v.expr not in ji.prg.p.runtimevars]
