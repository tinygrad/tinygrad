...
actions += [Opt(op=OptOps.TC, axis=0, amt=tc, arg=0) for tc in range(5)]
# covers resnet kernels (3 global * 3 reduce)
actions += [Opt(op=OptOps.TC, axis=axis, amt=tc, arg=getenv("TC_OPT", 2)) for axis in range(9) for tc in range(5)]
