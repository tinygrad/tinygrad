...
from tinygrad.helpers import prod, flatten, DEBUG, CACHELEVEL, NOSTATS, diskcache_get, diskcache_put, getenv, Context, colored, to_function_name
...
def _time_program(p:Program, lib:bytes, var_vals, rawbufs, early_stop=None, max_global_size=65536, clear_l2=False, cnt=3, name="test"):
  ...
  for _ in range(cnt):
    if clear_l2:
      with Context(DEBUG=0, BEAM=0, CACHECOLLECTING=0, NOSTATS=1): Tensor.ones(1024,1024).contiguous().realize()
