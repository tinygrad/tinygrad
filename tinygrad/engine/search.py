...
actions += [Opt(op=OptOps.TC, axis=0, arg=(-1, 0, 1))]
actions += [Opt(op=OptOps.TC, axis=axis, arg=(-1, getenv("TC_OPT", 2), 1)) for axis in range(9)] # covers resnet kernels (3 global * 3 reduce)
  ...
  if TC_SEARCH_OVER_SHAPE and len(lin.applied_opts) == 0: # tensor core opts must be first
    for i, action in enumerate(kernel_actions):
      if action.op == OptOps.TC and (tc_arg := cast(tuple, action.arg))[0] == -1:
        ...
        kernel_actions[i:i+1] = [Opt(op=OptOps.TC, axis=action.axis, arg=(tc_select,tc_arg[1],1)) for tc_select,_ in enumerate(lin.opts.tensor_cores)]
