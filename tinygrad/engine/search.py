...
actions = [
    Opt(op=OptOps.UPCAST, axis=axis, arg=amt)
    for amt in [0, 2, 3, 4, 5, 7]
    for axis in range(6)
]
actions += [
    Opt(op=OptOps.UNROLL, axis=axis, arg=amt) for amt in [0, 4, 7] for axis in range(5)
]
actions += [
    Opt(op=OptOps.LOCAL, axis=axis, arg=amt)
    for amt in [2, 3, 4, 8, 13, 16, 29]
    for axis in range(6)
]
actions += [
    Opt(op=OptOps.GROUPTOP, axis=axis, arg=amt)
    for amt in [13, 16, 28, 29, 32, 49, 64, 256]
    for axis in range(3)
]
actions += [
    Opt(op=OptOps.GROUP, axis=axis, arg=amt)
    for amt in [0, 4, 8, 16]
    for axis in range(3)
]
if getenv("BEAM_PADTO", 1):
    actions += [
        Opt(op=OptOps.PADTO, axis=axis, arg=amt) for amt in [32] for axis in range(7)
    ]
actions += [Opt(op=OptOps.LOCAL, axis=0, arg=32), Opt(op=OptOps.LOCAL, axis=6, arg=2)]
actions += [Opt(op=OptOps.TC, axis=0, arg=(-1, 0))]
actions += [
    Opt(op=OptOps.TC, axis=axis, arg=(-1, getenv("TC_OPT", 2))) for axis in range(9)
]  # covers resnet kernels (3 global * 3 reduce)
actions += [
    Opt(op=OptOps.SWAP, axis=axis_0, arg=axis_1)
    for axis_0 in range(5)
    for axis_1 in range(axis_0 + 1, 5)
]
if getenv("NOLOCALS"):
...
def _time_program(
    ...
    max_global_size: Optional[int] = 65536,
    clear_l2=False,
    cnt=3,
    name="test",
) -> list[float]:
    ...
    if p.global_size is not None and max_global_size is not None:
        global_size, factor = _get_test_global_size(
            p.global_size, max_global_size, var_vals
        )
        p = replace(p, global_size=global_size)
    try:
        car = CompiledRunner(p, precompiled=lib)
    ...
    for _ in range(cnt):
        if clear_l2:
            if hasattr(dev := Device[p.device], "invalidate_caches"):
                dev.invalidate_caches()
            else:
                with Context(DEBUG=0, BEAM=0, CAPTURING=0, TRACK_MATCH_STATS=0):
                    Tensor.ones(1024, 1024).contiguous().realize(do_update_stats=False)
        tms.append(cast(float, car(input_bufs, var_vals, wait=True)) * factor)
        if early_stop is not None and early_stop < min(tms):
            break
    return tms
...
def _try_compile_linearized_w_idx(
    x: tuple[int, Kernel], compiler: Compiler
) -> tuple[int, Optional[tuple[ProgramSpec, bytes, float]]]:
...
def _ensure_buffer_alloc(bufs: list[Buffer]) -> list[Buffer]:
    return [buf.ensure_allocated() for buf in bufs]
...
def bufs_from_lin(lin: Kernel, allocate: bool = True) -> list[Buffer]:
    ...
    for x in lin.bufs:
        if x.src[0].op is Ops.DEFINE_GLOBAL:
            bufsts[x.src[0].arg].append(x)
    rawbufs: list[Optional[Buffer]] = [None] * len(bufsts)
    for k, lx in bufsts.items():
        buf_size = (
            prod(dtype.shape)
            if isinstance(dtype := lx[0].src[0].dtype, ImageDType)
            else max(y.st_arg.real_size() for y in lx)
        )
        assert isinstance(dtype, (PtrDType, ImageDType))
        if buf_size == 0:
            buf_size = 1  # create a size 1 buffer if no cell is accessed in kernel. # TODO: remove from kernel input in this case.
        buf_dtype = dtype if isinstance(dtype, ImageDType) else dtype.base
        rawbufs[k] = (
            Buffer(lin.opts.device, buf_size, buf_dtype).allocate()
            if allocate
            else Buffer(lin.opts.device, buf_size, buf_dtype)
        )
...
def get_kernel_actions(lin: Kernel, include_0=True) -> dict[int, Kernel]:
    acted_lins, max_up, max_lcl = (
        {0: lin} if include_0 else {},
        getenv("BEAM_UPCAST_MAX", 256),
        getenv("BEAM_LOCAL_MAX", 1024),
    )
    kernel_actions = actions.copy()

    if (
        TC_SEARCH_OVER_SHAPE and len(lin.applied_opts) == 0
        ...
        for i, action in enumerate(kernel_actions):
            if action.op == OptOps.TC and (tc_arg := cast(tuple, action.arg))[0] == -1:
                ...
                kernel_actions[i : i + 1] = [
                    Opt(op=OptOps.TC, axis=action.axis, arg=(tc_select, tc_arg[1]))
                    for tc_select, _ in enumerate(lin.opts.tensor_cores)
                ]

    for i, a in enumerate(kernel_actions):
        if a.axis is not None and a.op is not OptOps.TC:
            if ((ax := a.real_axis(lin)) >= lin.shape_len) or (
                lin.full_shape[ax] == a.arg and Opt(a.op, ax, 0) in kernel_actions
            ):
                continue
        lin2 = lin.copy()
        try:
            ...
            up, lcl, tc_up = (
                1,
                1,
                prod(tc.dims) // tc.threads if (tc := lin2.tensor_core) else 1,
            )
...
def beam_search(
    ...
    default_parallel = (
        ...
        if lin.opts.device in {"CUDA", "AMD", "NV", "METAL"}
        else 0
    )
    if beam_pool is None and (workers := getenv("PARALLEL", default_parallel)):
        beam_pool = multiprocessing.get_context("spawn").Pool(
    ...
    if DEBUG >= 2:
        print(
            f"   0.00s:                 from   1 ->   1 actions {lin.colored_shape()}"
        )

    try:
        rawbufs = _ensure_buffer_alloc(rawbufs)
        var_vals: dict[Variable, int] = {
            k: int(k.vmax + k.vmin) // 2 for k in lin.ast.variables()
        }
        exiting, st = False, time.perf_counter()
        dev = Device[lin.opts.device]
        while not exiting:
            acted_lins: list[Kernel] = flatten(
                [get_kernel_actions(lin, include_0=False).values() for lin, _ in beam]
            )
            timed_lins: list[tuple[Kernel, float]] = []
            _compile_fn = functools.partial(
                _try_compile_linearized_w_idx, compiler=dev.compiler
            )
            least_compute_ops = math.inf
            for i, proc in (
                map(_compile_fn, enumerate(acted_lins))
                if beam_pool is None
                else beam_pool.imap_unordered(_compile_fn, enumerate(acted_lins))
            ):
                if proc is None:
                    continue
                ...
                least_compute_ops = min(
                    this_compute_ops := sym_infer(p.estimates.ops, var_vals),
                    least_compute_ops,
                )
                ...
                if BEAM_DEBUG > 1:
                    print(
                        f"{time.perf_counter() - st:7.2f}s: {i:5d} {len(cast(list, p.uops)):5d} uops {compile_et*1e6:12.2f} us compile/{timed_lins[-1][1]*1e6:12.2f} us run       {len(timed_lins):4d}/{len(acted_lins):4d}         {timed_lins[-1][0].colored_shape()}"
                    )  # noqa: E501
                elif DEBUG >= 2:
                    print(
    ...
    if BEAM_DEBUG:
        print(
            f"BEAM_SEARCH: final tm={beam[0][1]*1e6:0.2f} us, applied_opts={beam[0][0].applied_opts}"
        )
    return beam[0][0]


def optimize_local_size(
    _prg: Callable, global_size: list[int], rawbufs: list[Buffer]
) -> list[int]:
    test_rawbuffers = (
        [
            Buffer(rawbufs[0].device, rawbufs[0].size, rawbufs[0].dtype).allocate(),
            *rawbufs[1:],
        ]
