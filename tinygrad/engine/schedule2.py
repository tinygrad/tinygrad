...
def schedule_buffer(n:LazyBuffer, realizes:Dict[LazyBuffer, None], reduce_for_op:Dict[LazyBuffer, LazyBuffer]) -> ScheduleItem:
  ...
  def _lazyop_dfs(x:LazyBuffer, st:ShapeTracker) -> LazyOp:
    ...
    if x.op is LoadOps.CONTIGUOUS:
      assert x is n
      lop = _lazyop_dfs(x.srcs[0], st)
    else: lop = LazyOp(x.op, tuple(_lazyop_dfs(src, st) for src in x.srcs), x.arg)
    if x is n: lop = LazyOp(BufferOps.STORE, (lop, ), MemBuffer(0, x.dtype, st.simplify()))
    return lop
  ast = _lazyop_dfs(n, ShapeTracker.from_shape(n.st.shape if n not in reduce_for_op else reduce_for_op[n].st.shape))
...
def create_schedule(outs:List[LazyBuffer],_) -> Tuple[List[ScheduleItem], Dict[Variable, int]]:
  realizes = {out.base:None for out in outs}
  @functools.lru_cache(None)
  def _realize_dfs(u:LazyBuffer):
    if u.base.realized is not None: return
    for x in u.base.srcs: _realize_dfs(x)
    if u is not u.base:
      if prod(u.base.st.shape) < prod(u.st.shape): realizes[u.base] = None
      return _realize_dfs(u.base)
    if u.op in LoadOps or u.forced_realize: realizes[u] = None
  for out in outs: _realize_dfs(out)
  ...
  for out in outs: _get_parents_dfs(out.base, out.base, first=True)

  for x in realizes: assert x is x.base
