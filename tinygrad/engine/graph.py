...
def _tree(luop:Union[LazyOp,UOp], cycles, cnt, prefix=""):
  ...
  if len(src:=luop.vin if hasattr(luop,'vin')else luop.src) == 0:
    return [f"━━ {prefix}{(luop.op if hasattr(luop, 'op') else luop.op).name} {luop.arg if luop.arg else ''}"]
  ...
  lines = [f"━┳ {prefix}{(luop.op if hasattr(luop,'op')else luop.op).name} {luop.arg if luop.arg else ''}"]
