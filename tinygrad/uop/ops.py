...
class UOpMetaClass(type):
  ...
  def __call__(cls, op:Ops, dtype:DType|None=None, src:tuple[UOp,...]=tuple(), arg:Any=None, tag:Any=None,
    ...
    # an INDEX presents its access dtype and RANGE/SPECIAL their index-role dtype: a still-weak source matches up to weakness
    if SPEC == 2 and op is not Ops.CONST and \
       ...
       not (op in (Ops.INDEX, Ops.RANGE, Ops.SPECIAL) and weak_dtype(expected_dtype) == weak_dtype(dtype)):
...
class UOp(RandMixin, metaclass=UOpMetaClass):
  ...
  def store(self, src:UOp|ConstType, gate:UOp|None=None, **kwargs):
    srcs = (self, UOp.const(src).cast(self.dtype) if not isinstance(src, UOp) else src) + ((gate,) if gate is not None else ())
  ...
  def range(end:sint, axis_id, axis_type=AxisType.WEAK, *arg, dtype=dtypes.weakint, src=(), **kwargs):
    # the node dtype is the index role's; the size src stays a bare CONST
    return UOp(Ops.RANGE, dtype, src=(sint_to_uop(end),)+src, arg=(axis_id, axis_type)+arg, **kwargs)
  @staticmethod
  ...
  def valid(self, cond):
    return cond.where(self, UOp.const(Invalid, shape=self._shape))
  ...
  def _min_max(self) -> tuple[PyConst, PyConst]:
    ...
    if self.op is Ops.CAST:
      ...
      if self.is_const and self.src[0].arg is not Invalid: return (v:=self.dtype.const(self.src[0].arg)), v
...
# a size is widthless: a python int is a bare weak CONST, a symbolic one keeps its own width under the weak marker
def sint_to_uop(x:sint) -> UOp: return x.cast(dtypes.weakint) if isinstance(x, UOp) else UOp.const(x)
...
pm_lower_weak = PatternMatcher([
  ...
  # SPECIAL/RANGE commit their dtype from the index role (their bounds); the size src stays a bare CONST
  (UPat((Ops.RANGE, Ops.SPECIAL), dtype=dtypes.weaks, name="u"), lambda u: u.replace(dtype=select_dtype(u)).cast(u.dtype)),
...
pm_pair_weak = pm_commit_weak+PatternMatcher([
  ...
  # a concrete STACK's weak lanes commit at the node's scalar dtype
  (UPat(Ops.STACK, name="u"), lambda u: u.replace(dtype=None, src=tuple(
    s.cast(u.dtype.scalar()) if s.dtype in dtypes.weaks else s for s in u.src))
   if u.dtype not in dtypes.weaks and any(s.dtype in dtypes.weaks for s in u.src) else None),
])
...
pm_cast_weak = PatternMatcher([
  (UPat(Ops.CAST, name="c", src=(UPat(GroupOp.Broadcastable, dtype=dtypes.weaks, name="u"),)),
   lambda c,u: u.replace(dtype=None, src=tuple(s.cast(c.dtype) if s.dtype in dtypes.weaks else s for s in u.src)).cast(c.dtype)
   if c.dtype not in dtypes.weaks else None),
  # an all-weak STACK under a concrete cast: the lanes commit at the cast's scalar dtype
  (UPat(Ops.CAST, name="c", src=(UPat(Ops.STACK, dtype=dtypes.weaks, name="u"),)),
...
# index lowering. pm_cast_weak comes last: a concrete CAST is the demand for what lowering left weak (both branches of a gate-mask WHERE)
pm_lower_index_dtype = pm_commit_weak+PatternMatcher([
  (UPat(GroupOp.All, name="u"),
   lambda ctx,u: lower_weak_srcs(ctx, u) if u.dtype not in dtypes.weaks
   and any(s.dtype in dtypes.weaks and s.op is not Ops.CONST for s in u.src) else None),
