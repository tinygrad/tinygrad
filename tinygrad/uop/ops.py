...
class UOp(RandMixin, metaclass=UOpMetaClass):
  ...
  def tuplize(self:UOp) -> tuple:
    # NOTE: find better comparison for Ops.INS
    return (self.op.value, self.arg.__hash__() if self.op is Ops.INS else self.arg, self.dtype,)+tuple([x.tuplize for x in self.src])
