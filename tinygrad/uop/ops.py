...
class UOp(RandMixin, metaclass=UOpMetaClass):
  ...
  def has_buffer_identity(self, after_ok=False):
    ...
    # This branch also has unresolved inline-call outputs, which must not be mistaken for materialized storage.
    # Scheduler-local anonymous buffers and physical all-reduce views do have stable storage identity.
    return ((self.op in {Ops.BUFFER, Ops.PARAM} and not self.is_unbound) or
            (self.op is Ops.BUFFER and self.tag == ("anonymous",)) or
            (self.op is Ops.SHRINK and self.tag == ("allreduce",)))
  @property
