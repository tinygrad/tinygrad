...
range_start = {Ops.BUFFERIZE: 1, Ops.REDUCE: 1, Ops.STORE: 2, Ops.WMMA: 3, Ops.END: 1, Ops.CALL: 1, Ops.FUNCTION: 1, Ops.COPY: 2, Ops.BUFFER_VIEW: 1}


# https://en.wikipedia.org/wiki/Identity_element
def identity_element(op: Ops, dt: DType) -> PyConst:
  return dt.const({Ops.ADD: 0, Ops.MUL: 1, Ops.MAX: dt.min}[op])
...
def shape_to_shape_arg(arg: tuple[sint, ...]) -> UOp:
  if len(arg) == 0:
    return UOp(Ops.VECTORIZE, dtypes.weakint.vec(0))
  elif all_int(arg):
    return UOp.const(dtypes.weakint.vec(len(arg)), arg)
  else:
    return UOp(Ops.VECTORIZE, dtypes.weakint.vec(len(arg)), tuple(UOp.const(dtypes.weakint, x) if isinstance(x, int) else x for x in arg))
  ...
  def __call__(
    ...
    if SPEC > 1:
      ...
      with Context(CHECK_OOB=0):
        fret = cast(bool | None, full_spec.rewrite(created))
      if fret is not True:
        raise RuntimeError(f"SPEC ISSUE {fret}: {created}")
    return created
...
class recursive_property(property):
  ...
  def __get__(self, x: UOp | None, owner=None):
    ...
    if all(nm in s.__dict__ for s in x.src):
      xd[nm] = self.fxn(x)
      return xd[nm]
    for node in x.toposort(gate=lambda node: nm not in node.__dict__):
      node.__dict__[nm] = self.fxn(node)
    return xd[nm]
...
class UOp(OpMixin, metaclass=UOpMetaClass):
  ...
  def __reduce__(self):
    ...
    if self.op is Ops.BUFFER and self.realized is not None:
  ...
  def _shape(self) -> tuple[sint, ...] | None:
    match self.op:
      ...
      case Ops.CONST | Ops.VCONST | Ops.DEFINE_VAR | Ops.BIND:
        return ()
      case Ops.BUFFER:
        return (self.arg,)
      ...
      case Ops.SHAPED_WMMA:
        return self.src[2]._shape
      ...
      case Ops.MULTI if len(self.src) == 0:
        return None
  ...
  def shard_shape(self) -> tuple[sint, ...]:
    if not isinstance(self.device, tuple) or self.axis is None:
      return self.shape
  ...
  def shard_size(self) -> int:
    return prod(self.max_shard_shape)
  ...
  def ended_ranges(self) -> tuple[UOp, ...]:
    ...
    if self.op is Ops.AFTER:
      return tuple(flatten([x.ended_ranges for x in self.src[1:]]))
  ...
  def ranges(self) -> dict[UOp, None]:
    if self.op is Ops.RANGE:
      return {self: None} | self._ranges
    return self._ranges

  # *** uop evaluation ***

  def simplify(self, tracked=False):
    if self.op in {Ops.CONST, Ops.VCONST}:
      return self
    if self.op is Ops.SINK and all(s.op in {Ops.CONST, Ops.VCONST} or (s.op is Ops.VECTORIZE and len(s.src) == 0) for s in self.src):
      return self
    # late import!
  ...
  def __float__(self):
    return float(self._eval(dtypes.floats, float))
  ...
  def substitute(self, dvars: dict[UOp, UOp], name: str | None = None, extra_pm: PatternMatcher | None = None, walk: bool = False):
    ...
    with Context(TRACK_MATCH_STATS=(0 if name is None else TRACK_MATCH_STATS.value)):
      return graph_rewrite(self, (extra_pm + _substitute) if extra_pm is not None else _substitute, dvars, bottom_up=True, walk=walk, name=name)
  ...
  def __getitem__(self, idx):
    ...
    if len(slice_idx := [i for i, x in enumerate(idx) if isinstance(x, slice)]):
      ...
      bounds = tuple(
        (s.start or 0, s.stop if s.stop is not None else self.shape[i]) if isinstance(s, slice) else (0, self.shape[i]) for i, s in enumerate(idx)
      )
      ...
      if not non_slice_args:
        return src  # all dims are slices, no indexing needed
  ...
  def end(self, *src: UOp):
    return UOp(Ops.END, src=(self,) + src) if len(src) else self

  def after(self, *src: UOp, **kwargs):
    return UOp(Ops.AFTER, self.dtype, (self,) + src, **kwargs) if len(src) else self
  ...
  def const(dtype: DType, b: ConstLike, device: str | tuple[str, ...] | None = None, shape: tuple[sint, ...] | None = None):
    ...
    ret = UOp(
      Ops.VCONST if isinstance(b, tuple) else Ops.CONST, dtype, arg=dtype.const(b), src=(UOp(Ops.DEVICE, arg=device),) if device is not None else ()
    )
  ...
  @staticmethod
  def special(end: sint, name: str, dtype=dtypes.weakint):
  ...
  @staticmethod
  def invalid(count=1):
    return UOp(Ops.CONST, dtypes.weakint.vec(count), src=(), arg=Invalid)
  ...
  def axis(self) -> int | None:
    ...
    if self.op is Ops.RESHAPE:
      ...
      if self.shape[new_axis] % len(self.device) != 0:
  ...
  def _shard(self, axis: int) -> UOp:
    if len(self.shape) == 0:
      return self  # scalars broadcast, no sharding needed
  ...
  def mstack(self, *srcs: UOp) -> UOp:
    return UOp(Ops.MSTACK, self.dtype, (self,) + srcs)
  ...
  def base(self) -> UOp:
    ...
    if self.op is Ops.DETACH:
      return self.src[0].base  # DETACH can't change base
    return self
  ...
  def _mop(self, op: Ops, arg) -> UOp:
    ...
    if len(usrcs) == 0:
      return UOp(op, self.dtype, (self,), arg)
  ...
  def contiguous_view_offset(self) -> int | None:
    ...
    if out.op is not Ops.INDEX:
      return None
    if out.src[1].op is Ops.CONST and resolve(numel == 1, False):
      if not isinstance(out.src[1].arg, int):
        return None  # masked/padded regions produce InvalidType
    ...
    if out.src[1].op is Ops.ADD and out.src[1].src[0].op is Ops.RANGE and out.src[1].src[1].op is Ops.CONST:
      if not isinstance(out.src[1].src[1].arg, int):
        return None  # masked/padded regions produce InvalidType
  ...
  def has_buffer_identity(self):
    ...
    if self.op in {Ops.RESHAPE, Ops.MULTI}:
      return self.src[0].has_buffer_identity()
    if self.op is Ops.GETTUPLE and self.src[0].op is Ops.TUPLE:
      return self.src[0].src[self.arg].has_buffer_identity()
  ...
  def _base_buffer_is_realized(self) -> bool:
    ...
    u = self.base
    while u.op is Ops.AFTER:
      u = u.src[0]
    return u.is_realized

  @property
  def buffer(self) -> Buffer | MultiBuffer:
    if self.op in {Ops.CONTIGUOUS, Ops.RESHAPE, Ops.DETACH, Ops.AFTER}:
    ...
    if self is not self.base:
      ...
      if offset is None:
        raise RuntimeError(f"non-contiguous view is not supported for {buf.device} buffer")
    ...
    if isinstance(self.device, tuple):
      ret = MultiBuffer(self.device, self.arg, rdtype).ref(1)
    else:
      ret = Buffer(self.device, self.arg, rdtype).ref(1)
    buffers[self] = ret
    return ret
  ...
  def realized(self) -> Buffer | MultiBuffer | None:
    ...
    if self.op not in (Ops.BUFFER, Ops.MSTACK):
      return None
    # LUNIQUEs are never realized
    if self.op_in_backward_slice_with_self(Ops.LUNIQUE):
      return None
  ...
  def divide_exact(self, v: UOp) -> UOp | None:
    ...
    if self.op is Ops.MUL:
      ...
      if const % div_const == 0 and all(v >= 0 for v in new_count.values()):
        return math.prod(new_count.elements(), start=self.const_like(const // div_const))
    return None  # generic None if we aren't sure

  @property
  def vmin(self) -> PyConst:
    return self._min_max[0]

  @property
  def vmax(self) -> PyConst:
  ...
  def _min_max(self) -> tuple[PyConst, PyConst]:
    if self.op in GroupOp.Binary and not dtypes.is_float(self.dtype):
      ...
      if self.op is Ops.MOD:
        if (c := s1_vmin) == s1_vmax > 0:
          return (
            0 if s0_vmin > 0 else s0_vmin if 0 >= s0_vmin > -c else -(s1_vmax - 1),
            0 if s0_vmax < 0 else s0_vmax if 0 <= s0_vmax < c else c - 1,
          )
        if s1_vmin > 0:
    ...
    if self.op is Ops.PARAM and len(self.src) >= 4:
      return self.src[2].arg, self.src[3].arg
  ...
  def param(slot: int, dtype: DType, shape: tuple[sint, ...] | None = None, device=None, vmin_vmax: tuple[PyConst, PyConst] | None = None, name=None):
    ...
    if vmin_vmax is not None:
      src += (UOp.const(dtype, vmin_vmax[0]), UOp.const(dtype.scalar(), vmin_vmax[1]))
    if name is not None:
      src += (UOp(Ops.NOOP, arg=name),)
  ...
  def param_like(self, slot: int):
    ...
    if self.axis is not None:
      p = p.replace(src=p.src + (UOp(Ops.MULTI, arg=self.axis),))
    return p
...
def exec_alu(op: Ops, dtype: DType, operands, truncate_output=True):
  ...
  if dtype == dtypes.weakint and op in GroupOp.Binary and Invalid in operands:
    return Invalid
...
def print_uops(uops: list[UOp]):
  ...
  for i, u in enumerate(uops):
    ...
    print(f"{i:4d} {str(u.op):20s}: {multirange_str(u.ranges, color=True, pad=10)} {str(u.dtype):40s} {str(formatted_srcs):32s} {u.arg}")
...
def get_location() -> tuple[str, int]:
  ...
  while frm.f_back is not None and not frm.f_back.f_code.co_filename.startswith("<frozen"):
    ...
    if not (fn.endswith("/ops.py") or "/mixin/" in fn):
      break
    frm = frm.f_back
...
class UPat(OpMixin):
  ...
  def __init__(
    ...
    dtype: DType | tuple[DType, ...] | set[DType] | None = None,
  ...
  def dtype(self) -> DType:
    return self.match_dtype[0] if self.match_dtype is not None else dtypes.void

  def _check_dtype(self) -> None:
    pass

  def _ensure_float(self) -> UPat:
    return self
  ...
  @staticmethod
  def any(*src):
    return UPat(src=src, is_any=True)
  ...
  @staticmethod
  def const(dtype: DType | tuple[DType, ...] | None, b: ConstType):
  ...
  def cast(self, dtype=None, **kwargs):
    if dtype is not None and self.match_dtype == (dtype,):
      return self
  ...
  def store(self, *src: UPat, **kwargs):
    return UPat(Ops.STORE, self.match_dtype, (self,) + src, **kwargs)

  def reduce(self, *src: UPat, **kwargs):
    return UPat(Ops.REDUCE, self.match_dtype, src=(self,) + src, **kwargs)

  def broadcast(self, **kwargs):
    return UPat(Ops.VECTORIZE, self.match_dtype, src=self, **kwargs)

  def contiguous(self, *args, **kwargs):
    return UPat(Ops.CONTIGUOUS, dtype=self.match_dtype, src=(self,) + args, **kwargs)

  def after(self, *src: UPat, **kwargs):
    return UPat(Ops.AFTER, self.match_dtype, (self,) + src, **kwargs)

  def end(self, *src: UPat, **kwargs):
    return UPat(Ops.END, self.match_dtype, (self,) + src, **kwargs)

  def const_like(self, b: ConstLike):
    return UPat.const(self.match_dtype, cast(ConstType, b))
  ...
  def match(self: UPat, uop: UOp, store: dict[str, UOp]) -> list[dict[str, UOp]]:
    ...
    if (
      ...
      or (self.match_dtype is not None and uop.dtype not in self.match_dtype and uop.dtype.scalar() not in self.match_dtype)
...
if CAPTURE_PROCESS_REPLAY:
  ...
  def save_to_diskcache():
    ...
    for i, v in enumerate(replay_capture):
      diskcache_put("process_replay", f"{uid}_{i}", v, prepickled=True)
  ...
  def walk_rewrite(self, root: UOp) -> UOp:
    ...
    while stack:
      ...
      if not processed:
        ...
        stack.append((n, True))
        if not self.enter_calls and n.op in {Ops.CALL, Ops.FUNCTION}:
          self.replace[n.src[0]] = n.src[0]
        for x in reversed(n.src):
          if x not in self.replace:
        ...
        if self.pm is not None and (rewritten := self.pm_rewrite(new_n)) is not None:
          new_n = rewritten
  ...
  def unified_rewrite(self, root: UOp) -> UOp:
    ...
    while stack:
      ...
      if stage == 0:
        ...
        if not self.enter_calls and new_n.op in {Ops.CALL, Ops.FUNCTION}:
...
def sint_to_uop(x: sint, dtype=dtypes.weakint) -> UOp:
...
pm_lower_index_dtype = PatternMatcher([
  ...
  (
    UPat(Ops.WHERE, dtypes.weakint, src=(UPat.var("cond"), UPat.var("x").cast(dtypes.weakint), UPat.var("y").cast(dtypes.weakint))),
    lambda cond, x, y: cond.where(x.cast(dt := least_upper_dtype(x.dtype, y.dtype)), y.cast(dt)).cast(dtypes.weakint),
  ),
...
def _index_to_concrete_int(u: UOp) -> UOp:
...
def gate_kernel_sink(x: UOp) -> bool:
  if x.op is Ops.LINEAR:
    return False
  if x.op is Ops.SINK and isinstance(x.arg, KernelInfo):
    return False
  return True
...
def render_marg(ctx, x: UOp):
  if x.op is Ops.PERMUTE:
    return str(x.marg)
  if x.op is Ops.FLIP:
    return str(tuple([i for i, x in enumerate(x.marg) if x]))
  pieces = []
...
pm_pyrender_extra = PatternMatcher([
  ...
  (
    ...
    lambda x: f'UOp.variable("{x.arg[0]}", {x.arg[1]}, {x.arg[2]}{", dtype=" + str(x.dtype) if x.dtype is not dtypes.weakint else ""})',
  ),
  ...
  (
    ...
    lambda x, u, d: f"UOp.new_buffer({repr(d.arg)}, {x.arg}, {x.dtype}, {u.arg})",
  ),
  ...
  (
    ...
    lambda ctx, x, c: (
      "UOp.range("
      + ", ".join([str(c.arg)] + [repr(y) for y in x.arg])
      + (f", src={srcs(ctx, x.src[1:])}" if len(x.src) > 1 else "")
      + (", dtype=" + str(x.dtype) if x.dtype is not dtypes.weakint else "")
      + ")"
    ),
  ),
  ...
  (
    ...
    lambda ctx, x: (
      f"{ctx[x.src[0]]}.index({ctx[x.src[1]]}, "
      + "".join([f"{ctx[xx]}, " for xx in x.src[2:]])
  ...
  (
    UPat(set(syms.keys()) - {Ops.SUB}, src=(UPat(name="y"), UPat(Ops.CONST, src=(UPat(Ops.DEVICE),), name="z")), name="x"),
  ...
  (
    UPat(sugar, name="x"),
    lambda ctx, x: (
      f"{ctx[x.src[0]]}.{x.op.name.lower()}(" + ", ".join([ctx[y] for y in x.src[1:]] + ([f"arg={repr(x.arg)}"] if x.arg is not None else [])) + ")"
    ),
  ),
])
