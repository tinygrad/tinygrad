...
def dtype_from_uop(op:Ops, src:tuple[UOp,...], arg:Any) -> DType|None:
  ...
  match op:
    ...
    case Ops.CUSTOM | Ops.CUSTOMI | Ops.PYLITERAL:
      return dtypes.void
    case Ops.INS:
      # an instruction's dtype is a machine fact: the width of the register it defines, stated by isel and read by the encoding
      return None
    case Ops.NOOP:
...
class UOp(RandMixin, metaclass=UOpMetaClass):
  ...
  def ssimplify(self) -> UOp|ConstType:
    ...
    return ret.dtype.const(ret.src[0].arg) if ret.is_const and ret.src[0].arg is not Invalid else ret
