...
class UOp(MathTrait, metaclass=UOpMetaClass):
  ...
  def st(self) -> ShapeTracker|None:
    ...
    if self.op is Ops.STORE and self.src and self.src[0].st is not None: return self.src[0].st
  ...
  def assign(self, x:UOp): return self.store(x)
  ...
  def buf_uop(self) -> UOp:
    ...
    if self.op is Ops.STORE and self.src[0].op is Ops.DEFINE_ACC: return self.src[0].base
    raise AssertionError(f"buf_uop: unsupported op {self.op}")
  @property
...
class UPat(MathTrait):
  ...
  def assign(self, x:UPat, **kwargs): return self.store(x)
