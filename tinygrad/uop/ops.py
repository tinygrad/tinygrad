...
def sym_infer(uop: UOp | int, var_vals: dict[UOp, int]) -> int:
...
class UOp(MathTrait, metaclass=UOpMetaClass):
  ...
  def parents(self: UOp) -> dict[UOp, None]:
    ...
    for s in self.src:
      ret.update(s.parents)
    return ret

  @property
  def sparents(self: UOp) -> dict[UOp, None]:
    return {self: None, **self.parents}
    ...
    while stack:
      ...
      if not visited:
        if gate is None or gate(node):
          ...
          for parent in reversed(node.src):
            stack.append((parent, False))  # push parents on the stack
      else:
  ...
  def ptrdtype(self) -> PtrDType:
    if not isinstance(self.dtype, PtrDType):
      raise RuntimeError("ptrdtype called on UOp without PtrDType")
    return self.dtype
  ...
  def st(self) -> ShapeTracker | None:
    if self.op is Ops.INDEX and self.src[0].op in {
      Ops.DEFINE_GLOBAL,
      Ops.DEFINE_LOCAL,
      Ops.DEFINE_REG,
      Ops.BUFFER,
      ...
      return None
    if self.op is Ops.BARRIER:
      return None
    if self.op in GroupOp.Block:
      return None
    from tinygrad.shape.shapetracker import ShapeTracker
    ...
    if self.op is Ops.BUFFERIZE:
      return ShapeTracker.from_shape((prod(tuple([int(r.vmax + 1) for r in self.src[1:]])),))
    ...
    if self.op is Ops.CONST and len(self.src) and self.src[0].op is Ops.DEVICE:
      return ShapeTracker.from_shape(())
    if self.op is Ops.STORE and isinstance(self.dtype, PtrDType):
      return ShapeTracker.from_shape((self.dtype.size,))
    # BufferOps and ASSIGN flow ShapeTracker from a direct edge
    if self.op in {Ops.STORE, Ops.ASSIGN, Ops.LOAD}:
    ...
    if self.op in {Ops.BUFFER, Ops.BUFFER_VIEW}:
      return ShapeTracker.from_shape((self.size,))
    if self.op is Ops.KERNEL:
      return ShapeTracker.from_shape((self.arg.ast.size,))
    ...
    if self.op is Ops.CAST and self.src[0].op is Ops.DEFINE_GLOBAL:
      return None

    # otherwise we get the shape from sources
    if not (src_sts := [x.st for x in self.src if x.st is not None]):
      return None
    assert all_same([x.shape for x in src_sts]), f"UOp sources must have the same shape {self} {[x.shape for x in src_sts]}"
    match self.op:
      case Ops.MULTI:
        shape = tuple(self.src[0].shape[a] * len(self.device) if a == self.axis else s for a, s in enumerate(self.src[0].shape))
      case Ops.BITCAST:
        shape = src_sts[0].shape
        if self.dtype.itemsize != (input_sz := self.src[0].dtype.itemsize):
          shape = shape[:-1] + ((shape[-1] * input_sz) // self.dtype.itemsize,)
  ...
  @property
  def size(self) -> int:
    return self.arg[0] if self.op is Ops.BUFFER_VIEW else self.arg if self.op is Ops.BUFFER else unwrap(self.st).size
  ...
  def range(end: sint, *arg):
    if len(arg) == 0:
      raise RuntimeError("range needs an arg")
    if len(arg) == 1:
      arg = arg + (AxisType.LOOP,)
  ...
  def axis(self) -> int | None:
    ...
    if self.op is Ops.PERMUTE:
      return self.arg.index(src_axis) if src_axis is not None else None
    return src_axis
  ...
  @property
  def base(self) -> UOp:
    if (self.op is Ops.VIEW and len(self.src) != 0) or self.op in GroupOp.Movement:
  ...
  def _mop(self, op: Ops, arg) -> UOp:
    ...
    if self.st == ret.st:
      return self  # ignore NOOPs, also check ret.st
    return ret
  ...
  @staticmethod
  def unique():
    return UOp(Ops.UNIQUE, arg=next(UOp.unique_num))
  ...
  def as_buf(self) -> UOp:
    ...
    s = self
    while len(s.src) and s.op is not Ops.BUFFER:
      s = s.src[0]
    return s
  ...
  def pop_const(self) -> tuple[UOp, int]:
    return (self.src[0], self.src[1].arg) if self.op is Ops.ADD and self.src[1].op is Ops.CONST else (self, 0)
  ...
  def _min_max(self) -> tuple[ConstType, ConstType]:
    if self.op in GroupOp.Binary and not dtypes.is_float(self.dtype):
      ...
      if self.op is Ops.AND and s1_vmin == s1_vmax and s0_vmin >= 0 and s1_vmin >= 0:
    ...
    if self.op is Ops.SPECIAL:
      return 0, self.arg[1] - 1 if isinstance(self.arg[1], int) else self.arg[1].vmax - 1
    if self.op is Ops.CONST:
...
python_alu: dict[Ops, Callable] = {
  ...
  Ops.RECIP: lambda x: 1 / x if x != 0 else math.copysign(math.inf, x),
...
def print_uops(uops: list[UOp]):
  for i, u in enumerate(uops):
    ...
    print(f"{i:4d} {str(u.op):20s}: {str(u.dtype):30s} {str(formatted_parents):32s} {u.arg}")
...
class UPat(MathTrait):
  ...
  def index(self, idx: UPat, valid: UPat | None = None):
    return UPat(Ops.INDEX, self.dtype, (self, idx, valid) if valid is not None else (self, idx))

  def view(self, st=None, **kwargs):
    return UPat(Ops.VIEW, self.dtype, (self,), st, **kwargs)
  ...
  def or_broadcasted(self, **kwargs):
    return UPat.any(self, UPat(Ops.VECTORIZE, self.dtype, src=self, **kwargs))
  ...
  def __repr__(self):
    def rep(x):
      ...
      return form % (
        ...
        not x.strict_length,
        "[%s]" if x.src and len(x.src) > 1 else ("(%s)" if x.src else "%s"),
      )
...
def track_uop(u: UOp):
  if (cret := uop_number.get(u)) is not None:
    return cret
  ...
  def _track_func(*args, **kwargs):
    if tracking := (TRACK_MATCH_STATS >= 2 and tracked_ctxs):
      ...
      tracked_ctxs[-1].append(
        ctx := TrackedGraphRewrite(loc, track_uop(args[0]), [], kwargs.get("name", None), depth, kwargs.get("bottom_up", False))
      )
...
if TRACK_MATCH_STATS or PROFILE:
  ...
  def print_match_stats():
    ...
    if VIZ:
      launch_viz(VIZ, temp("rewrites.pkl", append_user=True))
...
class RewriteNotReady(Exception):
  pass
      ...
      try:
        if stage == 0:
          ...
          try:
            ...
            raise RewriteNotReady  # pylint: disable=raise-missing-from
...
def sint_to_uop(x: sint, dtype: DType = dtypes.int) -> UOp:
  return UOp.const(dtype, x) if isinstance(x, int) else x
