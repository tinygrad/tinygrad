...
class UOp(RandMixin, metaclass=UOpMetaClass):
  ...
  def buf_uop(self) -> UOp:
    ...
    s = self
    while len(s.src) and (s.op not in {Ops.BUFFER, Ops.ALLOC, Ops.PARAM, Ops.STAGE, Ops.MSTACK} and
  ...
  def has_buffer_identity(self, after_ok=False):
    ...
    # Tagged all-reduce views preserve the identity of their physical slice.
    return self.op in {Ops.BUFFER, Ops.ALLOC, Ops.PARAM} or (self.op is Ops.SHRINK and self.tag == ("allreduce",))
