...
def _get_clause(self:UPat, base:UOp, depth=0) -> UOp:
  ...
  if self.match_tag is not None:
    if len(self.match_tag) > 1: and_clause.append(UOp(Ops.CUSTOM, src=(base, UOp(Ops.BIND, arg=self.match_tag)), arg="{0}.tag in {1}"))
  ...
  if self.match_device is not None:
    and_clause.append(UOp(Ops.CUSTOM, src=(base, UOp(Ops.BIND, arg=self.match_device)),
                          arg="(isinstance({0}._device, str) and {0}._device.split(':')[0] in {1})"))
