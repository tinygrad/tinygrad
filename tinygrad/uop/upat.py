...
def _get_clause(self:UPat, base:UOp, depth=0) -> UOp:
  ...
  if self.name is not None: and_clause.append(UOp(Ops.STORE, dtype=base.dtype, src=(UOp(Ops.DEFINE_VAR, arg=self.name), base)))
...
def do_process_and(a:UOp) -> UOp|None:
  ...
  # handle assigns
  assigns, new_src = partition(new_src, lambda x: x.op is Ops.STORE and x.src[0].op is Ops.DEFINE_ACC)
  if len(assigns):
    if len(or_clause):
      ...
      for k,v in dict_assigns.items(): new_src.append(UOp(Ops.STORE, k.dtype, src=(k, v)))
