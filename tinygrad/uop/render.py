...
renderer = PatternMatcher([
  ...
  # a typed constant is the pair CAST(dt, CONST(weak v)): it reads as the value it commits to
  (UPat(Ops.CAST, name="x"), lambda ctx,x: str(x.dtype.const(x.src[0].arg)) if x.is_const else f"({str(x.dtype)[7:]})({ctx[x.src[0]]})"),
...
pm_pyrender_extra = PatternMatcher([
  ...
  (UPat(Ops.SPECIAL, src=(UPat(name="c"),), name="x"),
   lambda x,c: f"UOp.special({c.ssimplify()}, {repr(x.arg)}, dtype={x.dtype})" if c.is_const else None),
  ...
  (UPat(Ops.RANGE, src=(UPat(name="c"),), allow_any_len=True, name="x"), lambda ctx,x,c: None if not c.is_const else
    "UOp.range("+', '.join([str(c.ssimplify())] + [repr(y) for y in x.arg])+
