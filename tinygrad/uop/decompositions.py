...
def get_late_rewrite_patterns(ops:tuple[Ops, ...], device:str, disable_fast_idiv:bool) -> PatternMatcher:
  ...
  if Ops.MIN in ops:
    pat += [((UPat.var("a") < UPat.var("b")).where(UPat.var("a"), UPat.var("b")), lambda a,b: a.alu(Ops.MIN, b))]
    if Ops.MAX in ops and Ops.NEG in ops: # -max(-a, -b) -> min(a, b)
      pat += [(UPat.var("a").alu(Ops.NEG).alu(Ops.MAX, UPat.var("b").alu(Ops.NEG)).alu(Ops.NEG), lambda a,b: a.alu(Ops.MIN, b))]
