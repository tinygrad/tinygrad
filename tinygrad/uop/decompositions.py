...
def mantissa_bits(d: DType) -> int:
  return dtypes.finfo(d.scalar())[1]


def exponent_bias(d: DType) -> int:
  return {dtypes.float64: 1023, dtypes.float32: 127, dtypes.float16: 15}[d.scalar()]


def exponent_mask(d: DType) -> int:
  return {dtypes.float64: 2047, dtypes.float32: 255, dtypes.float16: 31}[d.scalar()]
...
def payne_hanek_reduction(d: UOp) -> tuple[UOp, UOp]:
  ...
  def _shl_lazy(x: UOp, y: UOp):
    return (x.cast(dtypes.uint64) * pow2if(y, d.dtype).cast(dtypes.uint64)).cast(dtypes.uint32)

  def _shr_lazy(x: UOp, y: UOp):
    return (x.cast(dtypes.uint64) // pow2if(y, d.dtype).cast(dtypes.uint64)).cast(dtypes.uint32)
  ...
  def _hp_mul(x: UOp, y: UOp) -> UOp:
    return x.cast(dtypes.uint64) * y.cast(dtypes.uint64)
...
def trig_poly(d: UOp, coeff32, coeff64):
  return d * (polyN(d * d, coeff64) if d.dtype.scalar() == dtypes.float64 else polyN(d * d, coeff32))
  ...
  if d.dtype.scalar() == dtypes.float64:
    u = polyN(
      ...
      [
        ...
        0.2402265069591012214e0,
        0.6931471805599452862e0,
        0.1000000000000000000e1,
      ],
    )
  else:
    u = polyN(s, [0.1535920892e-3, 0.1339262701e-2, 0.9618384764e-2, 0.5550347269e-1, 0.2402264476e0, 0.6931471825e0, 1.0])
  u = ldexp2k(u, q)  # u*2^q
...
def xlog2(d: UOp) -> UOp:
  ...
  if d.dtype.scalar() == dtypes.float16:
    return xlog2(d.cast(dtypes.float32)).cast(dtypes.float16)
  ...
  if d.dtype.scalar() == dtypes.float64:
    t = polyN(
      x2,
      [
        0.2211941750456081490e0,
        0.2200768693152277689e0,
        0.2623708057488514656e0,
        0.3205977477944495502e0,
        0.4121985945485324709e0,
        0.5770780162997058982e0,
        0.96179669392608091449,
      ],
    )
    s_hi, s_lo = e + x * 2.885390081777926774, e.const_like(0)
  else:
    t = polyN(x2, [0.4374550283e0, 0.5764790177e0, 0.9618012905120])
...
def fast_idiv(device: str, x: UOp, d: int, dont_cast=False) -> UOp | None:
  ...
  if (largest_factor_of_two_in_d := (d & -d)) > 1:
    if (ret := fast_idiv(device, x // largest_factor_of_two_in_d, d // largest_factor_of_two_in_d, dont_cast=True)) is not None:
      return ret
  if dont_cast:
    return None
  ...
  if dtypes.is_int(next_dtype := promo_lattice[x.dtype][-1]) and is_dtype_supported(next_dtype, None if device == "" else device):
    if m * vmin >= dtypes.min(next_dtype) and m * vmax <= dtypes.max(next_dtype):
      return (
        ((x.cast(next_dtype) * m) >> s).cast(x.dtype) if is_unsigned else ((x.cast(next_dtype) * m) >> s).cast(x.dtype) + (x < 0).where(x.ufix(1), 0)
      )
  return None
...
def get_late_rewrite_patterns(ops: tuple[Ops, ...], force_transcendental=False):
  ...
  for op, f in ((Ops.EXP2, xexp2), (Ops.LOG2, xlog2), (Ops.SIN, xsin)):
    if op not in ops or force_transcendental:
      pat += [
        (UPat(op, dtype=TRANSCENDENTAL_DTYPES, src=(UPat.var("d"),)), f),
        (
  ...
  if Ops.SQRT not in ops:
    pat.append((UPat(Ops.SQRT, src=UPat.var("d")), lambda d: xpow(d, d.const_like(0.5))))
  # rewrite MOD to AND (which should always be supported, but not for generic in tests): x % (2**y) -> x & (2**y-1)
  if Ops.AND in ops:
    pat += [(UPat.var("x", dtypes.ints) % UPat.cvar("c"), lambda x, c: x & (c.arg - 1) if c.arg in powers_of_two else None)]
  if Ops.OR in ops:
    pat += [(UPat.var("x", dtypes.bool).logical_not() & UPat.var("y", dtypes.bool).logical_not(), lambda x, y: (x | y).logical_not())]
  # rewrite MUL/IDIV to SHL+SHR: x*(2**y) -> shl(x,y) and x//(2**y) -> shr(x,y)
  if Ops.SHL in ops:
    pat += [(UPat.var("x", dtypes.ints) * UPat.cvar("c"), lambda c, x: x << v if (v := powers_of_two.get(c.arg, 0)) else None)]
  if Ops.SHR in ops:
  ...
  if Ops.CMPLT in ops:
    ...
    pat += [
      ...
      (
        ...
        lambda x, c1, c2: x.eq(c1 + 1) if c1.arg + 1 == c2.arg - 1 else None,
      ),  # (c-1)<x & x<(c+1) -> x==c
    ]
  if Ops.CMPEQ in ops:
    pat += [(UPat.var("x").ne(UPat.var("y")).logical_not(), lambda x, y: x.alu(Ops.CMPEQ, y))]
