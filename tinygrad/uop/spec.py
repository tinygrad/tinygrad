...
spec_tensor = PatternMatcher([
  ...
  # an UNSHARD carries the value and one sharding RANGE per sharded axis (usually DEVICE, but any typed range can be sharded over)
  (UPat(Ops.UNSHARD, name="multi"), lambda multi: len(multi.src) == 1+len(multi.arg) and matches_dtype(multi.src[0], multi.dtype)
    and all(isinstance(a, int) for a in multi.arg) and all(r.op is Ops.RANGE and isinstance(r.arg[-1], AxisType) for r in multi.src[1:])),
