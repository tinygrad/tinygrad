...
tensor_uop_spec = buffer_spec+assign_spec+PatternMatcher([
  ...
  (UPat(Ops.VIEW, src=(UPat.var("x"),)), lambda x: x.base.op in {Ops.BUFFER, Ops.BUFFER_VIEW, Ops.ASSIGN, Ops.CONST, Ops.FCONST, Ops.DEVICE}),
  ...
  (UPat(Ops.FCONST), lambda: True),
  # (UPat(Ops.FCONST, src=(UPat(Ops.VIEW, name="st", src=(UPat(Ops.DEVICE),)),), name="c"),
  #  lambda st,c: st.st.views[0].mask is None and len(st.st.views) == 1 and all(s == 0 for s in st.st.views[0].strides)
  #  and len(st.shape)==c.arg.__code__.co_argcount),
