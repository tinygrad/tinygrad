...
try:
  ...
  z3_alu: dict[Ops, Callable] = python_alu | {
    ...
    Ops.TRUNC: lambda a: a if a.is_int() else z3.ToReal(z3.If(a >= 0, z3.ToInt(a), -z3.ToInt(-a))),
  }
  ...
  z3_renderer = PatternMatcher([
    ...
    (
      ...
      lambda x, ctx: UOp(Ops.NOOP, arg=(ctx[0], create_bounded(x.arg, 0, x.src[0].arg[1] - 1, ctx[0]))),
    ),
    ...
    (
      UPat(Ops.CAST, dtype=dtypes.ints, name="x"),
      lambda x, ctx: UOp(Ops.NOOP, arg=(ctx[0], create_bounded(f"cast{ctx[1].setdefault(x, len(ctx[1]))}", x.dtype.min, x.dtype.max, ctx[0]))),
    ),
    (
      UPat(Ops.CAST, dtype=dtypes.bool, name="x"),
      lambda x, ctx: UOp(Ops.NOOP, arg=(ctx[0], z3.Bool(f"cast{ctx[1].setdefault(x, len(ctx[1]))}", ctx=ctx[0].ctx))),
    ),
    (
    ...
    (
      ...
      lambda x, ctx: UOp(Ops.NOOP, arg=(ctx[0], z3.Bool(f"float_cmp{ctx[1].setdefault(x, len(ctx[1]))}", ctx=ctx[0].ctx))),
    ),
  ])
...
buffer_spec = PatternMatcher([
  ...
  (
    ...
    lambda d: isinstance(d.arg, str) or (isinstance(d.arg, tuple) and all(isinstance(s, str) for s in d.arg)),
  ),
  (
...
tensor_uop_spec = (
  ...
  + PatternMatcher([
    (
      ...
      # naturally correct
      lambda mv, x: (isinstance(mv.arg, tuple) and mv.dtype == x.dtype)
      or
...
def validate_index(idx: UOp, gate: UOp = UOp.const(dtypes.bool, True)):
  if IGNORE_OOB or isinstance(idx.dtype, ImageDType) or (sz := idx.src[0].ptrdtype.size) == -1:
    return True
  ...
  if not z3_imported:
    raise ImportError('z3 is required for bounds checking, try IGNORE_OOB=0 or "pip install z3-solver"')
...
spec = PatternMatcher([
  ...
  (
    ...
    lambda x: len(x.src) > 1 and len(x.src) == x.dtype.count and all(x.dtype == y.dtype.vec(len(x.src)) for y in x.src),
  ),
