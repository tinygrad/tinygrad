...
spec_shared = PatternMatcher([
  ...
  # STORE: the target must be storage (a BUFFER/RETURNED/PARAM or an AFTER/BITCAST/view of one); INDEX stores are checked above
  (UPat(Ops.STORE, dtypes.void, (UPat(name="x"), UPat())), lambda x:
   True if (b:=store_target_base(x)).op in {Ops.BUFFER, Ops.PARAM, Ops.RETURNED} else None if b.op is Ops.INDEX else False),
