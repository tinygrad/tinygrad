...
def payne_hanek_reduction(d:UOp) -> tuple[UOp, UOp]:
  ...
  i = e.cast_vec(dtypes.uint64) >> 5
  e = e.cast_vec(dtypes.uint32) & 31
  offset = 32 - e
  ...
  def _shl_lazy(x, y): return (x.cast_vec(dtypes.uint64) << y.cast_vec(dtypes.uint64)).cast_vec(dtypes.uint32)
  def _shr_lazy(x, y): return (x.cast_vec(dtypes.uint64) >> y.cast_vec(dtypes.uint64)).cast_vec(dtypes.uint32)
