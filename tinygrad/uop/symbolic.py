...
symbolic_simple = propagate_invalid + PatternMatcher([
  ...
  # fold nested where with same condition: cond.where(cond.where(a, b), f) -> cond.where(a, f)
  (UPat.var("c").where(UPat.var("c").where(UPat.var("a"), UPat.var()), UPat.var("f")), lambda c,a,f: c.where(a,f)),
  # fold nested where with same condition: cond.where(t, cond.where(a, b)) -> cond.where(t, b)
