...
def simplify_pow(x: UOp, c: UOp) -> UOp | None:
  ...
  if c.arg == 0:
    return x.const_like(1)
  if int(c.arg - 0.5) + 0.5 == c.arg:
    return x.pow(c.const_like(c.arg - 0.5)) * x.sqrt()
  if int(c.arg) == c.arg:
    return (y := x.pow(c.const_like(c.arg // 2))) * y * (x if c.arg % 2 == 1 else 1)
  return None


def fold_bitcast(root: UOp, c: UOp) -> UOp | None:
  if (from_fmt := c.dtype.scalar().fmt) is None or (to_fmt := root.dtype.scalar().fmt) is None:
    return None
  if c.dtype.itemsize != root.dtype.itemsize:
    return None

  def convert(v: ConstType):
    return struct.unpack(to_fmt, struct.pack(from_fmt, v))[0]
...
symbolic_simple = PatternMatcher([
  ...
  (
    ...
    lambda x, c1, c2, c3: x * c2 if c1.arg * c2.arg == c3.arg else None,
  ),  # (x%c1)*c2+(x//c1)*c3 = x*c2 if c1*c2==c3
  ...
  (
    ...
    lambda x: x.const_like(False).cast(dtypes.bool.vec(x.dtype.count)),
  ),  # x != x -> False (only ints)
  # x*0 -> 0 or 0*x -> 0
  ...
  (
    ...
    lambda x, a, b: x.cast(b.dtype)
    if can_safe_cast(x.dtype, a.dtype) and not (a.dtype == dtypes.float and (b.dtype == dtypes.bfloat16 or x.dtype == dtypes.bfloat16))
    else None,
  ),
  # ** pow **
  ...
  (
    UPat.var("b").where(UPat.var("x", dtypes.uint32).cast(dtypes.uint64), UPat.const(dtypes.uint64, 0)).cast(dtypes.uint32),
    lambda b, x: b.where(x, 0),
  ),
])
...
def fold_unrolled_divs(divs: UOp, denominator: int, fac=1) -> UOp | None:
  ...
  for u in split_uop(divs, Ops.ADD):
    if fac != 1:
      if u.op is not Ops.MUL or u.src[1].op is not Ops.CONST or u.src[1].arg != fac:
        return None
      u = u.src[0]
    if not (u.op is Ops.IDIV and u.src[1].op is Ops.CONST):
      return None
    if denominator != u.src[1].arg:
      return None
    if (s0 := u.src[0]).vmin < 0:
      return None
  ...
  for i in range(denominator - len(seen_const)):
    if ans is not None and 0 <= ans.vmin and ans.vmax + i < denominator:
      seen_const.append(i)
...
def canonicalize_simplex(X: UOp) -> UOp | None:
  ...
  for u in split_uop(X, Ops.ADD):
    ...
    if u.op is Ops.MUL and u.src[1].op is Ops.CONST and u.src[1].arg > 0:
      changed = True
      u = u.src[0]
    if not (u.op in GroupOp.Irreducible and u.vmin >= 0):
      return None
    ret.append(u)
...
def cancel_divmod(d: UOp, x: UOp, y: UOp) -> UOp | None:
  ...
  if y_min == y_max == 0:
    raise ZeroDivisionError(f"{'Division' if d.op is Ops.IDIV else 'Mod'} by zero trying to rewrite {x.alu(d.op, y)}")
...
def remove_nested_mod(m: UOp, x: UOp, y: UOp) -> UOp | None:
  ...
  if something_changed and new_x.vmin >= 0:
    return new_x % y
  return None
...
def fold_binary_numerator(d: UOp, x: UOp, y: UOp) -> UOp | None:
  ...
  if ((c := y.arg) < 0) or (x.dtype.count > 1):
    return None
  x, const = x.pop_const()
...
def fold_divmod_congruence(d: UOp, x: UOp, y: UOp) -> UOp | None:
  ...
  if (x.vmin < 0 and CORRECT_DIVMOD_FOLDING) or ((c := y.arg) < 0) or (x.dtype.count > 1):
    return None
  x, const = x.pop_const()
  terms, factors = zip(*[(u.divides(f := u.const_factor()), f) for u in split_uop(x, Ops.ADD)])
...
def divide_by_gcd(d: UOp, x: UOp, y: UOp) -> UOp | None:
  ...
  terms, factors = zip(*[(u.divides(f := u.const_factor()), f) for u in split_uop(x, Ops.ADD)])
  if (gcd := math.gcd(y.arg, *factors)) == 1:
    return None
...
def nest_div_by_smallest_factor(d: UOp, x: UOp, y: UOp) -> UOp | None:
  ...
  if (1 < div < c) and (newxs := (newx := (x // div)).simplify()) is not newx and x.vmin >= 0 and newx.vmin >= 0:
    return newxs // (c // div)
  return None
...
def simplify_remainder(d: UOp, x: UOp, y: UOp) -> UOp | None:
  ...
  if const % c == const and gcd == 1 and not any(r == 0 or (r != f and d.op is Ops.MOD) for r, f in zip(remainders, factors)):
    return None
  ...
  if (x.vmin < 0 or rem.vmin < 0) and remainders:
    return None
  if d.op is Ops.MOD:
    return gcd * (rem % (c // gcd)) + const % gcd
...
def gep_through_wmma(gep: UOp, wmma: UOp):
  ...
  for i in range(out_sz):
    if tuple(x - i for x in gep.arg[i::out_sz]) != wmma_idxs:
      return None
  tsrcs = []
  for s, sz in zip(wmma.src, wmma.arg[6]):
    ...
    for w in wmma_idxs:
      src_args += list(range((w // out_sz) * ssz, (w // out_sz) * ssz + ssz))
...
gep_pushing = PatternMatcher([
  ...
  (UPat(Ops.GEP, name="g2").f(Ops.GEP, name="g1"), lambda g1, g2: g2.src[0].gep(tuple(g2.arg[g1.arg[i]] for i in range(len(g1.arg))))),
  (
  ...
  (
    ...
    if not isinstance(gep.dtype, PtrDType)
    else None,
  ),
  # CAT can't be rendered. it's a VECTORIZE on vectors, we expand to a single VECTORIZEs with GEPs (TODO: move this later)
  (
    UPat(Ops.CAT, name="x"),
    lambda x: UOp(Ops.VECTORIZE, x.dtype, tuple(y.gep(i) for y in x.src for i in range(y.dtype.count)))
    if not isinstance(x.dtype, PtrDType)
    else None,
  ),
...
symbolic = (
  ...
  + PatternMatcher([
    ...
    (
      ...
      lambda alu, c, t, tt, f, ff: c.where(t.alu(alu.op, tt), f.alu(alu.op, ff))
      if t.op == tt.op == Ops.CONST or f.op == ff.op == Ops.CONST
      else None,
    ),
    ...
    *(
      (UPat.var("x").alu(op, UPat.cvar("c1")).alu(op, UPat.cvar("c2")).named("f"), lambda f, x, c1, c2: x.alu(f.op, c1.alu(f.op, c2)))
      for op in GroupOp.Associative
    ),
    ...
    # div folding
    (
      (UPat.var("x") // UPat.cvar("c") + UPat.cvar("a")) // UPat.cvar("d"),
      lambda x, c, a, d: (x + a * c) // (c * d)
      if c.vmin > 0 and d.vmin > 0 and ((x.vmin >= 0 and a.vmin >= 0) or (x.vmax <= 0 and a.vmax <= 0))
      else None,
    ),  # (x//c+a)//d -> (x+a*c)//(c*d)
...
def parse_valid(valid: UOp) -> tuple[UOp, bool, int]:
  ...
  # (X < c).ne(True) -> X >= c
  if (
    valid.op is Ops.CMPNE
    and valid.src[1].op is Ops.CONST
    and valid.src[1].arg == 1
    and (s0 := valid.src[0]).op is Ops.CMPLT
    and s0.src[1].op is Ops.CONST
  ):
    return s0.src[0], False, s0.src[1].arg
  # X < c -> X <= c-1
  if valid.op is Ops.CMPLT and valid.src[1].op is Ops.CONST and dtypes.is_int(valid.src[0].dtype):
    return valid.src[0], True, valid.src[1].arg - 1
  ...
  for expr, v in bounds.items():
    ...
    for candidate in candidates:
      ...
      if uop.op is Ops.VECTORIZE and len(uop.src) == 2:
        if all_same([uops.src[0] for uops in newuops]):
          uop = uop.replace(src=(newuops[0].src[0], uop.src[1]))
        if all_same([uops.src[1] for uops in newuops]):
          uop = uop.replace(src=(uop.src[0], newuops[0].src[1]))
      elif all_same(newuops):
        uop = newuops[0]
...
def _valid_priority(v: UOp, valids: list[UOp]):
  ...
  try:
    return sum(-1 if parse_valid(v)[0] in other.toposort() else 0 for other in valids)
  except ValueError:
    return 0
...
def reduce_mul_chain(r: UOp):
  if r.arg not in {Ops.ADD, Ops.MAX}:
    return None
  if r.dtype != r.src[0].dtype:
    return None
  inside, outside = [], []
  for m in split_uop(r.src[0], Ops.MUL):
    m_parents = m.toposort()
    if all(r not in m_parents for r in r.src[1:]) and (r.arg != Ops.MAX or m.vmin >= 0):
      outside.append(m)
    else:
      inside.append(m)
  if len(outside) == 0:
...
sym = (
  ...
  + PatternMatcher([
    ...
    (
      UPat.store(
        UPat(Ops.INDEX, name="index"),
        UPat.var("gate").where(UPat.var("alt"), UPat.load(UPat(Ops.INDEX, name="index"))),
        allow_any_len=True,
        name="store",
      ),
    ...
    (
      ...
      lambda x: UOp(Ops.NOOP) if x.op is Ops.STORE else x.const_like(0),
    ),  # NULL pointer store does nothing. NULL pointer load produces 0
    ...
    (
      ...
      if any(x.op in REMOVE_FROM_BARRIER for x in root.src)
      else None,
    ),
    (
      ...
      if any(x.op in REMOVE_FROM_SINK for x in root.src)
      else None,
    ),
    ...
    (
      (UPat.var("x") * UPat.cvar("c", vec=False)).reduce(arg=Ops.ADD, name="r", allow_any_len=True),
