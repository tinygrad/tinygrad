...
from tinygrad.dtype import ConstType, dtypes, PtrDType, AddrSpace, can_safe_cast, Invalid, ImageDType
...
pm_remove_image_invalid = PatternMatcher([  # ImageDType buffers can never OOB
  (UPat(Ops.INDEX, src=(UPat.var("buf"), UPat().where(UPat.var("x"), UPat.const(dtypes.index, Invalid)))), lambda buf,x:
  buf.index(x) if isinstance(buf.dtype, ImageDType) else None),
])
