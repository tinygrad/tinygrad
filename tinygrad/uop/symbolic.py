...
propagate_invalid = PatternMatcher([
  ...
  *((invalid_pat.alu(op, UPat(dtype=dtypes.index)).named("alu"), lambda alu,i: i) for op in GroupOp.Binary-GroupOp.Comparison),
  # a.where(b.where(c, invalid), invalid) -> (a & b).where(c, invalid)
  (UPat.var("a").where(UPat.var("b").where(UPat.var("c"), invalid_pat), invalid_pat), lambda a,b,c,i: (a&b).where(c,i)),
])
