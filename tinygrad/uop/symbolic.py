...
sym = symbolic_flat+PatternMatcher([
  ...
  (UPat(Ops.STORE,src=(UPat.var("x", custom_early_reject={Ops.INDEX, Ops.GEP}),UPat.var("x"))),lambda x: x),
  ...
  (UPat(Ops.STORE, src=(UPat.cvar(), UPat.var("x")), allow_any_len=True),lambda x: x ),     # an ASSIGN to a const is a NOOP
  # x!=0 -> (bool)x
