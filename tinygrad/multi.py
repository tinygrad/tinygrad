...
class MultiLazyBuffer(MathTrait):
  ...
  def alu(self, op:Union[MetaOps, UnaryOps, BinaryOps, TernaryOps], *in_srcs:MultiLazyBuffer) -> MultiLazyBuffer:
    ...
    for mlb in msrcs:
      ...
      elif mlb.axis is None and axis is not None and bounds is not None: srcs.append(to_sharded(mlb.lbs, axis, bounds))
      elif axis is not None and bounds is not None: srcs.append(to_sharded([mlb.copy_to_device(lb.device) for lb in mlb.lbs], axis, bounds))
