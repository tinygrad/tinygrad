...
class Kernel:
  def __init__(self, ast:UOp, opts:Renderer|None=None):
    ...
    self.sts.append(ShapeTracker.from_shape(tuple([smax(*s) for s in zip(*[x.shape + (1,)*(max_ndim-len(x.shape)) for x in self.sts])]), (0,)*max_ndim))
  ...
  def apply_opt(self, opt:Opt, append_opt:bool=True):
    ...
    if opt.op is OptOps.LOCAL:    # cyan
      ...
      for i,st in enumerate(self.sts):
        if self.axis_types[axis] in {AxisType.REDUCE, AxisType.UNROLL, AxisType.GROUP_REDUCE} and len(st.shape) < len(self.full_shape): continue
        s = st.shape[axis]
