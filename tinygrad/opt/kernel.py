    ...
    def fixup_ast(op:UOp) -> UOp:
      ...
      if op.op is Ops.REDUCE_AXIS:
        ...
        if (tc := self.tensor_core) and self.use_tensor_cores == 1:
          ...
          tc_reduce_axes = self.shape_str_to_axis(["r0", "r1", "r2"])
          base_upcast_axes = tuple([(s,2) for s in self.shape_str_to_axis([f"u{i}" for i in range(len(tc.get_upcast_axes())-1, -1, -1)] + \
                                                                          [f"r{i}" for i in range(len(tc.get_reduce_axes())-1, -1, -1)])])
