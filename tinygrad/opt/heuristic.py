...
def hand_coded_optimizations(k:Kernel) -> list[Opt]:
  ...
  if buf.src[0].dtype.__class__ is ImageDType and len(unit_stride_axes_mul_4) and all(x not in {AxisType.UNROLL,AxisType.UPCAST} for x in unit_stride_axes_mul_4):
