...
class TransformerBlock(FFNBlock):
  ...
  def _attention_with_norm_fused(self, x_raw:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    B, T, _ = x_raw.shape
    assert isinstance(B, int) and isinstance(T, int), "fused_attn_qkv requires concrete B, T"
  ...
  def _attention_core(self, q:Tensor, k:Tensor, v:Tensor, x_shape, start_pos:int|UOp) -> Tensor:
    ...
    mask = Tensor.full((1, 1, T, start_pos+T), float("-inf"), dtype=q.dtype, device=q.device).triu(start_pos+1) if resolve(T != 1) else None
