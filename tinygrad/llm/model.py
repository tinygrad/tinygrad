...
class TransformerBlock(FFNBlock):
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    cache_pos = position if hasattr(self, "cache_kv_scale") and self.config.ssm is not None and resolve(T != 1) else start_pos
    attn = cached_attention(q, Tensor.stack(k, v), self.cache_kv, getattr(self, "cache_kv_scale", None), cache_pos)
