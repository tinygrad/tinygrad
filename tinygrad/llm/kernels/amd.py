...
def _reg(shape:tuple[int, ...], slot:int, value:float, dep:UOp|None=None) -> UOp:
  ret = UOp.alloc(shape, dtypes.float, slot=slot, addrspace=AddrSpace.REG)
...
def _wmma_stores(out, outputs, tokens, accs, update, half, lane, wave, output_waves, rdna4):
  ...
  lds = UOp.alloc((output_waves, 32, len(flat_accs)*8), dtypes.float32, slot=33, addrspace=AddrSpace.LOCAL)
...
def _quant_linear_wmma(out, x, out_features, in_features, type_words, layout, dequant, name, rdna4):
  ...
  accs = tuple(tuple(UOp.alloc((8,), dtypes.float32, slot=ot*(token_tile//16)+tile, addrspace=AddrSpace.REG)
...
def _iq4_linear_f16_wmma_kernel(out:UOp, raw:UOp, x:UOp, lut:UOp, out_features:int, in_features:int, rdna4:bool=False) -> UOp:
  ...
  local_lut = UOp.alloc((256,), dtypes.uint32, slot=32, addrspace=AddrSpace.LOCAL)
...
def _quant_linear_f16_wmma_kernel(out:UOp, raw:UOp, x:UOp, *grids:UOp,
  ...
  grid = None
  if grids:
    grid = UOp.alloc((int(grids[0].numel()),), dtypes.uint32, slot=32, addrspace=AddrSpace.LOCAL)
...
def _amd_flash_attention_decode_partial(out, stats, q, cache_kv, valid_kv_len, max_kv_len, block_n, waves=4):
  ...
  acc_lds = UOp.alloc((WAVES, G, D + (LDS_PAD if G == SEC else 0)), dtypes.half, slot=0, addrspace=AddrSpace.LOCAL)[:, :, :D]
  ml_lds = UOp.alloc((WAVES, G, 2), dtypes.float, slot=1, addrspace=AddrSpace.LOCAL)
...
def _amd_flash_decode_combine(o:UOp, partial:UOp, stats:UOp, live:int|UOp) -> UOp:
  ...
  chunk_max = UOp.alloc((1,), dtypes.float, slot=0, addrspace=AddrSpace.REG)
  ...
  acc = UOp.alloc((NPD,), dtypes.float, slot=1, addrspace=AddrSpace.REG)
  weight_sum = UOp.alloc((1,), dtypes.float, slot=2, addrspace=AddrSpace.REG)
...
def _amd_flash_attention(o:UOp, q:UOp, cache:UOp, valid_kv_len:int|UOp, q_start:int|UOp|None=None, rdna4:bool=False) -> UOp:
  ...
  QP_lds = UOp.alloc((BLOCK_M, D + LDS_PAD), dtypes.half, slot=0, addrspace=AddrSpace.LOCAL)
  KV_lds = UOp.alloc((BLOCK_N, D + LDS_PAD), dtypes.half, slot=1, addrspace=AddrSpace.LOCAL)[:, :D]
  ...
  beta_i, ri4, rj4 = UOp.alloc((TM,), dtypes.float, slot=9, addrspace=AddrSpace.REG), UOp.range(TM, 330), UOp.range(TD, 331)
  ...
  V_lds = UOp.alloc((D, BLOCK_N + LDS_PAD), dtypes.half, slot=1, addrspace=AddrSpace.LOCAL)[:, :BLOCK_N]
...
def _gated_delta_prefill_kernel(core:UOp, q:UOp, k:UOp, v:UOp, beta:UOp, alpha:UOp, state:UOp, kq:UOp, start_pos:UOp|None=None) -> UOp:
  ...
  current = UOp.alloc((row_tile*key_dim//32,), dtypes.float32, slot=0, addrspace=AddrSpace.REG)
