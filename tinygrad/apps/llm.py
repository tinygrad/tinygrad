...
class Handler(HTTPRequestHandler):
  ...
  def do_POST(self):
    ...
    if self.path == "/v1/chat/completions":
      ...
      if tok.preset == 'kimi-k2' and (not messages or messages[0]["role"] != "system"):
...
if __name__ == "__main__":
  ...
  # interactive chat
  ids: list[int] = [] if tok.preset == 'kimi-k2' else [bos_id] if bos_id is not None else []
  if tok.preset == 'kimi-k2': ids += tok.role("system") + tok.encode("You are a helpful assistant") + tok.end_turn(eos_id)
  while 1:
    try:
