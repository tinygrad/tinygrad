...
class TransformerBlock:
  ...
  @function
  def _attention(self, q:Tensor, k:Tensor, v:Tensor, start_pos:int|UOp):
    B, _, T = q.shape[:3]
    # NOTE: this mask is causal_lower_right, not the causal_upper_left generated by is_casual = True
    mask = Tensor.full((1, 1, T, start_pos+T), float("-inf"), dtype=q.dtype, device=q.device).triu(int(start_pos)+1) if T > 1 else None
    attn = q.scaled_dot_product_attention(k, v, attn_mask=mask, enable_gqa=True)     # (B,H,T,Hd)
    attn = attn.transpose(1, 2).reshape(B, T, -1)                                    # back to (B,T,D)
    return self.attn_output(attn)

  def _kv(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    # assign kv cache (TODO: support this in function)
    self.cache_kv[:, :, :, start_pos:start_pos+T, :].assign(Tensor.stack(k, v))
    k = self.cache_kv[0, :, :, 0:start_pos+T, :]
    v = self.cache_kv[1, :, :, 0:start_pos+T, :]
    return x + self._attention(q, k, v, start_pos)
  ...
  def __call__(self, x: Tensor, start_pos: int|UOp):
    # create KV cache outside the function
    ...
    return self._feed_forward(self._kv(x, start_pos)).contiguous()
