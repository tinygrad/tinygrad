...
merge_views = PatternMatcher([
  ...
  (UPat(GroupOp.All-{Ops.DEFINE_REG}).view(name="view"),
...
add_buffer_ops = PatternMatcher([
  # LOAD
  (UPat(Ops.BUFFER, name="x"), lambda ctx,x: UOp.load(UOp(Ops.DEFINE_REG, x.dtype.ptr(), (), ("global", ctx.index(x))).view(x.st),)),
  ...
  (UPat(Ops.SINK, src=UPat(GroupOp.All-{Ops.STORE}), name="sink"), lambda ctx,sink:
   UOp.sink(*[UOp.store(UOp(Ops.DEFINE_REG, (s:=x.base).dtype.ptr(), (), ("global", i)).view(s.st), s) for i,x in enumerate(sink.src)])),
  # passthrough ASSIGN
...
fix_kernel_ops = PatternMatcher([
  ...
  (UPat(GroupOp.All-{Ops.DEFINE_REG, Ops.VIEW}, name="x"), lambda x: x.replace(dtype=x.dtype.base) if isinstance(x.dtype, ImageDType) else None),
