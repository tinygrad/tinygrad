...
from tinygrad.shape.symbolic import Variable, NumNode, Node, SumNode, MulNode, DivNode, ModNode, MaxNode, LtNode, AndNode
...
class Linearizer(Kernel):
  ...
  def const(self, b:Union[int,float], dtype=None, insert_before=None) -> UOp:
    return self.uops.add(UOps.CONST, dtype or (dtypes.default_float if isinstance(b, float) else dtypes.default_int), tuple(), int(b) if dtype and dtypes.is_int(dtype) else b, insert_before=insert_before)
  ...
  render_ops: Any = { Variable: lambda self, ops, ctx: ctx.loop_uops[self.expr], NumNode: lambda self, ops, ctx: ctx.const(self.b),
                ...
                MaxNode: lambda self, ops, ctx: ctx.uop_alu_idx(self.a.render(ops, ctx), self.b, ops, ctx, BinaryOps.MAX),
  ...
  def linearize(self):
    ...
    # optimize the uops
    self.uops.uoptimize(self.render_ops, self)
