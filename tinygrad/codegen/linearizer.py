...
class Linearizer(Kernel):
  def uop_alu_idx(self, a:UOp, b, ops, ctx:Linearizer, op, dtype:DType=dtypes.int32) -> UOp:
    render_b:UOp = cast(UOp, (NumNode(b) if not isinstance(b, Node) else b).render(ops, ctx))
    # return UOp.alu(op, a, render_b) TODO: fix bool
  ...
  def linearize(self):
    ...
    for lb in self.local_alias.values():
      self.buf_uops[self.bufs.index(lb)] = self.uops.add(UOps.DEFINE_LOCAL, # TODO why is this float32?
    ...
    if self.group_for_reduces:
      ...
      self.bufs.append(LocalBuffer("temp", self.sts[-1].size, self.reduceop.dtype))
      self.buf_uops.append(self.uops.add(UOps.DEFINE_LOCAL, PtrDType(self.reduceop.dtype), (), ("temp", self.sts[-1].size)))
