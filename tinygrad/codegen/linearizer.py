...
class Linearizer(Kernel):
  ...
  def const(self, b:ConstType, dtype:DType=dtypes.int32, insert_before:Optional[UOp]=None) -> UOp:
      ...
      if key not in self.load_cache:
        if acc is not None:
          ...
          if valid.min == 0 and valid.max == 1:
            ...
            self.load_cache[key] = self.uops.add(UOps.ALU, localtype, (valid_rendered, self.load_cache[key], self.const(invalid_value, localtype)), TernaryOps.WHERE, insert_before=insert_before)  # noqa: E501
        elif isinstance(buf.dtype, ImageDType):
          buf_uop = self.buf_uops[i]
          assert buf_uop is not None, f"buffer {i} wasn't UOped"
          image_idx, valid = to_image_idx(buf.dtype.shape, idx, valid)
          rendered_idx = self.uops.add(UOps.CAST, dtypes.int.vec(2), tuple(x.render(self.render_ops, self) for x in image_idx), insert_before=insert_before)  # noqa: E501
  ...
  def global_store(self, i:int, idxs:List[Node], store:List[UOp], insert_before:Optional[UOp]=None) -> List[UOp]:
    ...
    for _idx, var in store_offset.items():
      ...
      else: stores.append(self.uops.add(UOps.STORE, None, (buf_uop, rendered_idx, var, valid.render(self.render_ops, self)), insert_before=insert_before))  # noqa: E501
    return stores
  ...
  def render_loop(self, xx:List[Variable], insert_before:Optional[UOp]=None) -> Tuple[UOp, ...]:
    new_loops = {x.expr:self.uops.add(UOps.LOOP, dtypes.int32, (
      ...
      self.const(x.max+1) if isinstance(x.max, int) else cast(Node, x.max+1).render(self.render_ops, self)), cachable=False, insert_before=insert_before) for x in xx if not isinstance(x, NumNode) and x.expr is not None}  # noqa: E501
  ...
  def _render_reduceop(self, reduceop: LazyOp, loaded_buffers:Dict[Union[MemBuffer, ConstBuffer, LocalBuffer], List[UOp]], \
                      global_idxs, local_idxs, upcast_idxs, reduce_idxs, insert_before=Optional[UOp]):
    # define indicies
    ...
    self.reduce_acc[reduceop] = acc = self.global_load(out_buf, global_idxs+local_idxs+fake_reduce_idxs+upcast_idxs, self.get_reduce_acc(reduceop), insert_before=insert_before)  # noqa: E501

    # reduce loop
    loop_ctx = self.render_loop(reduce_idxs, insert_before=insert_before)

    # store local aliases
    locals_to_store = [(localbuf_idx, buf_idxs, self.global_load(i, buf_idxs, insert_before=insert_before)) for i, localbuf_idx, buf_idxs in alias_buf_idxs]  # noqa: E501
    ...
    if (tc:=self.tensor_core):
      ...
      def upcast_strides(buf:int):
        strides, next = [], 1
        for (sz, stride, reduce) in self.upcasted_axis(buf)[cast(TensorCore, tc).num_upcasts():]:
      ...
      for iter in [x[::-1] for x in itertools.product(*[x for x in [range(sz) for _,sz in upcasts[0]][::-1]])]:
        ...
        ops = (self.uops.add(UOps.CAST, tc.dtype_in.vec(wmma_sz[0]), tuple(locals_to_store[0][2][offs[0]:offs[0]+wmma_sz[0]]), insert_before=insert_before),  # noqa: E501
                self.uops.add(UOps.CAST, tc.dtype_in.vec(wmma_sz[1]), tuple(locals_to_store[1][2][offs[1]:offs[1]+wmma_sz[1]]), insert_before=insert_before),  # noqa: E501
                self.uops.add(UOps.CAST, (dt3:=tc.dtype_out.vec(wmma_sz[2])), tuple(op3:=acc[offs[2]:offs[2]+wmma_sz[2]]), insert_before=insert_before)) # noqa: E501
        ...
        for z in range(wmma_sz[2]): # TODO: don't need to DEFINE_ACC, pass to WMMA in op3, or PHI accs that are not valid
          acc[offs[2]+z] = self.uops.add(UOps.PHI, tc.dtype_out, (op3[z], self.uops.add(UOps.GEP, tc.dtype_out, (ret,), z, insert_before=insert_before)) + loop_ctx, insert_before=insert_before)  # noqa: E501
    else:
    ...
    if self.group_for_reduces:
      ...
      stores = self.global_store(-1, fake_global_idxs+local_idxs+fake_reduce_idxs+upcast_idxs, acc, insert_before=insert_before)  # store accumulators
      ...
      self.reduce_acc[reduceop] = acc = self.global_load(0, fake_global_idxs+local_idxs+fake_reduce_idxs+upcast_idxs, self.get_reduce_acc(reduceop), insert_before=insert_before)  # noqa: E501

      # late reduce loop
      loop_ctx = self.render_loop(end_local_idxs, insert_before=insert_before)

      # load localbufs
      loaded_buffers[self.bufs[-1]] = self.global_load(-1, fake_global_idxs+local_idxs+fake_reduce_idxs+upcast_idxs, barrier=barrier, insert_before=insert_before)  # noqa: E501

      # there's no AST here (and there's no shape for the reduce LazyOp)
      self.ast_parse(LazyOp(reduceop.op, (LazyOp(BufferOps.LOAD, (), self.bufs[-1]),)), acc, self.acc_offsets(-1), loaded_buffers, do_reduce=True, loop_ctx=loop_ctx, if_ctx=if_cond, insert_before=insert_before)  # noqa: E501
  ...
  def linearize(self):
    ...
    self.render_reduceop = \
      lambda x,ldbuffer,insert_before: self._render_reduceop(x, ldbuffer, global_idxs, local_idxs, upcast_idxs, reduce_idxs, insert_before)
  ...
  def ast_parse(self, x:LazyOp, acc: List[UOp], offs:Optional[List[int]], loaded_buffers:Dict[Union[MemBuffer, ConstBuffer, LocalBuffer], List[UOp]], \
                do_reduce=False, loop_ctx=tuple(), if_ctx=tuple(), insert_before:Optional[UOp]=None, cache=None) -> List[UOp]: # noqa: E501
    if cache is None: cache = {}
    if x in cache: return cache[x]
    if x.op in BufferOps: return loaded_buffers[x.arg]
    if x.op is UnaryOps.CAST: return [self.uops.add(UOps.BITCAST if x.arg[1] else UOps.CAST, self.get_base_dtype(x.arg[0]), (u,), x.arg[0], insert_before=insert_before) \
                                      for u in self.ast_parse(x.src[0], acc, offs, loaded_buffers, insert_before=insert_before)]  # noqa: E501
    if x.op in ReduceOps and not do_reduce:
      # assert offs is None, "not available if we aren't doing reduce"
      # return acc
      if x not in self.reduce_acc:
        load_cache_copy,self.load_cache = self.load_cache,{}
        render_before=self.uops.uops[min([self.uops.uops.index(ctx) for ctx in loop_ctx+if_ctx])] if loop_ctx+if_ctx else None
        self.reduce_acc[x] = self.render_reduceop(x, loaded_buffers,render_before)
        self.load_cache = load_cache_copy
    ...
    if x.op in ops:
      ...
      ret = [self.uops.add(UOps.ALU, dtypes.bool if x.op in {BinaryOps.CMPLT, BinaryOps.CMPEQ} else val[-1].dtype, val, x.op, insert_before=insert_before) for val in zip(*values)] # noqa: E501
    cache[x] = ret
    return ret
