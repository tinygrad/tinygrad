...
from tinygrad.helpers import colored, ImageDType, DEBUG, dtypes, DType, partition, prod, getenv
...
class Linearizer(OptimizedKernel):
    ...
    for _idx in _idxs:
      ...
      if key not in self.load_cache:
        ...
        if acc is not None:
          ...
          var_type = dtypes.get_normal_type(accumtype)
        else:
          self.load_cache[key] = self.uop(UOps.LOAD, localtype, [], MemOp(self.get_buffer_name(i), idx, self.bufs[i].__class__ is LocalBuffer, self.bufs[i].dtype, valid, invalid_value)) if this_const is None else \
                                 self.uop(UOps.LOAD, accumtype, [], ConstOp(this_const, valid))
          var_type = dtypes.get_normal_type(localtype if this_const is None else accumtype)
      ret.append(self.uop(UOps.GEP, var_type, [self.load_cache[key]], expanded_nodes[dim].index(_idx[dim])) if localtype.is_vector_type else self.load_cache[key])
    return ret
  ...
  def global_store(self, i:int, idxs:List[VariableOrNum], store:List[UOp]) -> None:
    ...
    if len(upcast_dim) == 1 and len(expanded_nodes[upcast_dim[0]]) in self.opts.supported_vector_sizes.get(self.bufs[i].dtype, self.opts.supported_vector_sizes[dtypes.float]):
      ...
      for k,out_tokens in grouped_store_offset.items():
        ...
        store_offset_new[k] = self.uop(UOps.CAST, dt, out_tokens)
  ...
  def ast_parse(self, x, acc, loaded_buffers, do_reduce=False) -> List[UOp]:
    ...
    for idx, val in zip([[i] for i in range(len(values[0]))], zip(*values)):
      if use_accum:
        ret.append((idx, self.uop(UOps.STORE, cast_dtype, [val[-1], self.uop(UOps.ALU, cast_dtype, list(val), ops[x.op])])))
      else:
        ret.append((idx, self.uop(UOps.ALU, cast_dtype, list(val), x.op, cachable=True)))
