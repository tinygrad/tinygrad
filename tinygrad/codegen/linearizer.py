...
from tinygrad.helpers import colored, DEBUG, prod, getenv, to_function_name, flatten
...
class Linearizer(Kernel):
  ...
  def global_load(self, i:int, idxs:List[Node], reduceop=None, barrier:Optional[UOp]=None, insert_before:Optional[UOp]=None) -> List[UOp]:
    buf = self.bufs[i]
    localtype = self.get_base_dtype(buf.dtype if reduceop is None else reduceop.dtype)
    const = buf.val if isinstance(buf, ConstBuffer) else self.get_reduce_acc(reduceop) if reduceop is not None else None
    ...
    if reduceop is not None: g_valid = NumNode(1)  # store masks applied in global_store
    ...
    for idx, valid, rep_idx in zip(e_idxs, e_valids, iter_idxs(expand_vars)):
      ...
      key = f"{reduceop is not None}{localtype}{'CONST'+str(this_const) if this_const is not None and reduceop is None else (buf.idx if isinstance(buf, MemBuffer) else cast(LocalBuffer, buf).name)}{idx.render()}{valid.render()}"  # noqa: E501
  ...
  def render_reduceop(self, reduceops: List[LazyOp], loaded_buffers:Dict[Union[MemBuffer, ConstBuffer, LocalBuffer], List[UOp]], \
    ...
    if (tc:=self.tensor_core):
      assert len(reduceops) == 1, "TC not supported for more than one reduceop"
    ...
    accs = {reduceop: self.global_load(self.bufs.index(self.out_for_reduceop[reduceop].arg), global_idxs+local_idxs+fake_reduce_idxs+upcast_idxs, reduceop, insert_before=insert_before) for reduceop in reduceops}
    ...
    if (tc:=self.tensor_core):
      ...
      for iter in [x[::-1] for x in itertools.product(*[x for x in [range(sz) for _,sz in upcasts[0]][::-1]])]:
        ...
        ops = (
          ...
          self.uops.add(UOps.CAST, (dt3:=tc.dtype_out.vec(wmma_sz[2])), tuple(op3:=accs[reduceops[0]][offs[2]:offs[2]+wmma_sz[2]]), None, True, insert_before))
        ...
        for z in range(wmma_sz[2]): # TODO: don't need to DEFINE_ACC, pass to WMMA in op3, or PHI accs that are not valid
          accs[reduceops[0]][offs[2]+z] = self.uops.add(UOps.PHI, tc.dtype_out, (op3[z], \
    ...
    if self.group_for_reduces:
      ...
      # store accumulators
      stores = [self.global_store(-(ri+1), fake_global_idxs+local_idxs+fake_reduce_idxs+upcast_idxs, acc, insert_before=insert_before) for ri, acc in enumerate(accs)]
      barrier = self.uops.add(UOps.BARRIER, None, tuple(flatten(stores)), cachable=False, insert_before=insert_before)
      if self.opts.has_local:
        # all self.sts[-(ri+1)] should be the same
      ...
      accs = {reduceop: self.global_load(self.bufs.index(self.out_for_reduceop[reduceop].arg), fake_global_idxs+local_idxs+fake_reduce_idxs+upcast_idxs, reduceop, insert_before=insert_before) for reduceop in reduceops}

      # late reduce loop
      loop_ctx = self.render_loop(end_local_idxs, insert_before=insert_before)

      for ri, (reduceop, acc) in enumerate(zip(reduceops, accs)):
        # load localbufs
        loaded_buffers[self.bufs[-(ri+1)]] = self.global_load(-(ri+1), fake_global_idxs+local_idxs+fake_reduce_idxs+upcast_idxs, None, barrier, insert_before)

        # there's no AST here (and there's no shape for the reduce LazyOp)
        self.ast_parse(LazyOp(reduceop.op, (LazyOp(BufferOps.LOAD, (), self.bufs[-(ri+1)]),)), \
                       acc, self.acc_offsets(-(ri+1)), loaded_buffers, do_reduce=True, loop_ctx=loop_ctx, insert_before=insert_before)
  ...
  def linearize(self):
    ...
    if self.group_for_reduces:
      for ri, reduceop in enumerate(self.reduceops[::-1]):
        ...
        temp_dtype = self.get_base_dtype(reduceop.dtype)
        self.bufs.append(LocalBuffer(f"temp{ri}", self.sts[-1].size, temp_dtype))
        self.buf_uops.append(self.uops.add(UOps.DEFINE_LOCAL, PtrDType(temp_dtype), (), (f"temp{ri}", self.sts[-1].size)))

    # kernel name (before late upcast)
    self.name = ("r" if self.reduceops else ("C" if all(x.op in BufferOps for x in self.lazyops) else "E")) + \
    ...
    for reduceops in [self.reduceops] if self.reduceops is not None else []:
      accs,loaded_buffers,fake_reduce_idxs,local_idxs,upcast_idxs = self.render_reduceop(reduceops,loaded_buffers,global_idxs,local_idxs,upcast_idxs)
  ...
  def ast_parse(self, x:LazyOp, accs: Dict[LazyOp, List[UOp]], offs:Optional[List[int]], loaded_buffers:Dict[Union[MemBuffer, ConstBuffer, LocalBuffer], List[UOp]],\
