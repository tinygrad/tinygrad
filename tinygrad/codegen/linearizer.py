...
class Linearizer(Kernel):
  ...
  def global_load(self, i:int, idxs:List[Node], acc=None, barrier:Optional[UOp]=None) -> List[UOp]:
    ...
    for idx, valid, f4_idx in zip(e_idxs, e_valids, expand_node(f4idx, expand_vars)):
      ...
      ret.append(self.uop(UOps.GEP, localtype.scalar(), (self.load_cache[key],), f4_idx.b) if dim is not None else self.load_cache[key])
    return ret
