...
class Linearizer:
  ...
  def linearize(self):
    ...
    # load latebufs
    loaded_buffers.update({b:self.global_load(i, global_idxs[:removed]+fake_reduce_idxs) for i,b in enumerate(self.bufs) if b not in self.earlybufs and i != 0 and not b.__class__ == LocalBuffer})
  ...
  def ast_parse(self, x, acc, loaded_buffers, ssa, do_reduce=False) -> List[Token]:
    if not x.__class__ == LazyOp: return loaded_buffers[x]
    ...
    if x.op == ReduceOps.SUM and x.src[0].__class__ == LazyOp and x.src[0].op == BinaryOps.MUL:
      x = LazyOp(FusedOps.MULACC, x.src[0].src, x.arg)
    if x.op == ReduceOps.SUM and x.src[0].__class__ == LazyOp and x.src[0].op == UnaryOps.CAST and x.src[0].src[0].__class__ == LazyOp and x.src[0].src[0].op == BinaryOps.MUL:
    ...
    assert all(x.__class__ == Token for x in ordered_ret), "some tokens didn't get scattered?"
  ...
  def required_optimizations(self, early_only=False):
    for buf_index,buf in enumerate(self.bufs):
      ...
      if (not early_only or buf in self.earlybufs) and self.bufs[buf_index].dtype.__class__ == ImageDType:
