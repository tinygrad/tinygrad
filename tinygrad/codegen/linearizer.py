...
class Linearizer(OptimizedKernel):
  ...
  def uop(self, uop:UOps, dtype:Optional[DType], vin:Tuple[UOp, ...], arg:Any=None, cachable=False) -> UOp:
    ...
    if uop == UOps.ALU:
      ...
      # equal folding
      if arg == BinaryOps.SUB and vin[0] == vin[1]: return self.const(0. if dtypes.is_float(dtype) else 0, dtype)
      if arg == BinaryOps.CMPLT and vin[0] == vin[1]: return self.const(0. if dtypes.is_float(dtype) else 0, dtype)
