  ...
  def linearize(self):
    ...
    if self.reduceop is not None:
      ...
      if self.use_tensor_cores:
        ...
        # load earlybufs
        loaded_buffers.update({b:self.global_load(self.bufs.index(self.local_alias[i+1]) if i+1 in self.local_alias else i+1, global_idxs+local_idxs+reduce_idxs+full_upcast_idxs) for i,b in enumerate(self.bufs[1:]) if b in self.earlybufs})
