      ...
      for u in self.uops:
        if u.uop is UOps.PHI and len(u.vin) == 3:
          ...
          if all(x.uop is not UOps.LOOP for x in get_recursive_parents(UOp(u.uop, u.dtype, u.vin[0:2], u.arg))) and u.vin[1].arg is BinaryOps.ADD:
            ...
            changed_something = True
          try:
            # Match the subgraph generated by _cumsum and replace summation in the inner loop with multiplication.
            accum0, alu0, loop0 = u.vin
            loop0_start, loop0_stop = loop0.vin
            alu1, accum1 = alu0.vin
            cond1, v_true, v_false = alu1.vin
            alu2, v_limit = cond1.vin
            alu3, alu4 = alu2.vin
            node, m1 = alu3.vin
            loop1, m2 = alu4.vin
            ...
            if (accum0 == accum1 and alu0.uop == UOps.ALU and alu0.arg is BinaryOps.ADD and loop0 == loop1 and loop0_start.uop == UOps.CONST
                and loop0_start.arg == 0 and loop0_stop.uop == UOps.CONST and loop0_stop.arg + v_limit.arg == 2 and alu1.uop == UOps.ALU
                and alu1.arg == TernaryOps.WHERE and cond1.uop == UOps.ALU and cond1.arg == BinaryOps.CMPLT and v_true.uop == UOps.CONST
                and v_false.uop == UOps.CONST and v_false.arg == 0 and alu2.uop == UOps.ALU and alu2.arg == BinaryOps.ADD
                and v_limit.uop == UOps.CONST and alu3.uop == UOps.ALU and alu3.arg is BinaryOps.MUL and alu4.uop == UOps.ALU
                and node.dtype is not None and dtypes.is_int(node.dtype) and alu4.arg is BinaryOps.MUL and m1.uop == UOps.CONST and m1.arg == -1
                and m2.uop == UOps.CONST and m2.arg == -1):
                u_idx = self.uops.index(u)
                new = self.uop(UOps.ALU, node.dtype, (node, m1,), BinaryOps.SUB, insert_before=u_idx)
                new = self.uop(UOps.CAST, u.dtype, (new,), insert_before=u_idx + 1)
                new = self.uop(UOps.ALU, u.dtype, (new, v_true,), BinaryOps.MUL, insert_before=u_idx + 2)
                # replace u by node
                for v in self.uops: v.vin = tuple(new if x is u else x for x in v.vin)
                self.uops.remove(u)
                changed_something = True
