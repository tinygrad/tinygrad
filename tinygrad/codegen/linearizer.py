...
class Linearizer(Kernel):
  ...
  def ast_parse(self, x:LazyOp, acc: List[UOp], offs:Optional[List[int]], loaded_buffers:Dict[Union[MemBuffer, ConstBuffer, LocalBuffer], List[UOp]], do_reduce=False, loop_ctx=tuple(), ast_seen=None) -> List[UOp]:  # noqa: E501
    if ast_seen is None: ast_seen = {}
    if x in ast_seen: return ast_seen[x]
    ...
    values = [self.ast_parse(v, acc, offs, loaded_buffers, loop_ctx=loop_ctx, ast_seen=ast_seen) for v in x.src]
