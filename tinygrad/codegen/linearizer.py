...
class Linearizer(Kernel):
  ...
  def global_load(self, i:int, idxs:List[Node], barrier:Optional[UOp]=None) -> List[UOp]:
    buf = self.bufs[i]
    ...
    for idx, valid, rep_idx in zip(e_idxs, e_valids, iter_idxs(expand_vars)):
      ...
      key = f"{localtype}{'CONST'+str(this_const) if this_const is not None else (buf.idx if isinstance(buf, MemBuffer) else cast(LocalBuffer, buf).name)}{idx.render()}{valid.render()}"  # noqa: E501
      if key not in self.load_cache:
        if this_const is not None:
