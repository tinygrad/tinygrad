...
class Linearizer(Kernel):
  ...
  def global_load(self, i:int, idxs:List[Node], acc:Optional[LazyOp]=None, barrier:Optional[UOp]=None, loop_ctx:Tuple[UOp, ...]=()) -> List[UOp]:
    ...
    dim, amt = None, 1
    # float 4 grouping
    if len(upcast_dim := self.get_float4_upcast_dim(i)) == 1 and len(float4_expand := expand_node(idxs[upcast_dim[0]])) in ([32,16] if self.amx else [])+[8,4,2]:
    ...
    for idx, valid, rep_idx in zip(e_idxs, e_valids, iter_idxs(expand_vars)):
      ...
      if key not in self.load_cache:
        if acc is not None:
          self.load_cache[key] = UOp(UOps.DEFINE_ACC, localtype, (UOp.const(localtype.scalar(), self.get_reduce_acc(acc)), *loop_ctx), (i, acc_count, 'amx_acc' if self.amx else None)) # noqa: E501
          acc_count += 1
  ...
  def global_store(self, i:int, idxs:List[Node], store:List[UOp]) -> List[UOp]:
    ...
    # float4 grouping
    if len(upcast_dim := self.get_float4_upcast_dim(i)) == 1 and len(float4_expand := expand_node(idxs[upcast_dim[0]])) in [2,4,8]+([16,32] if self.amx else []):
