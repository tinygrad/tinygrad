...
class Kernel:
  ...
  def acc_offsets(self, i:int) -> List[int]:
    ...
    upcasted_i = self.upcasted_axis(i)[::-1]
    acc_strides = [x*(1-upcasted_i[i][2]) for i,x in enumerate(strides_for_shape(tuple(1 if r else s for s,_,r in upcasted_i)))]
    return [sum(t) for t in itertools.product(*[[y*acc_strides[i] for y in range(x[0])] for i,x in enumerate(upcasted_i)])]
