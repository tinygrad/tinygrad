  ...
  def get_optimized_ast(self) -> UOp:
    ...
    def fixup_ast(op:UOp, apply_to_st=None) -> UOp:
      ...
      if op.op in BUFFER_UOPS:
        ...
        if op.op is UOps.STORE: return replace(op, src=(op.src[0], st_uop, fixup_ast(op.src[1], apply_to_st)))
          ...
          if self.use_tensor_cores >= 2:
            if self.use_tensor_cores == 3:
              ...
              for i,(src,fix_st_fxn) in enumerate(zip(rsrc.src, [fix_st1, fix_st2])):
                ...
                st_uop = ShapeTracker.from_shape(local_shape).expand(ex_shape).st_uop()
...
def verify_ast(ast:UOp) -> Dict[UOp, ShapeTracker]:
  ...
  def assert_valid(op:UOp, st:ShapeTracker):
    ...
    if op.op is UOps.REDUCE_AXIS: st = ShapeTracker.from_shape(sts[op.src[0]].reduce(op.arg[1][-1] if op.arg[0] is ReduceOps.WMMA else op.arg[1]))
      ...
      st = op.arg if op.op is UOps.SHAPETRACKER else sts[op.src[0 if op.op is UOps.CONST else 1]]
