    ...
    def fixup_ast(op:UOp, apply_to_st=None) -> UOp:
      ...
      if op.op is UOps.REDUCE_AXIS:
        ...
        if self.group_for_reduces:
          ...
          idx, valid = ShapeTracker.from_shape(tuple([1 if i in second_axis else a for i,a in enumerate(local_shape)])).to_uops()
          return UOp(UOps.LOAD, op.dtype, (local_buffer, UOp.store(local_buffer, idx, grouped_reduce, valid), idx, valid))
        arg = (reduceop, axis)
