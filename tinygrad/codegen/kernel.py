...
class Kernel:
  ...
  def to_program(self, name_override:Optional[str]=None) -> Program:
    self.linearize()
    src = self.opts.render(name:=(name_override if name_override is not None else to_function_name(self.name)), self.uops)
    ...
    return Program(name_override if name_override is not None else self.name,
                   src, self.opts.device, self.global_size, self.local_size, self.uops, ops * run_count, min(mem * run_count, mem_bytes))
