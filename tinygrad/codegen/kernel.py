...
class Kernel:
  def __init__(self, ast:UOp, opts:Optional[Renderer]=None):
    ...
    self.range_split_axis: tuple[int, tuple[tuple[int, int], ...]] = ()
  ...
  def hand_coded_optimizations(self) -> Kernel:
    ...
    if not self.opts.has_local and self.first_reduce == self.first_upcast:
      ...
      for axis in range(len(self.full_unupcasted_shape)):
        smasks = tuple(sorted({st.views[-1].mask[axis] for st in self.sts if st.views[-1].mask}, key=lambda x: x[0]))  # noqa: E501
    ...
    def fixup_ast(op:UOp) -> UOp:
      ...
      if op.op is Ops.SINK:
        return ret.replace(arg = KernelInfo(to_function_name(self.name) if name_override is None else name_override,
                                            self.local_dims, self.upcasted, self.dont_use_locals, self.range_split_axis))
