...
class Opt:
    ...
    def real_axis(self, k: Kernel):
        if self.axis is None:
            return -1
        if self.op is OptOps.UNROLL:
            return k.first_reduce + self.axis
        if self.op in {OptOps.GROUP, OptOps.GROUPTOP}:
            return k.first_reduce + k.group_for_reduces + self.axis
        return self.axis
...
class TensorCoreOptions:
    axes: tuple[
        int, ...
    ]  # the location of the original N and M axes if still in the shape
    axes_exist: tuple[
        bool, ...
    ]  # true if the original N and M axes are still in the shape
    axis_pads: tuple[tuple[int, int], ...]

    def fix_axes(
        self, removed_axis: int
    ):  # adjust the TC axes if necessary when a dimension is removed
        axes, axes_exist = list(self.axes), list(self.axes_exist)
        for tc_dim in [i for i in range(2) if axes_exist[i]]:
            if removed_axis < axes[tc_dim]:
                axes[tc_dim] -= 1
            elif removed_axis == axes[tc_dim]:
                axes_exist[tc_dim] = False
...
class Kernel:
    def __init__(self, ast: UOp, opts: Optional[Renderer] = None):
        ...
        earlybufs: list[UOp] = [
            ...
            for x in reduceop.src[0].toposort
            if x.op in GroupOp.Buffer
        ]
    ...
    def copy(self):
        ...
        ret.sts = self.sts[
            : len(ret.bufs) + len(ret.reduceops) * 2
        ]  # NOTE: must redo the local buffers with TC in beam

        # parameters for optimizations
        (
            ret.applied_opts,
            ret.group_for_reduces,
            ret.upcasted,
            ret.local_dims,
            ret.dont_use_locals,
        ) = (
        ...
        ret.tensor_core, ret.tensor_core_opts, ret.use_tensor_cores = (
            self.tensor_core,
    ...
    @property
    def membufs(self) -> list[UOp]:
        return dedup([x.src[0] for x in self.bufs if x.op in {Ops.LOAD, Ops.STORE}])
    ...
    def float4_axis(self, i: int):
        return [
            ...
            if x >= self.first_upcast and self.sts[i].shape[x] % 4 == 0
        ]  # noqa: E501

    def upcasted_axis(self, i: int) -> list[tuple[int, Optional[sint], bool]]:
        upcasted_shape, upcasted_stride = (
            self.sts[i].shape[self.first_upcast :],
            self.sts[i].real_strides()[self.first_upcast :],
        )
        assert all_int(
            upcasted_shape
        ), f"cannot upcast a symbolic amount {upcasted_shape=}"
        return list(
            zip(
                upcasted_shape,
                upcasted_stride,
                [
                    x != y
                    for x, y in zip(
                        self.sts[0].shape[self.first_upcast :],
    ...
    def first_reduce(self) -> int:
        return [
            resolve(x != y)
            for x, y in zip(
                self.sts[0].shape[: self.first_upcast] + (0,),
                self.full_shape[: self.first_upcast] + (1,),
            )
        ].index(True)

    @property
    def first_upcast(self) -> int:
        return self.shape_len - self.upcasted

    @property
    def reduceop(self) -> UOp | None:
        return self.reduceops[0] if len(self.reduceops) > 0 else None
    ...
    def full_unupcasted_shape(self) -> tuple[sint, ...]:
        return self.full_shape[: self.first_upcast]
    ...
    def colors(self) -> list[str]:
        ...
        colors += ["red"] * (
            self.first_upcast - (self.first_reduce + self.group_for_reduces)
        )
    ...
    def colored_shape(self, pad: Optional[int] = None, dense=False) -> str:
        shape_strs = [
            (s if dense else f"{s:4d}") if isinstance(s, int) else s.render()
    ...
    def reshape_and_permute(
        self,
        new_shape_fxn: Optional[Callable[[tuple[sint, ...]], Sequence[sint]]],
        axis: Optional[Sequence[int]],
    ):
        def reshape(st: ShapeTracker):
            return (
        ...
        def permute(st: ShapeTracker):
            return st.permute(tuple(axis)) if axis is not None else st
    ...
    def simplify_ones(self) -> bool:
        ...
        self.upcasted -= sum(
            all_ones[self.first_upcast :]
        )  # TODO: no necessary since upcasted axis can't be un-upcasted
        self.reshape_and_permute(
        ...
        for i in range(1, len(shapes[0])):
            ...
            for s, st, ret in zip(shapes, strides, rets):
                ...
                can_merge.append(
                    (sti is not None)
                    and (
                        (sti != 0 and last_st == si * sti)
                        or (sti == 0 and last_st == 0)
                    )
                )
    ...
    def _create_tc_opts(
        self, reduceop: UOp, tc: TensorCore, axis: int, opt_level: int
    ) -> Optional[TensorCoreOptions]:
        has_cast = tc.dtype_in != tc.dtype_out
        if has_cast and not (
            reduceop.src[0].op is Ops.CAST and reduceop.src[0].dtype == tc.dtype_out
        ):
            return None
        ...
        def buf_index(src: UOp) -> Optional[int]:
            ...
            if src.op is Ops.LOAD and src.dtype == tc.dtype_in:
                return self.bufs.index(src)
            try:
                if opt_level >= 1 and src.op is Ops.CAST and src.dtype == tc.dtype_in:
                    return self.bufs.index(src.src[0])
            except ValueError:
                return None
            return None

        if (buf0 := buf_index(mul_op.src[0])) is None or (
        ...
        if not (
            axis_buf0
            and axis_buf1
            and ((self.shape_len - self.first_reduce) == 1 or (opt_level >= 1))
        ):
            return None

        axis_choices = list(
            itertools.product(
                axis_buf0, axis_buf1, range(self.first_reduce, self.shape_len)
            )
        )
        ...
        s0, s1, s2 = (
            axis_choices[-(axis + 1)][0][0],
            axis_choices[-(axis + 1)][1][0],
            axis_choices[-(axis + 1)][2],
        )  # s0 is n, s1 is m, s2 is k
        axis_pads = tuple(
            (x, tc.dims[i])
            for i, x in enumerate([s0, s1, s2])
            if resolve(self.full_shape[x] % tc.dims[i] != 0)
        )
        ...
        return TensorCoreOptions(
            axes=(s0, s1, s2), axes_exist=(True, True), axis_pads=axis_pads
        )
    ...
    def _apply_tc_opt(
        ...
        if (
            use_tensor_cores
            and self.reduceop is not None
            and self.reduceop.arg[0] is Ops.ADD
        ):
            tensor_cores = (
                self.opts.tensor_cores
                if tc_select == -1
                else [self.opts.tensor_cores[tc_select]]
            )
            for tc in tensor_cores:
                tensor_core_opts = [
                    self._create_tc_opts(reduceop, tc, axis, opt_level)
                    for reduceop in self.reduceops
                ]
                ...
                try:
                    for axis, dim in tc_opts.axis_pads:
                        self.apply_opt(
                            Opt(OptOps.PADTO, axis, dim), append_opt=False
                        )  # PADTO might fail
                ...
                for dim, amt in tc.get_reduce_axes():
                    self.apply_opt(
                        Opt(OptOps.UNROLL, tc_opts.axes[2] - self.first_reduce, amt),
                        append_opt=False,
                    )
                for opt in tc.opts:
                    self.apply_opt(
                        Opt(
                            {"u": OptOps.UPCAST, "l": OptOps.LOCAL}[opt[0]],
                            tc_opts.axes[int(opt[1])],
                            2,
                        ),
                        append_opt=False,
                    )
                self.tensor_core = tc
                self.use_tensor_cores = (
                    use_tensor_cores  # TC=2 will do the shape ops without the WMMA
                )
                return True
        return False
    ...
    def apply_tensor_cores(
        self,
        use_tensor_cores=1,
        extra_opts: Optional[list[Opt]] = None,
        axis: int = 0,
        tc_select: Optional[int] = None,
        tc_opt: Optional[int] = None,
    ) -> bool:
        ...
        try:  # check TC first and apply hand-coded opts if successful
            ...
            if (tc_opts := self.tensor_core_opts) is not None:
                if extra_opts is not None:
                    ...
                    # hand-coded TC opts
                    for tc_dim in [
                        tc_dim for tc_dim in [1, 0] if tc_opts.axes_exist[tc_dim]
                    ]:  # attempt to upcast M and N
                        szs = [
                            sz
                            for sz in [5, 4, 3, 2]
                            if self.full_shape[tc_opts.axes[tc_dim]] % sz == 0
                        ]
                        if szs:
                            self.apply_opt(
                                Opt(OptOps.UPCAST, tc_opts.axes[tc_dim], szs[0])
                            )

                    if tc_opts.axes_exist[0] and (
                        szs := [
                            sz
                            for sz in [4, 2]
                            if self.full_shape[tc_opts.axes[0]] % sz == 0
                        ]
                    ):  # attempt to local N
    ...
    def apply_opt(self, opt: Opt, append_opt: bool = True):
        if self.dont_use_locals:
            check(
                opt.op not in {OptOps.LOCAL, OptOps.GROUP, OptOps.GROUPTOP},
                "not using locals",
            )
        ...
        if opt.op is OptOps.TC:
            ...
            )  # TODO: things like PADTO might be fine
            check(
                (use_tensor_cores := USE_TC.value) == 2
                or len(self.opts.tensor_cores) > 0,
                "must have tensor cores or TC=2",
            )
            check(opt.axis is not None, "tensor core opts must have an axis")
            check(
                opt.arg is not None
                and isinstance(opt.arg, tuple)
                and len(opt.arg) == 2,
                "tensor core opts must have tc_select and tc_opt",
            )
            check(
                -1
                <= (tc_select := cast(tuple, opt.arg)[0])
                < len(self.opts.tensor_cores),
        ...
        if opt.op is OptOps.SWAP:
            amt = cast(int, opt.arg)  # arg is an axis in the SWAPs
            ...
            check(
                ...
                f"shift/padto of {amt=}, 1 or symbolic amount is meaningless",
            )
            if opt.op is not OptOps.PADTO:
                check(
                    self.full_shape[axis] % amt == 0,
        ...
        if self.reduceop is not None and (
            ...
            local_sz = prod(
                self.full_shape[
                    self.first_reduce
                    - self.local_dims : self.first_reduce
                    + self.group_for_reduces
                ]
            )
            smem_sz = amt * acc_sz * upcast_sz * local_sz
            check(
                smem_sz <= self.opts.shared_max,
                f"exceeds maximum shared memory size: needs {smem_sz}, max {self.opts.shared_max}",
            )
        ...
        if opt.op is OptOps.LOCAL:  # cyan
            ...
            check(
                len(reduce_axes := [i for r in self.reduceops for i in r.axis_arg])
                == len(set(reduce_axes)),
            ...
            self.shift_to(
                axis,
                amt,
                top=(opt.op is OptOps.GROUPTOP),
                insert_before=self.first_reduce + self.group_for_reduces,
            )
            ...
            if self.full_shape[axis] == amt and axis == self.first_reduce:
                self.local_dims += (
                    1  # first_reduce will ++, so offset loss in simplify_ones
                )
            if (
                self.full_shape[axis] == amt
                and axis < self.first_reduce + self.group_for_reduces
            ):
                self.group_for_reduces -= 1  # fully unrolling a GROUP
            ...
            check(
                not (
                    self.tensor_core
                    and self.global_dims
                    <= axis
                    < self.global_dims + len(self.tensor_core.get_local_axes())
                ),
                "can't upcast TC locals",
            )
            check(
                (self.opts is not None and self.opts.device == "DSP") or amt <= 16,
            ...
            check(
                self.opts.has_local and not self.dont_use_locals,
                "NOLOCALS is meaningless if target does not support local or already not using locals",
            )
            check(
                self.local_dims == 0 and self.group_for_reduces == 0,
                "can't have no locals with locals",
            )
            self.dont_use_locals = True
        elif opt.op is OptOps.SWAP:
            check(
                axis < amt < self.global_dims,
                f"swap is only for globals with axis < amt, getting {amt=}, {axis=}, {self.global_dims=}",
            )
            ...
            for i, st in enumerate(self.sts):
                if (s := st.shape[axis]) == 1:
                    continue  # reduced
                check(
                    s > amt // 4,
                    f"pad adds more than quadruple the work {st.shape[axis]=} > {amt//4=}",
                )
        ...
        if self.simplify_ones() and self.tensor_core_opts:
            self.tensor_core_opts.fix_axes(
                axis
    ...
    def hand_coded_optimizations(self) -> Kernel:
        ...
        if (
            ...
            and (mulop := self.reduceop.src[0]).op is Ops.MUL
            and mulop.src[0].op is Ops.LOAD
            and mulop.src[1].op is Ops.LOAD
        ):
            st0, st1 = (
                self.sts[self.bufs.index(mulop.src[0])],
                self.sts[self.bufs.index(mulop.src[1])],
            )
            strides0, strides1 = st0.real_strides(), st1.real_strides()

            def has_expanded_axis(shape, strides):
                return any(
                    resolve(s > 1) and not resolve(st != 0)
                    for s, st in zip(shape, strides)
                )

            if strides0[self.first_reduce] == 1 and not (
                has_expanded_axis(st0.shape, strides0)
                and has_expanded_axis(st1.shape, strides1)
            ):
                for global_idx in range(self.global_dims):
                    if (
                        ...
                        if DEBUG >= 3:
                            print(
                                f"MATVEC: {self.full_shape=} {self.first_reduce=} {strides0=} {MV_BLOCKSIZE=} {MV_THREADS_PER_ROW=} {MV_ROWS_PER_THREAD=}"
                            )
        ...
        if (
            self.opts.has_local
            and self.opts.has_shared
            and all_int(self.sts[0].shape[: self.first_reduce])
        ):
        ...
        for buf_index, buf in enumerate(self.bufs):
            ...
            if buf.src[0].dtype.__class__ is ImageDType:
                ...
                if len(unit_stride_axes_mul_4) and all(
                    x < self.first_upcast for x in unit_stride_axes_mul_4
                ):
        ...
        for axis in range(self.first_reduce):
            ...
            if (
                ...
                and prod(self.full_shape[self.first_upcast :])
        ...
        if (
            self.first_reduce < self.first_upcast
            and (
                prod(self.full_shape[self.first_upcast :]) <= 4
            ...
            if (
                isinstance(s := self.full_unupcasted_shape[-1], int) and s <= 32
                ...
                if (
                    self.first_reduce < self.first_upcast
                    and s <= 3
                    and isinstance(s2 := self.full_unupcasted_shape[-1], int)
                    and s2 <= 3
                ):
                    self.apply_opt(
                        Opt(
        ...
        if self.opts.has_local:
            if (
                ...
                and not self.group_for_reduces
            ):
    ...
    def name(self) -> str:
        ...
        kernel_type = (
            ...
            if self.reduceop is not None
                ...
                "C"
                if all(
                    x.op is Ops.SINK or x.op in GroupOp.Buffer
                    for x in self.ast.toposort
                )
                else "E"
            )
        )
        suffix = colored("_", "BLACK").join(
            [
                colored(x.render() if isinstance(x, UOp) else str(x), c)
                for x, c in zip(self.full_shape, self.colors())
            ]
        )
        name = (
            kernel_type
            + (f"{len(self.ast.src)}" if len(self.ast.src) > 1 else "")
            + "_"
            + suffix
        )
        ...
        num = (
            f"n{Kernel.kernel_cnt[function_name]-1}"
            if Kernel.kernel_cnt[function_name] > 1
            else ""
        )
    ...
    def get_optimized_ast(self) -> UOp:
        ...
        def fixup_ast(op: UOp) -> UOp:
            ...
            if op.op in GroupOp.Buffer and op in self.bufs:
                ...
                if op.op is Ops.CONST and any(
                    v.mask is not None for v in unwrap(st_uop.st).views
                ):
                    return op.valid(unwrap(st_uop.st))
                # otherwise we just replace the VIEW source
                return (
                    ret.replace(src=(st_uop,))
                    if len(op.src) == 1
                    else ret.replace(src=(ret.src[0], st_uop, *ret.src[2:]))
                )
            ...
            if op.op is Ops.REDUCE_AXIS:
                ...
                def reduced_axes(start, stop):
                    return tuple(
                        ...
                        if resolve(
                            self.sts[reduce_idx].shape[i]
                            != self.sts[reduce_idx + 1].shape[i]
                        )
                    )

                axes = reduced_axes(
                    self.first_reduce + self.group_for_reduces, self.shape_len
                )
                grouped_axes = reduced_axes(
                    self.first_reduce, self.first_reduce + self.group_for_reduces
                )

                if (tc := self.tensor_core) and (
                    self.use_tensor_cores == 1 or self.use_tensor_cores == 3
                ):
                    wd, tcd = self.global_dims, self.first_upcast

                    def get_upcast_axes(
                        buf,
                    ):  # upcast along non-zero dimensions of (tc_reduce + tc_upcast)
                        upcast_axes = int(math.log2(tc.elements_per_thread[buf]))
                        return tuple(
                            (
                                tcd
                                + len(tc.get_reduce_axes())
                                + len(tc.get_upcast_axes())
                                - (i + 1),
                                2,
                            )
                            for i in range(upcast_axes)
                        )
                    ...
                    def get_tc_swizzle_st(shape, local_perm, upcast_perm):
                        ...
                        permaxis = (
                            list(range(wd))
                            + [
                                wd + x + (offset if x >= len(local_perm) else 0)
                                for x in local_perm
                            ]
                            + list(range(wd + len(local_perm), tcd))
                            + [
                                wd + x + (offset if x >= len(local_perm) else 0)
                                for x in upcast_perm
                            ]
                            + list(range(tcd + len(upcast_perm), len(shape)))
                        )
                    ...
                    srcs = list(
                        (
                            ret.src[0]
                            if ret.src[0].op is not Ops.CAST
                            else ret.src[0].src[0]
                        ).src
                    )
                    for i, (src, swizzle) in enumerate(zip(srcs, tc.swizzle)):
                        if swizzle:
                            srcs[i] = src.view(
                                get_tc_swizzle_st(
                                    (
                                        src if src.op is Ops.LOAD else src.src[0]
                                    ).st_arg.shape,
                                    *swizzle,
                                )
                            )
                        ...
                        if (
                            self.use_tensor_cores == 3
                        ):  # for TC=3, emulate the warp addressing with locals
                            local_shape = tuple(
                                (
                                    1
                                    if i >= self.first_reduce and i < self.first_upcast
                                    else s
                                )
                            ...
                            local_buffer = UOp(
                                Ops.DEFINE_LOCAL,
                                tc.dtype_in.ptr(size=st.real_size(), local=True),
                                (),
                                f"temp{i}",
                            )
                            if swizzle:
                                store_st = get_tc_swizzle_st(store_st.shape, *swizzle)
                            local_store = UOp.store(
                                local_buffer, store_st.to_uop(), srcs[i]
                            )
                            srcs[i] = UOp(
                                Ops.LOAD,
                                tc.dtype_in,
                                (local_buffer, st.to_uop(), local_store),
                            )

                    tc_reduce_axes = tuple(tcd + ax for ax, _ in tc.get_reduce_axes())
                    if (
                        self.use_tensor_cores == 1
                    ):  # real WMMA, use CONTRACT/UNROLL to get the vectorization right
                        tc_upcast_axes = (
                            get_upcast_axes(0),
                            get_upcast_axes(1),
                            get_upcast_axes(2),
                        ...
                        wmma = UOp(
                            ...
                            src=(
                                ...
                                UOp.const(
                                    tc.dtype_out.vec(tc.elements_per_thread[2]), 0.0
                                ),
                            ),
                            arg=wmma_arg,
                        )
                        tc_uop = UOp(
                            Ops.UNROLL, tc.dtype_out, (wmma,), arg=tc_upcast_axes[2]
                        )
                        ...
                        tc_uop = UOp(
                            Ops.REDUCE_AXIS,
                            tc.dtype_out,
                            ((srcs[0] * srcs[1]).cast(tc.dtype_out),),
                            (Ops.ADD, tc_reduce_axes),
                        )

                    return (
                        ret.replace(src=(tc_uop,), arg=(Ops.ADD, new_axes))
                        if (
                            new_axes := tuple(
                                i for i in axes if i not in tc_reduce_axes
                            )
                        )
                        else tc_uop
                    )
                ...
                if self.group_for_reduces and grouped_axes:
                    local_shape = (
                        (1,) * self.global_dims
                        + self.full_shape[
                            self.global_dims : self.global_dims + self.local_dims
                        ]
                        + tuple(
                            [
                                (
                                    self.full_shape[i]
                                    if self.sts[reduce_idx].shape[i]
                                    != self.sts[reduce_idx + 1].shape[i]
                                    else 1
                                )
                                for i in range(
                                    self.first_reduce,
                                    self.first_reduce + self.group_for_reduces,
                                )
                            ]
                        )
                        + (1,)
                    ...
                    local_buffer = UOp(
                        Ops.DEFINE_LOCAL,
                        op.dtype.ptr(local_size, local=True),
                        (),
                        f"temp{self.reduceops.index(op)}",
                    )
                    local_load = UOp(
                        Ops.LOAD,
                        op.dtype,
                        (local_buffer, st_uop, UOp.store(local_buffer, st_uop, ret)),
                    )
                    grouped_reduce = UOp(
                        Ops.REDUCE_AXIS,
                        op.dtype,
                        (local_load,),
                        arg=(op.arg[0], grouped_axes),
                    )
                    if op is self.reduceops[-1]:
                        return grouped_reduce
                    st_uop = ShapeTracker.from_shape(
                        tuple(
                            [
                                1 if i in grouped_axes else a
                                for i, a in enumerate(local_shape)
                            ]
                        )
                    ).to_uop()
                    return UOp(
                        ...
                        (
                            local_buffer,
                            st_uop,
                            UOp.store(local_buffer, st_uop, grouped_reduce),
                        ),
                    )
    ...
    def linearize(self) -> Kernel:
        ...
        if DEBUG >= 3:
            ...
            if getenv("RAWAST"):
                print(self.ast)
            for i, (buf, st) in enumerate(
                [
                    (buf, st)
                    for buf, st in zip(self.bufs, self.sts)
                    if buf.op not in {Ops.CONST, Ops.VALID}
                ]
            ):
                print(
                    f"{i:2d}: {str(st.shape):25s} {str(buf.src[0].dtype).replace('dtypes.',''):20s}",
                    st.real_strides(),
                )
        ...
        self.uops: list[UOp] = linearize_uop(
            full_graph_rewrite(
                rewrite_shapetracker_with_index(modified_ast, self.opts), self.opts
            )
        )
        if DEBUG >= 5:
    ...
    def to_program(self, name_override: Optional[str] = None) -> ProgramSpec:
        self.linearize()
        src = self.opts.render(
            name := to_function_name(
                ansiname := (name_override if name_override is not None else self.name)
            ),
            self.uops,
        )
        ...
        if CAPTURE_PROCESS_REPLAY:
            diskcache_put(
                "kernel_process_replay",
                str(id(self)),
                (self.ast, self.opts, self.applied_opts, name, ContextVar._cache, src),
            )

        # group non-local bufs by the op type (LOAD or STORE) and the buffer arg. take the max access of that buffer in bytes
        # TODO: these max and min don't work on symbolic, and results are very wrong.
        mem_bytes = sum(
            max(x.src[0].dtype.itemsize * x.st_arg.real_size() for x in group)
            for _, group in itertools.groupby(
                [
                    x
                    for x in self.ast.toposort
                    if x.op in GroupOp.Buffer and x.src[0].op is Ops.DEFINE_GLOBAL
                ],
        ...
        return ProgramSpec(
            ...
            self.opts.device,
            self.uops,
            mem_estimate=mem_bytes,
            global_size=[1, 1, 1] if self.opts.has_local else None,
            local_size=[1, 1, 1] if self.opts.has_local else None,
        )
