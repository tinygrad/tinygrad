      ...
      if op.op is UOps.REDUCE_AXIS:
        ...
        if op in self.bufs_for_tensor_core and (tc := self.tensor_core):
          ...
          if self.use_tensor_cores >= 2:
            ...
            ret = UOp(UOps.WMMA_AXIS, tc.dtype_out, (fixup_ast(rsrc.src[0], fix_st1), fixup_ast(rsrc.src[1], fix_st2)), wmma_arg)
...
def _assert_valid_uop(uop:UOp, st:ShapeTracker, sts:Dict[UOp, ShapeTracker]) -> None:
  ...
  if op in {UOps.REDUCE_AXIS, UOps.WMMA_AXIS}: st = ShapeTracker.from_shape(sts[src[0]].reduce(arg[-1]))
  else:
