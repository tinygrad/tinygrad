...
class Kernel:
  def __init__(self, *ast:LazyOp, opts:Optional[CompilerOptions]=None):
    ...
    self.reduceops = flatten([reversed([x for x in out.lazyops if x.op in ReduceOps]) for out in self.ast])
    self.reduceops = [x for i,x in enumerate(self.reduceops) if self.reduceops.index(x)==i]
