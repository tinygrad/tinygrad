...
from tinygrad.helpers import all_same, colored, ansilen, dedup, getenv, prod, DEBUG, TC_OPT, USE_TC, round_up, all_int, \
                             get_contraction, to_function_name, diskcache_put, ContextVar, timeit, TIME_REPLAY
...
class Kernel:
  ...
  def to_program(self) -> Program:
    ...
    if getenv("RUN_PROCESS_REPLAY"):
      ...
      td = min(timeit(lin) for _ in range(5)) if TIME_REPLAY else None
      table_name = f"process_replay_{getenv('GITHUB_RUN_ID', 'HEAD')}"
      diskcache_put(table_name, id(self), (self.ast, self.opts, self.applied_opts, name, src, {k:v.value for k,v in ContextVar._cache.items()}, td))
