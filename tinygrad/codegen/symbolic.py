...
symbolic = symbolic_simple+commutative+PatternMatcher([
  ...
  *((UPat.var("x").alu(op, UPat.cvar("c1")).alu(op, UPat.cvar("c2")).named("f"),
  ...
  # x//d<c for positive int c0
  ((UPat.var("x", dtype=dtypes.ints)//UPat.cvar("d", vec=False))<UPat.cvar("c", vec=False),
   lambda x,d,c: x<(c.arg*d.arg) if d.arg > 0 and c.arg > 0 else None),
  ((UPat.var("x", dtype=dtypes.ints)//UPat.cvar("d", vec=False))<UPat.cvar("c", vec=False),
   lambda x,d,c: x<(c.arg*d.arg-(d.arg-1)) if d.arg > 0 and c.arg <= 0 else None),
