...
def get_rewrites_for_renderer(opts:Renderer) -> list[RewriteStep]:
  ...
  ret.append(RewriteStep(pm_delete_ignore+sym+expander, name="move_ignore"))
  ...
  ret.append(RewriteStep(
    symbolic_simple+get_late_rewrite_patterns(supported_ops, TRANSCENDENTAL>=2)+pm_render+extra_matcher, lambda _: opts, name="final rewrite"))
