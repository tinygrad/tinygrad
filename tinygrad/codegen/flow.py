...
from tinygrad.helpers import DEBUG, NOOPT, BEAM, DEVECTORIZE, TRANSCENDENTAL, CAPTURE_PROCESS_REPLAY
from tinygrad.helpers import getenv, to_function_name, diskcache_put, ContextVar
from tinygrad.renderer import ProgramSpec
from tinygrad.codegen.kernel import Kernel, Opt
from tinygrad.codegen.lowerer import pm_lowerer, get_index
from tinygrad.codegen.symbolic import sym, mulacc_unrolled, symbolic_simple
from tinygrad.codegen.expander import migrate_indexing, expander
from tinygrad.codegen.rewriter import devectorize, float4_folding, devectorize_load_store, load_store_indexing, get_late_rewrite_patterns, pm_render
from tinygrad.codegen.linearize import linearize_to_uop

logkerns, logkerns_level = open(getenv("LOGKERNS", ""), "a") if getenv("LOGKERNS", "") else None, getenv("LOGKERNS_LEVEL", 1)

def optimize_kernel_ast(ast:UOp, renderer:Renderer) -> tuple[str, UOp, list[Opt]]:
  ...
  return k.name, k.get_optimized_ast(), k.applied_opts
...
def ast_to_program(ast:UOp, renderer:Renderer) -> ProgramSpec:
  ...
  # generate the modified_ast with the OptOps applied
  name, sink, applied_opts = optimize_kernel_ast(ast, renderer)
  ...
  if CAPTURE_PROCESS_REPLAY:
    diskcache_put("kernel_process_replay", str(id(prg)), (ast, renderer, applied_opts, name, ContextVar._cache, prg.arg))
