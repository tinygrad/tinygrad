...
def full_rewrite_to_sink(sink:UOp, device:str="", ren:Renderer|None=None, optimize:bool=True) -> UOp:
  ...
  if optimize:
    ...
    sink = graph_rewrite(sink, pm_split_store, ctx=device, name="cut store ranges")

    # create image buffers
    sink = make_images(sink, device)

    # do postrange optimization, BEAM or hand_coded_optimizations
    sink = apply_opts(sink, device, ren)
  ...
  sink = graph_rewrite(sink, pm_devectorize, ctx=(device, ren.supports_float4), name="devectorize")

  # lower the index dtype to a concrete int
  sink = graph_rewrite(sink, pm_lower_index_dtype+load_store_indexing, ctx=device, name="lower all index dtypes")
  ...
  sink = graph_rewrite(sink, pm_decomp, ctx=device, name="decompositions")

  # final rules for the renderer (without sym)
  extra_matcher = ren.extra_matcher if ren.extra_matcher is not None else PatternMatcher([])
  pm_final_rewrite = pm_decomp+pm_render+extra_matcher+pm_split_ends
  sink = graph_rewrite(sink, pm_final_rewrite, ctx=device, name="final rewrite")
