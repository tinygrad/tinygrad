...
def _get_rewrites_for_renderer(opts:Renderer, linearizer:bool, _QUANTIZE, _DEVECTORIZE, _TRANSCENDENTAL) -> list[RewriteStep]:
  ...
  pre_matcher = opts.pre_matcher if opts.pre_matcher is not None else PatternMatcher([])
  extra_matcher = opts.extra_matcher if opts.extra_matcher is not None else PatternMatcher([])

  # late rewrite patterns (without sym)
  ret.append(RewriteStep(symbolic_simple+get_late_rewrite_patterns(supported_ops, _TRANSCENDENTAL>=2)+pre_matcher, lambda _: opts, name="late rewrite"))
