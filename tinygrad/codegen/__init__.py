...
def apply_rewrites(sink: UOp, rewrites: list[RewriteStep]):
  return functools.reduce(lambda x, f: f(x), rewrites, sink)
...
def _get_rewrites_for_renderer(opts: Renderer, linearizer: bool, _QUANTIZE, _DEVECTORIZE, _TRANSCENDENTAL, _RANGEIFY, _POSTOPT) -> list[RewriteStep]:
  ...
  if _QUANTIZE and opts.device in {"CPU", "DSP"}:
    ret.append(RewriteStep(pm_quant, name="quantize"))
  ...
  if opts.pre_matcher is not None:
    ret.append(RewriteStep(opts.pre_matcher, name="pre_matcher"))
