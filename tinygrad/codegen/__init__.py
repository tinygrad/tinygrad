...
def get_rewrites_for_renderer(opts:Renderer, linearizer:bool=True) -> list[RewriteStep]:
  ...
  return _get_rewrites_for_renderer(opts, linearizer, QUANTIZE.value, DEVECTORIZE.value, TRANSCENDENTAL.value, RANGEIFY.value or POSTOPT.value)

@functools.cache
def _get_rewrites_for_renderer(opts:Renderer, linearizer:bool, _QUANTIZE, _DEVECTORIZE, _TRANSCENDENTAL, _POSTOPT) -> list[RewriteStep]:
  ...
  if _POSTOPT:
    # new optimizer
    ret.append(RewriteStep(pm_postrange_opt, name="post optimize ast"))
