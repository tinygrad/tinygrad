import math
from tinygrad.uop.ops import UOp, Ops, sint, PatternMatcher, UPat, KernelInfo, ssimplify, AxisType, param_variable
...
def add_gpudims(ctx:Renderer, s:UOp):
  ...
  # get the idxs
  ki: KernelInfo = s.arg
  if ctx.has_threads: idxs = [param_variable("core_id", 0, int(global_shape[0])-1, dtypes.int).cast(dtypes.weakint)]
...
pm_device_to_var = PatternMatcher([
  ...
  (UPat(Ops.RANGE, name="r"), lambda r: param_variable("_device_num", 0, r.vmax, dtype=r.dtype) if r.arg[-1] is AxisType.DEVICE else None),
