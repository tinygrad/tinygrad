...
def _split_dims(dims, max_sizes):
  if all(d <= m for d, m in zip(dims, max_sizes)):
    return dims
...
def get_grouped_dims(prefix, dims: tuple[sint, ...], max_sizes: tuple[int, ...] | None, reverse=False) -> list[UOp]:
  ...
  if max_sizes is not None and len(limited) > len(max_sizes):
    raise RuntimeError(f"cannot limit dim {dims=}, {max_sizes=}")
  # try to split up dims: (a,) -> (b, c)
  if limited == dims:
    limited = _split_dims(dims, max_sizes) if max_sizes is not None else dims
  ...
  if len(limited) < len(dims):
    ...
    if (contraction := get_contraction(dims, limited)) is None:
      raise AssertionError(f"get_contraction should not be None {dims=} {limited=}")
    ...
    if a == 2 and b == 1:
      ret = [raw_idxs[0] * limited[1] + raw_idxs[1]]
    if a == 3 and b == 1:
      ret = [raw_idxs[0] * (limited[1] * limited[2]) + raw_idxs[1] * limited[2] + raw_idxs[2]]
    if a == 3 and b == 2:
      ret = [raw_idxs[0] * limited[1] + raw_idxs[1], raw_idxs[2]]
...
def add_gpudims(ctx: Renderer, s: UOp):
  ...
  if any(x.op is Ops.SPECIAL for x in s_topo):
    return None
  ...
  if not global_dims and not local_dims:
    return None
  ...
  subs = {}
  for r in s_topo:
    if r.op is not Ops.RANGE:
      continue
    try:
      ii = (global_dims + local_dims).index(r.arg[0:-1])
      if r.arg[1] == AxisType.REDUCE:
        continue
      subs[r] = idxs[ii]
    except ValueError:
      continue
