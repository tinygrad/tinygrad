...
class CStyleLanguage(NamedTuple):
  ...
  def render_for(self, expr: str, min:int, max:int) -> str:
    return f"for (int {expr} = {min}; {expr} <= {max}; ++{expr}) {{"
  
  def render_conditional(self, cond: str, x:str, y:str) -> str:
    return f"({cond})?({x}):{y}"

  def render_kernel(self, kernel:List[str], bufs:List[Union[LocalBuffer,LazyBuffer]], bufnames:List[str], local_size: List[int]) -> str:
...
def uops_to_cstyle(uops:List[UOp], bufs:List[Union[LocalBuffer,LazyBuffer]], lang:CStyleLanguage) -> Tuple[str, List[int], List[int]]:
  kernel,global_size,local_size,ext_bufs = [],[],[],[]
  pend_close = None
  ...
  for uop,newvar,vin,args in uops:
    if uop == UOps.LOOP:
      ...
      assert newvar is not None
      kk(f"{lang.generic_var_prefix}{newvar.render(newvar not in vin and lang.generic_var_prefix == '')} = {lang.code_for_op[args](*[x.render() for x in vin])};")
    elif uop == UOps.LOAD:
      assert newvar is not None
      ...
      if lang.external_local_bufs:
        ext_bufs.append(lang.render_local(args[0], args[1]))
      else:
  ...
  return lang.render_kernel(kernel, bufs+ext_bufs, bufnames, local_size), global_size[::-1], local_size
