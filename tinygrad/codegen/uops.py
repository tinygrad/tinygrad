...
class UOpGraph:
  def __init__(self, start_uops:Optional[List[UOp]]=None):
    ...
    # from where uops.add will add new uops, a value of None will to the end of the graph
  ...
  def add_ends(self):
    for u in self.uops:
      ...
      elif u.uop is UOps.IF and all([not(x.uop is UOps.ENDIF and x.vin[0] == u) for x in self.uops]):
