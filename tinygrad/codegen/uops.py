...
class UOpGraph:
  ...
  def simplify_phi_loops(self, get_recursive_parents):
    ...
    def const(x, insert_before=None): return self.add(UOps.CONST, dtypes.int64, tuple(), x, insert_before=insert_before)
    def neg(x): return self.add(UOps.ALU, dtypes.int64, (x,), UnaryOps.NEG)
    def max(x, y): return self.add(UOps.ALU, dtypes.int64, (x, y), BinaryOps.MAX)
    def uop_alu_idx(a: UOp, b, op, dtype=dtypes.int64):
    ...
    for loop_op in reversed([op for op in self.uops if op.uop is UOps.LOOP]):
      ...
      for where in sorted(wheres, key=lambda x: self.uops.index(x)):
        ...
        maybe_cast = self.add(UOps.CAST, where.dtype, (max_clamped,)) if where.dtype != dtypes.int64 else max_clamped
