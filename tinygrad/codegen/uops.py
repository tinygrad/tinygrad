...
class UOpGraph:
  ...
  def add(self, uop:UOps, dtype:Optional[DType]=None, vin:Tuple[UOp, ...]=tuple(), arg:Any=None, cachable=True, insert_before=None,
    ...
    if simplify and (rewritten:=constant_folder.rewrite(ret)) is not None:
      ...
      # NOTE: this only supports depth 2 rewrites
      rewritten.vin = tuple(self.add(vin.uop, vin.dtype, vin.vin, vin.arg, insert_before=insert_before) if vin not in self.uops else vin
                            for vin in rewritten.vin)
      ret = rewritten
