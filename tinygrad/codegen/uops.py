...
class UOp:
  ...
  def tuple(self):
    return (self.uop, self.dtype, self.vin, self.arg)
  ...
  def cmp_tuple(self):
    ...
    return (
      self.uop.value,
      (self.arg if self.uop is not UOps.DEFINE_VAR else self.arg.expr) if self.uop is not UOps.ALU else (type(self.uop), self.uop.value),
      self.dtype,
      self.vin,
    )
  ...
  def cast(self, dtype):
    return UOp(UOps.CAST, dtype, (self,))

  def __neg__(self):
    return UOp.alu(UnaryOps.NEG, self)

  def __add__(self, x):
    return UOp.alu(BinaryOps.ADD, self, x)

  def __sub__(self, x):
    return UOp.alu(BinaryOps.SUB, self, x)

  def __mul__(self, x):
    return UOp.alu(BinaryOps.MUL, self, x)

  @staticmethod
  def max(x, y):
    return UOp.alu(BinaryOps.MAX, x, y)
  ...
  @staticmethod
  def const(dtype, val):
    return UOp(UOps.CONST, dtype, arg=dtypes.as_const(val, dtype))

  @staticmethod
  def alu(arg, *vin: UOp):
    return UOp(UOps.ALU, vin[0].dtype, vin, arg)

  @functools.cached_property
  def parents(self) -> Set[UOp]:
    return set.union(set(self.vin), *[x.parents for x in self.vin])


def uop_alu_resolve(u: UOp) -> sint:
  if u.uop is UOps.CONST:
    return u.arg
  elif u.uop is UOps.DEFINE_VAR:
    return u.arg
  elif u.uop is UOps.SPECIAL:
    return u.arg[2] - 1
  elif u.uop is UOps.ALU and u.arg is BinaryOps.MUL:
    return uop_alu_resolve(u.vin[0]) * uop_alu_resolve(u.vin[1])
  elif u.uop is UOps.ALU and u.arg is BinaryOps.ADD:
    return uop_alu_resolve(u.vin[0]) + uop_alu_resolve(u.vin[1])
  else:
...
def _match(uop: UOp, pattern: UPat, store: Dict[str, UOp]) -> bool:
  ...
  if pattern.vin:
    for vp in (
      itertools.permutations(pattern.vin)
      if isinstance(pattern.vin, list)
      else ([pattern.vin] if isinstance(pattern.vin, tuple) else [(pattern.vin,) * len(uop.vin)])
    ):
...
def loop_collapse(loop_start, loop_end, compval, idx, mval, multconst):
  if mval.arg >= 0 or loop_start.arg != 0:
    ...
    if DEBUG >= 1:
      print(f"WARNING, NOT FOLDING: mval:{mval.arg} loop_start:{loop_start.arg}")
    return None
...
constant_folder = PatternMatcher([
  ...
  (
    UPat(
      uop=UOps.PHI,
      vin=(
        UPat(name="phi_input", uop=UOps.DEFINE_ACC, vin=(UPat(uop=UOps.RANGE, name="loop"),)),
        UPat(uop=UOps.ALU, arg=BinaryOps.ADD, vin=(UPat(name="val1"), UPat(name="val2"))),
      ),
    ),
    sum_collapse,
  ),
  (
    UPat(
      uop=UOps.PHI,
      vin=(
        UPat(name="phi_input", uop=UOps.GEP, vin=(UPat(uop=UOps.DEFINE_ACC, vin=(UPat(uop=UOps.RANGE, name="loop"),)),)),
        UPat(uop=UOps.ALU, arg=BinaryOps.ADD, vin=(UPat(name="val1"), UPat(name="val2"))),
      ),
    ),
    sum_collapse,
  ...
  # x+-y -> x-y
  (UPat(uop=UOps.ALU, arg=BinaryOps.ADD, vin=(UPat(name="x"), UPat(name="my", uop=UOps.ALU, arg=UnaryOps.NEG))), lambda x, my: x - my.vin[0]),
  # -1*x -> -x
  ...
  # ** load/store folding **
  (
    UPat(uop=UOps.STORE, vin=(UPat(name="buf"), UPat(name="idx"), UPat(uop=UOps.LOAD, vin=(UPat(name="buf"), UPat(name="idx"))))),
    lambda buf, idx: UOp(UOps.NOOP),
  ),
  ...
  (
    UPat(
      uop=UOps.ALU,
      arg=BinaryOps.ADD,
      vin=[UPat(uop=UOps.ALU, arg=BinaryOps.ADD, vin=[UPat(name="x"), UPat(name="c1", uop=UOps.CONST)]), UPat(name="c2", uop=UOps.CONST)],
    ),
  ...
  (
    UPat(
      uop=UOps.ALU,
      arg=BinaryOps.ADD,
      vin=[UPat(uop=UOps.ALU, arg=BinaryOps.SUB, vin=(UPat(name="x"), UPat(name="c1", uop=UOps.CONST))), UPat(name="c2", uop=UOps.CONST)],
    ),
  ...
  (
    UPat(
      ...
      vin=(
        ...
        UPat(name="idx"),
        UPat(
          uop=UOps.ALU, arg=TernaryOps.WHERE, vin=(UPat(name="gate"), UPat(name="alt"), UPat(uop=UOps.LOAD, vin=(UPat(name="buf"), UPat(name="idx"))))
        ),
      ),
    ),
  ...
  (
    UPat(
      uop=UOps.STORE,
      vin=(UPat(name="buf"), UPat(name="idx"), UPat(uop=UOps.CAST, vin=[UPat(uop=UOps.GEP, vin=(UPat(name="val"),), arg=i) for i in range(4)])),
    ),
  ...
  (
    UPat(
      uop=UOps.STORE,
      vin=(UPat(name="buf"), UPat(name="idx"), UPat(uop=UOps.CAST, vin=[UPat(uop=UOps.GEP, vin=(UPat(name="val"),), arg=i) for i in range(2)])),
    ),
    lambda buf, idx, val: UOp(UOps.STORE, None, (buf, idx, val)),
  ),
  ...
  (
    UPat(
      name="root",
      uop=UOps.CAST,
      vin=[UPat(uop=UOps.PHI, vin=(UPat(uop=UOps.GEP, vin=(UPat(name="val"),), arg=i), UPat(name=f"v{i}"))) for i in range(4)],
    ),
  ...
  (
    UPat(
      name="root",
      uop=UOps.CAST,
      vin=[UPat(uop=UOps.PHI, vin=(UPat(uop=UOps.GEP, vin=(UPat(name="val"),), arg=i), UPat(name=f"v{i}"))) for i in range(2)],
    ),
  ...
  (
    UPat(
      uop=UOps.ALU,
      arg=BinaryOps.CMPLT,
      vin=(UPat(uop=UOps.ALU, arg=UnaryOps.NEG, vin=(UPat(name="x"),)), UPat(name="c", uop=UOps.CONST, dtype=dtypes.int)),
    ),
...
class UOpGraph:
  ...
  def __getitem__(self, index) -> UOp:
    return self.uops[index]

  def vars(self) -> List[Variable]:
    return [x.arg for x in self.uops if x.uop is UOps.DEFINE_VAR]

  def globals(self) -> List[Tuple[int, bool]]:
    return [x.arg for x in self.uops if x.uop is UOps.DEFINE_GLOBAL]
  ...
  def graph_rewrite(self, sink, pm):
    ...
    while changed:
      ...
      def rewrite(u: UOp) -> UOp:
        ...
        if hasattr(up, "parents"):
          del up.parents
        if hasattr(up, "cmp_tuple"):
          del up.cmp_tuple
  ...
  def linearize(self, extra_pm: Optional[PatternMatcher] = None, type_verify=True):
    ...
    if extra_pm:
      sink = self.graph_rewrite(sink, PatternMatcher(constant_folder.patterns + extra_pm.patterns))
    ...
    while queue:
      ...
      for u, ss in loops_children.items():
        if x in ss:
          ss.remove(x)
          if len(ss) == 0:
            self._uops.append(UOp(UOps.ENDRANGE, None, (u,)))
      for u in graph[x]:
        in_degree[u] -= 1
        if in_degree[u] == 0:
          push(u)
  ...
  def add(self, uop: UOps, dtype: Optional[DType] = None, vin: Tuple[UOp, ...] = tuple(), arg: Any = None) -> UOp:
    if found := self.nodes.get(key := (uop, dtype, vin, arg)):
      return found
  ...
  def type_verify(self):
    for u in self.uops:
      ...
      if uop in {UOps.CAST, UOps.BITCAST}:
        assert arg is None  # type is the output type, not an arg
      if uop is UOps.ALU:
        if arg in UnaryOps:
