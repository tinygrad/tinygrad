...
def payne_hanek_reduction(d:UOp) -> tuple[UOp, UOp]:
  ...
  def _take(an:UOp, offset:int, count:int=0) -> UOp:
    ...
    return an
  def _shl_lazy(x:UOp, y:UOp): return UOp(Ops.SHL, dtypes.uint64, (x.cast(dtypes.uint64), y.cast(dtypes.uint32))).cast(dtypes.uint32)
  def _shr_lazy(x:UOp, y:UOp): return UOp(Ops.SHR, dtypes.uint64, (x.cast(dtypes.uint64), y.cast(dtypes.uint32))).cast(dtypes.uint32)
