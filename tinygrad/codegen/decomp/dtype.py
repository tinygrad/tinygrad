...
pm_long_decomp = PatternMatcher([
  (UPat(GroupOp.Defines, src=(UPat.var("sz"),), name="x"), lambda x,sz:
   x.replace(dtype=None, arg=replace(x.arg, dtype=l2i_dt[x.dtype]), src=(sz*2,)) if x.dtype in l2i_dt else None),
...
pm_float_decomp = PatternMatcher([
  (UPat((*GroupOp.Defines, Ops.INDEX, Ops.SHRINK), name="x"), lambda ctx,x:
   x.replace(dtype=None, arg=replace(x.arg, dtype=f2f_dt[ctx[0]]) if isinstance(x.arg, ParamArg) else x.arg, tag=ctx[0])
