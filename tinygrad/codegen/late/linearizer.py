...
def linearize(sink:UOp) -> list[UOp]:
  ...
  for u in reversed(lst):
    ...
    match u.op:
      ...
      case Ops.KERNEL: priority = 6   # Kernel is priority 6 for now
      case Ops.END: priority = -5     # placing END is bad
      case _: priority = 0            # everything else has priority 0
    priorities[u] = (run_count, priority, extra)

  # number the uops in "ideal" order
  nkey = {u:i for i,u in enumerate(sorted(lst, key=lambda x: priorities[x]+((x.tuplize,) if TUPLE_ORDER else (x.key,))+(id(x),)))}
