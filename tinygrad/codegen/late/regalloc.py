...
from tinygrad.renderer.isa import ISARenderer, Register, greg, machine_const
  ...
  def __init__(self, uops:list[UOp], ren:ISARenderer):
    ...
    def fill(v:Register, i:int, cons:tuple[Register, ...]|None=None) -> Register:
      if v not in self.spills:
        ...
        self.spills[v] = machine_const(dtypes.int32, offset)
    ...
    for i,u in enumerate(uops):
      ...
      if u.op is Ops.BUFFER:
        self.locals[u] = machine_const(dtypes.int32, self.stack_size)
...
def regalloc_rewrite(ctx:LinearScanRegallocContext, x:UOp):
  ...
  if ctx.stack_size > 0:
    ...
    offset = machine_const(sp.dtype, ctx.stack_size)
