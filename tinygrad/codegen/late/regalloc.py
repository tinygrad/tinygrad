...
from tinygrad.renderer.isa import ISARenderer, Register, reg
...
class LinearScanRegallocContext:
  ...
  def __init__(self, uops:list[UOp], ren:ISARenderer):
    ...
    for i,u in enumerate(reversed(uops)):
      ...
      for v in defs + tuple(reg(s) for s in dedup(u.src)):
      ...
      if u.op is Ops.RANGE: ranges.append(reg(u))
    ...
    for i,u in enumerate(uops):
      ...
      for s in u.src:
        ...
        if not isinstance(v:=reg(s), Register): continue
      ...
      if u.op is Ops.RANGE:
        ...
        used_in_loop = [v for v in live.keys() | self.spills.keys() if any(i <= l < lr[reg(u)][-1] for l in lr[v])]
...
def regalloc_rewrite(ctx:LinearScanRegallocContext, x:UOp):
  ...
  for j,s in enumerate(x.src):
    ...
    if i in ctx.reals and (v:=reg(ctx.uops[i].src[j])) in ctx.spills: nsrc.append(ctx.ren.fill(ctx.spills[v], ctx.vdef(v), ctx.reals[i][v]))
    else: nsrc.append(s)
