...
# the alt value of a gated LOAD is a data constant at the load's width (a bool load takes False)
def alt_value(l:UOp) -> UOp: return UOp.const(False if l.dtype is dtypes.bool else 0).cast(l.dtype).broadcast(l.max_numel())

def move_where_load(gate, l, a, w):
  return l.replace(src=(l.src[0], alt_value(l) if a.arg is Invalid else
...
pm_move_gates_from_index = PatternMatcher([
  ...
  (UPat.var("buf").index(UPat.var("gate").where(UPat.var("idx_y"), UPat(arg=Invalid)),
   ...
   lambda buf,gate,idx_y,idx_x,l: buf.index(idx_y, idx_x, dtype=dtypes.float).load(alt_value(l), gate)),
  (UPat.var("buf").index(UPat.var("gate").where(UPat.var("idx_y"), UPat(arg=Invalid)),
  ...
  (UPat((Ops.INDEX, Ops.SHRINK), src=(UPat(), UPat.var("gate").where(UPat.var("idx"), UPat(arg=Invalid)),), name="mop", allow_any_len=True) \
   .load(name="l"), lambda mop,gate,idx,l: mop.replace(src=(mop.src[0],idx)+mop.src[2:]).load(alt_value(l), gate)),
  (UPat((Ops.INDEX, Ops.SHRINK), src=(UPat(), UPat.var("gate").where(UPat.var("idx"), UPat(arg=Invalid)),), name="mop", allow_any_len=True) \
