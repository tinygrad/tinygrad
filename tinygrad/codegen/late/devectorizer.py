...
load_store_indexing = PatternMatcher([
  ...
  (
    UPat(
      Ops.STORE,
      src=(UPat.any(stidx := UPat.var("buf").index(UPat.var("idx"), UPat.var("store_gate")), stidx.cast().named("cast")), UPat.var("val")),
      name="store",
      allow_any_len=True,
    ),
...
def expand_index(buf: UOp, vec: UOp, mask: UOp | None = None):
  ...
  midx = graph_rewrite(
    UOp.sink(*[buf.index(vec.gep(i), mask.gep(i) if mask is not None else None) for i in range(vec.dtype.count)]),
  ...
  for i in range(vec.dtype.count):
    ...
    if idx.op is Ops.ADD and idx.src[1].op is Ops.CONST:
      ...
      root_src, arg = idx, 0
    if len(midx.src[i].src) == 3:
      root_src = (midx.src[i].src[2], root_src)
...
load_store_folding = PatternMatcher([
  ...
  (UPat(Ops.INDEX, src=(UPat(Ops.VECTORIZE, src=UPat(GroupOp.Defines, name="buf")), UPat.var("vec"), UPat.var("mask"))), expand_index),
  # GEP after LOAD
  (
...
def split_load_store(ctx: Renderer | None, ls: UOp, idx: UOp):
  ...
  if must_divide:
    lengths = [x for x in lengths if idx.src[1].divides(x) is not None]
...
pm_render = PatternMatcher([
  ...
  (
    ...
    if len(store.src) <= 2 or store.src[2].op != Ops.IF
    else None,
  ),
])
...
def reduce_rangeless(red: UOp):
  ...
  if red.arg not in {Ops.ADD, Ops.MAX}:
    return None
  if red.src[0].dtype != red.dtype:
    return None
  if any(x.op in {Ops.RANGE} for x in red.src[0].toposort()):
    return None
  ret = red.src[0]
...
def no_range(u: UOp) -> bool:
  return not any(x.op is Ops.RANGE for x in u.sparents)
...
def reduce_collapse(red: UOp):
  ...
  if any(x.op is Ops.RANGE for x in sink.toposort()):
    return None
