...
pm_render = PatternMatcher([
  ...
  (UPat.var("c").where(UPat(Ops.LOAD, src=(UPat().index(UPat()).or_casted(), UPat.var("c"), UPat()), allow_any_len=True, name="l").or_casted(),
    UPat.var("a")), lambda c,l,a: l.replace(src=(l.src[0], l.src[1],
      a.src[0] if a.op is Ops.CAST and a.src[0].dtype == l.dtype else a.cast(l.dtype))+l.src[3:]).cast(a.dtype)),
  (UPat.var("c").where(UPat.var("a"), UPat(Ops.LOAD, src=(UPat().index(UPat()).or_casted(), UPat.var("c", dtype=dtypes.bool).logical_not(), UPat()),
    allow_any_len=True, name="l").or_casted()), lambda c,l,a: l.replace(src=(l.src[0], l.src[1],
      a.src[0] if a.op is Ops.CAST and a.src[0].dtype == l.dtype else a.cast(l.dtype))+l.src[3:]).cast(a.dtype)),
])
