...
def fold_expanded_index(sink:UOp):
...
pm_fold_expanded_index = PatternMatcher([
  (UPat(Ops.SINK, name="sink"), fold_expanded_index),
])

pm_expand_index_pure = PatternMatcher([
  (UPat(Ops.INDEX, src=(UPat(Ops.VECTORIZE, src=UPat(GroupOp.Defines).or_after(name="buf")), UPat.var("vec"))), expand_index),
])

pm_expand_index = pm_expand_index_pure + pm_fold_expanded_index
