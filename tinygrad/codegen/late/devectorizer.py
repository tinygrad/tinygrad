...
load_store_indexing = PatternMatcher([
  ...
  (((UPat.var("x",dtypes.index)*UPat.cvar("c1",vec=False)+UPat.var("y")) % UPat.cvar("d",vec=False))<UPat.cvar("c2",vec=False), lambda x,y,c1,d,c2:
    x<=y if c1.arg==d.arg-1 and x.vmin>=0 and y.vmin>=0 and x.vmax<(d.arg-c2.arg) and y.vmax<c2.arg else None),
])
