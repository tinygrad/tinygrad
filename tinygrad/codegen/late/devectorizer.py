...
def reduce_to_acc(ctx:ReduceContext, red:UOp):
  ...
  if len(reduce_range) != 0:
    topo = inp.toposort()
    ended_ranges = set(flatten([x.ended_ranges for x in topo if x.op is Ops.END]))
    reduce_range_set = set(reduce_range)
    input_ranges = tuple([x for x in topo if x.op is Ops.RANGE and x not in reduce_range_set and x not in ended_ranges])
