...
class AssemblyCodegen(Linearizer):
  ...
  def codegen(self):
    ...
    def addr_w_offset(args):
      idx = args.idx*self.raw_bufs[args.i].dtype.itemsize
    ...
    ins = []
    ins += [AssemblyInstruction(UOps.SPECIAL, newreg(f"buf{i}", dtype=dtypes.uint64, scalar=True), [], f"buf{i}") for i in range(len(self.raw_bufs))]
