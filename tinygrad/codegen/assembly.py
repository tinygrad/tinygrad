...
class AssemblyCodegen(Linearizer):
  ...
  def codegen(self):
    ...
    ins = []
    ins += [AssemblyInstruction(UOps.SPECIAL, newreg(f"buf{i}", dtype=dtypes.uint64, scalar=True), [], f"buf{i}") for i in range(len(self.dedup_bufs)) if self.dedup_bufs[i].__class__ is not LocalBuffer]
