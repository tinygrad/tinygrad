...
def reduce_collapse(red:UOp, u:UOp, pm:PatternMatcher=pm_reduce_collapse) -> UOp|None:
  ...
  # only at the exit: a cast between the REDUCE and its source would blind the next range's collapse pattern
  return u.cast(red.dtype) if u.dtype in dtypes.weaks and red.dtype not in dtypes.weaks else u
