...
pm_render = PatternMatcher([
  ...
  (UPat(Ops.STORE, src=(UPat(src=(UPat(), UPat(), UPat(name="gate", dtype=dtypes.bool))).or_casted(), UPat()), name="store"),
    lambda gate,store: UOp(Ops.STORE, dtype=store.dtype, src=store.src[:2]+(UOp(Ops.IF, src=(gate,)),)+store.src[2:])),
])
