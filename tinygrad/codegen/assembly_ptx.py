...
class PTXCodegen(AssemblyCodegen):
  ...
  def specialize(self, asm):
    ...
    alu = {BinaryOps.ADD: "add", BinaryOps.SUB: "sub", BinaryOps.MUL: "mul", BinaryOps.DIV: "div", BinaryOps.MAX: "max",
           ...
           UnaryOps.NOOP: "mov", UnaryOps.SIN: "sin.approx", UnaryOps.LOG2: "lg2.approx", UnaryOps.EXP2: "ex2.approx.ftz", UnaryOps.FLOOR: "cvt.rmi",
    ...
    for uop, out, vin, arg in asm:
      if uop == UOps.DEFINE_REGISTER:
        ...
        if arg == BinaryOps.MUL and out.dtype == dtypes.bool:
          ...
          ins.append(f"{alu[arg]}{'.lo' if arg == BinaryOps.MUL and out.dtype != dtypes.float32 else ''}{'.rn' if arg == BinaryOps.DIV and out.dtype == dtypes.float32 else ''}.{dtype_to_nvtype[otype]}{f'.{dtype_to_nvtype[vin[0].dtype]}' if arg == UnaryOps.FLOOR else ''} {out}, {', '.join(str(x) for x in vin)};")
      elif uop == UOps.LOAD:
