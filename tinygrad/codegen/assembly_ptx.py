...
def specialize_to_ptx(lang, function_name, asm):
  ...
  for uop, out, vin, arg in asm:
    if uop == UOps.ENDLOOP:
      ...
      if arg == BinaryOps.MUL and out.dtype == dtypes.bool:
        ...
        # if arg == TernaryOps.WHERE: vin[0], vin[2] = vin[2], vin[0]
      ...
      if arg.__class__ in (int, float):
        ...
        ins.append(f"ld.{arg[1]}.{dtype_to_nvtype[dtypes.float]} {out}, [{vin[0]}{f'+{arg[0]}' if arg[0] is not None else ''}];")
    elif uop == UOps.STORE:
  ...
  for arg in registers: ins_prefix.append(f".reg .{dtype_to_nvtype[arg[0][0]]} %{arg[1]}<{arg[2]}>;",)
...
def uops_to_ptx_asm(function_name:str, uops:List[UOp]):
  ...
  return specialize_to_ptx(lang, function_name, lang.ins), global_size[::-1], local_size[::-1], True
