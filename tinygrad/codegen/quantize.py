...
pm_quant = symbolic + PatternMatcher([
  ...
  (
    ...
    * UPat(Ops.LOAD, src=(UPat().view(name="v"),)).cast(dtypes.int).cast(dtypes.float).named("ld"),
  ...
  (
    ...
    * (
      ...
      + UPat(Ops.VALID, src=(UPat(Ops.VIEW, name="v"),)).where(UPat.cvar(), UPat(Ops.CONST, arg=0))
    )
    .cast(dtypes.float)
    .named("ld"),
    lambda ld, v, c1: ld * c1,
  ...
  (
    ...
    lambda x, c1, y, c2, cc: ((x * (c1 * FP).cast(x.dtype) + y.cast(x.dtype) * (c2 * FP).cast(x.dtype) + (cc * FP).cast(x.dtype)) // FP).cast(
      dtypes.int
    ),
  ),
  # where move
  (
    UPat.var("valid").where(UPat.var("yes"), UPat(Ops.CONST, arg=0)) * UPat.var("mul"),
    lambda valid, yes, mul: (yes * mul * valid.where(UOp.const(mul.dtype, 1), UOp.const(mul.dtype, 0)))
    if yes.op is not Ops.CONST or yes.arg != 1
    else None,
  ),
  ((UPat.var("x") * UPat.cvar("c")) * (UPat.var().where(UPat(Ops.CONST, arg=1), UPat(Ops.CONST, arg=0)).named("v")), lambda x, c, v: (x * v) * c),
  (
    UPat.var("x").cast().named("c") * UPat.var("valid").where(UPat(Ops.CONST, arg=1), UPat(Ops.CONST, arg=0)),
    lambda x, c, valid: (x * valid.where(UOp.const(x.dtype, 1), UOp.const(x.dtype, 0))).cast(c.dtype),
  ),
  (
    (
      UPat.var("x")
      * UPat.var("v1").where(UPat(Ops.CONST, arg=1), UPat(Ops.CONST, arg=0))
      * UPat.var("v2").where(UPat(Ops.CONST, arg=1), UPat(Ops.CONST, arg=0))
    ).named("mul"),
    lambda x, mul, v1, v2: x * (v1 & v2).where(UOp.const(mul.dtype, 1), UOp.const(mul.dtype, 0)),
  ),
  # where on two adds
  (
  ...
  (
    UPat(
      Ops.REDUCE_AXIS,
      src=(
        (UPat(Ops.CAST, name="v1") + UPat.var("c1"))
        * (
          UPat(
            Ops.CAST,
            name="v2",
