...
def hand_coded_optimizations(k: Kernel) -> list[Opt]:
  ...
  if (
    ...
    def has_expanded_axis(shape, strides):
      return any(resolve(s > 1) and not resolve(st != 0) for s, st in zip(shape, strides))

    if strides0[first_reduce := (k.axes_of(AxisType.REDUCE)[0])] == 1 and not (
      has_expanded_axis(st0.shape, strides0) and has_expanded_axis(st1.shape, strides1)
    ):
  ...
  for axis in k.upcastable_dims:
    if (
      k.full_shape[axis] <= 7
      and any(st.axis_is_masked(axis) for st in k.sts)
  ...
  while resolve(prod(k.output_shape[i] for i in k.upcastable_dims) >= 1024):
    ...
    for axis, upcast_amount in itertools.product(k.upcastable_dims, ([128] if not len(upcasted_axis) else []) if is_dsp else [3, 4]):
      ...
      if axis in upcasted_axis or k.full_shape[axis] % upcast_amount != 0:
        continue
      if any(
        st.views[-1].strides[axis] == 0 and all(x != 0 for t, x in zip(k.axis_types, st.real_strides()) if t in (AxisType.UPCAST, AxisType.UNROLL))
        for st in k.sts
      ):
