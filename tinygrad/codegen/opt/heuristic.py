...
def hand_coded_optimizations(k:Scheduler) -> Scheduler:
  ...
  for axis in k.upcastable_dims:
    ...
    is_masked = any(k.rngs[axis] is not u.src[0] and k.rngs[axis] in u.src[0].toposort() for u in k.ast.toposort() if u.op is Ops.WHERE)
  ...
  while resolve(prod(k.output_shape[i] for i in k.upcastable_dims) >= 1024) and (k.upcast_size() < 32):
    ...
    for axis, upcast_amount in itertools.product(k.upcastable_dims, ([128] if not len(upcasted_axis) else []) if is_dsp else [3,4]):
      ...
      rng = k.rngs[axis]
      if any((rng is (idx_t:=b.src[1].get_idx()) or rng not in idx_t.toposort()) and all(r2 is not idx_t and r2 in idx_t.toposort()
          for r2 in k.ranges_of(AxisType.UPCAST, AxisType.UNROLL)) for b in k.bufs):
        num_strides, sum_strides = 0, 0
        for b in k.bufs:
          idx = b.src[1].get_idx()
          if rng is not idx and rng in idx.toposort(): num_strides += 1
  ...
  if k.ren.has_local:
    if NOLOCALS:
      ...
      local_axis_ranking = [(any(k.rngs[axis] is (idx:=b.src[1].get_idx()) or k.rngs[axis] not in idx.toposort() for b in k.bufs), axis) \
