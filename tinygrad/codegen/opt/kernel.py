...
class Kernel:
  def __init__(self, ast: UOp, opts: Renderer | None = None):
    ...
    if __debug__:
      type_verify(list(self.ast.toposort()), ast_spec)
  ...
  def axes_of(self, *axis_type: AxisType) -> list[int]:
    return [i for i, t in enumerate(self.axis_types) if t in argfix(axis_type)]
  ...
  def permute(self, new_axes: Sequence[int]):
    self.sts = [st.permute(tuple(new_axes)) for st in self.sts]
  ...
  def shift_to(self, axis: int, amount: int, new_type: AxisType, top: bool = False, insert_at: int | None = None) -> int:
    if insert_at is None:
      insert_at = self.shape_len
    self.axis_types.insert(insert_at, new_type)
    move_axis = axis if top else axis + 1
    if move_axis < insert_at:
      insert_at += 1

    def new_shape_fxn(x):
      return x[0:axis] + (((amount, x[axis] // amount) if top else (x[axis] // amount, amount)) if x[axis] > 1 else (1, 1)) + x[axis + 1 :]
  ...
  def apply_opt(self, opt: Opt, append_opt: bool = True) -> int | None:
    if self.finalized:
      raise RuntimeError("can't optimize Kernel after it's finalized")
    if self.dont_use_locals:
      check(opt.op not in {OptOps.LOCAL, OptOps.GROUP, OptOps.GROUPTOP}, "not using locals")
    ...
    if opt.op is OptOps.SWAP:
      amt = self.real_axis(opt.op, cast(int, opt.arg))  # arg is an axis in the SWAPs
    ...
    if opt.op is OptOps.LOCAL:  # cyan
      ...
      new_axis = self.shift_to(
        axis, amt, AxisType.UPCAST, insert_at=max(self.axes_of(AxisType.GLOBAL, AxisType.LOCAL, AxisType.LOOP, AxisType.UPCAST)) + 1
      )
      ...
      check(
        self.axis_types[axis] == self.axis_types[amt] == AxisType.GLOBAL, f"swap is for globals {self.axis_types[axis]=}, {self.axis_types[amt]=}"
      )
  ...
  def _create_tc_opts(self, reduceop: UOp, tc: TensorCore, axis: int, opt_level: int) -> TensorCoreOptions | None:
    ...
    if (buf0 := buf_index(mul_op.src[0])) is None or (buf1 := buf_index(mul_op.src[1])) is None:
      return None
    ...
    if not (axis_buf0 and axis_buf1 and (len(self.axes_of(AxisType.GROUP_REDUCE, AxisType.REDUCE)) == 1 or (opt_level >= 1))):
      return None
      ...
      for tc in tensor_cores:
        ...
        try:
          for axis, dim in tc_opts.axis_pads:
            self.apply_opt(Opt(OptOps.PADTO, axis, dim), append_opt=False)  # PADTO might fail
  ...
  def apply_tensor_cores(
    self, use_tensor_cores=1, extra_opts: list[Opt] | None = None, axis: int = 0, tc_select: int | None = None, tc_opt: int | None = None
  ) -> bool:
