...
def bufs_from_lin(lin: Kernel, allocate: bool = True) -> list[Buffer]:
  ...
  for x in lin.bufs:
    if x.src[0].base.op is Ops.DEFINE_GLOBAL:
      bufsts[x.src[0].base.arg].append(x)
...
def get_kernel_actions(lin: Kernel, include_0=True, candidates: list[Opt] | None = None) -> dict[int, Kernel]:
  ...
  for i, a in enumerate(kernel_actions):
    if a.axis is not None and a.op is not OptOps.TC:
      try:
        ax = lin.real_axis(a.op, a.axis)
      except KernelOptError:
        continue
      if (ax >= lin.shape_len) or (lin.full_shape[ax] == a.arg and Opt(a.op, a.axis, 0) in kernel_actions):
        continue
    lin2 = lin.copy()
    try:
      lin2.apply_opt(a)
      ...
      for s, c in zip(lin2.full_shape, lin2.axis_types):
        if c in (AxisType.UPCAST, AxisType.UNROLL):
          up *= s
        elif c in (AxisType.LOCAL, AxisType.GROUP_REDUCE):
          lcl *= s
...
def beam_search(lin: Kernel, rawbufs: list[Buffer], amt: int, allow_test_size=True, disable_cache=IGNORE_BEAM_CACHE.value) -> Kernel:
  ...
  if BEAM_DEBUG:
    print(f"BEAM_SEARCH:\n{lin.ast}")
  if DEBUG >= 2:
    print(f"   0.00s:                from   1 ->   1 actions {lin.colored_shape()}")
    ...
    while not exiting:
      ...
      for i, proc in map(_compile_fn, enumerate(acted_lins)) if beam_pool is None else beam_pool.imap_unordered(_compile_fn, enumerate(acted_lins)):
        ...
        try:
          ...
          if BEAM_DEBUG:
            print(f"BEAM failed for opts: {acted_lins[i].applied_opts}\n{e}")
        ...
        if BEAM_DEBUG > 1:
          print(
            f"{time.perf_counter() - st:7.2f}s: {i:5d} {len(cast(list, p.uops)):5d} uops {time_to_str(compile_et, w=12)} compile/{time_to_str(timed_lins[-1][1], w=12)} run       {len(timed_lins):4d}/{len(acted_lins):4d}         {timed_lins[-1][0].colored_shape()}"
          )  # noqa: E501
        elif DEBUG >= 2:
          print(
            f"\r{time.perf_counter() - st:7.2f}s: {time_to_str(timed_lins[-1][1], w=12)}       {len(timed_lins):4d}/{len(acted_lins):4d}         {timed_lins[-1][0].colored_shape()}\033[K",
            end="",
          )  # noqa: E501
