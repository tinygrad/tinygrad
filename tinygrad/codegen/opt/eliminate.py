...
def generate_omega(max_tile: int = 128, tile_step: int = 8) -> Iterator[MatmulConfig]:
  tiles = [t for t in range(tile_step, max_tile + 1, tile_step)]
...
def conv2d_opts(shape: tuple, hw: MetalHardware) -> list:
  ...
  def find_div(n, targets):
    ...
    return 1
  if Wo >= 4 and Wo % 4 == 0: opts.append(Opt(op=OptOps.UPCAST, axis=2, arg=4)); Wo = Wo // 4
  if Co >= 4 and Co % 4 == 0: opts.append(Opt(op=OptOps.UPCAST, axis=0, arg=4)); Co = Co // 4
