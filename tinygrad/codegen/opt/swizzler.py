...
def reduce_push_add_ones(src: UOp, r: UOp, view: UOp):
  ...
  if (
    unwrap(view.st).contiguous
    and len(r.shape) < len(view.shape)
    and tuple(x for x in r.shape if resolve(x != 1)) == tuple(x for x in view.shape if resolve(x != 1))
  ):
    new_shape: list[sint] = []
    new_reduce_axis = []
    if (contraction := get_contraction_with_reduce(view.shape, r.shape, r.arg[1])) is None:
      return None
...
view_left_through_load = PatternMatcher([
  # view before load
  (UPat(Ops.VIEW, src=(UPat(Ops.LOAD, name="e"),), name="view"), lambda e, view: e.replace(src=tuple(s.view(view.st) for s in e.src))),
])


def apply_swizzle(u: UOp) -> UOp:
  return graph_rewrite(u, view_left, name="Sub View Left")
...
def swizzle_reduceop(r: UOp, src: UOp, view: UOp, fuse=False):
  ...
  if (
    unwrap(view.st).contiguous
    and view.size == r.size
    and (
      not (len(r.arg) == 3 and r.arg[2])  # arg[2] = True is fuse marker
      or tuple((i, x) for i, x in enumerate(r.shape) if resolve(x != 1)) == tuple((i, x) for i, x in enumerate(view.shape) if resolve(x != 1))
    )
  ):
    return None
  # swizzle the input
  ...
  if fuse:
    red = UOp(Ops.REDUCE_AXIS, r.dtype, (swizzled_input.fuse(),), (r.arg[0], new_axis, True))
  else:
    red = UOp(Ops.REDUCE_AXIS, r.dtype, (swizzled_input,), (r.arg[0], new_axis))
...
def elementwise_view_right(root: UOp):
  if not (swizzles := [x for x in root.src if x.op is Ops.VIEW and x.base.op not in ALWAYS_CONTIGUOUS]):
    return None
...
def check_load_st(glbl: UOp, view: UOp):
  ...
  if len(st.views) == 1 and st.shrink(tuple((0, 1) if st == 0 else (0, s) for s, st in zip(st.shape, st.views[0].strides))).contiguous:
    return
  # if it has a single view and it's equal when you shrink a contig, it's fine
  if len(st.views) == 1 and (mask := st.views[0].mask) is not None and ShapeTracker.from_shape(st.shape).shrink(mask) == st.shrink(mask):
    return
