...
constant_folder = PatternMatcher([
  ...
  #(UPat(UOps.GEP, name="gep", src=(UPat(UOps.ALU, name='alu'),)),
  #  lambda gep,alu: UOp(UOps.ALU, gep.dtype, tuple(y.gep(gep.arg) for y in alu.src), alu.arg)),
  ((NOp.var('x') + NOp.var('c1')).lt(NOp.var('c2')), lambda x,c1,c2: x.lt(c2-c1)),
  ...
  *[(NOp(UOps.REDUCE, src=(NOp(UOps.EXPAND, src=tuple(NOp(UOps.GEP, dtypes.float, src=(NOp.var('x'),), arg=i) for i in range(j)), name="expand"),)
  ...
  # ** mod **
  # mod folding
  (NOp.var('x') % NOp.cvar('c'),
   lambda x,c: newx if not isinstance(c.arg, tuple) and 0 < c.arg and (newx:=mod_folding(x,c.arg)) is not None else None),
  # mul mod
