...
float4_folding = PatternMatcher([
  ...
  (NOp(UOps.STORE, src=(_nop(name="buf"), _nop(name="idx")+
    (NOp(UOps.EXPAND, src=tuple(NOp.const(dtypes.int, i) for i in range(4)), name="ex")+_nop(name="idx2")), _nop(name="var")), name="store"),
    lambda buf, store, idx, idx2, ex, var: UOp(UOps.STORE, store.dtype, (buf, idx+idx2+ex, var), store.arg)),
  # float(2,4) load
  (NOp(UOps.LOAD, src=(_nop(name="buf"), NOp(UOps.EXPAND, name="ex")+_nop(name="idx")+_nop(name="idx2")+_nop(name="idx3")), name="load"),
   float4_expand_load),
  (NOp(UOps.LOAD, src=(_nop(name="buf"), NOp(UOps.EXPAND, name="ex")+_nop(name="idx")+_nop(name="idx2")), name="load"), float4_expand_load),
  (NOp(UOps.LOAD, src=(_nop(name="buf"), NOp(UOps.EXPAND, name="ex")+_nop(name="idx")), name="load"), float4_expand_load),
  (NOp(UOps.LOAD, src=(_nop(name="buf"), NOp(UOps.EXPAND, name="ex")), name="load"), float4_expand_load),
  # float(2,4) store
  # TODO: fold ADDs into one UOp and remove add chains
  (NOp(UOps.STORE, src=(_nop(name="buf"),
    NOp(UOps.EXPAND, name="ex")+_nop(name="idx")+_nop(name="idx2")+_nop(name="idx3"), _nop(name="var")), name="store"),
    float4_contract_store),
  (NOp(UOps.STORE, src=(_nop(name="buf"),
    NOp(UOps.EXPAND, name="ex")+_nop(name="idx")+_nop(name="idx2"), _nop(name="var")), name="store"),
    float4_contract_store),
  (NOp(UOps.STORE, src=(_nop(name="buf"),
    NOp(UOps.EXPAND, name="ex")+_nop(name="idx"), _nop(name="var")), name="store"), float4_contract_store),
  (NOp(UOps.STORE, src=(_nop(name="buf"),
    NOp(UOps.EXPAND, name="ex"), _nop(name="var")), name="store"), float4_contract_store),
  # image handling
  (NOp(UOps.LOAD, src=(_nop(name="buf"), NOp(UOps.VECTORIZE, dtypes.int.vec(3), (_nop(name="idx"), _nop(name="idy"),
     _nop(name="id4")))), name="ls"), image_contract_load),
  (NOp(UOps.STORE, src=(_nop(name="buf"), NOp(UOps.VECTORIZE, dtypes.int.vec(3), (_nop(name="idx"), _nop(name="idy"),
     NOp(UOps.EXPAND, src=tuple(NOp.const(dtypes.int, i) for i in range(4)), name="ex"))), _nop(name="var")), name="ls"),
...
transcendental_folding = PatternMatcher([
  (_nop(UOps.ALU, dtype=TRANSCENDENTAL_SUPPORTED_DTYPES, src=(_nop(name="x"),), arg=UnaryOps.EXP2), xexp2),
  (_nop(UOps.ALU, dtype=TRANSCENDENTAL_SUPPORTED_DTYPES, src=(_nop(name="d"),), arg=UnaryOps.LOG2), xlog2),
  (_nop(UOps.ALU, dtype=TRANSCENDENTAL_SUPPORTED_DTYPES, src=(_nop(name="d"),), arg=UnaryOps.SIN), xsin),
])
...
constant_folder = PatternMatcher([
  ...
  (NOp(UOps.CONTRACT, name="con", src=(NOp(UOps.ALU, name="alu"),)),
  ...
  (NOp(UOps.CONTRACT, name="con", src=(_nop(UOps.REDUCE, dtype={dtypes.half, dtypes.bfloat16, dtypes.float}, name="red"),)),
   lambda con, red: UOp(UOps.REDUCE, con.dtype, (UOp(UOps.CONTRACT, con.dtype, red.src[0:1], con.arg),)+red.src[1:], red.arg)),
  (_nop(UOps.CONTRACT, name="con", src=(_nop(UOps.CAST, dtype={dtypes.half, dtypes.bfloat16, dtypes.float}, src=(_nop(name="casted"),)),)),
  ...
  (_nop({UOps.CONST, UOps.ALU, UOps.SPECIAL, UOps.RANGE, UOps.EXPAND}, dtype=dtypes.bigint, name="x"),
  ...
  *[(NOp(UOps.VECTORIZE, dtypes.float.vec(i), tuple(NOp(UOps.GEP, dtypes.float,
                         src=(_nop(name='x', dtype=dtypes.float.vec(i)),), arg=j) for j in range(i))), lambda x: x) for i in [2, 4, 8, 16]],
  *[(NOp(UOps.VECTORIZE, dtypes.half.vec(i), tuple(NOp(UOps.GEP, dtypes.half,
                         src=(_nop(name='x', dtype=dtypes.half.vec(i)),), arg=j) for j in range(i))), lambda x: x) for i in [2, 4, 8, 16]],
  # tensor core with a 0 input is acc
  (NOp(UOps.WMMA, src=(NOp.const(None, 0.0), _nop(), _nop(name="acc"))), lambda acc: acc),
  ...
  *[(NOp(UOps.REDUCE, src=(NOp(UOps.EXPAND, src=tuple(NOp(UOps.GEP, dtypes.float, src=(_nop(name="x"),), arg=i) for i in range(j)), name="expand"),)
    ,name="reduce"), reduce_before_expand) for j in [2,4,8]],
  (_nop(name="add") + NOp(UOps.WMMA, name="wmma"),
    lambda add, wmma: UOp(wmma.op, wmma.dtype, (wmma.src[0], wmma.src[1], wmma.src[2]+add), wmma.arg)),
  # threefry
  (NOp(UOps.ALU, dtype=dtypes.uint64, src=(_nop(name="x"), _nop(name="seed")), arg=BinaryOps.THREEFRY), threefry2x32),
  ...
  (NOp(UOps.REDUCE, src=((_nop(name="idx") + NOp.cvar("mval") * NOp(UOps.RANGE, src=(_nop(name="loop_start"), _nop(name="loop_end")), name="rng") +
    ...
    .lt(NOp.cvar("compval")).where(NOp.cvar("multconst"), NOp.const(None, 0)),), arg=ReduceOps.SUM, name="reduce"), loop_collapse),
  (NOp(UOps.REDUCE, src=((_nop(name="idx") + NOp.cvar("mval") * NOp(UOps.RANGE, src=(_nop(name="loop_start"), _nop(name="loop_end")), name="rng") +
                          _nop(name="idx2"))
    .lt(NOp.cvar("compval")).where(NOp.cvar("multconst"), NOp.const(None, 0)),), arg=ReduceOps.SUM, name="reduce"), loop_collapse),
  ...
  (NOp(UOps.REDUCE, src=((_nop(name="idx") - NOp(UOps.RANGE, src=(_nop(name="loop_start"), _nop(name="loop_end")), name="rng"))
    .lt(NOp.cvar("compval")).where(NOp.cvar("multconst"), NOp.const(None, 0)),), arg=ReduceOps.SUM, name="reduce"),
  ...
  (NOp(UOps.REDUCE, src=(_nop(name="idx").eq(NOp(UOps.RANGE, name="rng")).cast()*
    ...
    arg=ReduceOps.SUM, name="reduce"), index_collapse),
  ...
  # max folding
  (NOp.max(_nop(name="x"), _nop(name="y")), lambda x,y: x if x.vmin.arg >= y.vmax.arg else y if x.vmax.arg <= y.vmin.arg else None),
  ...
  (NOp(UOps.PHI, src=(NOp(UOps.DEFINE_ACC, name="acc"), _nop(name="acc"))), lambda acc: UOp.cast(acc.src[0], acc.dtype)),
  (NOp(UOps.PHI, src=(NOp(UOps.DEFINE_ACC, src=(NOp.cvar(),)), _nop(name="x"))), lambda x: x),
  (NOp(UOps.PHI, src=(NOp.cvar(), _nop(name="x"))), lambda x: x),
  ...
  (_nop().where(_nop(name="val"), _nop(name="val")), lambda val: val),
  (NOp.cvar('gate').where(_nop(name="c0"), _nop(name="c1")), lambda gate, c0, c1: c0 if gate.arg else c1),
  # ** constant folding **
  (UPat(UOps.ALU, name="root", src=UPat(UOps.CONST)), lambda root: root.const(exec_alu(root.arg, root.dtype, [x.arg for x in root.src]))),
  # ** self folding **
  (-(-_nop(name="x")), lambda x: x),    # -(-x) -> x
  (_nop(name="x") + 0, lambda x: x),    # x+0 -> x
  (_nop(name="x") * 1, lambda x: x),    # x*1 -> x
  (_nop(name="x") * -1, lambda x: -x),  # x*-1 -> -x
  (_nop(name="x") // _nop(name="x"), lambda x: x.const(1)), # x//x -> 1
  (_nop(name="x") // 1, lambda x: x),   # x//1 -> x
  (_nop(name="x") // -1, lambda x: -x), # x//-1 -> -x
  (_nop(name="x") / _nop(name="x"), lambda x: x.const(1)), # x/x -> 1
  (_nop(name="x") / NOp.cvar('c'), lambda x,c: x*exec_alu(UnaryOps.RECIP, c.dtype, [c.arg])),    # x/c -> x*(1/c)
  (_nop(name='x', dtype=dtypes.bool).max(NOp.const(dtypes.bool, False)), lambda x: x),  # max(x, False) -> x
  # ** zero folding **
  #x*0 -> 0 or 0*x -> 0
  #if x is nan or inf it should render the nan value.
  # NOTE: this can be wrong for loaded NaN
  (_nop(name="x") * 0, lambda x: x.const(float('nan') if isinstance(x.arg, float) and (math.isnan(x.arg) or math.isinf(x.arg)) else 0)),
  (_nop(name="x") - _nop(name="x"), lambda x: x.const(0)),   # x-x -> 0
  # lt folding
  (_nop(name="x").lt(_nop(name="y")),
  ...
  ((NOp.cvar('c0',dtypes.int)*_nop(name="x")).lt(NOp.cvar('c1',dtypes.int)),
   lambda x,c0,c1: x.lt(math.ceil(c1.arg/c0.arg)) if c0.arg > 0 and c1.arg > 0 else None),
  # mul -> add -> lt
  (((NOp.cvar('c0')*_nop(name="x"))+_nop(name="x2")).lt(NOp.cvar('c1')),
   lambda x,x2,c0,c1: x.lt(c1.arg//c0.arg) if c1.arg % c0.arg == 0 and c0.arg > x2.vmax.arg and x2.vmin.arg >= 0 else None),
  # ** load/store folding **
  (NOp.store(_nop(name="buf"), _nop(name="idx"), NOp.load(_nop(name="buf"), _nop(name="idx"))), lambda buf,idx:UOp(UOps.NOOP)),
  # ** two stage add/sub folding **
  ((_nop(name="x") + NOp.cvar('c1')) + NOp.cvar('c2'), lambda x,c1,c2: x+x.const(exec_alu(BinaryOps.ADD, x.dtype, [c1.arg, c2.arg]))),
  ((_nop(name="x") - NOp.cvar('c1')) + NOp.cvar('c2'), lambda x,c1,c2: x+x.const(exec_alu(BinaryOps.ADD, x.dtype, [c2.arg, -c1.arg]))),
  # *** rules from symbolic ***
  # div folding
  (_nop(name="x") // NOp.cvar('c'), lambda x,c: x.const(x.vmin.arg//c.arg) if c.arg > 0 and x.vmin.arg//c.arg == x.vmax.arg//c.arg else None),
  (_nop(name="x") // NOp.cvar('c'), lambda x,c: d if c.arg > 0 and (d:=x.divides(c.arg)) is not None else None),
  # mod folding
  (_nop(name="x") % NOp.cvar('c'), lambda x,c: x if 0 <= x.vmin.arg <= x.vmax.arg < c.arg else None),
  # mod reduction
  (_nop(name="x") % NOp.cvar('c'), lambda x,c: (x-(x.vmin.arg//c.arg)*c.arg)%c if 0 < c.arg <= x.vmin.arg else None),
  # mul -> mod
  ((NOp.cvar('c0')*_nop(name="x")) % NOp.cvar('c1'), lambda x,c0,c1: x*(c0.arg%c1.arg)%c1 if c0.arg >= c1.arg > 0 else None),
  # mul -> add -> mod
  (((NOp.cvar('c0')*_nop(name="x"))+_nop(name="x2")) % NOp.cvar('c1'),
   lambda x,x2,c0,c1: x2%c1 if (r:=c0.arg%c1.arg) == 0 else (x*r+x2)%c1 if c0.arg >= c1.arg > 0 else None),
  # mul -> add -> div
  (((NOp.cvar('c0')*_nop(name="x"))+_nop(name="x2")) // NOp.cvar('c1'), lambda x,x2,c0,c1:\
  ...
  # mod mod
  ((_nop(name="x") % NOp.cvar('c0')) % NOp.cvar('c1'), lambda x,c0,c1: x % c0 if 0 < c0.arg < c1.arg else x % c1 if c0.arg % c1.arg == 0 else None),
  (((_nop(name='x') * NOp.cvar('c0')) % NOp.cvar('c1')) % NOp.cvar('c0'), lambda x,c0,c1: x.const(0)),
  # two stage mul, (x*c1)*c2 = x*(c1*c2)
  ((_nop(name="x") * NOp.cvar("c1")) * NOp.cvar("c2"), lambda x,c1,c2: x*x.const(exec_alu(BinaryOps.MUL, x.dtype, [c1.arg, c2.arg]))),
  # -(x+y) -> -x + -y
  #(-(_nop(name="x") + _nop(name="y")), lambda x,y: (-x)+(-y)),
  # x%1 -> 0
  (_nop(name="x") % NOp.const(None, 1), lambda x: x.const(0)),
  # (x*c0)+(x*c1) -> x*(c0+c1)
  (_nop(name="x") * NOp.cvar("c0") + _nop(name="x") * NOp.cvar("c1"), lambda x,c0,c1: x*exec_alu(BinaryOps.ADD, x.dtype, [c0.arg, c1.arg])),
  # (x*c0)+(y*c0) -> (x+y)*c0
  #((_nop(name="x") * NOp.cvar("c0")) + (_nop(name="y") * NOp.cvar("c0")), lambda x,y,c0: c0*(x+y)),
  # mul div
  ((_nop(name="x") * NOp.cvar("c0")) // NOp.cvar("c1"),
   lambda x,c0,c1: x*(c0.arg//gcd)//(c1.arg//gcd) if c1.arg!=0 and (gcd:=math.gcd(c0.arg,c1.arg))> 1 else None),
  # (x*x2)/x2 -> x
  ((_nop(name="x") * _nop(name="x2")) / _nop(name="x2"), lambda x,x2: x),
  # (x//c0)//c1 -> x//(c0*c1)
  ((_nop(name="x") // NOp.cvar("c0")) // NOp.cvar("c1"), lambda x,c0,c1: x//x.const(exec_alu(BinaryOps.MUL, x.dtype, [c0.arg, c1.arg]))),
  # (x/x1)/x2 -> x/(x1*x2)
  ((_nop(name="x") / _nop(name="x2")) / _nop(name="x3"), lambda x,x2,x3: x/(x2*x3)),
  # c0 + x < c1 -> x < c1 - c0
  ((NOp.cvar("c0") + _nop(name="x")).lt(NOp.cvar("c1")), lambda x,c0,c1: UOp.lt(x, x.const(exec_alu(BinaryOps.ADD, x.dtype, [c1.arg, -c0.arg])))),
  # (x+x*c0)-> x*(c0+1)
  (_nop(name="x") + _nop(name="x") * NOp.cvar("c0"), lambda x,c0: x*(c0.arg+1)),
  # x!=0 -> (bool)x
  (_nop(name="x").ne(0), lambda x: x.cast(dtypes.bool)),
  # bool != 1 -> not bool
  (_nop(name="x", dtype=dtypes.bool).ne(1), lambda x: -x),
  # TODO: can do the invert of this (flip alt/load) when we fix double ops
  (NOp.store(_nop(name="buf"), _nop(name="idx"), _nop(name="gate").where(_nop(name="alt"), NOp.load(_nop(name="buf"), _nop(name="idx")))),
   lambda buf, idx, gate, alt: UOp.store(buf, idx, alt, gate)),
  # VECTORIZE-PHI-GEP -> PHI-VECTORIZE
  (NOp(UOps.VECTORIZE, src=tuple(NOp(UOps.PHI, src=(NOp(UOps.GEP, src=(_nop(name="val"),), arg=i), _nop(name=f"v{i}"))) for i in range(4)),
    name="root"), lambda root, val, v0, v1, v2, v3: UOp(UOps.PHI, root.dtype, (val, UOp(UOps.VECTORIZE, val.dtype, (v0, v1, v2, v3))))),
  (NOp(UOps.VECTORIZE, src=tuple(NOp(UOps.PHI, src=(NOp(UOps.GEP, src=(_nop(name="val"),), arg=i), _nop(name=f"v{i}"))) for i in range(2)),
    name="root"), lambda root, val, v0, v1: UOp(UOps.PHI, root.dtype, (val, UOp(UOps.VECTORIZE, val.dtype, (v0, v1))))),
  # NEG/CMPLT -> CMPLT
  (NOp.lt(-_nop(name='x'), NOp.cvar('c', dtypes.int)), lambda c,x: UOp.lt(c.const(-c.arg), x)),
  ...
  (NOp.load(_nop(name="buf"), _nop(name="idx"), NOp.const(dtypes.bool, True), NOp.cvar("var")),
   lambda buf,idx,var: UOp.load(buf, idx, dtype=var.dtype)),
  (NOp.load(_nop(name="buf"), _nop(name="idx"), NOp.const(dtypes.bool, True), NOp.cvar("var"), _nop(name="barrier")),
   lambda buf,idx,var,barrier: UOp.load(buf, idx, barrier, dtype=var.dtype)),
  (NOp.load(_nop(), _nop(), NOp.const(dtypes.bool, False), NOp.cvar("var")), lambda var: var),
  (NOp.load(_nop(), _nop(), NOp.const(dtypes.bool, False), NOp.cvar("var"), _nop()), lambda var: var),
  (NOp.store(_nop(name="buf"), _nop(name="idx"), _nop(name="val"), NOp.const(dtypes.bool, True)), NOp.store),
  (NOp.store(_nop(), _nop(), _nop(), NOp.const(dtypes.bool, False)), lambda: UOp(UOps.NOOP)),
...
expander = PatternMatcher([
  ...
  (NOp(UOps.EXPAND, src=(_nop(name="x"),), arg=()), lambda x: x),
