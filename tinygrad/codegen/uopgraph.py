...
from tinygrad.helpers import DEBUG, getenv, flatten, all_same, dedup, TRANSCENDENTAL, pretty_print
...
class UPat:
  ...
  def __repr__(self):
    def rep(x): return f"UPat(({x.op if isinstance(x.op, UOps) else ', '.join(str(x) for x in x.op)}), {x.arg}, name={x.name}, dtype={x.dtype}, src=("
