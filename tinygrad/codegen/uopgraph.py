  ...
  for uop,v in bounds.items():
    ...
    for candidate in candidates:
      if idx.op is UOps.VECTORIZE:
        ...
        buf_newidxs = []
        for X,newX in candidate:
          buf_newidxs.append(replace_uop(graph_rewrite(replace_uop(idx, X, newX), constant_folder), newX, X))
        if len(buf_newidxs)==1 or (len(buf_newidxs) > 1 and all_same([i.key for i in buf_newidxs])): idx = buf_newidxs[0]
