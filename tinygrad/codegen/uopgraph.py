...
float4_folding = PatternMatcher([
  ...
  (NOp(UOps.STORE, src=(NOp.var("buf"), NOp.var("idx")+
    (NOp(UOps.EXPAND, src=tuple(NOp.const(dtypes.int, i) for i in range(4)), vnm="ex")+NOp.var("idx2")), NOp.var("var")), vnm="store"),
    lambda buf, store, idx, idx2, ex, var: UOp(UOps.STORE, store.dtype, (buf, idx+idx2+ex, var), store.arg)),
  # float(2,4) load
  (NOp(UOps.LOAD, src=(NOp.var("buf"), NOp(UOps.EXPAND, vnm="ex")+NOp.var("idx")+NOp.var("idx2")+NOp.var("idx3")), vnm="load"), float4_expand_load),
  (NOp(UOps.LOAD, src=(NOp.var("buf"), NOp(UOps.EXPAND, vnm="ex")+NOp.var("idx")+NOp.var("idx2")), vnm="load"), float4_expand_load),
  (NOp(UOps.LOAD, src=(NOp.var("buf"), NOp(UOps.EXPAND, vnm="ex")+NOp.var("idx")), vnm="load"), float4_expand_load),
  (NOp(UOps.LOAD, src=(NOp.var("buf"), NOp(UOps.EXPAND, vnm="ex")), vnm="load"), float4_expand_load),
  # float(2,4) store
  # TODO: fold ADDs into one UOp and remove add chains
  (NOp(UOps.STORE, src=(NOp.var("buf"),
    NOp(UOps.EXPAND, vnm="ex")+NOp.var("idx")+NOp.var("idx2")+NOp.var("idx3"), NOp.var("var")), vnm="store_allow_any_len"),
    float4_contract_store),
  (NOp(UOps.STORE, src=(NOp.var("buf"),
    NOp(UOps.EXPAND, vnm="ex")+NOp.var("idx")+NOp.var("idx2"), NOp.var("var")), vnm="store_allow_any_len"),
    float4_contract_store),
  (NOp(UOps.STORE, src=(NOp.var("buf"),
    NOp(UOps.EXPAND, vnm="ex")+NOp.var("idx"), NOp.var("var")), vnm="store_allow_any_len"), float4_contract_store),
  (NOp(UOps.STORE, src=(NOp.var("buf"),
    NOp(UOps.EXPAND, vnm="ex"), NOp.var("var")), vnm="store_allow_any_len"), float4_contract_store),
  # image handling
  (NOp(UOps.LOAD, src=(NOp.var("buf"), NOp(UOps.VECTORIZE, dtypes.int.vec(3), (NOp.var('idx'), NOp.var('idy'),
     NOp.var('id4')))), vnm="ls_allow_any_len"), image_contract_load),
  (NOp(UOps.STORE, src=(NOp.var("buf"), NOp(UOps.VECTORIZE, dtypes.int.vec(3), (NOp.var('idx'), NOp.var('idy'),
     NOp(UOps.EXPAND, src=tuple(NOp.const(dtypes.int, i) for i in range(4)), vnm="ex"))), NOp.var("var")), vnm="ls_allow_any_len"),
...
constant_folder = PatternMatcher([
  ...
  # VECTORIZE/GEP
  (NOp(UOps.GEP, src=(NOp(UOps.VECTORIZE, vnm="cast"),), vnm="gep"), lambda gep, cast: cast.src[gep.arg]),
  ...
  *[(NOp(UOps.REDUCE, src=(NOp(UOps.EXPAND, src=tuple(NOp(UOps.GEP, dtypes.float, src=(NOp.var('x'),), arg=i) for i in range(j)), vnm="expand"),)
    ,vnm="reduce_allow_any_len"), reduce_before_expand) for j in [2,4,8]],
  (NOp.var("add") + NOp(UOps.WMMA, vnm="wmma"),
  ...
  (NOp(UOps.REDUCE, src=((NOp.var("idx") + NOp.cvar("mval") * NOp(UOps.RANGE, src=(NOp.var("loop_start"), NOp.var("loop_end")), vnm="rng") +
                          NOp.var("idx2") + NOp.var("idx3"))
    .lt(NOp.cvar("compval")).where(NOp.cvar("multconst"), NOp.const(None, 0)),), arg=ReduceOps.SUM, vnm="reduce_allow_any_len"), loop_collapse),
  (NOp(UOps.REDUCE, src=((NOp.var("idx") + NOp.cvar("mval") * NOp(UOps.RANGE, src=(NOp.var("loop_start"), NOp.var("loop_end")), vnm="rng") +
                          NOp.var("idx2"))
    .lt(NOp.cvar("compval")).where(NOp.cvar("multconst"), NOp.const(None, 0)),), arg=ReduceOps.SUM, vnm="reduce_allow_any_len"), loop_collapse),
  ...
  (NOp(UOps.REDUCE, src=((NOp.var("idx") + NOp.cvar("mval") * NOp(UOps.RANGE, src=(NOp.var("loop_start"), NOp.var("loop_end")), vnm="rng"))
    .lt(NOp.cvar("compval")).where(NOp.cvar("multconst"), NOp.const(None, 0)),), arg=ReduceOps.SUM, vnm="reduce_allow_any_len"), loop_collapse),
  (NOp(UOps.REDUCE, src=((NOp.var("idx") - NOp(UOps.RANGE, src=(NOp.var("loop_start"), NOp.var("loop_end")), vnm="rng"))
    .lt(NOp.cvar("compval")).where(NOp.cvar("multconst"), NOp.const(None, 0)),), arg=ReduceOps.SUM, vnm="reduce_allow_any_len"),
    lambda **kwargs: loop_collapse(mval=UOp.const(dtypes.int, -1), **kwargs)),
  # indexing (with a multiply offset)!
  (NOp(UOps.REDUCE, src=(NOp.var('idx').eq(NOp(UOps.RANGE, vnm="rng")).cast()*
    NOp(UOps.LOAD, src=(NOp.var("buf"), NOp.var('add')+NOp.var('mul')*NOp(UOps.RANGE, vnm="rng")), vnm="ld"),),
    arg=ReduceOps.SUM, vnm="reduce_allow_any_len"), index_collapse),
  (NOp(UOps.REDUCE, src=(NOp.var('idx').eq(NOp(UOps.RANGE, vnm="rng")).where(
    NOp(UOps.LOAD, src=(NOp.var("buf"), NOp.var('add')+NOp.var('mul')*NOp(UOps.RANGE, vnm="rng")), vnm="ld"), NOp.const(None, 0.0)),),
    arg=ReduceOps.SUM, vnm="reduce_allow_any_len"), index_collapse),
  ...
  (NOp.max(NOp.cvar('c'), NOp(UOps.SPECIAL, vnm="s")+NOp.cvar('c2')), lambda c,s,c2: (s+c2) if 0 >= c.arg else None),  # TODO: generic
  (NOp.max(NOp.cvar('c'), -(NOp(UOps.SPECIAL, vnm="s")+NOp.cvar('c2'))), lambda c,s,c2: -(s+c2) if -(s.arg[1]-1+c2.arg) >= c.arg else None),
  # max on range can go away (ugh: copy of SPECIAL, and with/without const)
  (NOp.max(NOp.cvar('c'), NOp(UOps.RANGE, vnm="s")+NOp.cvar('c2')), lambda c,s,c2: (s+c2) if s.src[0].arg >= c.arg else None),  # TODO: generic
  (NOp.max(NOp.cvar('c'), -(NOp(UOps.RANGE, vnm="s")+NOp.cvar('c2'))), lambda c,s,c2: -(s+c2) if -(s.src[1].arg-1+c2.arg) >= c.arg else None),
  # const rules
  (NOp(UOps.GEP, src=(NOp.cvar("c"),), vnm="root"), lambda root, c: root.const(c.arg)),
  ...
  (NOp(UOps.PHI, src=(NOp(UOps.DEFINE_ACC, vnm="acc"), NOp.var("acc"))), lambda acc: UOp.cast(acc.src[0], acc.dtype)),
  ...
  (NOp(UOps.DEFINE_ACC, src=(NOp.cvar(),), vnm="root"), lambda root: UOp.cast(root.src[0], root.dtype)),
  (NOp(UOps.GEP, src=(NOp.cvar("x"),), vnm="root"), lambda root,x: root.const(x.arg)),
  ...
  (NOp(UOps.VECTORIZE, src=tuple(NOp(UOps.PHI, src=(NOp(UOps.GEP, src=(NOp.var("val"),), arg=i), NOp.var(f"v{i}"))) for i in range(4)), vnm="root"),
   lambda root, val, v0, v1, v2, v3: UOp(UOps.PHI, root.dtype, (val, UOp(UOps.VECTORIZE, val.dtype, (v0, v1, v2, v3))))),
  (NOp(UOps.VECTORIZE, src=tuple(NOp(UOps.PHI, src=(NOp(UOps.GEP, src=(NOp.var("val"),), arg=i), NOp.var(f"v{i}"))) for i in range(2)), vnm="root"),
  ...
  (NOp(UOps.CAST, vnm="root"), lambda root: root.src[0] if str(root.dtype) == str(root.src[0].dtype) else None),
  (NOp(UOps.VECTORIZE, vnm="root"), lambda root: root.src[0] if str(root.dtype) == str(root.src[0].dtype) else None),
...
expander = PatternMatcher([
  ...
  (NOp(UOps.REDUCE, vnm="root"), do_reduce_with_expand),
  (NOp(UOps.CONTRACT, vnm="con"), do_contract),
  ...
  (NOp(UOps.BARRIER, src=(NOp(UOps.EXPAND, vnm="ex"),)), lambda ex: UOp(UOps.EXPAND, None, (UOp(UOps.BARRIER, None, ex.src),)*len(ex.src), ex.arg)),
