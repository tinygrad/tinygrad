...
from tinygrad.ops import UPat, PatternMatcher, graph_rewrite, TernaryOps, sym, _get_chain, is_irreducible
...
def full_graph_rewrite(sink:UOp, opts:Optional[Renderer]=None) -> UOp:
  ...
  if linearize_cnt != (de:=getenv("DEBUG_EXPAND", 0)) and de != -1:
    sink = graph_rewrite(sink, bigsym+expander)
    if getenv("DO_REDUCE", 1):
      sink = graph_rewrite(sink, bigsym+just_reduce)
      sink = graph_rewrite(sink, bigsym+(devectorize+float4_folding if opts is not None and opts.supports_float4 else devectorize))
      sink = graph_rewrite(sink, bigsym+reducer)
      sink = graph_rewrite(sink, bigsym+get_extra_patterns(tuple(opts.code_for_op.keys()) if opts is not None else (), TRANSCENDENTAL>=2))
