...
def fold_expanded(ex, buf):
  if buf.dtype != PtrDType(dtypes.float) and buf.dtype != PtrDType(dtypes.half) and buf.dtype != PtrDType(dtypes.f8e5m2) and buf.dtype != PtrDType(dtypes.f8e4m3) and not isinstance(buf.dtype, ImageDType): return None
