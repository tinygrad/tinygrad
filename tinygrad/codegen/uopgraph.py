...
constant_folder = PatternMatcher([
  ...
  *[(UOp(UOps.REDUCE, src=(UOp(UOps.EXPAND, src=tuple(UOp(UOps.GEP, dtypes.float, src=(UOp.var('x'),), arg=i) for i in range(j))).name("expand"),))
  ...
  # (x+c0)*c1 -> x*c1+(c0*c1)  # NOTE: not applicable to float due to potential cast
  ((UOp.var("x") + UOp.cvar("c0")) * UOp.cvar("c1"),
   lambda x,c0,c1: x*c1+exec_alu(BinaryOps.MUL, x.dtype, [c0.arg, c1.arg]) if dtypes.is_int(x.dtype) else None),
  # (x*c0)+(y*c0) -> (x+y)*c0
