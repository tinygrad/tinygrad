...
def lower_load_store(ctx: IndexContext, x: UOp):
  ...
  if x.op is UOps.LOAD:
    ...
    return UOp(UOps.LOAD, x.dtype, (buf.index(idx, valid if has_valid else None),) + barrier)
  ...
  if x.src[0].op is UOps.DEFINE_GLOBAL or store_back:
    ...
    has_valid = valid.op is not UOps.CONST or valid.arg is not True
  return UOp(UOps.STORE, dtypes.void, (buf.index(idx, valid if has_valid else None), x.src[2]))
