...
def get_children_dfs(u:UOp, children:Dict[UOp, List[UOp]], srcs:Dict[UOp, Dict[UOp, None]], in_degree:Dict[UOp, int]):
  ...
  for x in u.src:
    ...
    if x.op is UOps.RANGE and x.arg % 2: srcs[u][x] = None
    children[x].append(u)
...
def linearize_uop(sink:UOp, skip_check:bool=not __debug__) -> List[UOp]:
  ...
  def get_priority(u:UOp):
    ...
    if u.op is UOps.RANGE and u.arg % 2:
      priority += (u.arg // 2)
      ...
      priority -= sum([(l.arg // 2)+1 + 1000*(l.arg % 2) for l,ss in scope_children.items() if l.op is UOps.RANGE and u in ss])
