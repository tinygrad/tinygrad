...
class BasicBlock:
    ...
    def __lt__(self, o: BasicBlock):
        return tuple(x.tuplize for x in self.ctx + self.lst) < tuple(
            x.tuplize for x in o.ctx + o.lst
        )
...
def append_to_block(
    ctx: tuple[dict[UOp, tuple[UOp, ...]], dict[UOp, list[UOp]]], x: UOp
):
    ...
    for u in x.src:
        ...
        elif u.op not in DONT_PLACE_IN_BLOCK and set(children[u]).issubset(
            in_this_block
        ):
    ...
    for rng, lst in new_blocks.items():
        ...
        new_block = UOp(
            Ops.BLOCK, dtypes.void, tuple(dedup(srcs)), BasicBlock(rng, tuple(lst))
        )
        lrng = list(rng)
        for r in rng[::-1]:
            if r not in x.arg.ctx and r.op is not Ops.BLOCKSTART:
                ...
                new_block = UOp(
                    ...
                    arg=BasicBlock(
                        tuple(lrng),
                        (UOp(Ops.ENDIF if r.op is Ops.IF else Ops.ENDRANGE, src=(r,)),),
                        r,
                    ),
                )
    ...
    return UOp(
        Ops.BLOCK,
        dtypes.void,
        tuple(dedup(list(old_blocks.values()) + new_srcs)),
        BasicBlock(x.arg.ctx, tuple(to_append) + x.arg.lst),
    )
...
def block_merge(ctx, x: UOp):
    ...
    if x.op is Ops.BLOCKEND:
        ...
        if len([y for y in ctx[x.arg.end] if y not in in_this_block]) == 0:
            ...
            parent_blocks = [
                y
                for y in x.src
                if y.op is Ops.BLOCK
                and UOp(Ops.BLOCKSTART, src=(x.arg.end,)) in y.arg.ctx
            ]
            assert len(parent_blocks) <= 1, "should never have two parent blocks"
            if len(parent_blocks) == 1:
                parent_block = parent_blocks[0]
                # range needs DEFINE_ACC to be before the range (never in DEFINE_ACC for if)
                early_ops, late_ops = partition(
                    x.arg.lst, lambda y: y.op is Ops.DEFINE_ACC and x.arg.end in y.src
                )
                return UOp(
                    Ops.BLOCK,
                    dtypes.void,
                    ...
                    BasicBlock(
                        tuple(y for y in x.arg.ctx if y is not x.arg.end),
    ...
    for u in x.src:
        if u.op is Ops.BLOCK and (
            tuple(u.arg.ctx) == tuple(x.arg.ctx)
            or (x.arg.end is not None and x.arg.end in u.arg.ctx)
        ):
            ...
            u.op is Ops.BLOCKFORK and x.src.count(u) == u.arg
    ...
    return UOp(
        ...
        tuple(new_srcs),
        BasicBlock(
            tuple(sorted(new_ctx, key=lambda x: x.tuplize)),
...
def block_reorder(in_block: UOp):
    ...
    local_children: collections.defaultdict[UOp, list[UOp]] = collections.defaultdict(
        list
    )
    ...
    for u in reversed(in_block.arg.lst):
        ...
        priorities[u] = min(
            [-1000 if u.op is Ops.LOAD else 0]
            + [priorities[x] for x in local_children[u]]
        )
    ...
    assert len(newlst) == len(
        in_block.arg.lst
    ), f"len mismatch {len(newlst)} != {len(in_block.arg.lst)}"
...
def linearize_uop(sink: UOp, skip_check: bool = not __debug__) -> list[UOp]:
        ...
        for s in u.src:
            ...
            # don't flow (fully) through assign and store
                ...
                if isinstance(s.src[0].dtype, PtrDType) and s.src[0].dtype.local:
                    idx_context, store_context = (
                        temp_block_ctxs[s.src[0]],
                        temp_block_ctxs[s],
                    )
                    this_block_ctx += [
                        x
                        for x in store_context
                        if x not in idx_context and x.op is Ops.RANGE
                    ]
            elif s.op is Ops.ASSIGN:
                # flow though assign, but remove the ranges used in the assign
                assert s.src[0].op is Ops.DEFINE_ACC
                this_block_ctx += [
                    x for x in temp_block_ctxs[s.src[1]] if x not in s.src[0].src[1:]
                ]
            else:
    ...
    for u in sink.toposort:
        block_ctxs[u] = (
            ((UOp(Ops.BLOCKSTART, src=(u,)),) + tuple(temp_block_ctxs[u]))
            if u.op in {Ops.IF, Ops.RANGE}
            else tuple(temp_block_ctxs[u])
        )
    ...
    while 1:
        ...
        # add BLOCKFORK (slow!)
        block_parent_count = collections.Counter(
            flatten([x.src for x in sink.toposort if x.op is Ops.BLOCK])
        )
        non_block_parents = set(
            flatten([x.src for x in sink.toposort if x.op is not Ops.BLOCK])
        )
        forks = {
            u: UOp(
                Ops.BLOCKFORK,
                src=(UOp(Ops.BLOCK, src=u.src, arg=BasicBlock(block_ctxs[u], (u,))),),
                arg=child_count,
            )
            for u, child_count in block_parent_count.items()
            if u.op not in DONT_PLACE_IN_BLOCK
            and child_count > 1
            and u not in non_block_parents
        }
        ...
        if len(v) > 1:
            out = UOp(
                ...
                src=(
                    UOp(
                        Ops.BLOCKEND,
                        src=tuple(flatten(x.src for x in v)),
                        arg=BasicBlock(
                            tuple(dedup(flatten([y.arg.ctx for y in v]))),
                            v[0].arg.lst,
                            k,
                        ),
                    ),
    ...
    sink = sink.substitute(
        {
            u: newu
            for u in sink.toposort
            if u.op is Ops.BLOCK and (newu := block_reorder(u)) is not u
        }
    )
    ...
    assert all(
        len(x.src) == 0
        and x.op not in {Ops.BLOCK, Ops.BLOCKSTART, Ops.BLOCKEND, Ops.BLOCKFORK}
        for x in _uops
    )
    _uops += sink.arg.lst
