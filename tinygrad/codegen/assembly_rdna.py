...
class RDNACodegen(AssemblyCodegen):
  ...
  def specialize(self, asm) -> Tuple[str, str]:
    args = []
    for i,b in enumerate(ASTRunner.dedup_kernel_inputs(self.dedup_bufs)): args.append({'.address_space': 'global', '.name': f'buf_{i}', '.offset': i * 8, '.size': 8, '.type_name': b.dtype.name + "*", '.value_kind': 'global_buffer'})
    ins = []
