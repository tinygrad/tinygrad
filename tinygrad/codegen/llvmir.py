...
render_llvm = {
  ...
  SumNode: lambda self,ops,ctx: reduce(lambda a,b: ctx.add(a,b.render(ops,ctx)), self.nodes[1:], self.nodes[0].render(ops,ctx)),
  AndNode: lambda self,ops,ctx: reduce(lambda a,b: ctx.and_(a,b.render(ops,ctx)), self.nodes[1:], self.nodes[0].render(ops,ctx))
}
...
def uops_to_llvm_ir(uops:List[UOp]) -> str:
  ...
  buf_to_dtype = {args[0]:args[1] for uop,_,_,args in uops if uop is UOps.DEFINE_GLOBAL}
  ...
  for uop,newvar,vin,args in uops:
    ...
    if uop is UOps.LOAD:
      ...
      assert newvar.dtype is dtypes.float, "newvar must be float"
      ...
      if isinstance(args, ConstOp):
        ...
        if args.memory_dtype != newvar.dtype:
          ...
          elif args.memory_dtype is dtypes.bfloat16:
