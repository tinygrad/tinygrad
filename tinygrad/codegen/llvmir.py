...
from tinygrad.ops import Op, ASTRunner, UnaryOps, BinaryOps, TrinaryOps
from tinygrad.lazy import LazyBuffer

from tinygrad.shape.symbolic import Variable, NumNode, MulNode, DivNode, ModNode, LtNode, SumNode, AndNode
def int_const(x): return ir.Constant(ir.IntType(64), x)
def fp_const(x): return ir.Constant(ir.FloatType(), x)
render_llvm = {
...
code_for_op: Final[Dict[Op, Callable]] = {
  ...
  TrinaryOps.MULACC: lambda builder,x,y,z: builder.fadd(builder.fmul(x,y, flags=('fast',)), z, flags=('fast',)),
  TrinaryOps.WHERE: lambda builder,x,y,z: builder.select(builder.fcmp_ordered("!=", x, fp_const(0), flags=('fast',)), y, z, flags=('fast',)),
}
