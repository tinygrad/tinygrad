...
def mantissa_bits(d: DType) -> int:
    return dtypes.finfo(d)[1]


def exponent_bias(d: DType) -> int:
    return {dtypes.float64: 1023, dtypes.float32: 127, dtypes.float16: 15}[d]


def exponent_mask(d: DType) -> int:
    return {dtypes.float64: 2047, dtypes.float32: 255, dtypes.float16: 31}[d]
...
def ldexp3k(d: UOp, e: UOp) -> UOp:
    ...
    assert (
        d.dtype in TRANSCENDENTAL_SUPPORTED_DTYPES
        and e.dtype in TRANSCENDENTAL_SUPPORTED_DTYPES
    )
    cast_map = {
        dtypes.float64: dtypes.int64,
        dtypes.float32: dtypes.int32,
...
def ldexp2k(d: UOp, e: UOp) -> UOp:
    ...
    assert d.dtype in TRANSCENDENTAL_SUPPORTED_DTYPES and e.dtype in (
        dtypes.int16,
        dtypes.int32,
        dtypes.int64,
    )
...
def cody_waite_reduction(d: UOp) -> tuple[UOp, UOp]:
    ...
    quadrant = (
        rintk(d * m_1_pi - qdh) if d.dtype == dtypes.float64 else rintk(d * m_1_pi)
    )
...
def trig_poly(d: UOp, coeff32, coeff64):
    return d * (
        polyN(d * d, coeff64) if d.dtype == dtypes.float64 else polyN(d * d, coeff32)
    )
