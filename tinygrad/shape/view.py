...
def canonicalize_strides(
    shape: tuple[sint, ...], strides: tuple[sint, ...]
) -> tuple[sint, ...]:
...
def merge_dims(
    ...
    for i, (s, st) in enumerate(zip(shape[1:], strides[1:]), start=1):
        ...
        if merging or last_st == s * st:
            ret[-1] = (last_s * s, st, (s if merging else last_pre_expand_s * s))
        else:
            ret.append((s, st, s))
        # merge this dim to next dim if size is 1
...
@functools.lru_cache(maxsize=None)
def _reshape_mask(
    _mask: Optional[tuple[tuple[sint, sint], ...]],
    old_shape: tuple[sint, ...],
    new_shape: tuple[sint, ...],
) -> Optional[tuple[tuple[sint, sint], ...]]:
    """Returns the new mask if reshape is possible, and None if not possible."""
    if _mask is None:
        return tuple((0, s) for s in new_shape)
    if not all_int(flatten(_mask)):
        return None

    new_mask: list[tuple[int, int]] = []
    # _mask is all int here
    r_masks, r_shape, r_new_shape = (
        reversed(cast(tuple[tuple[int, int], ...], _mask)),
        reversed(old_shape),
        reversed(new_shape),
    )
    curr_stride, old_dim, new_dim, mask = (
        1,
        next(r_shape, 1),
        next(r_new_shape, 1),
        next(r_masks, (0, 1)),
    ...
    while len(new_mask) < len(new_shape):
        ...
        if (
            ...
            old_dim > next_stride
        ):  # mask can only be splitted if reshape doesn't cut across the mask.
            if old_dim % next_stride != 0:
                return None
            if (l % next_stride != 0 or r % next_stride != 0) and l // next_stride != (
                r - 1
            ) // next_stride:
                return None
            new_mask.append(
            ...
            if mask != (0, old_dim) and l != r and next_mask[1] - next_mask[0] != 1:
                return None
            mask, old_dim = (
...
class View:
    ...
    def to_indexed_uops(
        self: View,
        idxs: Optional[Sequence[UOp]] = None,
        vexpr: UOp = UOp.const(dtypes.bool, True),
    ) -> tuple[UOp, UOp]:
        """(idx, valid)"""
        if idxs is None:
            idxs = [UOp.range(dtypes.int, 0, s, i) for i, s in enumerate(self.shape)]
        iexpr = sint_to_uop(self.offset)
        for idx, sh, st, m in zip(
            idxs,
            self.shape,
            self.strides,
            self.mask if self.mask is not None else itertools.repeat(None),
        ):
    ...
    def create(
        ...
        if not all(s >= 0 for s in shape):
            raise ValueError(f"Trying to create View with negative dimension: {shape=}")
        strides = (
            canonicalize_strides(shape, strides)
            if strides
            else strides_for_shape(shape)
        )
        # canonicalize 0 in shape
        if 0 in shape:
            return View(shape, (0,) * len(shape), offset=0, mask=None, contiguous=True)
        # canonicalize no-op mask
        if mask is not None and all(m == (0, s) for m, s in zip(mask, shape)):
            mask = None
        ...
        if mask and any(elim := [not resolve(b + 1 < e) for b, e in mask]):
            ...
            offset += sum(
                (strides[i] * mask[i][0]) if e else 0 for i, e in enumerate(elim)
            )
            strides = tuple(0 if e else st for st, e in zip(strides, elim))
        # simplify as we go
        if isinstance(offset, UOp):
            offset = cast(sint, offset.ssimplify())
        shape = tuple(
            cast(sint, x.ssimplify()) if isinstance(x, UOp) else x for x in shape
        )
    ...
    def unbind(self) -> tuple[View, dict[Variable, int]]:
        ...
        def substitute(x: sint):
            return x if isinstance(x, int) else x.substitute(unbound_vars)

        new_shape = tuple(map(substitute, self.shape))
        new_strides = tuple(map(substitute, self.strides))
        new_offset = substitute(self.offset)
        new_mask = (
            tuple((substitute(x[0]), substitute(x[1])) for x in self.mask)
            if self.mask is not None
            else None
        )
        return View.create(new_shape, new_strides, new_offset, new_mask), dict(
            x[1] for x in var_unboundvar_val
        )
    ...
    def __add__(self, vm1: View) -> Optional[View]:
        ...
        if vm2.contiguous:
            return vm1
        if vm1.contiguous and vm1.shape == vm2.shape:
            return vm2
        if (
            vm1.contiguous
            and vm1.size() == vm2.size()
            and (ret := vm2.reshape(vm1.shape)) is not None
        ):
            return ret
        if vm1.mask:
            if (new_vm1 := vm1.shrink(vm1.mask)) == vm1 or (
                merged := vm2 + new_vm1
            ) is None:
                return None
            return merged.pad(
                tuple((b, s - e) for (b, e), s in zip(vm1.mask, vm1.shape))
            )
        ...
        for d1, st in enumerate(vm1.strides):
            if st == 0:
                continue
            for d2, (o, s1) in enumerate(
                zip(origin, unravel(vm2.shape, vm1.offset + st))
            ):
                if (s1 := s1 - o) == 0:
                    continue
        ...
        idxs: list[UOp] = [
            UOp.variable(f"idx{i}", 0, s - 1) for i, s in enumerate(vm1.shape)
        ]
        ...
        for term, s, o in zip(reversed(terms), reversed(vm2.shape), reversed(origin)):
            ...
            merged_size *= s
            if resolve(merged_term < merged_size, False) and resolve(
                0 <= merged_term, False
            ):
                extents.append((merged_size, merged_term))
                merged_size, merged_term = 1, UOp.const(dtypes.int, 0)
        if resolve(merged_term != 0):
            return None
        if (vm2_shape := tuple(s for s, _ in reversed(extents))) != vm2.shape:
            if (reshaped_vm2 := vm2.reshape(vm2_shape)) is None:
                return None
            # NOTE: this != to prevent infinite loop
            if reshaped_vm2.shape != vm2.shape:
                return reshaped_vm2 + vm1

        if vm2.mask:
            # Try to project vm2's mask on to vm1.
            newb, newe, bad = [0] * len(vm1.shape), list(vm1.shape), False
            for (b, e), o, term, (_, t) in zip(
                vm2.mask, origin, terms, reversed(extents)
            ):
                if resolve(b <= t.vmin and t.vmax < e, False):
                    continue
                if len(term) != 1:
            ...
            if any((b, e) != (0, s) for b, e, s in zip(newb, newe, vm1.shape)):
                return vm2 + View.create(
                    vm1.shape, vm1.strides, vm1.offset, tuple(zip(newb, newe))
                )
        ...
        return View.create(
            vm1.shape,
            tuple(strides),
            sum(o * s for o, s in zip(origin, vm2.strides)) + vm2.offset,
        )
    ...
    def invert(self, out_shape: tuple[sint, ...]) -> Optional[View]:
        ...
        ret = ret.flip(tuple(x < 0 for x in self.strides)).permute(
            argsort(tuple(-x if x > 0 else x for x in self.strides))
        )
        return (
            ret if prod(ret.shape) == prod(out_shape) else None
        )  # don't support shrink, expand, or stride != (-1, 1)
    ...
    def __unsafe_resize(self, arg: tuple[tuple[sint, sint], ...], mask=None) -> View:
        ...
        if self.mask:
            ...
            nmask = tuple(
                [
                    (smax(0, smin(mx - ax, ay - ax)), smax(0, smin(my - ax, ay - ax)))
        ...
        return View.create(
            tuple([y - x for x, y in arg]), self.strides, self.offset + offset, mask
        )
    ...
    def pad(self, arg: tuple[tuple[sint, sint], ...]) -> View:
        ...
        for b, e in arg:
            assert (
                not all_int([b, e]) or b >= 0 and e >= 0
            ), f"invalid pad {arg} for {self.shape}"
    ...
    def shrink(self, arg: tuple[tuple[sint, sint], ...]) -> View:
        ...
        # NOTE: not checking for symbolic arg
        for s, (b, e) in zip(self.shape, arg):
            assert not all_int([s, b, e]) or (
                0 <= b <= e <= s
    ...
    def expand(self, new_shape: tuple[sint, ...]) -> View:
        ...
        assert all(
            resolve(s == ns) or s == 1 for s, ns in zip(self.shape, new_shape)
        ...
        mask = (
            tuple(
                [
                    (
                        ((0, 0) if m != (0, 1) else (0, ns))
                        if resolve(s != ns, False)
                        else m
                    )
    ...
    def permute(self, axis: tuple[int, ...]) -> View:
        assert sorted(axis) == list(
            range(len(self.shape))
        ), f"invalid permutation {axis} of len {len(self.shape)}"
        return View.create(
    ...
    def flip(self, arg: tuple[bool, ...]) -> View:
        ...
        mask = (
            tuple(
                (s - my, s - mx) if f else (mx, my)
                for (mx, my), s, f in zip(self.mask, self.shape, arg)
            )
            if self.mask is not None
            else None
        )
        return View.create(
            self.shape,
            tuple(-z if f else z for z, f in zip(self.strides, arg)),
            self.offset + offset,
            mask,
        )
    ...
    def reshape(self, new_shape: tuple[sint, ...]) -> Optional[View]:
        ...
        if self_all_int := all_int(self.shape):
            assert all(
                isinstance(s, (int, UOp)) for s in new_shape
        ...
        if self_all_int and not all_int(new_shape):
            if len(self.shape) != len(new_shape):
                raise ValueError(
                    f"cannot symbolic reshape non-contiguous {self} -> {new_shape}"
                )
            for si, so in zip(self.shape, new_shape):
                if not isinstance(so, int):
                    so = sym_infer(so, dict([v.unbind() for v in so.vars()]))
                if si != so:
                    raise ValueError(
                        f"cannot symbolic reshape non-contiguous {self} -> {new_shape}"
                    )
        ...
        for merged_size, new_stride, real_size in reversed(
            merge_dims(self.shape, self.strides, self.mask)
        ):
            # TODO: write with get_contraction
            acc = 1
            # TODO: third resolve shouldn't be needed
            while (
                resolve(acc <= merged_size)
                and resolve(acc != merged_size)
                and resolve((new_dim := next(r_new_shape, 0)) > 0)
            ):
                r_strides.append(new_stride * acc)
                acc = acc * new_dim
                if not resolve(acc < real_size):
                    new_stride = 0
            if resolve(acc != merged_size):
                return None
        ...
        if (new_mask := _reshape_mask(self.mask, self.shape, new_shape)) is not None:
            extra_offset = (
                sum(m[0] * s for m, s in zip(self.mask, self.strides))
                if self.mask
                else 0
            ) - (sum(m[0] * s for m, s in zip(new_mask, new_strides)))
            return View.create(
                new_shape, new_strides, self.offset + extra_offset, new_mask
            )
