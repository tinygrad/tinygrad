...
def get_contraction_with_reduce(old_shape: tuple[sint, ...], new_shape: tuple[sint, ...], reduce_axis: tuple[int, ...]) -> list[list[int]] | None:
  if (contraction := get_contraction(old_shape, new_shape)) is None:
    return None
  ...
  for i in range(len(contraction)):
    if i in reduce_axis and len(contraction[i]) == 0:
      ...
      while take_from < len(contraction) and len(contraction[take_from]) == 0:
        ...
        take_from += 1
      if take_from == len(contraction) or new_shape[take_from] != 1:
...
@functools.cache
def _reshape_mask(
  _mask: tuple[tuple[sint, sint], ...] | None, old_shape: tuple[sint, ...], new_shape: tuple[sint, ...]
...
class View:
  ...
  def to_indexed_uops(self: View, idxs: Sequence[UOp] | None = None, vexpr: UOp = UOp.const(dtypes.bool, True)) -> tuple[UOp, UOp]:
    """(idx, valid)"""
    if idxs is None:
      idxs = [UOp.range(s, i) for i, s in enumerate(self.shape)]
  ...
  def create(shape: tuple[sint, ...], strides: tuple[sint, ...] | None = None, offset: sint = 0, mask: tuple[tuple[sint, sint], ...] | None = None):
    ...
    if not all(resolve(s >= 0) for s in shape):
  ...
  def substitute(self, dvars: dict[UOp, UOp]):
    def _substitute(x: sint):
      return x if isinstance(x, int) else x.substitute(dvars)
  ...
  def __add__(self, vm1: View) -> View | None:
    vm2 = self
    if vm2.contiguous or vm1.size() == 0:
      return vm1
    if vm1.contiguous and vm1.shape == vm2.shape:
      return vm2
    if vm1.contiguous and vm1.size() == vm2.size() and (ret := vm2.reshape(vm1.shape)) is not None:
      return ret
    if vm1.mask:
      if (new_vm1 := vm1.shrink(vm1.mask)) == vm1 or (merged := vm2 + new_vm1) is None:
        return None
    ...
    if not all_int(vm1.shape):
      ...
      if all(x == 0 for x in vm2.strides + vm1.strides) and vm2.mask is None:
        return vm1
    ...
    for d1, st in enumerate(vm1.strides):
      ...
      for d2, (o, s1) in enumerate(zip(origin, unravel(vm2.shape, vm1.offset + st))):
        if not resolve((s1 := s1 - o) != 0):
          continue  # if s1 can possibly be 0
    ...
    if vm2.mask:
      ...
      for (b, e), o, term, (_, t) in zip(vm2.mask, origin, terms, reversed(extents)):
        if resolve(b <= (t := t.simplify()).vmin and t.vmax < e, False):
          continue
        if len(term) != 1:
  ...
  def shrink(self, arg: tuple[tuple[sint, sint], ...]) -> View:
    ...
    for s, (b, e) in zip(self.shape, arg):
      assert not all_int([s, b, e]) or (0 <= b <= e <= s), f"invalid shrink {arg} for {self.shape}"
  ...
  def expand(self, new_shape: tuple[sint, ...]) -> View:
    ...
    mask = (
      tuple([
        (((0, 0) if m != (0, 1) else (0, ns)) if resolve(s != ns) and resolve(s == 1, False) else m)
