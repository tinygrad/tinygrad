...
class View:
  ...
  contiguous:bool
  offset_noreal:Optional[bool]=None

  @staticmethod
  @functools.lru_cache(maxsize=None)
  def create(shape:Tuple[sint, ...], strides:Optional[Tuple[sint, ...]]=None, offset:sint=0, mask:Optional[Tuple[Tuple[sint, sint], ...]]=None, offset_noreal:Optional[bool]=None):
    ...
    return View(shape, strides, offset, mask, contiguous, offset_noreal)
  ...
  def unbind(self) -> View:
    ...
    return View.create(new_shape, new_strides, new_offset, new_mask, self.offset_noreal)

  # MovementOps live here now

  def __unsafe_resize(self, arg: Tuple[Tuple[sint, sint], ...], mask=None) -> View:
    offset = sum([s * x[0] for s, x in zip(self.strides,arg)])
    offset_noreal = any(ay <= mx or ax>=my for (mx,my),(ax,ay) in zip(self.mask, arg)) if self.mask else None
    if self.mask:
      # move the old mask
      nmask = tuple([(min(max(mx-ax, 0), ay-ax), max(min(my-ax, ay-ax), 0)) for (mx,my),(ax,ay) in zip(self.mask, arg)])
      # merge the masks if we have two
      mask = tuple([(max(mx1, mx2), min(my1, my2)) for (mx1, my1), (mx2, my2) in zip(nmask, mask)]) if mask is not None else nmask
    shape = [y-x for x,y in arg]
    return View.create(tuple(s.b if isinstance(s, NumNode) else s for s in shape), self.strides, self.offset+offset, mask, offset_noreal)
  ...
  def expand(self, new_shape: Tuple[sint, ...]) -> View:
    ...
    return View.create(new_shape, self.strides, self.offset, mask, self.offset_noreal)

  @functools.lru_cache(maxsize=None)  # pylint: disable=method-cache-max-size-none
  def permute(self, axis: Tuple[int, ...]) -> View:
    assert all(isinstance(x, int) and x >= 0 and x < len(self.shape) for x in axis), f"invalid permute {axis} for {self.shape}"
    assert len(set(axis)) == len(axis) and len(axis) == len(self.shape), f"can't permute {self.shape} with {axis}"
    return View.create(tuple([self.shape[a] for a in axis]), tuple([self.strides[a] for a in axis]), self.offset, tuple([self.mask[a] for a in axis]) if self.mask is not None else None, self.offset_noreal)
  ...
  def stride(self, mul: Tuple[int, ...]) -> View:
    ...
    return View.create(new_shape, strides, self.offset + offset, mask, self.offset_noreal)
  ...
  def reshape(self, new_shape: Tuple[sint, ...]) -> Optional[View]:
    ...
    if self.contiguous: return View.create(new_shape, offset_noreal=self.offset_noreal)
    ...
    if [x for x in self.shape if x != 1] == [x for x in new_shape if x != 1]:
      ...
      return View.create(new_shape, new_strides_tuple, self.offset, new_mask_tuple, self.offset_noreal)
