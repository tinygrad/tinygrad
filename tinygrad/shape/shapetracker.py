...
def overflow(u: UOp):
    return u.vmax > dtypes.max(dtypes.int) or u.vmin < dtypes.min(dtypes.int)
...
def upcast(u: UOp):
    ...
    if u.dtype.scalar() is dtypes.int:
        ...
        upcasted = u.replace(
            dtype=dtype, src=tuple([_src.cast(dtype) for _src in srcs])
        )
        if overflow(u):
            return upcasted
...
def views_to_indexed_uops(
    views: tuple[View, ...], _idxs: Optional[tuple[UOp, ...]] = None
) -> tuple[UOp, UOp]:
    idx, valid = views[-1].to_indexed_uops(_idxs)
    for view in reversed(views[0:-1]):
        view = view.minify()
        idx, valid = view.to_indexed_uops(
            [sint_to_uop(i) for i in unravel(view.shape, idx)], valid
        )
    return idx, valid


@functools.lru_cache(None)
def views_to_real_strides(
    views: tuple[View, ...], ignore_valid=False
) -> tuple[Optional[sint], ...]:
    # NOTE: if a stride is not always valid, it will be None
    if len(views) == 1 and views[-1].mask is None:
        return views[-1].strides
    ...
    if (newvalid := simplify_valid(valid)) is not None:
        valid = newvalid
    if (newidx := uop_given_valid(valid, idx)) is not None:
        idx = graph_rewrite(newidx, symbolic_flat)
    for c in split_uop(idx, Ops.ADD):
        if c.op is Ops.RANGE:
            ret[c.arg] = 1
        if c.op is Ops.MUL and c.src[0].op is Ops.RANGE and c.src[1].op is Ops.CONST:
            ret[c.src[0].arg] = c.src[1].arg
        if c.op is Ops.MUL and c.src[1].op is Ops.RANGE and c.src[0].op is Ops.CONST:
            ret[c.src[1].arg] = c.src[0].arg
    used_ranges = [x.arg for x in idx.toposort if x.op is Ops.RANGE]
    ret = [x if i in used_ranges else 0 for i, x in enumerate(ret)]
    if not ignore_valid:
        for masked_axis in [x.arg for x in valid.toposort if x.op is Ops.RANGE]:
            ret[masked_axis] = None
    return tuple(ret)
...
class ShapeTracker:
    ...
    def __add__(self, st: ShapeTracker) -> ShapeTracker:
        ...
        for v in st.views:
            ret = ShapeTracker(
                ret.views + (v,)
            ).simplify()  # one view at a time = better simplification
        return ret

    def invert(self, out_shape: tuple[sint, ...]) -> Optional[ShapeTracker]:
        inverted_views: list[View] = []
        for v, s in zip(
            self.views[::-1], [x.shape for x in self.views[::-1][1:]] + [out_shape]
        ):
            if (inverted := v.invert(s)) is None:
                return None
            inverted_views.append(inverted)
        return ShapeTracker(tuple(inverted_views)).reshape(out_shape)

    @staticmethod
    def from_shape(shape: tuple[sint, ...]) -> ShapeTracker:
    ...
    def consecutive(self) -> bool:
        return (
            len(self.views) == 1
            and (v := self.views[0]).mask is None
            and v.strides == strides_for_shape(v.shape)
        )
    ...
    def reduce(self, axis: tuple[int, ...]) -> tuple[sint, ...]:
        return tuple(1 if i in axis else s for i, s in enumerate(self.shape))

    def to_uop(self) -> UOp:
        return UOp(Ops.VIEW, dtypes.void, (), self)

    def to_indexed_uops(
        self, _idxs: Optional[list[UOp] | tuple[UOp, ...]] = None
    ) -> tuple[UOp, UOp]:
        idx, valid = views_to_indexed_uops(
            self.views, tuple(_idxs) if _idxs is not None else None
        )
    ...
    def vars(self) -> set[Variable]:
        return set().union(*[v.vars() for v in self.views])
    ...
    def unbind(self) -> tuple[ShapeTracker, dict[Variable, int]]:
        ...
        if all(len(x) == 0 for x in var_vals):
            return self, {}
        return ShapeTracker(tuple(unbound_views)), merge_dicts(var_vals)

    def real_strides(self, ignore_valid=False) -> tuple[Optional[sint], ...]:
        return views_to_real_strides(self.views, ignore_valid)
    ...
    def axis_is_masked(self, axis: int) -> bool:
        with Context(TRACK_MATCH_STATS=0):
            ...
            return axis in [
                x.arg
                for x in graph_rewrite(valid, symbolic_flat).toposort
    ...
    def simplify(self) -> ShapeTracker:
        if (
            len(self.views) >= 2
            and (new_view := self.views[-2] + self.views[-1]) is not None
        ):
    ...
    def flip(self, mul: tuple[int, ...]) -> ShapeTracker:
        return ShapeTracker(self.views[0:-1] + (self.views[-1].flip(mul),))
    ...
    def mop(self, op, arg):
        return mops[op](self, arg)


mops: dict[Ops, Callable] = {
    Ops.RESHAPE: ShapeTracker.reshape,
    Ops.PERMUTE: ShapeTracker.permute,
    Ops.EXPAND: ShapeTracker.expand,
    Ops.SHRINK: ShapeTracker.shrink,
    Ops.FLIP: ShapeTracker.flip,
    Ops.PAD: ShapeTracker.pad,
}
