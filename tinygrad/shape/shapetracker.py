...
class View:
  ...
  def __init__(self, shape:tuple[int, ...], strides:tuple[int, ...], offset:int=0, mask:tuple[tuple[int, int], ...] | None=None):
...
@functools.lru_cache(maxsize=None)
def merge_views(vm2:View, vm1:View) -> View | None:
...
class ShapeTracker:
  __slots__ = "views"
  def __init__(self, shape:ShapeTracker | tuple[int, ...], views:list[View] | None=None):
  ...
  def real_strides(self, ignore_valid=False) -> tuple[int | None, ...]:
    if len(self.views) == 1 and self.views[-1].mask is None: return self.views[-1].strides
    idxs = [Variable(f"idx{i}", 0, s-1) for i,s in enumerate(self.shape)]
    idx, valid = self.expr_idxs(idxs)
    ret: list[int | None] = [None] * len(self.views[-1].shape)
  ...
  def movement_op(self, op: MovementOps, arg:tuple[int, ...] | tuple[tuple[int, int], ...]) -> ShapeTracker:
...
def get_contraction(old_shape:tuple[int, ...], new_shape:tuple[int, ...]) -> list[list[int]] | None:
