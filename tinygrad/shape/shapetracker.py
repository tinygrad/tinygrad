...
def merge_views(vm2:View, vm1:View) -> Optional[View]:
  ...
  if vm2.mask or vm1.offset != 0:
    real_shape = tuple(y-x for x,y in vm2.mask) if vm2.mask else vm2.shape
    newstrides = []
    def safe_mask(_v): return _v.mask if _v.mask else tuple([(0, ss) for ss in _v.shape])
    vm2_mask, vm1_mask = [safe_mask(_v) for _v in (vm2, vm1)]
    print(f"vm1_mask {vm1_mask}")
    newmask = [[0,0] for _ in range(len(vm1.shape))]
    def calc_offset(pos, s, m):
      if s == 0: return 0
      if s > 0: return s * (m[0] + pos)
      return s * (m[0] + pos - m[1])
    for idim in range(len(vm1.shape)):
      v = []
      newmask[idim] = [vm1_mask[idim][0] for _ in range(2)]
      for i in range(*vm1_mask[idim]):
        d = [((calc_offset(i, vm1.strides[idim], vm1_mask[idim])) // _s) if _s != 0 else 0 for _s in strides_for_shape(real_shape)]
        d[1:] = [(_d % _r) if _d >= 0 else -(-_d % _r) for _d, _r in zip(d[1:], real_shape[1:])]
        if d[0] < real_shape[0]: newmask[idim][1] += 1
        else:
          newmask[idim] = [k + vm2_mask[0][0] for k in newmask[idim]]
          break
        v.append(0)
        for t, dn in enumerate(d):
          v[-1] += calc_offset(dn, vm2.strides[t], vm2_mask[t])
        #if idim == 0 and i == 0: newoffset = vm2.offset + v[0]
      s = v[1] - v[0] if len(v) > 1 else 0
      newstrides.append(s)
    newmask = tuple(tuple(x) for x in newmask)
    d = [((abs(vm1.offset) // _s) if _s != 0 else 0)  for _s in strides_for_shape(real_shape)]
    d[1:] = [_d % _r for _d, _r in zip(d[1:], real_shape[1:])]
    newoffset = vm2.offset
    for t, dn in enumerate(d):
      newoffset += calc_offset(dn if vm2.offset >= 0 else -dn, abs(vm2.strides[t]), vm2_mask[t])
    if all(tuple(nst) == (0, nsh) for nst, nsh in zip(newmask, vm1.shape)): newmask = None
    print(f"{newstrides=} {newoffset=} {newmask=}")
    return View.create(vm1.shape, cast(Tuple[sint, ...], tuple(newstrides)), newoffset, newmask)
