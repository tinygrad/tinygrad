...
class Variable(Node):
  ...
  def evaluate(self):
    ...
    raise ValueError(f"Variable {self.expr} is not bound to any value")
...
class RangeNode(Node):
  def __init__(self, start: Union[int, Node], end: Union[int, Node], step: Union[int, Node] = 1):
    super().__init__()
    self.start = start
    self.end = end
    self.step = step
    self.min, self.max = self.calculate_bounds()

  def calculate_bounds(self) -> Tuple[int, int]:
    start_min, start_max = (self.start.min, self.start.max) if isinstance(self.start, Node) else (self.start, self.start)
    end_min, end_max = (self.end.min, self.end.max) if isinstance(self.end, Node) else (self.end, self.end)
    step_min, step_max = (self.step.min, self.step.max) if isinstance(self.step, Node) else (self.step, self.step)
