...
class Node:
  ...
  # traverse max attribute looking for an int
  def find_max_int(self) -> int:
    tryint = self.max
    while(not isinstance(tryint, int)): tryint = tryint.max
    assert isinstance(tryint, int), f"tryint must be integer, {tryint=} isn't"
    return tryint
