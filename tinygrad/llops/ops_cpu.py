...
class CPUBuffer(GenericExecAST):
  fxn_for_op = {
    UnaryOps.NOOP: lambda x: x.ravel().reshape(x.shape), UnaryOps.NEG: lambda x: -x, UnaryOps.RELU: lambda x: np.maximum(x, 0),
    UnaryOps.EXP: lambda x: np.exp(x), UnaryOps.LOG: lambda x: np.log(x), UnaryOps.GT0: lambda x: operator.gt(x, 0.0), UnaryOps.RECIPROCAL: lambda x: 1.0/x,
    BinaryOps.ADD: operator.add, BinaryOps.SUB: operator.sub, BinaryOps.MUL: operator.mul,
    BinaryOps.DIV: operator.truediv, BinaryOps.POW: operator.pow, BinaryOps.CMPEQ: lambda x,y: (x==y).astype(np.float32),
    ...
    ReduceOps.MAX: lambda x, new_shape: np.amax(x, shape_to_axis(x.shape, new_shape), keepdims=True) if tuple(x.shape) != tuple(new_shape) else x[:],
  ...
  def processing_op(x,op,w,C):
    ...
    tw = w.buf.reshape(C.groups, C.rcout, C.cin, C.H, C.W)
    out = np.einsum("nGhwCHW, GkCHW -> nGkhw", tx.buf.ravel().reshape(tx.shape), tw.ravel().reshape(tw.shape))
