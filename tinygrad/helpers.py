...
if TYPE_CHECKING:
  from typing_extensions import TypeGuard # TODO: remove this and import TypeGuard from typing once minimum python supported version is 3.10
  import numpy as np
...
class dtypes:
  ...
  @staticmethod
  def from_np(x: Union[str, np.dtype]) -> DType: return DTYPES_DICT[x if isinstance(x, str) else x.name]
  @staticmethod
  ...
  @staticmethod
  def imageh(shp): return ImageDType(100, 2, "imageh", "float16", shp, dtypes.float32)
  @staticmethod
  def imagef(shp): return ImageDType(100, 4, "imagef", "float32", shp, dtypes.float32)
