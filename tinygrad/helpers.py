...
def get_mv(x: Any, dtype: DType, _shape=[], _base=True) -> Tuple[Tuple[int, ...], memoryview]:
  l = x
  ...
  if isinstance(x, _Scalars) or isinstance(x, np.generic): return tuple(_shape), memoryview(struct.pack(f'{prod(_shape)}{dtype.structf}', *l if not dtypes.is_int(dtype) else list(map(int, l)))) if _base else memoryview(b'')
