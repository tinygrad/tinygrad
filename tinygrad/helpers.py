...
def argsort(x):
  return type(x)(sorted(range(len(x)), key=x.__getitem__))  # https://stackoverflow.com/questions/3382352/equivalent-of-numpy-argsort-in-basic-python


def all_same(items: tuple[T, ...] | list[T]):
...
def colorize_float(x: float):
  return colored(f"{x:7.2f}x", "green" if x < 0.75 else "red" if x > 1.15 else "yellow")


def time_to_str(t: float, w=8) -> str:
  return next((f"{t * d:{w}.2f}{pr}" for d, pr in [(1, "s "), (1e3, "ms")] if t > 10 / d), f"{t * 1e6:{w}.2f}us")
...
def make_tuple(x: int | Sequence[int], cnt: int) -> tuple[int, ...]:
...
def strip_parens(fst: str):
  return fst[1:-1] if fst[0] == "(" and fst[-1] == ")" and fst[1:-1].find("(") <= fst[1:-1].find(")") else fst
...
def round_down(num: int, amt: int) -> int:
  return -round_up(-num, amt)


# cstyle div and mod
def cdiv(x: int, y: int) -> int:
  return abs(x) // abs(y) * (1, -1)[x * y < 0] if y != 0 else 0
...
def word_wrap(x, wrap=80):
  if len(ansistrip(x)) <= wrap:
    return x
  if len(lines := x.splitlines()) > 1:
    return "\n".join(word_wrap(line, wrap) for line in lines)
  i = 0
  while len(ansistrip(x[:i])) < wrap and i < len(x):
    i += 1
  return x[:i] + "\n" + word_wrap(x[i:], wrap)
  ...
  def wrapper(*args, **kwargs):
    try:
      ...
      if not getattr(sys, "is_finalizing", lambda: True)():
        raise  # re-raise if not finalizing

  return wrapper


def pluralize(st: str, cnt: int):
  return f"{cnt} {st}" + ("" if cnt == 1 else "s")
...
class LazySeq(Generic[T]):  # NOTE: Mapping requires __iter__ and __len__, Sequence requires supporting __len__ and slicing in __getitem__
  def __init__(self, gen: Callable[[int], T]):
    self.gen = gen
...
def to_function_name(s: str):
  return "".join([c if c in (string.ascii_letters + string.digits + "_") else f"{ord(c):02X}" for c in ansistrip(s)])
...
def diskcache_put(table: str, key: dict | str | int, val: Any, prepickled=False):
  ...
  cur.execute(
    f"REPLACE INTO '{table}_{VERSION}' ({', '.join(key.keys())}, val) VALUES ({', '.join(['?'] * len(key))}, ?)",
...
def diskcache(func: Callable[..., T]):
  def wrapper(*args, **kwargs) -> T:
    ...
    if (ret := diskcache_get(table, key)) is not None:
      return ret
...
def fetch(
  ...
  if name is not None and (isinstance(name, pathlib.Path) or "/" in name):
    ...
    fp = _ensure_downloads_dir() / (subdir or "") / ((name or hashlib.md5(url.encode("utf-8")).hexdigest()) + (".gunzip" if gunzip else ""))
...
def capstone_flatdump(lib: bytes):
  ...
  match platform.machine():
    ...
    case machine:
      raise NotImplementedError(f"Capstone disassembly isn't supported for {machine}")
  cs.skipdata = True
...
class tqdm(Generic[T]):
  ...
  def update(self, n: int = 0, close: bool = False):
    ...
    def SI(x):
      return (
        (f"{x / 1000 ** int(g := round(math.log(x, 1000), 6)):.{int(3 - 3 * math.fmod(g, 1))}f}"[:4].rstrip(".") + " kMGTPEZY"[int(g)].strip())
        if x
        else "0.00"
      )
    ...
    bar = (
      "\r"
      + self.desc
      + (f"{100 * prog:3.0f}%|{('█' * int(num := sz * prog) + ' ▏▎▍▌▋▊▉'[int(8 * num) % 8].strip()).ljust(sz, ' ')}| " if self.t else "")
      + suf
    )
