...
def all_same(items: Union[tuple[T, ...], list[T]]):
...
def colorize_float(x: float):
    return colored(
        f"{x:7.2f}x", "green" if x < 0.75 else "red" if x > 1.15 else "yellow"
    )


def memsize_to_str(_bytes: int) -> str:
    return [
        f"{(_bytes / d):.2f} {pr}"
        for d, pr in [(1e9, "GB"), (1e6, "MB"), (1e3, "KB"), (1, "B")]
        if _bytes > d
    ][0]
...
def make_tuple(x: Union[int, Sequence[int]], cnt: int) -> tuple[int, ...]:
    return (x,) * cnt if isinstance(x, int) else tuple(x)
...
def fully_flatten(l):
    if hasattr(l, "__len__") and hasattr(l, "__getitem__") and not isinstance(l, str):
        ...
        flattened = []
        for li in l:
            flattened.extend(fully_flatten(li))
        return flattened
    return [l]
...
def lo32(x: Any) -> Any:
    return x & 0xFFFFFFFF  # Any is sint


def hi32(x: Any) -> Any:
    return x >> 32  # Any is sint


def data64(data: Any) -> tuple[Any, Any]:
    return (data >> 32, data & 0xFFFFFFFF)  # Any is sint


def data64_le(data: Any) -> tuple[Any, Any]:
    return (data & 0xFFFFFFFF, data >> 32)  # Any is sint


def getbits(value: int, start: int, end: int):
    return (value >> start) & ((1 << end - start + 1) - 1)


def i2u(bits: int, value: int):
...
def partition(itr: Iterable[T], fxn: Callable[[T], bool]) -> tuple[list[T], list[T]]:
    ...
    for s in itr:
        (ret[0] if fxn(s) else ret[1]).append(s)
    return ret
...
def polyN(x: T, p: list[float]) -> T:
    return functools.reduce(lambda acc, c: acc * x + c, p, 0.0)  # type: ignore
...
def temp(x: str, append_user: bool = False) -> str:
    return (
        ...
        / (f"{x}.{getpass.getuser()}" if append_user else x)
    ).as_posix()
...
JIT = ContextVar(
    ...
    (
        ...
        and ("Intel" in platform.processor() or "i386" in platform.processor())
        else 1
    ),
)
WINO, CAPTURING, TRACEMETA = (
    ContextVar("WINO", 0),
    ContextVar("CAPTURING", 1),
    ContextVar("TRACEMETA", 1),
)
USE_TC, TC_SELECT, TC_OPT, AMX = (
    ContextVar("TC", 1),
    ContextVar("TC_SELECT", -1),
    ContextVar("TC_OPT", 0),
    ContextVar("AMX", 0),
)
TRANSCENDENTAL, TC_SEARCH_OVER_SHAPE = ContextVar("TRANSCENDENTAL", 1), ContextVar(
    "TC_SEARCH_OVER_SHAPE", 1
)
FUSE_ARANGE, FUSE_CONV_BW = ContextVar("FUSE_ARANGE", 0), ContextVar("FUSE_CONV_BW", 0)
SPLIT_REDUCEOP, NO_MEMORY_PLANNER, RING = (
    ContextVar("SPLIT_REDUCEOP", 1),
    ContextVar("NO_MEMORY_PLANNER", 0),
    ContextVar("RING", 1),
)
PICKLE_BUFFERS, PROFILE, LRU = (
    ContextVar("PICKLE_BUFFERS", 1),
    ContextVar("PROFILE", getenv("VIZ")),
    ContextVar("LRU", 1),
)
CACHELEVEL, IGNORE_BEAM_CACHE, DEVECTORIZE = (
    ContextVar("CACHELEVEL", 2),
    ContextVar("IGNORE_BEAM_CACHE", 0),
    ContextVar("DEVECTORIZE", 1),
)
...
class Metadata:
    ...
    def __repr__(self):
        return str(self) + (f" - {self.caller}" if self.caller else "")

    def __str__(self):
        return self.name + (" bw" if self.backward else "")


_METADATA: contextvars.ContextVar[Optional[Metadata]] = contextvars.ContextVar(
    "_METADATA", default=None
)
...
def db_connection():
    ...
    if _db_connection is None:
        ...
        _db_connection = sqlite3.connect(
            CACHEDB, timeout=60, isolation_level="IMMEDIATE"
        )
...
def diskcache_clear():
    ...
    drop_tables = cur.execute(
        "SELECT 'DROP TABLE IF EXISTS ' || quote(name) || ';' FROM sqlite_master WHERE type = 'table';"
    ).fetchall()
...
CAPTURE_PROCESS_REPLAY = getenv("RUN_PROCESS_REPLAY") or getenv(
    ...
    if not fp.is_file() or not allow_caching:
        ...
        with urllib.request.urlopen(url, timeout=10) as r:
            ...
            progress_bar: tqdm = tqdm(
                total=length, unit="B", unit_scale=True, desc=f"{url}", disable=CI
            )
    ...
    match platform.machine():
        ...
        case machine:
            raise NotImplementedError(
                f"Capstone disassembly isn't supported for {machine}"
            )
    cs.skipdata = True
...
def from_mv(mv: memoryview, to_type=ctypes.c_char):
    return ctypes.cast(
        ctypes.addressof(to_type.from_buffer(mv)), ctypes.POINTER(to_type * len(mv))
    ).contents


def to_mv(ptr: int, sz: int) -> memoryview:
    return memoryview(
        ctypes.cast(ptr, ctypes.POINTER(ctypes.c_uint8 * sz)).contents
    ).cast("B")
...
def flat_mv(mv: memoryview):
    return mv if len(mv) == 0 else mv.cast("B", shape=(mv.nbytes,))
...
def _serialize_code(code: types.CodeType):
    args = inspect.signature(
        types.CodeType
    ).parameters  # NOTE: this works in Python 3.10 and up
    return _reconstruct_code, tuple(
        code.__getattribute__(
            "co_" + x.replace("codestring", "code").replace("constants", "consts")
        )
        for x in args
    )
...
def _serialize_module(module: types.ModuleType):
