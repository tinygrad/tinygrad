...
T = TypeVar("T")
U = TypeVar("U")
_Scalars = (int, float, bool, bytes, type(None), )
...
def get_shape(x: Any, _shape=tuple()) -> Tuple[int, ...]:
  ...
  import numpy as np # TODO: onnx embedds numpy elements in Lists. I can't think of a way to avoid the import
...
def to_mv(l: Any, dtype: DType) -> Tuple[memoryview, Tuple[int, ...]]:
  ...
  if dtype.ctype is None:
    ...
    return memoryview(np.array(l).astype(dtype.np).data), shape
