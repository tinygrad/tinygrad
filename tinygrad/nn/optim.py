  ...
  def schedule_step(self) -> list[Tensor]:
    ...
    if self.fused:
      ...
      out, extra = self._step(
        [Tensor.cat(*[t.flatten() for t in self.params], dim=0)], [Tensor.cat(*[unwrap(t.grad).flatten() for t in self.params], dim=0)]
      )
...
def Muon(
  params: list[Tensor], lr=0.02, momentum=0.95, weight_decay=0.0, ns_steps=5, ns_params=(3.4445, -4.775, 2.0315), nesterov=True, fused=FUSE_OPTIM
):
  """
...
class LARS(Optimizer):
  ...
  def _step(self, params: list[Tensor], grads: list[Tensor]) -> tuple[list[Tensor], list[Tensor]]:
    ...
    for i, (t, g) in enumerate(zip(params, grads)):
      ...
      if self.ns_params:
        g = g.reshape(g.shape[0], -1).newton_schulz(self.ns_steps, self.ns_params).reshape(g.shape)
