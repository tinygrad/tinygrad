...
class TensorIO(io.RawIOBase, BinaryIO):
  ...
  def read(self, size: int = -1) -> bytes:
    if (buf := super().read(size)) is None:
      raise ValueError("io.RawIOBase.read returned None")  # only happens if readinto returns None (never)
    return buf
...
def accept_filename(func: Callable[[Tensor], T]) -> Callable[[Tensor | str | pathlib.Path], T]:
  ...
  def wrapper(fn: Tensor | str | pathlib.Path) -> T:
...
def safe_load(fn: Tensor | str | pathlib.Path) -> dict[str, Tensor]:
  ...
  data = t[data_start:]
  return {
    k: data[v["data_offsets"][0] : v["data_offsets"][1]].bitcast(safe_dtypes[v["dtype"]]).reshape(v["shape"])
    ...
    for k, v in (t := tqdm(model_state_dict.items(), disable=CI or not verbose)):
      ...
      if isinstance(v.device, tuple):
        if isinstance(state_dict[k].device, tuple):
          ...
          v.replace(state_dict[k].shard(v.device, v.uop.axis))
      else:
        v.replace(state_dict[k].to(v.device))
      if realize:
        v.realize()
      if consume:
        del state_dict[k]
...
def torch_load(t: Tensor) -> dict[str, Tensor]:
  ...
  def _rebuild_tensor_v2(storage, storage_offset, size, stride, requires_grad=None, backward_hooks=None, metadata=None):
    ...
    if tuple(permute_indexes) != tuple(range(len(permute_indexes))):
      ...
      if DEBUG >= 3:
        print(f"WARNING: this torch load is slow. to permute {intermediate_shape} with {permute_indexes}")
...
def ggml_data_to_tensor(t: Tensor, n: int, ggml_type: int) -> Tensor:
  ...
  if (nelements_nbytes := {2: (32, 18), 3: (32, 20), 14: (256, 210), 8: (32, 34), 39: (32, 17)}.get(ggml_type)) is not None:
    ...
    if ggml_type == 39:
      ...
      d = (
        (e_int >= 2).cast(dtypes.float32) * (e_int.cast(dtypes.float32) - 128).exp2()
        + (e_int == 1).cast(dtypes.float32) * 2.0 ** (-127)
        + (e_int == 0).cast(dtypes.float32) * 2.0 ** (-128)
      ).unsqueeze(-1)
      ...
      fp4_val = sign * ((exp != 0).cast(dtypes.float32) * (1.0 + 0.5 * mant) * (exp - 1.0).exp2() + (exp == 0).cast(dtypes.float32) * 0.5 * mant)
