...
class TensorIO(io.RawIOBase, BinaryIO):
    def __init__(self, t: Tensor):
        if t.ndim != 1 or t.dtype != dtypes.uint8:
    ...
    def read(self, size: int = -1) -> bytes:
        if (buf := super().read(size)) is None:
            raise ValueError(
                "io.RawIOBase.read returned None"
            )  # only happens if readinto returns None (never)
        return buf
    ...
    def writelines(self, lines: Iterable[Any]):
...
def accept_filename(
    func: Callable[[Tensor], T],
) -> Callable[[Union[Tensor, str, pathlib.Path]], T]:
    @functools.wraps(func)
    def wrapper(fn: Union[Tensor, str, pathlib.Path]) -> T:
        return func(Tensor(pathlib.Path(fn)) if not isinstance(fn, Tensor) else fn)
...
def safe_load(fn: Union[Tensor, str, pathlib.Path]) -> dict[str, Tensor]:
    ...
    data = t[data_start:]
    return {
        k: data[v["data_offsets"][0] : v["data_offsets"][1]]
        .bitcast(safe_dtypes[v["dtype"]])
        .reshape(v["shape"])
...
def load_state_dict(
    model, state_dict: dict[str, Tensor], strict=True, verbose=True, consume=False
) -> None:
    """
    ...
    with Timing(
        "loaded weights in ",
        lambda et_ns: f", {(B:=(GlobalCounters.mem_used-start_mem_used))/1e9:.2f} GB loaded at {B/et_ns:.2f} GB/s",
    ):
        ...
        for k, v in (t := tqdm(model_state_dict.items(), disable=CI or not verbose)):
            ...
            if v.shape != state_dict[k].shape:
                raise ValueError(
                    f"Shape mismatch in layer `{k}`: Expected shape {v.shape}, but found {state_dict[k].shape} in state dict."
                )
            if isinstance(v.device, tuple):
                if isinstance(state_dict[k].device, tuple):
...
def tar_extract(t: Tensor) -> dict[str, Tensor]:
    ...
    with tarfile.open(fileobj=TensorIO(t), mode="r") as tar:
        return {
            member.name: t[member.offset_data : member.offset_data + member.size]
            for member in tar
...
def torch_load(t: Tensor) -> dict[str, Tensor]:
    ...
    def _rebuild_tensor_v2(
        ...
        ret = t[byte_offset : byte_offset + prod(size) * storage[1].itemsize].bitcast(
            storage[1]
        )
        ...
        if tuple(permute_indexes) != tuple(range(len(permute_indexes))):
            ...
            if DEBUG >= 3:
                print(
                    f"WARNING: this torch load is slow. CLANG to permute {intermediate_shape} with {permute_indexes}"
                )
    ...
    class TorchPickle(pickle.Unpickler):
        ...
        def persistent_load(self, pid):
            return deserialized_objects.get(pid, pid)

    fobj = io.BufferedReader(TensorIO(t))

    def passthrough_reset(v: bool):
        return fobj.seek(0, 0) or v

    if passthrough_reset(
        zipfile.is_zipfile(fobj)
    ):  # NOTE: passthrough_reset required to support python < 3.14
    ...
    elif passthrough_reset(
        tarfile.is_tarfile(fobj)
    ):  # NOTE: passthrough_reset required to support python < 3.11
        with tarfile.open(fileobj=fobj, mode="r") as tar:
            ...
            for _ in range(TorchPickle(f).load()):  # num_tensors
                ...
                size, stride = struct.unpack(
                    f"<{ndim}q", f.read(8 * ndim)
...
def ggml_data_to_tensor(t: Tensor, n: int, ggml_type: int) -> Tensor:
    ...
    def q_to_uint8(t: Tensor, b: int) -> Tensor:
        ...
        shift_tensor, bitmask = Tensor.stack(
            *[
                Tensor(2 ** (i * b), device=t.device, dtype=t.dtype)
                for i in range(8 // b)
            ]
        ), 0xFF >> (8 - b)
        return (
    ...
    if (
        nelements_nbytes := {2: (32, 18), 3: (32, 20), 14: (256, 210), 8: (32, 34)}.get(
            ggml_type
        )
    ) is not None:
        blocks = t[: (n // nelements_nbytes[0]) * nelements_nbytes[1]].reshape(
            (-1, nelements_nbytes[1])
        )
        if ggml_type == 2:
            return (q_to_uint8(blocks[:, 2:], 4).bitcast(dtypes.int8) - 8) * blocks[
                :, :2
            ].bitcast(dtypes.float16).cast(dtypes.float32)
        if ggml_type == 3:
            d, m = (
                blocks[:, s : s + 2].bitcast(dtypes.float16).cast(dtypes.float32)
                for s in [0, 2]
            )
            return q_to_uint8(blocks[:, 4:], 4).bitcast(dtypes.int8) * d + m
        if ggml_type == 8:
            return blocks[:, :2].bitcast(dtypes.float16).cast(dtypes.float32) * blocks[
                :, 2:
            ].bitcast(dtypes.int8)
        if ggml_type == 14:
            xl, xh = q_to_uint8(blocks[:, :128].reshape((-1, 2, 64)), 4), q_to_uint8(
                blocks[:, 128:192].reshape((-1, 2, 32)), 2
            ).lshift(4)
            scales = (
                blocks[:, 192:208]
                .bitcast(dtypes.int8)
            ...
            d = (
                blocks[:, -2:]
                .bitcast(dtypes.float16)
                .cast(dtypes.float32)
                .expand((-1, 256))
            )
            return (
                d * (xl.bitwise_or(xh).bitcast(dtypes.int8) - 32).flatten(-2) * scales
            )
...
def gguf_load(tensor: Tensor) -> tuple[dict, dict[str, Tensor]]:
    ...
    def read_unpack(fmt: str, n: int):
        return struct.unpack(fmt, reader.read(n))[0]

    def read_str():
        return str(reader.read(read_uint64()), "utf-8")
    ...
    readers: dict[int, Callable[[], Any]] = {
        ...
        9: read_arr,
        **{
            t: functools.partial(read_unpack, "<" + f, nb)
            for t, f, nb in [
                (0, "c", 1),
                (1, "b", 1),
                (2, "H", 2),
    ...
    read_uint32, read_int32, read_uint64, read_int64 = (
        readers[4],
        readers[5],
        readers[10],
        readers[11],
    ...
    if magic != b"GGUF" or version not in [2, 3]:
        raise ValueError("Invalid GGUF format!")
    for _ in range(n_kv):
    ...
    t_infos = [
        (
            read_str(),
            tuple(read_uint64() for _ in range(read_uint32())),
            read_int32(),
            read_uint64(),
        )
    ...
    for name, dims, typ, off in t_infos:
        state_dict[name] = ggml_data_to_tensor(
            tensor[data_start + off :], prod(dims), typ
        ).reshape(*reversed(dims))
