...
def safe_load(fn:Union[Tensor,str]) -> Dict[str, Tensor]:
  ...
  if metadata['lm_head.weight']['dtype'] == "BF16":
    ret = {k:t[8+json_len+v['data_offsets'][0]:].bitcast(dtypes.uint8).to("CPU").cast(dtypes.uint32).mul(1<<16).bitcast(dtypes.float32).to(Device.DEFAULT).half() [:prod(v['shape'])].reshape(v['shape']) for k,v in metadata.items() if k != "__metadata__"}
  else:
    ret = {k:t[8+json_len+v['data_offsets'][0]:].cast(safe_dtypes[v['dtype']])[:prod(v['shape'])].reshape(v['shape']) for k,v in metadata.items() if k != "__metadata__"}
  return ret
