...
from tinygrad.helpers import CI, OSX, LRU, getenv, diskcache_get, diskcache_put, DEBUG, GlobalCounters, flat_mv, from_mv, PROFILE, temp, mv_address, \
                             cpu_time_execution, colored, Context, round_up, DISABLE_COMPILER_CACHE, DISALLOW_DEVICE_OPEN
...
class _Device:
  ...
  def __get_canonicalized_item(self, ix:str) -> Compiled:
    assert not DISALLOW_DEVICE_OPEN or ix.split(":")[0] in ["DISK", "NPY", "PYTHON"], f"open device {ix} disallowed"
    x = ix.split(":")[0].lower()
