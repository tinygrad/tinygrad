...
from tinygrad.helpers import ansilen, DEBUG, getenv, GlobalCounters, colored, BEAM, NOOPT, all_int, to_function_name, from_mv, flat_mv, create_gl_tex_dims
...
class CompiledASTRunner(JITRunner):
  def __init__(self, ast:Optional[LazyOp], name:str, prg:str, global_size:Optional[List[int]]=None, local_size:Optional[List[int]]=None, has_local:bool=True, runtime_args:Optional[dict]=None):  # noqa: E501
    super().__init__()
    ...
    self.name, self.display_name, self.prg, self.global_size, self.local_size, self.has_local, self.runtime_args, self.first_run = \
      to_function_name(name), name, prg, global_size, local_size, has_local, runtime_args if runtime_args is not None else {}, True
