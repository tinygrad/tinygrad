...
class _Device:
  ...
  def __get_canonicalized_item(self, ix:str) -> Union[Interpreted, Compiled]:
    x = ix.split(':')[0].upper()
    mod = importlib.import_module(f'tinygrad.runtime.ops_{x.lower()}')
    cls = next(c for n, c in inspect.getmembers(mod) if n.lower() == f"{x.lower()}device" and x in self._buffers)
    return cls(ix) if isinstance(cls, type) else cls
