...
class _Device:
    ...
    def _canonicalize(self, device: str) -> str:
        return re.sub(
            r":0$", "", (d := device.split(":", 1)[0].upper()) + device[len(d) :]
        )

    # NOTE: you can't cache canonicalize in case Device.DEFAULT changes
    def canonicalize(self, device: Optional[str]) -> str:
        return self._canonicalize(device) if device is not None else Device.DEFAULT
    ...
    def __get_canonicalized_item(self, ix: str) -> Compiled:
        ...
        assert (cpn == "MainProcess") or ix.split(":")[0] in [
            "DISK",
            "NPY",
            "PYTHON",
        ...
        ret = [
            ...
            if (cname.lower() == x.lower() + "device")
        ][0](ix)
        if DEBUG >= 1:
            print(f"opened device {ix} from pid:{os.getpid()}")
        self._opened_devices.add(ix)
        return ret

    @property
    def default(self) -> Compiled:
...
class ProfileDeviceEvent(ProfileEvent):
    device: str
    comp_tdiff: decimal.Decimal = decimal.Decimal(0)
    copy_tdiff: decimal.Decimal = decimal.Decimal(0)  # noqa: E702
...
@dataclass(frozen=True)
class ProfileGraphEvent(ProfileEvent):
    ents: list[ProfileGraphEntry]
    deps: list[list[int]]
    sigs: list[decimal.Decimal]  # noqa: E702


@dataclass
class ProfileResult:
    st: Optional[int] = None
    en: Optional[int] = None  # noqa: E702


@contextlib.contextmanager
def cpu_profile(
    name, device="CPU", is_copy=False, display=True
    ...
    if PROFILE and display:
        Compiled.profile_events += [
            ProfileRangeEvent(
                device,
                name,
                decimal.Decimal(st) / 1000,
                decimal.Decimal(en) / 1000,
                is_copy=is_copy,
            )
        ]
...
class Buffer:
    def __init__(
        ...
        initial_value: Optional[bytes] = None,
        lb_refcount=0,
        ...
        self.device, self.size, self.dtype, self.options, self.offset = (
            device,
            size,
            dtype,
            options,
    ...
    @property
    def lb_refcount(self):
        return self.base._lb_refcount
    ...
    def ensure_allocated(self) -> Buffer:
        return self.allocate() if not self.is_allocated() else self
    ...
    def allocate(self, opaque=None, external_ptr=None) -> Buffer:
        ...
        if external_ptr is not None:
            self.options = (
                replace(self.options, external_ptr=external_ptr)
                if self.options
        ...
        if self._base is not None:
            ...
            assert hasattr(
                self.allocator, "_offset"
            ), "offset function required for view"
            self._buf: Any = self.allocator._offset(
                self.base._buf, self.nbytes, self.offset
            )
        else:
            self._buf = (
                opaque
                if opaque is not None
                else self.allocator.alloc(self.nbytes, self.options)
            )
    ...
    def deallocate(self):
        ...
        if self._base is None and (
            self.options is None or self.options.external_ptr is None
        ):
            if not self.device.startswith("DISK"):
                GlobalCounters.mem_used -= self.nbytes
    ...
    def __del__(self):
        (not self.is_allocated()) or self.deallocate()

    def __repr__(self):
        return (
            f"<buf real:{self.is_allocated()} device:{self.device} size:{self.size} dtype:{self.dtype}"
            + (f" offset:{self.offset}" if hasattr(self, "base") else "")
            + (f" {self.options=}" if self.options is not None else "")
            + ">"
        )
    ...
    def as_buffer(self, allow_zero_copy=False, force_zero_copy=False) -> memoryview:
        ...
        if (
            (force_zero_copy or allow_zero_copy)
            and hasattr(self.allocator, "_as_buffer")
            and (self.options is None or self.options.image is None)
        ):
    ...
    def view(self, size: int, dtype: DType, offset: int) -> Buffer:
        ...
        if self._base is not None:
            return Buffer(
                self.device, size, dtype, base=self._base, offset=self.offset + offset
            )
...
class Allocator:
    ...
    def free(self, opaque, size: int, options: Optional[BufferSpec] = None):
        self._free(opaque, options if options is not None else BufferSpec())
    ...
    def _free(self, opaque, options: BufferSpec):
        pass  # if opaque is a Python object, you don't need a free
    ...
    def free_cache(self):
        for (sz, options), opaques in self.cache.items():
            for opaque in opaques:
                super().free(opaque, sz, options)
            opaques.clear()

    def free(self, opaque: Any, size: int, options: Optional[BufferSpec] = None):
        if LRU and (options is None or not options.nolru):
...
class _MallocAllocator(LRUAllocator):
    def _alloc(self, size: int, options: BufferSpec):
        ...
        return (
            (ctypes.c_uint8 * size).from_address(options.external_ptr)
            if options.external_ptr
            else self._alloc_aligned(size, alignment)
        )

    def _alloc_aligned(self, size: int, alignment: int):
        buffer = (ctypes.c_uint8 * (size + alignment))()
        offset = round_up(ctypes.addressof(buffer), alignment) - ctypes.addressof(
            buffer
        )
    ...
    def _offset(self, buf, size: int, offset: int):
        return from_mv(self._as_buffer(buf)[offset : offset + size])
...
class CPUProgram:
    rt_lib = ctypes.CDLL(
        ctypes.util.find_library("System" if OSX else "kernel32")
        if OSX or sys.platform == "win32"
        else "libgcc_s.so.1"
    )
    atomic_lib = (
        ctypes.CDLL(ctypes.util.find_library("atomic"))
    ...
    def __init__(self, name: str, lib: bytes):
        if sys.platform == "win32":
            ...
            if OSX:
                CPUProgram.rt_lib.pthread_jit_write_protect_np(False)
            self.mem.write(lib)
            if OSX:
                CPUProgram.rt_lib.pthread_jit_write_protect_np(True)

            # __clear_cache isn't a normal libc function, but a compiler support routine found in libgcc_s for gcc and compiler-rt for clang.
            # libgcc_s comes as shared library but compiler-rt is only a bunch of static library archives which we can't directly load, but fortunately
            # it somehow found its way into libSystem on macos (likely because it used __builtin_clear_cache) and libgcc_s is ~always present on linux
            # Using ["name"] instead of .name because otherwise name is getting mangled: https://docs.python.org/3.12/reference/expressions.html#index-5
            CPUProgram.rt_lib["__clear_cache"](
                ctypes.c_void_p(mv_address(self.mem)),
                ctypes.c_void_p(mv_address(self.mem) + len(lib)),
            )
    ...
    def __call__(self, *bufs, vals=(), wait=False):
        ...
        if platform.machine() == "arm64" and OSX:
            args = args[:8] + [
                ctypes.c_int64(a) if isinstance(a, int) else a for a in args[8:]
            ]
...
class Compiler:
    ...
    def compile_cached(self, src: str) -> bytes:
        if self.cachekey is None or (lib := diskcache_get(self.cachekey, src)) is None:
            assert not getenv(
                "ASSERT_COMPILE"
            ), f"tried to compile with ASSERT_COMPILE set\n{src}"
...
class Compiled:
    profile_events: list[ProfileEvent] = [
        ProfileDeviceEvent("CPU")
    ]  # NOTE: CPU is the default device.
    ...
    def __init__(
        ...
        self.device, self.allocator, self.compiler, self.runtime, self.graph = (
            device,
            allocator,
...
def is_dtype_supported(dtype: DType, device: Optional[str] = None) -> bool:
    ...
    if dtype == dtypes.bfloat16:
        ...
        return device in {"AMD"} or (
            device in {"CUDA", "NV"} and not CI and not getenv("PTX")
        )
    if device == "WEBGPU":
        return dtype in [
            dtypes.bool,
    ...
    if dtype == dtypes.half:
        if device == "GPU":
            return not CI and not OSX
        if device in ["CUDA", "NV"]:
            return not CI
        if device == "LLVM":
            return OSX
        if device == "PYTHON":
    ...
    if dtype == dtypes.float64:
        return device != "METAL" and not (OSX and device == "GPU")
    return True
...
if PROFILE:
    ...
    def finalize_profile():
        ...
        with open(fn := temp("profile.pkl", append_user=True), "wb") as f:
            pickle.dump(Compiled.profile_events, f)
    ...
    for device in ALL_DEVICES:
        try:
            ...
            try:
                ...
                if test != [2, 4, 6]:
                    raise ValueError(f"got {test} instead of [2, 4, 6]")
