...
from tinygrad.helpers import ansilen, DEBUG, getenv, GlobalCounters, colored, BEAM, NOOPT, all_int, to_function_name, DType, from_mv, dtypes, flat_mv, ImageDType, diskcache
...
class Compiler:
  def compile(self, prg:str, **kwargs) -> bytes: raise NotImplementedError("must be implemeted")

class CachedCompiler(Compiler):
  def compile(self, prg:str, **kwargs) -> bytes: return self._compile(prg, **kwargs) if getenv("DISABLE_COMPILER_CACHE") else diskcache(self._compile)(prg, _cache_key=self._cache_key(), **kwargs)
