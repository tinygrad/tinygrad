...
class _Device:
  ...
  def _canonicalize(self, device: str) -> str:
    return re.sub(r":0$", "", (d := device.split(":", 1)[0].upper()) + device[len(d) :])

  # NOTE: you can't cache canonicalize in case Device.DEFAULT changes
  def canonicalize(self, device: str | None) -> str:
  ...
  def __get_canonicalized_item(self, ix: str) -> Compiled:
    ...
    x = ix.split(":")[0].lower()
    ret = [cls for cname, cls in inspect.getmembers(importlib.import_module(f"{base}.runtime.ops_{x}")) if (cname.lower() == x + "device")][0](ix)
    if DEBUG >= 1:
...
@dataclass(frozen=True)
class ProfileProgramEvent(ProfileEvent):
  device: str
  name: str
  lib: bytes | None
  base: int | None  # noqa: E702
...
class MultiBuffer:
  ...
  @property
  def size(self):
    return self.bufs[0].size

  @property
  def dtype(self):
    return self.bufs[0].dtype
  ...
  def is_allocated(self):
    return all(x.is_allocated() for x in self.bufs)

  def __repr__(self):
    return f"<multibuf real:{self.is_allocated()} device:{tuple(x.device for x in self.bufs)} size:{self.size} dtype:{self.dtype}>"
...
class Buffer:
  ...
  def __init__(
    ...
    if isinstance(dtype, ImageDType):
      options = BufferSpec(image=dtype)  # TODO: image hack shouldn't be here. where should it be?
    else:
  ...
  @property
  def uop_refcount(self):
    return self.base._uop_refcount
  ...
  def is_initialized(self) -> bool:
    return self.is_allocated() and hasattr(self, "_buf")

  # check if the underlying buffer is allocated, possibly from the base object
  def is_allocated(self) -> bool:
    return self.base.is_allocated() if self._base is not None else hasattr(self, "_buf")

  def ensure_allocated(self) -> Buffer:
    return self.allocate() if not self.is_initialized() else self

  def allocate(self, opaque=None, external_ptr=None) -> Buffer:
    assert not self.is_initialized(), "can't allocate already allocated buffer"
    if DEBUG >= 7:
      print(f"buffer: allocate {self.nbytes} bytes on {self.device}")
    if MAX_BUFFER_SIZE > 0 and self.size > MAX_BUFFER_SIZE:
  ...
  def deallocate(self):
    ...
    if DEBUG is not None and DEBUG >= 7:
      print(f"buffer: deallocate {self.nbytes} bytes on {self.device}")
    if self._base is None and (self.options is None or self.options.external_ptr is None):
      if GlobalCounters is not None and not self.device.startswith("DISK"):
        GlobalCounters.mem_used -= self.nbytes
      if PROFILE:
        Buffer.profile_events.append(ProfilePointEvent(self.device, "free", self._prof_num))
  ...
  def __del__(self):
    (not hasattr(self, "_buf")) or self.deallocate()

  def __repr__(self):
    return (
      f"<buf real:{self.is_allocated()} device:{self.device} size:{self.size} dtype:{self.dtype}"
      + (f" offset:{self.offset}" if self._base is not None else "")
  ...
  def view(self, size: int, dtype: DType, offset: int) -> Buffer:
    ...
    if self._base is not None:
      return Buffer(self.device, size, dtype, base=self._base, offset=self.offset + offset)
...
class Compiler:
  def __init__(self, cachekey: str | None = None):
    self.cachekey = None if DISABLE_COMPILER_CACHE else cachekey
...
def is_dtype_supported(dtype: DType, device: str | None = None) -> bool:
  ...
  if dtype == dtypes.bfloat16:
    ...
    if device in {"CUDA", "NV"}:
      return not CI and not getenv("PTX")
    if device in {"CPU", "LLVM"}:
      return not CI and platform.machine() in {"arm", "arm64", "aarch64", "x86_64", "amd64"}
    return device in {"AMD", "PYTHON"}
  if dtype in dtypes.fp8s:
    # not supported yet - in progress
    return False
  if device == "WEBGPU":
    return dtype in [dtypes.bool, dtypes.char, dtypes.uchar, dtypes.short, dtypes.ushort, dtypes.float, dtypes.int32, dtypes.uint32, dtypes.half]
...
if PROFILE:
  ...
  def finalize_profile():
    ...
    with open(fn := temp("profile.pkl", append_user=True), "wb") as f:
      pickle.dump(cpu_events + Compiled.profile_events + Buffer.profile_events, f)
