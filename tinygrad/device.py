...
class JITRunner:
  ...
  def exec(self, rawbufs:List[Buffer], outbufs:List[Buffer], var_vals:Optional[Dict[Variable, int]]=None) -> Optional[float]:
    var_vals = var_vals if var_vals is not None else {}
    from tinygrad.features.jit import CacheCollector
    et = self(rawbufs, var_vals)
    CacheCollector.add(self, rawbufs, outbufs, var_vals)
    return et
