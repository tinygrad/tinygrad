...
class TinyJit(Generic[ReturnType]):
  ...
  def __call__(self, *args, **kwargs) -> ReturnType:
    ...
    input_tensors: Dict[Union[int, str], Union[LazyBuffer, MultiLazyBuffer]] = { cast(Union[int, str], k):v.realize().lazydata for k,v in itertools.chain(enumerate(args), sorted(kwargs.items(), key=lambda x: str(x[0]))) if v.__class__ is Tensor }  # noqa: E501
