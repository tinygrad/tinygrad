...
def bufs_from_lin(lin:Linearizer) -> List[RawBuffer]:
  ...
  for k,lx in bufsts.items():
    rawbufs[k] = cast(Compiled, Device[Device.DEFAULT]).buffer(prod(lx[0].dtype.shape) if isinstance(lx[0].dtype, ImageDType) else max(1, max(y.st.size() for y in lx)), lx[0].dtype)
