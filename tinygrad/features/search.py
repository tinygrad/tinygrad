...
actions += flatten([[Opt(op=OptOps.UNROLL, axis=axis, amt=amt) for amt in [0,4,8]] for axis in range(4)])
...
def beam_search(lin:Linearizer, baseline_lins:List[Tuple[str, Linearizer]], rawbufs, amt:int, allow_test_size=True) -> Linearizer:
  ...
  if timed[0][2]*1e6 > getenv("BEAM_MIN_TIME", 0):
    ...
    while 1:
      ...
      if len(opts) == 0:
        timed.append((f"beam{amt}", beam[0][0], beam[0][1]))
        break # no more opts
      if (beam[0][1]-opts[0][1])*1e6 < getenv("BEAM_MIN_PROGRESS",0):
        if beam[0][1] > opts[0][1]: timed.append((f"beam{amt}", opts[0][0], opts[0][1]))
        else: timed.append((f"beam{amt}", beam[0][0], beam[0][1]))
        break
