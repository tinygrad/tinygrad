...
class Tensor(OpMixin):
  ...
  def _pool(self, k_:tuple[sint, ...], stride:int|tuple[int, ...]=1, dilation:int|tuple[int, ...]=1) -> Tensor:
    ...
    if all(resolve(d == 1) and resolve(s == k) and resolve(i == o*s) for k,s,d,i,o in zip(k_,s_,d_,i_,o_)):
      x = self.reshape(noop + flatten((o,k) for o,k in zip(o_,k_)))
      return x.permute(*range(len(noop)), *[len(noop)+i*2 for i in range(len(i_))], *[len(noop)+i*2+1 for i in range(len(i_))])
    # input size scaling factor to make sure shrink for stride is possible without padding
    f_ = [ceildiv(o*s, i) if resolve(o*s > i, default=False) else 1 for o,s,i in zip(o_,s_,i_)]
    # # repeats such that we don't need padding - repeat to cover stride shrink size
    ...
    o_ = [smin(o, i*f // s) for o, i, f, s in zip(o_, i_, f_, s_)]
