...
class Tensor(OpMixin):
  ...
  def _pool(self, k_:tuple[sint, ...], stride:int|tuple[int, ...]=1, dilation:int|tuple[int, ...]=1) -> Tensor:
    ...
    def _to_int(v:sint|int) -> int:
      if isinstance(v, int): return v
      uop = sint_to_uop(v)
      return int(_index_to_concrete_int(uop)) if isinstance(uop, UOp) else int(uop)
    f_ = [max(1, ceildiv(max(_to_int(o) * _to_int(s) - _to_int(d), 0), _to_int(i))) for o, s, i, d, k in zip(o_, s_, i_, d_, k_)]
