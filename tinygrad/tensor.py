...
class Tensor(OpMixin):
  ...
  def as_param(self, slot:int): return Tensor(UOp.param(slot, self.dtype, self.shape, self.device), device=self.device)
  def call(self, *lst:Tensor, fxn:Tensor|UOp, grad_fxn:Callable|None=None) -> Tensor:
    return Tensor(UOp.call(*[t.uop for t in (self,)+lst], fxn=fxn.uop if isinstance(fxn, Tensor) else fxn, arg=grad_fxn))
