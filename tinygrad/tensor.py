...
class Tensor:
  ...
  def sparse_categorical_crossentropy(self, Y:Tensor, ignore_index=-1, label_smoothing=0.0) -> Tensor:
    ...
    log_probs, loss_mask = self.log_softmax(), (Y != ignore_index).reshape(-1, 1)
    y_counter = Tensor.arange(self.shape[-1], requires_grad=False, device=self.device).unsqueeze(0).expand(Y.numel(), self.shape[-1])
    y = ((y_counter == Y.flatten().reshape(-1, 1)) * loss_mask).reshape(*Y.shape, self.shape[-1])
    return -((1 - label_smoothing) * log_probs.mul(y).sum() + label_smoothing * (log_probs.mul(loss_mask)).sum() / self.shape[-1]) / loss_mask.sum()
