...
class Tensor:
  ...
  def image_conv2d(self, weight:Tensor, bias:Optional[Tensor]=None, groups=1, stride=1, dilation=1, padding=0, acc_dtype=None):
    base_image_type = dtypes.imageh if (hlf:=getenv("FLOAT16", 0) or self.dtype==dtypes.half) else dtypes.imagef
    ...
    # NCHW output
    ret = ret.reshape(bs, oy, ox, cout).permute(0,3,1,2).cast(dtypes.half if hlf else dtypes.float32)
