...
class Tensor(OpMixin):
  ...
  def image_dot(self, w:Tensor, dtype:DTypeLike|None=None) -> Tensor:
    ...
    cx = self.transpose(-1, -2).reshape(bs//groups, groups*cin, -1, 1)
    # groups*cout x cin x H, W
    cw = w.transpose(-1, -2).reshape(groups*cout, cin, 1, 1)
    return cx.image_conv2d(cw, groups=groups, dtype=dtype).reshape(out_shape_t).transpose(-1, -2)
