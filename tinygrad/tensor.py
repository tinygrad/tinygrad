...
class Tensor(MathTrait):
  ...
  def __setitem__(self, indices, v:Tensor|ConstType) -> None:
    if isinstance(self.device, str) and self.device.startswith("DISK"):
      # make DISK setitem lazy as well (no realize here)
    ...
    # Fast path: 1-D scalar int indexing -> build lazy masked assignment into self
    if isinstance(indices, int) and self.ndim == 1 and isinstance(self.shape[0], int):
      size = int(self.shape[0])
      if indices >= size or indices < -size: raise IndexError(f"{indices=} is out of bounds with {size=}")
      idx = indices if indices >= 0 else indices + size
      val = v.cast(self.dtype)
      mask = (Tensor.arange(size, device=self.device, dtype=dtypes.int32) == idx)
