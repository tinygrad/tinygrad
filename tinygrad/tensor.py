...
class Tensor(OpMixin):
  ...
  @staticmethod
  def const(dtype:DTypeLike, b:ConstType|UOp, device:str|tuple[str, ...]|None=None) -> Tensor:
    return Tensor(b if isinstance(b, UOp) else UOp.const(to_dtype(dtype), b, device))
  @staticmethod
