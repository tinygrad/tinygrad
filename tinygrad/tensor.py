...
def _frompy(x: list | tuple | bytes, dtype: DType) -> UOp:
    if isinstance(x, bytes):
        ...
        ret = UOp.new_buffer("PYTHON", prod(
            shape := get_shape(x)), dtype).reshape(shape)
        assert dtype.fmt is not None, f"{dtype=} has None fmt"
        truncate_function = truncate[dtype]
        data = struct.pack(f"{ret.size}{dtype.fmt}", *[truncate_function(
            dtypes.as_const(xi, dtype)) for xi in fully_flatten(x)])
    # fake realize
    ret.buffer.allocate(memoryview(data if Device.DEFAULT !=
                        "PYTHON" else bytearray(data)))
    return ret
...
def _apply_winograd_matrix(mat, t: Tensor, dims: int) -> Tensor:
    ...
    t_ = t.reshape(t.shape[:dims] + (1,) * dims + t.shape[dims:]).expand(
        t.shape[:dims] + (len(mat),) * dims + t.shape[dims:])  # add output dims
    # precalculate mat columns for each dim; prod(itertools.product(matcols)) gives the columns of kron(mat, mat, ...)
    matcols = _get_winograd_matcols(
        mat, dims, t_.shape[dims:], t_.device, t_.dtype)
...
def _masked_setitem(target: Tensor, values: Tensor, mask: Tensor, axes: tuple[int, ...]) -> Tensor:
    ...
    for dim in reversed(axes):
        mask, values = functools.reduce(lambda x, y: (x[0] | y[0], y[0].where(
            y[1], x[1])), zip(mask.split(1, dim), values.split(1, dim)))
...
def _flat_to_grouped(padding: Sequence[sint]) -> tuple[tuple[sint,
                                                             sint], ...]: return tuple(zip(padding[-2::-2], padding[::-2]))
...
class Tensor(MathTrait):
    ...
    def __init__(self, data: ConstType | bytes | list | tuple | UOp | 'np.ndarray' | pathlib.Path | None,  # type: ignore [name-defined] # noqa: F821
        ...
        if device is None and isinstance(data, pathlib.Path):
            # keep it on the disk if device is None
            device = f"DISK:{data.resolve()}"
        device = tuple(Device.canonicalize(x) for x in device) if isinstance(
            device, (tuple, list)) else Device.canonicalize(device)
        ...
        if isinstance(data, UOp):
            ...
            if data.op is Ops.BIND:  # type: ignore  # mypy type narrowing is bugged here
                ...
                data = data.replace(
                    src=(var.replace(src=const.src), const))  # type: ignore
        elif data is None:
            data = UOp.const(dtype or dtypes.default_float, 0, device, ())
        elif isinstance(data, get_args(ConstType)):
            data = UOp.const(dtype or dtypes.from_py(data), data, device, ())
            ...
            if dtype in [dtypes.bfloat16, *dtypes.fp8s]:
                data = Tensor(_frompy(data, dtypes.float32),
                              device=device).cast(dtype).uop
            else:
                data = _frompy(data, dtype)
        elif is_numpy_ndarray(data):
            import numpy as np
            assert isinstance(
                data, np.ndarray), f"expected np.ndarray, got {data}"
            if data.shape == ():
                data = UOp.const(dtype or _from_np_dtype(
                    data.dtype), data.item(), device, ())
            else:
                data = _fromnp(data.astype(npdtype) if dtype is not None and (
                    npdtype := _to_np_dtype(dtype)) is not None else data)  # type: ignore [name-defined]
        ...
        if isinstance(device, str):
            self.uop: UOp = data if data.device == device else data.copy_to_device(
                device)
        # if device is a tuple, we should have/construct a MultiLazyBuffer
        elif isinstance(data.device, str):
            self.uop = Tensor(data).shard(device).uop
        else:
    ...
    def _binop(self, op, x, reverse): return self._apply_broadcasted_uop(
    ...
    class train(ContextDecorator):
        ...
        def __enter__(
            self): self.prev, Tensor.training = Tensor.training, self.mode
        def __exit__(self, exc_type, exc_value,
                     traceback): Tensor.training = self.prev
    ...
    def kernelize(self, *lst: Tensor) -> Tensor:
        ...
        if any(isinstance(x._device, tuple) for x in big_sink.toposort()):
            ...
            big_sink = UOp.sink(
                *flatten([x.uop.src if x.uop.op is Ops.MULTI else [x.uop] for x in (self,)+lst]))
    ...
    def realize(self, *lst: Tensor, do_update_stats=True) -> Tensor:
        ...
        run_schedule(*self.schedule_with_vars(*lst),
                     do_update_stats=do_update_stats)
        return self
    ...
    def data(self) -> memoryview:
        ...
        assert all_int(
            self.shape), f"no data if shape is symbolic, {self.shape=}"
    ...
    def shard(self, devices: tuple[str, ...], axis: int | None = None) -> Tensor:
        ...
        mlb = self.uop.shard(devices, self._resolve_dim(
            axis)) if axis is not None else self.uop.copy_to_device(devices)
    ...
    def load(self, size: int) -> Tensor:
        ...
        if i > 0 or tree_depth == 0:
            level_chunks = max(1, math.ceil(
                base_chunks / (Tensor.CHUNK_SIZE // 16)**(i-1)))
    ...
    def store(self) -> Tensor:
        ...
        to_device = "CPU" if isinstance(
            self.device, str) and self.device.startswith("DISK") else self.device

        level_chunks = base_chunks
        for _ in range(tree_depth + 1):
            data = data.to("tinyfs:store")[
                :level_chunks * 16].contiguous().to(to_device)
        if (tsize := data.shape[0]) % Tensor.CHUNK_SIZE != 0:
            data = data.pad((0, Tensor.CHUNK_SIZE - tsize % Tensor.CHUNK_SIZE))
    ...
    def empty(*shape, device: str | tuple[str, ...] | None = None, dtype: DTypeLike | None = None, **kwargs) -> Tensor:
        ...
        ```
        """
        dtype, shape = to_dtype(
            dtype) if dtype is not None else dtypes.default_float, argfix(*shape)
        ...
        device = tuple(Device.canonicalize(d) for d in device) if isinstance(
            device, tuple) else Device.canonicalize(device)
    ...
    def _threefry_random_bits(key: Tensor, counts0: Tensor, counts1: Tensor) -> Tensor:
        ...
        x = x._apply_uop(UOp.threefry, (key[1]._broadcast_to(x.shape).cast(
            dtypes.uint64) << 32) | key[0]._broadcast_to(x.shape).cast(dtypes.uint64))
        counts0, counts1 = (x & 0xffffffff).cast(
            dtypes.uint32), ((x >> 32) & 0xffffffff).cast(dtypes.uint32)
    ...
    def rand(*shape, device: str | None = None, dtype: DTypeLike | None = None, contiguous: bool = True, **kwargs) -> Tensor:
        ...
        if device is not None and not isinstance(device, str):
            raise ValueError(
                f"rand only supports single device, got {device=}")
        ...
        if device not in Tensor._device_seeds:
            Tensor._device_seeds[device] = Tensor(
                [int.from_bytes(hashlib.sha256(len(Tensor._device_seeds).to_bytes(
                    4, "big")).digest(), "big"), Tensor._seed],
        ...
        counts0 = (Tensor.arange(ceildiv(num, 2), device=device,
                   dtype=dtypes.uint32, requires_grad=False)+bits_count)
        counts1 = counts0 + ceildiv(num, 2)
        bits = Tensor._threefry_random_bits(
            Tensor._device_seeds[device], counts0, counts1)[:num]

        # bitcast to uint with same number of bits
        _, nmant = dtypes.finfo(dtype)
        uint_dtype = {1: dtypes.uint8, 2: dtypes.uint16,
                      4: dtypes.uint32, 8: dtypes.uint64}[dtype.itemsize]
        bits = bits.bitcast(uint_dtype)
        # only randomize the mantissa bits and set the exponent to 1
        one = Tensor.ones_like(bits, device=bits.device,
                               dtype=dtype).bitcast(uint_dtype)
        bits = bits.rshift((dtype.itemsize * 8) - nmant).bitwise_or(one)
        # bitcast back to the original dtype and reshape
        out = bits.bitcast(dtype)[:numel].sub(1).reshape(
            shape).requires_grad_(kwargs.get("requires_grad"))
    ...
    def arange(start, stop=None, step=1, **kwargs) -> Tensor:
        ...
        if stop is None:
            stop, start = start, 0
        dtype = kwargs.pop("dtype", dtypes.default_float if any(isinstance(
            x, float) for x in (start, stop, step)) else dtypes.default_int)
    ...
    def rand_like(self, **kwargs) -> Tensor:
        ...
        if isinstance(self.device, tuple):
            ...
            sharded_shape = tuple(
                s//len(self.device) if a == self.uop.axis else s for a, s in enumerate(self.shape))
    ...
    def multinomial(self: Tensor, num_samples: int = 1, replacement: bool = False) -> Tensor:
        ...
        unif_samples = Tensor.rand(
            num_samples, cdf.shape[0], 1).to(self.device)
        indices = (unif_samples.expand(
            (-1, -1, cdf.shape[1])) >= cdf).sum(2).permute((1, 0))
    ...
    def reshape(self, shape, *args) -> Tensor:
        ...
        new_shape = tuple([s if s is not None else self.shape[i]
        ...
        if resolve(prod(self.shape) != prod(new_shape), True):
            raise ValueError(
                f"size mismatch, can't reshape ({', '.join(srender(d) for d in self.shape)}) -> ({', '.join(srender(d) for d in new_shape)})")
    ...
    def expand(self, shape, *args) -> Tensor:
        ...
        ```
        """
        new_shape = tuple(from_ if to == -1 or to is None else to for from_,
                          to in zip(*(_align_left(self.shape, argfix(shape, *args)))))
    ...
    def permute(self, order, *args) -> Tensor:
        ...
        if sorted(order_arg) != list(range(self.ndim)):
            raise RuntimeError(
                f"order is not a valid permutation, getting {order_arg}")
    ...
    def flip(self, axis, *args) -> Tensor:
        ...
        if len(axis_arg) != len(dedup(axis_arg)):
            raise RuntimeError(
                f"dim can appear at most once, getting {axis_arg}")
    ...
    def pad(self, padding: Sequence[sint] | Sequence[tuple[sint, sint] | None], mode: str = "constant", value: float = 0.0) -> Tensor:
        ...
        # group padding
            ...
            pX = tuple((0, 0) if p is None else p for p in cast(
                Sequence[tuple[sint, sint] | None], padding))
        if len(pX) != self.ndim:
            raise ValueError(
                f"padding length is improper, {padding=} {self.ndim=}")
        ...
        if mode == "constant":
            ...
            return _constant(X, pX, value) if all(resolve(p >= 0) for p in flatten(pX)) else \
                _constant(X.shrink(tuple((-smin(pB, 0), smin(pA+s, s))
                          for (pB, pA), s in zip(pX, X.shape))), pads, value)
        assert all_int(
        ...
        for d, (pB, pA) in enumerate(pads):
            if mode == "reflect":
                ...
                slcB, slcA, = slice(pB, 0, -1), slice(s-2 if s-2 >=
                                                      0 else None, s-2-pA if s-2-pA >= 0 else None, -1)
                xB, xA = (X[[slc if i == d else slice(None) for i in range(
                    X.ndim)]] if p > 0 else None for slc, p in ((slcB, pB), (slcA, pA)))
            if mode == "replicate":
                shrB, shrA, = tuple((0, 1) if i == d else None for i in range(X.ndim)), tuple(
                    (X.shape[i]-1, X.shape[i]) if i == d else None for i in range(X.ndim))
                xB, xA = (X.shrink(shr).expand(tuple(p if i == d else None for i in range(
                    X.ndim))) if p > 0 else None for shr, p in ((shrB, pB), (shrA, pA)))
            X = Tensor.cat(*(X_ for X_ in (xB, X, xA)
                           if X_ is not None), dim=d)
        return X.shrink(tuple((-min(pB, 0), min(pA+s, s)) for (pB, pA), s in zip(pX, X.shape)))

    # convenience
    def pad_to(self, shape, *args): return self.pad(tuple(
        [(0, ns-s) for s, ns in itertools.zip_longest(self.shape, argfix(shape, *args))]))

    def shrink_to(self, shape, *args): return self.shrink(
        tuple([(0, ns) for ns in argfix(shape, *args)]))
    ...
    def _getitem(self, indices, v: Tensor | None = None) -> Tensor:
        ...
        num_indices = len(indices) - len(ellipsis_idx) - \
            sum(1 for i in indices if i is None)
        if num_indices > self.ndim:
            raise IndexError(f"too many {num_indices=} for {self.ndim=}")
        fill_idx = ellipsis_idx[0] if ellipsis_idx else len(indices)
        indices[fill_idx:fill_idx +
                1] = [slice(None)] * (self.ndim - num_indices)
            ...
            match index:
                case Tensor():
                    if not dtypes.is_int(index.dtype):
                        raise IndexError(
                            f"index dtype {index.dtype} is not supported")
                    # treat negative index values
                    index = (index < 0).where(
                        index+size, index).to(self.device)
                case list() | tuple():
                    if not dtypes.is_int((ti := Tensor(index)).dtype):
                        raise IndexError(f"{index=} contains non-int element")
                    index = Tensor([i+size if i < 0 else i for i in fully_flatten(index)],
                                   self.device, requires_grad=False).reshape(ti.shape)
                case int() | UOp():  # sint
                    if index >= size or index < -size:
                        raise IndexError(
                            f"{index=} is out of bounds with {size=}")
                    # TODO: is this right for (negative) symbolic?
                    boundary = [index, index +
                                1] if index >= 0 else [index+size, index+size+1]
                case slice():
                    if index.step == 0:
                    ...
                    if all(isinstance(s, int) for s in (start, stop, step)):
                        ...
                        *boundary, stride = index.indices(cast(SupportsIndex, size))
                        ...
                        size = ceildiv(
                            (boundary[1] - boundary[0]), abs(stride))
        ...
        if mops := [i for i in indices_parsed if i['index'] is not None]:
            ...
            shrinks, strides = zip(
                *((i['boundary'], i['stride']) for i in mops))
            ...
            if any(st != 1 for st in strides):
                ...
                x = x.pad(tuple((0, round_up(s, st) - s)
                          for s, st in zip(x.shape, strides)))
                x = x.reshape(tuple(flatten((s // st, st)
                              for s, st in zip(x.shape, strides))))
                x = x.shrink(tuple(flatten(((0, s), (0, 1))
                             for s in x.shape[::2]))).reshape(x.shape[::2])

        # dim injection from None by including None dim size (which is 1) and dim collapse by skipping int dim size
        x = x.reshape(tuple(index['size'] for index in indices_parsed if not isinstance(
            index['index'], sint)))
        ...
        if tops := [(d, i) for d, i in enumerate(i_ for i_ in indices_parsed if not isinstance(i_['index'], int)) if isinstance(i['index'], Tensor)]:
            ...
            dims, tensors, masks = [d for d, _ in tops], cast(
                list[Tensor], [i['index'] for _, i in tops]), []
            pre_reduce_shape = x.shape[:dims[0]] + (big_shape := _broadcast_shape(
                *(t.shape for t in tensors))) + x.shape[dims[0]:]

            # create index masks
            for dim, tensor in zip(dims, tensors):
                try:
                    i = tensor.reshape(
                        tensor.shape + (1,)*(x.ndim - dims[0])).expand(pre_reduce_shape)
                except ValueError as e:
                    raise IndexError(f"cannot broadcast indices: {e}") from e
                masks.append(i._one_hot_along_dim(
                    num_classes=x.shape[dim], dim=(dim - x.ndim)))

            # reduce masks to 1 mask
            mask: Tensor = functools.reduce(lambda x, y: x.mul(y), masks)

            # inject 1's for the extra dims added in create masks
            reshape_arg = x.shape[:dims[0]] + \
                (1,) * len(big_shape) + x.shape[dims[0]:]
            # sum reduce the extra dims introduced in create masks
            x = (mask.where(x.reshape(reshape_arg), 0)).sum(sum_axis :=
                                                            tuple(d + len(big_shape) for d in dims), dtype=x.dtype)

            # special permute case
            if (permuted := dims[0] != 0 and len(dims) != 1 and tuple(dims) != tuple(range(dims[0], dims[-1]+1))):
                mask, x = (y.permute(*range(dims[0], dims[0]+len(big_shape)), *range(
                    0, dims[0]), *range(dims[0]+len(big_shape), y.ndim)) for y in (mask, x))
            ...
            if v is not None:
                ...
                x = _masked_setitem(self, vb, mask, tuple(
    ...
    def __setitem__(self, indices, v: Tensor | ConstType) -> None:
        ...
        if res.shape == self.shape and res.uop is not self.uop:
            ...
            v = v.cast(res.dtype)._broadcast_to(
                _broadcast_shape(res.shape, v.shape)).contiguous()
            res.assign(v)
    ...
    def gather(self: Tensor, dim: int, index: Tensor) -> Tensor:
        ...
        assert all(s >= i for d, (s, i) in enumerate(zip(self.shape, index.shape))
                   if d != dim), "requires self.shape[d] >= index.shape[d] for all d != dim"
        index = index.to(self.device)
        x = self.shrink(tuple((0, i) if d != dim else None for d, i in enumerate(
            index.shape))).unsqueeze(-1).transpose(-1, dim)
    ...
    def cat(self: Tensor, *args: Tensor, dim: int = 0) -> Tensor:
        ...
        tensors = [self, *args]
        dim_cumsum = list(itertools.accumulate(
            [t.shape[dim] for t in tensors], initial=0))
    ...
    def repeat_interleave(self, repeats: int, dim: int | None = None) -> Tensor:
        ...
        ```
        """
        x, dim = (self.flatten(), 0) if dim is None else (
            self, self._resolve_dim(dim))
        shp = x.shape
    ...
    def _resolve_dim(self, dim: int, *, extra: bool = False) -> int:
        ...
        if not -max(1, total) <= dim <= max(1, total)-1:
            raise IndexError(
                f"{dim=} out of range {[-max(1, total), max(1, total)-1]}")
    ...
    def split(self, sizes: int | Sequence[int], dim: int = 0) -> tuple[Tensor, ...]:
        ...
        if isinstance(sizes, int):
            sizes = [min(sizes, self.shape[dim]-i)
                     for i in range(0, max(1, self.shape[dim]), max(1, sizes))]
        assert sum(
            sizes) == self.shape[dim], f"expect sizes to sum exactly to {self.shape[dim]}, but got {sum(sizes)}"
    ...
    def meshgrid(self: Tensor, *args: Tensor, indexing: Literal["ij", "xy"] = "ij") -> tuple[Tensor, ...]:
        ...
        if indexing not in ("ij", "xy"):
            raise RuntimeError(
                f'indexing must be in ("ij", "xy"), got {indexing}')
        if len(tensors := (self, *args)) == 1:
            return tensors
        basis = tuple(range(len(tensors))) if indexing == "ij" else (
            1, 0) + tuple(range(2, len(tensors)))
        tensors = tuple(t.reshape((-1,) + (1,)*(len(args) - i))
                        for i, t in zip(basis, tensors))
    ...
    def flatten(self, start_dim=0, end_dim=-1) -> Tensor:
        ...
        ```
        """
        start_dim, end_dim = self._resolve_dim(
            start_dim), self._resolve_dim(end_dim)
    ...
    def diagonal(self) -> Tensor:
        ...
        if self.ndim != 2 or (n := self.shape[0]) != self.shape[1]:
            raise ValueError(
                f"only 2-D square tensor is supported, getting {self.shape=}")
    ...
    def roll(self, shifts: int | tuple[int, ...], dims: int | tuple[int, ...] | None = None) -> Tensor:
        ...
        dims, shifts, slices = tuple(self._resolve_dim(d) for d in make_tuple(
            dims, 1)), make_tuple(shifts, 1), [slice(None)] * self.ndim
        if len(dims) != len(shifts):
            raise RuntimeError(f"{len(dims)=} != {len(shifts)=}")
        for dim, shift in zip(dims, shifts):
            slices[dim] = slice(delta := self.shape[dim]-shift %
                                self.shape[dim], delta+self.shape[dim])
    ...
    def rearrange(self, formula: str, **sizes) -> Tensor:
        ...
        def parse_formula(formula: str):
            tokens = f" {formula} ".replace("…", "...").replace(
                "(", " ( ").replace(")", " ) ").replace(" ", "  ").replace(" 1 ", " ( ) ").split()
            lparens, rparens = map(
                lambda x: [i for i, ch in enumerate(tokens) if ch == x], ("(", ")"))
            pairs = list(zip(lparens, rparens))
            assert len(lparens) == len(rparens) and sorted(
                flatten(pairs)) == flatten(pairs), "bracket mismatch"
        ...
        assert sorted(lhs) == sorted(rhs) and len(lhs) == len(
            set(lhs)), f"name mismatch in {formula}"
        for name in flatten((lhs, rhs)):
            assert name == "..." or (name.isidentifier() and "_" not in (
                name[0], name[-1])), f"invalid axis name {name}"
        ...
        # resolve ellipsis
        if "..." in lhs:
            ell_len = len(self.shape) - len(lhs) + 1 + \
                sum(e - s - 1 for s, e in unflatten_dims)
        lhs, rhs = map(lambda l: l[:(i := l.index("..."))] + [f"...{j}" for j in range(
            ell_len)] + l[i + 1:] if "..." in l else l, (lhs, rhs))
        unflatten_dims = [(s + (ell_len - 1 if "...0" in lhs[:s] else 0), e +
                           (ell_len - 1 if "...0" in lhs[:e] else 0)) for s, e in unflatten_dims]
        flatten_dims = [(s + (ell_len - 1 if "...0" in rhs[:s] else 0), e +
                         (ell_len - 1 if "...0" in rhs[:e] else 0)) for s, e in flatten_dims]

        # apply movement ops in order unflatten -> permute -> flatten/unsqueeze
        t = functools.reduce(lambda x, dims: x.unflatten(dims[0], tuple(
            sizes.get(lhs[d], -1) for d in range(*dims))), unflatten_dims, self)
        for i, name in enumerate(lhs):
            assert (
                name not in sizes) or sizes[name] == t.shape[i], f"size provided for dimension {name} incorrect"
    ...
    def _reduce(self, op: Ops, axis: int | Sequence[int] | None = None, keepdim=False) -> Tensor:
        axis = tuple(self._resolve_dim(x) for x in (
            range(self.ndim) if axis is None else make_tuple(axis, 1)))
        if self.ndim == 0:
            axis = ()
    ...
    def _inverse(self) -> Tensor: return -self if self.is_floating_point(
    ...
    def isclose(self, other: Tensor, rtol: float = 1e-05, atol: float = 1e-08, equal_nan=False) -> Tensor:
        ...
        ```
        """
        is_finite_close = self.isfinite() & other.isfinite() & (
    ...
    def mean(self, axis: int | Sequence[int] | None = None, keepdim=False) -> Tensor:
        ...
        ```
        """
        output_dtype = self.dtype if dtypes.is_float(
            self.dtype) else dtypes.float32
        numerator = self.cast(sum_acc_dtype(self.dtype)).sum(
    ...
    def var(self, axis: int | Sequence[int] | None = None, keepdim=False, correction=1) -> Tensor:
        ...
        n = prod([si for si, so in zip(self.shape, squares.sum(
            axis=axis, keepdim=True).shape) if resolve(si != so)])
    ...
    def keccak(self, cfg: str | tuple[int, int] = "sha3_256"):
        ...
        rot_offsets = [44, 43, 21, 14, 28, 20, 3, 45, 61, 1, 6,
                       25, 8, 18, 27, 36, 10, 15, 56, 62, 55, 39, 41, 2]
        rot_offsets_v0, rot_offsets_v1 = ctensor(
            [0] + [1 << v for v in rot_offsets]), ctensor([1] + [1 << (64 - v) for v in rot_offsets])

        # calculated from π step
        reorder_indexes = ctensor([0, 6, 12, 18, 24, 3, 9, 10, 16, 22, 1, 7,
                                  13, 19, 20, 4, 5, 11, 17, 23, 2, 8, 14, 15, 21], dtype=dtypes.int32)
        ...
        rate, dsbyte = {"sha3_224": (144, 6), "sha3_256": (136, 6), "shake_128": (168, 31)}[
            cfg] if isinstance(cfg, str) else cfg
        data, data_pad = self.bitcast(dtypes.uint8).reshape(prod(
            self.shape[:-1]), self.shape[-1]), rate - (self.shape[-1] * self.dtype.itemsize % rate)
        # pad batches then pad blocks
        data = data.pad((None, (0, data_pad))).reshape(
            bs := data.shape[0], -1, rate).pad((None, None, (0, 200 - rate)))
        ...
        if data_pad == 1:
            ...
            mb = [(lbe, 0), (1, dsbyte), (data_pad - 2, 0),
                  (1, 0x80), (200 - rate, 0)]
        pad_mask = Tensor.cat(*(Tensor(v, dtype=dtypes.uint8, device=data.device).expand(l)
                              for l, v in mb if l > 0)).unsqueeze(0)

        data = (data.flatten(1) ^ pad_mask).reshape(
            *data.shape[:2], 200).bitcast(dtypes.uint64)
        ...
        for k in range(int(data.shape[1])):
            ...
            for i in range(24):  # f1600
                ...
                t1 = (p[:, :, 0] ^ p[:, :, 1] ^ p[:, :, 2] ^ p[:, :, 3]
                      ^ p[:, :, 4]).roll(-1, 1)  # xor reduce
                state = state ^ (t1.roll(2, 1).bitwise_xor((t1 << 1) ^ (t1 >> 63)).unsqueeze(
                    2).expand(bs, 5, 5).transpose(2, 1).flatten(1))
                # ρ and π steps
                state = state[:, reorder_indexes]
                state = (state * rot_offsets_v0).bitwise_or(state //
                                                            rot_offsets_v1).reshape(bs, 5, 5)
                # χ and ι step
                state = state.bitwise_xor(~state.roll(
                    shifts=-1, dims=2) & state.roll(shifts=-2, dims=2))
    ...
    def logcumsumexp(self, axis=0) -> Tensor:
        ...
        x_unsqueezed = x.unsqueeze(-2).expand((None,)
                                              * (self.ndim-1)+(last_dim_size, None))
        x_cummax = x.cummax(-1)
        mask = Tensor.ones(last_dim_size, last_dim_size,
                           requires_grad=False, device=self.device).tril()
        ret = mask.where(x_unsqueezed - x_cummax.unsqueeze(-1),
                         dtypes.min(self.dtype)).exp().sum(-1).log() + x_cummax
    ...
    def argmax(self, axis=None, keepdim=False) -> Tensor:
        ...
        idx = m * Tensor.arange(self.shape[axis], 0, -1, requires_grad=False,
                                device=self.device).reshape(self.shape[axis], *[1]*(self.ndim-axis-1))
    ...
    def einsum(formula: str, *operands: Tensor | Sequence[Tensor], dtype: DTypeLike | None = None) -> Tensor:
        ...
        def parse_formula(formula: str, *operands: Tensor):
            if "..." in (formula := formula.replace(" ", "")):
                ell_chars, ell_longest = "".join(
                    c for c in string.ascii_letters if c not in formula), 0
                ...
                inputs_str, out_ellipse = ",".join(
                    inputs), ell_chars[-ell_longest:]
                return (inputs_str, formula.split("->")[1].replace("...", out_ellipse)) if "->" in formula else \
                    (inputs_str, out_ellipse + ''.join(sorted(c for c in inputs_str if inputs_str.count(
                        c) == 1 and c.isalpha() and c not in out_ellipse)))
        ...
        for x, (order, letters) in zip(xs, [list(zip(*l)) for l in lhs]):
            ...
            xs_.append(x.permute(order).reshape(
                [val if letter in letters else 1 for letter, val in letter_val]).expand([val for _, val in letter_val]))
    ...
    def _pool(self, k_: tuple[sint, ...], stride: int | tuple[int, ...] = 1, dilation: int | tuple[int, ...] = 1) -> Tensor:
        ...
        if any(resolve(k > s) for k, s in zip(k_, s_)) or any(d != 1 for d in d_):
            ...
            f_ = [1 + int(resolve(o*s > (i - d*(k-1))))
                  for o, s, i, d, k in zip(o_, s_, i_, d_, k_)]
            # # repeats such that we don't need padding
            x = self.repeat([1]*len(noop) + [ceildiv(k*(i*f+d), i)
        ...
        x = self.pad(tuple(noop + [(0, max(0, o*s-i)) for i, o, s in zip(i_, o_, s_)])
                     ).shrink(tuple(noop + [(0, o*s) for o, s in zip(o_, s_)]))
        x = x.reshape(noop + flatten(((o, s) for o, s in zip(o_, s_))))
        x = x.shrink(tuple(noop + flatten(((0, o), (0, k))
                     for o, k in zip(o_, k_))))
    ...
    def _apply_ceil_mode(self, pads: Sequence[int], k_: tuple[sint, ...], s_: int | tuple[int, ...], d_: int | tuple[int, ...]) -> list[int]:
        (d_, s_), i_ = (make_tuple(x, len(k_))
                        for x in (d_, s_)), self.shape[-len(k_):]
        pads, grouped_pads = list(pads), _flat_to_grouped(pads)
        # https://arxiv.org/pdf/1603.07285 section 5.1, relationship 15.
        o_ = [ceildiv(i+pB+pA - (d*(k-1)+1), s) + 1 for i, d, k,
              s, (pB, pA) in zip(i_, d_, k_, s_, grouped_pads)]
        for dim, (o, i, s, k, d, (pB, pA)) in enumerate(zip(o_, i_, s_, k_, d_, grouped_pads)):
            ...
            pads[-1-dim*2] += s*(o-1) + (d*(k-1)+1) - \
                (i+pB+pA) - smax(s*(o-1) - (pB+i-1), 0)
        return pads
    ...
    def avg_pool2d(self, kernel_size: tuple[int, ...] = (2, 2), stride=None, dilation=1, padding: int | tuple[int, ...] = 0,
        ...
        def pool(x: Tensor, padding_: Sequence[int]) -> Tensor: return x.pad(
            padding_)._pool(k_, stride if stride is not None else k_, dilation)
    ...
    def max_pool2d(self, kernel_size: tuple[int, ...] = (2, 2), stride=None, dilation=1, padding: int | tuple[int, ...] = 0,
        ...
        idx = Tensor.arange(spatial_sz, 0, -1, requires_grad=False,
                            device=self.device).reshape(spatial_shape)
        m = pooled == pooled.max(axis, keepdim=True)
        idx = m * idx.pad(pads, value=dtypes.min(idx.dtype))._pool(k_,
                                                                   stride if stride is not None else k_, dilation)
    ...
    def max_unpool2d(self, indices: Tensor, kernel_size: tuple[int, ...] = (2, 2), stride=None, dilation=1, padding: int | tuple[int, ...] = 0, output_size=None):
        ...
        if output_size is None:
            k_, d_, s_ = (make_tuple(x, len(spatial_shape)) for x in (
                kernel_size, dilation, stride if stride is not None else kernel_size))
            p_ = _flat_to_grouped(self._resolve_pool_pads(
                padding, len(spatial_shape)))
            # https://arxiv.org/pdf/1603.07285 inverse of relationship 15 in section 5.1.
            output_size = tuple((i-1)*s - (pB+pA) + (d*(k-1)+1)
                                for i, k, d, s, (pA, pB) in zip(spatial_shape, k_, d_, s_, p_))
        else:
            output_size = output_size[-len(spatial_shape):]
        ret = (indices.reshape(bs, c, 1, -1)._one_hot_along_dim(prod(output_size),
               2).where(self.reshape(bs, c, 1, -1), 0)).sum(3)
    ...
    def conv2d(self, weight: Tensor, bias: Tensor | None = None, groups=1, stride=1, dilation=1, padding: int | tuple[int, ...] = 0,
        ...
        x = self.pad(padding_)._pool(HW, stride, dilation)
        ...
        winograd_G = [[1/4, 0, 0], [-1/6, -1/6, -1/6], [-1/6, 1 /
                                                        6, -1/6], [1/24, 1/12, 1/6], [1/24, -1/12, 1/6], [0, 0, 1]]
        winograd_Bt = [[4, 0, -5, 0, 1, 0], [0, -4, -4, 1, 1, 0], [0, 4, -4, -1,
                                                                   1, 0], [0, -2, -1, 2, 1, 0], [0, 2, -1, -2, 1, 0], [0, 4, 0, -5, 0, 1]]
        winograd_At = [[1, 1, 1, 1, 1, 0], [0, 1, -1, 2, -2, 0], [0, 1, 1, 4, 4, 0],
                       [0, 1, -1, 8, -8, 1]]  # applying At in pre-order doubles compile time

        # TODO: stride == dilation
        # use padding to round up to 4x4 output tiles
        # (bs, cin_, tyx, HWI)
        pads = [[padding_[i*2], padding_[i*2+1] + (-(dim + sum(padding_[i * 2:(
            i + 1) * 2]) - 2) % 4)] for i, dim in enumerate(self.shape[-len(HW):])]
        d = self.pad(sum(pads, []))._pool(HWI, HWO)
        # move HW to the front: # (HWI, bs, cin_, tyx)
        d = d.permute(*range(len(d.shape)-len(HW), len(d.shape)),
                      *range(len(d.shape)-len(HW)))
        tyx = d.shape[-len(HWI):]  # dim of tiling

        g = weight.permute(*range(len(weight.shape)-len(HW), len(weight.shape)),
        ...
        dfactors = _apply_winograd_matrix(winograd_Bt, d, len(
            HW)).reshape(*HWI, bs, groups, 1, cin, *tyx)
        ...
        ret = ret.permute([*range(len(HW), len(ret.shape)-len(HW)), *
                          [i+o for i in range(len(HW)) for o in [len(ret.shape)-len(HW), 0]]])
        # merge groups and rcout, tyx and HWO: (bs, groups, cout, *yx), shrink to final
        ret = ret.reshape(bs, cout, *[c * HWO[i] for i, c in enumerate(tyx)]
                          ).shrink(tuple((0, s) for s in [bs, cout, *oyx]))
    ...
    def conv_transpose2d(self, weight: Tensor, bias: Tensor | None = None, groups=1, stride=1, dilation=1, padding=0, output_padding=0) -> Tensor:
        ...
        ```
        """
        x, w = self, weight.unflatten(
            0, (groups, -1)).transpose(1, 2).flip(*range(3, len(weight.shape)+1))
        HW = weight.shape[2:]
        padding = _flat_to_grouped(self._resolve_pool_pads(padding, len(HW)))
        stride, dilation, output_padding = [make_tuple(
            x, len(HW)) for x in (stride, dilation, output_padding)]
        if any(s > 1 for s in stride):
            # handle strides: (k) -> reshape -> (k,1) -> pad -> (k,s) -> reshape -> (k*s) -> shrink (k-(s-1))
            x = x.reshape(None, None, *flatten((k, 1) for k in x.shape[2:]))
            x = x.pad((None, None, *flatten((None, (0, s-1)) for s in stride)))
            x = x.reshape(
                None, None, *[k*s for k, s in zip(x.shape[2::2], stride)])
            x = x.shrink((None, None, *[(0, k-(s-1))
                         for k, s in zip(x.shape[2:], stride)]))
        padding = flatten((((k-1)*d-pB, (k-1)*d-pA+op) for k, d, (pB, pA),
                          op in reversed(list(zip(HW, dilation, padding, output_padding)))))
    ...
    def dot(self, w: Tensor, dtype: DTypeLike | None = None) -> Tensor:
        ...
        w = w.reshape(*w.shape[0:-2], *[1]*min(dx-1, dw-1, 1),
    ...
    def _cumalu(self, axis: int, op: Ops, _include_initial=False) -> Tensor:
        ...
        pooled = self.transpose(axis, -1).pad((pl_sz, -int(_include_initial)),
                                              value=identity_element(op, self.dtype))._pool((self.shape[axis],))
    ...
    def _split_cumalu(self, axis: int, op: Ops) -> Tensor:
        ...
        ret = self.transpose(axis, -1).pad((round_up(s, SPLIT)-s, 0), value=identity_element(
            op, self.dtype)).unflatten(-1, (-1, SPLIT))._cumalu(-1, op)
        base = ret[..., -1]._cumalu(-1, op, _include_initial=True)
        base = base.unsqueeze(-1).expand(*base.shape, ret.shape[-1])
        def fix(x: Tensor) -> Tensor: return x.flatten(start_dim=-
                                                       2)[..., -s:].transpose(axis, -1)
    ...
    def _tri(r: sint, c: sint, diagonal: int = 0, **kwargs) -> Tensor:
        assert isinstance(r, int) and isinstance(
            c, int), f"does not support symbolic, getting {r=}, {c=}"
        ...
        t = Tensor.ones(s, s, **kwargs).pad((None, (0, s))).flatten().shrink(
            ((0, s*(2*s-1)),)).reshape(s, -1).shrink((None, (0, s)))
    ...
    def interpolate(self, size: tuple[int, ...], mode: str = "linear", align_corners: bool = False) -> Tensor:
        ...
        ```
        """
        assert isinstance(size, (tuple, list)) and all_int(
            size) and 0 < len(size) <= self.ndim, f"invalid {size=}"
        assert mode in (
            "linear", "nearest", "nearest-exact"), "only supports linear, nearest or nearest-exact interpolate"
        assert not (align_corners and mode !=
                    "linear"), "align_corners option can only be set with the interpolating mode linear"
        x, expand = self, list(self.shape)
        for i in range(-1, -len(size)-1, -1):
            scale = (self.shape[i] - int(align_corners)) / \
                (size[i] - int(align_corners))
            arr, reshape = Tensor.arange(
                size[i], dtype=dtypes.float32, device=self.device), [1] * self.ndim
            reshape[i] = expand[i] = size[i]
            if mode == "linear":
                index = (scale*arr if align_corners else (scale *
                         (arr+0.5))-0.5).clip(0, self.shape[i]-1)
                low, high, perc = [y.reshape(reshape).expand(expand) for y in (
                    index.floor().int(), index.ceil().int(), index - index.floor())]
                x = x.gather(i, low).lerp(x.gather(i, high), perc)
            else:
                index = (scale*(arr+0.5) if mode == "nearest-exact" else scale *
                         arr).cast(dtypes.int32).reshape(reshape).expand(expand)
                x = x.gather(i, index)
    ...
    def _pre_scatter(self, dim: int, index: Tensor, src: Tensor) -> tuple[Tensor, Tensor]:
        ...
        if self.dtype != src.dtype:
            raise RuntimeError(
                f"expect {self.dtype=} to be equal to {src.dtype=}")
        ...
        src = src.unsqueeze(-1).expand(*src.shape,
                                       self.shape[dim]).transpose(-1, dim)
        mask = index.unsqueeze(-1)._one_hot_along_dim(
            self.shape[dim]).transpose(-1, dim)
        # pad src and mask to self.shape so that reduce can be done with padded values as no-ops
        src, mask = (x.pad(tuple((0, self.shape[i] - x.shape[i]) if i !=
                     dim else None for i in range(self.ndim)) + (None,)) for x in (src, mask))
        return src, mask
    ...
    def scatter(self, dim: int, index: Tensor, src: Tensor | ConstType, reduce: Literal['multiply', 'add'] | None = None) -> Tensor:
        ...
        if reduce not in {None, "add", "multiply"}:
            raise TypeError(
                f"{reduce=} must be one of None, 'multiply', or 'add'")
    ...
    def scatter_reduce(self, dim: int, index: Tensor, src: Tensor, reduce: Literal["sum", "prod", "mean", "amax", "amin"],
        ...
        """
        src, mask = self._pre_scatter(dim, index, src)
        def _inv_mask(a: Tensor | ConstType, b: Tensor |
                      ConstType) -> Tensor: return mask.any(-1).logical_not().where(a, b)
        if reduce == "sum":
        ...
        if reduce == "mean":
            count = mask.where(
                1, 0).sum(-1).add(1 if include_self else _inv_mask(1, 0))
    ...
    def sort(self, dim: int = -1, descending: bool = False) -> tuple[Tensor, Tensor]:
        ...
        pads = tuple((0, 2**n_stages - orig_len) if i ==
                     dim else None for i in range(x.ndim))
        x = x.pad(pads, value=dtypes.min(x.dtype) if descending else dtypes.max(
            x.dtype)).unflatten(dim, (2,)*n_stages)
        ...
        for stage in range(1, n_stages+1):
            if stage != n_stages:
                ...
                flip_dims = tuple(-i for i in range(1,
                                  stage+1+(self.ndim-dim)))
                x = (blue_box.cat(green_box.flip(flip_dims),
                     dim=crossover_dim)).contiguous()
            for substage in range(stage-1, -1, -1):
                partner_dim = dim + n_stages - substage - 1
                x_top, x_bottom = x.split(1, partner_dim)
                x_larger, x_smaller = x_top.maximum(
                    x_bottom), x_top.minimum(x_bottom)
                x = (x_larger.cat(x_smaller, dim=partner_dim) if descending else x_smaller.cat(
                    x_larger, dim=partner_dim)).contiguous()
            if stage != n_stages:
                # flip wires back to undo the crossover
                blue_box, flipped_green_box = x.split(1, crossover_dim)
                x = blue_box.cat(flipped_green_box.flip(
                    flip_dims), dim=crossover_dim)
        x = x.flatten(dim, dim+n_stages-1).shrink(tuple((0, s)
                                                        for s in self.shape))
        # compute indices for sorted values
        mask = Tensor.ones(orig_len, orig_len, dtype=dtypes.bool, device=self.device).tril(
        ).reshape((None, None) + (1,)*(self.ndim-dim-1))
        def compute_counts(t: Tensor): return (
            mask & (t.unsqueeze(dim) == t.unsqueeze(dim+1))).sum(dim+1)
    ...
    def topk(self, k: int, dim: int = -1, largest: bool = True, sorted_: bool = True) -> tuple[Tensor, Tensor]:
        ...
        if not sorted_:
            raise NotImplementedError(
                "topk with sorted_=False is not supported")
        ...
        shrink_to_k = tuple(
            (0, k) if i == dim else None for i in range(self.ndim))
    ...
    def asin(self) -> Tensor:
        ...
        coefficients = [-0.0012624911, 0.0066700901, -0.0170881256,
                        0.0308918810, -0.0501743046, 0.0889789874, -0.2145988016, 1.5707963050]
        x = math.pi / 2 - (1.0 - self.abs()).sqrt() * \
            polyN(self.abs(), coefficients)
        return self.sign() * x
    ...
    def clamp(self, min_=None, max_=None) -> Tensor:
        ...
        if min_ is None and max_ is None:
            raise RuntimeError(
                "at least one of 'min_' or 'max_' must not be None")
    ...
    def _broadcasted(self, y: Tensor | ConstType | UOp, reverse: bool = False, match_dtype: bool = True) -> tuple[Tensor, Tensor]:
        ...
        if not isinstance(y, Tensor):
            # make y a Tensor
            assert isinstance(y, (*get_args(ConstType), UOp)
                              ), f"{type(y)=}, {y=}"
            ...
            if isinstance(y, UOp):
                ...
                y = Tensor(dtypes.as_const(y, y_dtype), x.device,
                           y_dtype, requires_grad=False)
    ...
    def div(self, x: Tensor | ConstType, reverse=False, rounding_mode: Literal["trunc", "floor"] | None = None) -> Tensor:
        ...
        d = numerator.cast(least_upper_float(numerator.dtype)) * \
            denominator.cast(least_upper_float(denominator.dtype)).reciprocal()
        output_dtype = numerator.dtype if dtypes.is_int(
            numerator.dtype) else d.dtype
        if dtypes.is_int(dt := least_upper_dtype(numerator.dtype, denominator.dtype)) and rounding_mode is not None:
            ...
            if rounding_mode == "floor":
                truncate_div, truncate_mod = numerator.idiv(
                    denominator), numerator._apply_broadcasted_uop(UOp.mod, denominator)
    ...
    def lshift(self, x: int, reverse=False) -> Tensor:
        ...
        ```
        """
        assert dtypes.is_unsigned(self.dtype) and isinstance(
            x, int) and x >= 0 and not reverse, f"not supported {self.dtype=} {x=}"
    ...
    def __rfloordiv__(self, x): return self.div(
        x, rounding_mode="floor", reverse=True)
    ...
    def __ifloordiv__(
        self, x) -> Tensor: return self.assign(self.__floordiv__(x))
    ...
    def __lt__(
        self, x) -> Tensor: return self._apply_broadcasted_uop(UOp.__lt__, x, False)
    def __gt__(
        self, x) -> Tensor: return self._apply_broadcasted_uop(UOp.__lt__, x, True)
    ...
    def batchnorm(self, weight: Tensor | None, bias: Tensor | None, mean: Tensor, invstd: Tensor, axis: int | tuple[int, ...] = 1) -> Tensor:
        ...
        """
        axis_ = argfix(axis)
        shape = tuple(s if ax in axis_ else 1 for ax,
                      s in enumerate(self.shape))
        x = self - mean.reshape(shape)
        if weight is not None:
            x = x * weight.reshape(shape)
        ret = x.mul(invstd.reshape(shape) if len(
            invstd.shape) == len(axis_) else invstd)
    ...
    def _one_hot_along_dim(self: Tensor, num_classes: sint, dim: int = -1) -> Tensor:
        ...
        dt = dtypes.int64 if sint_to_uop(num_classes).overflows(
            dtypes.int32) else dtypes.int32
    ...
    def scaled_dot_product_attention(self, key: Tensor, value: Tensor, attn_mask: Tensor | None = None, dropout_p: float = 0.0,
        ...
        if enable_gqa:
            key = key.repeat_interleave(
                self.shape[-3] // key.shape[-3], dim=-3)
            value = value.repeat_interleave(
                self.shape[-3] // value.shape[-3], dim=-3)
        ...
        qk = q.matmul(key.transpose(-2, -1), dtype=least_upper_dtype(q.dtype,
                      key.dtype, dtypes.float32)) / math.sqrt(q.shape[-1])
        # handle attention mask
        if is_causal:
            if attn_mask is not None:
                raise RuntimeError("cannot set attn_mask when is_causal=True")
            attn_mask = qk.ones_like(
                requires_grad=False, device=self.device, dtype=dtypes.bool).tril()
        if attn_mask is not None:
    ...
    def _do_reduction(self, reduction: ReductionStr = "mean") -> Tensor:
        ...
        reductions: dict[str, Callable[[Tensor], Tensor]] = {
            "mean": Tensor.mean, "sum": Tensor.sum, "none": lambda x: x}
    ...
    def sparse_categorical_crossentropy(self, Y: Tensor, ignore_index: int = -1, label_smoothing=0.0, reduction: ReductionStr = "mean") -> Tensor:
        ...
        assert reduction in get_args(
            ReductionStr), f"reduction must be one of {get_args(ReductionStr)}"
        log_probs = self.log_softmax()
        loss_mask = (Y != ignore_index) if ignore_index != - \
            1 else Y.ones_like(dtype=dtypes.bool)
        y = Y.to(self.device).unsqueeze(-1)._one_hot_along_dim(
            self.shape[-1], dim=-1) * loss_mask.unsqueeze(-1)
        smoothing = label_smoothing * (log_probs.mean(-1) * loss_mask)
        unreduced = ((1 - label_smoothing) *
                     (log_probs * y).sum(-1) + smoothing)
    ...
    def cross_entropy(self, Y: Tensor, reduction: ReductionStr = "mean", label_smoothing: float = 0.0) -> Tensor:
        ...
        if self.shape != Y.shape:
            if self.max(classes_dim).shape != Y.shape:
                raise RuntimeError(
                    f"shape mismatch: {self.shape=}, {Y.shape=}")
            Y = Y.unsqueeze(classes_dim)._one_hot_along_dim(
                num_classes=self.shape[classes_dim], dim=classes_dim)
        Y = (1 - label_smoothing)*Y + \
            label_smoothing / int(Y.shape[classes_dim])
    ...
    def nll_loss(self, Y: Tensor, weight: Tensor | None = None, ignore_index: int | None = None, reduction: ReductionStr = "mean") -> Tensor:
        ...
        ```
        """
        weight = Tensor.ones_like(
            Y, requires_grad=False) if weight is None else weight[Y]
        masked_weight = weight if ignore_index is None else weight * \
            (Y != ignore_index)
    ...
    def newton_schulz(self, steps: int, params: tuple[int, ...], eps: float = 1.0e-7) -> Tensor:
        ...
        G = self / (self.square().sum(axis=(-2, -1),
                    keepdim=True).sqrt() + eps)
        if (swap := self.shape[-2] > self.shape[-1]):
            G = G.transpose(-2, -1)
        for _ in range(steps):
            G = sum(p * functools.reduce(lambda x, y: (y @ y.transpose(-2, -1))
                    @ x, [G]*i, G) for i, p in enumerate(params))
        return G.transpose(-2, -1) if swap else G

    def qr(self) -> tuple[Tensor, Tensor]:
        assert self.ndim > 1, f"expected two or more dimensions, got {self.ndim}"
        b_shape, m, n = self.shape[:-
                                   2], int(self.shape[-2]), int(self.shape[-1])
        R = self.clone()
        Q = Tensor.eye(m, dtype=self.dtype).reshape(
            (1,) * len(b_shape) + (m, m)).expand(b_shape + (m, m)).contiguous()
        for i in range(min(m, n)):
            # TODO: without contigous this can silently be wrong, should at least assert
            x = R[..., i:m, i].contiguous()
            s = -x[..., 0].sign()
            u1 = x[..., 0] - s * x.square().sum(-1).sqrt()
            w = x.unsqueeze(-1) / u1.reshape(b_shape + (1, 1))
            w[..., 0, 0] = 1
            tau = (-s * u1 / x.square().sum(-1).sqrt()
                   ).reshape(b_shape + (1, 1))
            R[..., i:m, :] = R[..., i:m, :] - \
                (w * tau) @ (w.transpose(-2, -1) @ R[..., i:m, :])
            Q[..., :, i:m] = Q[..., :, i:m] - \
                (Q[..., :, i:m] @ w) @ (tau * w).transpose(-2, -1)
        return Q, R
    ...
    def svd(self, full_matrices=True) -> tuple[Tensor, Tensor, Tensor]:
        ...
        U = R.shrink(tuple([None] * len(b_shape) +
                     [(0, num), (0, num)])).contiguous()
        V = Tensor.eye(num, dtype=self.dtype).reshape(
            (1,) * len(b_shape) + (num, num)).expand(b_shape + (num, num)).contiguous()
        # prepare round robin pairing
        permute, inverse_permute = Tensor.arange(
            0, num, dtype=dtypes.int), Tensor.zeros(num, dtype=dtypes.int).contiguous()
        ...
        def one_round_jacobi(U, V, permute, inverse_permute):
            ...
            V_permuted, runoff_V = (V[..., permute].split(
                num - 1, -1)) if num % 2 == 1 else (V[..., permute], None)
            V_left, V_right = V_permuted.split(num//2, -1)
            U_permuted, runoff_U = (U[..., permute].split(
                num - 1, -1)) if num % 2 == 1 else (U[..., permute], None)
            ...
            U = U_left.cat(U_right.cat(runoff_U, dim=-1) if num %
                           2 == 1 else U_right, dim=-1)[..., inverse_permute]
            V_left, V_right = c * V_left - s * V_right, s * V_left + c * V_right
            V = V_left.cat(V_right.cat(runoff_V, dim=-1) if num %
                           2 == 1 else V_right, dim=-1)[..., inverse_permute]
            # prepare the next round robin pairings
            if num % 2 == 1:
                permute = ((permute - 1) % num)
            else:
                permute = permute[0].reshape(1).cat(
                    ((permute[1:num] - 2) % (num - 1)) + 1)
            inverse_permute = inverse_permute.scatter(
                0, permute, Tensor.arange(num, dtype=dtypes.int32))
            return U, V, permute, inverse_permute
        max_iterations, iterations_per_round = 1, int(
            num * math.log2(num) * 2 + 2)  # sorta heuristic, most use num*log2(num)
        ...
        new_indices = Tensor.arange(num).reshape(
            (1,) * (self.ndim - 1) + (num,)).expand(b_shape + (num, num)).contiguous()
        new_indices[..., :num] = indices.reshape(
            b_shape + (1, num)).expand(b_shape + (num, num))
        U, V = U.gather(-1, new_indices[..., 0:num, 0:num]) / \
            S.unsqueeze(-2), V.gather(-1,
                                      new_indices[..., 0:num, 0:num]).realize()

        padded_u = Tensor.eye(q_num, dtype=U.dtype).reshape(
            (1,) * len(b_shape) + (q_num, q_num)).expand(b_shape + (q_num, q_num)).contiguous()
    ...
    def bitcast(self, dtype: DTypeLike) -> Tensor:
        ...
        if (not isinstance(self.device, str) or not self.device.startswith("DISK")) and ns != os:
            new_uint, old_uint = to_dtype(
                f"uint{8*ns}"), to_dtype(f"uint{8*os}")
    ...
    def image_dot(self, w: Tensor, dtype: DTypeLike | None = None) -> Tensor:
        ...
        bs, groups, cin, cout = prod(
            self.shape[0:-2]), prod(w.shape[0:-2]), w.shape[-2], w.shape[-1]
        out_shape_t = self.shape[0:-2] + \
            (cout, -1) if len(self.shape) > 1 else (cout, )

        # NOTE: with NHWC we can remove the transposes
        # bs x groups*cin x H x W
        cx = self.transpose(self.ndim-1, self.ndim -
                            2).reshape((bs//groups, groups*cin, -1, 1))
    ...
    def image_conv2d(self, weight: Tensor, bias: Tensor | None = None, groups=1, stride=1, dilation=1, padding=0, dtype=None) -> Tensor:
        base_image_type = dtypes.imageh if getenv(
            "FLOAT16", 0) else dtypes.imagef
        ...
        if cin % 4 != 0 and not (cin == 1 and groups % 4 == 0):
            ...
            w = w.pad(tuple((0, added_input_channels) if i ==
                      2 else None for i in range(w.ndim)))
            x = x.pad(tuple((0, added_input_channels) if i ==
                      2 else None for i in range(x.ndim)))
        ...
        if rcout % 4 != 0 and not (rcout == 1 and groups % 4 == 0):
            ...
            cout = groups * rcout
            w = w.pad(tuple((0, added_output_channels) if i ==
                      1 else None for i in range(w.ndim)))
        ...
        if cin == 1:
            ...
            w = w.reshape(cout//4, 4, cin//4, 4, H,
                          W).permute(0, 4, 2, 5, 1, 3)
        else:
            w = w.reshape(cout//4, 4, cin//4, 4, H,
                          W).permute(0, 4, 2, 5, 3, 1)

        # contiguous creates the image, and early realize static weights (TODO: test for the static weight)
        if IMAGE >= 2:
            x, w = x.cast(base_image_type((bs*iy, ix*groups*cin//4, 4))
                          ), w.cast(base_image_type((cout//4, H*W*cin, 4)))
        x, w = x.contiguous(), w.contiguous()

        # expand out
        rcin_hi, rcin_lo = cin//4 if cin >= 4 else 1, 4 if cin >= 4 else 1
        cout_expand = [groups//4 if cin == 1 else groups, 4 if cin ==
                       1 else 1, rcout//4 if rcout >= 4 else 1, 4 if rcout >= 4 else 1]
        x = x.reshape(bs, iy, ix, groups, rcin_hi, rcin_lo)
        if cin_last:
            w = w.reshape(cout//4, H, rcin_hi, W, 4, rcin_lo)
        else:
            w = w.reshape(cout//4, H, rcin_hi, W, rcin_lo,
                          4).permute(0, 1, 2, 3, 5, 4)

        # prepare input
        x = x.permute(0, 3, 4, 5, 1, 2).pad(self._resolve_pool_pads(padding, 2))._pool(
            (H, W), stride, dilation)  # -> (bs, groups, rcin_hi, rcin_lo, oy, ox, H, W)
        x = x.permute(0, 4, 5, 1, 2, 3, 6, 7).reshape(
            bs, (oy := x.shape[4]), (ox := x.shape[5]), *cout_expand[0:2], 1, 1, rcin_hi, rcin_lo, H, W)
        ...
        # the conv!
        ret = (x*w).cast(base_image_type((bs*oy, ox*cout//4, 4)) if IMAGE >=
                         2 else dtypes.float32).sum((-4, -3, -2, -1), dtype=dtype)
