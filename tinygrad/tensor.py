...
class Tensor:
  ...
  def __getitem__(self, indices: Union[int, slice, Tensor, None, List, Tuple]) -> Tensor: # no ellipsis type...
    ...
    if isinstance(indices, (tuple, list)):
      ...
      if isinstance(indices, list) and all_int(fully_flatten(indices)): indices = [Tensor(indices, requires_grad=False, device=self.device)]
    ...
    if type_dim[Tensor]:
      ...
      except AssertionError as exc: raise IndexError(f"shape mismatch: indexing tensors could not be broadcast together with shapes {', '.join([str(i.shape) for i in idx])}") from exc   # noqa: E501
