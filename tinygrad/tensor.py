  ...
  def bitcast(self, dtype:DType) -> Tensor:
    if self.dtype.itemsize != dtype.itemsize:
      ...
      if self.dtype.itemsize < dtype.itemsize:
        ...
        shift = ((2 ** (8 * self.dtype.itemsize)) ** Tensor.arange(m)).cast(to_uint[dtype.itemsize])
        return self.bitcast(to_uint[self.dtype.itemsize]).reshape(self.shape[:-1] + (-1, m)).cast(to_uint[dtype.itemsize])\
          .mul(shift).sum(-1, acc_dtype=to_uint[dtype.itemsize]).bitcast(dtype)
