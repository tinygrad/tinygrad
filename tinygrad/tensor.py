...
from tinygrad.helpers import ImageDType, argfix, make_pair, getenv, IMAGE, DEBUG, flatten, DType, dtypes, prod, all_int, round_up, merge_dicts, flatten_all_int  # noqa: E501
...
class Tensor:
  ...
  def __init__(self, data:Union[None, int, float, list, LazyBuffer, np.ndarray, bytes], device:Optional[str]=None, dtype:Optional[DType]=None, requires_grad:Optional[bool]=None):  # noqa: E501
    ...
    if isinstance(data, LazyBuffer): assert dtype is None or dtype == data.dtype, "dtype doesn't match, and casting isn't supported"
      ...
      data = LazyBuffer.fromCPU(np.array(data, (dtype or (dtypes.int32 if data and flatten_all_int(data) else Tensor.default_type)).np))
