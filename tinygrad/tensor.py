...
class Tensor(RandMixin):
  ...
  def __init__(self, data:ConstType|bytes|list|tuple|UOp|'numpy.ndarray'|pathlib.Path|None,
    ...
    if isinstance(data, get_args(ConstType)):
      ...
      data = UOp.const(data) if _dtype is None else const_uop(data, _dtype)
    elif is_numpy_ndarray(data) and data.shape == ():
      data = const_uop(data.item(), _dtype or _from_np_dtype(data.dtype))
  ...
  @staticmethod
  def const(b:ConstLike, dtype:DType) -> Tensor: return Tensor(const_uop(b, dtype))
