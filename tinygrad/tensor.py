...
class Tensor(MathTrait):
  ...
  def _pool(self, k_:tuple[sint, ...], stride:int|tuple[int, ...]=1, dilation:int|tuple[int, ...]=1) -> Tensor:
    ...
    # insert output singletons before the kernel dims so later .mean(axis) reduces correctly
    if all(k==i for k,i in zip(k_,i_)): return self.reshape(self.shape[:self.ndim-len(k_)] + (1,)*len(k_) + tuple(i_))
