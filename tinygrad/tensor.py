...
class Tensor(OpMixin):
  ...
  def _cumalu(self, axis:int, op:Ops, _include_initial=False) -> Tensor:
    ...
    return {Ops.ADD: Tensor.sum, Ops.MAX: Tensor.max, Ops.MUL: Tensor.prod}[op](pooled, -1).transpose(axis, -1)
  ...
  def _split_cumalu(self, axis:int, op:Ops) -> Tensor:
    ...
    return {Ops.ADD: Tensor.add, Ops.MAX: Tensor.maximum, Ops.MUL: Tensor.mul}[op](fix(ret), fix(base))
