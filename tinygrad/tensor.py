...
class Tensor:
  ...
  def __init__(self, data:Union[None, int, float, list, LazyBuffer, np.ndarray, bytes], device:Optional[str]=None, dtype:Optional[DType]=None, requires_grad:Optional[bool]=None):
    ...
    if isinstance(data, LazyBuffer): assert dtype is None or dtype == data.dtype, "dtype doesn't match, and casting isn't supported"
      ...
      if isinstance(data, list): data = LazyBuffer.fromCPU(np.array(data, dtype=(dtype or (dtypes.int32 if all(isinstance(i, int) for i in data) else Tensor.default_type)).np))
  ...
  @staticmethod
  def zeros(*shape, **kwargs): return Tensor.full(argfix(*shape), 0, dtype=kwargs.pop("dtype", Tensor.default_type), **kwargs)

  @staticmethod
  def ones(*shape, **kwargs): return Tensor.full(argfix(*shape), 1, dtype=kwargs.pop("dtype", Tensor.default_type), **kwargs)
