...
class Tensor:
  ...
  def keccak(self: Tensor, cfg: Union[str, Tuple[int, int]] = "sha3_256"):
    ...
    tkwargs, targs = dict(device=self.device, dtype=dtypes.uint64), (self.device, dtypes.uint64)
    ...
    rate, dsbyte = ({ "sha3_224": 144, "sha3_256": 136, "sha3_384": 104, "sha3_512": 72 }[cfg], 0x06) if isinstance(cfg, str) else cfg
    ...
    else: mb = [(lbe, 0), (1, dsbyte), (data.shape[-1] + rate - 202 - lbe, 0), (1, 0x80), (200 - rate, 0)]
