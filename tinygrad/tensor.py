...
class Tensor(MathTrait):
  ...
  def image_dot(self, w:Tensor, dtype:DTypeLike|None=None) -> Tensor:
    ...
    result = cx.image_conv2d(cw, groups=groups, dtype=dtype).cast(least_upper_dtype(x.dtype, w.dtype) if dtype is None else dtype)
    return result.reshape(out_shape_t).transpose(self.ndim-1, self.ndim-2)
