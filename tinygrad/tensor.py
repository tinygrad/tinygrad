...
from tinygrad.helpers import WINO_OLD, argfix, make_tuple, flatten, prod, all_int, round_up, merge_dicts, argsort, getenv, all_same, fully_flatten, dedup
...
class Tensor(MathTrait):
  ...
  def conv2d(self, weight:Tensor, bias:Tensor|None=None, groups=1, stride=1, dilation=1, padding:int|tuple[int, ...]=0,
    ...
    # Need enough tiles so that tyx[i] * HWO[i] >= oyx[i] for each dimension
    # ntiles = ceil(oyx[i] / HWO[i]), input_needed = (ntiles-1)*HWO[i] + HWI[i]
    extra_pad = sum([[padding_[i*2], padding_[i*2+1] + max(0, (math.ceil(oyx[i] / HWO[i]) - 1) * HWO[i] + HWI[i] - self.shape[-len(HW)+i] - padding_[i*2] - padding_[i*2+1])] for i in range(len(HW))], [])  # noqa: E501
    # (bs, cin_, tyx, HWI)
    d = self.pad(extra_pad)._pool(HWI, HWO)  # noqa: E501
