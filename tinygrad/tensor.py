...
def inGPU(data):
  return GPU and ( isinstance(data, GPUBuffer) or (CUDA and isinstance(data, cp.ndarray)) )
...
try:
  import cupy as cp
  from tinygrad import ops_cuda
  _register_ops(ops_cuda, device=Device.GPU)
  GPU = True
  CUDA = True
