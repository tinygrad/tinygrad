...
class Tensor(SimpleMathTrait):
  ...
  def requires_grad_(self, requires_grad=True) -> Tensor:
    if not (dtypes.is_float(self.lazydata.dtype) or dtypes.is_bool(self.lazydata.dtype)) and requires_grad:
  ...
  def argmax(self, axis=None, keepdim=False) -> Tensor:
    ...
    axis = self._resolve_dim(axis)
    m = (self == self.max(axis=axis, keepdim=True)).requires_grad_(False) # explicit requires_grad=False, as casting later to int32
