...
from tinygrad.ops import MetaOps, smax, smin, resolve, UOp, UOps, BinaryOps, sint, Variable, MathTrait, UnaryOps
...
class Tensor(MathTrait):
  ...
  def const_like(self, b): return Tensor(b).reshape([1]*len(self.shape)).expand(self.shape)
  def alu(self, arg, *src:Tensor):
    if arg is BinaryOps.SHL: return self * src[0].exp2()
