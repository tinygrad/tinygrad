...
class Tensor:
  ...
  def rand(*shape, dtype:Optional[DType]=None, **kwargs):
    ...
    out = (x[0].cat(x[1])[:num].realize().cast(dtypes.float32) / (2 ** 32 - 1)).reshape(shape).cast(Tensor.default_type if dtype is None else dtype)
  ...
  def arange(start, stop=None, step=1, **kwargs):
    ...
    return Tensor.full((math.ceil((stop-start)/step) if step != 1 else (stop-start),), step, dtype=dtype, **kwargs).cumsum() + (start - step)
