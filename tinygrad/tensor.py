...
class Tensor:
  ...
  def __init__(self, data:Union[None, ConstType, List, Tuple, LazyBuffer, np.ndarray, bytes, MultiLazyBuffer, Variable, pathlib.Path],
    ...
    if isinstance(data, LazyBuffer): assert dtype is None or dtype == data.dtype, "dtype doesn't match, and casting isn't supported"
      ...
      if dtype in (dtypes.fp8_e4m3, dtypes.fp8_e5m2, dtypes.bfloat16):
  ...
  def numpy(self) -> np.ndarray:
    ...
    ```
    """
    if self.dtype in (dtypes.fp8_e4m3, dtypes.fp8_e5m2, dtypes.bfloat16): return self.float().numpy()
  ...
  def rand(*shape, device:Optional[Union[Tuple[str, ...], str]]=None, dtype:Optional[DTypeLike]=None, **kwargs) -> Tensor:
    ...
    if not THREEFRY:
      ...
      if (dt:=to_dtype(dtype or dtypes.default_float)) in (dtypes.fp8_e4m3, dtypes.fp8_e5m2, dtypes.bfloat16):
  ...
  def sum(self, axis:Optional[Union[int, Sequence[int]]]=None, keepdim=False, acc_dtype:Optional[DTypeLike]=None):
    ...
    return ret.cast(self.dtype) if acc_dtype is None and self.dtype in (dtypes.fp8_e4m3, dtypes.fp8_e5m2, dtypes.float16, dtypes.bfloat16) else ret
