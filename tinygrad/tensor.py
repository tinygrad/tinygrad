...
class Tensor:
  ...
  def div(self, x:Union[Tensor, Scalar], reverse=False) -> Tensor:
    ...
    if isinstance(x, Tensor) and not dtypes.is_float(x.dtype): x = x.cast(least_upper_float(x.dtype))
    return mlops.Div.apply(*self._broadcasted(x if isinstance(x, Tensor) else float(x), reverse))
