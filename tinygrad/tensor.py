...
class Tensor(MathTrait):
  ...
  def __init__(
    ...
    if isinstance(data, UOp):
      ...
      if data.dtype == dtypes.index:
        data = index_to_concrete_int(data)
      ...
      if dtype is None:
        if (d := fully_flatten(data)) and all(isinstance(s, bool) for s in d):
          ...
          dtype = dtypes.default_int if d and all_int(d) else dtypes.default_float  # NOTE: this works because all_int([True, False]) is True
      if dtype in [dtypes.bfloat16, *dtypes.fp8s]:
        data = Tensor(_frompy(data, dtypes.float32), device=device).cast(dtype).uop
      else:
      ...
      if data.shape == ():
        data = UOp.const(dtype or _from_np_dtype(data.dtype), data.item(), device, ())
      else:
    ...
    if isinstance(device, str):
      self.uop: UOp = data if data.device == device else data.copy_to_device(device)
  ...
  def pad_to(self, shape, *args):
    return self.pad(tuple([(0, ns - s) for s, ns in itertools.zip_longest(self.shape, argfix(shape, *args))]))

  def shrink_to(self, shape, *args):
    return self.shrink(tuple([(0, ns) for ns in argfix(shape, *args)]))
  ...
  def __setitem__(self, indices, v: Tensor | ConstType) -> None:
    ...
    use_where_optimization = (
      isinstance(indices, int)  # Simple integer indexing
      and self.ndim == 1  # 1D tensor only
      and isinstance(v, (int, float, Tensor))  # Simple value assignment
    )
