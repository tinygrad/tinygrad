...
class Tensor:
  ...
  def mean(self, axis=None, keepdim=False):
    out = self.cast(sum_acc_dtype(self.dtype)).sum(axis=axis, keepdim=keepdim)
    return out.div(prod([si for si, so in zip(self.shape, self.sum(axis=axis, keepdim=True).shape) if si != so])).cast(self.dtype)
