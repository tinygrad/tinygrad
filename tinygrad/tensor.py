...
class Tensor(MathTrait):
  ...
  def schedule_with_vars(self, *lst:Tensor) -> tuple[list[ScheduleItem], dict[Variable, int]]:
    ...
    # remove all buffer STOREs, after scheduling, the tensors are just buffers
    remove_store_map = {u:u.buf_uop for u in sink.toposort() if u.op is Ops.STORE and u.src[0].op in {Ops.BUFFER, Ops.BUFFER_VIEW, Ops.MSTACK}}
  ...
  def assign(self, x) -> Tensor:
    ...
    self.uop = self.uop.store(x.uop, dtype=x.uop.dtype)
    return self
