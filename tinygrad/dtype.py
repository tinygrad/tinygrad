...
class PtrDType(DType):
    ...
    def ptr(self, size=-1, local=False):
    ...
    def __repr__(self):
        return (
            f"{self.base.__repr__()}.ptr({self.size}{', local=True' if self.local else ''})"
...
class dtypes:
    ...
    def as_const(val: tuple[ConstType, ...] | ConstType, dtype: DType):
        ...
        return (
            ...
            else float(val) if dtypes.is_float(dtype) else bool(val)
        )
    ...
    def min(dtype: DType):
        if dtypes.is_int(dtype):
            return 0 if dtypes.is_unsigned(dtype) else -(2 ** (dtype.itemsize * 8 - 1))
    ...
    def max(dtype: DType):
        if dtypes.is_int(dtype):
            return 2 ** (dtype.itemsize * 8) - 1 + dtypes.min(dtype)
    ...
    def imageh(shp):
        return ImageDType(
            100, 2, "imageh", "e", 1, None, dtypes.float32, False, 1, prod(shp), shp
        )

    @staticmethod
    def imagef(shp):
        return ImageDType(
            100, 4, "imagef", "f", 1, None, dtypes.float32, False, 1, prod(shp), shp
        )
...
def to_dtype(dtype: DTypeLike) -> DType:
    return dtype if isinstance(dtype, DType) else getattr(dtypes, dtype)
...
def least_upper_float(dt: DType) -> DType:
    return dt if dtypes.is_float(dt) else least_upper_dtype(dt, dtypes.float32)


DTYPES_DICT = {
    k: v
    for k, v in dtypes.__dict__.items()
    if isinstance(v, DType) and not k.startswith(("default", "void"))
}
