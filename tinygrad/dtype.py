...
class DType:
  ...
  def vec(self, sz:int):
    ...
    return DType(self.priority, self.itemsize*sz, f"{INVERSE_DTYPES_DICT[self.name]}{sz}", None, None, sz)
...
class PtrDType(DType):
  def __init__(self, dt:DType): super().__init__(dt.priority, dt.itemsize, dt.name, dt.fmt, dt.np, dt.sz)
...
class dtypes:
  ...
  int8: Final[DType] = DType(1, 1, "char", 'b', np.int8)
  uint8: Final[DType] = DType(2, 1, "unsigned char", 'B', np.uint8)
  int16: Final[DType] = DType(3, 2, "short", 'h', np.int16)
  uint16: Final[DType] = DType(4, 2, "unsigned short", 'H', np.uint16)
  int32: Final[DType] = DType(5, 4, "int", 'i', np.int32)
  uint32: Final[DType] = DType(6, 4, "unsigned int", 'I', np.uint32)
  int64: Final[DType] = DType(7, 8, "long", 'l', np.int64)
  uint64: Final[DType] = DType(8, 8, "unsigned long", 'L', np.uint64)
  float16: Final[DType] = DType(9, 2, "half", 'e', np.float16)
  # bfloat16 has higher priority than float16, so least_upper_dtype(dtypes.int64, dtypes.uint64) = dtypes.float16
  bfloat16: Final[DType] = DType(10, 2, "__bf16", None, None)
  float32: Final[DType] = DType(11, 4, "float", 'f', np.float32)
  float64: Final[DType] = DType(12, 8, "double", 'd', np.float64)
  ...
  @staticmethod
  def imageh(shp): return ImageDType(100, 2, "imageh", 'e', np.float16, shape=shp, base=dtypes.float32)
  @staticmethod
  def imagef(shp): return ImageDType(100, 4, "imagef", 'f', np.float32, shape=shp, base=dtypes.float32)
