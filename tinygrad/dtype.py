...
class PtrDType(DType):
  ...
  def ptr(self, size=-1, addrspace=AddrSpace.GLOBAL):
    raise RuntimeError("can't make a pointer from a pointer")

  def nbytes(self) -> int:
    if self.size == -1:
      return 0  # TODO: this should be an exception
...
def least_upper_dtype(*ds: DType) -> DType:
  return (
    min(set.intersection(*[_get_recursive_parents(d) for d in ds])) if not (images := [d for d in ds if isinstance(d, ImageDType)]) else images[0]
  )
...
def can_safe_cast(dt0: DType, dt1: DType) -> bool:
  ...
  match dt1:
    case dtypes.double:
      return dt0 in (dtypes.float, dtypes.half, dtypes.bfloat16, dtypes.uint32, dtypes.uint16, dtypes.uint8, dtypes.int32, dtypes.int16, dtypes.int8)
    case dtypes.float:
      return dt0 in (dtypes.half, dtypes.bfloat16, dtypes.uint16, dtypes.uint8, dtypes.int16, dtypes.int8)
    case dtypes.uint64:
