    ...
    function renderTable(root, ret) {
      ...
      for (const r of ret.rows) {
        ...
        for (const [i,value] of r.entries()) {
          ...
          td.append(() => typeof value === "string" ? colored(value) : d3.create("p").text(ret.cols[i] === "Duration" ? formatMicroseconds(value) : ret.cols[i] === "Clk" ? formatCycles(value)  : formatUnit(value)).node());
        }
      }
      return table;
    }
