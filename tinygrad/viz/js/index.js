...
async function renderDag(graph, additions, recenter=false) {
  ...
  worker.onmessage = (e) => {
    ...
    const tags = nodes.selectAll("g.tag").data(d => d.tag != null ? [d] : []).join("g").attr("transform", d => `translate(${-d.width/2+8}, ${-d.height/2+8})`);
    tags.selectAll("circle").data(d => [d]).join("circle").attr("class", "tag");
    tags.selectAll("text").data(d => [d.tag]).join("text").text(d => d).attr("dy", "0.35em").attr("class", "tag");
    // draw edges
    ...
    edgeLabels.selectAll("circle").data(e => [g.edge(e).label]).join("circle").attr("class", "tag");
    edgeLabels.selectAll("text").data(e => [g.edge(e).label]).join("text").text(d => d).attr("dy", "0.35em").attr("class", "tag");
