...
def uop_to_json(x: UOp) -> dict[int, tuple[str, list[int], str]]:
    ...
    for u in (toposort := x.toposort):
        ...
        if u.op in {Ops.DEVICE, Ops.CONST}:
            excluded.add(u)
...
def get_metadata(
    ...
    return [
        (
            to_function_name(k.name) if isinstance(k, Kernel) else str(k),
            [{"loc": v.loc, "match_count": len(v.matches)} for v in vals],
        )
...
def get_details(
    ...
    ret: GraphRewriteDetails = {
        "uops": [pcall(str, sink := ctx.sink)],
        "graphs": [uop_to_json(sink)],
        "code_line": lines(ctx.loc[0])[ctx.loc[1] - 1].strip(),
        "kernel_code": pcall(_prg, k) if isinstance(k, Kernel) else None,
        **metadata,
        "diffs": [[]],
        "upats": [None],
        "changed_nodes": [[]],
    }  # NOTE: the first graph just renders the input UOp
    ...
    for i, (u0, u1, upat) in enumerate(tqdm(ctx.matches)):
        ...
        ret["changed_nodes"].append(
            [id(x) for x in u1.toposort if id(x) in new_sink_js]
        )
        ret["diffs"].append(
            list(
    ...
    return cast(
        GraphRewriteDetails,
        {
            k: v[offset : offset + limit] if isinstance(v, list) else v
    ...
    def do_GET(self):
        ...
        if (url := urlparse(self.path)).path == "/":
            ...
            if "kernel" in (query := parse_qs(url.query)):
                ...
                jret: Any = get_details(
                    contexts[0][kidx],
                    contexts[1][kidx][ridx],
                    kernels[kidx][1][ridx],
                    getarg("offset", 0),
                    getarg("limit", 200),
                )
...
if __name__ == "__main__":
    ...
    if getenv("FUZZ_VIZ"):
        ret = [
            get_details(contexts[0][i], contexts[1][i][j], args)
            for i, v in tqdm(enumerate(kernels))
            for j, args in enumerate(v[1])
        ]
