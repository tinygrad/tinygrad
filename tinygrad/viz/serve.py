...
def load_sqtt(profile:list[ProfileEvent]) -> None:
  ...
  for name in rctx.occ_events:
    ...
    if wave_insts: steps.append(create_step("Instruction Timing", ("/sqtt-insts", len(ctxs), len(steps)), data=wave_insts, depth=2))
    for cu in prg_cu:
...
def get_render(i:int, j:int, fmt:str) -> dict:
  ...
  if fmt == "sqtt-insts":
    ...
    inst_columns = ["N", "Loc", "Clk", "Idle", "Dur", "Stall"]
    ...
    rows:dict[int, dict] = {}
    cycle_count:list[int] = []  # total cycles per trace
    for cu,waves in data.items():
      for v in waves.values():
        prev_instr = (w:=v["wave"]).begin_time
        cycle_count.append(w.end_time-w.begin_time)
        pc_to_inst = v["disasm"]
        start_pc = None
        for e in w.unpack_insts():
          ...
          inst["dur"] += e.dur
          inst["stall"] += e.stall
          inst["hits"]["rows"].append((inst["hit_count"]-1, w.simd_loc, e.time, max(0, e.time-prev_instr), e.dur, e.stall))
          prev_instr = max(prev_instr, e.time + e.dur)
    summary = [{"label":"Avg. Cycles", "value":sum(cycle_count)/len(cycle_count)}, {"label":"Traced CUs", "value":len(data)}]
