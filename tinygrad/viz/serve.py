...
def get_metadata(trace_bufs: list[tuple]) -> list[dict]:
  ...
  for keys, contexts, uop_fields in trace_bufs:
    for k, v in zip(keys, contexts):
      ...
      if getenv("PROFILE_VALUE") >= 2 and k.keys:
        r["runtime_stats"] = get_runtime_stats(k.keys[0])
...
def uop_to_json(x: UOp) -> dict[int, dict]:
  ...
  for u in toposort:
    ...
    if u.op is Ops.VIEW:
      argst = "\n".join([
        f"{shape_to_str(v.shape)} / {shape_to_str(v.strides)}"
        + ("" if v.offset == 0 else f" / {srender(v.offset)}")
        + (f"\nMASK {mask_to_str(v.mask)}" if v.mask is not None else "")
...
def get_details(ctx: TrackedGraphRewrite, i: int = 0) -> Generator[GraphRewriteDetails, None, None]:
  ...
  for u0_num, u1_num, upat_loc in tqdm(ctx.matches):
    ...
    yield {
      ...
      "diff": list(difflib.unified_diff(str(u0).splitlines(), str(u1).splitlines())),
      "upat": (upat_loc, printable(upat_loc)),
    }
  ...
  for st, et, dur, e in dev_events:
    ...
    if (ref := ref_map.get(name)) is not None:
      ...
      if isinstance(p := traces[ref][0].ret, ProgramSpec) and (ei := exec_points.get(p.name)) is not None:
        info = (
          f"{sym_infer(p.estimates.ops, ei['var_vals']) / (t := dur * 1e3):.2f} GFLOPS {sym_infer(p.estimates.mem, ei['var_vals']) / t:4.1f}"
...
def get_disassembly(ctx: list[str]):
  if not isinstance(prg := traces[int(ctx[0])][0].ret, ProgramSpec):
    return
...
class Handler(BaseHTTPRequestHandler):
  def do_GET(self):
    ...
    if (url := urlparse(self.path)).path == "/":
      ...
      if url.path == "/disasm":
        ret, content_type = get_disassembly(**query), "application/json"
      else:
        return self.stream_json(get_details(traces[i := int(query["ctx"][0])][1][int(query["idx"][0])], i))
    elif url.path == "/ctxs":
