...
class GraphRewriteDetails(TypedDict):
  ...
  changed: list[int]|None                # the changed UOp id + all its parents ids
...
def get_full_rewrite(ctx:TrackedGraphRewrite) -> Generator[GraphRewriteDetails, None, None]:
  ...
  yield {"graph":uop_to_json(next_sink), "uop":pystr(next_sink), "changed":None, "diff":None, "upat":None}
  ...
  for u0_num,u1_num,upat_loc,dur in tqdm(ctx.matches):
    ...
    yield {"graph":(sink_json:=uop_to_json(new_sink)), "uop":pystr(new_sink), "changed":[id(x) for x in u1.toposort() if id(x) in sink_json],
