  ...
  for u in toposort:
    ...
    try:
      ...
      if u.op in {Ops.INDEX, Ops.BUFFERIZE}:
        label += f"\n{u.render()}"
      if u.op in {Ops.END, Ops.STORE, Ops.REDUCE} and len(rngs:=list(UOp.sink(*u.src[range_start[u.op]:]).ranges)):
        label += "\n"+' '.join([f"{colored(s.arg[0], axis_colors[s.arg[-1]])}({s.vmax+1})" for s in rngs])
    except Exception:
      label += "\n<ISSUE GETTING LABEL>"
