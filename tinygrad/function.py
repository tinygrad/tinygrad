...
class Reciprocal(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    self.ret = x.e(UnaryOps.RECIP) if dtypes.is_float(x.dtype) else x.const(1).e(BinaryOps.IDIV, x)
    return self.ret
...
class Log(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    return grad_output.e(BinaryOps.MUL, self.x.e(UnaryOps.RECIP)) if dtypes.is_float(grad_output.dtype) else grad_output.e(BinaryOps.IDIV, self.x)
...
class Sqrt(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    div = self.ret.e(BinaryOps.MUL, self.ret.const(2))
    return grad_output.e(BinaryOps.MUL, div.e(UnaryOps.RECIP)) if dtypes.is_float(grad_output.dtype) else grad_output.e(BinaryOps.IDIV, div)
...
class Sigmoid(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    div = x.const(1).e(BinaryOps.ADD, x.e(BinaryOps.MUL, x.const(-1/math.log(2))).e(UnaryOps.EXP2))
    self.ret = div.e(UnaryOps.RECIP) if dtypes.is_float(x.dtype) else x.const(1).e(BinaryOps.IDIV, div)
    return self.ret
...
class Div(Function):
  def forward(self, x:LazyBuffer, y:LazyBuffer) -> LazyBuffer:
    self.x, self.y = x, y
    return x.e(BinaryOps.MUL, y.e(UnaryOps.RECIP)) if dtypes.is_float(x.dtype) else x.e(BinaryOps.IDIV, y)

  def backward(self, grad_output:LazyBuffer) -> Tuple[Optional[LazyBuffer], Optional[LazyBuffer]]:
    grad_x = grad_output.e(BinaryOps.MUL, self.y.e(UnaryOps.RECIP)) if dtypes.is_float(grad_output.dtype) else grad_output.e(BinaryOps.IDIV, self.y)
    div = self.y.e(BinaryOps.MUL, self.y)
    grad_y = grad_output.e(UnaryOps.NEG).e(BinaryOps.MUL, self.x)
    grad_y = grad_y.e(BinaryOps.MUL, div.e(UnaryOps.RECIP)) if dtypes.is_float(grad_y.dtype) else grad_y.e(BinaryOps.IDIV, div)
    return grad_x if self.needs_input_grad[0] else None, \
            grad_y if self.needs_input_grad[1] else None  # noqa: E501
