...
class Exp(Function):
    ...
    def _abs_w_lt(self, x: LazyBuffer, lt: LazyBuffer) -> LazyBuffer:
        return lt.e(TernaryOps.WHERE, x.e(UnaryOps.NEG), x)

        
    def _pade(self, x: LazyBuffer) -> LazyBuffer:

        # Pade 10
        PC = [1.0, 0.500000002734005, 0.11842105398892322, 0.01754385996556851, 0.0018059855977014816, 0.0001354489208912936, 7.524940113100287e-06, 3.071404154872436e-07, 8.859819752794675e-09, 1.6407073718200185e-10, 1.4915521524114695e-12]
        QC = [1.0, -0.499999997265995, 0.1184210512549182, -0.017543859323018895, 0.001805985504260442, -0.00013544891159627956, 7.5249394551197106e-06, -3.071403822471257e-07, 8.859818595453254e-09, -1.6407071206103022e-10, 1.4915518992207407e-12]
    ...
    def _divmod(self, x: LazyBuffer, o_y: LazyBuffer, y: LazyBuffer) -> LazyBuffer:
            divres = x.e(BinaryOps.MUL, o_y).cast(dtypes.uint64).cast(x.dtype)
            modres = x.e(BinaryOps.SUB, divres.e(BinaryOps.MUL, y))
            return divres, modres

    def _exp(self, x: LazyBuffer) -> LazyBuffer:
        lt0 = x.e(BinaryOps.CMPLT, x.const(0))
        x = self._abs_w_lt(x, lt0)


        d50, m50 = self._divmod(x, x.const(1/50), x.const(50))
        ...
        d25, m25 = self._divmod(m50, x.const(1/25), x.const(25))
        ...
        d12, m12 = self._divmod(m25, x.const(1/12), x.const(12))
        ...
        d6, m6 = self._divmod(m12, x.const(1/6), x.const(6))
        ...
        d3, m3 = self._divmod(m6, x.const(1/3), x.const(3))
