...
class Reciprocal(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    return grad_output.e(BinaryOps.MUL, grad_output.const_like(-1)).e(BinaryOps.MUL, self.ret).e(BinaryOps.MUL, self.ret)
...
class Sin(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    return self.x.const_like(math.pi / 2).e(BinaryOps.ADD,
                                            self.x.e(BinaryOps.MUL, self.x.const_like(-1))).e(UnaryOps.SIN).e(BinaryOps.MUL, grad_output)

# NOTE: maximum(x, 0) behaves differently where x=0
class Relu(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    self.ret = x.e(BinaryOps.MAX, x.const_like(0))
    return self.ret

  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    return self.ret.const_like(0).e(BinaryOps.CMPLT, self.ret).cast(grad_output.dtype).e(BinaryOps.MUL, grad_output)

class Log(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    self.x = x
    return x.e(UnaryOps.LOG2).e(BinaryOps.MUL, x.const_like(math.log(2)))

  def backward(self, grad_output:LazyBuffer) -> LazyBuffer: return grad_output.e(BinaryOps.MUL, self.x.e(UnaryOps.RECIP))

class Exp(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    self.ret = x.e(BinaryOps.MUL, x.const_like(1/math.log(2))).e(UnaryOps.EXP2)
    return self.ret
...
class Sqrt(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    return grad_output.e(BinaryOps.MUL, self.ret.e(BinaryOps.MUL, self.ret.const_like(2)).e(UnaryOps.RECIP))
...
class Sigmoid(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    self.ret = x.const_like(1).e(BinaryOps.ADD, x.e(BinaryOps.MUL, x.const_like(-1/math.log(2))).e(UnaryOps.EXP2)).e(UnaryOps.RECIP)
    return self.ret

  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    return self.ret.e(BinaryOps.MUL, self.ret.const_like(1).e(BinaryOps.ADD,
                                                              self.ret.e(BinaryOps.MUL, self.ret.const_like(-1)))).e(BinaryOps.MUL, grad_output)

class Sign(Function):
  def forward(self, x:LazyBuffer) -> LazyBuffer:
    return x.e(BinaryOps.CMPNE, x.const_like(0)).e(
      TernaryOps.WHERE, x.e(BinaryOps.CMPLT, x.const_like(0)).e(TernaryOps.WHERE, x.const_like(-1), x.const_like(1)), x.const_like(0))
...
class Where(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> Tuple[None, Optional[LazyBuffer], Optional[LazyBuffer]]:
    return None, \
      self.x.e(TernaryOps.WHERE, grad_output, grad_output.const_like(0)) if self.needs_input_grad[1] else None, \
      self.x.e(TernaryOps.WHERE, grad_output.const_like(0), grad_output) if self.needs_input_grad[2] else None
...
class Max(Function):
  ...
  def backward(self, grad_output:LazyBuffer) -> LazyBuffer:
    ...
    max_is_1s = self.x.e(BinaryOps.CMPNE, self.ret.expand(self.x.shape)).e(BinaryOps.CMPNE,
                                                                           self.x.const_like(1).cast(dtypes.bool)).cast(grad_output.dtype)
