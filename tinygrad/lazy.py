...
def _ast_binaryops(self:LazyBuffer) -> LazyOp:
  ...
  psrcs: List[Tuple[LazyBuffer, LazyBuffer]] = [(k,x) for k,x in zip(real_srcs.keys(), map(get_movementroot_contiguous, real_srcs.keys())) if x.optype is ReduceOps and not x.realized and prod(k.shape) == prod(x.shape) and len(x.children) <= 1 and len(k.children) <= 1]
...
def get_movementroot(root:LazyBuffer, allow_contiguous=False) -> LazyBuffer: return get_movementroot(cast(LazyBuffer, root.op.src[0]), allow_contiguous) if not root.realized and (root.optype is MovementOps or (root.op.op is LoadOps.CONTIGUOUS and allow_contiguous and root.op.src[0].st.contiguous)) else root
def get_movementroot_contiguous(x:LazyBuffer) -> LazyBuffer: return get_movementroot_contiguous(cast(LazyBuffer, x.op.src[0])) if not x.realized and x.op.op is LoadOps.CONTIGUOUS else (get_movementroot(x, True) if x.optype is MovementOps and x.st.contiguous else x)
...
class LazyBuffer:
  ...
  def realize(self:LazyBuffer) -> LazyBuffer:
    if not self.realized:
      ...
      if not self.realized:
        ...
        if self.dtype is ImageDType and self.optype != MovementOps and (prod(self.shape) != prod(cast(ImageDType, self.dtype).shape) or not any([self.shape[x]%4 == 0 for x in self.st.unit_stride_axes()])):
          if self.op.op is MovementOps.RESHAPE:
      ...
      assert self.realized.dtype is self.dtype or self.dtype is ImageDType, f"dtype mismatch on realize got {self.realized.dtype} expected {self.dtype}"
  ...
  def contiguous(self:LazyBuffer) -> LazyBuffer:
    if not self.realized and self.op.op is LoadOps.CONTIGUOUS: return self  # two CONTIGUOUS in a row is one
  ...
  def shuffle_and_prune_movement_ops(self, st: ShapeTracker, op: MovementOps, arg: Union[Tuple[int, ...], Tuple[Tuple[int, int], ...]]) -> LazyBuffer:
    if SHUFFLE_MOVEMENT_OPS and self.optype is BinaryOps and not self.realized and (op in {MovementOps.SHRINK, MovementOps.STRIDE, MovementOps.PERMUTE} or (op == MovementOps.RESHAPE and self.op.op in UnaryOps)) and len(self.children) == 0:
  ...
  def reshape(self:LazyBuffer, arg:Tuple[int, ...]) -> LazyBuffer:
    ...
    if not self.realized and self.op.op is MovementOps.RESHAPE:
  ...
  def pad(self:LazyBuffer, arg:Tuple[Tuple[int, int], ...]) -> LazyBuffer:
    ...
    if not self.realized and self.op.op is MovementOps.PAD: return self.op.src[0].pad(tuple([(b1+b2, e1+e2) for (b1,e1),(b2,e2) in zip(self.op.arg, arg)]))
  ...
  def expand(self: LazyBuffer, arg:Tuple[int, ...]) -> LazyBuffer:
    if self.shape == arg: return self
    if not self.realized and self.op.op is MovementOps.EXPAND:
  ...
  def permute(self: LazyBuffer, arg:Tuple[int, ...]) -> LazyBuffer:
    ...
    if not self.realized and self.op.op is MovementOps.PERMUTE: return self.op.src[0].permute(tuple([self.op.arg[i] for i in arg]))
    if not self.realized:
      ...
      if self.op.op is MovementOps.EXPAND:
        return self.op.src[0].permute(arg).expand(tuple([self.op.arg[a] for a in arg]))

      # move permutes before reshapes if we can
      if PUSH_PERMUTES and self.op.op is MovementOps.RESHAPE and self.op.src[0].__class__ is LazyBuffer:
  ...
  def shrink(self:LazyBuffer, arg:Tuple[Tuple[int, int], ...]) -> LazyBuffer:
    ...
    if not self.realized and self.op.op is MovementOps.SHRINK: return self.op.src[0].shrink(tuple([(b1+b2, b1+e2) for (b1,_),(b2,e2) in zip(self.op.arg, arg)]))
...
def _push_movement_ops(srcs:Tuple[LazyBuffer, ...]) -> Tuple[LazyBuffer, ...]:
  ...
  for x in srcs:
    ...
    while not bx.realized and bx.optype is MovementOps and bx.op.op != MovementOps.EXPAND and (SHUFFLE_PAD_OPS or bx.op.op != MovementOps.PAD) and len(bx.children) <= 1:
    ...
    if not bx.realized and bx.optype is BinaryOps and len(bx.children) <= 1 and len(mops) and (all([x[0] != MovementOps.PAD for x in mops]) or all([x.op != BinaryOps.DIV for x in bx.op.get_lazyops()])):
...
def elementwise_op(op:Union[UnaryOps, BinaryOps], *srcs:LazyBuffer, arg:Optional[Any]=None) -> LazyBuffer:
  ...
  if PUSH_CONTIGUOUS and any([not x.realized and x.op.op is LoadOps.CONTIGUOUS and len(x.op.src[0].children) <= 1 for x in srcs]):
    new_srcs: List[LazyBuffer] = []
    for x in srcs:
      if not x.realized and x.op.op is LoadOps.CONTIGUOUS and len(x.op.src[0].children) <= 1:
  ...
  if MERGE_ELEMENTWISE_OPS:
    ...
    srcs = tuple([x.op if x.optype is BinaryOps and len(x.children) == 0 and not x.realized else x for x in srcs])  # type: ignore
