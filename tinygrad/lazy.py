...
class LazyBuffer:
  ...
  def _movement_op(self, st: ShapeTracker, op: MovementOps, arg: Union[Tuple[sint, ...], Tuple[Tuple[sint, sint], ...]]) -> LazyBuffer:
    if SHUFFLE_MOVEMENT_OPS and not self.realized and self.optype == BinaryOps and len(self.children) <= 1 and (op is not MovementOps.PAD or all(x.op not in UNSAFE_PAD_OPS for x in self.op.get_lazyops())) and not all(x.realized for x in self.op.buffers):
...
def _push_movement_ops(srcs:Tuple[LazyBuffer, ...]) -> Tuple[LazyBuffer, ...]:
  ...
  for x in srcs:
    ...
    while not bx.realized and bx.optype is MovementOps and bx.op.op is not MovementOps.EXPAND and (SHUFFLE_PAD_OPS or bx.op.op is not MovementOps.PAD) and len(bx.children) <= 1 and not all(x.realized for x in bx.op.buffers):
