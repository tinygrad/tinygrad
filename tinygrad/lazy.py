...
class LazyBuffer:
  ...
  def cast(self, dtype:DType, bitcast:bool=False, allow_buffer_view=True):
    ...
    if bitcast and self.dtype.itemsize != dtype.itemsize:
      if not self.device.startswith("DISK") and not self.device.startswith("METAL"): raise RuntimeError("shape changing bitcast only supported on DISK and METAL right now")
