...
class LazyBuffer:
  ...
  def e(self:LazyBuffer, op:Union[UnaryOps, BinaryOps, TernaryOps], *srcs:LazyBuffer, arg:Optional[Any]=None) -> LazyBuffer:
    ...
    if MERGE_ELEMENTWISE_OPS:
      ...
      _srcs = tuple([cast(LazyBuffer, x.op) if x.optype == BinaryOps and not x.children and not x.realized else x for x in srcs])
      # TODO: needs general merge limiting
      if out_device != "WEBGPU" or len(dedup([x.base for _src in _srcs for x in _src.buffers if not x.is_unrealized_const()])) < 7: srcs = _srcs
  ...
  def r(self:LazyBuffer, op:ReduceOps, new_shape:Tuple[sint, ...]) -> LazyBuffer:
    ...
    heuristic, divisor, dim_to_split = max(((divisor := math.gcd(256, old))/(cast(int, stride) or math.inf), divisor, i) for i, (old, new, stride) in enumerate(zip(self.shape, new_shape, self.st.real_strides())) if old != new)
