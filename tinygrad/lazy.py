...
class LazyBuffer:
  ...
  def r(self, op:ReduceOps, axis:Tuple[int, ...]) -> LazyBuffer:
    ...
    split_candidates = [(i, x) for i in axis for x in range(min(256,2**19//prod(new_shape)),8-1,-1) if self.shape[i] % x == 0 and self.st.real_strides(ignore_valid=True)[i] != 0]
