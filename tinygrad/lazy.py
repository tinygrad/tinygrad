...
def vars_from_ast(ast:LazyOp) -> List[Variable]: 
  return dedup(functools.reduce(operator.add, [x.arg.st.vars() for x in ast.get_lazyops() if x.op in BufferOps], []))
...
class LazyBuffer:
  ...
  def schedule(self, seen=None) -> List[ScheduleItem]:
    ...
    ret = ret + [ScheduleItem(op, self, tuple(base_bufs), {k:var_vals[k] for k in vars_from_ast(op)})]
    Timing("sched else")
    return ret
