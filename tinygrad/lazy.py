...
class LazyBuffer:
  ...
  def e(self:LazyBuffer, op:Union[UnaryOps, BinaryOps, TernaryOps], *srcs:LazyBuffer, arg:Optional[Any]=None) -> LazyBuffer:
    ...
    if MERGE_ELEMENTWISE_OPS:
      ...
      # llama METAL hits this too
      if out_device not in ["WEBGPU", "METAL"] or len(dedup([x.base for _src in _srcs for x in _src.buffers if not x.is_unrealized_const()])) < 7: srcs = _srcs # type: ignore
