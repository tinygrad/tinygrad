...
view_supported_devices = {"LLVM", "CLANG", "CUDA", "NV", "AMD", "METAL", "DISK", "PYTHON"}
class LazyBuffer:
  ...
  def cast(self, dtype:DType, bitcast:bool=False, allow_buffer_view=True) -> LazyBuffer:
    ...
    if bitcast and self.dtype.itemsize != dtype.itemsize:
      if not self.st.contiguous and not self.device.startswith("DISK"): raise RuntimeError("non-contiguous bitcast not supported")
