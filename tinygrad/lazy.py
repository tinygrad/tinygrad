...
def _realize_rand(buffer: LazyBuffer) -> None:
  ...
  buffer.realized = Device[buffer.device].buffer.fromCPU(rng.random(size=buffer.shape, dtype=np.float32).astype(buffer.dtype.np), **buffer._device_extra_args()) # type: ignore
