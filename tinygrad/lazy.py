...
def _ast_binaryops(op:LazyOp, shape: Tuple[sint, ...]) -> LazyOp:
  real_srcs: Dict[LazyBuffer, Optional[Union[LazyOp, LazyBuffer]]] = {x:None for x in op.unique_buffers()}
  ...
  if MERGE_ONE_REDUCE_INTO_ELEMENTWISE and psrcs:
    ...
    real_srcs.update({x:x for x in top.unique_buffers()})  # the reduce op buffers are not modified
...
def _replace_bufferops(op:LazyOp) -> Tuple[LazyOp, List[LazyBuffer]]:
  ...
  base_bufs = dedup([x.base for x in op.unique_buffers() if not x.is_unrealized_const()])
  for x in op.unique_buffers():
...
def create_lazybuffer(device:str, st:ShapeTracker, optype:OpType, op:LazyOp, dtype:DType, base:Optional[LazyBuffer]=None):
  ...
  if wop in lazycache:
    for x in op.unique_buffers(): x.children.add(lazycache[wop])
...
class LazyBuffer:
  ...
  def schedule(self, seen=None) -> List[ScheduleItem]:
    ...
    # schedule the past
    ret = []
    for x in op.unique_buffers(): ret += x.schedule(seen)
    var_vals = dict(sorted(merge_dicts([self.st.var_vals] + [buf.st.var_vals for buf in op.unique_buffers()]).items(), key=lambda kv:cast(Variable,kv[0]).key))
