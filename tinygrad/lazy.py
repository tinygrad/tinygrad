...
class LazyBuffer:
  ...
  def _reduce_op(self, op:ReduceOps, axis:Tuple[int, ...]) -> LazyBuffer:
    ...
    def _handle(input_to_reduce, top_reduce_axes):
      ...
      new_axis = axis + tuple(range(len(st.shape)-len(rshape), len(st.shape)))
      ret = input_to_reduce.base._view(st)._reduce_op(op, new_axis)
      raise Exception(new_axis)
    ...
    if self.base != self and self.base.op == op and getenv("REF"):
