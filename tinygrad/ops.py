...
def exec_alu(op:Op, dtype:DType, operands, do_truncate=True):
  if dtype.count > 1:
    return tuple([exec_alu(op, dtype.scalar(), [x[i] if isinstance(x, tuple) else x for x in operands], do_truncate) for i in range(dtype.count)])
  ret = python_alu[op](*operands)
  return truncate.get(dtype, lambda x: x)(ret) if do_truncate else ret
...
symbolic = PatternMatcher([
  ...
  (UPat(UOps.ALU, name="root", src=UPat((UOps.VCONST, UOps.CONST))),
   lambda root: root.const_like(exec_alu(root.arg, root.dtype, [x.arg for x in root.src], do_truncate=False))),
