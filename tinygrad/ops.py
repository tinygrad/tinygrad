  ...
  def _min_max(self) -> Tuple[Optional[UOp], Optional[UOp]]:
    ...
    if self.op is UOps.ALU and cast(DType, self.dtype).count == 1:
      ...
      if self.arg is BinaryOps.MUL and (s0.vmin.arg >= 0 or s1.vmin.arg >= 0):
        ...
        return self.const_like(Lmin*Rmin), self.const_like(Lmax*Rmax)
      if self.arg is BinaryOps.MOD and s1.vmin.arg > 0: return self.const_like(0), (s1.vmax-1)
      if self.arg is BinaryOps.IDIV and s1.op is UOps.CONST:
        if s1.arg > 0: return self.const_like(s0.vmin.arg//s1.arg), self.const_like(s0.vmax.arg//s1.arg)
        if s1.arg < 0: return self.const_like(-(s0.vmax.arg//-s1.arg)), self.const_like(-(s0.vmin.arg//-s1.arg))
