...
Op = UnaryOps | BinaryOps | ReduceOps | MovementOps | LoadOps | FusedOps
OpType = type[UnaryOps] | type[BinaryOps] | type[ReduceOps] | type[MovementOps] | type[LoadOps] | type[FusedOps]
...
class LazyOp:
  ...
  op: Op
  src: tuple[LazyOp | LazyBuffer, ...]
  arg: Any
  buffers: tuple[LazyBuffer, ...]

  def __init__(self, op: Op, src: tuple[LazyOp | LazyBuffer, ...], arg: Any = None):
    self.op = op
    self.src = src
    self.arg = arg
    try:
...
class ASTRunner:
  def __init__(self, name, prg, global_size:list[int] | None=None, local_size:list[int] | None=None, op_estimate=0, mem_estimate=0, display_name:str | None=None, runtime_args:dict | None=None):
  ...
  def __call__(self, rawbufs:list[RawBuffer], jit=False, force_wait=False) -> float | None:
    ...
    if DEBUG >= 2:
      print(f"{colored(f'*** {GlobalCounters.kernel_count:4d}', 'magenta' if jit else None)} {(self.display_name+' '*(29-ansilen(self.display_name))) if self.display_name is not None else self.name:26s} arg {len(rawbufs):3d} sz {str(self.global_size):18s} {str(self.local_size):12s} OPs {int(self.op_estimate/1e6):6d}M/{GlobalCounters.global_ops/1e9:7.2f}G  mem {GlobalCounters.mem_used/1e9:5.2f} GB " +
            ('' if et is None else f"tm {et*1e6:9.2f}us/{GlobalCounters.time_sum_s*1e3:9.2f}ms ({self.op_estimate/((et or 1e-20)*1e9):8.2f} GFLOPS, {self.mem_estimate/((et or 1e-20)*1e9):7.2f} GB/s)"))
