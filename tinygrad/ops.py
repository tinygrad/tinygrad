...
class FlopCounter:
  ...
  dtype: DType
  flops: int
  mem: Dict[int, Tuple[DType, int]]
  @property
  def mem_estimate(self): return sum([dtype.itemsize*size for dtype,size in self.mem.values()])
  def consume_flops(self):
    self.flops, ret = 0, self.flops
    return ret

InterpretedFlopCounter: Dict[Op, Callable] = {
  BufferOps.LOAD: lambda arg: FlopCounter(arg.st.shape, arg.dtype, 0, {arg.idx: (arg.dtype, arg.st.size())}), BufferOps.CONST: lambda arg: FlopCounter(arg.st.shape, arg.dtype, 0, {}),
  BufferOps.STORE: lambda self,arg: FlopCounter(arg.st.shape, arg.dtype, self.consume_flops(), {**self.mem, arg.idx: (arg.dtype, arg.st.size())}), UnaryOps.CAST: lambda self,arg: FlopCounter(self.shape, arg[0], self.consume_flops(), self.mem),   # cast uses no flops
