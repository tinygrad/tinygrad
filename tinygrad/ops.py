    ...
    def get_program():
      ...
      if not getenv("NOOPT"):
        if BEAM >= 1 and not vars_from_ast(ast):
          ...
          baseline_lins = [("hc", Linearizer(ast, self.linearizer_opts))]
          baseline_lins[-1][1].hand_coded_optimizations()
          tc_lin = Linearizer(ast, self.linearizer_opts)
          if tc_lin.apply_tensor_cores(getenv("TC", 1)):
            baseline_lins.append(("tc", tc_lin))
          k = beam_search(kb, baseline_lins, rawbuffers, BEAM.value)
        else:
