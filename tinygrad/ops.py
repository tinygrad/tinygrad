...
class UOp(MathTrait):
  ...
  def __new__(cls, op:UOps, dtype:DType=dtypes.void, src:Tuple[UOp,...]=tuple(), arg:Any=None):
    if op is UOps.ALU and arg in COMMUTATIVE and src[0].tuplize < src[1].tuplize: src = src[::-1]
  ...
  def __init__(self, op:UOps, dtype:DType=dtypes.void, src: Tuple[UOp,...]=tuple(), arg:Any=None):
    ...
    if op is UOps.ALU and arg in COMMUTATIVE and src[0].tuplize < src[1].tuplize: self.src = self.src[::-1]
