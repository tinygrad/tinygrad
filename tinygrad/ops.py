  ...
  def rewrite(self, uop:UOp) -> Optional[UOp]:
    ...
    for p,fxn,early_reject in self.pdict[(uop.op, uop.arg)] + self.pdict[(uop.op, None)]:
      ...
      if (matches := _match(uop, p, {})) and (ret:=fxn(**matches[0])) is not None:
        ...
        if TRACK_MATCH_STATS >= 2: contexts[-1].rewrite_steps.append((uop, ret, p.printable()))
  ...
  def print_match_stats():
    ...
    if TRACK_MATCH_STATS >= 2:
      with open("/tmp/rewrites.pkl", "wb") as f:
        print(f"rewrote {len(contexts)} graphs and applied {sum(len(x.rewrite_steps) for x in contexts)} rules, saved to /tmp/rewrites.pkl")
