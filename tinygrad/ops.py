...
symbolic = symbolic_simple+PatternMatcher([
  ...
  *((UPat.var("x").alu(op, UPat.cvar("c1")).alu(op, UPat.cvar("c2")).named("f"),
  ...
  (UPat.var("x") // UPat.var("y"), lambda x,y: \
    q if y.vmin != 0 != y.vmax and (q:=x.vmin//y.vmin) == x.vmax//y.vmin == x.vmin//y.vmax == x.vmax//y.vmax else None),
  (UPat.var("x") % UPat.var("y"), lambda x,y: \
    x-y*q if y.vmin != 0 != y.vmax and (q:=x.vmin//y.vmin) == x.vmax//y.vmin == x.vmin//y.vmax == x.vmax//y.vmax else None),
])
