    ...
    def get_program():
      ...
      if not getenv("NOOPT"):
        ...
        if BEAM:
          ...
          baseline, beamtime = time_linearizer(k, rawbuffers, allow_test_size=False), time_linearizer(kb, rawbuffers, allow_test_size=False)
          if beamtime < baseline:
            if DEBUG >= 1: print(f"beam search beat baseline by {baseline/beamtime:.2f}x")
            k = kb
