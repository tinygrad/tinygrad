...
class UOp(MathTrait, metaclass=UOpMetaClass):
  ...
  def _min_max(self) -> Tuple[ConstType, ConstType]:
    ...
    if self.op is UOps.ALU:
      ...
      if self.arg is BinaryOps.MUL and not dtypes.is_float(self.dtype):
        return min(vals:=(s0.vmin*s1.vmin, s0.vmin*s1.vmax, s0.vmax*s1.vmin, s0.vmax*s1.vmax)), max(vals)
