...
def pool(x, py, px, fxn=None):
  s = x if fxn else x.shape
  my, mx = (s[2]//py)*py, (s[3]//px)*px
  stack, xup = (None,None) if fxn else (list([]),x[:,:,:my,:mx])
  for Y, X in itertools.product(range(py), range(px)):
    if fxn:
      ll = fxn(Y*px+X)
      if X == 0 and Y == 0:
  ...
  return ret if fxn else np.concatenate(stack, axis=0)

class MaxPool2D(Function):
  @staticmethod
  def forward(ctx, x, kernel_size=(2, 2)):
    stack = pool(x, *kernel_size)
