...
class Compiled:
  ...
  def exec_ast(self, ast:LazyOp, output, **kwargs):
    ...
    def apply_opt(k, x):
      ...
      """
      optstring = [[(x[0], "U"), (x[1], "L")], [(x[2], "U"), (x[3], "L")], [(x[4], "U")]]
      for axis,work in list(enumerate(optstring))[::-1]:
        for amt, typ in work:
          if typ == "U":
            k.shift_to(axis, amt)
            k.upcast()
            ...
            k.shift_to(axis, amt, insert_before=k.first_reduce)
            k.local_dims += 1
          #print(axis, amt, typ, k.colored_shape())
      """
    ...
    def opt(x):
      try:
        ...
        tm = min([prg.exec(k.bufs, force_wait=True) for _ in range(10)])*1000
        #print(x, tm)
        return tm
      except Exception:
        #import traceback
    ...
    if hasattr(k, 'key') and getenv("ENABLE_METHOD_CACHE", 1):
      if k.key not in self.method_cache:
        ...
        for i in range(k.first_reduce):
          opts.append(ng.p.TransitionChoice([(None, None, None), (i,2,"U"), (i,4,"U"), (i,8,"U"), (i,16,"U")]))
          opts.append(ng.p.TransitionChoice([(None, None, None), (i,4,"L"), (i,8,"L"), (i,16,"L"), (i,32,"L")]))
        for i in range(k.shape_len-k.first_reduce):
          opts.append(ng.p.TransitionChoice([(None, None, None), (i,2,"R"), (i,4,"R"), (i,8,"R")]))
