...
class UOp(MathTrait, metaclass=UOpMetaClass):
  ...
  def valid(self, st:ShapeTracker):
    ...
    true_val = UOp.metaop(Ops.CONST, st.shape, self.dtype, self.device, self.const_arg)
    return UOp(Ops.VALID, dtypes.bool, (st.to_uop(),)).where(true_val, 0)
  @staticmethod
  ...
  def copy_to_device(self, device:str, force=False, clone:bool=False) -> UOp:
    ...
    return UOp.metaop(Ops.COPY, src.shape, src.dtype, device, clone, (src,)).view(unwrap(self.st))
...
class UPat(MathTrait):
  ...
  def sink(self, *x:UPat, **kwargs): return UPat(Ops.SINK, dtypes.void, (self,)+x, **kwargs)
...
symbolic_simple = PatternMatcher([
  ...
  (UPat.var("x", dtype=dtypes.bool).where(UPat.const(dtypes.bool, True), UPat.const(dtypes.bool, False)), lambda x: None if x.op is Ops.VALID else x),
  # ** zero folding **
