...
class UOp(MathTrait, metaclass=UOpMetaClass):
  ...
  def metaop(op:Ops, shape:Tuple[sint, ...], dtype:DType, device:str, arg=None, src:Tuple[UOp, ...]=()) -> UOp:
    from tinygrad.shape.shapetracker import ShapeTracker
    if op is Ops.CONST:
      tensor_const = UOp.const(dtype, unwrap(arg)).replace(src=(UOp(Ops.DEVICE, arg=device).view(ShapeTracker.from_shape(())),))
      return tensor_const.reshape((1,)*len(shape)).expand(shape)
