...
def gated_load(ctx, addr:UOp, alt:UOp, gate:UOp, x:UOp):
  ...
  # the gate picks one of two places to access, the AFTER orders the access after the scratch store
  sel = gate.where(addr, local_idx)
  return UOp(Ops.AFTER, (sel, (local_idx if x.max_numel() == 1 else local).store(alt))).load()
...
def gated_store(addr:UOp, gate:UOp, val:UOp):
  ...
  return UOp(Ops.AFTER, (sel,)).store(val)
...
pre_isel_matcher = PatternMatcher([
  ...
  # a cast between same width ints keeps every bit (a bool is a 0/1 byte), the honest op is a bitcast
...
# a float comparison consumed as a mask writes all ones in the width of its operands, the instruction states that width. a bool comparison can't
def mask(x:UOp) -> UOp:
...
# the value of a place is its address, computed into a 64bit register
def lea(x:UOp) -> UOp: return x.ins(X86Ops.LEA, dtype=dtypes.uint64, src=fold_address(x)) if x.op in {Ops.INDEX, Ops.SHRINK} else x
def where_places(m:UOp, a:UOp, b:UOp) -> UOp|None:
  p = a if a.op in {Ops.INDEX, Ops.SHRINK} else b if b.op in {Ops.INDEX, Ops.SHRINK} else None
  return m.where(lea(a), lea(b)) if p is not None and not _is_vec_xmm(p.src[0]) else None

def abi(ctx:IselContext, x:UOp) -> UOp|None:
  if isinstance(x.tag, tuple): return None
  i = ctx.func_args.index(x)
  # a buffer param holds an address, the abi passes it as a 64bit int and a ParamArg owns the dtype its param is passed as
...
isel_matcher = PatternMatcher([
  ...
  # a gate that picks between two places moves an address, not a value: lea each place arm so the select is a 64bit address
  (UPat.var("m").where(UPat.var("a"), UPat.var("b")), where_places),
  ...
  # a float comparison has no flag form, the blend consumes it as a mask. mask and value are the same width
  ...
  # in this case we have a mask producing comparison whose user expects a bool, the mask is all ones or all zeros so the bool is mask & 1
  ...
  # movzx only extends from an 8 or 16bit src, there is no 32 to 64bit form
  ...
  # the remaining scalar int casts have no extending instruction, they are a MOV sized by the destination:
  # narrowing writes the low bits, and widening is only from uint32 whose upper half a 32bit write already zeroed
...
class X86Renderer(ISARenderer):
  ...
  def copy(self, x:UOp, reg:Register):
    ...
    ret = isel_matcher.rewrite(UOp(Ops.COPY, (x,), tag=reg))
