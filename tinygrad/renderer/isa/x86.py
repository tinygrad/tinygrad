...
def abi(ctx:IselContext, x:UOp) -> UOp|None:
  ...
  if x.op is Ops.SPECIAL or x.arg.name == "core_id": src = _reg_arg(runtime_reg)
  else:
    packed = [u for u in ctx.func_args if u.op is Ops.PARAM and u.arg.name != "core_id"]
    off = dict(ISARenderer.param_layout(packed)[0])[x]
    disp = imm(dtypes.int32 if off > dtypes.int8.max else dtypes.int8, off)
    src = (def_reg(dtypes.uint64, args_buf_reg), UOp(Ops.NOOP), disp, imm(dtypes.uint8, dt.itemsize))
