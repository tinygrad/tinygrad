...
class WGSLRenderer(CStyleLanguage):
  ...
  string_rewrite = PatternMatcher([
    ...
    (UPat(Ops.DEFINE_REG, name="x"), lambda ctx,x:
      f"var<workgroup> {ctx[x]}: array<{ctx.buf_map(x.dtype.base)}," \
      f"{x.dtype.size//(4//x.dtype.itemsize) if is_packed(x.dtype) else x.dtype.size}>;" if x.arg[0] == "local" else None),
