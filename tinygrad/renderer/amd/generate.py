...
def write_enum(enums, path):
  lines = ["# autogenerated from AMD ISA XML - do not edit", "from tinygrad.runtime.autogen.amd.common import ReprEnum, Fmt, FMT_BITS, OpType  # noqa: F401", ""]
...
def write_ins(encodings, enums, suffix_only_ops, types, arch, path):
  ...
  lines = ["# autogenerated from AMD ISA XML - do not edit", "# ruff: noqa: F401,F403",
           "from tinygrad.renderer.amd.dsl import *", f"from tinygrad.runtime.autogen.amd.{arch}.enum import *", "import functools", ""]
...
def write_operands(types, enums, arch, path):
  ...
  lines = ["# autogenerated from AMD ISA XML - do not edit",
           ...
           f"from tinygrad.runtime.autogen.amd.{arch}.enum import *", ""]
...
if __name__ == "__main__":
  ...
  # Write common.py
  common_path = pathlib.Path(__file__).parent.parent.parent / "runtime" / "autogen" / "amd" / "common.py"
  ...
  for arch, data in arch_data.items():
    base = pathlib.Path(__file__).parent.parent.parent / "runtime" / "autogen" / "amd" / arch
