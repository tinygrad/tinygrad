...
asm_for_op: Dict[Ops, Callable] = {
  ...
  Ops.WHERE: lambda d,a,b,c,dt,name: [f"@{a} mov.{name} {d}, {b};",
    f"@!{a} mov.{name} {d}, {c};" if name == "pred" else f"selp.{'b16' if name == 'f16' else name} {d}, {b}, {c}, {a};"]
}
...
string_rewrite = PatternMatcher([
  ...
  (UPat(Ops.LOAD, name="x", src=(UPat.var('loc'),), allow_any_len=True), lambda ctx, x, loc:
   ...
   f"{('{' + ', '.join(ctx.r[x]) + '}') if x.dtype.count > 1 else ctx.r[x]}, [{ctx.r[loc]}+0];"),
