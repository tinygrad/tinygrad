...
def render_wmma(ctx: "PTXRenderer", wmma: UOp):
  ...
  for i, reg in enumerate(ctx.wmma_r[2]): # unpack output registers
...
string_rewrite = PatternMatcher([
  ...
  (UPat(Ops.LOAD, name="x", src=(UPat.var('loc'), UPat(name='alt'), UPat(name="gate", op=GroupOp.ALU))), lambda ctx, x, loc, alt, gate: flatten([
    ...
    f"@!{ctx.r[gate]} mov.b{ctx.types[x.dtype.scalar()][1:]} {ctx.r[x]}, {render_val(alt.arg, alt.dtype) if alt.op == Ops.CONST else ctx.r[alt]};"]),
