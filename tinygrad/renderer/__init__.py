...
class Estimates:
  ...
  def from_uops(uops:list[UOp], ignore_indexing=False) -> Estimates:
    ...
    if ignore_indexing:
      for u in uops:
        if u.op in {Ops.LOAD, Ops.STORE} and u.src[0].dtype.addrspace != AddrSpace.REG:
    ...
    for u in uops:
      ...
      elif u.op is Ops.LOAD and u.src[0].dtype.addrspace != AddrSpace.REG: lds += u.dtype.itemsize * mults
      elif u.op is Ops.STORE and u.src[0].dtype.addrspace != AddrSpace.REG: lds += u.src[1].dtype.itemsize * mults
