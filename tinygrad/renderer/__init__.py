...
class TensorCore: # D = A * B + C, A is (M x K), B is (K x N), C and D are (M x N)
  ...
  early_upcast_axes_shape: Optional[List[Tuple[int,int]]] = None
  @property
  def early_upcast_axes(self) -> List[Tuple[int,int]]: # list of (TC dim,amt) that upcasts the threads remainders of dims [0,1]
    return self.early_upcast_axes_shape or \
      [(d,self.dims[d]//sz) for d,sz in [(dim,prod(sz for d,sz in self.threads if d==dim)) for dim in range(2)] if self.dims[d]>sz]
