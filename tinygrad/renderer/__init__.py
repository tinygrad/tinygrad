...
class ProgramSpec:
    ...
    if not self._ran_post_init and self.uops is not None:
      ...
      for u in self.uops:
        ...
        if u.op is Ops.STORE:
          self.outs.extend([x.arg for x in u.src[0].toposort() if x.op is Ops.DEFINE_GLOBAL])
        if u.op is Ops.LOAD:
          self.ins.extend([x.arg for x in u.src[0].toposort() if x.op is Ops.DEFINE_GLOBAL])
  ...
  def applied_opts(self) -> tuple[Opt, ...] | None:
    return self.uops[-1].arg.applied_opts if self.uops is not None and self.uops[-1].op is Ops.SINK and self.uops[-1].arg is not None else None
