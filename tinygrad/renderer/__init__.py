...
class TensorCore:  # D = A * B + C, A is (M x K), B is (K x N), C and D are (M x N)
    ...
    opts: tuple[
        str, ...
    ]  # ordered tuple of "ux" or "lx" specifing kernel opts to perform. "ux" upcasts dim x and "lx" localizes dim x
    swizzle: tuple[
...
class Estimates:
    ...
    def from_uops(uops: list[UOp], ignore_indexing=False) -> Estimates:
        ...
        return Estimates(
            flops, lds, lds
        )  # TODO: properly track memory, lds is always a high estimate
...
class ProgramSpec:
    ...
    mem_estimate: sint = (
        0  # TODO: get this from the load/store uops once min/max are good
    )
        ...
        if not self._ran_post_init and self.uops is not None:
            ...
            for u in self.uops:
                ...
                if u.op is Ops.STORE:
                    self.outs.extend(
                        [x.arg for x in u.src[0].toposort if x.op is Ops.DEFINE_GLOBAL]
                    )
                if u.op is Ops.LOAD:
                    self.ins.extend(
                        [x.arg for x in u.src[0].toposort if x.op is Ops.DEFINE_GLOBAL]
                ...
                if u.op is Ops.SPECIAL:
                    ...
                    special_size = (
                        self.local_size if u.arg[0][0] == "l" else self.global_size
                    )
...
class Renderer:
    ...
    global_max: Optional[tuple[int, ...]] = (0x8FFFFFFF,) * (
        3
    )  # TODO: Ops.SPECIAL int32 indexes right now
    local_max: Optional[tuple[int, ...]] = (0x8FFFFFFF,) * (
        3
    )  # TODO: Ops.SPECIAL int32 indexes right now
