...
math_ops = {Ops.EXP2: ("exp2", "pure"), Ops.LOG2: ("log2", "pure"), Ops.SQRT: ("sqrt", "const"), Ops.SIN: ("sin", ""), Ops.TRUNC: ("trunc", "")}
def render_math_op(op: Ops): return lambda x, dtype,op=op: f"__ocml_{math_ops[op][0]}_f{ {dtypes.half:16, dtypes.double:64}.get(dtype, 32) }({x})"
...
class AMDRenderer(CStyleLanguage):
  ...
  code_for_op = {**CStyleLanguage.code_for_op, **{op: render_math_op(op) for op in math_ops}}
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
    ...
    used_ocml = {(f"__ocml_{math_ops[uop.op][0]}_f{dt.itemsize * 8}", dt.name, dt.name, math_ops[uop.op][1]) for uop in uops if uop.op in math_ops
                  for dt in used_dtypes if dt in [dtypes.float,dtypes.double,dtypes.half]}
  ...
  return (unsigned char)(is_bf8?__builtin_amdgcn_cvt_pk_bf8_f32(v,v,0,false):__builtin_amdgcn_cvt_pk_fp8_f32(v,v,0,false));\n}""")
    prefix += [f'extern "C" __attribute__((device{f", {atr}" if atr else ""})) {dto} {meth}({dti});' for meth,dti,dto,atr in ockl+list(used_ocml)]
