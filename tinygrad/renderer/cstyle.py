    ...
    for u in uops:
      ...
      if uop is UOps.IF:
        ...
        if uop is UOps.RANGE:
          ...
          elif args is BinaryOps.MAX: operands = [self.render_cast(r[v], v.real_dtype) if v.op is UOps.CONST else r[v] for v in src]
          ...
          if uop is UOps.BITCAST:
            precast = ssa('precast')
            kk(f"{self.render_dtype(src[0].real_dtype)} {precast} = {r[src[0]]};")
