...
class DSPRenderer(ClangRenderer):
  ...
  type_map = { **({k:v for k,v in ClangRenderer().type_map.items()}), dtypes.uint64: "unsigned long long", dtypes.int64: "long long" }
  code_for_op = {**({k:v for k,v in ClangRenderer().code_for_op.items()}), UnaryOps.SIN: lambda x,dtype: f"__builtin_sin({x})",
