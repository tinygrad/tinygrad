  ...
  def render(self, name:str, uops:List[UOp]) -> str:
    ...
    for u in uops:
      ...
      if uop is UOps.IF:
        ...
        assert src[0].dtype is not dtypes.void and src[2].dtype is not dtypes.void
        ...
        if uop is UOps.RANGE:
          ...
          assert src[0].dtype is not dtypes.void
...
class ClangRenderer(CStyleLanguage):
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
    prefix = [self.render_vector_prefix(dtype) for dtype in dedup(uop.dtype for uop in uops if uop.dtype is not dtypes.void and uop.dtype.count>1)]
...
class CUDARenderer(CStyleLanguage):
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None):
    ...
    for dtype in dedup(uop.dtype for uop in uops if uop.dtype is not dtypes.void and uop.dtype in (dtypes.half, dtypes.bfloat16)):
...
""")
    ...
    for dtype in dedup(uop.dtype for uop in uops if uop.dtype is not dtypes.void and uop.dtype.count > 1):
      prefix.append(self.render_vector_prefix(dtype))
