...
class CStyleLanguage(Renderer):
  ...
  code_for_op: dict = {
    ...
    Ops.EXP: lambda x,dtype: f"exp({x})", Ops.EXP2: lambda x,dtype: f"exp2({x})",
    Ops.LOG2: lambda x,dtype: f"log2({x})", Ops.SIN: lambda x,dtype: f"sin({x})",
...
class MetalRenderer(CStyleLanguage):
  ...
  extra_matcher = PatternMatcher([
    ...
    (UPat((Ops.SQRT, Ops.EXP, Ops.EXP2, Ops.LOG2, Ops.SIN), dtype=dtypes.bfloat16, name="x"),
...
class CUDARenderer(CStyleLanguage):
  ...
  code_for_op = { **CStyleLanguage.code_for_op,
    ...
    Ops.EXP: lambda x,dtype: f"hexp({x})" if dtype in (dtypes.half, dtypes.bfloat16) else f"exp({x})",
...
class AMDRenderer(CStyleLanguage):
  ...
  ocml = [(f"__ocml_{name}_f{n}", f"{dt}, {dt}" if "fmax" == name else dt, dt, atr)
            ...
            for name, atr in [("fmax", "const"), ("exp", "pure"), ("exp2", "pure"), ("log2", "pure"), ("sqrt", "const"), ("sin", ""), ("trunc", "")]]
  ...
  code_for_op = { **CStyleLanguage.code_for_op,
    ...
    Ops.EXP: lambda x,dtype: f"__ocml_exp_f{ {dtypes.half:16, dtypes.double:64}.get(dtype, 32)}({x})",
