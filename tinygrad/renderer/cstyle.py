...
class CStyleLanguage(Renderer):
  ...
  code_for_op: dict = {
    ...
    Ops.WHERE: lambda a,b,c,dtype: f"({a}?{b}:{c})", Ops.CMPEQ: lambda a,b,dtype: f"({a}=={b})", Ops.MAX: lambda a,b,dtype: f"max({a},{b})"}
  ...
  def _render(self, uops:list[UOp]) -> tuple[str, list[str], list[tuple[str,tuple[DType,bool]]]]:
    ...
    for u in uops:
      ...
      elif u.op is Ops.RANGE: r[u] = f"ridx{u.arg[0]}" if isinstance(u.arg, tuple) else f"ridx{u.arg}"
      else:
