...
class CStyleLanguage(Renderer):
  ...
  code_for_op: Dict = {
    ...
    BinaryOps.IDIV: lambda a,b,dtype: f"{'(int)' if dtypes.is_int(dtype) else ''}({a}/{b})", BinaryOps.MAX: lambda a,b,dtype: f"max({a},{b})", 
    BinaryOps.MOD: lambda a,b,dtype: f"({a}%{b})", BinaryOps.CMPLT: lambda a,b,dtype: f"({a}<{b})", BinaryOps.CMPNE: lambda a,b,dtype: f"({a}!={b})", 
...
code_for_op_half = {UnaryOps.RECIP: lambda x,dtype: f"({'(half)'if dtype == dtypes.half else ''}1/{x})",
