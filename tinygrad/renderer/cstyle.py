...
class CStyleLanguage(Renderer):
  ...
  def render_store(self, buf_name:str, buf_dtype:DType, var_name:str, var_dtype:DType, idx:str, local=False) -> str:
    ...
    if var_dtype.count > 1:
      if(var_name.startswith('zreg')): return f"__stz(&{buf_name}[{idx}], {int(var_name[4:var_name.index('.')])*buf_dtype.itemsize%64}); //{var_name[:var_name.index('.')]}"
  ...
  def render(self, name:str, uops:UOpGraph) -> str:
    ...
    for u in uops:
      ...
      if uop is UOps.IF:
        ...
        elif uop is UOps.MMA: kk(f"__{args[0]}({r[src[0]]}, {r[src[1]]}); // {', '.join([r[src[x]] for x in range(2, len(src))])}")
