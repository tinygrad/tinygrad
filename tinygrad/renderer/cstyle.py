...
class CStyleLanguage(NamedTuple):
  ...
  def render_cast(self, x: List[str], var_dtype: DType, bitcast=False) -> str:
    if bitcast:
      return f"(*(({self.buffer_prefix}{self.render_dtype(var_dtype)}*)&{x[0]}))"
    if len(x) == 1:
  ...
  def render_const(self, x:Union[float,int,bool], var_dtype) -> str:
    ...
    else: val = f"{float(x)}l" if var_dtype == dtypes.float64 else f"{float(x)}f" if dtypes.is_float(var_dtype) else f"{int(x)}" if dtypes.is_int(var_dtype) else f"{bool(x)}".lower()  # noqa: E501
  ...
  def render_load(self, output_dtype, buf_name, buf_dtype, idx, local=False) -> str:
    ...
    if output_dtype.sz > 1:
      out_val = (
        f"*(({self.smem_prefix if local and self.smem_prefix_for_cast else self.buffer_prefix}{buf_dtype.name}{output_dtype.sz}*)({buf_name}+{idx}))"  # noqa: E501
      )
    else:
  ...
  def render_kernel(self, function_name: str, kernel: List[str], bufs: List[Tuple[str, DType]], local_size: List[int]) -> str:
    ...
    buftypes = [
      (
        ...
        else ("const " if i > 0 else "") + self.buffer_prefix + self.render_dtype(dtype) + "*" + self.buffer_suffix
    ...
    if self.half_prekernel and any(dtype in [dtypes.float16, dtypes.bfloat16] for _, dtype in bufs):
      prg = "".join((self.half_prekernel, "\n", prg))
    return prg
  ...
  for u in uops:
    ...
    if uop is UOps.IF:
      ...
      if uop is UOps.LOOP:
        ...
        if len(vin) > 3:
          val = lang.code_for_op[TernaryOps.WHERE](r[vin[2]], val, r[vin[3]], dtype)
        ...
        if child_count[u] <= 1:
          ...
          kk(f"{dtype.name} {ssa(u,'cast')} = {val};")
        ...
        kk(f"{dtype.name} {ssa(u, 'wmma')} = {args}({r[vin[0]]}, {r[vin[1]]}, {r[vin[2]]});")
      elif uop is UOps.DEFINE_ACC:
        kk(f"{dtype.name} {ssa(u,'acc')} = {lang.render_const(args, dtype)};")
      elif uop is UOps.CONST:
        r[u] = lang.render_const(args, dtype) if args >= 0 else f"({lang.render_const(args, dtype)})"
      elif uop is UOps.GEP:
        r[u] = f"({r[vin[0]]})[{args}]" if cast(DType, vin[0].dtype).sz > 4 else f"({r[vin[0]]}).{'xyzw'[args]}"
      else:
...
def _make_hip_dtype(base_type, name, cnt):
  ...
  return (
    f"typedef {base_type} {name}{cnt} __attribute__((ext_vector_type({cnt})));\n"
    + f"static inline __attribute__((device)) {name}{cnt} make_{name}{cnt}("
    + ", ".join([f"{base_type} {x}" for x in nms])
    + ") { return {"
    + ", ".join(nms)
    + "}; }"
  )
