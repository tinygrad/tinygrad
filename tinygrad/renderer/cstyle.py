...
class ClangRenderer(CStyleLanguage):
  ...
  def _render_defines(self, function_name):
    defs = [self.render_vector_prefix(dt) for dt in dedup(uop.dtype for uop in uops if uop.dtype.count>1)]
    if AMX:
      ...
      for name, (N, M, _), dtype_in, _, _, _, _, _ in dedup([uop.arg for uop in uops if uop.op is UOps.WMMA]):
        ...
        defs += [f"""{(out := self.render_dtype(dtype_in.vec(N*N)))} __{name}({self.render_dtype(dtype_in.vec(N))} data1, {self.render_dtype(dtype_in.vec(M))} data2, {out} data0){{
...
class DSPRenderer(ClangRenderer):
  ...
  def _render_entry(self, function_name:str, bufs:List[Tuple[str,Tuple[DType,bool]]]) -> str:
    ...
    msrc += [f'int off_{i} = ((int*)pra[1].buf.pv)[{i}];' for i,b in enumerate(bufs) if isinstance(b[1][0], PtrDType)]
    msrc += [f'void *buf_{i} = HAP_mmap(0, sz_or_val_{i}, 3, 0, pra[{i+3}].dma.fd, 0) + off_{i};' for i,b in enumerate(bufs) if isinstance(b[1][0], PtrDType)]
