  ...
  def _render(self, uops: list[UOp]) -> tuple[str, list[str], list[tuple[str, tuple[DType, bool]]]]:
    ...
    for u in uops:
      ...
      if u.op is Ops.SPECIAL:
        ...
        r[u] = "ridx" + "_".join([str(x) if x >= 0 else "m" + str(-x) for x in u.arg[0:-1]])
      else:
        prefix = {
          Ops.WMMA: "wmma",
...
class ClangRenderer(CStyleLanguage):
  ...
  code_for_op = {
    **({k: v for k, v in CStyleLanguage.code_for_op.items() if k not in [Ops.EXP2, Ops.SIN, Ops.LOG2, Ops.TRUNC, Ops.RECIP]}),
...
class AMDRenderer(CStyleLanguage):
  ...
  def __init__(self, arch: str):  # gfx942 => MI300, gfx1100 => RX 7900, gfx1201 => RX 9700
    ...
    if self.tensor_cores == tc.amd_cdna:
      self.string_rewrite = (
        PatternMatcher([(UPat(Ops.WMMA, name="x"), lambda ctx, x: f"__{x.arg[0]}({ctx[x.src[0]]}, {ctx[x.src[1]]}, {ctx[x.src[2]]}, 0, 0, 0)")])
        + base_rewrite
      )
