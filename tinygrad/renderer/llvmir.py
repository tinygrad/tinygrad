...
def lconst(x, dtype: DType):
    if dtype in dtypes.floats:
        if math.isinf(x) or math.isnan(x):
            return "0x%02X%02X%02X%02X%02X%02X%02X%02X" % tuple(
                struct.pack("d", x)[::-1]
            )
...
base_rewrite = PatternMatcher(
    [
        ...
        (
            ...
            lambda ctx, x: f"  {ctx[x]} = getelementptr inbounds {ldt(x.dtype.base)}, {ldt(x.src[0].dtype)} {ctx[x.src[0]]}, {ldt(x.src[1].dtype)} {ctx[x.src[1]]}",
        ),
        (
            UPat(
                Ops.LOAD,
                src=(UPat.var("idx"), UPat.var("alt"), UPat.var("mask")),
                name="x",
            ),
        ...
        (
            ...
            lambda ctx, x: f"  store {ldt(x.src[1].dtype)} {ctx[x.src[1]]}, {ldt(x.src[0].dtype)} {ctx[x.src[0]]}",
        ),
        # GEP/VECTORIZE/CAST for float4 support
        (
            UPat(Ops.GEP, name="x"),
            lambda ctx, x: f"  {ctx[x]} = extractelement {ldt(x.src[0].dtype)} {ctx[x.src[0]]}, i32 {x.arg[0]}",
        ),
        (
        ...
        (
            ...
            lambda ctx, x: (
                f"  {ctx[x]} = bitcast {ldt(x.src[0].dtype)} {ctx[x.src[0]]} to {ldt(x.dtype)}"
        ...
        (
            ...
            lambda ctx, x: f"  {ctx[x]} = bitcast {ldt(x.src[0].dtype)} {ctx[x.src[0]]} to {ldt(x.dtype)}",
        ),
        (
            UPat(Ops.CAST, name="x"),
            lambda ctx, x: f"  {ctx[x]} = {lcast(x.src[0].dtype, x.dtype)} {ldt(x.src[0].dtype)} {ctx[x.src[0]]} to {ldt(x.dtype)}",
        ),
        (
        ...
        (
            ...
            lambda ctx, x: f"  {ctx[x]} = select {ldt(x.src[0].dtype)} {ctx[x.src[0]]}, {ldt(x.src[1].dtype)} {ctx[x.src[1]]}, {ldt(x.src[2].dtype)} {ctx[x.src[2]]}",
        ),
        # range
        (
            UPat(Ops.RANGE, name="x"),
            lambda ctx, x: f"  br label %loop_entry_{x.arg}\nloop_entry_{x.arg}:\n"
            f"  br label %loop_body_{x.arg}\nloop_body_{x.arg}:\n"
            f"  {ctx[x]} = phi {ldt(x.dtype)} [{ctx[x.src[0]]}, %loop_entry_{x.arg}], [{ctx[x]}phi, %loop_latch_{x.arg}]",
        ),
        (
            UPat(Ops.ENDRANGE, name="x"),
            lambda ctx, x: f"  br label %loop_latch_{x.src[0].arg}\nloop_latch_{x.src[0].arg}:\n"
        ...
        (
            UPat(Ops.IF, name="x"),
            lambda ctx, x: f"  br i1 {ctx[x.src[0]]}, label %ifbody_{ctx[x][1:]}, label %ifskip_{ctx[x][1:]}\nifbody_{ctx[x][1:]}:",
        ),
        (
            UPat(Ops.ENDIF, name="x"),
            lambda ctx, x: f"  br label %ifskip_{ctx[x.src[0]][1:]}\nifskip_{ctx[x.src[0]][1:]}:",
        ),
        # wmma
...
def llvm_bf16_cast(buf: UOp, idx: UOp, root: UOp):
    ...
    return (
        UOp.load(UOp.index(u16_buf, idx), dtype=dtypes.ushort)
        .cast(dtypes.uint)
        .mul(1 << 16)
...
class LLVMRenderer(Renderer):
    ...
    if AMX:
        tensor_cores = ClangRenderer.amx_tc

    extra_matcher = PatternMatcher(
        [
            # rewrite RECIP with FDIV
            (
                UPat(Ops.RECIP, name="x"),
                lambda x: UOp(Ops.FDIV, x.dtype, (x.const_like(1), x.src[0])),
            ),
            # rewrite cast to bool to CMPNE 0
            (
                UPat(Ops.CAST, dtype=dtypes.bool, name="x"),
                lambda x: x.src[0] != x.src[0].const_like(0),
            ),
            ...
            (
                UPat(
                    ...
                    src=(
                        UPat.load(
                            UPat.index(UPat.var("buf"), UPat.var("idx")),
    ...
    def render(self, name: str, uops: list[UOp]) -> str:
        ...
        for u in uops:
            if u.op in (Ops.DEFINE_GLOBAL, Ops.DEFINE_VAR):
                ...
                args.append(
                    f"{ldt(u.dtype)}{' noalias align 32' if isinstance(u.dtype, PtrDType) else ''} {r[u]}"
                )
            elif u.op is Ops.ASSIGN:
                pass  # assign is already handled by the first pass
            elif u.op is Ops.DEFINE_ACC:
                r[u] = r[u.src[0]]  # a define acc can be used and never be assigned to
                ...
                if u.op is Ops.RANGE:
                    for x in acc_to_assign:
                        if u in x.src:  # if this range is relevant for this acc
                            vc += 1
                            kernel.append(
                                f"  %acc{vc} = phi {ldt(x.dtype)}"
                                f"[{r[x]}, %loop_entry_{u.arg}], [{r[acc_to_assign[x]]}, %loop_latch_{u.arg}]"
                            )
                            r[x] = f"%acc{vc}"
