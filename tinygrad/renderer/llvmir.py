  ...
  def _render_kernel(self, uops: list[UOp], prefix:list[str]|None=None) -> tuple[tuple[str, ...], str]:
    ...
    for u in uops:
      ...
      if u.op in (Ops.DEFINE_GLOBAL, Ops.DEFINE_VAR):
        ...
        if u.op is Ops.WMMA and isinstance(self, AMDLLVMRenderer): l = render_wmma_amd(r, u, self.arch)
        # do the rendering of the llvm ir code
        elif (l:=self.string_rewrite.rewrite(u, ctx=r)) is None:
          raise RuntimeError(f"failed to render {u.op} with {u.dtype} srcs {[x.dtype for x in u.src]}")
        kernel.append(cast(str, l) if not isinstance(l, str) else l)
