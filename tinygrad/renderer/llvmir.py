  ...
  def __init__(self, arch:str):
    ...
    if self.arch.split(":")[0] == "gfx1100":
      self.extra_matcher += PatternMatcher([
        ...
        (UPat(Ops.WMMA, name="x"), lambda x: UOp(Ops.WMMA, dtypes.float.vec(8), (x.src[0].bitcast(dtypes.uint16.vec(16)),
          x.src[1].bitcast(dtypes.uint16.vec(16)), x.src[2]), (*x.arg,)) if x.src[0].dtype == dtypes.bfloat16.vec(16) else None),
      ])
