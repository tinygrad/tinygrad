...
class WebGPUJSRenderer(GraphRenderer):
  def render_graph(self) -> str:
    ...
    kernel_bufs = cast(dict[Buffer|Variable, str], self.bufs) # Variable args will map to WebGPU uniform buffers
    # render inputs
    for i, uop in enumerate(self.inputs):
      if uop.base.op is Ops.BUFFER: # from Tensor input
        arg_names.append(f"bufArg_{i}")
        copyins.append(copyin((buf:=self.bufs[uop.base.buffer]), f"bufArg_{i}"))
        validators.append(f"if (bufArg_{i}.byteLength !== {buf}.size) throw new Error(`bufArg_{i} does not have expected number of bytes`);")
