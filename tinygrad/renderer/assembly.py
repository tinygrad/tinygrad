...
class PTXRenderer(Renderer):
  ...
  tensor_cores = [
    TensorCore(
      dims=(8, 16, 16),
      threads=[(0, 2), (0, 2), (1, 2), (1, 2), (0, 2)],
      thread_local_sizes=[[2, 2, 2], [2, 2], [2, 2]],
      thread_local_aliases=[
        [[0], [0], [5], [-2], [0], [-1, 1, 2, -3], [3, 4]],
        [[3], [4], [0], [0], [5], [-1, 1, 2, -2], [0]],
        [[-1], [1], [5], [-2], [2], [0], [3, 4]],
      ],
      dtype_in=di,
  ...
  def __init__(self, arch: str):
    self.tensor_cores = PTXRenderer.tensor_cores if int(arch[3:]) >= 80 else []
...
.visible .entry"""
  ...
  asm_for_op: Dict[Op, Callable] = {
    ...
    UnaryOps.EXP2: lambda d, a, dt, name: f"ex2.approx.{name} {d}, {a};",
    UnaryOps.LOG2: lambda d, a, dt, name: f"lg2.approx.{name} {d}, {a};",
    ...
    TernaryOps.WHERE: lambda d, a, b, c, dt, name: f"@{a} mov.{name} {d}, {b};\n@!{a} mov.{name} {d}, {c};"
    if name == "pred"
  ...
  def render_const(self, x: ConstType, dtype: DType, mov=None) -> Union[List[str], str]:
    ...
    if dtype == dtypes.bool:
      return [f"setp.ne.s16 {mov}, {val}, 0;"]
  ...
  def render_loop(self, idx, start, label, acc=None) -> List[str]:
    return [f"mov.u32 {idx}, {start};", f"{label}:"]

  def render_bra(self, b1, pred=None, b2=None) -> List[str]:
    return [f"@{pred} bra {b1};", f"@!{pred} bra {b2};"] if pred else [f"bra {b1};"]

  def mem_type(self, dtype):
    return "s8" if dtype.itemsize == 1 else "b16" if dtype == dtypes.float16 else self.types[dtype]

  def render_load(self, loc, dest, dtype, gate=None, alt=None, ss="", offset=0) -> List[str]:
    assert dtype != dtypes.bool
    if gate:
      return [f"@{gate} ld{ss}.{self.mem_type(dtype)} {dest}, [{loc}+{offset}];", f"@!{gate} mov.b{self.types[dtype][1:]} {dest}, {alt};"]
  ...
  def render_cast(self, d: str, a: str, dtype: DType, atype: DType, bitcast=False, pred=False) -> List[str]:
    if bitcast:
      return [f"mov.b{self.types[dtype][1:]} {d}, {a};"]
    if atype == dtypes.bool:
      return [f"selp.b{self.types[dtype][1:]} {d}, {render_val(1, dtype)}, {render_val(0, dtype)}, {a};"]
    if dtype == dtypes.bool:
      return [f"setp.ne.b{self.types[atype][1:]} {d}, {a}, {self.render_const(0, atype)};"]
    rnd = (
      ".rzi"
      if dtypes.is_int(dtype) and dtypes.is_float(atype)
      else ".rn"
      if dtypes.is_float(dtype) and (dtype.itemsize < atype.itemsize or dtypes.is_int(atype) or atype == dtypes.bool)
      else ""
    )
  ...
  def render(self, name: str, _uops: UOpGraph) -> str:
    ...
    for u in uops:
      ...
      if uop is UOps.IF:
      ...
      elif uop is UOps.BARRIER and self.barrier:
        kk(self.barrier)
      elif uop is UOps.ENDRANGE:
        kk(
          self.asm_for_op[BinaryOps.ADD](r[vin[0]], r[vin[0]], "1", dtypes.int, self.types[dtypes.int]),
        ...
        if uop is UOps.RANGE:
          kk(*self.render_loop(ssa("ridx", u), r[vin[0]], ssa_label("loop", u)))
          ...
          if dtype.count > 1:
            ...
            for uu in r[u]:
              kk(f"mov.b{self.types[dtype.scalar()][1:]} {uu}, {const(args[0], dtype.scalar())};")
          else:
            kk(f"mov.b{self.types[dtype][1:]} {ssa('acc', u)}, {const(args[0], dtype)};")
          ...
          if dtype.count > 1:
            ...
            if len(vin) > 3:
              for v in r[u]:
                kk(f"mov.{self.mem_type(dtype.scalar())} {v}, {render_val(0, dtype.scalar())};")
            kk(
              (f"@{r[vin[2]]}" if len(vin) > 3 else "")
            ...
            kk(
              *self.render_load(
                ...
                ssa("val", u),
                dtype,
                gate=r[vin[2]] if len(vin) > 3 else None,
                alt=r[vin[3]] if len(vin) > 3 else None,
                ss=mem_type,
                offset=vin[1].arg,
              )
            )
          ...
          if dtype.count > 1:
            for x0, x1 in zip(r[vin[0]], r[vin[1]]):
              kk(f"mov.b{self.types[dtype.scalar()][1:]} {x0}, {x1};")
          else:
          ...
          if dtype.count > 1:
            ...
            _cast(r[vin[0]], dtype, vin[0].dtype, bitcast=uop is UOps.BITCAST, u=u)
          ...
          raise NotImplementedError(f"no code for {uop}")
...
ptx_matcher = PatternMatcher([
  ...
  (
    UPat(
      ...
      uop=UOps.ALU,
      arg=BinaryOps.ADD,
      dtype={dtypes.float16, dtypes.bfloat16, dtypes.float32, dtypes.float64},
  ...
  *[
    ...
    for op in PTXRenderer.asm_for_op.keys()
    if op not in PTXRenderer.supports_half
  ],
  (
    UPat(name="root", uop=UOps.LOAD, dtype=dtypes.bool, vin=(UPat(name="x"), UPat(name="y"), UPat(name="z"), UPat(name="k"))),
  ...
  (
    UPat(
      name="root",
      uop={UOps.LOAD, UOps.STORE},
      vin=(UPat(uop={UOps.DEFINE_LOCAL, UOps.DEFINE_GLOBAL}), UPat(name="const", uop=UOps.CONST)),
      __allow_len__=[2, 3, 4, 5],
    ),
  ...
  (
    UPat(
      name="root", uop={UOps.LOAD, UOps.STORE}, vin=(UPat(uop={UOps.DEFINE_LOCAL, UOps.DEFINE_GLOBAL}), UPat(name="alu")), __allow_len__=[2, 3, 4, 5]
    ),  # no const here
    lambda root, alu: UOp(
      root.uop,
      root.dtype,
      (alu.cast(dtypes.int64) * UOp.const(dtypes.int64, root.vin[0].dtype.itemsize) + root.vin[0].cast(dtypes.int64), UOp.const(dtypes.int64, 0))
      + root.vin[2:],
    ),
  ),
])