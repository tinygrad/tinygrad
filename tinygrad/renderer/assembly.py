...
ptx_matcher = PatternMatcher([
  (UPat(UOps.ALU, BinaryOps.CMPEQ, (UPat(dtype = dtypes.bool),UPat()), "root"),
  ...
  *[(UPat(UOps.ALU, op, name = "x", dtype = dtypes.half),
  ...
  (UPat(UOps.LOAD, vin = (UPat(name = "x"),UPat(name = "y"),UPat(name = "z"),UPat(name = "k")), name = "root", dtype = dtypes.bool),
  lambda root,x,y,z,k: UOp(UOps.CAST, dtypes.bool, (UOp(root.uop, dtypes.int8, (x,y,z,UOp(UOps.CAST, dtypes.uint8, (k,)))),), root.arg)),
  (UPat(UOps.LOAD, vin = (UPat(),UPat()), name = "root", dtype = dtypes.bool),
  lambda root: UOp(UOps.CAST, dtypes.bool, (UOp(root.uop, dtypes.uint8, root.vin, root.arg),))),
  (UPat(UOps.STORE, vin = (UPat(),UPat(),UPat(name = "z",dtype = dtypes.bool), UPat()), name = "root"),
  lambda root,z: UOp(root.uop, root.dtype, root.vin[:2] + (UOp(UOps.CAST, dtypes.uint8, (z,)),), root.arg)),
  (UPat(UOps.STORE, vin = (UPat(),UPat(),UPat(name = "z",dtype = dtypes.bool)), name = "root"),
  lambda root,z: UOp(root.uop, root.dtype, root.vin[:2] + (UOp(UOps.CAST, dtypes.uint8, (z,)),), root.arg)),
  (UPat(UOps.STORE, vin = (UPat(),UPat(),UPat(),UPat(name = "g", dtype = dtypes.int)), name = "root"),
  lambda root,g: UOp(root.uop, root.dtype, root.vin[:3] + (UOp(UOps.CAST, dtypes.bool, (g,)),), root.arg)),
  # ptr_ar (load/store)
  (UPat({UOps.LOAD, UOps.STORE}, vin = (UPat({UOps.DEFINE_LOCAL,UOps.DEFINE_GLOBAL}),
                      UPat(UOps.ALU, BinaryOps.ADD, [UPat(name = "alu"), UPat(UOps.CONST, name = "const")])), name = "root", __allow_len__ = {2,3,4,5}),
  ...
  (UPat({UOps.LOAD, UOps.STORE}, vin = (UPat({UOps.DEFINE_LOCAL,UOps.DEFINE_GLOBAL}), UPat(UOps.CONST, name = "const")), name = "root",
