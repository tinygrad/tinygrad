  ...
  def render(self, name:str, uops:UOpGraph) -> str:
    ...
    for u in uops:
      ...
      if uop is UOps.IF:
        ...
        kk(*self.render_bra(f"IF_{r[vin[0]][1:]}_{[x for x in uops].index(u)}", _cast(r[vin[0]], dtypes.bool, vin[0].dtype, u=u, pred=True)))
        ...
        kk(f"IF_{r[vin[0].vin[0]][1:]}_{[x for x in uops].index(vin[0])}:")
