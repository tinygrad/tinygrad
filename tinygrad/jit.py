  ...
  def __call__(self, *args, **kwargs) -> Any:
    ...
    for k,v in itertools.chain(enumerate(args), kwargs.items()):
      if isinstance(v, Tensor):
        ...
        input_symbols.update(v.lazydata.st.symbols)
    ...
    if self.cnt >= 2:
      ...
      for (j,i), (arg,st) in self.outputs_resize.items():
      ...
      for j, (prg,args,sts) in enumerate(self.jit_cache):
        ...
        for st in sts: st.symbols.update(input_symbols)
      ...
      for j,(prg,args,sts) in enumerate(self.jit_cache):  # pylint: disable=E1133
