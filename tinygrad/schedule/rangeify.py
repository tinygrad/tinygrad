...
def bufferize_to_store(ctx:itertools.count, x:UOp, idx:UOp, allow_locals=True):
  ...
  if x.arg.addrspace == AddrSpace.GLOBAL:
    buf = UOp(Ops.BUFFER, src=(shape_to_shape_arg((size,)),), arg=ParamArg(next(ctx), device=x.arg.device, addrspace=AddrSpace.GLOBAL, dtype=x.dtype))
...
def debuf(ctx:LocalAddBufferContext, buf:UOp):
  param = UOp(Ops.PARAM, src=(UOp.const(dtypes.int, prod(buf.max_shape)),),
              arg=ParamArg(ctx.dg, addrspace=buf.addrspace, device=buf.device, dtype=buf.dtype))
