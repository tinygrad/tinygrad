...
earliest_rewrites = double_reshape + PatternMatcher([
  ...
  (
    ...
    lambda x: x.replace(src=(x.src[0].src[0],)).reshape((1,) * len(x.shape)).expand(x.shape)
    if len(x.src) and x.src[0].op is Ops.VIEW and not any(s == 0 for s in x.shape)
    else None,
  ),
...
def mark_children(ctx: ChildrenContext, x: UOp):
  ...
  new_srcs = [
    (
      UOp(Ops.CHILD, s.dtype, src=(UOp(Ops.CHILDREN, s.dtype, (s,), arg=len(ctx.children[s])),), arg=(ctx.children[s].index(x), len(ctx.children[s])))
...
def map_expand(r: UOp, idx: UOp):
  ...
  if idx.arg is not None:
    ending_ranges.append(idx.arg)
...
def map_partial_contiguous(ctx: RangeifyContext, x: UOp, idx: UOp):
  if x.arg is None:
    return None  # map_contiguous can handle this
...
def index_child(ctx: RangeifyContext, c: UOp, x: UOp, idx: UOp):
  ...
  if len(ctx.seen_children[c]) != x.arg[1]:
    ctx.progress += 1
    if ctx.progress > 10000:
      raise RuntimeError("children not making progress")
  ...
  if len(idx_ranges) > 0:
    ret = ret.bufferize(*end_ranges, arg=x.device).index(*[idx.src[1 + i] for i in idx_ranges])
  return ret


def children_gate(ctx: RangeifyContext, idx: UOp, c: UOp):
  if len(ctx.seen_children[c]) != c.arg:
    raise RuntimeError("all children should have been seen by now")
...
def might_end_axis(idx: UOp):
  ...
  if all(x.op not in {Ops.BUFFER, Ops.CONTIGUOUS, Ops.BUFFERIZE} for x in idx.toposort()):
    return None
  if all(x.op not in {Ops.REDUCE_AXIS} for x in idx.toposort()):
    return None
  to_end_axis = []
  ...
  if to_end_axis:
    return idx.replace(src=(idx.src[0].contiguous(arg=tuple(to_end_axis)),) + idx.src[1:], arg=None)
...
pm_add_buffers = pm_mops + PatternMatcher([
  ...
  (UPat((Ops.MSELECT, Ops.MSTACK), src=UPat(Ops.RESHAPE), name="m"), lambda m: m.replace(src=tuple([x.src[0] for x in m.src])).reshape(m.src[0].arg)),
])
...
rangeify_codegen = PatternMatcher([
  ...
  (
    UPat(
      ...
      src=(
        UPat.var("gate"),
        UPat(Ops.LOAD, src=(UPat.var("src"), UPat.var("barrier"))),
      ),
    ),
...
def get_rangeify_map(sink: UOp) -> dict[UOp, UOp]:
  ...
  if getenv("VIZ"):
    graph_rewrite(tensor_map[sink], PatternMatcher([]), name="View Rangeify Graph")
  ...
  if getenv("VIZ"):
    graph_rewrite(tensor_map[sink], PatternMatcher([]), name="View Kernel Graph")
  return tensor_map
