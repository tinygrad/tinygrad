...
split_kernels = PatternMatcher([
  (UPat((Ops.STORE, Ops.END), name="x"), lambda ctx,x: split_store(x) if x in ctx else None),
])
...
def get_kernel_graph(sink:UOp) -> UOp:
  ...
  split_candidates = {u for u in tsink_after_buffers_topo
                      if (u.op is Ops.END and not u.ranges) or (u.op is Ops.STORE and not u.ranges and u.src[0]._shape is None)}
  if split_candidates: tsink = graph_rewrite(tsink, split_kernels, ctx=split_candidates, bottom_up=True, name="split kernels")

  # WAR deps: if kernel U reads buffer S, and S is also written by another kernel, S's write must wait for U to finish
  afters = [u for u in (tsink_after_buffers_topo if not split_candidates else tsink.toposort()) if u.op is Ops.AFTER]
