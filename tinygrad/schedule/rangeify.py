...
earliest_rewrites = double_reshape+PatternMatcher([
  ...
  # remove contiguous on movement ops on disk
  (UPat(GroupOp.Movement-{Ops.SHRINK}, name="x").f(Ops.CONTIGUOUS),
   lambda x: x if isinstance(x.device, str) and x.device.startswith("DISK") else None),
  # push copy past movement ops to disk
  (UPat(GroupOp.Movement-{Ops.SHRINK}, name="x").f(Ops.COPY, allow_any_len=True, name="copy"),
