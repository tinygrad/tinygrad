...
def bufferize_to_store(x:UOp):
  ...
  if x.src[0].op is Ops.ASSIGN:
    ...
    do_store = assign_target.replace(dtype=sdtype).store(assign_src).replace(tag=x.tag).end(*[x for x in rngs[::-1] if x.op is Ops.RANGE])
  ...
  if sdtype.addrspace == AddrSpace.GLOBAL:
    ...
    do_store = buf.reshape(shape).index(*rngs, dtype=sdtype).store(x.src[0]).replace(tag=x.tag).end(*[x for x in rngs[::-1] if x.op is Ops.RANGE])
  ...
  do_store = buf.reshape(shape).index(*rngs, dtype=sdtype).store(x.src[0]).end(*[x for x in rngs[::-1] if x.op is Ops.RANGE])
