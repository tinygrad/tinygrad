...
earliest_rewrites = mop_cleanup+PatternMatcher([
  ...
  (UPat(Ops.REDUCE, name="reduce", src=(UPat.var("x"),)),
   lambda reduce,x: UOp.const(identity_element(reduce.arg[0], reduce.dtype), shape=reduce._shape)
...
pm_const_buffer_folding = pm_mops+PatternMatcher([
  ...
  (UPat(Ops.CONST, name='c').f(Ops.STAGE, allow_any_len=True, name="b"), lambda c,b: UOp.const(c.arg, shape=b._shape)),
  ...
  (UPat(Ops.INDEX, src=(UPat(Ops.AFTER, name="after"),), allow_any_len=True, name="idx"),
   lambda idx,after: UOp.const(Invalid, shape=idx._shape) if after_all_invalid(after) else None),
