...
def _pointwise_self_store(ctx:IndexingContext, dest:UOp, src:UOp) -> bool:
  ...
  if dest.op is Ops.SHRINK and dest.tag == ("allreduce",) and src.op is Ops.ADD and src.tag == ("allreduce_accumulate",):
    ...
    if old is not None and reduced is not None and base not in reduced.toposort(enter_calls=False):
      ...
      readers = [x for x in ctx.store_srcs if dest in x.toposort(gate=lambda y: y.op is not Ops.STAGE, enter_calls=False)]
  ...
  nodes = src.toposort(gate=lambda x: x.op is not Ops.STAGE, enter_calls=False)
  ...
  return sum(base in x.toposort(gate=lambda x: x.op is not Ops.STAGE, enter_calls=False) for x in ctx.store_srcs) == 1
