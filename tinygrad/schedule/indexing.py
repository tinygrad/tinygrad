...
from tinygrad.uop.symbolic import symbolic_no_commutative, pm_simplify_valid, pm_drop_and_clauses
...
def _apply_reshape(in_shape:tuple[sint,...], out_shape:tuple[sint, ...], urngs:UOp) -> UOp:
  ...
  return graph_rewrite(UOp.sink(*axes_out[::-1]), symbolic_no_commutative+pm_simplify_valid+pm_drop_and_clauses, name="reshape")
  ...
  match op:
    ...
    case Ops.PAD:
      ...
      rngs = tuple(r if (s == 0 and e == 0) else graph_rewrite((r >= s) & (r < (sh+s)),
        symbolic_no_commutative+pm_simplify_valid, name="pad").where(r-s, UOp.invalid()) for r,sh,(s,e) in zip(rngs, in_shape, arg))
    case Ops.RESHAPE:
      sink = UOp.sink(*rngs)
    ...
    if x in rctx.realize_map:
      ...
      for i,(local_rngs,valids) in enumerate(rngs_valids):
        ...
        if all_all_same or (PCONTIG and all_same(local_rngs)):
          ...
          _out_rngs.append(graph_rewrite(minimum_valid.where(local_rngs[0], UOp.invalid()), symbolic_no_commutative, name="minimum_valid"))
        else:
