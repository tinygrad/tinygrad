...
def simplify_stride0_reduce(reduce: UOp, x: UOp):
  ...
  if not all(unwrap(x.st).views[-1].strides[axis] == 0 for axis in reduce.arg[1]) or not all_int(x.shape):
    return None
...
def split_reduceop(reduce: UOp, x: UOp):
  ...
  if not (
    split_candidates := [
      ...
      if x.shape[i] % d == 0 and real_strides[i] != 0
    ]
  ):
    return None
...
kernelize_sym = symbolic_simple + PatternMatcher([
  ...
  (
    UPat(GroupOp.All - {Ops.BUFFER, Ops.VIEW, Ops.CONST, Ops.DEVICE}, name="u"),
    lambda u: u.replace(dtype=dt.base)
    if isinstance(dt := u.dtype, ImageDType) and (prod(u.shape) != prod(dt.shape) or not any(u.shape[x] % 4 == 0 for x in u.st.unit_stride_axes()))
    else None,
  ),
  ...
  (
    ...
    lambda x, t: UOp(Ops.BUFFER_VIEW, t.dtype, (x.base,), (t.size, x.st.views[0].offset)).reshape(t.shape)
  ...
  (
    UPat(Ops.ASSIGN, src=(UPat.var("t"), UPat.var("x"))),
    lambda x, t: x.contiguous()
    if t.base.op not in {Ops.BUFFER, Ops.BUFFER_VIEW} and not (t.base.op is Ops.MSTACK and all(x.op is Ops.BUFFER for x in t.base.src))
    else None,
  ),
  # put CAST to smaller dtype before EXPAND
  (
    UPat(Ops.CAST, name="cast", src=(UPat(Ops.VIEW, name="vm"),)),
    lambda cast, vm: vm.base.cast(cast.dtype).view(vm.st)
    if cast.dtype.itemsize <= vm.dtype.itemsize and resolve(prod(vm.shape) > vm.st.real_size())
    else None,
  ),
...
def append_to_kernel(x: UOp):
  ...
  for s in x.src:
    if s.op in DONT_PLACE_IN_KERNEL:
      new_srcs.append(s)
    else:
      ...
      if s.base.op not in {Ops.CONST, Ops.DEVICE} and (not (s.op is Ops.RESHAPE and s.base.op is Ops.ASSIGN)) and (m := s.metadata):
        metadata += m
  if (new_src := tuple(dedup(new_srcs))) != x.src:
    return x.replace(src=new_src, arg=Kernel(x.arg.ast, tuple(dedup(metadata))))
...
create_kernels = PatternMatcher([
  ...
  (UPat(Ops.MSTACK, src=UPat(Ops.RESHAPE), name="ms"), lambda ms: UOp(Ops.MSTACK, ms.dtype, tuple(x.src[0] for x in ms.src)).reshape(ms.src[0].arg)),
])
...
def unbind_view(x: UOp):
  if any(x.op is Ops.BIND for x in x.arg.vars()):
    return x.replace(arg=x.arg.unbind()[0])
  return None
...
replace_buffers = PatternMatcher([
  ...
  (
    UPat(Ops.SINK, src=UPat(GroupOp.All - {Ops.STORE}), name="sink"),
    lambda ctx, sink: UOp.sink(
      *[UOp.store(UOp(Ops.DEFINE_GLOBAL, (s := x.base).dtype.ptr(ctx[i].size), (), i).view(s.st), s) for i, x in enumerate(sink.src)], arg=sink.arg
    ),
  ),
...
def fix_kernel_ast(k: UOp) -> UOp | None:
  if k.arg.ast.op in GroupOp.Meta or all(s.op is Ops.STORE for s in k.arg.ast.src):
    return None
  ...
  for s in k.src:
    ...
    while s.op in {Ops.MSELECT, Ops.MSTACK}:
      s = s.src[0]
    bufs.append(s)
...
def append_metadata(root: UOp, k: UOp):
  if not root.metadata or (new_metadata := tuple(dedup(k.arg.metadata + root.metadata))) == k.arg.metadata:
    return None
  return root.replace(src=(root.src[0], k.replace(arg=Kernel(k.arg.ast, new_metadata))) + root.src[2:])


replace_metadata = PatternMatcher([
  (UPat(Ops.ASSIGN, src=(UPat(), UPat(Ops.KERNEL, name="k")), name="root", allow_any_len=True), append_metadata),
])
...
pm_fuse = PatternMatcher([
  ...
  (UPat(Ops.REDUCE_AXIS, name="r").fuse(), lambda r: r.replace(src=(r.src[0].fuse(),), arg=r.arg + (True,)) if len(r.arg) == 2 else None),
...
def do_fusion(x: UOp):
  ...
  def gate_contiguous(x):
    if is_contiguous := (x.op is Ops.CONTIGUOUS):
      found_contiguous[x] = x.replace(src=(UOp(Ops.VIEW, arg=x.st), UOp.unique()))
...
def fuse_arange(root: UOp):
  ...
  if not FUSE_ARANGE or root.src[0].base.op is Ops.CONST:
    return None
  ...
  for u in toposort:
    for s in u.src:
      local_children.setdefault(s, []).append(u)
  ...
  for r in local_arange:
    ...
    while q:
      u = q.pop()
      if not (curr_children := local_children.get(u, [])):
        continue
...
add_contiguous = PatternMatcher([
  (UPat(GroupOp.All - {Ops.CONTIGUOUS, Ops.ASSIGN}, name="x"), lambda ctx, x: x.replace(tag=1).contiguous() if x in ctx and x.tag is None else None)
])
...
def limit_bufs(root: UOp):
  ...
  def gate_input(u: UOp):
    if is_load := (u.op in {Ops.BUFFER, Ops.CONTIGUOUS, Ops.ASSIGN, Ops.MSTACK}):
      bufs.add(u)
    return not is_load
...
def get_kernelize_map(sink: UOp) -> dict[UOp, UOp]:
  ...
  if getenv("VIZ"):
    graph_rewrite(tensor_map[sink], PatternMatcher([]), name="View Tensor Graph")
  ...
  if getenv("VIZ"):
    graph_rewrite(sched_sink, PatternMatcher([]), name="View Kernel Graph")

  # verify Kernels match the spec
  if __debug__:
    type_verify(list(sched_sink.toposort()), tensor_uop_spec)
