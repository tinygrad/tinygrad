...
sym = symbolic_simple+PatternMatcher([
  ...
  (UPat(Ops.CONTIGUOUS, name="root", src=(UPat((Ops.CONTIGUOUS, Ops.BUFFER, Ops.COPY, Ops.ASSIGN)),)), lambda root: root.src[0] \
     if root.src[0].src[0].op != Ops.MSELECT else None),
...
finalize_contiguous = PatternMatcher([
  ...
  # merge contiguous
  (UPat(Ops.CONTIGUOUS, src=(UPat(Ops.CONTIGUOUS),), name="x"), lambda x: x.src[0] if x.src[0].src[0].src[0].op != Ops.MSELECT else None),
  # simplify views
