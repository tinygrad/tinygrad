...
def forward_call_outputs(sink:UOp) -> UOp:
  placed:dict[UOp, UOp] = {}
  items:list[UOp] = []
  # An opaque call consumes the materialized state, not the inline producer's expression.
  # Forwarding that AFTER to the producer would let resolve_returned_after erase this boundary.
  opaque_inputs = {a for c in sink.toposort(enter_calls=False) if c.op is Ops.CALL and not c.is_inline_call for a in c.src[1:]}
  for item in sink.src:
    if item in opaque_inputs:
      items.append(item)
      continue
