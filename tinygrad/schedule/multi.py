...
def mstack_reorder_view(ms: UOp):
  ...
  if not all_same(args) or len([x for x in args[0].vars() if x.arg[0] == "_device_num"]) != 0:
    return None
  return UOp(Ops.MSTACK, ms.dtype, tuple(x.src[0] for x in ms.src)).view(args[0])


def mstack_early_shrink(view: UOp, ms: UOp):
  if resolve(prod(view.shape) >= prod(ms.shape)) or _replace_dnum(view.st, 0) == view.st:
    return None
  ret = []
...
replace_allreduce = PatternMatcher([
  ...
  (
    UPat(Ops.MSELECT, src=(UPat(Ops.VIEW, src=(UPat.var("base"),), name="view"),), name="ms"),
    lambda ms, view, base: base.mselect(ms.arg).view(_replace_dnum(unwrap(view.st), ms.arg)),
  ),
