...
multi_pm = PatternMatcher([
  ...
  (UPat(Ops.CALL, name="call"), lambda call: rewrite_into_function(call) if any(a.unsharded_base.is_unbound for a in call.src[1:]) else None),
  ...
  (UPat(Ops.CALL, dtype=dtypes.void, name="root", custom_early_reject=set([Ops.UNSHARD])), lambda root:
    ...
    if not any(a.unsharded_base.is_unbound for a in root.src[1:]) else None),
