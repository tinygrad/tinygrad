...
multi_pm = PatternMatcher([
  ...
  (UPat(Ops.CALL, name="call"), lambda call: rewrite_into_function(call) if call.has_outputs else None),
  ...
  (UPat(Ops.CALL, dtype=dtypes.void, name="root", custom_early_reject=set([Ops.UNSHARD])), lambda root:
    UOp(root.op, src=tuple(x.src[0] if x.op is Ops.UNSHARD else x for x in root.src), arg=root.arg) if not root.has_outputs else None),
