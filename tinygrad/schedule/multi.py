...
def rewrite_into_function(call:UOp):
  if call.arg is None or call.arg.precompile or not any(x.unsharded_base.is_unbound for x in call.src[1:]): return None
...
multi_pm = PatternMatcher([
  ...
  (UPat(Ops.CALL, dtype=dtypes.void, name="root", custom_early_reject=set([Ops.UNSHARD])), lambda root:
    ...
    if not any(x.unsharded_base.is_unbound for x in root.src[1:]) else None),
