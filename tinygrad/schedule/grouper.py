...
def realize(ctx: dict[UOp, None], tr: UOp) -> None:
  ctx[tr] = None
...
def realize_before_view(ctx: dict[UOp, None], view: UOp, tr: UOp) -> None:
  ...
  if any(v.mask is not None for v in st.views) and not can_pad(tr, ctx):
    return realize(ctx, tr)
  # fold simple pads
  if len(st.views) == 1 and (m := st.views[-1].mask) is not None and all_int(tr.shape) and resolve(prod(tr.shape) >= prod([y - x for x, y in m])):
    return
  # realize before expand
  if resolve(prod(tr.shape) < prod(st.shape)) and not DONT_REALIZE_EXPAND:
    return realize(ctx, tr)
...
def recursive_group(
  ...
  group: dict[UOp, None],
  cache: dict[tuple[UOp, ShapeTracker], None],
) -> None:
  if (tr, st) in cache:
    return
  ...
  if tr in realizes and tr is not r:
    ...
    if not st.contiguous or st.size != rsize or tr in reduce_for_op:
      group.setdefault(r)
    return group.setdefault(tr)
  for tr_next in children.get(tr, {}):
    # max one reduceop per kernel
    if tr_next.op is Ops.REDUCE_AXIS:
      return group.setdefault(r)
    # can only fuse contiguous
    if len(st_childs := dedup(unwrap(x.st) for x in tr_next.src if x.base == tr)) > 1:
      return group.setdefault(r)
...
def group_realizes(sink: UOp) -> dict[UOp, None]:
  ...
  for u in (toposort := sink.toposort()):
    if u.op in {Ops.VIEW, Ops.SINK}:
      continue
    if u.op is Ops.ASSIGN:
      assigns[u.buf_uop] = None
    for s in u.src:
      children.setdefault(s.base, {})[u] = None

  # find all reduces, and pair them to a elementwise op. if they can't be cleanly paired, force realize the reduce (or a contig child)
  reduce_for_op: dict[UOp, UOp] = {}
  double_reduces: list[UOp] = []
  for r in toposort:
    if r.op is not Ops.REDUCE_AXIS:
      continue
    if len(r.arg) == 3 and r.arg[2] is True:
      continue
    if FUSE_CONV_BW and r.src[0].base.op is Ops.REDUCE_AXIS and r.src[0] is not r.src[0].base:
      double_reduces.append(r)
    if r in realizes:
      continue
    group: dict[UOp, None] = {}
    ...
    if not forced_realize and (assign_targets := {x.buf_uop for x in group if x.op is Ops.ASSIGN}):
      ...
      while parents and not forced_realize:
        p = parents.pop().base
        if p.op is Ops.BUFFER and p in assigns and p not in assign_targets:
          forced_realize, can_chase = True, False
        if p in realizes:
          continue
    ...
    if forced_realize or not group:
      ...
      if can_chase:
        ...
        while len(lst := children.get(tr, {})) == 1:
          ...
          if len(st_childs) > 1:
            break
          if st.size != st_childs[0].size:
            break
          st = st + st_childs[0]
          if not st.contiguous or tr_next.op is Ops.REDUCE_AXIS:
            break
          tr = tr_next
  ...
  for reduceop in double_reduces:
    ...
    if len(children.get(top_reduce, {})) == 1:
