...
from tinygrad.runtime.ops_disk import DiskRunner
  ...
  while len(schedule):
    ...
    if si.out.size > 0:
      ...
      si.out.realized = si.out.output_buffer if si.out.output_buffer is not None else \
        Buffer(si.out.device, si.out.size, si.out.dtype, "PLACEHOLDER" if isinstance(prg, DiskRunner) else None, options=options)
      del si.out.srcs
