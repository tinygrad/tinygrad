...
numpy_fxn_for_op: Dict[Op, Callable] = {
  ...
  UnaryOps.CAST: lambda x,y: x.view(np.uint16) if y[0] == dtypes.bfloat16 else x.view(y[0].np) if y[1] else x.astype(y[0].np, copy=False), # noqa: E501 numpy does not support bfloat16
