...
class CPUProgram(Program['CPUDevice']):
  ...
  def __call__(self, *bufs:HCQBuffer, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1),
    ...
    if self.lvp:
      ...
      it = [iter(vals), (b.va_addr for b in bufs)]
      struct.pack_into('<3I', lvp_args, 0, *data64_le(addr+12), (len(bufs)+len(vals))*2)
      for o,dt,s in TinyELF.iter_sig(self.signature): struct.pack_into('<Q' if s!=() else f'<{dt.fmt}', lvp_args, 12+o, next(it[s!=()]))
      self.fxn(addr)
    else:
      #args = [*[cast(int, b.va_addr) for b in bufs], *cast(tuple[int, ...], vals)]
      args = [next(it[s!=()]) for it in [[iter(cast(tuple[int, ...], vals)), (cast(int, b.va_addr) for b in bufs)]] for *_,_,s in self.signature]
