...
from tinygrad.helpers import from_mv, to_mv, OSX, WIN, mv_address, wait_cond, cpu_profile, suppress_finalizing, unwrap, data64_le, findlib
...
class CPUProgram(HCQProgram):
  rt_lib = None
  try: rt_lib = ctypes.CDLL(findlib('System' if OSX else 'kernel32') if OSX or WIN else 'libgcc_s.so.1')
  except OSError: pass
