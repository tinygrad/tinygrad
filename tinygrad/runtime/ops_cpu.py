...
def timestamp_prog():
  if WIN: val = UOp.const(0).cast(dtypes.uint64)
  else:
    fn, ts = UOp.param(1, dtypes.uint64, (1,)), UOp.placeholder((2,), dtypes.uint64, slot=0, addrspace=AddrSpace.REG)
    call = fn[0].load().call(UOp.const(6 if OSX else 1).cast(dtypes.int), ts[0], ret_dtype=dtypes.void) # clock_gettime(CLOCK_MONOTONIC, &ts)
...
def quit_prog():
  ...
  if WIN: return fn[0].load().call(UOp.const(0).cast(dtypes.uint64), ret_dtype=dtypes.void) # ExitThread(0)
  sem = UOp.param(1, dtypes.uint64, (1,))

  close = fn[2].load().call(sem[0], ret_dtype=dtypes.void) # sem_close(sem)
  return fn.after(close)[0].load().call(UOp.const(0).cast(dtypes.uint64), ret_dtype=dtypes.void) # pthread_exit(0)
...
def host_wait(ctx, dst:UOp, val:UOp) -> UOp:
  return (cur:=dst.after(loop:=UOp.loop(next(ctx))).index(UOp.const(0).cast(dtypes.int)).load()).end(loop, cur < val)
...
def encode_host_queue(q:UOp) -> UOp:
  ...
  return store.src[0].after(*spins).index(UOp.const(0).cast(dtypes.int)).store(store.src[1])
