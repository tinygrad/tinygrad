...
class ClangJITCompiler(Compiler):
  ...
  def compile(self, src:str) -> bytes:
    ...
    # Use compatible march flag for ARM64 instead of march=native which is not supported
    march_flag = '-march=armv8-a' if target == 'aarch64' else '-march=native'
    args = [march_flag, f'--target={target}-none-unknown-elf', '-O2', '-fPIC', '-ffreestanding', '-fno-math-errno', '-nostdlib', '-fno-ident']
