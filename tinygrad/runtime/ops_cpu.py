...
numpy_fxn_for_op: Dict[Op, Callable] = {
  ...
  UnaryOps.CAST: lambda x,y: x.view(y[0].np) if y[1] else x.astype(y[0].np, copy=False), UnaryOps.NEG: lambda x: np.logical_not(x) if x.dtype == dtypes.bool else np.negative(x),
