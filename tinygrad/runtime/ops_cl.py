...
class CLCompiler(Compiler):
  ...
  def compile(self, src:str) -> bytes:
    program = checked(cl.clCreateProgramWithSource(self.dev.context, 1, ctypes.cast(to_char_p_p([src.encode()]), ctypes.POINTER(ctypes.c_char_p)), None, status := ctypes.c_int32()), status)
