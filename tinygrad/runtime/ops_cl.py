...
from tinygrad.device import BufferSpec, LRUAllocator, Compiled, CompileError, CompilerSet
...
def cl_compile(dev:CLDevice, src:str) -> bytes:
  program = checked(cl.clCreateProgramWithSource(dev.context, 1, to_char_p_p([src.encode()]), None, status := ctypes.c_int32()), status)
  build_status: int = cl.clBuildProgram(program, 1, dev.device_id, None, BP_CB(), None)
  if build_status != 0:
    cl.clGetProgramBuildInfo(program, dev.device_id, cl.CL_PROGRAM_BUILD_LOG, 0, None, log_size := ctypes.c_size_t())
    cl.clGetProgramBuildInfo(program, dev.device_id, cl.CL_PROGRAM_BUILD_LOG,
...
class CLProgram:
  def __init__(self, device:CLDevice, name:str, lib:bytes, buf_dtypes=[], **kwargs):
    self.dev, self.name, self.lib, self.buf_dtypes = device, name, cl_compile(device, lib.decode()), buf_dtypes
