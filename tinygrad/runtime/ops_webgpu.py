...
from tinygrad.device import Compiled, Allocator, CompiledKernel
...
class WebGPUProgram:
  ...
  def __call__(self, *bufs, global_size, local_size, wait=False):
    ...
    compute_pipeline = wgpu_device.create_compute_pipeline(layout=pipeline_layout,compute={"module": self.prg, "entry_point": self.k.name},)
