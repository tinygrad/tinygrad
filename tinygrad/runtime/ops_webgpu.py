...
def pop_error(device):
  return _run(webgpu.wgpuDevicePopErrorScopeF, webgpu.WGPUPopErrorScopeCallbackInfo, webgpu.WGPUPopErrorScopeCallback, None, 2, 2, device)
    ...
    cb_info = create_cb_info(
        webgpu.WGPUBufferMapCallbackInfo2, webgpu.WGPUBufferMapCallback2, cb
    )
    wgpu_wait(
        webgpu.wgpuBufferMapAsync2(buf, webgpu.WGPUMapMode_Read, 0, size, cb_info)
    )

    if result[0] != webgpu.WGPUBufferMapAsyncStatus_Success:
        raise RuntimeError(
            f"Failed to map buffer: [{webgpu.WGPUBufferMapAsyncStatus__enumvalues[result[0]]}] {result[1]}"
        )
...
def pop_error(device):
    ...
    cb_info = create_cb_info(
        webgpu.WGPUPopErrorScopeCallbackInfo, webgpu.WGPUPopErrorScopeCallback, cb
    )
    ...
    def __call__(
        ...
        wgpu_wait(
            webgpu.wgpuDeviceCreateComputePipelineAsync2(
                self.dev, compute_desc, cb_info
            )
        )

        if pipeline_result[0] != webgpu.WGPUCreatePipelineAsyncStatus_Success:
            raise RuntimeError(
                f"{webgpu.WGPUCreatePipelineAsyncStatus__enumvalues[pipeline_result[0]]}: {pipeline_result[2]}, {pop_error(self.dev)}"
            )
...
class WebGpuDevice(Compiled):
    def __init__(self, device: str):
        ...
        def adapter_cb(status, adapter, msg, _):
        ...
        if adapter_result[0] != webgpu.WGPURequestAdapterStatus_Success:
            raise RuntimeError(
                f"Error requesting adapter: [{webgpu.WGPURequestAdapterStatus__enumvalues[adapter_result[0]]}] {adapter_result[2]}"
            )
        ...
        def dev_cb(status, device_impl, msg, _):
        ...
        wgpu_wait(
            webgpu.wgpuAdapterRequestDeviceF(adapter_result[1], dev_desc, cb_info)
        )

        if device_result[0] != webgpu.WGPURequestDeviceStatus_Success:
            raise RuntimeError(
                f"Failed to request device: [{webgpu.WGPURequestDeviceStatus__enumvalues[device_result[0]]}] {device_result[2]}"
            )

        super().__init__(
            device,
            WebGpuAllocator(device_result[1]),
            WGSLRenderer(),
            Compiler(),
            functools.partial(
                WebGPUProgram,
                (device_result[1], webgpu.WGPUFeatureName_TimestampQuery in supported),
            ),
        )
    ...
    def synchronize(self):
        ...
        cb_info = create_cb_info(
            webgpu.WGPUQueueWorkDoneCallbackInfo2, webgpu.WGPUQueueWorkDoneCallback2, cb
        )
        wgpu_wait(
            webgpu.wgpuQueueOnSubmittedWorkDone2(
                webgpu.wgpuDeviceGetQueue(self.runtime.args[0][0]), cb_info
            )
        )
        if result[0] != webgpu.WGPUQueueWorkDoneStatus_Success:
            raise RuntimeError(webgpu.WGPUQueueWorkDoneStatus__enumvalues[result[0]])
