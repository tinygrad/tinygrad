...
class WebGPUProgram(Program['WebGpuDevice']):
  ...
  def __call__(self, *args:webgpu.WGPUBuffer|int, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1),
    ...
    bind_entries = (webgpu.WGPUBindGroupLayoutEntry * (1+len(args)))(
      bgl_entry(0, 'Uniform'), *(bgl_entry(i+1, 'Storage' if isinstance(a, webgpu.WGPUBuffer) else 'Uniform') for i,a in enumerate(args)))
