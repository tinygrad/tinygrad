...
class WebGpuDevice(Compiled):
  def __init__(self, device:str):
    ...
    features = []
    if timestamp_supported: features += [webgpu.WGPUFeatureName_TimestampQuery]
    if webgpu.WGPUFeatureName_ShaderF16 in [supported_features.features[i] for i in range(supported_features.featureCount)]:
      features += [webgpu.WGPUFeatureName_ShaderF16]
