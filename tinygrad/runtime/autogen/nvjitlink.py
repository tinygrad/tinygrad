...
dll = DLL('nvjitlink', 'nvJitLink' if not WIN else (glob.glob(os.path.join(os.environ.get('CUDA_PATH', ''), 'bin', 'nvJitLink_*.dll'))+[''])[0], f'/usr/local/cuda/targets/{sysconfig.get_config_vars().get("MULTIARCH", "").rsplit("-", 1)[0]}/lib')
