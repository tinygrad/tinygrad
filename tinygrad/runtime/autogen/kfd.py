...
def _do_ioctl(__idir, __base, __nr, __user_struct, __fd: HWInterface, **kwargs):
    ret = __fd.ioctl(
        (__idir << 30)
        | (ctypes.sizeof(made := __user_struct(**kwargs)) << 16)
        | (__base << 8)
        | __nr,
        made,
    )
...
def _IO(base, nr):
    return functools.partial(
        _do_ioctl, 0, ord(base) if isinstance(base, str) else base, nr, None
    )


def _IOW(base, nr, type):
    return functools.partial(
        _do_ioctl, 1, ord(base) if isinstance(base, str) else base, nr, type
    )


def _IOR(base, nr, type):
    return functools.partial(
        _do_ioctl, 2, ord(base) if isinstance(base, str) else base, nr, type
    )


def _IOWR(base, nr, type):
    return functools.partial(
        _do_ioctl, 3, ord(base) if isinstance(base, str) else base, nr, type
    )
...
AMDKFD_IOC_GET_VERSION = AMDKFD_IOR(
    0x01, struct_kfd_ioctl_get_version_args
)  # macro (from list)
...
AMDKFD_IOC_CREATE_QUEUE = AMDKFD_IOWR(
    0x02, struct_kfd_ioctl_create_queue_args
)  # macro (from list)
...
AMDKFD_IOC_DESTROY_QUEUE = AMDKFD_IOWR(
    0x03, struct_kfd_ioctl_destroy_queue_args
)  # macro (from list)
...
AMDKFD_IOC_UPDATE_QUEUE = AMDKFD_IOW(
    0x07, struct_kfd_ioctl_update_queue_args
)  # macro (from list)
...
AMDKFD_IOC_SET_CU_MASK = AMDKFD_IOW(
    0x1A, struct_kfd_ioctl_set_cu_mask_args
)  # macro (from list)
...
AMDKFD_IOC_GET_QUEUE_WAVE_STATE = AMDKFD_IOWR(
    0x1B, struct_kfd_ioctl_get_queue_wave_state_args
)  # macro (from list)
...
AMDKFD_IOC_SET_MEMORY_POLICY = AMDKFD_IOW(
    0x04, struct_kfd_ioctl_set_memory_policy_args
)  # macro (from list)
...
AMDKFD_IOC_GET_CLOCK_COUNTERS = AMDKFD_IOWR(
    0x05, struct_kfd_ioctl_get_clock_counters_args
)  # macro (from list)
...
AMDKFD_IOC_GET_PROCESS_APERTURES = AMDKFD_IOR(
...
AMDKFD_IOC_GET_PROCESS_APERTURES_NEW = AMDKFD_IOWR(
    0x14, struct_kfd_ioctl_get_process_apertures_new_args
)  # macro (from list)
...
AMDKFD_IOC_DBG_REGISTER = AMDKFD_IOW(
    0x0D, struct_kfd_ioctl_dbg_register_args
)  # macro (from list)
...
AMDKFD_IOC_DBG_UNREGISTER = AMDKFD_IOW(
    0x0E, struct_kfd_ioctl_dbg_unregister_args
)  # macro (from list)
...
AMDKFD_IOC_DBG_ADDRESS_WATCH = AMDKFD_IOW(
    0x0F, struct_kfd_ioctl_dbg_address_watch_args
)  # macro (from list)
...
AMDKFD_IOC_DBG_WAVE_CONTROL = AMDKFD_IOW(
    0x10, struct_kfd_ioctl_dbg_wave_control_args
)  # macro (from list)
...
AMDKFD_IOC_CREATE_EVENT = AMDKFD_IOWR(
    0x08, struct_kfd_ioctl_create_event_args
)  # macro (from list)
...
AMDKFD_IOC_DESTROY_EVENT = AMDKFD_IOW(
    0x09, struct_kfd_ioctl_destroy_event_args
)  # macro (from list)
...
AMDKFD_IOC_SET_EVENT = AMDKFD_IOW(
    0x0A, struct_kfd_ioctl_set_event_args
)  # macro (from list)
...
AMDKFD_IOC_RESET_EVENT = AMDKFD_IOW(
    0x0B, struct_kfd_ioctl_reset_event_args
)  # macro (from list)
...
AMDKFD_IOC_WAIT_EVENTS = AMDKFD_IOWR(
    0x0C, struct_kfd_ioctl_wait_events_args
)  # macro (from list)
...
AMDKFD_IOC_SET_SCRATCH_BACKING_VA = AMDKFD_IOWR(
...
AMDKFD_IOC_GET_TILE_CONFIG = AMDKFD_IOWR(
    0x12, struct_kfd_ioctl_get_tile_config_args
)  # macro (from list)
...
AMDKFD_IOC_SET_TRAP_HANDLER = AMDKFD_IOW(
    0x13, struct_kfd_ioctl_set_trap_handler_args
)  # macro (from list)
...
AMDKFD_IOC_ACQUIRE_VM = AMDKFD_IOW(
    0x15, struct_kfd_ioctl_acquire_vm_args
)  # macro (from list)
...
AMDKFD_IOC_ALLOC_MEMORY_OF_GPU = AMDKFD_IOWR(
    0x16, struct_kfd_ioctl_alloc_memory_of_gpu_args
)  # macro (from list)
...
AMDKFD_IOC_FREE_MEMORY_OF_GPU = AMDKFD_IOW(
    0x17, struct_kfd_ioctl_free_memory_of_gpu_args
)  # macro (from list)
...
AMDKFD_IOC_MAP_MEMORY_TO_GPU = AMDKFD_IOWR(
    0x18, struct_kfd_ioctl_map_memory_to_gpu_args
)  # macro (from list)
...
AMDKFD_IOC_UNMAP_MEMORY_FROM_GPU = AMDKFD_IOWR(
    0x19, struct_kfd_ioctl_unmap_memory_from_gpu_args
)  # macro (from list)
...
AMDKFD_IOC_ALLOC_QUEUE_GWS = AMDKFD_IOWR(
    0x1E, struct_kfd_ioctl_alloc_queue_gws_args
)  # macro (from list)
...
AMDKFD_IOC_GET_DMABUF_INFO = AMDKFD_IOWR(
    0x1C, struct_kfd_ioctl_get_dmabuf_info_args
)  # macro (from list)
...
AMDKFD_IOC_IMPORT_DMABUF = AMDKFD_IOWR(
    0x1D, struct_kfd_ioctl_import_dmabuf_args
)  # macro (from list)
...
AMDKFD_IOC_SMI_EVENTS = AMDKFD_IOWR(
    0x1F, struct_kfd_ioctl_smi_events_args
)  # macro (from list)
...
AMDKFD_IOC_SET_XNACK_MODE = AMDKFD_IOWR(
    0x21, struct_kfd_ioctl_set_xnack_mode_args
)  # macro (from list)
__all__ = [
    "AMDKFD_COMMAND_END",
    ...
    "struct_kfd_ioctl_smi_events_args",
    "struct_kfd_ioctl_svm_args",
