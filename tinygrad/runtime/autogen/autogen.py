...
class Autogen:
  def __init__(self, name, dll, files, inc=[], args=[]):
    self.name, self.dll, self.files, self.args, self.types, self.macros, self.anoncnt = name, dll, files, [f"-I{i}" for i in inc] + args, {}, set(), 0
    self._mod = self.gen()
  ...
  def render(self, c):
    ...
    match c.kind:
      ...
      case CK.INTEGER_LITERAL: return next(c.get_tokens()).spelling
          ...
          for c in tu.cursor.walk_preorder():
            ...
            match c.kind:
              ...
              case CK.MACRO_DEFINITION if len(toks:=list(c.get_tokens())) > 1:
                if toks[1].kind == ToK.LITERAL: print(f"{c.spelling} = {lit.replace('L', '') if (lit:=toks[1].spelling)[0].isdigit() else lit}", file=self.f)
                if toks[1].spelling == '(' and toks[0].extent.end.column == toks[1].extent.start.column:
                  ...
                  mtu = self.idx.parse("tmp.c", unsaved_files=[("tmp.c", f"{';'.join(args)};_x = {body};")])
                  try: print(f"def {c.spelling}({', '.join(a for a in args)}): return " + self.render(next(list(mtu.cursor.get_children())[-1].get_children())), file=self.f)
  ...
  def tname(self, t) -> str:
    ...
    match t.kind:
      case TK.POINTER: return f"ctypes.POINTER({self.tname(t.get_pointee())})"
      case TK.ELABORATED: return self.tname(t.get_named_type())
      case TK.TYPEDEF:
        if t.spelling not in self.types:
          self.types[t.spelling] = t.spelling
          print(f"{t.spelling} = {self.tname(t.get_canonical())}", file=self.f)
        return t.spelling
      case TK.RECORD:
        if t.spelling not in self.types:
          ...
          print(f"class {self.types[t.spelling]}(ctypes.{'Structure' if decl.kind == CK.STRUCT_DECL else 'Union'}):\n" +
      ...
      case TK.ENUM:
        if t.spelling not in self.types:
          ...
          print(f"{self.types[t.spelling]} = {self.tname(decl.enum_type)}\n" +
                "\n".join(f"{e.spelling} = {e.enum_value}" for e in decl.get_children()) + "\n", file=self.f)
        return self.types[t.spelling]
