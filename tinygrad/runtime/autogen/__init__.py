import importlib, pathlib, subprocess, tarfile
...
def load(name, dll, files, **kwargs):
  if not (f:=(root/(path:=kwargs.pop("path", __name__)).replace('.','/')/f"{name}.py")).exists():
    files, kwargs['args'] = files() if callable(files) else files, args() if callable(kwargs.get('args', [])) else kwargs.get('args', [])
    ...
    f.write_text(importlib.import_module(f"{__name__}.autogen").gen(dll, files, **kwargs))
...
def __getattr__(nm):
  ...
  *[f"{{}}/src/common/sdk/nvidia/inc/ctrl/ctrl{s}/*.h" for s in ["0000","0080","2080","83de"]],
  ...
  args=["-I{}/src/common/inc", "-I{}/kernel-open/nvidia-uvm","-I{}/kernel-open/common/inc","-I{}/src/common/sdk/nvidia/inc",
  "-I{}/src/nvidia/arch/nvalloc/unix/include","-I{}/src/common/sdk/nvidia/inc/ctrl"], rules=[(r'MW\(([^:]+):(.+)\)',r'(\1, \2)')],
  tarball=nv_src, anon_names={"{}/kernel-open/common/inc/nvstatus.h:37":"nv_status_codes"})
  ...
  args=["-DRPC_MESSAGE_STRUCTURES","-DRPC_STRUCTURES","-I{}/src/nvidia/generated","-I{}/src/common/inc","-I{}/src/nvidia/inc",
  ...
  "-I{}/src/common/sdk/nvidia/inc/ctrl"], tarball=nv_src,
  anon_names={"{}/src/nvidia/inc/kernel/vgpu/rpc_global_enums.h:8":"rpc_fns","{}/src/nvidia/inc/kernel/vgpu/rpc_global_enums.h:244":"rpc_events"})
    ...
    case "io_uring":
      return load("io_uring",[],["/usr/include/liburing.h","/usr/include/linux/io_uring.h","/usr/include/asm-generic/unistd.h"],rules=[('__NR','NR')])
    ...
    case "llvm": return load("llvm", ["LLVM_PATH"], lambda: [sys("llvm-config-20 --includedir")+"/llvm-c/**/*.h"],
      args=lambda: sys("llvm-config-20 --cflags").split(), recsym=True, prolog=["from tinygrad.runtime.support.llvm import LLVM_PATH"])
  ...
  ["{}/src/compiler/glsl_types.h", "{}/src/util/blob.h", "{}/src/util/ralloc.h"], args=lambda:["-DHAVE_ENDIAN_H","-DHAVE_STRUCT_TIMESPEC",
  "-DHAVE_PTHREAD","-DHAVE_FUNC_ATTRIBUTE_PACKED","-I{}/src","-I{}/include","-I{}/gen","-I{}/src/compiler/nir","-I{}/src/gallium/auxiliary",
  "-I{}/src/gallium/include",f"-I{sys('llvm-config-20 --includedir')}"], preprocess=lambda path: subprocess.run("""mkdir -p gen/util/format
...
python3 src/compiler/builtin_types_h.py gen/builtin_types.h""", cwd=path, shell=True, check=True),
  ...
  prolog=["import gzip, base64", "from tinygrad.helpers import OSX"], epilog=lambda path: sys(f"{root}/extra/mesa/lvp_nir_options.sh {path}"))
    case "libclang": return load("libclang", ["os.getenv('LIBCLANG_PATH', find_library('clang-20'))"],
                                 lambda: [sys("llvm-config-20 --includedir")+"/clang-c/Index.h"], args=lambda: sys("llvm-config-20 --cflags").split(),
