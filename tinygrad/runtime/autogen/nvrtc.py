...
dll = DLL('nvrtc', 'nvrtc' if not WIN else (glob.glob(os.path.join(os.environ.get('CUDA_PATH', ''), 'bin', 'nvrtc64_*.dll'))+[''])[0], f'/usr/local/cuda/targets/{sysconfig.get_config_vars().get("MULTIARCH", "").rsplit("-", 1)[0]}/lib')
