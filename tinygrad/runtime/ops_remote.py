...
@dataclass(frozen=True)
class IBXConnect(RemoteRequest): host: str; gid: bytes; qp_num: int # noqa: E702
...
eval_globals = {x.__name__:x for x in [SessionKey, SessionFree, RemoteProperties, GetProperties, Event, Wait, BufferAlloc, BufferIOVA, BufferOffset,
                                       BufferFree, CopyIn, CopyOut, Transfer, IBXConnect, ProgramAlloc, ProgramFree, ProgramExec, GraphComputeItem,
            ...
            if c.dsession.host == unwrap(c.session).host:
              ...
              if self.ib_ctx is not None and c.dsession.host not in self.ib_conns:
                ...
                if props.ib_gid is not None:
                  ...
                  ibxc_ret = conn.q(IBXConnect(unwrap(c.session).host, ib_conn.gid, ib_conn.qp_num, session=c.dsession), wait=True)
