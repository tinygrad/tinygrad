        ...
        match c:
          ...
          case Event():
            if c.session == c.event_session:
              ...
              [Device[d].synchronize() for d in Device._opened_devices] # wait for device*s* to finish executing previous stuff
          ...
          case BufferIOVAS():
            rets = []
            for session,buffer_num in c.buffer_nums:
              iova, mr = unwrap(self.ib_ctx).reg(buf:=self.sessions[session].buffers[buffer_num])
          ...
          case Transfer():
            if c.dsession.host == unwrap(c.session).host:
              ...
              conn, ib_conn = RemoteConnection(c.dsession.host), await self.ib_connect(c.session, c.dsession)
              ...
              if ib_conn is not None:
                ...
                [Device[d].synchronize() for d in Device._opened_devices]
          ...
          case BatchTransfer():
            ...
            if ib_conn is not None:
              sbufs = [self.ib_ctx.reg(self.sessions[s].buffers[bi]) for s,bi in c.sbuffer_nums]
