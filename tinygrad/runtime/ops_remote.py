...
import multiprocessing, functools, asyncio, http.client, hashlib, json, time, os, binascii, struct, ast, contextlib
from tinygrad.runtime.support.http import HTTPServer, HTTPStatus
...
class RemoteHandler(HTTPServer):
  def __init__(self, device: str):
    self.device = device
    self.sessions: defaultdict[str, RemoteSession] = defaultdict(RemoteSession)

  def handle(self, method:str, path:str, headers:dict[str, str], body:bytes) -> tuple[HTTPStatus, bytes]:
    session = self.sessions[unwrap(headers.get("cookie")).split("session=")[1]]
    status, ret = HTTPStatus.OK, b""
    ...
    else: status, ret = HTTPStatus.NOT_FOUND, b"Not Found"
    return status, ret
