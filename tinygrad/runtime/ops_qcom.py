...
class QCOMAllocator(HCQAllocator):
  def _alloc(self, size:int, options:BufferOptions) -> HCQBuffer:
    if options.image is not None:
      ...
      texture = SimpleNamespace(va_addr=options.external_ptr, size=size) if options.external_ptr else self.device._gpu_alloc(pitch * imgh, kgsl.KGSL_MEMTYPE_TEXTURE, map_to_cpu=True)
    ...
    return SimpleNamespace(va_addr=options.external_ptr, size=size) if options.external_ptr else self.device._gpu_alloc(size, map_to_cpu=True)
