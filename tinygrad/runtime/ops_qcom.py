...
class QCOMArgsState(HCQArgsState):
  def __init__(self, buf:HCQBuffer, prg:QCOMProgram, bufs:tuple[HCQBuffer, ...], vals:tuple[int, ...]=()):
    ...
    ubos, uavs, itb = [], [], iter(bufs)
    for *_,dt,s in prg.signature:
      if s == (): continue
      b = next(itb)
      (uavs if is_image_shape(s) else ubos).append((dt,s,b) if is_image_shape(s) else b)
    ...
    if prg.NIR:
      ...
      it = [iter(vals), iter(ubos)]
      for o,dt,s in TinyELF.iter_sig(prg.signature):
        self.bind_sints_to_buf((x:=next(it[s!=()])).va_addr if s!=() else x, buf=self.buf, fmt='Q' if s!=() else dt.fmt, offset=prg.buf_off+o)

    else:
      it = [iter(vals), iter(ubos)]
      for i,(*_,dt,s) in enumerate(x for x in prg.signature if not is_image_shape(x[3])):
        self.bind_sints_to_buf((x:=next(it[s!=()])).va_addr if s!=() else x, buf=self.buf, fmt='Q' if s!=() else dt.fmt, offset=prg.buf_offs[i])
