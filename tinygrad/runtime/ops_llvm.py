...
class LLVMCompiler(Compiler):
    def __init__(self, host_arch: str):
        ...
        cpu, feats = ctypes.string_at(llvm.LLVMGetHostCPUName()), (
            b"+reserve-x18," if OSX else b""
        ) + ctypes.string_at(llvm.LLVMGetHostCPUFeatures())
        if DEBUG >= 2:
            print(f"LLVM init for {cpu!r} with {feats!r}")
    ...
    def compile(self, src: str) -> bytes:
        src_buf = llvm.LLVMCreateMemoryBufferWithMemoryRangeCopy(
            ctypes.create_string_buffer(src_bytes := src.encode()),
            len(src_bytes),
            b"src",
        )
        mod = expect(
...
class LLVMDevice(Compiled):
    def __init__(self, device: str):
        compiler = LLVMCompiler(
            {"arm64": "AArch64", "aarch64": "AArch64", "x86_64": "X86", "AMD64": "X86"}[
                platform.machine()
            ]
        )
