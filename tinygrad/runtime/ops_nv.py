...
class NVDevice(HCQCompiled[NVSignal]):
  ...
  def __init__(self, device:str=""):
    ...
    # Initialize video decode support
    self._init_video_decode_support()
...
# Video decode extensions for HEVC support
class NVVideoSurface(HCQBuffer):
  """Video surface buffer for HEVC decode output"""
  def __init__(self, va_addr:int, size:int, width:int, height:int, format:str="NV12", **kwargs):
    super().__init__(va_addr, size, **kwargs)
    self.width, self.height, self.format = width, height, format
    self.is_video_surface = True

  @property
  def pitch(self) -> int:
    """Get surface pitch (stride) in bytes"""
    # NV12: Y plane is width bytes per pixel, UV is width/2 * 2 = width bytes per pixel
    return self.width if self.format == "NV12" else self.width * (4 if self.format == "RGBA" else 2)
  ...
  def uv_size(self) -> int:
    """Get UV plane size in bytes (for NV12)"""
    return (self.height // 2) * self.pitch if self.format == "NV12" else 0

class NVVideoQueue(NVCommandQueue):
  """Video decode command queue extending HWQueue pattern"""
  def __init__(self):
    super().__init__()
    self.decode_engine_class = None  # Will be set during device init
    self.nvdec_channel = None        # NVDEC channel handle

  def setup(self, decode_engine_class=None, nvdec_channel=None):
    """Initialize video decode queue with NVDEC engine"""
    self.decode_engine_class = decode_engine_class
    self.nvdec_channel = nvdec_channel
    
    # Add setup commands for video decode engine
    if decode_engine_class and nvdec_channel:
      # Video decode engine initialization commands would go here
      # Following pattern from NVComputeQueue.setup()
      pass
    
    return self

  def decode_hevc(self, decoder_handle, bitstream_data:bytes, output_surface:NVVideoSurface):
    """Submit HEVC decode command to video engine"""
    # Create CUVIDPICPARAMS structure for decode
    from tinygrad.runtime.support.cuvid import CUVIDPICPARAMS
    
    if not decoder_handle:
      raise RuntimeError("Invalid decoder handle")

    # Prepare picture parameters
    pic_params = CUVIDPICPARAMS()
    pic_params.PicWidthInMbs = (output_surface.width + 15) // 16
    pic_params.FrameHeightInMbs = (output_surface.height + 15) // 16
    pic_params.CurrPicIdx = 0  # Surface index
    pic_params.nBitstreamDataLen = len(bitstream_data)
    
    # Allocate bitstream buffer on GPU if needed
    # For now, simplified implementation
    
    # Submit decode via device interface 
    # This would integrate with NVDEC hardware engine
    self._add_video_decode_cmd(decoder_handle, pic_params, output_surface)
    
    return self

  def _add_video_decode_cmd(self, decoder_handle, pic_params, output_surface:NVVideoSurface):
    """Add video decode command to queue (internal)"""
    from tinygrad.runtime.support.nvdec import NVDECBitstreamBuffer, NVDECSurfaceDesc, NVDECDecodeParams, NVDECCommandType
    
    # Create NVDEC hardware descriptors
    bitstream_desc = NVDECBitstreamBuffer(
      gpu_addr=decoder_handle,  # Simplified, would be actual bitstream GPU buffer
      size=pic_params.nBitstreamDataLen if hasattr(pic_params, 'nBitstreamDataLen') else 1024
    )
    
    surface_desc = NVDECSurfaceDesc(
      gpu_addr=int(output_surface.va_addr),
      width=output_surface.width,
      height=output_surface.height,
      format=output_surface.format
    )
    
    decode_params = NVDECDecodeParams(
      pic_width_in_mbs=(output_surface.width + 15) // 16,
      pic_height_in_mbs=(output_surface.height + 15) // 16
    )
    
    # Add NVDEC commands to queue following NV command pattern
    if self.decode_engine_class:
      # Add bitstream buffer setup command
      self._q.append(['nvm', self.nvdec_channel, NVDECCommandType.SET_BITSTREAM_BUFFER, bitstream_desc.to_hw_desc()])
      
      # Add output surface setup command  
      self._q.append(['nvm', self.nvdec_channel, NVDECCommandType.SET_OUTPUT_SURFACE, surface_desc.to_hw_desc()])
      
      # Add decode parameters command
      self._q.append(['nvm', self.nvdec_channel, NVDECCommandType.SET_DECODE_PARAMS, decode_params.to_hw_desc()])
      
      # Add execute decode command
      self._q.append(['nvm', self.nvdec_channel, NVDECCommandType.EXECUTE_DECODE, b''])
    else:
      # Log command for testing
      print(f"🎬 NVDEC decode command: {decode_params.pic_width_in_mbs}x{decode_params.pic_height_in_mbs} MBs -> {surface_desc.format}")

  def convert_surface_format(self, src_surface:NVVideoSurface, dst_surface:NVVideoSurface):
    """Convert between video surface formats (e.g., NV12 to RGB)"""
    # This would use 2D engine or compute shaders for format conversion
    # Following NVCopyQueue pattern for memory operations
    
    # Calculate conversion parameters
    if src_surface.format == "NV12" and dst_surface.format == "RGBA":
      # NV12 to RGBA conversion via 2D engine or compute shader
      # self.nvm(channel, FORMAT_CONVERT_COMMAND, conversion_params)
      pass
    
    return self

  def _submit(self, dev:'NVDevice'): 
    """Submit video queue to device"""
    # Video queues would use video GPFIFO once implemented
    # For now, use dedicated video GPFIFO if available, else compute GPFIFO
    if hasattr(dev, 'video_gpfifo') and dev.video_gpfifo:
      self._submit_to_gpfifo(dev, dev.video_gpfifo)
    else:
      # Fallback to compute GPFIFO for testing
      self._submit_to_gpfifo(dev, dev.compute_gpfifo)

# Extension to NVDevice for video capabilities
def _init_video_decode_support(self:'NVDevice'):
  """Initialize video decode capabilities on NV device"""
  # Check if device supports NVDEC engines
  try:
    from tinygrad.runtime.support.cuvid import check_hevc_support
    caps = check_hevc_support()
    self.video_decode_caps = caps
    self.has_video_decode = True
    
    # Setup video channel group and GPFIFO
    self._setup_video_channels()
    
    # Initialize NVDEC engines
    self._init_nvdec_engines()
    
    # Create video command queue
    self.video_queue = NVVideoQueue()
    
    print(f"✅ NVDEC support initialized: {caps.nNumNVDECs} engines, max {caps.nMaxWidth}x{caps.nMaxHeight}")
    
  except Exception as e:
    self.has_video_decode = False
    self.video_decode_caps = None
    self.video_queue = None
    self.video_gpfifo = None
    print(f"⚠️  Video decode not available: {e}")

def _setup_video_channels(self:'NVDevice'):
  """Setup video decode channel group and GPFIFO"""
  import tinygrad.runtime.autogen.nv_gpu as nv_gpu
  
  try:
    # Create video channel group with NVDEC0 engine
    video_channel_params = nv_gpu.NV_CHANNEL_GROUP_ALLOCATION_PARAMETERS(
      engineType=nv_gpu.NV2080_ENGINE_TYPE_NVDEC0
    )
    self.video_channel_group = self.iface.rm_alloc(self.nvdevice, nv_gpu.KEPLER_CHANNEL_GROUP_A, video_channel_params)
    
    # Reuse existing GPFIFO area for video channels (offset 0x180000)
    video_gpfifo_offset = 0x180000
    video_gpfifo_entries = 0x8000  # Smaller queue for video
    
    # Create context share for video operations
    video_ctxshare_params = nv_gpu.NV_CTXSHARE_ALLOCATION_PARAMETERS(
      hVASpace=getattr(self.iface, '_vaspace', 0),
      flags=nv_gpu.NV_CTXSHARE_ALLOCATION_FLAGS_SUBCONTEXT_ASYNC
    )
    self.video_ctxshare = self.iface.rm_alloc(self.video_channel_group, nv_gpu.FERMI_CONTEXT_SHARE_A, video_ctxshare_params)
    
    # Create video GPFIFO 
    # Note: Reusing compute GPFIFO area but different offset
    gpfifo_area = None
    for attr in ['compute_gpfifo', 'dma_gpfifo']:
      if hasattr(self, attr):
        gpfifo_area = getattr(self, attr)
        break
    
    if gpfifo_area:
      # Extract the underlying buffer from existing GPFIFO
      # This is a simplified approach - real implementation would allocate separate area
      self.video_gpfifo = self._new_gpu_fifo(
        gpfifo_area=self.iface.alloc(0x100000, contiguous=True, cpu_access=True, map_flags=0x10d0000),
        ctxshare=self.video_ctxshare,
        channel_group=self.video_channel_group,
        offset=0,
        entries=video_gpfifo_entries,
        compute=False  # Video decode, not compute
      )
      
      # Enable video channel group scheduling
      self.iface.rm_control(self.video_channel_group, nv_gpu.NVA06C_CTRL_CMD_GPFIFO_SCHEDULE, 
        nv_gpu.NVA06C_CTRL_GPFIFO_SCHEDULE_PARAMS(bEnable=1))
      
      print(f"✅ Video GPFIFO created with {video_gpfifo_entries} entries")
    else:
      print(f"⚠️  No existing GPFIFO found for video channel setup")
      self.video_gpfifo = None
      
  except Exception as e:
    print(f"⚠️  Video channel setup failed: {e}")
    self.video_gpfifo = None
    self.video_channel_group = None

def _init_nvdec_engines(self:'NVDevice'):
    """Initialize NVDEC hardware engines"""
    try:
      from tinygrad.runtime.support.nvdec import get_available_nvdec_engines, create_nvdec_engine
      
      # Get available NVDEC engines
      available_engines = get_available_nvdec_engines()
      
      if not available_engines:
        print("⚠️  No NVDEC engines found")
        self.nvdec_engines = []
        return
      
      # Initialize primary NVDEC engine (engine 0)
      self.primary_nvdec = create_nvdec_engine(engine_id=0, device_interface=self)
      if self.primary_nvdec and self.primary_nvdec.initialize():
        print(f"✅ Primary NVDEC engine initialized")
        
        # Setup video queue with NVDEC engine
        if hasattr(self, 'video_queue') and self.video_queue:
          self.video_queue.setup(
            decode_engine_class=self.primary_nvdec,
            nvdec_channel=getattr(self, 'video_channel_group', None)
          )
          print(f"✅ Video queue linked to NVDEC engine")
      else:
        print("⚠️  Primary NVDEC engine initialization failed")
        self.primary_nvdec = None
      
      # Store all available engines
      self.nvdec_engines = available_engines
      
    except Exception as e:
      print(f"⚠️  NVDEC engine initialization failed: {e}")
      self.nvdec_engines = []
      self.primary_nvdec = None

def _alloc_video_surface(self:'NVDevice', width:int, height:int, format:str="NV12") -> NVVideoSurface:
  """Allocate video surface for decode output"""
  if not self.has_video_decode:
    raise RuntimeError("Video decode not supported on this device")
  
  # Calculate surface size based on format
  if format == "NV12":
    y_size = width * height          # Y plane
    uv_size = (width * height) // 2  # UV plane (interleaved)
  ...
  # Allocate GPU memory for video surface
  buf = self.iface.alloc(total_size, cpu_access=False)
  
  # Return video surface wrapper
  return NVVideoSurface(buf.va_addr, total_size, width, height, format, meta=buf.meta)

def decode_hevc(self:'NVDevice', bitstream:bytes, width:int, height:int, 
                output_format:str="NV12", wait:bool=True, profile:str="default") -> NVVideoSurface:
  """
  High-level API to decode HEVC bitstream to video surface
  
  Args:
    bitstream: HEVC encoded data bytes
    width: Frame width in pixels
    height: Frame height in pixels  
    output_format: Output surface format ("NV12", "RGBA")
    wait: Wait for decode completion before returning
    profile: Memory pool profile for surface allocation
    
  Returns:
    NVVideoSurface containing decoded frame
    
  Raises:
    RuntimeError: If video decode not supported or operation fails
    ValueError: If parameters are invalid
  """
  # Input validation
  if not self.has_video_decode:
    raise RuntimeError("HEVC decode not supported on this device")
    
  if not bitstream or len(bitstream) == 0:
    raise ValueError("Empty bitstream data")
    
  if width <= 0 or height <= 0:
    raise ValueError(f"Invalid dimensions: {width}x{height}")
    
  if output_format not in ["NV12", "RGBA"]:
    raise ValueError(f"Unsupported output format: {output_format}")
  
  try:
    # Use integrated decoder for full pipeline
    from tinygrad.runtime.support.hevc_decoder import create_hevc_decoder
    
    # Get or create decoder for this resolution
    decoder_key = f"{width}x{height}"
    if not hasattr(self, '_hevc_decoders'):
      self._hevc_decoders = {}
      
    if decoder_key not in self._hevc_decoders:
      decoder = create_hevc_decoder(self, width, height)
      if not decoder:
        raise RuntimeError("Failed to create HEVC decoder")
      self._hevc_decoders[decoder_key] = decoder
    else:
      decoder = self._hevc_decoders[decoder_key]
    
    # Decode frame through integrated pipeline
    surface = decoder.decode_frame(bitstream, wait=wait)
    if not surface:
      raise RuntimeError("HEVC decode operation failed")
    
    # Format conversion if requested
    if surface.format != output_format:
      surface = self.convert_video_format(surface, output_format)
    
    return surface
    
  except Exception as e:
    print(f"❌ HEVC decode failed: {e}")
    raise

def decode_hevc_frame(self:'NVDevice', bitstream:bytes, width:int, height:int) -> NVVideoSurface:
  """Legacy API - decode single HEVC frame (deprecated, use decode_hevc)"""
  return self.decode_hevc(bitstream, width, height, output_format="NV12", wait=True)

def convert_video_format(self:'NVDevice', src_surface:NVVideoSurface, target_format:str) -> NVVideoSurface:
  """
  Convert video surface between formats (NV12 ↔ RGBA)
  
  Args:
    src_surface: Source video surface
    target_format: Target format ("NV12", "RGBA")
    
  Returns:
    New video surface in target format
  """
  if src_surface.format == target_format:
    return src_surface
    
  # Allocate target surface
  dst_surface = self._alloc_video_surface(src_surface.width, src_surface.height, target_format)
  
  # Submit format conversion command
  if hasattr(self, 'video_queue') and self.video_queue:
    self.video_queue.convert_surface_format(src_surface, dst_surface).submit(self)
  else:
    # Fallback: CPU-based conversion (simplified)
    print(f"⚠️  GPU format conversion not available, using CPU fallback")
    self._cpu_convert_surface(src_surface, dst_surface)
  
  return dst_surface

def _cpu_convert_surface(self:'NVDevice', src:NVVideoSurface, dst:NVVideoSurface):
  """CPU fallback for format conversion (simplified)"""
  # This would implement CPU-based NV12 ↔ RGBA conversion
  # Real implementation would copy GPU data to CPU, convert, and upload back
  print(f"🔄 CPU format conversion: {src.format} → {dst.format} ({src.width}x{src.height})")

def get_video_decode_caps(self:'NVDevice') -> dict:
  """
  Get video decode capabilities
  
  Returns:
    Dictionary with decode capabilities and statistics
  """
  if not self.has_video_decode:
    return {"supported": False, "reason": "Video decode not available"}
    
  caps = {
    "supported": True,
    "hevc_support": True,
    "max_width": self.video_decode_caps.nMaxWidth if self.video_decode_caps else 4096,
    "max_height": self.video_decode_caps.nMaxHeight if self.video_decode_caps else 4096,
    "num_engines": self.video_decode_caps.nNumNVDECs if self.video_decode_caps else 0,
    "supported_formats": ["NV12", "RGBA"],
    "max_concurrent_streams": 4
  }
  
  # Add engine information
  if hasattr(self, 'nvdec_engines'):
    caps["available_engines"] = len(self.nvdec_engines)
    caps["primary_engine"] = bool(getattr(self, 'primary_nvdec', None))
  
  return caps

def cleanup_video_decoders(self:'NVDevice'):
  """Clean up all cached video decoders"""
  if hasattr(self, '_hevc_decoders'):
    for decoder in self._hevc_decoders.values():
      decoder.destroy()
    self._hevc_decoders.clear()
    print(f"🧹 Cleaned up video decoders")

def decode_hevc_to_tensor(self:'NVDevice', bitstream:bytes, width:int, height:int,
                         output_format:str="RGB", normalize:bool=False, device:Optional[str]=None):
  """
  Decode HEVC bitstream directly to tinygrad Tensor
  
  Args:
    bitstream: HEVC encoded data bytes
    width: Frame width in pixels
    height: Frame height in pixels
    output_format: Tensor format ("RGB", "RGBA", "NV12")
    normalize: Normalize values to [0, 1] range
    device: Target device for tensor
    
  Returns:
    Tensor containing decoded video frame
  """
  from typing import Optional
  
  # Decode to video surface
  surface = self.decode_hevc(bitstream, width, height, output_format="NV12", wait=True)
  
  # Convert surface to tensor
  try:
    from tinygrad.runtime.support.video_tensor import create_video_tensor_converter
    converter = create_video_tensor_converter(self)
    tensor = converter.surface_to_tensor(surface, output_format, normalize, device)
    
    # Release surface memory after conversion
    if hasattr(self, 'memory_manager') and self.memory_manager:
      self.memory_manager.release_surface(surface)
    
    return tensor
    
  except ImportError:
    print(f"⚠️  Video tensor conversion not available")
    return surface

def decode_hevc_batch_to_tensor(self:'NVDevice', bitstreams:list[bytes], width:int, height:int,
                               output_format:str="RGB", normalize:bool=False, device:Optional[str]=None):
  """
  Decode multiple HEVC bitstreams to batched tensor
  
  Args:
    bitstreams: List of HEVC encoded data bytes
    width: Frame width in pixels  
    height: Frame height in pixels
    output_format: Tensor format ("RGB", "RGBA", "NV12")
    normalize: Normalize values to [0, 1] range
  ...
  Returns:
    Batched tensor [N, H, W, C] containing decoded frames
  """
  from typing import Optional
  
  if not bitstreams:
    raise ValueError("Empty bitstream list")
...
def tensor_to_hevc_surface(self:'NVDevice', tensor, width:int, height:int, format:str="RGBA"):
  """
  Convert tinygrad Tensor to video surface
  
  Args:
    tensor: Input tensor [H, W, C] or [H, W]
    width: Surface width
    height: Surface height
    format: Surface format ("NV12", "RGBA")
    
  Returns:
    NVVideoSurface containing tensor data
  """
  try:
    from tinygrad.runtime.support.video_tensor import tensor_to_video_surface
    return tensor_to_video_surface(tensor, width, height, format, self)
  except ImportError:
    print(f"⚠️  Tensor to surface conversion not available")
    raise RuntimeError("Video tensor integration not available")

# Monkey patch methods to NVDevice class
NVDevice._init_video_decode_support = _init_video_decode_support
NVDevice._alloc_video_surface = _alloc_video_surface  
NVDevice.decode_hevc = decode_hevc
NVDevice.decode_hevc_frame = decode_hevc_frame  # Legacy API
NVDevice.convert_video_format = convert_video_format
NVDevice._cpu_convert_surface = _cpu_convert_surface
NVDevice.get_video_decode_caps = get_video_decode_caps
NVDevice.cleanup_video_decoders = cleanup_video_decoders
# Tensor integration APIs
NVDevice.decode_hevc_to_tensor = decode_hevc_to_tensor
NVDevice.decode_hevc_batch_to_tensor = decode_hevc_batch_to_tensor
NVDevice.tensor_to_hevc_surface = tensor_to_hevc_surface
