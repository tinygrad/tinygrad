  ...
  def __init__(self, dev: NVDevice, name: str, lib: bytes):
    ...
    if dev.iface.compute_class >= nv_gpu.BLACKWELL_COMPUTE_A:
      ...
      qmd = {
        ...
        "program_address_upper_shifted4": hi32(self.prog_addr >> 4),
        "program_address_lower_shifted4": lo32(self.prog_addr >> 4),
      ...
      qmd = {
        ...
        "program_address_upper": hi32(self.prog_addr),
        "program_address_lower": lo32(self.prog_addr),
    ...
    self.qmd: QMD = QMD(
      ...
      **qmd,
      ...
      program_prefetch_size=min(self.prog_sz >> 8, 0x1FF),
...
class PCIIface(PCIIfaceBase):
  ...
  def __init__(self, dev, dev_id):
    ...
    self.dev_impl: NVDev = NVDev(
      self.pci_dev.pcibus,
      self.pci_dev.map_bar(0, fmt="I"),
      self.pci_dev.map_bar(1),
      self.pci_dev.read_config(pci.PCI_VENDOR_ID, 4),
      self.pci_dev.read_config(pci.PCI_SUBSYSTEM_VENDOR_ID, 4),
      self.pci_dev.read_config(pci.PCI_REVISION_ID, 1),
      self.pci_dev.bar_info,
    )
...
class NVDevice(HCQCompiled[HCQSignal]):
  ...
  def _new_gpu_fifo(self, gpfifo_area, ctxshare, channel_group, offset=0, entries=0x400, compute=False) -> GPFifo:
    ...
    return GPFifo(
      ring=MMIOInterface(gpfifo_area.va_addr + offset, entries * 8, fmt="Q"),
