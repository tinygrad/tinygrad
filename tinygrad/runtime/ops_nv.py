...
class NVAllocator(LRUAllocator):
  ...
  def copy_from_fd2(self, dest, fd, offset, size, device=None):
    PAGE_SIZE, BATCH_SIZE = (4 << 10), (2 << 20)
  ...
  def copy_from_fd(self, dest, fd, offset, size, device=None):
    ...
    free_bs = [i for i in range(len(self.b)) if self.b_timeline[i] <= self.device.timeline_signal[1]]
    ...
    while True:
      ...
      if len(free_bs) == 0 and len(free_bs_order) > 0 and self.b_timeline[free_bs_order[0]] <= self.device.timeline_signal[1]:
