...
def compile_hip(dev:HIPDevice, prg:str) -> bytes: return compile_cuda_style(prg, [f'--offload-arch={dev.arch}', '-I/opt/rocm/include'], hip.hiprtcProgram, hip.hiprtcCreateProgram, hip.hiprtcCompileProgram, hip.hiprtcGetCode, hip.hiprtcGetCodeSize, hip.hiprtcGetProgramLog, hip.hiprtcGetProgramLogSize, check)  # noqa: E501
...
class HIPDevice(Compiled):
  def __init__(self, device:str=""):
    ...
    super().__init__(MallocAllocator if MOCKHIP else HIPAllocator(self), LinearizerOptions("HIP"), HIPRenderer,
                     functools.partial(compile_hip, self), f"compile_hip_{self.arch}", functools.partial(HIPProgram, self.device), HIPGraph)
