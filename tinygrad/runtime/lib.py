...
class RawBufferMapped(RawBufferCopyIn):
  ...
  def toCPU(self) -> np.ndarray: return self.buffer_view().copy() if hasattr(self, '_needs_explicit_copy') else self.buffer_view() # JIT sets this to not overwrite buffers holded by CPU.
