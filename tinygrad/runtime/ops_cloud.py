...
class CloudHandler(BaseHTTPRequestHandler):
  ...
  def _do(self, method):
    ...
    if self.path == "/batch" and method == "POST":
      ...
      ret = json.dumps({'clouddev': CloudHandler.device, 'renderer': (cls.__module__, cls.__name__, args), 'graph': graph_cls is not None}).encode()
...
class CloudDevice(Compiled):
  def __init__(self, device:str):
    ...
    super().__init__(device, CloudAllocator(self), renderer_class(*renderer[2]), Compiler(), functools.partial(CloudProgram, self), graph)
