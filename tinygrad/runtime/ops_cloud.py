...
class BufferAlloc(CloudRequest):
    buffer_num: int
    size: int
...
@dataclass(frozen=True)
class BufferFree(CloudRequest):
    buffer_num: int  # noqa: E702


@dataclass(frozen=True)
class CopyIn(CloudRequest):
    buffer_num: int
    datahash: str  # noqa: E702


@dataclass(frozen=True)
class CopyOut(CloudRequest):
    buffer_num: int


@dataclass(frozen=True)
class ProgramAlloc(CloudRequest):
    name: str
    datahash: str  # noqa: E702


@dataclass(frozen=True)
class ProgramFree(CloudRequest):
    name: str
    datahash: str  # noqa: E702
...
eval_fxns = {
    ...
    ast.Name: lambda x: whitelist[x.id],
    ast.Attribute: lambda x: {"imagef": dtypes.imagef, "imageh": dtypes.imageh}[x.attr],
}
...
class BatchRequest:
    ...
    def h(self, d: bytes) -> str:
        ...
        self._h[datahash := binascii.hexlify(binhash).decode()] = (
            binhash + struct.pack("<Q", len(d)) + d
        )
        return datahash

    def q(self, x: CloudRequest):
        self._q.append(x)
...
class CloudSession:
    ...
    buffers: dict[int, tuple[Any, int, Optional[BufferSpec]]] = field(
...
class CloudHandler(BaseHTTPRequestHandler):
    ...
    def setup(self):
        super().setup()
        print(
            f"connection established with {self.client_address}, socket: {self.connection.fileno()}"
        )

    def _do(self, method):
        session = CloudHandler.sessions[
            unwrap(self.headers.get("Cookie")).split("session=")[1]
        ]
        ret, status_code = b"", 200
        if self.path == "/batch" and method == "POST":
            # TODO: streaming deserialize?
            req = BatchRequest().deserialize(
                self.rfile.read(int(unwrap(self.headers.get("Content-Length"))))
            )
            ...
            for c in req._q:
                ...
                match c:
                    case BufferAlloc():
                        assert (
                            c.buffer_num not in session.buffers
                        ), f"buffer {c.buffer_num} already allocated"
                        session.buffers[c.buffer_num] = (
                            Device[CloudHandler.device].allocator.alloc(
                                c.size, c.options
                            ),
                            c.size,
                            c.options,
                    ...
                    case BufferFree():
                        ...
                        Device[CloudHandler.device].allocator.free(
                            buf, sz, buffer_options
                        )
                        del session.buffers[c.buffer_num]
                    case CopyIn():
                        Device[CloudHandler.device].allocator._copyin(
                            session.buffers[c.buffer_num][0],
                            memoryview(bytearray(req._h[c.datahash])),
                        )
                    case CopyOut():
                        buf, sz, _ = session.buffers[c.buffer_num]
                        Device[CloudHandler.device].allocator._copyout(
                            memoryview(ret := bytearray(sz)), buf
                        )
                    case ProgramAlloc():
                        lib = Device[CloudHandler.device].compiler.compile_cached(
                            req._h[c.datahash].decode()
                        )
                        session.programs[(c.name, c.datahash)] = Device[
                            CloudHandler.device
                        ].runtime(c.name, lib)
                    case ProgramFree():
                        del session.programs[(c.name, c.datahash)]
                    case ProgramExec():
...
def cloud_server(port: int):
    ...
    CloudHandler.device = (
        getenv("CLOUDDEV", "METAL") if Device.DEFAULT == "CLOUD" else Device.DEFAULT
    )
...
class CloudAllocator(Allocator):
    ...
    def _free(self, opaque: int, options):
        self.device.req.q(BufferFree(opaque))

    def _copyin(self, dest: int, src: memoryview):
        self.device.req.q(CopyIn(dest, self.device.req.h(bytes(src))))
...
class CloudProgram:
    ...
    def __del__(self):
        self.dev.req.q(ProgramFree(self.name, self.datahash))
    ...
    def __call__(
        ...
        *bufs,
        ...
        if wait:
            return float(self.dev.batch_submit())
...
class CloudDevice(Compiled):
    def __init__(self, device: str):
        ...
        while 1:
            try:
                ...
                time.sleep(0.1)
        if DEBUG >= 1:
            print(f"remote has device {clouddev}")
        # TODO: how to we have BEAM be cached on the backend? this should just send a specification of the compute. rethink what goes in Renderer
        if not clouddev[0].startswith("tinygrad.renderer.") or not clouddev[1].endswith(
            "Renderer"
        ):
            raise RuntimeError(f"bad renderer {clouddev}")
        renderer_class = fromimport(clouddev[0], clouddev[1])  # TODO: is this secure?
        if not issubclass(renderer_class, Renderer):
            raise RuntimeError(f"renderer isn't a Renderer {clouddev}")
        super().__init__(
            device,
            CloudAllocator(self),
            renderer_class(*clouddev[2]),
            Compiler(),
            functools.partial(CloudProgram, self),
        )
    ...
    def batch_submit(self):
        ...
        with Timing(
            f"*** send {len(self.req._q):-3d} requests {len(self.req._h):-3d} hashes with len {len(data)/1024:.2f} kB in ",
            enabled=DEBUG >= 1,
        ):
    ...
    def send(self, method, path, data: Optional[bytes] = None) -> bytes:
        # TODO: retry logic
        self.conn.request(
            method, "/" + path, data, headers={"Cookie": f"session={self.session}"}
        )
