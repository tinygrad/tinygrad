...
from tinygrad.engine.jit import ExecItem, GraphRunner, MultiGraphRunner, graph_class
...
class CloudHandler(BaseHTTPRequestHandler):
  ...
  def _do(self, method):
    ...
    if self.path == "/batch" and method == "POST":
      ...
      props = {'clouddev': CloudHandler.device, 'renderer': (cls.__module__, cls.__name__, args), 'transfer': hasattr(dev.allocator, '_transfer'),
               'graph': dev.graph is not None, 'graph_multi': dev.graph is not None and issubclass(graph_class(dev), MultiGraphRunner)}
      ret = json.dumps(props).encode()
...
class CloudDevice(Compiled):
  def __init__(self, device:str):
    ...
    if DEBUG >= 1: print(f"remote has device {self.conn.properties['clouddev']}")
    # TODO: how to we have BEAM be cached on the backend? this should just send a specification of the compute. rethink what goes in Renderer
    renderer = self.conn.properties['renderer']
