...
AQL_HDR = (
  (1 << hsa.HSA_PACKET_HEADER_BARRIER)
...
class AMDSignal(HCQSignal):
  def __init__(self, *args, **kwargs):
    super().__init__(*args, **{**kwargs, "timestamp_divider": 100})

  def _sleep(self, time_spent_waiting_ms: int):
    # Resonable to sleep for long workloads (which take more than 2s) and only timeline signals.
    if time_spent_waiting_ms > 2000 and self.is_timeline and self.owner is not None:
...
class AMDComputeQueue(HWQueue):
  ...
  def pkt3(self, cmd, *vals):
    self.q(self.pm4.PACKET3(cmd, len(vals) - 1), *vals)

  def wreg(self, reg: AMDReg, *args: sint, **kwargs: int):
    if bool(args) == bool(kwargs):
      raise RuntimeError("One (and only one) of *args or **kwargs must be specified")
    if self.pm4.PACKET3_SET_SH_REG_START <= reg.addr[0] < self.pm4.PACKET3_SET_SH_REG_END:
      ...
      raise RuntimeError(f"Cannot set {reg.name} ({reg.addr[0]}) via pm4 packet")
  ...
  def wait_reg_mem(self, value, mask=0xFFFFFFFF, mem=None, reg_req=None, reg_done=None):
    wrm_info_dw = (
      self.pm4.WAIT_REG_MEM_MEM_SPACE(int(mem is not None))
      | self.pm4.WAIT_REG_MEM_OPERATION(int(mem is None))
      | self.pm4.WAIT_REG_MEM_FUNCTION(WAIT_REG_MEM_FUNCTION_GEQ)
      | self.pm4.WAIT_REG_MEM_ENGINE(0)
    )
  ...
  def acquire_mem(self, addr=0x0, sz=(1 << 64) - 1, gli=1, glm=1, glk=1, glv=1, gl1=1, gl2=1):
    if self.dev.target >= (10, 0, 0):
      cache_flags_dw = (
        self.pm4.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLI_INV(gli)
        | self.pm4.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLM_INV(glm)
        | self.pm4.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLM_WB(glm)
        | self.pm4.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLK_INV(glk)
        | self.pm4.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLK_WB(glk)
        | self.pm4.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLV_INV(glv)
        | self.pm4.PACKET3_ACQUIRE_MEM_GCR_CNTL_GL1_INV(gl1)
        | self.pm4.PACKET3_ACQUIRE_MEM_GCR_CNTL_GL2_INV(gl2)
        | self.pm4.PACKET3_ACQUIRE_MEM_GCR_CNTL_GL2_WB(gl2)
      )

      self.pkt3(self.pm4.PACKET3_ACQUIRE_MEM, 0, *data64_le(sz), *data64_le(addr), 0, cache_flags_dw)
    else:
      cp_coher_cntl = (
        self.pm4.PACKET3_ACQUIRE_MEM_CP_COHER_CNTL_SH_ICACHE_ACTION_ENA(gli)
    ...
    if self.dev.target >= (10, 0, 0):
      cache_flags_dw = (
        ...
        if not cache_flush
          ...
          self.pm4.PACKET3_RELEASE_MEM_GCR_GLV_INV
          | self.pm4.PACKET3_RELEASE_MEM_GCR_GL1_INV
          | self.pm4.PACKET3_RELEASE_MEM_GCR_GL2_INV
          | self.pm4.PACKET3_RELEASE_MEM_GCR_GLM_WB
          | self.pm4.PACKET3_RELEASE_MEM_GCR_GLM_INV
          | self.pm4.PACKET3_RELEASE_MEM_GCR_GL2_WB
          | self.pm4.PACKET3_RELEASE_MEM_GCR_SEQ
        )
      )

      event_dw = self.pm4.PACKET3_RELEASE_MEM_EVENT_TYPE(self.pm4.CACHE_FLUSH_AND_INV_TS_EVENT) | self.pm4.PACKET3_RELEASE_MEM_EVENT_INDEX(
        self.pm4.event_index__mec_release_mem__end_of_pipe
      )

      memsel_dw = (
        self.pm4.PACKET3_RELEASE_MEM_DATA_SEL(data_sel) | self.pm4.PACKET3_RELEASE_MEM_INT_SEL(int_sel) | self.pm4.PACKET3_RELEASE_MEM_DST_SEL(0)
      )
    else:
  ...
  def memory_barrier(self):
    ...
    self.wait_reg_mem(
      reg_req=getattr(self.nbio, f"regBIF_BX_PF{pf}_GPU_HDP_FLUSH_REQ").addr[0],
      reg_done=getattr(self.nbio, f"regBIF_BX_PF{pf}_GPU_HDP_FLUSH_DONE").addr[0],
      value=0xFFFFFFFF,
    )
    self.acquire_mem()
    return self
  ...
  def spi_config(self, tracing: bool):
    self.wreg(
      self.gc.regSPI_CONFIG_CNTL,
      ps_pkr_priority_cntl=3,
      exp_priority_order=3,
      gpr_write_priority=0x2C688,
      enable_sqg_bop_events=int(tracing),
      enable_sqg_top_events=int(tracing),
    )
  ...
  def sqtt_config(self, tracing: bool):
    self.wreg(
      self.gc.regSQ_THREAD_TRACE_CTRL,
      draw_event_en=1,
      spi_stall_en=1,
      sq_stall_en=1,
      reg_at_hwm=2,
      ...
      util_timer=self.soc.SQ_TT_UTIL_TIMER_250_CLK,
      mode=int(tracing),
    )
  ...
  def sqtt_start(self, buf0s: list[HCQBuffer], se_mask: int):
    ...
    for se in range(len(buf0s)):
      ...
      REG_INCLUDE = (
        self.soc.SQ_TT_TOKEN_MASK_SQDEC_BIT
        | self.soc.SQ_TT_TOKEN_MASK_SHDEC_BIT
        | self.soc.SQ_TT_TOKEN_MASK_GFXUDEC_BIT
        | self.soc.SQ_TT_TOKEN_MASK_COMP_BIT
        | self.soc.SQ_TT_TOKEN_MASK_CONTEXT_BIT
        | self.soc.SQ_TT_TOKEN_MASK_CONTEXT_BIT
      )
      TOKEN_EXCLUDE = 1 << self.soc.SQ_TT_TOKEN_EXCLUDE_PERF_SHIFT
      if not (se_mask >> se) & 0b1:
        TOKEN_EXCLUDE |= (
          1 << self.soc.SQ_TT_TOKEN_EXCLUDE_VMEMEXEC_SHIFT
          | 1 << self.soc.SQ_TT_TOKEN_EXCLUDE_ALUEXEC_SHIFT
          | 1 << self.soc.SQ_TT_TOKEN_EXCLUDE_VALUINST_SHIFT
  ...
  def sqtt_stop(self, ses: int, wptrs: HCQBuffer):
    ...
    for se in range(ses):
      ...
      self.pkt3(
        self.pm4.PACKET3_WAIT_REG_MEM,
        self.pm4.WAIT_REG_MEM_FUNCTION(WAIT_REG_MEM_FUNCTION_EQ),
        self.gc.regSQ_THREAD_TRACE_STATUS.addr[0],
        0,
        0,
        self.gc.regSQ_THREAD_TRACE_STATUS.fields_mask("finish_pending"),
        4,
      )
      # Wait for FINISH_DONE!=0
      self.pkt3(
        self.pm4.PACKET3_WAIT_REG_MEM,
        self.pm4.WAIT_REG_MEM_FUNCTION(WAIT_REG_MEM_FUNCTION_NEQ),
        self.gc.regSQ_THREAD_TRACE_STATUS.addr[0],
        0,
        0,
        self.gc.regSQ_THREAD_TRACE_STATUS.fields_mask("finish_done"),
        4,
      )
      # Disable SQTT
      ...
      self.pkt3(
        ...
        0,
        0,
        self.gc.regSQ_THREAD_TRACE_STATUS.fields_mask("busy"),
        4,
      )
  ...
  def sqtt_prg_marker(self, prg: AMDProgram, global_size: tuple[sint, ...]):
    ...
    self.sqtt_userdata(
      sqtt.struct_rgp_sqtt_marker_pipeline_bind(
        _0=sqtt.union_rgp_sqtt_marker_pipeline_bind_0(
          _0=sqtt.struct_rgp_sqtt_marker_pipeline_bind_0_0(identifier=sqtt.RGP_SQTT_MARKER_IDENTIFIER_BIND_PIPELINE, bind_point=BIND_POINT_COMPUTE)
        ),
    ...
    self.sqtt_userdata(
      sqtt.struct_rgp_sqtt_marker_event(
  ...
  def exec(self, prg: AMDProgram, args_state: CLikeArgsState, global_size: tuple[sint, ...], local_size: tuple[sint, ...]):
    ...
    if prg.dev.sqtt_enabled:
      self.sqtt_prg_marker(prg, global_size)
    ...
    if (10, 0, 0) <= prg.dev.target < (11, 0, 0):
  ...
  def bind(self, dev: AMDDevice):
    ...
    self.indirect_cmd = [
      self.pm4.PACKET3(self.pm4.PACKET3_INDIRECT_BUFFER, 2),
  ...
  def _submit(self, dev: AMDDevice):
    ...
    if self.dev.xccs > 1 and dev != self.binded_device:
      ...
      cmds = [
        self.pm4.PACKET3(self.pm4.PACKET3_INDIRECT_BUFFER, 2),
        *data64_le(ib_ptr),
        len(cmds) | self.pm4.INDIRECT_BUFFER_VALID,
        self.pm4.PACKET3(self.pm4.PACKET3_NOP, ib_pad + len(cmds) - 1),
        *((0,) * ib_pad),
        *cmds,
      ]
...
class AMDComputeAQLQueue(AMDComputeQueue):
  ...
  def _submit(self, dev: AMDDevice):
    ...
    def flush_pm4_batch():
      ...
      pkt = [
        AQL_HDR | (hsa.HSA_PACKET_TYPE_VENDOR_SPECIFIC << hsa.HSA_PACKET_HEADER_TYPE) | (1 << 16),
        self.pm4.PACKET3(self.pm4.PACKET3_INDIRECT_BUFFER, 2),
        *data64_le(dev.pm4_ibs.va_addr + off),
        len(pm4_batch) | self.pm4.INDIRECT_BUFFER_VALID,
        10,
      ]
      pm4_batch.clear()
    ...
    for cmd in self._q:
      if isinstance(cmd, hsa.hsa_kernel_dispatch_packet_t):
        aql_bytes += flush_pm4_batch() + bytes(cmd)
      else:
        pm4_batch.append(cmd)
    ...
    if (tail_bytes := (len(aql_bytes) - cp_bytes)) > 0:
      dev.compute_queue.ring.view(offset=0, fmt="B")[:tail_bytes] = aql_bytes[cp_bytes:]
...
class AMDCopyQueue(HWQueue):
  ...
  def copy(self, dest: sint, src: sint, copy_size: int):
    ...
    for _ in range(copy_commands):
      ...
      self.q(
        self.sdma.SDMA_OP_COPY | self.sdma.SDMA_PKT_COPY_LINEAR_HEADER_SUB_OP(self.sdma.SDMA_SUBOP_COPY_LINEAR),
        self.sdma.SDMA_PKT_COPY_LINEAR_COUNT_COUNT(step_copy_size - 1),
        0,
  ...
  def signal(self, signal: AMDSignal, value: sint = 0):
    ...
    elif dev is not None and dev.is_am():
      self.q(self.sdma.SDMA_OP_TRAP, self.sdma.SDMA_PKT_TRAP_INT_CONTEXT_INT_CONTEXT(0))

    return self

  def wait(self, signal: AMDSignal, value: sint = 0):
    self.q(
      self.sdma.SDMA_OP_POLL_REGMEM
      | self.sdma.SDMA_PKT_POLL_REGMEM_HEADER_FUNC(WAIT_REG_MEM_FUNCTION_GEQ)
  ...
  def timestamp(self, signal: AMDSignal):
    self.q(
      self.sdma.SDMA_OP_TIMESTAMP | self.sdma.SDMA_PKT_TIMESTAMP_GET_HEADER_SUB_OP(self.sdma.SDMA_SUBOP_TIMESTAMP_GET_GLOBAL),
      *data64_le(signal.timestamp_addr),
    )
    return self

  def bind(self, dev: AMDDevice):
    if not getenv("AMD_SDMA_BIND", 0) or not dev.is_am():
      return
    ...
    self.indirect_cmd = [
      self.sdma.SDMA_OP_INDIRECT | self.sdma.SDMA_PKT_INDIRECT_HEADER_VMID(0),
      *data64_le(self.hw_page.va_addr),
      qsz,
      *data64_le(0),
    ]
  ...
  def _submit(self, dev: AMDDevice):
    ...
    for cmdsz in cmd_sizes:
      if (tail_blit_dword + cmdsz) * 4 >= dev.sdma_queue.ring.nbytes - dev.sdma_queue.put_value % dev.sdma_queue.ring.nbytes:
        break
      tail_blit_dword += cmdsz

    # Force align of submits to hit our usb layer write cache.
    if (rem_packet_cnt := len(cmds) - tail_blit_dword) > 0 and dev.is_usb():
      tail_blit_dword = 0
    ...
    while not dev.is_usb() and dev.sdma_queue.put_value + total_bytes - dev.sdma_queue.read_ptr > dev.sdma_queue.ring.nbytes:
      pass
...
class AMDProgram(HCQProgram):
  def __init__(self, dev: AMDDevice, name: str, lib: bytes):
    ...
    for apply_image_offset, rel_sym_offset, typ, addent in relocs:
      if typ == 5:
        image[apply_image_offset : apply_image_offset + 8] = struct.pack("<q", rel_sym_offset - apply_image_offset + addent)  # R_AMDGPU_REL64
      else:
    ...
    if dev.sqtt_enabled:
      self.libhash: tuple[int, int] = struct.unpack("<Q", hashlib.md5(self.lib).digest()[:8]) * 2
...
class AMDAllocator(HCQAllocator["AMDDevice"]):
  def __init__(self, dev: AMDDevice):
    ...
    if hasattr(dev.iface, "as_dmaref"):
      self._as_dmaref = dev.iface.as_dmaref
  ...
  def _map(self, buf: HCQBuffer):
    return self.dev.iface.map(buf._base if buf._base is not None else buf)
...
class AMDQueueDesc:
  ...
  @property
  def read_ptr(self):
    return min(p[0] for p in self.read_ptrs)

  def signal_doorbell(self, dev, doorbell_value: int | None = None):
    for write_ptr in self.write_ptrs:
      write_ptr[0] = self.put_value
    ...
    for doorbell in self.doorbells:
      doorbell[0] = self.put_value if doorbell_value is None else doorbell_value
...
class KFDIface:
  ...
  def __init__(self, dev, device_id):
    ...
    self.props = {
      (p := l.split())[0]: int(p[1]) for l in FileIOInterface(f"{kfd_topo_path}/{KFDIface.gpus[device_id]}/properties").read().splitlines()
    }
    ...
    self.ip_offsets = {
      ip: {
        int(i): tuple(int(x, 16) for x in FileIOInterface(f"{ip_base}/{hw}/{i}/base_addr").read().splitlines())
        for i in FileIOInterface(f"{ip_base}/{hw}").listdir()
      }
      for ip, hw in ip_hw
    }
  ...
  def alloc(self, size: int, host=False, uncached=False, cpu_access=False, contiguous=False, cpu_addr=None) -> HCQBuffer:
    ...
    if uncached:
      ...
      flags |= kfd.KFD_IOC_ALLOC_MEM_FLAGS_USERPTR if host else kfd.KFD_IOC_ALLOC_MEM_FLAGS_VRAM
    ...
    if flags & kfd.KFD_IOC_ALLOC_MEM_FLAGS_USERPTR:
      ...
      buf, addr = 0, FileIOInterface.anon_mmap(0, size, 0, mmap.MAP_PRIVATE | mmap.MAP_ANONYMOUS | MAP_NORESERVE, 0)

    try:
      mem = kfd.AMDKFD_IOC_ALLOC_MEMORY_OF_GPU(
        self.kfd, va_addr=addr, size=size, base=addr, length=size, gpu_id=self.gpu_id, flags=flags, mmap_offset=buf
      )
    except OSError as e:
      ...
      if e.errno == errno.ENOMEM:
        raise MemoryError(f"Cannot allocate {size} bytes: no memory is available.") from e
      raise
  ...
  def create_queue(self, queue_type, ring, gart, rptr, wptr, eop_buffer=None, cwsr_buffer=None, ctl_stack_size=0, ctx_save_restore_size=0, xcc_id=0):
    queue = kfd.AMDKFD_IOC_CREATE_QUEUE(
      ...
      queue_percentage=kfd.KFD_MAX_QUEUE_PERCENTAGE | (xcc_id << 8),
      ...
      ctx_save_restore_address=cwsr_buffer.va_addr if cwsr_buffer else 0,
      ctx_save_restore_size=ctx_save_restore_size,
      write_pointer_address=gart.va_addr + wptr,
      read_pointer_address=gart.va_addr + rptr + 8 * xcc_id,
    )
    ...
    return AMDQueueDesc(
      ring=MMIOInterface(ring.va_addr, ring.size, fmt="I"),
      read_ptrs=[MMIOInterface(queue.read_pointer_address, 8, fmt="Q")],
...
class PCIIface(PCIIfaceBase):
  ...
  def __init__(self, dev, dev_id):
    super().__init__(
      ...
      dev_id,
      vendor=0x1002,
      devices=[0x744C, 0x7480, 0x7550],
      bars=[0, 2, 5],
      vram_bar=0,
      va_start=AMMemoryManager.va_allocator.base,
      va_size=AMMemoryManager.va_allocator.size,
    )
  ...
  def _setup_adev(
    self, name, vram: MMIOInterface, doorbell: MMIOInterface, mmio: MMIOInterface, dma_regions: list[tuple[int, MMIOInterface]] | None = None
  ):
    ...
    self.props = {
      ...
      "max_slots_scratch_cu": self.dev_impl.gc_info.gc_max_scratch_slots_per_cu,
      "max_waves_per_simd": self.dev_impl.gc_info.gc_max_waves_per_simd,
      "simd_arrays_per_engine": self.dev_impl.gc_info.gc_num_sa_per_se,
      "lds_size_in_kb": self.dev_impl.gc_info.gc_lds_size,
    }
  ...
  def create_queue(self, queue_type, ring, gart, rptr, wptr, eop_buffer=None, cwsr_buffer=None, ctl_stack_size=0, ctx_save_restore_size=0, xcc_id=0):
    ...
    if queue_type == kfd.KFD_IOC_QUEUE_TYPE_SDMA:
      self.dev_impl.sdma.setup_ring(
        ring_addr=ring.va_addr,
        ring_size=ring.size,
        rptr_addr=gart.va_addr + rptr,
        wptr_addr=gart.va_addr + wptr,
        doorbell=(doorbell_index := am.AMDGPU_NAVI10_DOORBELL_sDMA_ENGINE0),
        pipe=0,
        queue=0,
      )
    else:
      self.dev_impl.gfx.setup_ring(
        ring_addr=ring.va_addr,
        ring_size=ring.size,
        rptr_addr=gart.va_addr + rptr,
        wptr_addr=gart.va_addr + wptr,
    ...
    return AMDQueueDesc(
      ring=ring.cpu_view().view(fmt="I"),
      doorbells=[self.dev_impl.doorbell64.view(doorbell_index * 8, 8, fmt="Q")],
      read_ptrs=[gart.cpu_view().view(offset=rptr, size=8, fmt="Q")],
      write_ptrs=[gart.cpu_view().view(offset=wptr, size=8, fmt="Q")],
    )
  ...
  def on_device_hang(self):
    ...
    for d in devs:
      d.iface.dev_impl.gmc.on_interrupt()
...
class USBIface(PCIIface):
  def __init__(self, dev, dev_id):
    ...
    self._setup_adev(
      f"usb:{dev_id}",
      USBMMIOInterface(self.usb, *self.bars[0], fmt="B"),
      USBMMIOInterface(self.usb, *self.bars[2], fmt="Q"),
      USBMMIOInterface(self.usb, *self.bars[5], fmt="I"),
      dma_regions=[(0x200000, self._dma_view(0xF000, 0x80000))],
    )
  ...
  def _dma_view(self, ctrl_addr, size):
    return USBMMIOInterface(self.usb, ctrl_addr, size, fmt="B", pcimem=False)
...
class AMDDevice(HCQCompiled):
  def is_am(self) -> bool:
    return isinstance(self.iface, (PCIIface, USBIface))

  def is_usb(self) -> bool:
    return isinstance(self.iface, USBIface)
  ...
  def __init__(self, device: str = ""):
    ...
    if self.target < (9, 4, 2) or self.target >= (13, 0, 0):
      raise RuntimeError(f"Unsupported arch: {self.arch}")
    if DEBUG >= 1:
      print(f"AMDDevice: opening {self.device_id} with target {self.target} arch {self.arch}")

    self.max_cu_id = self.iface.props["simd_count"] // self.iface.props["simd_per_cu"] // self.iface.props.get("num_xcc", 1) - 1
    self.max_wave_id = (
      (self.iface.props["max_waves_per_simd"] * self.iface.props["simd_per_cu"] - 1)
      if self.target >= (10, 1, 0)
      else (min((self.max_cu_id + 1) * 40, self.iface.props["array_count"] // self.iface.props["simd_arrays_per_engine"] * 512) - 1)
    )
    ...
    if self.target[:2] == (9, 5):
      lds_size_per_cu = self.iface.props["lds_size_in_kb"] << 10
    vgpr_size_per_cu = (
      0x60000
      if self.target in {(11, 0, 0), (11, 0, 1), (12, 0, 0), (12, 0, 1)}
      else 0x80000
      if (self.target[:2]) in {(9, 4), (9, 5)} or self.target in {(9, 0, 8), (9, 0, 10)}
      else 0x40000
    )
    wg_data_size = round_up((vgpr_size_per_cu + sgrp_size_per_cu + lds_size_per_cu + hwreg_size_per_cu) * (self.max_cu_id + 1), mmap.PAGESIZE)
    ctl_stack_size = (
      round_up(12 * (self.max_cu_id + 1) * (self.max_wave_id + 1) + 8 + 40, mmap.PAGESIZE)
      if self.target >= (10, 1, 0)
      else round_up((self.max_wave_id + 1) * 8 + 8 + 40, mmap.PAGESIZE)
    )
    ...
    self.compute_queue = self.create_queue(
      kfd.KFD_IOC_QUEUE_TYPE_COMPUTE_AQL if self.is_aql else kfd.KFD_IOC_QUEUE_TYPE_COMPUTE,
      0x2000 if self.is_usb() else (16 << 20),
      eop_buffer_size=0x1000,
      ctx_save_restore_size=0 if self.is_am() else wg_data_size + ctl_stack_size,
      ctl_stack_size=ctl_stack_size,
    ...
    super().__init__(
      device,
      AMDAllocator(self),
      AMDLLVMRenderer(self.arch) if AMD_LLVM else AMDRenderer(self.arch),
      AMDLLVMCompiler(self.arch) if AMD_LLVM else HIPCompiler(self.arch),
    ...
    if self.sqtt_enabled:
      if self.arch != "gfx1100":
        raise RuntimeError("SQ Thread Tracing is only supported on 7900XTX")
      if not self.is_am() and (ppfeaturemask := int(FileIOInterface("/sys/module/amdgpu/parameters/ppfeaturemask", os.O_RDONLY).read(), 16)) & 0x8000:
        raise RuntimeError(
          "SQTT can't be enabled because of hardware bug, to workaround either use AMD_IFACE=PCI or add "
  ...
  def create_queue(self, queue_type, ring_size, ctx_save_restore_size=0, eop_buffer_size=0, ctl_stack_size=0, debug_memory_size=0):
    ...
    if queue_type == kfd.KFD_IOC_QUEUE_TYPE_COMPUTE_AQL:
      aql_desc = hsa.amd_queue_t(
        queue_properties=hsa.AMD_QUEUE_PROPERTIES_IS_PTR64 | hsa.AMD_QUEUE_PROPERTIES_ENABLE_PROFILING,
        read_dispatch_id_field_base_byte_offset=getattr(hsa.amd_queue_t, "read_dispatch_id").offset,
        max_cu_id=self.max_cu_id,
        max_wave_id=self.max_wave_id,
      )
    ...
    return self.iface.create_queue(
      queue_type,
      ring,
      gart,
      rptr=getattr(hsa.amd_queue_t, "read_dispatch_id").offset,
      wptr=getattr(hsa.amd_queue_t, "write_dispatch_id").offset,
      eop_buffer=eop_buffer,
      cwsr_buffer=cwsr_buffer,
      ctx_save_restore_size=ctx_save_restore_size,
      ctl_stack_size=ctl_stack_size,
    )
  ...
  def _ensure_has_local_memory(self, required):
    ...
    if ok:
      ...
      if hasattr(self, "aql_desc"):
        ...
        self.aql_desc.scratch_resource_descriptor[:] = [
          lo32(self.scratch.va_addr),
          hi32(self.scratch.va_addr) | (1 << 30),
          lo32(self.scratch.size),
          0x20814FAC,
        ]  # FORMAT=BUF_FORMAT_32_UINT,OOB_SELECT=2,ADD_TID_ENABLE=1,TYPE=SQ_RSRC_BUF,SQ_SELs
    ...
    if self.sqtt_enabled:
      ...
      for i, buf0 in enumerate(self.sqtt_buffers):
        ...
        if wptr >= buf0.size - 32:
          print(f"WARNING: SQTT BUFFER IS FULL (SE {i})! INCREASE SQTT BUFFER SIZE WITH SQTT_BUFFER_SIZE=X (in MB)")
