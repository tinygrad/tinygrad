...
class AMDComputeAQLQueue(AMDComputeQueue):
  ...
  def _submit(self, dev:AMDDevice):
    ...
    dev.compute_queue.ring.view(offset=dev.compute_queue.put_value * 64, fmt='B')[:len(aql_bytes)] = aql_bytes
...
class KFDIface:
  ...
  def create_queue(self, queue_type, ring, gart, eop_buffer=None, cwsr_buffer=None, ctl_stack_size=0, ctx_save_restore_size=0, off_rptr=8, off_wptr=0, xcc_id=0):
...
class AMDDevice(HCQCompiled):
  ...
  def create_queue(self, queue_type, ring_size, ctx_save_restore_size=0, eop_buffer_size=0, ctl_stack_size=0, debug_memory_size=0):
    ...
    return AMDQueueDesc.multi(*(self.iface.create_queue(queue_type, ring, gart, eop_buffer=eop_buffer, cwsr_buffer=cwsr_buffer, xcc_id=xcc_id,
      ...
      off_rptr=getattr(hsa.amd_queue_t, 'read_dispatch_id').offset, off_wptr=getattr(hsa.amd_queue_t, 'write_dispatch_id').offset)
