...
from tinygrad.device import Compiled, ProfileEvent, BufferSpec, CPUProgram, DMAFdRef, PROFILE
...
class AMDAllocator(HCQAllocator['AMDDevice']):
  def __init__(self, dev:AMDDevice):
    ...
    if isinstance(dev.dev_iface, KFDIface): self._as_dmaref = dev.dev_iface.as_dmaref
...
class KFDIface:
  ...
  def as_dmaref(self, mem:HCQBuffer) -> DMAFdRef:
    ...
    return DMAFdRef(kfd.AMDKFD_IOC_EXPORT_DMABUF(KFDIface.kfd, handle=base.meta.handle, flags=0).dmabuf_fd, mem.va_addr-base.va_addr, mem.size)
