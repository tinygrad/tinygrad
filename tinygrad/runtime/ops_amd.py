...
class AMDComputeQueue(HWQueue):
  ...
  def exec(self, prg:AMDProgram, args_state:CLikeArgsState, global_size:tuple[sint, ...], local_size:tuple[sint, ...]):
    ...
    if (10,0,0) <= prg.dev.gfx_ver < (11,0,0): self.wreg(self.gc.mmCP_COHER_START_DELAY, 0x20)
    ...
    gfx10p = {'cs_w32_en': int(prg.wave32)} if prg.dev.gfx_ver >= (10,0,0) else {}
...
class AMDCopyQueue(HWQueue):
  ...
  def signal(self, signal:AMDSignal, value:sint=0):
    fence_flags = self.sdma.SDMA_PKT_FENCE_HEADER_MTYPE(3) if self.dev.gfx_ver >= (10,0,0) else 0
...
class AMDProgram(HCQProgram):
  def __init__(self, dev:AMDDevice, name:str, lib:bytes):
    ...
    self.rsrc1: int = code.compute_pgm_rsrc1 | ((1 << 20) if (11,0,0) <= self.dev.gfx_ver < (12,0,0) else 0)
...
class AMDDevice(HCQCompiled):
  ...
  def __init__(self, device:str=""):
    ...
    self.gfx_ver:tuple[int, ...] = ((trgt:=self.dev_iface.props['gfx_target_version']) // 10000, (trgt // 100) % 100, trgt % 100)
    ...
    self.max_wave_id = (self.dev_iface.props['max_waves_per_simd'] * self.dev_iface.props['simd_per_cu'] - 1) if self.gfx_ver >= (10,1,0) else \
    ...
    self.has_scratch_base_registers = self.gfx_ver >= (11,0,0) or self.gfx_ver == (9,4,2) # this is what llvm refers to as "architected flat scratch"

    # https://gitlab.freedesktop.org/agd5f/linux/-/blob/a1fc9f584c4aaf8bc1ebfa459fc57a3f26a290d8/drivers/gpu/drm/amd/amdkfd/kfd_queue.c#L391
    sgrp_size_per_cu, lds_size_per_cu, hwreg_size_per_cu = 0x4000, 0x10000, 0x1000
    vgpr_size_per_cu = 0x60000 if self.gfx_ver in {(11,0,0), (11,0,1), (12,0,0), (12,0,1)} else \
                       0x80000 if (self.gfx_ver[:2]) == (9,4) or self.gfx_ver in {(9,0,8), (9,0,10)} else 0x40000
    wg_data_size = round_up((vgpr_size_per_cu + sgrp_size_per_cu + lds_size_per_cu + hwreg_size_per_cu) * (self.max_cu_id + 1), mmap.PAGESIZE)
    ctl_stack_size = round_up(12 * (self.max_cu_id + 1) * (self.max_wave_id + 1) + 8 + 40, mmap.PAGESIZE) if self.gfx_ver >= (10,1,0) else \
                     round_up((self.max_wave_id + 1) * 8 + 8 + 40, mmap.PAGESIZE)
    debug_memory_size = round_up((self.max_cu_id + 1 if self.gfx_ver >= (10,1,0) else 1) * (self.max_wave_id + 1) * 32, 64)
    if self.gfx_ver[0] == 10: ctl_stack_size = min(ctl_stack_size, 0x7000)

    self.soc = importlib.import_module(f"tinygrad.runtime.autogen.am.{({9: 'vega10', 10: 'navi10', 11: 'soc21', 12: 'soc24'}[self.gfx_ver[0]])}")
    self.pm4 = importlib.import_module(f"tinygrad.runtime.autogen.am.pm4_{'nv' if self.gfx_ver[0] >= 10 else 'soc15'}")
    ...
    pad = (0,) if self.gfx_ver[0] == 9 else ()
    self.nbio = AMDIP('nbio' if self.gfx_ver[0]<12 else 'nbif', self.dev_iface.ip_versions[am.NBIF_HWIP], pad+self.dev_iface.ip_offsets[am.NBIF_HWIP])
  ...
  def _ensure_has_local_memory(self, required):
    ...
    wave_scratch_len = round_up(((self.max_wave_id + 1) * required), 256 if self.gfx_ver >= (11,0,0) else 1024)
    ...
    if ok:
      ...
      waves = wave_scratch_len // (256 if self.gfx_ver >= (11,0,0) else 1024)
      # >=gfx11 wavesize is per SE
      wavesize = scratch_size // ((wave_scratch_len * engines) if self.gfx_ver >= (11,0,0) else wave_scratch_len)
