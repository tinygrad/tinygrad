...
class PCIIface:
  ...
  def __init__(self, dev, dev_id):
    ...
    if first_dev:=len(PCIIface.gpus) == 0:
      ...
      visible_devices = [int(x) for x in (getenv('VISIBLE_DEVICES', getenv('HIP_VISIBLE_DEVICES', ''))).split(',') if x.strip()]
      PCIIface.gpus_rl = HWInterface.pci_scan(vendor_id=0x1002, device_id=0x744c)
      PCIIface.gpus = [list(PCIIface.gpus_rl.keys())[x] for x in visible_devices] if visible_devices else list(PCIIface.gpus_rl.keys())

    self.pcibus = PCIIface.gpus[dev_id]
    self.pcidev = PCIIface.gpus_rl[self.pcibus]
    ...
    # Probe device
    HWInterface.pci_probe(self.pcidev)
    ...
    else: HWInterface.pci_enable(self.pcidev)
  ...
  def _map_pci_range(self, bar, off=0, addr=0, size=None):
    ...
    else: fd, sz = self.bar_fds[bar], size or self.pcidev.regions[bar].size # TODO: libpciaccess
