...
from tinygrad.runtime.support.amd_common import IPReg, collect_registers
...
class AMDComputeQueue(HWQueue):
  def __init__(self, dev:AMDDevice):
    self.gc, self.nbio = dev.gc, dev.nbio
    super().__init__()
  ...
  def pkt3(self, cmd, *vals): self.q(self.gc.PACKET3(cmd, len(vals) - 1), *vals)

  def wreg(self, reg:AMDReg, *args:sint, **kwargs:int):
    if bool(args) == bool(kwargs): raise RuntimeError('One (and only one) of *args or *kwargs must be specified')
    if self.gc.PACKET3_SET_SH_REG_START <= reg.addr < self.gc.PACKET3_SET_SH_REG_END:
      set_packet, set_packet_start = self.gc.PACKET3_SET_SH_REG, self.gc.PACKET3_SET_SH_REG_START
    elif self.gc.PACKET3_SET_UCONFIG_REG_START <= reg.addr < self.gc.PACKET3_SET_UCONFIG_REG_START + 2**16-1:
      set_packet, set_packet_start = self.gc.PACKET3_SET_UCONFIG_REG, self.gc.PACKET3_SET_UCONFIG_REG_START
    else: raise RuntimeError(f'Cannot set {reg.name} ({reg.addr}) via pm4 packet')
    self.pkt3(set_packet, reg.addr - set_packet_start, *(args or (reg.encode_bitfields(**kwargs),)))
  ...
  def wait_reg_mem(self, value, mask=0xffffffff, mem=None, reg_req=None, reg_done=None):
    wrm_info_dw = self.gc.WAIT_REG_MEM_MEM_SPACE(int(mem is not None)) | self.gc.WAIT_REG_MEM_OPERATION(int(mem is None)) \
                | self.gc.WAIT_REG_MEM_FUNCTION(WAIT_REG_MEM_FUNCTION_GEQ) | self.gc.WAIT_REG_MEM_ENGINE(0)

    self.pkt3(self.gc.PACKET3_WAIT_REG_MEM, wrm_info_dw, *(data64_le(mem) if mem is not None else (reg_req, reg_done)), value, mask, 4)

  def acquire_mem(self, addr=0x0, sz=(1 << 64)-1, gli=1, glm=1, glk=1, glv=1, gl1=1, gl2=1):
    cache_flags_dw = self.gc.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLI_INV(gli) \
                   | self.gc.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLM_INV(glm) | self.gc.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLM_WB(glm) \
                   | self.gc.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLK_INV(glk) | self.gc.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLK_WB(glk) \
                   | self.gc.PACKET3_ACQUIRE_MEM_GCR_CNTL_GLV_INV(glv) | self.gc.PACKET3_ACQUIRE_MEM_GCR_CNTL_GL1_INV(gl1) \
                   | self.gc.PACKET3_ACQUIRE_MEM_GCR_CNTL_GL2_INV(gl2) | self.gc.PACKET3_ACQUIRE_MEM_GCR_CNTL_GL2_WB(gl2)

    self.pkt3(self.gc.PACKET3_ACQUIRE_MEM, 0, *data64_le(sz), *data64_le(addr), 0, cache_flags_dw)

  def release_mem(self, address, value, data_sel, int_sel, ctxid=0, cache_flush=False):
    cache_flags_dw = 0 if not cache_flush else (self.gc.PACKET3_RELEASE_MEM_GCR_GLV_INV | self.gc.PACKET3_RELEASE_MEM_GCR_GL1_INV \
                   | self.gc.PACKET3_RELEASE_MEM_GCR_GL2_INV | self.gc.PACKET3_RELEASE_MEM_GCR_GLM_WB \
                   | self.gc.PACKET3_RELEASE_MEM_GCR_GLM_INV | self.gc.PACKET3_RELEASE_MEM_GCR_GL2_WB | self.gc.PACKET3_RELEASE_MEM_GCR_SEQ)

    event_dw = self.gc.PACKET3_RELEASE_MEM_EVENT_TYPE(self.gc.CACHE_FLUSH_AND_INV_TS_EVENT) \
             | self.gc.PACKET3_RELEASE_MEM_EVENT_INDEX(self.gc.event_index__mec_release_mem__end_of_pipe)

    memsel_dw = self.gc.PACKET3_RELEASE_MEM_DATA_SEL(data_sel) | self.gc.PACKET3_RELEASE_MEM_INT_SEL(int_sel) | self.gc.PACKET3_RELEASE_MEM_DST_SEL(0)

    self.pkt3(self.gc.PACKET3_RELEASE_MEM, event_dw | cache_flags_dw, memsel_dw, *data64_le(address), *data64_le(value), ctxid)

  def memory_barrier(self):
    self.wait_reg_mem(reg_req=self.nbio.regBIF_BX_PF0_GPU_HDP_FLUSH_REQ.addr, reg_done=self.nbio.regBIF_BX_PF0_GPU_HDP_FLUSH_DONE.addr, value=0xffffffff) # noqa: E501
    self.acquire_mem()
    return self
  ...
  def sqtt_config(self, tracing:bool):
    self.wreg(self.gc.regSQ_THREAD_TRACE_CTRL, draw_event_en=1, spi_stall_en=1, sq_stall_en=1, reg_at_hwm=2, hiwater=1,
              rt_freq=self.gc.SQ_TT_RT_FREQ_4096_CLK, util_timer=self.gc.SQ_TT_UTIL_TIMER_250_CLK, mode=int(tracing))
  ...
  def start_trace(self, buf0s:list[HCQBuffer], se_mask:int):
    ...
    for se in range(len(buf0s)):
      ...
      self.wreg(self.gc.regSQ_THREAD_TRACE_MASK, wtype_include=self.gc.SQ_TT_WTYPE_INCLUDE_CS_BIT, simd_sel=0, wgp_sel=0, sa_sel=0)
      REG_INCLUDE = self.gc.SQ_TT_TOKEN_MASK_SQDEC_BIT | self.gc.SQ_TT_TOKEN_MASK_SHDEC_BIT | self.gc.SQ_TT_TOKEN_MASK_GFXUDEC_BIT | \
                    self.gc.SQ_TT_TOKEN_MASK_COMP_BIT | self.gc.SQ_TT_TOKEN_MASK_CONTEXT_BIT | self.gc.SQ_TT_TOKEN_MASK_CONTEXT_BIT
      TOKEN_EXCLUDE = 1 << self.gc.SQ_TT_TOKEN_EXCLUDE_PERF_SHIFT
      if not (se_mask >> se) & 0b1:
        TOKEN_EXCLUDE |= 1 << self.gc.SQ_TT_TOKEN_EXCLUDE_VMEMEXEC_SHIFT | 1 << self.gc.SQ_TT_TOKEN_EXCLUDE_ALUEXEC_SHIFT | \
                         1 << self.gc.SQ_TT_TOKEN_EXCLUDE_VALUINST_SHIFT | 1 << self.gc.SQ_TT_TOKEN_EXCLUDE_IMMEDIATE_SHIFT | \
                         1 << self.gc.SQ_TT_TOKEN_EXCLUDE_INST_SHIFT
  ...
  def stop_trace(self, ses: int, wptrs: HCQBuffer):
    ...
    self.pkt3(self.gc.PACKET3_EVENT_WRITE, self.gc.EVENT_TYPE(self.gc.THREAD_TRACE_FINISH) | self.gc.EVENT_INDEX(0))
    ...
    for se in range(ses):
      ...
      self.pkt3(self.gc.PACKET3_WAIT_REG_MEM, self.gc.WAIT_REG_MEM_FUNCTION(WAIT_REG_MEM_FUNCTION_EQ),
                self.gc.regSQ_THREAD_TRACE_STATUS.addr, 0, 0, self.gc.SQ_THREAD_TRACE_STATUS__FINISH_PENDING_MASK, 4)
      # Wait for FINISH_DONE!=0
      self.pkt3(self.gc.PACKET3_WAIT_REG_MEM, self.gc.WAIT_REG_MEM_FUNCTION(WAIT_REG_MEM_FUNCTION_NEQ),
      ...
      self.pkt3(self.gc.PACKET3_COPY_DATA, 1 << 20 | 2 << 8 | 4, self.gc.regSQ_THREAD_TRACE_WPTR.addr, 0, *data64_le(wptrs.va_addr+(se*4)))
    # Restore global broadcasting
    self.wreg(self.gc.regGRBM_GFX_INDEX, se_broadcast_writes=1, sa_broadcast_writes=1, instance_broadcast_writes=1)
  ...
  def exec(self, prg:AMDProgram, args_state:CLikeArgsState, global_size:tuple[sint, ...], local_size:tuple[sint, ...]):
    ...
    self.wreg(self.gc.regCOMPUTE_PGM_RSRC3, 0)
    self.wreg(self.gc.regCOMPUTE_TMPRING_SIZE, prg.dev.tmpring_size)
    if prg.dev.has_scratch_base_registers: self.wreg(self.gc.regCOMPUTE_DISPATCH_SCRATCH_BASE_LO, *data64_le(prg.dev.scratch.va_addr >> 8))
    if prg.dev.target < 110000: self.wreg(self.gc.mmCP_COHER_START_DELAY, 0x20)
    ...
    DISPATCH_INITIATOR = self.gc.regCOMPUTE_DISPATCH_INITIATOR.encode_bitfields(cs_w32_en=1, force_start_at_000=1, compute_shader_en=1)
    self.pkt3(self.gc.PACKET3_DISPATCH_DIRECT, *global_size, DISPATCH_INITIATOR)
    if prg.dev.sqtt_enabled: self.pkt3(self.gc.PACKET3_EVENT_WRITE, self.gc.EVENT_TYPE(self.gc.THREAD_TRACE_MARKER) | self.gc.EVENT_INDEX(0))
    self.pkt3(self.gc.PACKET3_EVENT_WRITE, self.gc.EVENT_TYPE(self.gc.CS_PARTIAL_FLUSH) | self.gc.EVENT_INDEX(EVENT_INDEX_PARTIAL_FLUSH))
    return self
  ...
  def timestamp(self, signal:AMDSignal):
    self.release_mem(signal.timestamp_addr, 0, self.gc.data_sel__mec_release_mem__send_gpu_clock_counter, self.gc.int_sel__mec_release_mem__none)
    return self

  def signal(self, signal:AMDSignal, value:sint=0):
    # NOTE: this needs an EOP buffer on the queue or it will NULL pointer
    self.release_mem(signal.value_addr, value, self.gc.data_sel__mec_release_mem__send_32_bit_low,
                     self.gc.int_sel__mec_release_mem__send_interrupt_after_write_confirm, cache_flush=True)

    if not AMDDevice.driverless and (dev:=signal.timeline_for_device) is not None:
      self.release_mem(dev.queue_event_mailbox_ptr, dev.queue_event.event_id, self.gc.data_sel__mec_release_mem__send_32_bit_low,
                       self.gc.int_sel__mec_release_mem__send_interrupt_after_write_confirm, ctxid=dev.queue_event.event_id)
    return self
  ...
  def bind(self, dev:AMDDevice):
    ...
    self.indirect_cmd = [self.gc.PACKET3(self.gc.PACKET3_INDIRECT_BUFFER, 2), *data64_le(self.hw_page.va_addr),
                         len(self._q) | self.gc.INDIRECT_BUFFER_VALID]
    self._q = hw_view
    return self
...
class AMDIP:
  ...
  regs: dict[str, AMDReg] = field(init=False)
  def __post_init__(self):
    # TODO: Is object.__setattr__ really the "right" way to use field(init=False) on frozen dataclass ??!!??!
    object.__setattr__(self, 'module', importlib.import_module(f"tinygrad.runtime.autogen.am.{self.name}_{'_'.join(map(str, self.version))}"))
    object.__setattr__(self, 'regs', collect_registers(self.module, cls=functools.partial(AMDReg, ip=self)))
...
class KFDIface:
  ...
  def __init__(self, dev, device_id):
    ...
    self.ip_versions = {int(hwid):tuple(int(HWInterface(f'{ip_base}/{hwid}/0/{part}').read()) for part in ['major', 'minor', 'revision'])
                        for hwid in HWInterface(ip_base).listdir() if hwid.isnumeric()}
    self.ip_offsets = {int(hwid):tuple(int(x, 16) for x in HWInterface(f'{ip_base}/{hwid}/0/base_addr').read().splitlines())
                        for hwid in HWInterface(ip_base).listdir() if hwid.isnumeric()}
...
class PCIIface:
  ...
  def __init__(self, dev, dev_id):
    ...
    self.ip_versions = {hwip: (ver//10000, (ver//100)%100, ver%100) for hwip,ver in self.adev.ip_versions.items()}
...
class AMDDevice(HCQCompiled):
  ...
  def __init__(self, device:str=""):
    ...
    # Graphics & Compute. Used for compute queue packet definitions and asic register definitions
    gc_id = am.GC_HWIP if self.driverless else am.GC_HWID
    self.gc = AMDIP('gc', self.dev_iface.ip_versions[gc_id], self.dev_iface.ip_offsets[gc_id])
    # System Direct Memory Access. Used for copy queue packet definitions
    sdma_id = am.SDMA0_HWIP if self.driverless else am.SDMA0_HWID
    self.sdma = AMDIP('sdma', self.dev_iface.ip_versions[sdma_id], self.dev_iface.ip_offsets[sdma_id])
    # North bridge I/O. Used for asic register definitions (memory flush)
    nbio_id = am.NBIF_HWIP if self.driverless else am.NBIF_HWID
    self.nbio = AMDIP('nbio', self.dev_iface.ip_versions[nbio_id], self.dev_iface.ip_offsets[nbio_id])
  ...
  def invalidate_caches(self):
    AMDComputeQueue(self).memory_barrier().signal(self.timeline_signal, self.timeline_value).submit(self)
  ...
  def _at_profile_finalize(self):
    if self.sqtt_enabled:
      ...
      AMDComputeQueue(self).stop_trace(len(self.sqtt_buffers), wptrs_buf).signal(self.timeline_signal, self.timeline_value).submit(self)
      self.timeline_value += 1
      self.synchronize()
