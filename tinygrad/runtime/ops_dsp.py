...
dsp_pm = PatternMatcher([
  (
    ...
    lambda x: UOp(
      Ops.CUSTOM,
      dtypes.uchar.vec(128),
      src=tuple(x.gep(tuple(range(i, i + 32))) for i in range(0, 128, 32)),
  ...
  (
    UPat(Ops.GEP, name="x"),
    lambda x: UOp(Ops.CUSTOM, x.dtype, x.src + x.src, "__builtin_shufflevector({0}, {1}, " + ",".join([str(y) for y in x.arg]) + ")")
    if len(x.arg) > 1 and x.src[0].dtype.count > 1
    else None,
  ),
])
...
dsp_pm_late = PatternMatcher([
  (
    ...
    lambda x, y: x + UOp(Ops.CUSTOMI, x.dtype, (y,), arg="{0}") if x.op is not Ops.CUSTOMI else None,
  ),
  (
    UPat.var("x") * UPat(Ops.VECTORIZE, src=UPat.var("y")),
    lambda x, y: x * UOp(Ops.CUSTOMI, x.dtype, (y,), arg="{0}") if x.op is not Ops.CUSTOMI else None,
...
class DSPRenderer(ClangRenderer):
  ...
  def _render_entry(self, function_name: str, bufs: list[tuple[str, tuple[DType, bool]]]) -> str:
    ...
    msrc += [
      f"void *buf_{i} = HAP_mmap(0,sz_or_val_{i},3,0,pra[{i + 3}].dma.fd,0)+off{i};" for i, b in enumerate(bufs) if isinstance(b[1][0], PtrDType)
    ]
...
def rpc_prep_args(ins=None, outs=None, in_fds=None):
  ...
  for i, mv in enumerate(ins + outs):
    pra[i].buf.pv, pra[i].buf.len = mv_address(mv) if mv.nbytes > 0 else 0, mv.nbytes
...
class DSPProgram:
  ...
  def __call__(self, *bufs, vals: tuple[int, ...] = (), wait=False):
    ...
    pra, fds, attrs, _ = rpc_prep_args(
      ins=[var_vals_mv := memoryview(bytearray((len(bufs) + len(vals)) * 4)), off_mv := memoryview(bytearray(len(bufs) * 4))],
...
class DSPAllocator(Allocator["DSPDevice"]):
  ...
  def _copyin(self, dest: DSPBuffer, src: memoryview):
    ctypes.memmove(dest.va_addr, mv_address(src), src.nbytes)

  def _copyout(self, dest: memoryview, src: DSPBuffer):
    ctypes.memmove(mv_address(dest), src.va_addr, dest.nbytes)

  def _offset(self, buf, size: int, offset: int):
    return DSPBuffer(buf.va_addr + offset, size, buf.share_info, buf.offset + offset)
...
class DSPDevice(Compiled):
  def __init__(self, device: str = ""):
    ...
    if getenv("MOCKDSP"):
      super().__init__(
        device, CPUAllocator(self), MockDSPRenderer(), ClangCompiler(None, ["-static"] + compiler_args, "llvm-objdump"), MockDSPProgram
      )
    else:
      ...
      super().__init__(
        ...
        DSPAllocator(self),
        DSPRenderer(),
        ClangCompiler("compile_dsp", ["-shared"] + compiler_args + [f"-T{self.link_ld.name}"], "llvm-objdump"),
  ...
  def open_lib(self, lib):
    ...
    pra, _, _, _ = rpc_prep_args(
      ins=[memoryview(array.array("I", [len(fp), 0xFF])), memoryview(bytearray(fp.encode()))],
...
class RPCListener(threading.Thread):
  ...
  def run(self):
    ...
    req_args, _, _, _ = rpc_prep_args(
      ins=[msg_send := memoryview(bytearray(0x10)).cast("I"), out_buf := memoryview(bytearray(0x10000)).cast("I")],
    ...
    while True:
      ...
      if sc == 0x20200:
        ...
        try:
          out_args[0].cast("I")[0] = TINYFD if (name := in_args[3].tobytes()[:-1].decode()) == "tinylib" else os.open(name, os.O_RDONLY)
        except OSError:
          status = 1
      elif sc == 0x3010000:
...
class MockDSPRenderer(DSPRenderer):
  def _render_defines(self, uops) -> list[str]:
    return ClangRenderer._render_defines(self, uops)
    ...
    with tempfile.NamedTemporaryFile(suffix=".out") as dsp_lib:
      ...
      proc = subprocess.run(
        [
          ...
          "qemu-hexagon",
          "-c",
          f"qemu-hexagon {'-strace' if DEBUG >= 5 else ''} /work/" + os.path.basename(dsp_lib.name),
        ],
        input=b"".join([bytes(to_mv(x.va_addr, x.size)) for x in bufs] + [struct.pack("I", x) for x in vals]),
