...
if getenv("IOCTL"):
    import extra.dsp.run  # noqa: F401 # pylint: disable=unused-import
...
dsp_pm = PatternMatcher(
    [
        (
            UPat(Ops.VECTORIZE, src=UPat.var("y")) * UPat.var("x"),
            lambda x, y: UOp(Ops.CUSTOM, x.dtype, (y,), arg="{0}") * x,
        ),
        (
            UPat(
                Ops.DEFINE_ACC,
                src=(UPat(Ops.VECTORIZE, src=UPat(Ops.CONST, arg=0)),),
                dtype=dtypes.uchar.vec(128),
                name="d",
                allow_any_len=True,
            ),
            lambda d: d.replace(
                src=(UOp(Ops.CUSTOM, d.dtype, arg="__builtin_HEXAGON_V6_vd0_128B()"),)
                + d.src[1:]
            ),
        ),
    ]
...
class DSPRenderer(ClangRenderer):
    ...
    type_map = {
        **ClangRenderer.type_map,
        dtypes.uint64: "unsigned long long",
        dtypes.int64: "long long",
    }
    code_for_op = {
        **ClangRenderer.code_for_op,
        Ops.SIN: lambda x, dtype: f"__builtin_sin({x})",
        Ops.LOG2: lambda x, dtype: (
            f"__builtin_log2l({x})"
            if dtype == dtypes.float64
            else f"__builtin_log2f({x})"
        ),
        Ops.EXP2: lambda x, dtype: (
            f"__builtin_exp2l({x})"
            if dtype == dtypes.float64
            else f"__builtin_exp2f({x})"
        ),
    }
    ...
    def render_kernel(
      ...
      _Bool set_dcvs_params; short _pad2; char target_corner; char min_corner; char max_corner; int _pad3[3];};""",
        ...
        msrc += [
            f"int sz_or_val_{i} = ((int*)pra[0].buf.pv)[{i}];"
            for i, b in enumerate(bufs)
        ]
        msrc += [
            f"int off{i} = ((int*)pra[1].buf.pv)[{i}];"
            for i, b in enumerate(bufs)
            if isinstance(b[1][0], PtrDType)
        ]
        msrc += [
            f"void *buf_{i} = HAP_mmap(0,sz_or_val_{i},3,0,pra[{i+3}].dma.fd,0)+off{i};"
            for i, b in enumerate(bufs)
            if isinstance(b[1][0], PtrDType)
        ]
        msrc += ["unsigned long long start = HAP_perf_get_time_us();"]
        msrc += [
            f"{function_name}({', '.join([(f'buf_{i}' if isinstance(b[1][0], PtrDType) else f'sz_or_val_{i}') for i,b in enumerate(bufs)])});"
        ]
        msrc += [
            "*(unsigned long long *)(pra[2].buf.pv) = HAP_perf_get_time_us() - start;"
        ]
        msrc += [
            f"HAP_munmap(buf_{i}, sz_or_val_{i});"
            for i, b in enumerate(bufs)
            if isinstance(b[1][0], PtrDType)
        ]
        msrc += ["return 0; }"]
...
def rpc_sc(method=0, ins=0, outs=0, fds=0):
    return (method << 24) | (ins << 16) | (outs << 8) | fds
...
def rpc_prep_args(ins=None, outs=None, in_fds=None):
    ...
    fds = (ctypes.c_int32 * (len(ins) + len(outs) + len(in_fds)))(
        *([-1] * (len(ins) + len(outs))), *in_fds
    )
    attrs = (ctypes.c_uint32 * (len(ins) + len(outs) + len(in_fds)))(
        *([0] * (len(ins) + len(outs))), *([1] * (len(in_fds)))
    )

    for i, mv in enumerate(ins + outs):
        pra[i].buf.pv, pra[i].buf.len = (
            mv_address(mv) if mv.nbytes > 0 else 0
        ), mv.nbytes
...
class DSPProgram:
    ...
    def __call__(self, *bufs, vals: tuple[int, ...] = (), wait=False):
        if len(bufs) >= 16:
            raise RuntimeError(f"Too many buffers to execute: {len(bufs)}")

        pra, fds, attrs, _ = rpc_prep_args(
            ins=[
                var_vals_mv := memoryview(bytearray((len(bufs) + len(vals)) * 4)),
                off_mv := memoryview(bytearray(len(bufs) * 4)),
            ],
            outs=[timer := memoryview(bytearray(8)).cast("Q")],
            in_fds=[b.share_info.fd for b in bufs],
        )
        var_vals_mv.cast("i")[:] = array.array("i", tuple(b.size for b in bufs) + vals)
        off_mv.cast("I")[:] = array.array("I", tuple(b.offset for b in bufs))
        self.dev.exec_lib(
            self.lib, rpc_sc(method=2, ins=2, outs=1, fds=len(bufs)), pra, fds, attrs
        )
        return timer[0] / 1e6
...
class DSPBuffer:
    def __init__(self, va_addr: int, size: int, share_info, offset: int = 0):
        self.va_addr, self.size, self.share_info, self.offset = (
            va_addr,
            size,
            share_info,
            offset,
...
class DSPAllocator(Allocator):
    ...
    def _alloc(self, size: int, options: BufferSpec):
        b = qcom_dsp.ION_IOC_ALLOC(
            self.dev.ion_fd,
            len=size,
            align=0x200,
            heap_id_mask=1 << qcom_dsp.ION_SYSTEM_HEAP_ID,
            flags=qcom_dsp.ION_FLAG_CACHED,
        )
        share_info = qcom_dsp.ION_IOC_SHARE(self.dev.ion_fd, handle=b.handle)
        va_addr = libc.mmap(
            0, size, mmap.PROT_READ | mmap.PROT_WRITE, mmap.MAP_SHARED, share_info.fd, 0
        )
    ...
    def _as_buffer(self, src: DSPBuffer) -> memoryview:
        return to_mv(src.va_addr, src.size)

    def _copyin(self, dest: DSPBuffer, src: memoryview):
        ctypes.memmove(dest.va_addr, from_mv(src), src.nbytes)

    def _copyout(self, dest: memoryview, src: DSPBuffer):
        ctypes.memmove(from_mv(dest), src.va_addr, dest.nbytes)

    def _offset(self, buf, size: int, offset: int):
        return DSPBuffer(
            buf.va_addr + offset, size, buf.share_info, buf.offset + offset
        )
...
class ClangCompiler(Compiler):
    ...
    def disassemble(self, lib: bytes):
        return cpu_objdump(lib, self.objdump_tool)
...
class DSPDevice(Compiled):
    def __init__(self, device: str = ""):
        ...
        try:
            ...
            super().__init__(
                ...
                ClangCompiler(
                    "compile_dsp",
                    ["-shared"] + compiler_args + [f"-T{self.link_ld.name}"],
                    "llvm-objdump",
                ),
                functools.partial(DSPProgram, self),
            )
            fastrpc_shell = memoryview(
                bytearray(pathlib.Path("/dsp/cdsp/fastrpc_shell_3").read_bytes())
            )
            self.shell_buf = self.allocator.alloc(
                round_up(fastrpc_shell.nbytes, 0x1000), BufferSpec(nolru=True)
            )
            ctypes.memmove(
                self.shell_buf.va_addr, mv_address(fastrpc_shell), fastrpc_shell.nbytes
            )
            ...
            super().__init__(
                ...
                MallocAllocator,
                MockDSPRenderer(),
                ClangCompiler(None, ["-static"] + compiler_args, "llvm-objdump"),
                MockDSPProgram,
            )
    ...
    def open_lib(self, lib):
        ...
        pra, _, _, _ = rpc_prep_args(
            ins=[
                memoryview(array.array("I", [len(fp), 0xFF])),
                memoryview(bytearray(fp.encode())),
            ],
            outs=[o1 := memoryview(bytearray(0x8)), o2 := memoryview(bytearray(0xFF))],
        )
        qcom_dsp.FASTRPC_IOCTL_INVOKE(
            self.rpc_fd, handle=0, sc=rpc_sc(method=0, ins=2, outs=2), pra=pra
        )
        if o1.cast("i")[1] < 0:
            raise RuntimeError(f"Cannot open lib: {o2.tobytes().decode()}")
        return o1.cast("I")[0]

    def close_lib(self, handle):
        pra, _, _, _ = rpc_prep_args(
            ins=[memoryview(array.array("I", [handle, 0xFF]))],
            outs=[memoryview(bytearray(0x8)), memoryview(bytearray(0xFF))],
        )
        qcom_dsp.FASTRPC_IOCTL_INVOKE(
            self.rpc_fd, handle=0, sc=rpc_sc(method=1, ins=1, outs=2), pra=pra
        )

    def exec_lib(self, lib, sc, args, fds, attrs):
        def _exec_lib():
            handle = self.open_lib(lib)
            qcom_dsp.FASTRPC_IOCTL_INVOKE_ATTRS(
                self.rpc_fd,
                fds=fds,
                attrs=attrs,
                inv=qcom_dsp.struct_fastrpc_ioctl_invoke(
                    handle=handle, sc=sc, pra=args
                ),
            )
    ...
    def init_dsp(self):
        if hasattr(self, "rpc_fd"):
            with contextlib.suppress(OSError):
                qcom_dsp.FASTRPC_IOCTL_INVOKE(
                    self.rpc_fd, handle=4, sc=rpc_sc(method=2, ins=0, outs=0)
        ...
        qcom_dsp.FASTRPC_IOCTL_INIT(
            self.rpc_fd,
            flags=0x1,
            file=self.shell_buf.va_addr,
            filelen=self.shell_buf.size,
            filefd=self.shell_buf.share_info.fd,
        )
        qcom_dsp.FASTRPC_IOCTL_INVOKE(
            self.rpc_fd, handle=3, sc=rpc_sc(method=3, ins=0, outs=0)
        )
...
class RPCListener(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            msg_send[:] = array.array(
                "I", [context, status, req_args[1].buf.len, in_buf.nbytes]
            )

            try:
                qcom_dsp.FASTRPC_IOCTL_INVOKE(
                    self.device.rpc_fd, handle=0x3, sc=0x04020200, pra=req_args
                )
            except OSError:
                continue  # retry

            context, inbufs, outbufs = (
                msg_recv[0],
                ((sc := msg_recv[2]) >> 16) & 0xFF,
                (msg_recv[2] >> 8) & 0xFF,
            )
            ...
            for i in range(inbufs + outbufs):
                obj_ptr = (
                    round_up(in_ptr + 4, 8) if i < inbufs else round_up(out_ptr + 4, 8)
                )
                objs.append(to_mv(obj_ptr, obj_size := to_mv(in_ptr, 4).cast("I")[0]))
                if i < inbufs:
                    in_ptr = obj_ptr + obj_size
                else:
            ...
            if sc == 0x20200:
                ...
                try:
                    out_args[0].cast("I")[0] = (
                        TINYFD
                        if (name := in_args[3].tobytes()[:-1].decode()) == "tinylib"
                        else os.open(name, os.O_RDONLY)
                    )
                except OSError:
                    status = 1
            elif sc == 0x3010000:
                ...
                if (fd := in_args[0].cast("I")[0]) == TINYFD:
                    assert (
                        in_args[0].cast("I")[2] == qcom_dsp.APPS_STD_SEEK_SET
                    ), "Supported only SEEK_SET"
                    res, self.device.binded_lib_off = 0, in_args[0].cast("I")[1]
                else:
                    res = os.lseek(fd, in_args[0].cast("I")[1], in_args[0].cast("I")[2])
                status = 0 if res >= 0 else res
            elif sc == 0x4010200:  # read
                if (fd := in_args[0].cast("I")[0]) == TINYFD:
                    buf = self.device.binded_lib[
                        self.device.binded_lib_off : self.device.binded_lib_off
                        + in_args[0].cast("I")[1]
                    ]
                    self.device.binded_lib_off += len(buf)
                else:
                    buf = os.read(fd, in_args[0].cast("I")[1])
                out_args[1][: len(buf)] = buf
                out_args[0].cast("I")[0:2] = array.array(
                    "I", [len(buf), int(len(buf) == 0)]
                )
            elif sc == 0x1F020100:  # stat
                stat = os.stat(in_args[1].tobytes()[:-1].decode())
                out_stat = qcom_dsp.struct_apps_std_STAT.from_address(
                    mv_address(out_args[0])
                )
                for f in out_stat._fields_:
                    out_stat.__setattr__(f[0], int(getattr(stat, f"st_{f[0]}", 0)))
            elif sc == 0x2010100:  # mmap
                st = qcom_dsp.FASTRPC_IOCTL_MMAP(
                    self.device.rpc_fd,
                    fd=-1,
                    flags=in_args[0].cast("I")[2],
                    vaddrin=0,
                    size=in_args[0].cast("Q")[3],
                )
                out_args[0].cast("Q")[0:2] = array.array("Q", [0, st.vaddrout])
            else:
                raise RuntimeError(f"Unknown op: {sc=:X}")
...
class MockDSPRenderer(DSPRenderer):
    def render_kernel(
        ...
        ret = ClangRenderer.render_kernel(
            self, function_name, kernel, bufs, uops, prefix
        )
        # https://gpages.juszkiewicz.com.pl/syscalls-table/syscalls.html
        # control register 21 is HEX_REG_QEMU_INSN_CNT, 0x6a15c000 loads it
        msrc = [
            """/* DSP boilerplate */ static long syscall(long r0, long r1, long r2, long r3, long r4, long r5, long r6) {
      ...
      static void *mmap2(void *addr, unsigned int length, int prot, int flags, int fd, unsigned long offset) {{
        ...
        for i, b in enumerate(bufs):
            if isinstance(b[1][0], PtrDType):
                ...
                msrc.append(
                    f"void *buf{i} = mmap2(0, {sz}, 3, 0x21, -1, 0); for(int rd = 0; rd < {sz}; rd += read(0, buf{i}+rd, {sz}-rd));"
                )
            else:
                msrc.append(f"unsigned int val{i}; read(0, &val{i}, 4);")
        msrc.append("unsigned int st = inscount();")
        msrc.append(
            f"{function_name}({', '.join([(f'(void*)buf{i}' if isinstance(b[1][0], PtrDType) else f'val{i}') for i,b in enumerate(bufs)])});"
        )
        msrc.append("unsigned int et = inscount() - st; write(1, &et, sizeof(et));")
        for i, b in enumerate(bufs):
            if isinstance(b[1][0], PtrDType):
                msrc.append(f"write(1, buf{i}, {b[1][0].size*b[1][0].itemsize});")
        msrc.append("exit(0); }")
        return ret + "\n" + "\n".join(msrc)
...
class MockDSPProgram:
    ...
    def __call__(self, *bufs, vals: tuple[int, ...] = (), wait=False):
        with tempfile.NamedTemporaryFile(suffix=".out") as dsp_lib:
            ...
            proc = subprocess.run(
                [
                    ...
                    "-i",
                    "-v",
                    f"{os.path.abspath(os.path.dirname(dsp_lib.name))}:/work",
                    "-w",
                    "/work",
                    "qemu-hexagon",
                    "-c",
                    f"qemu-hexagon {'-strace' if DEBUG >= 5 else ''} /work/"
                    + os.path.basename(dsp_lib.name),
                ],
                input=b"".join(
                    [bytes(x) for x in bufs] + [struct.pack("I", x) for x in vals]
                ),
        ...
        return (
            struct.unpack("I", proc.stdout[0:4])[0] / 1e9
        )  # pretend it's 1 Ghz, but this is an inscount, not a time
