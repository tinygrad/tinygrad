...
if MOCKGPU := getenv("MOCKGPU"):
    from test.mockgpu.cuda import cuda  # type: ignore # pylint: disable=reimported
...
def encode_args(args, vals) -> tuple[ctypes.Structure, ctypes.Array]:
    c_args = init_c_struct_t(
        tuple(
            [(f"f{i}", cuda.CUdeviceptr_v2) for i in range(len(args))]
            + [(f"v{i}", ctypes.c_int) for i in range(len(vals))]
        )
    )(*args, *vals)
    vargs = (ctypes.c_void_p * 5)(
        ctypes.c_void_p(1),
        ctypes.cast(ctypes.byref(c_args), ctypes.c_void_p),
        ctypes.c_void_p(2),
        ctypes.cast(
...
def cu_time_execution(cb, enable=False) -> float | None:
    if not enable:
        return cb()
    evs = [
        init_c_var(cuda.CUevent(), lambda x: cuda.cuEventCreate(ctypes.byref(x), 0))
        for _ in range(2)
    ]
...
class CUDAProgram:
    def __init__(self, dev: CUDADevice, name: str, lib: bytes, smem: int = 0):
                ...
                "\n".join(
                    [
                        ...
                        for i, line in enumerate(
                            pretty_ptx(lib.decode("utf-8")).split("\n")
                        )
                    ]
                )
            )
        ...
        if status != 0:
            ...
            cuda_disassemble(lib, dev.arch)
            raise RuntimeError(
                f"module load failed with status code {status}: {cuda.cudaError_enum__enumvalues[status]}"
            )
        check(
    ...
    def __call__(
        ...
        *args,
        ...
        if not hasattr(self, "vargs"):
            ...
            if MOCKGPU:
                self.vargs = self.c_args  # type: ignore[assignment]
        else:
            for i in range(len(args)):
                self.c_args.__setattr__(f"f{i}", args[i])
            for i in range(len(vals)):
                self.c_args.__setattr__(f"v{i}", vals[i])
...
class CUDAAllocator(LRUAllocator):
    ...
    def _alloc(self, size, options: BufferSpec):
        ...
        if options.host:
            return init_c_var(
                ctypes.c_void_p(),
                lambda x: check(cuda.cuMemHostAlloc(ctypes.byref(x), size, 0x01)),
            )
        return init_c_var(
            cuda.CUdeviceptr(),
    ...
    def _free(self, opaque, options: BufferSpec):
        if options.host:
            check(cuda.cuMemFreeHost(opaque))
        else:
    ...
    def _transfer(self, dest, src, sz: int, src_dev, dest_dev):
        ...
        check(
            cuda.cuStreamWaitEvent(None, sync_event, 0)
        )  # sync the default stream on the dest dev

    def _offset(self, buf, size: int, offset: int):
        return cuda.CUdeviceptr_v2(buf.value + offset)
...
class CUDADevice(Compiled):
    ...
    def __init__(self, device: str):
        ...
        check(cuda.cuInit(0))
        self.cu_device = init_c_var(
            cuda.CUdevice(),
            lambda x: check(cuda.cuDeviceGet(ctypes.byref(x), device_id)),
        )
        self.context = init_c_var(
            cuda.CUcontext(),
            lambda x: check(cuda.cuCtxCreate_v2(ctypes.byref(x), 0, self.cu_device)),
        )
        check(
        ...
        for dev in CUDADevice.devices:
            check(
                cuda.cuDeviceCanAccessPeer(
                    ctypes.byref(val := ctypes.c_int()), self.cu_device, dev.cu_device
                )
            )
            if val.value != 1:
                continue
        ...
        super().__init__(
            device,
            CUDAAllocator(self),
            PTXRenderer(self.arch) if PTX else CUDARenderer(self.arch),
            PTXCompiler(self.arch) if PTX else CUDACompiler(self.arch),
            functools.partial(CUDAProgram, self),
            None if MOCKGPU else CUDAGraph,
        )

    def synchronize(self):
        check(cuda.cuCtxSetCurrent(self.context))
        check(cuda.cuCtxSynchronize())
        for opaque, sz, options in self.pending_copyin:
            self.allocator.free(opaque, sz, options)
