...
class CUDADevice(Compiled):
  def __init__(self, device:str):
    ...
    super().__init__(CUDAAllocator(self) if not CUDACPU else MallocAllocator,
                     ...
                     CUDARenderer, functools.partial(compile_cuda, self), f"compile_cuda_{self.arch}", functools.partial(CUDAProgram, self), graph=CUDAGraph if not CUDACPU else None)
