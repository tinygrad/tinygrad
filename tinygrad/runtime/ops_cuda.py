...
class CUDAProgram:
  def __init__(self, name:str, prg:str, binary=False):
    if not binary:
      try: prg = cuda_compile(prg, target="ptx", no_extern_c=True, options=['-Wno-deprecated-gpu-targets'])
      except cuda.CompileError as e:
        if DEBUG >= 3: print("FAILED TO BUILD", prg)
        raise e
    if DEBUG >= 5: print(pretty_ptx(prg.decode('utf-8') if isinstance(prg, bytes) else prg))
    if DEBUG >= 6:
      try:
