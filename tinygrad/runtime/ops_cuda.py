...
def encode_args(args, vals) -> tuple[ctypes.Structure, ctypes.Array]:
  c_args = init_c_struct_t(tuple([(f"f{i}", cuda.CUdeviceptr_v2) for i in range(len(args))] + [(f"v{i}", ctypes.c_int) for i in range(len(vals))]))(
    *args, *vals
  )
  vargs = (ctypes.c_void_p * 5)(
    ...
    ctypes.c_void_p(2),
    ctypes.cast(ctypes.pointer(ctypes.c_size_t(ctypes.sizeof(c_args))), ctypes.c_void_p),
    ctypes.c_void_p(0),
  )
  return c_args, vargs
...
class CUDAProgram:
  def __init__(self, dev: CUDADevice, name: str, lib: bytes, smem: int = 0):
    ...
    self.prg = prg
    if self.smem > 0:
      check(cuda.cuFuncSetAttribute(self.prg, cuda.CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, self.smem))
...
class CUDAAllocator(LRUAllocator["CUDADevice"]):
  def _alloc(self, size, options: BufferSpec):
    ...
    if options.external_ptr:
      return cuda.CUdeviceptr_v2(options.external_ptr)
    if options.host:
      return init_c_var(ctypes.c_void_p(), lambda x: check(cuda.cuMemHostAlloc(ctypes.byref(x), size, 0x01)))
