...
renderer = functools.partial(uops_to_cstyle, CStyleLanguage(
  ...
  half_prekernel = """
    ...
    """)) if not getenv("PTX") and not getenv("TRITON") else fromimport("tinygrad.renderer.assembly_ptx", "uops_to_ptx_asm") if not getenv("TRITON") else fromimport("tinygrad.renderer.triton", "uops_to_triton")
CUDABuffer = Compiled(RawCUDABuffer, LinearizerOptions(supports_float4=False if getenv("PTX") or getenv("TRITON") else True, supports_float4_alu=False, global_max = [65535, 65535, 2147483647], local_max = [64, 1024, 1024], has_shared=not getenv("TRITON")), renderer, CUDAProgram, cuda.Context.synchronize)