...
def encode_args(args, vals, signature) -> tuple[ctypes.Structure, ctypes.Array]:
  fields = ([(f'f{i}', cuda.CUdeviceptr_v2, i*8) for i,_ in enumerate(TinyELF.zip_bufs(signature, args))] +
            [(f'v{i}', getattr(ctypes, f"c_int{a.dtype.bitsize}"), off)
            for i, (_, off, a, _) in enumerate(TinyELF.zip_vals(signature, vals, len(args)*8))])
