...
from tinygrad.device import Compiled, BufferSpec, LRUAllocator, RendererList
...
class CUDADevice(Compiled):
  ...
  def __init__(self, device:str):
    ...
    compilers:RendererList = [(functools.partial(CUDARenderer, self.arch, device="CUDA"), None),
                              (functools.partial(PTXRenderer, self.arch, device="CUDA"), CUDA_PTX),
                              (functools.partial(CUDARenderer, self.arch, device="CUDA", use_nvcc=True), CUDA_NVCC)]
    super().__init__(device, CUDAAllocator(self), compilers, functools.partial(CUDAProgram, self), None if MOCKGPU else CUDAGraph, ctrl_var=CUDA_CC)
