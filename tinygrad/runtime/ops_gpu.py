    ...
    def __call__(
        ...
        *bufs: tuple[ctypes._CData, BufferSpec],
        ...
        for i, v in enumerate(vals, start=len(bufs)):
            cl.clSetKernelArg(self.kernel, i, 4, ctypes.byref(ctypes.c_int32(v)))
            ...
            return (
                checked(
                    cl.clCreateImage2D(
                        ...
                        cl.cl_image_format(
                            cl.CL_RGBA,
                            {2: cl.CL_HALF_FLOAT, 4: cl.CL_FLOAT}[
                                options.image.itemsize
                            ],
                        ),
        ...
        if dest[1].image is not None:
            check(
                cl.clEnqueueWriteImage(
                    ...
                    (ctypes.c_size_t * 3)(
                        dest[1].image.shape[1], dest[1].image.shape[0], 1
                    ),
                    0,
                    0,
                    from_mv(src),
        ...
        if src[1].image is not None:
            check(
                cl.clEnqueueReadImage(
                    ...
                    (ctypes.c_size_t * 3)(0, 0, 0),
                    (ctypes.c_size_t * 3)(
                        src[1].image.shape[1], src[1].image.shape[0], 1
                    ),
                    0,
                    0,
                    from_mv(dest),
...
class CLDevice(Compiled):
    ...
    def __init__(self, device: str = ""):
        ...
        compile_key = hashlib.md5(
            self.device_name.encode() + self.driver_version.encode()
        ).hexdigest()
        renderer = (
            IntelRenderer()
        ...
        super().__init__(
            ...
            CLAllocator(self),
            renderer,
            CLCompiler(self, f"compile_cl_{compile_key}"),
