...
class _CL:
  def __init__(self):
    platforms: List[List[cl.Device]] = [x.get_devices(device_type=cl.device_type.GPU) for x in cl.get_platforms()] + [x.get_devices(device_type=cl.device_type.CPU) for x in cl.get_platforms()]
    if len(platforms) > 1 or DEBUG >= 1: print(f"using {platforms[getenv('CL_PLATFORM', 0)]}")
