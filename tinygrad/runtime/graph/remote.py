...
def _dev_key(dev:RemoteDevice): return dev.conn if dev.properties.graph_supports_multi else dev
def _map_rawbuf(rawbuf:Buffer): return (cast(RemoteDevice, Device[rawbuf.device]).session, rawbuf._buf)
...
class RemoteGraph(MultiGraphRunner):
  def __init__(self, jit_cache: list[ExecItem], rawbufs: list[Buffer], var_vals: dict[Variable, int]):
    ...
    self.handle_indexes = {_map_rawbuf(rawbufs[i]): i for i in sorted(dedup(self.input_replace.values()))}
      ...
      for dev in devices:
        ...
        match cur_staging_type:
          case StagingType.GRAPH:
            bufs = [_map_rawbuf(rawbufs[i]) for i in sorted(dedup(self.input_replace.values())) if _dev_key(Device[rawbufs[i].device]) == dk]
          ...
          case StagingType.TRANSFER:
            for host in dedup(t.dsession.host for t in staging):
              sbuffer_nums = [(t.session, t.buffer_num) for t in staging if t.dsession.host == host]
              dbuffer_nums = [(t.dsession, t.dbuffer_num) for t in staging if t.dsession.host == host]
    ...
    for ji in jit_cache:
      match ji.prg:
        case CompiledRunner():
          ...
          stagings[_dev_key(ji.prg.dev)].append(gi)
        case BufferXfer():
          dest, src = ji.bufs[0:2]
          dest_dev, src_dev = cast(RemoteDevice, Device[dest.device]), cast(RemoteDevice, Device[src.device])
          ...
          if _dev_key(dest_dev) == _dev_key(src_dev):
            _flush(StagingType.GRAPH)
            stagings[_dev_key(src_dev)].append(ti)
  ...
  def __del__(self):
    for req in self.template:
      match req:
        case GraphExec(): RemoteConnection(req.session.host).q(GraphFree(req.graph_num, session=req.session))
  def __call__(self, rawbufs: list[Buffer], var_vals: dict[Variable, int], wait=False):
    if wait: st = time.perf_counter()
    rmap = {orig: _map_rawbuf(rawbufs[replace_idx]) for orig,replace_idx in self.handle_indexes.items()}
    for req in self.template:
      match req:
        case GraphExec():
      ...
      RemoteConnection(req.session.host).q(req)
    if wait:
      RemoteConnection(req.session.host).batch_submit()
