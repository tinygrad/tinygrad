...
from tinygrad.helpers import init_c_var, init_arc_c_var
...
class HIPGraph(CUDAGraph):
  ...
  def graph_create(self): return init_arc_c_var(hip.hipGraph_t(), lambda x: check(hip.hipGraphCreate(ctypes.byref(x), 0)), hip.hipGraphDestroy)
  def graph_instantiate(self, graph): return init_arc_c_var(hip.hipGraphExec_t(), lambda x: check(hip.hipGraphInstantiate(ctypes.byref(x), graph, None, None, 0)), hip.hipGraphExecDestroy)
