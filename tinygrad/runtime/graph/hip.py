...
from tinygrad.helpers import init_ctypes_var, init_arc_ctypes_var
...
class HIPGraph(CUDAGraph):
  ...
  def graph_create(self): return init_arc_ctypes_var(hip.hipGraph_t(), lambda x: check(hip.hipGraphCreate(ctypes.byref(x), 0)), hip.hipGraphDestroy)
  def graph_instantiate(self, graph): return init_arc_ctypes_var(hip.hipGraphExec_t(), lambda x: check(hip.hipGraphInstantiate(ctypes.byref(x), graph, None, None, 0)), hip.hipGraphExecDestroy)
  def graph_add_kernel_node(self, graph, c_deps, c_params): return init_ctypes_var(hip.hipGraphNode_t(), lambda x: check(hip.hipGraphAddKernelNode(ctypes.byref(x), graph, c_deps, ctypes.sizeof(c_deps)//8 if c_deps else 0, ctypes.byref(c_params))))
