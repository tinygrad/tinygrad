...
from tinygrad.engine.jit import get_input_replace, get_jit_stats, get_jc_idxs_with_updatable_launch_dims, GraphRunner
  ...
  def __init__(self, jit_cache: List[ExecItem], input_rawbuffers: List[Buffer]):
    ...
    for j,ji in enumerate(self.jit_cache):
      ...
      for i,v in enumerate(prg.vars):
        icb_command.setKernelBuffer_offset_atIndex_(self.int_buf, self.vars.index(v)*4, len(ji.bufs)+i)
