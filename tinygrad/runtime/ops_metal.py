...
class MetalDevice(Compiled):
    def __init__(self, device: str):
        ...
        self.mtl_queue = msg(
            "newCommandQueueWithMaxCommandBufferCount:", objc_instance
        )(self.sysdevice, 1024)
        ...
        super().__init__(
            ...
            MetalAllocator(self),
            MetalRenderer(),
            MetalCompiler() if getenv("METAL_DIRECT", 1) else Compiler(),
        ...
        for cbuf in self.mtl_buffers_in_flight:
            ...
            if PROFILE and (lb := cmdbuf_label(cbuf)) is not None:
                Compiled.profile_events += [
                    ProfileRangeEvent(
                        self.device, lb, st, en, is_copy=lb.startswith("COPY")
                    )
                ]
...
class MetalCompiler(Compiler):
    ...
    def compile(self, src: str) -> bytes:
        ret: Union[Exception, bytes] = CompileError(
...
class MetalProgram:
    def __init__(self, dev: MetalDevice, name: str, lib: bytes):
        ...
        self.max_total_threads: int = cast(
            int,
            msg("maxTotalThreadsPerThreadgroup", ctypes.c_ulong)(self.pipeline_state),
        )
    ...
    def __call__(
        ...
        *bufs,
        ...
        if prod(local_size) > self.max_total_threads:
            exec_width = msg("threadExecutionWidth", ctypes.c_ulong)(
                self.pipeline_state
            )
...
class MetalAllocator(LRUAllocator):
    ...
    def _transfer(
        ...
        if src_dev != dest_dev:
            ...
            dest_command_buffer = msg("commandBuffer", objc_instance)(
                dest_dev.mtl_queue
            )
    ...
    def _copyin(self, dest: MetalBuffer, src: memoryview):
        self._cp_mv(self._as_buffer(dest), src, "CPU -> METAL")

    def _copyout(self, dest: memoryview, src: MetalBuffer):
        self._cp_mv(dest, self._as_buffer(src), "METAL -> CPU")
