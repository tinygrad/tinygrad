...
class MetalProgram:
  def __init__(self, device:MetalDevice, name:str, lib:bytes):
    ...
    if DEBUG >= 6:
      applegpu_path = pathlib.Path(__file__).parents[2] / "extra" / "disassemblers" / "applegpu"
      if not applegpu_path.exists():
        print("Apple GPU tools not installed: skipping Metal assembly output")
      else:
        with tempfile.NamedTemporaryFile(delete=True) as shader:
          shader.write(lib)
          shader.flush()
          os.system(f"cd {applegpu_path} && python3 compiler_explorer.py {shader.name}")
