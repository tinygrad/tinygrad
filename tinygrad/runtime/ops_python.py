...
from tinygrad.device import Compiled, Compiler, Allocator, MMIOInterface
      ...
      while i < len(self.uops):
        ...
        if u.op is Ops.AFTER: values[u] = src_values[0]
          ...
          if u.addrspace == AddrSpace.REG:
            ...
            buf = MMIOInterface.from_memoryview(memoryview(bytearray(u.max_numel()*u.dtype.itemsize))) if u.op is not Ops.PARAM else pbufs.pop(0)
            values[u] = [buf.view(fmt=storage_fmt)] * warp_size
