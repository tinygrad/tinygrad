...
def storage_fmt_for_dtype(dtype: DType):
  return "H" if dtype == dtypes.bfloat16 else dtype.fmt
        ...
        if uop in {Ops.DEFINE_GLOBAL, Ops.DEFINE_LOCAL, Ops.DEFINE_REG}:
          ...
          if isinstance(dtp[0], ImageDType):
            for m, ox, oy in zip(inp[0], inp[1][0], inp[1][1]):
              if ox < 0 or ox >= dtp[0].shape[1] or oy < 0 or oy >= dtp[0].shape[0]:
                ret.append((m, None))
              else:
          ...
          if dtype.count > 1:
            ul[i] = [
              load([inp[i][j] if i != 0 and dtp[i].count > 1 else inp[i] for i in range(len(inp))], j, dtype.scalar()) for j in range(dtype.count)
            ]
          else:
...
class PythonRenderer(Renderer):
  ...
  def __init__(self):
    ...
    if getenv("EMULATE_AMD_MFMA"):
      self.device, self.tensor_cores = "AMD", tc.amd_cdna
    if getenv("EMULATE_AMD_RDNA4"):
