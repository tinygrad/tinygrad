  ...
  def __call__(self, *bufs, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1), vals:tuple[int, ...]=(), wait=False):
    ...
    for idxs in itertools.product(*[range(x) for x in global_size[::-1]]):
      ...
      while i < len(self.uops):
        ...
        if uop is Ops.DEFINE_REG and arg[0] == "register": idp = [idp[0]]
        inp = [ul[v] for v in idp if self.uops[v][0] not in void_ops]
        dtp = [dl[v] for v in idp if self.uops[v][0] not in void_ops]
        if getenv("TRACE"): print(i, uop, dtype, arg, inp, dtp)
        if uop is Ops.STORE:
          if self.uops[idp[0]][0] == Ops.DEFINE_REG and self.uops[idp[0]][3][0] == "register":
            ul[idp[0]] = inp[1]
          else:
        ...
        dl[i] = dtype
        if uop is Ops.DEFINE_REG and isinstance(arg[0], str):
          if arg[0] in ("global", "local"):
            assert dtype.fmt is not None and isinstance(dtype, PtrDType)
            if TYPE_CHECKING or sys.version_info < (3, 12): assert dtype.fmt != "e"
            buf = memoryview(bytearray(dtype.size*dtype.itemsize)) if arg[0] == "local" else pbufs.pop(0)
        ...
        elif uop is Ops.DEFINE_REG and isinstance(arg[0], str):
          if arg[0] == "register":
