...
def exec_alu(arg, dtype, p):
  ...
  operations: Dict[Op, Callable] = {
    TernaryOps.MULACC: lambda: None if len(p) != 3 else p[0]*p[1]+p[2],
    TernaryOps.WHERE: lambda: None if len(p) != 3 or not dtypes.is_bool(dtypes.from_py(p[0])) else p[1] if p[0] else p[2],
    UnaryOps.LOG2: lambda: None if len(p) != 1 else math.nan if p[0] < 0 else -math.inf if p[0] == 0 else math.log2(p[0]),
    UnaryOps.EXP2: lambda: None if len(p) != 1 else safe_exp2(p[0]),
    UnaryOps.SQRT: lambda: None if len(p) != 1 else math.nan if p[0] < 0 else math.sqrt(p[0]),
    UnaryOps.SIN: lambda: None if len(p) != 1 else math.sin(p[0]),
    UnaryOps.NEG: lambda: None if len(p) != 1 else -p[0],
    BinaryOps.MUL: lambda: None if len(p) != 2 else p[0]*p[1],
    BinaryOps.ADD: lambda: None if len(p) != 2 else p[0]+p[1],
    BinaryOps.SUB: lambda: None if len(p) != 2 else p[0]-p[1],
    BinaryOps.XOR: lambda: None if len(p) != 2 or not dtypes.is_int(dtype) else p[0]^p[1],
    BinaryOps.MAX: lambda: None if len(p) != 2 else max(p[0], p[1]),
    ...
    BinaryOps.DIV: lambda: None if len(p) != 2 else math.nan if p[1] == 0 else p[0]//p[1] if dtypes.is_int(dtype) else p[0]/p[1],
    BinaryOps.MOD: lambda: None if len(p) != 2 or not dtypes.is_int(dtype) else math.nan if p[1] == 0 else p[0]%p[1],
  }
