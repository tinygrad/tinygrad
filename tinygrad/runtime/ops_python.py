      ...
      while i < len(self.uops):
        ...
        if uop is UOps.DEFINE_GLOBAL:
          ...
          if arg[4] == "METAL":
            ...
            if getenv("EMULATE_CUDA"): ul[i] = wmma_helper(32, 16, 8, 4, 4, a_elem, b_elem, c_map)
            elif getenv("EMULATE_CUDA_75"): ul[i] = wmma_helper(32, 8, 4, 2, 4, a_elem, b_elem, c_map)
          elif arg[4] == "INTEL":
...
class PythonRenderer(Renderer):
  ...
  def __init__(self):
    ...
    if getenv("EMULATE_CUDA_75"): self.device, self.tensor_cores = "CUDA", CUDARenderer.tensor_cores_75
