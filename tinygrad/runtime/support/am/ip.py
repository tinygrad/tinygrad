...
class AM_SDMA(AM_IP):
  ...
  def init_hw(self):
    # aqua (NBIO 7.9): writing SDMA engine CNTL registers tears the fabric on this PF (PTE table scans turn to 0xff
    # garbage mid-boot); what mirrors the kernel here is soc15_sdma_doorbell_range_init -> nbio_v7_9_sdma_doorbell_range
  ...
  def setup_ring(self, ring_addr:int, ring_size:int, rptr_addr:int, wptr_addr:int, idx:int) -> int:
    ...
    self.adev.reg(f"{reg}_RB_CNTL").write(**({f'{self.sdma_name.lower()}_wptr_poll_enable':1} if self.adev.ip_ver[am.SDMA0_HWIP][:2]!=(4,4) else {}),
      rb_vmid=0, rptr_writeback_enable=1, rptr_writeback_timer=4, rb_enable=1, rb_priv=0, rb_size=(ring_size//4).bit_length()-1, inst=inst)
