...
class AM_IP:
    ...
    def init(self):
        raise NotImplementedError("IP block init must be implemeted")
...
class AM_GMC(AM_IP):
    def __init__(self, adev):
        ...
        self.memscratch_paddr = self.adev.mm.palloc(
            0x1000, zero=not self.adev.partial_boot, boot=True
        )
        self.dummy_page_paddr = self.adev.mm.palloc(
            0x1000, zero=not self.adev.partial_boot, boot=True
        )
    ...
    def flush_hdp(self):
        self.adev.regBIF_BX_PF0_GPU_HDP_FLUSH_REQ.write(0xFFFFFFFF)
    ...
    def flush_tlb(self, ip: Literal["MM", "GC"], vmid, flush_type=0):
        ...
        if ip == "MM":
            self.adev.wait_reg(
                self.adev.regMMVM_INVALIDATE_ENG17_SEM, mask=0x1, value=0x1
            )
        ...
        self.adev.wait_reg(
            self.adev.reg(f"reg{ip}VM_INVALIDATE_ENG17_ACK"),
            mask=(1 << vmid),
            value=(1 << vmid),
        )
    ...
    def enable_vm_addressing(self, page_table, ip: Literal["MM", "GC"], vmid):
        ...
        self.adev.reg(f"reg{ip}VM_CONTEXT{vmid}_CNTL").write(
            0x1FFFE00, enable_context=1, page_table_depth=(3 - page_table.lv)
        )

    def init_hub(self, ip: Literal["MM", "GC"]):
        # Init system apertures
        self.adev.reg(f"reg{ip}MC_VM_AGP_BASE").write(0)
        self.adev.reg(f"reg{ip}MC_VM_AGP_BOT").write(
            0xFFFFFFFFFFFF >> 24
        )  # disable AGP
        self.adev.reg(f"reg{ip}MC_VM_AGP_TOP").write(0)

        self.adev.reg(f"reg{ip}MC_VM_SYSTEM_APERTURE_LOW_ADDR").write(
            self.mc_base >> 18
        )
        self.adev.reg(f"reg{ip}MC_VM_SYSTEM_APERTURE_HIGH_ADDR").write(
            self.mc_end >> 18
        )
        self.adev.wreg_pair(
        ...
        for eng_i in range(18):
            self.adev.wreg_pair(
                f"reg{ip}VM_INVALIDATE_ENG{eng_i}_ADDR_RANGE",
                "_LO32",
                "_HI32",
                0x1FFFFFFFFF,
            )
    ...
    def on_interrupt(self):
        for ip in ["MM", "GC"]:
            st, va = (
                self.adev.reg(f"reg{ip}VM_L2_PROTECTION_FAULT_STATUS").read(),
                self.adev.reg(f"reg{ip}VM_L2_PROTECTION_FAULT_ADDR_LO32").read(),
            )
            va = (
                va
                | (self.adev.reg(f"reg{ip}VM_L2_PROTECTION_FAULT_ADDR_HI32").read())
                << 32
            ) << 12
            if self.adev.reg(f"reg{ip}VM_L2_PROTECTION_FAULT_STATUS").read():
                raise RuntimeError(
                    f"{ip}VM_L2_PROTECTION_FAULT_STATUS: {st:#x} {va:#x}"
                )
...
class AM_SMU(AM_IP):
    def __init__(self, adev):
        super().__init__(adev)
        self.driver_table_paddr = self.adev.mm.palloc(
            0x4000, zero=not self.adev.partial_boot, boot=True
        )

    def init(self):
        self._send_msg(
            smu_v13_0_0.PPSMC_MSG_SetDriverDramAddrHigh,
            hi32(self.adev.paddr2mc(self.driver_table_paddr)),
            poll=True,
        )
        self._send_msg(
            smu_v13_0_0.PPSMC_MSG_SetDriverDramAddrLow,
    ...
    def is_smu_alive(self):
        with contextlib.suppress(RuntimeError):
            self._send_msg(smu_v13_0_0.PPSMC_MSG_GetSmuVersion, 0, timeout=100)
    ...
    def read_table(self, table_t, cmd):
        ...
        return table_t.from_buffer(
            to_mv(self.adev.paddr2cpu(self.driver_table_paddr), ctypes.sizeof(table_t))
        )

    def read_metrics(self):
        return self.read_table(
            smu_v13_0_0.SmuMetricsExternal_t, smu_v13_0_0.TABLE_SMU_METRICS
        )
    ...
    def set_clocks(self, level):
        if not hasattr(self, "clcks"):
            self.clcks = {}
            for clck in [
                smu_v13_0_0.PPCLK_GFXCLK,
                smu_v13_0_0.PPCLK_UCLK,
                smu_v13_0_0.PPCLK_FCLK,
                smu_v13_0_0.PPCLK_SOCCLK,
            ]:
                cnt = (
                    self._send_msg(
                        smu_v13_0_0.PPSMC_MSG_GetDpmFreqByIndex,
                        (clck << 16) | 0xFF,
                        read_back_arg=True,
                    )
                    & 0x7FFFFFFF
        ...
        for clck, vals in self.clcks.items():
            ...
            self._send_msg(
                smu_v13_0_0.PPSMC_MSG_SetSoftMaxByFreq,
                clck << 16 | (vals[level]),
                poll=True,
            )

    def _smu_cmn_poll_stat(self, timeout=10000):
        self.adev.wait_reg(
            self.adev.mmMP1_SMN_C2PMSG_90, mask=0xFFFFFFFF, value=1, timeout=timeout
        )
    ...
    def _send_msg(
        self, msg, param, poll=True, read_back_arg=False, timeout=10000
    ):  # 10s
        if poll:
...
class AM_GFX(AM_IP):
    def init(self):
        ...
        while (
            ...
            and self.adev.regRLC_RLCS_BOOTLOAD_STATUS.read(bootload_complete=1) != 0
        ):
            pass
        ...
        for i in range(0, 16):
            ...
            self.adev.regSH_MEM_CONFIG.write(
                address_mode=am.SH_MEM_ADDRESS_MODE_64,
                alignment_mode=am.SH_MEM_ALIGNMENT_MODE_UNALIGNED,
    ...
    def fini(self):
        ...
        self.adev.regCP_HQD_DEQUEUE_REQUEST.write(
            0x2
    ...
    def setup_ring(
        ...
        mqd_struct = am.struct_v11_compute_mqd(
            header=0xC0310800,
            cp_mqd_base_addr_lo=lo32(mqd.va_addr),
            cp_mqd_base_addr_hi=hi32(mqd.va_addr),
            cp_hqd_persistent_state=self.adev.regCP_HQD_PERSISTENT_STATE.build(
            ...
            cp_hqd_pq_doorbell_control=self.adev.regCP_HQD_PQ_DOORBELL_CONTROL.build(
                doorbell_offset=doorbell * 2, doorbell_en=1
            ),
            cp_hqd_pq_control=self.adev.regCP_HQD_PQ_CONTROL.build(
                rptr_block_size=5,
                unord_dispatch=1,
                queue_size=(ring_size // 4).bit_length() - 2,
            ),
            cp_hqd_ib_control=self.adev.regCP_HQD_IB_CONTROL.build(
                min_ib_avail_size=0x3
            ),
            ...
            cp_hqd_eop_control=self.adev.regCP_HQD_EOP_CONTROL.build(
        ...
        ctypes.memmove(
            self.adev.paddr2cpu(mqd.paddrs[0][0]),
            ctypes.addressof(mqd_struct),
            ctypes.sizeof(mqd_struct),
        )
        ...
        for i, reg in enumerate(
            range(
                self.adev.regCP_MQD_BASE_ADDR.reg_off,
                self.adev.regCP_HQD_PQ_WPTR_HI.reg_off + 1,
            )
        ):
    ...
    def _config_gfx_rs64(self):
        def _config_helper(eng_name, cntl_reg, eng_reg, pipe_cnt, me=0):
            ...
            self.adev.reg(f"regCP_{cntl_reg}_CNTL").update(
                **{
                    f"{eng_name.lower()}_pipe{pipe}_reset": 1
                    for pipe in range(pipe_cnt)
                }
            )
            self.adev.reg(f"regCP_{cntl_reg}_CNTL").update(
                **{
                    f"{eng_name.lower()}_pipe{pipe}_reset": 0
                    for pipe in range(pipe_cnt)
                }
            )
        ...
        _config_helper(
            eng_name="MEC", cntl_reg="MEC_RS64", eng_reg="MEC_RS64", pipe_cnt=4, me=1
        )
...
class AM_IH(AM_IP):
    def __init__(self, adev):
        ...
        def _alloc_ring(size):
            return (
                self.adev.mm.palloc(size, zero=not self.adev.partial_boot, boot=True),
    ...
    def init(self):
        for ring_vm, rwptr_vm, suf, ring_id in self.rings:
            ...
            self.adev.reg(f"regIH_DOORBELL_RPTR{suf}").write(
                ((am.AMDGPU_NAVI10_DOORBELL_IH + ring_id) * 2), enable=1
            )
    ...
    def init(self):
        for pipe in range(2):
            ...
            self.adev.reg(f"regSDMA{pipe}_UTCL1_PAGE").update(
                rd_l2_policy=0x2, wr_l2_policy=0x3, llc_noalloc=1
            )  # rd=noa, wr=bypass
...
class AM_PSP(AM_IP):
    def __init__(self, adev):
        ...
        self.msg1_paddr = self.adev.mm.palloc(
            am.PSP_1_MEG, align=am.PSP_1_MEG, zero=not self.adev.partial_boot, boot=True
        )
        self.cmd_paddr = self.adev.mm.palloc(
            am.PSP_CMD_BUFFER_SIZE, zero=not self.adev.partial_boot, boot=True
        )
        self.fence_paddr = self.adev.mm.palloc(
            am.PSP_FENCE_BUFFER_SIZE, zero=not self.adev.partial_boot, boot=True
        )

        self.ring_size = 0x10000
        self.ring_paddr = self.adev.mm.palloc(
            self.ring_size, zero=not self.adev.partial_boot, boot=True
        )

        self.max_tmr_size = 0x1300000
        self.tmr_paddr = self.adev.mm.palloc(
            self.max_tmr_size,
            align=am.PSP_TMR_ALIGNMENT,
            zero=not self.adev.partial_boot,
            boot=True,
        )

    def is_sos_alive(self):
        return self.adev.regMP0_SMN_C2PMSG_81.read() != 0x0
    ...
    def init(self):
        ...
        if not self.is_sos_alive():
            for fw, compid in sos_components_load_order:
    ...
    def _wait_for_bootloader(self):
        self.adev.wait_reg(
            self.adev.regMP0_SMN_C2PMSG_35, mask=0xFFFFFFFF, value=0x80000000
        )
    ...
    def _ring_create(self):
        ...
        self.adev.wait_reg(
            self.adev.regMP0_SMN_C2PMSG_64, mask=0x80000000, value=0x80000000
        )

        self.adev.wreg_pair(
            "regMP0_SMN_C2PMSG", "_69", "_70", self.adev.paddr2mc(self.ring_paddr)
        )
        ...
        self.adev.wait_reg(
            self.adev.regMP0_SMN_C2PMSG_64, mask=0x8000FFFF, value=0x80000000
        )
    ...
    def _ring_submit(self):
        ...
        write_loc.cmd_buf_addr_hi, write_loc.cmd_buf_addr_lo = data64(
            self.adev.paddr2mc(self.cmd_paddr)
        )
        write_loc.fence_addr_hi, write_loc.fence_addr_lo = data64(
            self.adev.paddr2mc(self.fence_paddr)
        )
        write_loc.fence_value = prev_wptr

        # Move the wptr
        self.adev.regMP0_SMN_C2PMSG_67.write(
            prev_wptr + ctypes.sizeof(am.struct_psp_gfx_rb_frame) // 4
        )

        while to_mv(self.adev.paddr2cpu(self.fence_paddr), 4).cast("I")[0] != prev_wptr:
            pass
        time.sleep(0.005)

        resp = am.struct_psp_gfx_cmd_resp.from_address(
            self.adev.paddr2cpu(self.cmd_paddr)
        )
    ...
    def _prep_ring_cmd(self, hdr):
        ...
        cmd = am.struct_psp_gfx_cmd_resp.from_address(
            self.adev.paddr2cpu(self.cmd_paddr)
        )
        cmd.cmd_id = hdr
        return cmd
    ...
    def _load_ip_fw_cmd(self, psp_desc):
        if DEBUG >= 2:
            print(
                f"am {self.adev.devfmt}: loading fw: {am.psp_gfx_fw_type__enumvalues[psp_desc[0]]}"
            )
        ...
        cmd.cmd.cmd_load_ip_fw.fw_phy_addr_hi, cmd.cmd.cmd_load_ip_fw.fw_phy_addr_lo = (
            data64(self.adev.paddr2mc(self.msg1_paddr))
        )
