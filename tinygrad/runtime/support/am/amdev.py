...
class AMRegister(AMDReg):
  ...
  def read(self, inst=0):
    return self.adev.rreg(self.addr[inst])

  def read_bitfields(self, inst=0) -> dict[str, int]:
    return self.decode(self.read(inst=inst))

  def write(self, _am_val: int = 0, inst=0, **kwargs):
    self.adev.wreg(self.addr[inst], _am_val | self.encode(**kwargs))

  def update(self, inst=0, **kwargs):
    self.write(self.read(inst=inst) & ~self.fields_mask(*kwargs.keys()), inst=inst, **kwargs)
...
class AMFirmware:
  def __init__(self, adev):
    ...
    def fmt_ver(hwip):
      return "_".join(map(str, adev.ip_ver[hwip]))
    ...
    # RLC firmware
    blob, hdr0, _hdr1, hdr2, hdr3 = self.load_fw(
  ...
  def load_fw(self, fname: str, *headers, versioned_header: str | None = None):
    ...
    if AM_DEBUG >= 1:
      print(f"am {self.adev.devfmt}: loading firmware {fname}: {hashlib.sha256(blob).hexdigest()}")
    if versioned_header:
  ...
  def desc(self, blob: memoryview, offset: int, size: int, *types: int) -> tuple[list[int], memoryview]:
    return (list(types), blob[offset : offset + size])


class AMPageTableEntry:
  def __init__(self, adev, paddr, lv):
    self.adev, self.paddr, self.lv, self.entries = adev, paddr, lv, adev.vram.view(paddr, 0x1000, fmt="Q")
  ...
  def valid(self, entry_id: int) -> bool:
    return (self.entries[entry_id] & am.AMDGPU_PTE_VALID) != 0

  def address(self, entry_id: int) -> int:
    return self.entries[entry_id] & 0x0000FFFFFFFFF000

  def is_page(self, entry_id: int) -> bool:
    return self.lv == am.AMDGPU_VM_PTB or self.adev.gmc.is_pte_huge_page(self.entries[entry_id])

  def supports_huge_page(self, paddr: int):
    return self.lv >= am.AMDGPU_VM_PDB2
...
class AMDev(PCIDevImplBase):
  ...
  def __init__(
    self, devfmt, vram: MMIOInterface, doorbell: MMIOInterface, mmio: MMIOInterface, dma_regions: list[tuple[int, MMIOInterface]] | None = None
  ):
    ...
    if self.partial_boot and (self.reg("regGCVM_CONTEXT0_CNTL").read() != 0 or self.reg(self.gmc.pf_status_reg("GC")).read() != 0):
      if DEBUG >= 2:
        print(f"am {self.devfmt}: Malformed state. Issuing a full reset.")
  ...
  def init_sw(self, smi_dev=False):
    ...
    self.mm = AMMemoryManager(
      self,
      self.vram_size,
      boot_size=(32 << 20),
      pt_t=AMPageTableEntry,
  ...
  def fini(self):
    if DEBUG >= 2:
      print(f"am {self.devfmt}: Finalizing")
  ...
  def _ip_module(self, prefix: str, hwip, prever_prefix: str = ""):
    return import_module(prefix, self.ip_ver[hwip], prever_prefix)
