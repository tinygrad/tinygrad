...
class AMRegister(IPReg):
  ...
  def read_bitfields(self) -> dict[str, int]: return self.decode_bitfields(self.read())

  # *args instead of something like value:int|None=None to avoid key collision with **kwargs if there is a bitfield named value
  def write(self, *args, **kwargs):
    if bool(args) == bool(kwargs) or (bool(args) and len(args) != 1): raise RuntimeError('Either value or **kwargs must be specified, but not both')
    self.adev.wreg(self.addr, args[0] if args else self.encode_bitfields(**kwargs))

  def update(self, **kwargs): self.write(self.encode_bitfields(**{**self.read_bitfields(), **kwargs}))
