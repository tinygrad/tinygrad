...
def elf_loader(blob: bytes, force_section_align: int = 1) -> tuple[memoryview, list[ElfSection], list[tuple]]:
  ...
  def _to_carray(sh, ctype):
    return (ctype * (sh.header.sh_size // sh.header.sh_entsize)).from_buffer_copy(sh.content)
  ...
  for sh, trgt_sh_name, c_rels in rel + rela:
    ...
    for roff, sym, r_type_, r_addend in rels:
      if sym.st_shndx == 0:
        raise RuntimeError(f"Attempting to relocate against an undefined symbol {repr(_strtab(sh_strtab, sym.st_name))}")
