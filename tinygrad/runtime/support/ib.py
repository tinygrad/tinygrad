import ctypes, tinygrad.runtime.autogen.ib as ib
from tinygrad.helpers import DEBUG, mv_address

DEFAULT_PORT, DEFAULT_GID = 1, 3
...
class IBCtx:
  def __init__(self, idx:int):
    ...
    # Free the list (can only be done after device is opened)
    ib.ibv_free_device_list(devs)

    # Alloc protection domain
    self.pd = ib.ibv_alloc_pd(self.ctx)

    self.mrs = {} # Memory regions

    self.hmv = memoryview(bytearray(b'\x00'*0x1000)) # Helper memoryview (used to send messages)
    self.hmr = self.reg(mv_address(self.hmv), len(self.hmv)) # ib memory region corresponding to this memoryview
  ...
  def get_gid(self, port_idx:int, gid_idx:int) -> bytes:
    ...
    return checkz(ib.ibv_query_gid(self.ctx, port_idx, gid_idx, ctypes.byref(gidu:=ib.union_ibv_gid())), bytes(gidu.raw))

  def reg(self, base:int, sz:int):
    if base not in self.mrs:
      if DEBUG>=2: print(f"Registred {base:#x}:{base+base-1:#x}")
      mr = ib.ibv_reg_mr_iova2(self.pd, ctypes.c_void_p(base), sz, base,
                               ib.IBV_ACCESS_LOCAL_WRITE | ib.IBV_ACCESS_REMOTE_READ | ib.IBV_ACCESS_REMOTE_WRITE)
      if mr is None: raise RuntimeError(f"Couldn't register memory region for {base:#x}:{base+base-1:#x}")
      self.mrs[base] = mr
    return self.mrs[base]
...
class IBConn:
  def __init__(self, ctx:IBCtx, port_idx:int, gid_idx:int):
    self.ctx, self.port_idx, self.gid_idx = ctx, port_idx, gid_idx
    ...
    # Create Queue Pair. It's the closest thing to a socket in infiniband with QP num being the closest thing to a port, except we can't choose it
    ...
    self.gid, self.qp_num = self.ctx.get_gid(port_idx, gid_idx), self.qp.contents.qp_num
  ...
  def connect(self, remote_gid:bytes, remote_qp_num:int):
    # RTR
    qp_ah_attr_grh = ib.struct_ibv_global_route(hop_limit=1, dgid=ib.union_ibv_gid(raw=(ctypes.c_ubyte * 16)(*remote_gid)),
                                                sgid_index=self.gid_idx)
    qp_ah_attr = ib.struct_ibv_ah_attr(is_global=1, port_num=self.port_idx, grh=qp_ah_attr_grh)
...
#     IBDevice.ctxs[int(idx)] = self.ctx = IBDev(IBDevice.dev_list[int(idx)]) if int(idx) not in IBDevice.ctxs else IBDevice.ctxs[int(idx)]
