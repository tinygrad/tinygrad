...
class IBCtx:
  def __init__(self, idx:int):
    ...
    self.last_iova: int = IOVA_ALIGN # don't start at zero (nullptr)
  ...
  def __del__(self):
    for _,_,finalizer in self.mrs.values(): finalizer() # must deallocate all mrs in protection domain before deallocating the protection domain
  ...
  def reg(self, buf:Buffer) -> tuple[int, ctypes._Pointer[ib.struct_ibv_mr]]:
    ...
    if buf not in self.mrs:
      ...
      match (dmaref:=buf.as_dmaref()):
        case DMACPURef(): mr = ib.ibv_reg_mr_iova2(self.pd, ctypes.c_void_p(dmaref.addr), dmaref.size, self.last_iova, mr_flags)
        case DMAFdRef(): mr = ib.ibv_reg_dmabuf_mr(self.pd, dmaref.offset, dmaref.size, self.last_iova, dmaref.fd, mr_flags)
        case _: raise RuntimeError(f"Unknown type of dma ref: {dmaref}")
      if mr is None: raise RuntimeError(f"Couldn't register memory region for {buf} (errno={ctypes.get_errno()})")
      self.mrs[buf] = (self.last_iova, mr, weakref.finalize(buf, ib.ibv_dereg_mr, mr))
      self.last_iova = round_up(self.last_iova + dmaref.size, IOVA_ALIGN)
