...
from tinygrad.helpers import SPILL
...
def compile_hip(prg:str, arch="gfx1100", asm=False) -> bytes:
  ...
  if asm:
    ...
    if not SPILL: options += ["-mllvm -amdgpu-spill-sgpr-to-vgpr=false", "-mllvm -amdgpu-spill-vgpr-to-agpr=false", "-fno-plt -fno-stack-protector"]
  ...
  def compile(self, src:str) -> bytes:
    with tempfile.NamedTemporaryFile(suffix=".cpp") as srcf, tempfile.NamedTemporaryFile(suffix=".bc") as bcf:
      with tempfile.NamedTemporaryFile(suffix=".hsaco") as libf:
        ...
        subprocess.run(["hipcc", "-target", "amdgcn-amd-amdhsa", f"-mcpu={self.arch}", "-mllvm -amdgpu-spill-sgpr-to-vgpr=false" * (not SPILL),
