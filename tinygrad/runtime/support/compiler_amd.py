...
try:
  ...
  pass  # ignore if ROCm isn't installed
...
def compile_hip(prg: str, arch="gfx1100", asm=False) -> bytes:
  ...
  if asm:
    ...
    check(
      set_options(
        action_info,
        f"-O3 -mcumode --hip-version=6.0.32830 -DHIP_VERSION_MAJOR=6 -DHIP_VERSION_MINOR=0 -DHIP_VERSION_PATCH=32830 -D__HIPCC_RTC__ -std=c++14 -nogpuinc -Wno-gnu-line-marker -Wno-missing-prototypes --offload-arch={arch} -I/opt/rocm/include -Xclang -disable-llvm-passes -Xclang -aux-triple -Xclang x86_64-unknown-linux-gnu".encode(),
      )
    )  # noqa: E501
  ...
  for x in [data_set_src, data_set_bc, data_set_reloc, data_set_exec]:
    check(comgr.amd_comgr_destroy_data_set(x))
...
class HIPCompiler(Compiler):
  ...
  def compile(self, src: str) -> bytes:
    try:
      return compile_hip(src, self.arch, src.split("\n", 1)[0].strip() == ".text")
...
class AMDLLVMCompiler(LLVMCompiler):
  ...
  def __reduce__(self):
    return (AMDLLVMCompiler, (self.arch,))

  def compile(self, src: str) -> bytes:
    try:
      return super().compile(src)
    except RuntimeError as e:
      if "undefined value '@llvm.amdgcn." in str(e):
        raise CompileError(str(e) + "AMD with LLVM backend requires LLVM >= 18") from e
      raise CompileError(e) from e
