...
static const double d_huge = 0x1p1000, d_redux = 0x1.8p52 / D_TBLSIZE,
                    D_P1 = 0x1.62e42fefa39efp-1, D_P2 = 0x1.ebfbdff82c575p-3,
                    D_P3 = 0x1.c6b08d704a0a6p-5, D_P4 = 0x1.3b2ab88f70400p-7,
                    D_P5 = 0x1.5d88003875c74p-10;

static volatile double d_twom1000 = 0x1p-1000;

static const double d_tbl[D_TBLSIZE * 2] = {
    /*	exp2(z + eps)		eps	*/
...
double exp2(double x) {
  ...
  if (ix >= 0x40900000) { /* |x| >= 1024 */
    ...
    if (x >= 0x1.0p10)
      return (d_huge * d_huge); /* overflow */
    if (x <= -0x1.0ccp10)
      return (d_twom1000 * d_twom1000); /* underflow */
  } else if (ix < 0x3c900000) {         /* |x| < 0x1p-54 */
    return (1.0 + x);
  }

  /* Reduce x, computing z, i0, and k. */
  STRICT_ASSIGN(double, t, x + d_redux);
  GET_LOW_WORD(i0, t);
  i0 += D_TBLSIZE / 2;
  k = (i0 >> D_TBLBITS) << 20;
  i0 = (i0 & (D_TBLSIZE - 1)) << 1;
  ...
  r = t + t * z * (D_P1 + z * (D_P2 + z * (D_P3 + z * (D_P4 + z * D_P5))));
  ...
  if (k >= -(1021 << 20)) {
    ...
    return (r * twopkp1000 * d_twom1000);
  }
}
...
static const float huge = 0x1p100f, redux = 0x1.8p23f / TBLSIZE,
                   P1 = 0x1.62e430p-1f, P2 = 0x1.ebfbe0p-3f,
