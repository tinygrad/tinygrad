...
def hcq_profile(dev: HCQCompiled, enabled, desc, queue_type: Callable[[], HWQueue] | None = None, queue: HWQueue | None = None):
  ...
  try:
    ...
    if enabled and PROFILE:
      dev.sig_prof_records.append((cast(HCQSignal, st), cast(HCQSignal, en), desc, queue_type is dev.hw_copy_queue_t))
...
class HCQProgram(Generic[HCQDeviceType]):
  ...
  def fill_kernargs(self, bufs: tuple[HCQBuffer, ...], vals: tuple[int, ...] = (), kernargs: HCQBuffer | None = None) -> HCQArgsState:
    ...
    argsbuf = kernargs or self.dev.kernargs_buf.offset(
      offset=self.dev.kernargs_offset_allocator.alloc(self.kernargs_alloc_size), size=self.kernargs_alloc_size
    )
...
class HCQCompiled(Compiled, Generic[SignalType]):
  ...
  def _is_cpu(self) -> bool:
    return hasattr(self, "device") and self.device.split(":")[0] in ("CPU", "LLVM")
