...
class HWInterface:
    ...
    def eventfd(initval, flags=None):
        return HWInterface(fd=os.eventfd(initval, flags))  # type: ignore[attr-defined]


if MOCKGPU := getenv("MOCKGPU"):
    from test.mockgpu.mockgpu import (
        MockHWInterface as HWInterface,
...
class HWQueue(Generic[SignalType, DeviceType, ProgramType, ArgsStateType]):
    ...
    def bind_args_state(self, args_state: ArgsStateType):
        for vals, ptr, fmt in args_state.bind_data:
            self.bind_sints_to_ptr(*vals, ptr=ptr, fmt=fmt)
    ...
    def bind_sints(
        ...
        *vals: sint,
        ...
        self.bind_sints_to_ptr(
            *vals,
            ptr=ctypes.addressof(struct) + getattr(type(struct), start_field).offset,
            fmt=fmt,
            mask=mask,
        )
    ...
    def _submit(self, dev: DeviceType):
...
class HCQSignal(Generic[DeviceType]):
    def __init__(
        ...
        base_addr: sint = 0,
        value: int = 0,
        timeline_for_device: DeviceType | None = None,
        timestamp_divider=1,
        value_off=0,
        timestamp_off=8,
    ):
        self.base_addr, self.value_addr, self.timestamp_addr = (
            base_addr,
            base_addr + value_off,
            base_addr + timestamp_off,
        )
        self.timestamp_divider: decimal.Decimal = decimal.Decimal(timestamp_divider)
        self.timeline_for_device: DeviceType | None = timeline_for_device

        if isinstance(base_addr, int):
            self.value_mv, self.timestamp_mv = to_mv(self.value_addr, 8).cast(
                "Q"
            ), to_mv(self.timestamp_addr, 8).cast("Q")
    ...
    def wait(self, value: int, timeout: int = getenv("HCQDEV_WAIT_TIMEOUT_MS", 30000)):
        ...
        while (
            self.value < value
            and (time_spent := int(time.perf_counter() * 1000) - start_time) < timeout
        ):
...
def hcq_profile(
    ...
    enabled,
    desc,
    queue_type: Type[HWQueue] | None = None,
    ...
    if enabled and queue is not None:
        ...
        queue_type().wait(dev.timeline_signal, dev.timeline_value - 1).timestamp(
            st
        ).signal(dev.timeline_signal, dev.timeline_value).submit(dev)
        ...
        if enabled and PROFILE:
            dev.sig_prof_records.append(
                (
                    ...
                    cast(HCQSignal, en),
                    desc,
                    queue_type is dev.hw_copy_queue_t,
                )
            )
...
class CLikeArgsState(HCQArgsState[ProgramType]):
    def __init__(
        ...
        if prefix is not None:
            to_mv(self.ptr, len(prefix) * 4).cast("I")[:] = array.array("I", prefix)

        self.bind_sints_to_ptr(
            *[b.va_addr for b in bufs], ptr=self.ptr + len(prefix or []) * 4, fmt="Q"
        )
        self.bind_sints_to_ptr(
            *vals, ptr=self.ptr + len(prefix or []) * 4 + len(bufs) * 8, fmt="I"
        )
...
class HCQProgram(Generic[DeviceType]):
    ...
    def fill_kernargs(
        ...
        """
        return self.args_state_t(
            kernargs_ptr or self.dev.kernargs_allocator.alloc(self.kernargs_alloc_size),
            self,
            bufs,
            vals=vals,
        )
    ...
    def __call__(
        ...
        *bufs: HCQBuffer,
        ...
        with hcq_profile(
            self.dev, queue=q, desc=self.name, enabled=wait or PROFILE
        ) as (sig_st, sig_en):
...
class HCQCompiled(Compiled, Generic[SignalType]):
    ...
    def __init__(
        ...
        copy_queue_t: Type[HWQueue] | None,
    ):
        ...
        self.timeline_signal: SignalType = self.signal_t(
            value=0, timeline_for_device=self
        )
        self._shadow_timeline_signal: SignalType = self.signal_t(
            value=0, timeline_for_device=self
        )
        ...
        self.kernargs_page: HCQBuffer = self.allocator.alloc(
            16 << 20, BufferSpec(cpu_access=True)
        )
        self.kernargs_allocator: BumpAllocator = BumpAllocator(
            self.kernargs_page.size,
            base=cast(int, self.kernargs_page.va_addr),
            wrap=True,
        )
...
class HCQBuffer:
    def __init__(
        ...
        self.va_addr, self.size, self.texture_info, self.meta, self._base = (
            va_addr,
            size,
            texture_info,
            meta,
...
class HCQAllocatorBase(LRUAllocator, Generic[DeviceType]):
    ...
    def __init__(
        self, dev: DeviceType, batch_size: int = (2 << 20), batch_cnt: int = 32
    ):
    ...
    def _offset(self, buf, size: int, offset: int) -> HCQBuffer:
        return HCQBuffer(
            va_addr=buf.va_addr + offset,
            size=size,
            texture_info=buf.texture_info,
            meta=buf.meta,
            _base=buf._base or buf,
        )
        ...
        with hcq_profile(
            ...
            for i in range(0, src.nbytes, self.b[0].size):
                ...
                self.dev.hw_copy_queue_t().wait(
                    ...
                    self.dev.timeline_signal, self.dev.timeline_value
                ).submit(
                    self.dev
                )
        ...
        with hcq_profile(
            ...
            for i in range(0, dest.nbytes, self.b[0].size):
                self.dev.hw_copy_queue_t().wait(
                    ...
                    self.b[0].va_addr,
                    src.va_addr + i,
                    lsize := min(self.b[0].size, dest.nbytes - i),
                ).signal(
                    self.dev.timeline_signal, self.dev.timeline_value
                ).submit(
                    self.dev
    ...
    def _transfer(
        ...
        with hcq_profile(
            ...
            src_dev.hw_copy_queue_t().wait(
                ...
                src_dev.timeline_signal, src_dev.timeline_value
            ).submit(
                src_dev
            )
        ...
        if src_dev != dest_dev:
            dest_dev.hw_compute_queue_t().wait(
                ...
                dest_dev.timeline_signal, dest_dev.timeline_value
            ).submit(
                dest_dev
            )
