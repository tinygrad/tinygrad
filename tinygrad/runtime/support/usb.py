...
def usb_ctrl(h:UOp, rtype:int, req:int, val:UOp|int, idx:UOp|int, data:UOp, n:UOp|int, timeout:int=1000) -> UOp:
  return ccall(libusb.libusb_control_transfer, h.index(0).load(), rtype, req, val, idx, data, n, timeout, ret_dtype=dtypes.void)

def usb_bulk(h:UOp, ep:int, data:UOp, n:UOp|int, timeout:int=10000) -> UOp: # NULL actual_length
  return ccall(libusb.libusb_bulk_transfer, h.index(0).load(), ep, data, n, UOp.const(0, dtypes.uint64), timeout, ret_dtype=dtypes.void)
...
def usb_chunk(h:UOp, table:UOp, i:UOp, half:int, run:int) -> UOp: # send chunk i, numbered run + i
  ...
  return ccall(libusb.libusb_submit_transfer, xfer.index(0), ret_dtype=dtypes.void)
...
def usb_copyout(h:UOp, table:UOp, n:int, run:int) -> UOp: # read back through both halves
  ...
  done = ccall(libc.memcpy, addr + CHUNK, stage.after(hi).index(HALF), second.cast(dtypes.uint64), ret_dtype=dtypes.void)
