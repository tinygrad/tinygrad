...
if sys.platform == "win32":
    ...
    if not os.path.exists(brew_prefix):
        raise FileNotFoundError(
            "LLVM not found, you can install it with `brew install llvm`"
        )
    ...
    if LLVM_PATH is None:
        raise FileNotFoundError(
            "No LLVM library found on the system. Install it via your distro's package manager and ensure it's findable as 'LLVM'"
        )

if DEBUG >= 3:
    print(f"Using LLVM at {repr(LLVM_PATH)}")
