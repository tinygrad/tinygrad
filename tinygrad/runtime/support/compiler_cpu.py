...
class ClangJITCompiler(Compiler):
  ...
  def compile_to_obj(self, src:str) -> bytes:
    ...
    target = {'amd64': 'x86_64'}.get(machine, machine)
    # on arm march means "runs on this arch and superset" instead of "optimize for this arch". x86 march == arm mcpu
    arch = {'x86_64': '-march=native', 'amd64': '-march=native', 'riscv64': '-march=rv64g'}.get(machine, "-mcpu=native")
...
class LLVMCompiler(Compiler):
  jit = True
  target_arch = {'arm64': 'AArch64', 'aarch64': 'AArch64', 'x86_64': 'X86', 'amd64': 'X86', 'riscv64': 'riscv64'}[platform.machine().lower()]
