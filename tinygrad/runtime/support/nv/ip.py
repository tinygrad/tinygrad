...
class NV_GSP(NV_IP):
  ...
  def init_rm_args(self, queue_size=0x40000):
    ...
    for i, sysmem in enumerate(queues_sysmem):
      to_mv(queues_va + i * 0x8, 0x8).cast("Q")[0] = sysmem
  ...
  def rpc_set_gsp_system_info(self):
    ...
    data = nv.GspSystemInfo(
      gpuPhysAddr=self.nvdev.bars[0][0],
      gpuPhysFbAddr=self.nvdev.bars[1][0],
      gpuPhysInstAddr=self.nvdev.bars[3][0],
      ...
      bIsPassthru=1,
      PCIDeviceID=self.nvdev.venid,
      PCISubDeviceID=self.nvdev.subvenid,
      PCIRevisionID=self.nvdev.rev,
