...
class NVDev(PCIDevImplBase):
  ...
  def extract_fw(self, file: str, dname: str) -> bytes:
    ...
    info, sl = (
      text[text[: text.index(dnm := f"{file}_{self.chip_name}_{dname}")].rindex("COMPRESSION:") :][:16],
  ...
  def include(self, file: str):
    ...
    for raw in self._download(file).splitlines():
      ...
      if m := re.match(r"#define\s+(\w+)\s+([0-9\+\-\*\(\)]+):([0-9\+\-\*\(\)]+)", raw):  # bitfields
        ...
        if reg is not None:
          self.__dict__[reg].add_field(name[len(reg) + 1 :].lower(), eval(lo), eval(hi))
        else:
          self.reg_offsets[name] = (eval(lo), eval(hi))
        continue
