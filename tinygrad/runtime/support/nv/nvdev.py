...
class NVPageTableEntry:
  ...
  def read_fields(self, entry_id:int) -> dict:
    ...
    return (self.nvdev.NV_MMU_VER3_DUAL_PDE if self._is_dual_pde() else self.nvdev.NV_MMU_VER3_PDE).decode(self.entry(entry_id))
  ...
  def address(self, entry_id:int) -> int: return self.read_fields(entry_id)['address_small' if self._is_dual_pde() else ('address_sys' if self.lv == self.nvdev.mm.level_cnt - 1 else 'address')] << 12
