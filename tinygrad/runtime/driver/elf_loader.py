...
def mkstub(absaddr: int) -> bytes:
  if platform.machine() == "arm64":
    return b'\x50\x00\x00\x58\x00\x02\x1f\xd6' + struct.pack("<Q", absaddr)
  else: raise NotImplementedError(f"architecture {platform.machine()} isn't supported")
EXTERNAL_SYMBOLS = ["sin", "sinf", "sqrt", "sqrtf", 'exp2', 'exp2f', 'log2', 'log2f']
PLT_ENTRY_SIZE = len(mkstub(0))
PLT = b''.join([mkstub(ctypes.cast(getattr(libc, x), ctypes.c_void_p).value) for x in EXTERNAL_SYMBOLS])
PLTDICT = {n:i*PLT_ENTRY_SIZE for i,n in enumerate(EXTERNAL_SYMBOLS)}
...
def load_elf(blob: bytes) -> Dict[str, Callable]:
  ...
  alloc_size = round_up(len(PLT), ALIGNMENT)
  ...
  JitAlloc.alloc(alloc_size, ensure_only=True)
  plt_start = JitAlloc.alloc(len(PLT))
  ctypes.memmove(plt_start, PLT, len(PLT))
  for idx,blob in progbits.items():
    loaded[idx] = section = JitAlloc.alloc(len(blob))
  ...
  for i,(sh_link,syms) in symtab.items():
    ...
    for j,(st_name,st_info,st_shndx,st_value) in enumerate(syms):
      ...
      if (load_base:=loaded.get(st_shndx, None)) is not None:
        ...
        if st_info == 0x12: exports[cname] = ctypes.cast(load_base+st_value, ctypes.CFUNCTYPE(ctypes.c_uint64))
      elif (off:=PLTDICT.get(cname, None)) is not None: saddrs[(i,j)] = plt_start+off
  # patch relocations
  for sh_link,sh_info,relas in rela.values():
    ...
    for r_offset,r_type,r_sym,r_addend in relas:
      ...
      rel = tgt - ploc
      if r_type == 0x113: # R_AARCH64_ADR_PREL_PG_HI21
        ...
        ctypes.cast(ploc, ctypes.POINTER(ctypes.c_uint32)).contents.value |= 2**26+(rel>>2) if rel < 0 else (rel>>2)
      elif r_type == 0x11c: # R_AARCH64_LDST16_ABS_LO12_NC
        assert tgt&0b1 == 0, f"unaligned 16-bit load {tgt=:#x}"
        ctypes.cast(ploc, ctypes.POINTER(ctypes.c_uint32)).contents.value |= (tgt&0xFFF)<<9
      elif r_type == 0x11d: # R_AARCH64_LDST64_ABS_LO12_NC
        assert tgt&0b11 == 0, f"unaligned 32-bit load {tgt=:#x}"
        ctypes.cast(ploc, ctypes.POINTER(ctypes.c_uint32)).contents.value |= (tgt&0xFFF)<<8
      elif r_type == 0x11e: # R_AARCH64_LDST64_ABS_LO12_NC
        assert tgt&0b111 == 0, f"unaligned 64-bit load {tgt=:#x}"
        ctypes.cast(ploc, ctypes.POINTER(ctypes.c_uint32)).contents.value |= (tgt&0xFFF)<<7
      elif r_type == 0x12b: # R_AARCH64_LDST128_ABS_LO12_NC
        assert tgt&0b1111 == 0, f"unaligned 128-bit load {tgt=:#x}"
        ctypes.cast(ploc, ctypes.POINTER(ctypes.c_uint32)).contents.value |= (tgt&0xFFF)<<6
