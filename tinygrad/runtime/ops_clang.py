...
class ClangCompiler(CachedCompiler):
  def _compile(self, prg:str, **compiler_args) -> bytes:
    ...
    with tempfile.NamedTemporaryFile(delete=True) as output_file:
      subprocess.check_output(args=('clang -shared -march=native -O2 -Wall -Werror -x c -fPIC - -o '+str(output_file.name)).split(), input=(compiler_args['header']+prg).encode('utf-8'))
...
ClangDevice = Compiled(MallocAllocator, LinearizerOptions(supports_float4=False, has_local=False), renderer, ClangCompiler(header=CLANG_PROGRAM_HEADER), ClangProgram)
