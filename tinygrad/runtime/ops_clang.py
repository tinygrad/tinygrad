...
if OSX and FASTCLANG: mac_libc = ctypes.CDLL(ctypes.util.find_library('c')) # needed for jit write protect and sys icache invalidate
...
class ClangCompiler(Compiler):
  ...
  def compile_jit(self, src:str):
    ...
    for ploc,tgt,r_type in [(a,b+d,c) for a,b,c,d in relocs]:
      def _patchuint32(new: int): image[ploc:ploc+4] = struct.pack("<I", struct.unpack("<I", image[ploc:ploc+4])[0] | new)
      tgt_pg, ploc_pg, rel = tgt >> 12, ploc >> 12, tgt - ploc
      if r_type in {libc.R_X86_64_PC32, libc.R_X86_64_PLT32}: _patchuint32(2**32+rel if rel < 0 else rel)
      elif r_type == libc.R_AARCH64_ADR_PREL_PG_HI21: _patchuint32(((tgt_pg-ploc_pg)&0b11)<<29 | ((tgt_pg-ploc_pg)>>2)<<5)
      elif r_type == libc.R_AARCH64_ADD_ABS_LO12_NC: _patchuint32((tgt&0xFFF)<<10)
      elif r_type in {libc.R_AARCH64_CALL26, libc.R_AARCH64_JUMP26}: _patchuint32(2**26+(rel>>2) if rel < 0 else (rel>>2))
      elif r_type == libc.R_AARCH64_LDST16_ABS_LO12_NC: _patchuint32((tgt&0xFFF)<<9)
      elif r_type == libc.R_AARCH64_LDST32_ABS_LO12_NC: _patchuint32((tgt&0xFFF)<<8)
      elif r_type == libc.R_AARCH64_LDST64_ABS_LO12_NC: _patchuint32((tgt&0xFFF)<<7)
      elif r_type == libc.R_AARCH64_LDST128_ABS_LO12_NC: _patchuint32((tgt&0xFFF)<<6)
