...
loop_pragma, openmp_args = (lambda n: f"#pragma omp parallel for collapse({n})", "-fopenmp=libomp ") if getenv("CLANG_MP", 0) > 0 else (None, "")
...
class ClangProgram:
  def __init__(self, name:str, prg:str):
    ...
    if not os.path.exists(fn):
      subprocess.check_output(args=('clang -shared -O2 -Wall -Werror -x c '+openmp_args+args['cflags']+' - -o '+fn+'.tmp').split(), input=prg.encode('utf-8'))
      os.rename(fn+'.tmp', fn)
...
class ClangCodegen(CStyleCodegen):
  lang = CStyleLanguage(kernel_prefix=args['exp'], buffer_suffix=" restrict", loop_pragma=loop_pragma)
