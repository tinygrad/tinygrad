...
torch_fxn_for_op: Dict[Op, Callable] = {**base_fxn_for_op, **{
  ...
  BinaryOps.MAX: torch.maximum, BinaryOps.CMPLT: lambda x,y: (x<y).type(torch.promote_types(x.dtype, y.dtype)), BinaryOps.SUB: lambda x,y: torch.add(x, -1*y).type(torch.promote_types(x.dtype, y.dtype)),
