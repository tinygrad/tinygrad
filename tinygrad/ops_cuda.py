...
class Sum(Function):
  def forward(ctx, input, axis=None):
    ...
    return cp.array([input.sum()]) if axis is None else input.sum(axis=axis)

  def backward(ctx, grad_output):
    input, axis = ctx.saved_tensors
    if isinstance(axis, int): axis = [axis]
    shape = [1 if axis is None or i in axis else input.shape[i] for i in range(len(input.shape))]
    return grad_output.reshape(shape) + cp.zeros_like(input)

class Max(Function):
  def forward(ctx, inp, axis=None):
    if isinstance(axis, int): axis = [axis]
    ret = cp.amax(inp, axis=None if axis is None else tuple(axis), keepdims=True)
...
class Pow(Function):
  ...
  def backward(ctx, grad_output):
    ...
    return unbroadcast(y * (x**(y-1.0)) * grad_output, x.shape), \
           unbroadcast((x**y) * cp.log(x) * grad_output, y.shape)
...
class Transpose(Function):
  def forward(ctx, x, order):
    ...
    return cp.transpose(x, order)

  def backward(ctx, x):
    return cp.transpose(x, np.argsort(ctx.order))
...
class Conv2D(Function):
  def forward(ctx, x, w, stride=1, groups=1):
    ...
    tx = cp.lib.stride_tricks.as_strided(gx,
    ...
    ret = cp.zeros((bs,ctx.groups,oy,ox,rcout),dtype=x.dtype)
    for g in range(ctx.groups):
      #ijYXyx,kjyx -> iYXk ->ikYX
      ret[:,g] += cp.tensordot(tx[:,g], tw[g], ((1,4,5),(1,2,3)))
    return cp.moveaxis(ret,4,2).reshape(bs, cout, oy, ox)
  ...
  def backward(ctx, grad_output):
    ...
    gdw = cp.zeros((ctx.groups,rcout,cin,H,W), dtype=tx.dtype)
    for g in range(ctx.groups):
      #'ikYX,ijYXyx -> kjyx'
      gdw[g] += cp.tensordot(ggg[:,g], tx[:,g], ((0,2,3),(0,2,3)))

    # needs to be optimized
    gdx = cp.zeros((bs,ctx.groups,cin,OY,OX), dtype=tx.dtype)
    for k in range(oy*ox):
      Y, X = k//ox, k%ox
      iY,iX = Y*ys, X*xs
      #gdx[:,:,: , iY:iY+H, iX:iX+W] += cp.einsum('igk,gkjyx->igjyx', ggg[:,:,:,Y,X], tw)
      for g in range(ctx.groups):
        tg = cp.dot(ggg[:,g,:,Y,X].reshape(bs, -1), tw[g].reshape(rcout, -1))
