...
def call_gradient(ctx:UOp, k:UOp, needed:set[int]) -> tuple[UOp|None, ...]:
  ...
  if k.arg.grad_fxn is not None:
    ...
    def arg_grads(g):
      git = iter(g)
      return (None,) + tuple(next(git) if a.unsharded_base.op is not Ops.ALLOC else None for a in k.src[1:])
  ...
  ret_pos = [i for i, a in enumerate(args) if a.unsharded_base.op is Ops.ALLOC]
