...
pm_gradient = PatternMatcher([
  ...
  (UPat(Ops.TRUNC), lambda ctx: (UOp.const(0, shape=ctx._shape),)),
  (UPat((Ops.CMPLT, Ops.CMPNE)), lambda: (None, None)),
  (UPat(Ops.ADD), lambda ctx: (ctx, ctx)),
  (UPat(Ops.POW, name="ret", src=(UPat.var("b"), UPat.var("e"))), lambda ctx, ret, b, e:
    (ctx * (b.eq(0)&e.eq(0)).where(e, e*b.pow(e-1)),
     ctx * b.eq(0).where((e<0).where(UOp.const(-math.inf, shape=ret._shape), 0), ret*b.log2()*math.log(2.0)))),
  ...
  (UPat(Ops.WHERE, name="ret"), lambda ctx, ret: (None, ret.src[0].where(ctx, UOp.const(0, shape=ctx._shape)),
                                                  ret.src[0].where(UOp.const(0, shape=ctx._shape), ctx))),
