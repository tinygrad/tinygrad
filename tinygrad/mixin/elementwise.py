...
class ElementwiseMixin(CreationMixin):
  ...
  def _broadcasted(self, y: 'Self|ConstType|UOp', reverse: bool = False) -> tuple[Self, Self]:
    ...
    def promote(t):
      ...
      if t.dtype in dtypes.weaks and (b:=t._uop.base).op is Ops.CONST:
        return t if t.dtype == (dt:=weak_dtype(out_dtype)) else t._wrap_uop(t._uop.substitute({b: b.const_like(b.val, dt)}))
