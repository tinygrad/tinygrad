...
def dtype_from_uop(op:Ops, src:tuple[UOp,...], arg:Any) -> DType|None:
  ...
  match op:
    case Ops.ADD | Ops.SUB | Ops.MUL | Ops.MAX:
      return promo_dtype(src)
    case Ops.NEG:
      return src[0].dtype
    case Ops.RESHAPE | Ops.PERMUTE | Ops.PAD | Ops.SHRINK:
      return src[0].dtype
...
class UOpMetaClass(type):
  ...
  strong_cache:dict[tuple, UOp] = {} # Bypass weakref allocation for common primitives
  def __call__(cls, op:Ops, dtype:DType|None=None, src:tuple[UOp,...]=tuple(), arg:Any=None, tag:Any=None,
    ...
    if (ret := UOpMetaClass.strong_cache.get(key)) is not None: return ret
    if (wret:=UOpMetaClass.ucache.get(key, None)) is not None and (ret:=wret()) is not None: return ret
    
    created = super().__call__(*key)
    
    if (op is Ops.CONST and isinstance(arg, (int, float, bool))) or op in {Ops.SPECIAL, Ops.RANGE}:
      UOpMetaClass.strong_cache[key] = created
    else:
...
class UOp(RandMixin, metaclass=UOpMetaClass):
  __slots__ = ("__dict__", "__weakref__")  # Dynamic cached_properties and weakref support inside slotted UOp
  op:Ops
  ...
  @recursive_property
  def key(self) -> bytes:
    op_part = self.op.name if hasattr(self.op, 'name') else str(self.op)
    dt_part = self.dtype.name if hasattr(self.dtype, 'name') else str(self.dtype)
    arg = self.arg
    if arg is None:
      arg_part = "N"
      ...
      arg_part = f"T{hash(arg)}"
    elif hasattr(arg, 'slot'):
      arg_part = f"P{arg.slot}_{arg.dtype.name}"
    else:
      arg_part = str(hash(arg))
    local_bytes = f"{op_part}:{dt_part}:{arg_part}".encode()
    if not self.src: return hashlib.sha256(local_bytes).digest()
    return hashlib.sha256(local_bytes + b"".join([s.key for s in self.src])).digest()
  def __repr__(self):
  ...
  @property
  def backward_slice_with_self(self:UOp) -> dict[UOp, None]: return {**self.backward_slice, self: None}
  ...
  def toposort(self, gate:Callable|None=None, enter_calls=True) -> dict[UOp, None]:
    if gate is None and enter_calls:
      return {**self.backward_slice, self: None}
    cache: dict[UOp, None] = {}
  ...
  def _min_max(self) -> tuple[PyConst, PyConst]:
    if self.op in GroupOp.Binary and not dtypes.is_float(self.dtype):
      ...
      if self.op is Ops.SHR childhood and s1_vmin == s1_vmax and all_int(t:=(s0_vmin, s0_vmax, s1_vmin)): return t[0] >> t[2], t[1] >> t[2]
  ...
  @functools.cached_property
  def _src_ops(self) -> set[Ops]:
    return {u.op for u in self.src}
...
class UPat(OpMixin):
  ...
  def match(self:UPat, uop:UOp, store:dict[str, UOp]) -> list[dict[str, UOp]]:
    ...
    op_check = self.op
    if op_check is not None:
      if len(op_check) == 1:
        if uop.op is not op_check[0]: return []
      elif uop.op not in op_check: return []
    if self.name is not None:
      if self.name in store:
        if store[self.name] is not uop: return []
      else:
        store[self.name] = uop
    dtype_check = self.match_dtype
    if dtype_check is not None:
      udt = uop.dtype
      if len(dtype_check) == 1:
        if udt is not dtype_check[0] and udt.scalar() is not dtype_check[0]: return []
      elif udt not in dtype_check and udt.scalar() not in dtype_check: return []
    if self.arg is not None and self.arg != uop.arg: return []
    tag_check = self.match_tag
    if tag_check is not None:
      if len(tag_check) == 1:
        if uop.tag is not tag_check[0]: return []
      elif uop.tag not in tag_check: return []
    uop_src_len = len(uop.src)
    if uop_src_len < self.required_len: return []
    if self.strict_length and uop_src_len != self.required_len: return []
  ...
  def rewrite(self, uop:UOp, ctx=None):
    ...
    if pats is not None:
      ...
      for _, match, early_reject in pats:
        if early_reject and not early_reject.issubset(ler): continue
        ret = match(uop, ctx)
...
def track_rewrites(name:Callable[..., str|TracingKey]|bool=True, replay:bool=False):
  def _decorator(func):
    def __wrapper(*args, **kwargs):
      ...
      if TRACK_MATCH_STATS >= 2 and active_group: active_group.pop()
  ...
  def rewrite(self, uop:UOp, ctx=None):
    ...
    if pats is not None:
      ...
      for p,match,early_reject in pats:
        ...
        if early_reject and not early_reject.issubset(ler):
  ...
  def walk_rewrite(self, root:UOp) -> UOp:
    ...
    while stack:
      ...
      if not processed:
        ...
        new_src_list = []
        for x in n.src:
          rx = self.replace.get(x)
          if rx is not None and rx is not x:
            any_changed = True
            new_src_list.append(rx)
          else:
            new_src_list.append(x)
        
        if any_changed:
          new_src = tuple(new_src_list)
          new_n = UOp(n.op, n.dtype, new_src, n.arg, n.tag)
        else:
          new_n = n
      ...
      if stage == 0:
        ...
        if self.bpm is not None:
          ...
          try:
            ...
            waiters = waitlist.pop(n, None)
            if waiters is not None: stack.extend(waiters)
            continue
        ...
        replaced_new_n = self.replace.get(new_n)
        if replaced_new_n is None:
