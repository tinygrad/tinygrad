...
def fixed_pos_embedding(x):
    ...
    sinusoid_inp = (
        np.einsum("i , j -> i j", Tensor.arange(0, seq_len, dtype=dtypes.float), inv_freq).to(x)
    )
...
class DilatedTransformer:
  ...
  def forward(self, x):
    bs = x.shape[0]
    xnp = x.cpu().numpy().astype(dtypes.int32)
    onehot = np.zeros((bs, x.shape[1], self.maxlen+self.syms), dtype=dtypes.float32)
