...
extern "C" void* ptx_kernel_create(const char* source) {
	state* S = new state;
...
extern "C" void ptx_call(void* hstate, int n_args, void* args[], int blck_x, int blck_y, int blck_z, int grid_x, int grid_y, int grid_z) {
	...
	static_cast<state*>(hstate)->emuk->updateArgumentMemory();
	static_cast<state*>(hstate)->emuk->setKernelShape(blck_x, blck_y, blck_z);
	static_cast<state*>(hstate)->emuk->launchGrid(grid_x, grid_y, grid_z); }
