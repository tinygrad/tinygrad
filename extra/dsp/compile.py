...
if getenv("IOCTL"):
    import run  # noqa: F401 # pylint: disable=unused-import
...
def ion_iowr(fd, nr, args):
    ret = fcntl.ioctl(
        fd,
        (3 << 30)
        | (ctypes.sizeof(args) & 0x1FFF) << 16
        | (ord(ion.ION_IOC_MAGIC) & 0xFF) << 8
        | (nr & 0xFF),
        args,
    )
    if ret != 0:
...
if __name__ == "__main__":
    ...
    arg3 = ion.struct_ion_allocation_data(
        len=0x1000,
        align=0x1000,
        heap_id_mask=1 << msm_ion.ION_SYSTEM_HEAP_ID,
        flags=ion.ION_FLAG_CACHED,
    )
    ...
    res = libc.mmap(
        0, 0x1000, mmap.PROT_READ | mmap.PROT_WRITE, mmap.MAP_SHARED, arg2.fd, 0
    )
    ...
    cc = ClangCompiler(
        args=["--target=hexagon", "-mcpu=hexagonv65", "-fuse-ld=lld", "-nostdlib"]
    )
  ...
  """
    ...
    with open("/tmp/swag.so", "wb") as f:
        f.write(obj)

    handle = ctypes.c_int64(-1)
    adsp.remote_handle64_open(
        ctypes.create_string_buffer(
            b"file:////tmp/swag.so?entry&_modver=1.0&_dom=cdsp"
        ),
        ctypes.byref(handle),
    )
