...
def format_struct(s):
    ...
    for field in s._fields_:
        ...
        elif hasattr(dat, "_fields_"):
            sdats.append((field[0], format_struct(dat)))
        elif field[0] == "PADDING_0":
            pass
        else:
    ...
    if fn == "/dev/ion":
        ...
        if nr == 8:
                ...
                for arg in range(in_args + out_args):
                    ...
                    if st.pra[arg].buf.pv is not None:
                        ...
                        if st.pra[arg].buf.len == 0x68:
                            print(
                                bytearray(
                                    to_mv(st.pra[arg].buf.pv, st.pra[arg].buf.len)
                                )
                            )

                        cut = (
                            0x2000
                            if st.pra[arg].buf.len == 0x2000
                            or st.pra[arg].buf.len == 0x258
                            else 0x100
                        )
            ...
            print(
                ...
                "FASTRPC_IOCTL_INIT",
                format_struct(
                    ini := get_struct(argp, qcom_dsp.struct_fastrpc_ioctl_init)
                ),
            )
            ...
            print(
                ...
                format_struct(
                    ini := get_struct(argp, qcom_dsp.struct_fastrpc_ioctl_invoke_attrs)
                ),
            )
        elif nr == 12:
            print(
                ...
                format_struct(get_struct(argp, qcom_dsp.struct_fastrpc_ioctl_control)),
            )
        else:
...
def listner_worker():
    ...
    while True:
        ...
        qcom_dsp.FASTRPC_IOCTL_INVOKE(
            rpcfd, handle=0x3, sc=0x04020200, pra=req_args
        )  # listener
        ...
        if sc == 0x20200:  # greating?
            ...
            st = qcom_dsp.FASTRPC_IOCTL_MMAP(
                rpcfd, fd=-1, flags=rflags, vaddrin=0, size=sz
            )
            out_args[0].cast("Q")[0] = 0
            out_args[0].cast("Q")[1] = st.vaddrout
            prev_res = 0
        else:
            raise RuntimeError(f"Unknown {sc=:X}")
...
if __name__ == "__main__":
    ...
    with contextlib.suppress(RuntimeError, OSError):
        qcom_dsp.ION_IOC_FREE(ionfd, handle=0)
    ...
    fastrpc_shell = memoryview(
        bytearray(pathlib.Path("/vendor/dsp/cdsp/fastrpc_shell_3").read_bytes())
    )
    shell_mem = qcom_dsp.ION_IOC_ALLOC(
        ionfd,
        len=round_up(fastrpc_shell.nbytes, 0x1000),
        align=0x1000,
        heap_id_mask=0x2000000,
        flags=0x1,
    )
    shell_mapped = qcom_dsp.ION_IOC_MAP(ionfd, handle=shell_mem.handle)
    fastrpc_shell_addr = libc.mmap(
        0,
        shell_mem.len,
    ...
    init = qcom_dsp.FASTRPC_IOCTL_INIT(
        rpcfd,
        flags=0x1,
        file=fastrpc_shell_addr,
        filelen=fastrpc_shell.nbytes,
        filefd=shell_mapped.fd,
    )
    ...
    a2 = memoryview(
        bytearray(
            b"file:///libcalculator_skel.so?calculator_skel_handle_invoke&_modver=1.0&_dom=cdsp\0"
        )
    )
    ...
    pra[2].buf.len = 8
    qcom_dsp.FASTRPC_IOCTL_INVOKE(
        rpcfd, handle=prg_handle, sc=(2 << 24) | (2 << 16) | (1 << 8), pra=pra
    )
