...
def check(status):
    if status != 0:
        hsa.hsa_status_string(
            status, ctypes.byref(status_str := ctypes.POINTER(ctypes.c_char)())
        )
        raise RuntimeError(
            f"HSA Error {status}: {ctypes.string_at(status_str).decode()}"
        )
...
class AQLQueue:
    def __init__(self, device, sz=-1):
        ...
        check(
            hsa.hsa_agent_get_info(
                self.device.agent,
                hsa.HSA_AGENT_INFO_QUEUE_MAX_SIZE,
                ctypes.byref(max_queue_size := ctypes.c_uint32()),
            )
        )
        queue_size = min(max_queue_size.value, sz) if sz != -1 else max_queue_size.value

        null_func = ctypes.CFUNCTYPE(
            None,
            hsa.hsa_status_t,
            ctypes.POINTER(hsa.struct_hsa_queue_s),
            ctypes.c_void_p,
        )()
        ...
        self.write_addr_end = (
            self.queue_base + self.queue_size - 1
        )  # precalc saves some time
        self.available_packet_slots = self.hw_queue.contents.size

        check(
            hsa.hsa_amd_queue_set_priority(
                self.hw_queue, hsa.HSA_AMD_QUEUE_PRIORITY_HIGH
            )
        )
    ...
    def submit_kernel(
        self, prg, global_size, local_size, kernargs, completion_signal=None
    ):
        ...
        packet.completion_signal = (
            completion_signal if completion_signal else EMPTY_SIGNAL
        )
    ...
    def submit_barrier(self, wait_signals=None, completion_signal=None):
        ...
        for i in range(5):
            packet.dep_signal[i] = (
                wait_signals[i]
                if wait_signals and len(wait_signals) > i
                else EMPTY_SIGNAL
            )
        packet.reserved2 = 0
        packet.completion_signal = (
    ...
    def blit_packets(self, packet_addr, packet_cnt):
        ...
        tail_blit_packets = min(
            (self.queue_base + self.queue_size - self.write_addr) // AQL_PACKET_SIZE,
            packet_cnt,
        )
        rem_packet_cnt = packet_cnt - tail_blit_packets
        ctypes.memmove(
            self.write_addr, packet_addr, AQL_PACKET_SIZE * tail_blit_packets
        )
        if rem_packet_cnt > 0:
            ctypes.memmove(
                self.queue_base,
                packet_addr + AQL_PACKET_SIZE * tail_blit_packets,
                AQL_PACKET_SIZE * rem_packet_cnt,
            )
    ...
    def _wait_queue(self, need_packets=1):
        while self.available_packet_slots < need_packets:
            ...
            self.available_packet_slots = self.queue_size // AQL_PACKET_SIZE - (
                self.next_doorbell_index - rindex
            )
    ...
    def _submit_packet(self, cnt=1):
        ...
        hsa.hsa_signal_store_screlease(
            self.hw_queue.contents.doorbell_signal, self.next_doorbell_index - 1
        )

        self.write_addr += AQL_PACKET_SIZE * cnt
        if self.write_addr > self.write_addr_end:
            self.write_addr = (
                self.queue_base + (self.write_addr - self.queue_base) % self.queue_size
            )
...
def scan_agents():
    ...
    def __scan_agents(agent, data):
        status = hsa.hsa_agent_get_info(
            agent,
            hsa.HSA_AGENT_INFO_DEVICE,
            ctypes.byref(device_type := hsa.hsa_device_type_t()),
        )
        if status == 0:
...
def find_memory_pool(agent, segtyp=-1, location=-1):
    ...
    def __filter_amd_memory_pools(mem_pool, data):
        check(
            hsa.hsa_amd_memory_pool_get_info(
                mem_pool,
                hsa.HSA_AMD_MEMORY_POOL_INFO_SEGMENT,
                ctypes.byref(segment := hsa.hsa_amd_segment_t()),
            )
        )
        ...
        check(
            hsa.hsa_amd_memory_pool_get_info(
                mem_pool,
                hsa.HSA_AMD_MEMORY_POOL_INFO_LOCATION,
                ctypes.byref(loc := hsa.hsa_amd_memory_pool_location_t()),
            )
        )
        ...
        check(
            hsa.hsa_amd_memory_pool_get_info(
                mem_pool,
                hsa.HSA_AMD_MEMORY_POOL_INFO_SIZE,
    ...
    hsa.hsa_amd_agent_iterate_memory_pools(
        agent,
        __filter_amd_memory_pools,
        ctypes.byref(region := hsa.hsa_amd_memory_pool_t()),
    )
    return region
