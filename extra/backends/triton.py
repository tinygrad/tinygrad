...
def fill_dims_for_idx(idx, dims):
    return (
        "(" + idx + "+ (" + (f"0*({'+'.join(d for d in dims)})))") if len(dims) else idx
    )
...
def remove_single_scalar_curly_braces(ptx_code):
    return "\n".join(
        [re.sub(r"\{\s*(%\w+)\s*\}", r"\1", line) for line in ptx_code.split("\n")]
    )


def render_const(args, dtype: DType):
    return (
        (("-" if args < 0 else "") + 'tl.where(1,float("inf"),0)')
        if math.isinf(args)
        else (
            'tl.where(1,float("nan"),0)'
            if math.isnan(args)
            else f"{int(args)}" if dtypes.is_int(dtype) else str(args)
        )
    )
...
def define_scalar(local_size, dtype, args):
    if len(local_size) > 0:
        return f"tl.full(({','.join([str(next_power_of_2(x)) for x in local_size])},),{render_const(args,dtype)}, dtype={triton_dtypes[dtype]})"
...
def uops_to_triton(function_name: str, uops: List[UOp]):
    ...
    code_for_op: Final[Dict[Op, Callable]] = {
        ...
        BinaryOps.DIV: lambda x, y,: (
            f"({x}/{y})"
            if y != "0.0"
            else f"{x}*tl.where({x}==0.0, float('nan'), float('inf'))"
        ),
    ...
    def int_div(x, y):
        return (
            f"({x}//{y})"
            if y != "0"
            else f"{x}*tl.where({x}==0, float('nan'), float('inf'))"
        )
    ...
    for u in uops:
        ...
        if uop == Ops.LOOP:
            ...
            if child_count[u] <= 1 or dtypes.is_int(dtype):
                r[u] = (
                    int_div(*[r[x] for x in vin])
                    if args == BinaryOps.DIV and dtypes.is_int(dtype)
                    else val
                )
            else:
                kk(f"{ssa(u, 'alu')} = ({val})")
            ...
            if len(vin) == 2:
                kk(
                    f"{ssa(u, 'val')} = {render_cast(f'tl.load({r[vin[0]]} + { fill_dims_for_idx(r[vin[1]], dims)}, mask = {render_valid(valid)})', dtype)}"
                )
            else:
                kk(
                    f"{ssa(u, 'val')} = {render_cast(f'tl.where({r[vin[2]]}, tl.load({r[vin[0]]}+{fill_dims_for_idx(r[vin[1]],dims)} , mask={render_valid(valid+[r[vin[2]]])}), 0.0)', dtype)}"
                )
        elif uop == Ops.DEFINE_ACC:
            kk(
                f"{ssa(u, 'acc')} = {define_scalar(local_size, dtype, args).replace('//', '/')}"
            )
        elif uop == Ops.CONST:
            r[u] = define_scalar([], dtype, args)
        elif uop == Ops.ASSIGN:
            ...
            kk(
                f"{'if '+r[vin[3]]+': ' if len(vin)>3 else ''}tl.store({r[vin[0]]} + {r[vin[1]]}, {r[vin[2]].replace('//', '/')}, mask = {render_valid(valid)}) "
            )
        elif uop == Ops.DEFINE_GLOBAL:
            bufs.append(args)
            signatures.append(
                "*" if isinstance(dtype, PtrDType) else "" + signature_dtypes[dtype]
            )
            r[u] = args
            ...
            r[u] = args[1]
        elif uop == Ops.CAST and dtype is not None:
            r[u] = render_cast(r[vin[0]], dtype, isinstance(args, tuple) and args[1])
        else:
    ...
    for i, line in enumerate(list(filter(lambda line: "tl.arange" in line, kernel))):
        kernel[
            kernel.index(line)
        ] += f"[{', '.join([':' if i == j else 'None' for j in range(len(local_size))])}]"
    prg += "\n".join(kernel)
    ...
    linecache.getlines = lambda filename, module_globals=None: (
        prg.splitlines(keepends=True)
        if "<triton>" == filename
        else getlines(filename, module_globals)
    )
    ...
    prg = remove_single_scalar_curly_braces(
        compiled.asm["ptx"].split(".file")[0].split(".visible .func")[0]
    )
    max_local_size = [
        int(x) for x in prg.split(".maxntid ")[1].split("\n")[0].split(", ")
    ]
