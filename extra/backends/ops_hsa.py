...
if getenv("IOCTL"):
    import extra.hip_gpu_driver.hip_ioctl  # noqa: F401
...
class HSAProfiler:
    ...
    def track(self, signal, device, name, is_copy=False):
        self.tracked_signals[device].append((signal, name, is_copy))
    ...
    def process(self, device):
        ...
        for sig, name, is_copy in self.tracked_signals[device]:
            if is_copy:
                check(
                    hsa.hsa_amd_profiling_get_async_copy_time(
                        sig, ctypes.byref(timings := self.copy_timings)
                    )
                )
            else:
                check(
                    hsa.hsa_amd_profiling_get_dispatch_time(
                        device.agent, sig, ctypes.byref(timings := self.disp_timings)
                    )
                )  # type:ignore
    ...
    def save(self, path):
        ...
        for i in range(len(HSADevice.devices)):
            mjson.append(
                {"name": "process_name", "ph": "M", "pid": i, "args": {"name": "HSA"}}
            )
            mjson.append(
                {
                    "name": "thread_name",
...
class HSAProgram:
    def __init__(self, device: HSADevice, name: str, lib: bytes):
        ...
        self.exec = init_c_var(
            hsa.hsa_executable_t(),
            lambda x: check(
                hsa.hsa_executable_create_alt(
        ...
        self.kernargs_segment_size = init_c_var(
            ctypes.c_uint32(),
            lambda x: check(
                hsa.hsa_executable_symbol_get_info(
                    self.kernel,
                    hsa.HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_KERNARG_SEGMENT_SIZE,
                    ctypes.byref(x),
                )
            ),
        ).value  # noqa: E501
        self.group_segment_size = init_c_var(
            ctypes.c_uint32(),
            lambda x: check(
                hsa.hsa_executable_symbol_get_info(
                    self.kernel,
                    hsa.HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_GROUP_SEGMENT_SIZE,
                    ctypes.byref(x),
                )
            ),
        ).value  # noqa: E501
        self.private_segment_size = init_c_var(
            ctypes.c_uint32(),
            lambda x: check(
                hsa.hsa_executable_symbol_get_info(
                    self.kernel,
                    hsa.HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_PRIVATE_SEGMENT_SIZE,
                    ctypes.byref(x),
                )
            ),
        ).value  # noqa: E501
    ...
    def __del__(self):
        ...
        if hasattr(self, "code_reader"):
            check(hsa.hsa_code_object_reader_destroy(self.code_reader))
        if hasattr(self, "exec"):
            check(hsa.hsa_executable_destroy(self.exec))
    ...
    def __call__(
        ...
        *args,
        ...
        if not hasattr(self, "args_struct_t"):
            self.args_struct_t = init_c_struct_t(
                tuple(
                    [(f"f{i}", ctypes.c_void_p) for i in range(len(args))]
                    + [(f"v{i}", ctypes.c_int) for i in range(len(vals))]
                )
            )
            if ctypes.sizeof(self.args_struct_t) != self.kernargs_segment_size:
                raise RuntimeError(
                    f"HSAProgram.__call__: incorrect args struct size {ctypes.sizeof(self.args_struct_t)} != {self.kernargs_segment_size}"
                )
        ...
        if self.kernargs_segment_size > 0:
            ...
            for i in range(len(args)):
                args_st.__setattr__(f"f{i}", args[i])
            for i in range(len(vals)):
                args_st.__setattr__(f"v{i}", vals[i])
            self.device.flush_hdp()

        signal = self.device.alloc_signal(reusable=True) if wait or PROFILE else None
        self.device.hw_queue.submit_kernel(
            self, global_size, local_size, kernargs, completion_signal=signal
        )
        if PROFILE:
            Profiler.track(signal, self.device, self.name)
        if wait:
            ...
            check(
                hsa.hsa_amd_profiling_get_dispatch_time(
                    self.device.agent,
                    signal,
                    ctypes.byref(timings := hsa.hsa_amd_profiling_dispatch_time_t()),
                )
            )
...
class HSAAllocator(LRUAllocator):
        ...
        if options.host:
            check(
                hsa.hsa_amd_memory_pool_allocate(
                    HSADevice.cpu_mempool,
                    size,
                    0,
                    ctypes.byref(mem := ctypes.c_void_p()),
                )
            )
            check(
                hsa.hsa_amd_agents_allow_access(
                    2,
                    (hsa.hsa_agent_t * 2)(HSADevice.cpu_agent, self.device.agent),
                    None,
                    mem,
                )
            )
            return mem.value
        c_agents = (hsa.hsa_agent_t * len(HSADevice.agents[hsa.HSA_DEVICE_TYPE_GPU]))(
            *HSADevice.agents[hsa.HSA_DEVICE_TYPE_GPU]
        )
        check(
            hsa.hsa_amd_memory_pool_allocate(
                self.device.gpu_mempool, size, 0, ctypes.byref(buf := ctypes.c_void_p())
            )
        )
        check(
            hsa.hsa_amd_agents_allow_access(
                len(HSADevice.agents[hsa.HSA_DEVICE_TYPE_GPU]), c_agents, None, buf
            )
        )
        return buf.value
    ...
    def _copyin(self, dest: T, src: memoryview):
        ...
        self.device.hw_queue.submit_barrier(
            [], sync_signal := self.device.alloc_signal(reusable=True)
        )
        mem = self._alloc(src.nbytes, BufferSpec(host=True))
        ctypes.memmove(mem, from_mv(src), src.nbytes)
        check(
            hsa.hsa_amd_memory_async_copy_on_engine(
                dest,
                self.device.agent,
                mem,
                HSADevice.cpu_agent,
        ...
        if PROFILE:
            Profiler.track(
                copy_signal,
                self.device,
                f"copyin: CPU -> HSA:{self.device.device_id}",
                is_copy=True,
            )
    ...
    def copy_from_fd(self, dest, fd, offset, size):
        ...
        if not hasattr(self, "hb"):
            ...
            self.hb_signals = [
                self.device.alloc_signal(reusable=False) for _ in range(2)
            ]
            self.hb_polarity = 0
            self.sdma = [hsa.HSA_AMD_SDMA_ENGINE_0, hsa.HSA_AMD_SDMA_ENGINE_1]
            for sig in self.hb_signals:
                hsa.hsa_signal_store_relaxed(sig, 0)
        ...
        for local_offset in range(0, size + minor_offset, CHUNK_SIZE):
            local_size = min(
                round_up(size + minor_offset, PAGE_SIZE) - local_offset, CHUNK_SIZE
            )
            ...
            hsa.hsa_signal_wait_scacquire(
                self.hb_signals[self.hb_polarity],
                hsa.HSA_SIGNAL_CONDITION_LT,
                1,
                (1 << 64) - 1,
                hsa.HSA_WAIT_STATE_ACTIVE,
            )
            self.device.reusable_signals.append(
                self.hb_signals[self.hb_polarity]
            )  # it's free now and can be reused
            ...
            check(
                hsa.hsa_amd_memory_async_copy_on_engine(
                    dest + copied_in,
                    self.device.agent,
                    self.hb[self.hb_polarity] + minor_offset,
                    HSADevice.cpu_agent,
                    copy_size,
                    1,
                    ctypes.byref(sync_signal),
                    self.hb_signals[self.hb_polarity],
                    self.sdma[self.hb_polarity],
                    True,
                )
            )
            copied_in += copy_size
        ...
        if copies_called > 1:
            wait_signals.append(self.hb_signals[self.hb_polarity])
    ...
    def _copyout(self, dest: memoryview, src: T):
        ...
        check(
            hsa.hsa_amd_memory_lock_to_pool(
                from_mv(dest),
                dest.nbytes,
                c_agents,
                2,
                HSADevice.cpu_mempool,
                0,
                ctypes.byref(addr := ctypes.c_void_p()),
            )
        )
        check(
            hsa.hsa_amd_memory_async_copy(
                addr,
                HSADevice.cpu_agent,
                src,
                self.device.agent,
        ...
        if PROFILE:
            Profiler.track(
                copy_signal,
                self.device,
                f"copyout: HSA:{self.device.device_id} -> CPU",
                is_copy=True,
            )

    def transfer(self, dest: T, src: T, sz: int, src_dev=None, dest_dev=None):
        src_dev.hw_queue.submit_barrier(
            [], sync_signal_1 := src_dev.alloc_signal(reusable=True)
        )
        dest_dev.hw_queue.submit_barrier(
            [], sync_signal_2 := dest_dev.alloc_signal(reusable=True)
        )
...
class HSADevice(Compiled):
    ...
    def __init__(self, device: str = ""):
        if not HSADevice.agents:
            ...
            HSADevice.cpu_mempool = find_memory_pool(
                HSADevice.cpu_agent,
                segtyp=hsa.HSA_AMD_SEGMENT_GLOBAL,
                location=hsa.HSA_AMD_MEMORY_POOL_LOCATION_CPU,
            )
            if PROFILE:
                check(hsa.hsa_amd_profiling_async_copy_enable(1))

        self.device_id = int(device.split(":")[1]) if ":" in device else 0
        self.agent = HSADevice.agents[hsa.HSA_DEVICE_TYPE_GPU][self.device_id]
        self.gpu_mempool = find_memory_pool(
            self.agent,
            segtyp=hsa.HSA_AMD_SEGMENT_GLOBAL,
            location=hsa.HSA_AMD_MEMORY_POOL_LOCATION_GPU,
        )
        ...
        check(
            hsa.hsa_agent_get_info(
                self.agent,
                hsa.HSA_AGENT_INFO_NAME,
                ctypes.byref(agent_name_buf := ctypes.create_string_buffer(256)),
            )
        )
        self.arch = ctypes.string_at(agent_name_buf).decode()

        check(
            hsa.hsa_system_get_info(
                hsa.HSA_SYSTEM_INFO_TIMESTAMP_FREQUENCY,
                ctypes.byref(gpu_freq := ctypes.c_uint64()),
            )
        )
        ...
        check(
            hsa.hsa_agent_get_info(
                self.agent,
                hsa.HSA_AMD_AGENT_INFO_HDP_FLUSH,
                ctypes.byref(hdp_flush := hsa.hsa_amd_hdp_flush_t()),
            )
        )
        ...
        super().__init__(
            ...
            HSAAllocator(self),
            HIPRenderer(),
            HSACompiler(self.arch),
            functools.partial(HSAProgram, self),
            HSAGraph,
        )
        ...
        self.signal_pool = [
            init_c_var(
                hsa.hsa_signal_t(),
                lambda x: check(hsa.hsa_signal_create(1, 0, None, ctypes.byref(x))),
            )
            for _ in range(4096)
        ]
    ...
    def synchronize(self):
        ...
        for sig in self.reusable_signals:
        ...
        for opaque_to_free in self.delayed_free:
            check(hsa.hsa_amd_memory_pool_free(opaque_to_free))
    ...
    def synchronize_system():
        for d in HSADevice.devices:
            d.synchronize()

    def alloc_signal(self, reusable=False):
        if len(self.signal_pool):
            signal = self.signal_pool.pop()
        else:
            check(
                hsa.hsa_amd_signal_create(
                    1, 0, None, 0, ctypes.byref(signal := hsa.hsa_signal_t())
                )
            )

        # reusable means a signal could be reused after synchronize for the device it's allocated from is called.
        if reusable:
            self.reusable_signals.append(signal)
        return signal

    def alloc_kernargs(self, sz):
        if (
            self.kernarg_next_addr + sz
            >= self.kernarg_start_addr + self.kernarg_pool_sz
        ):
            self._new_kernargs_region(int(self.kernarg_pool_sz * 2))
    ...
    def _new_kernargs_region(self, sz: int):
        if hasattr(self, "kernarg_start_addr"):
    ...
    def flush_hdp(self):
        self.hdp_flush.HDP_MEM_FLUSH_CNTL[0] = 1
