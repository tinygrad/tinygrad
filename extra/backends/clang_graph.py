...
class ClangGraph(GraphRunner):
    def __init__(
        ...
        prgs = "\n".join(
            dedup([cast(CompiledRunner, ji.prg).p.src for ji in jit_cache])
        )
        args = [
            f"{render_dtype(x.dtype)}* arg{i}" for i, x in enumerate(input_rawbuffers)
        ]
        ...
        for ji in jit_cache:
            ...
            for buf in ji.bufs:
                ...
                if buf in input_rawbuffers:
                    ...
                    args.append(
                        f"({render_dtype(buf.dtype)}*)0x{ctypes.addressof(buf._buf):X}"
                    )
            args += [x.expr for x in cast(CompiledRunner, ji.prg).p.vars]
            code.append(
                f"  {cast(CompiledRunner, ji.prg).p.function_name}({','.join(args)});"
            )
        code.append("}")
        if DEBUG >= 4:
            print("\n".join(code))
        ...
        self._prg = ClangProgram(
            "batched", compiler.compile(prgs + "\n" + "\n".join(code))
        )  # no point in caching the pointers
    ...
    def __call__(
        ...
        return cpu_time_execution(
            lambda: self._prg(
                ...
                *[x[1] for x in sorted(var_vals.items(), key=lambda x: x[0].expr)],
            ),
            enable=wait,
        )
