...
constexpr int VEC = 8;   // bf16 elements per thread per iteration (16 bytes = one float4 load)
...
fused_silu_mul_bwd_w13(
    __hip_bfloat16*       __restrict__ grad_xw13_out,    // bf16, 2*N_ELEMS (grad_xw1 || grad_xw3)
    const __hip_bfloat16* __restrict__ xw13,             // bf16, 2*N_ELEMS (xw1 || xw3)
    ...
    const __hip_bfloat16* __restrict__ amax_state)       // bf16 scalar
{
  ...
  const int stride_elems = NUM_WG * THREADS_PER_WG * VEC;   // elems per grid-stride step

  const float scale = FP8_MAX / (static_cast<float>(*amax_state) + 1e-8f);

  // grid-stride over 8-element groups using 128-bit loads/stores
  for (int base = gid * VEC; base < N_ELEMS; base += stride_elems) {
    // 16-byte loads (float4 = 8 bf16 = 128 bits)
    ...
    *reinterpret_cast<float4*>(&grad_xw13_out[base])           = *reinterpret_cast<float4*>(out1);
    *reinterpret_cast<float4*>(&grad_xw13_out[base + N_ELEMS]) = *reinterpret_cast<float4*>(out3);
  }
}
