...
from extra.nvdec.hevc import NAL_UNIT_START_CODE, NAL_UNIT_START_CODE_SIZE, NAL_UNIT_HEADER_SIZE, HevcNalUnitType, BitReader
...
class NVVidCtx:
  def __init__(self, dev:NVDECDevice):
    ...
    self.luma_chroma_bufs = [dev.allocator.alloc(0x2fd000) for i in range(3)]
...
class NVVidDecoder:
  ...
  def stream_slice(self, nal_unit_type, data):
    if nal_unit_type == HevcNalUnitType.SPS_NUT:
      ...
      self.desc.curr_pic_idx = next(self.pic_counter)
      ...
      NVVideoQueue().wait(self.ctx.dev.timeline_signal, self.ctx.dev.timeline_value - 1) \
                    .decode_hevc_chunk(self.ctx.buf_in_data, self.ctx.luma_chroma_bufs, self.ctx.coloc_buf, self.ctx.filter_buf,
                                       self.ctx.scaling_list, self.ctx.tile_sizes, self.ctx.status_buf, self.ctx.pic_desc, self.desc.curr_pic_idx) \
    ...
    elif nal_unit_type == HevcNalUnitType.TRAIL_R:
      reader = BitReader(hevc_get_rbsp(data, off=5))
      ...
      if not dependent_slice_segment_flag:
        ...
        print('slice_type', slice_type)
        if self.pps_output_flag_present_flag: pic_output_flag = reader.u(1)
      ...
      # hardcoded for now:
      # self.desc.num_ref_frames=1
      # self.desc.RefDiffPicOrderCnts[0] = 4
      self.desc.sw_hdr_skip_length = 19 # 16 + 3 ? 
      # self.desc.sw_start_code_e = 0
