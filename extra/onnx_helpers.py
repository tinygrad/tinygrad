...
def get_example_inputs(graph_inputs: dict[str, OnnxValue]):
    ...
    for name, spec in graph_inputs.items():
        assert (
            not spec.is_optional and not spec.is_sequence
        ), "only allow tensor input for now"
        shape = tuple(dim if isinstance(dim, int) else 1 for dim in spec.shape)
        value = Tensor(
            np.random.uniform(size=shape).astype(_to_np_dtype(spec.dtype)) * 8
        ).realize()
...
def validate(onnx_file, inputs, rtol=1e-5, atol=1e-5):
    ...
    np_inputs = {
        k: v.numpy() if isinstance(v, Tensor) else v for k, v in inputs.items()
    }
    ...
    for k in tinygrad_out.keys():
        ...
        if tiny_v is None:
            ...
            np.testing.assert_allclose(
                ...
                rtol=rtol,
                atol=atol,
                err_msg=f"For tensor '{k}' in {tinygrad_out.keys()}",
            )
