...
def dump_struct(st):
    ...
    for v in type(st)._fields_:
        print(f"{v[0]}={getattr(st, v[0])}", end=" ")
    print("}")
...
def format_struct(s):
    ...
    for field in s._fields_:
        ...
        if isinstance(dat, int):
            sdats.append(f"{field[0]}:0x{dat:X}")
        else:
            sdats.append(f"{field[0]}:{dat}")
    return sdats
...
def install_hook(c_function, python_function):
    ...
    python_function_addr = ctypes.cast(
    ...
    assert ret == 0
    ret = libc.mprotect(
        ctypes.c_ulong((ctypes.addressof(orig_func) // 0x1000) * 0x1000), 0x3000, 7
    )
    assert ret == 0
...
nvcmds = {
    getattr(nv_gpu, x): (
        ...
        getattr(
            nv_gpu,
            "struct_" + x + "_PARAMS",
            getattr(nv_gpu, "struct_" + x.replace("_CMD_", "_") + "_PARAMS", None),
        ),
    )
    for x in dir(nv_gpu)
    if x.startswith("NV")
    and x[6:].startswith("_CTRL_")
    and isinstance(getattr(nv_gpu, x), int)
}
...
def get_classes():
    ...
    clss = re.search(
        r"NV01_ROOT.*?NV_SEMAPHORE_SURFACE = \(0x000000da\) # macro", hdrpy, re.DOTALL
    ).group()
...
nvuvms = {
    getattr(nv_gpu, x): x
    for x in dir(nv_gpu)
    if x.startswith("UVM_") and nv_gpu.__dict__.get(x + "_PARAMS")
}
nvqcmds = {
    int(getattr(nv_gpu, x)): x
    for x in dir(nv_gpu)
    if x[:7] in {"NVC6C0_", "NVC56F_", "NVC6B5_"}
    and isinstance(getattr(nv_gpu, x), int)
}
...
def ioctl(fd, request, argp):
    ...
    ret = libc.syscall(
        IOCTL_SYSCALL, ctypes.c_int(fd), ctypes.c_ulong(request), ctypes.c_void_p(argp)
    )
    ...
    if itype == ord(nv_gpu.NV_IOCTL_MAGIC):
        if nr == nv_gpu.NV_ESC_RM_CONTROL:
            ...
            if s.cmd in nvcmds:
                ...
                if getenv("IOCTL", 0) >= 1:
                    print(
                        f"NV_ESC_RM_CONTROL    cmd={name:30s} hClient={s.hClient}, hObject={s.hObject}, flags={s.flags}, params={s.params}, paramsSize={s.paramsSize}, status={s.status}"
                    )

                if struc is not None:
                    dump_struct(get_struct(s.params, struc))
                elif hasattr(nv_gpu, name + "_PARAMS"):
                    dump_struct(get_struct(argp, getattr(nv_gpu, name + "_PARAMS")))
                elif name == "NVA06C_CTRL_CMD_GPFIFO_SCHEDULE":
                    dump_struct(
                        get_struct(argp, nv_gpu.NVA06C_CTRL_GPFIFO_SCHEDULE_PARAMS)
                    )
                elif name == "NV83DE_CTRL_CMD_GET_MAPPINGS":
                    dump_struct(
                        get_struct(
                            s.params, nv_gpu.NV83DE_CTRL_DEBUG_GET_MAPPINGS_PARAMETERS
                        )
                    )
            else:
                if getenv("IOCTL", 0) >= 1:
            ...
            if getenv("IOCTL", 0) >= 1:
                print(
                    f"NV_ESC_RM_ALLOC    hClass={nvclasses.get(s.hClass, f'unk=0x{s.hClass:X}'):30s}, hRoot={s.hRoot}, hObjectParent={s.hObjectParent}, pAllocParms={s.pAllocParms}, hObjectNew={s.hObjectNew} status={s.status}"
                )
            if s.pAllocParms is not None:
                if s.hClass == nv_gpu.NV01_DEVICE_0:
                    dump_struct(
                        get_struct(s.pAllocParms, nv_gpu.NV0080_ALLOC_PARAMETERS)
                    )
                if s.hClass == nv_gpu.FERMI_VASPACE_A:
                    dump_struct(
                        get_struct(
                            s.pAllocParms, nv_gpu.NV_VASPACE_ALLOCATION_PARAMETERS
                        )
                    )
                if s.hClass == nv_gpu.NV50_MEMORY_VIRTUAL:
                    dump_struct(
                        get_struct(s.pAllocParms, nv_gpu.NV_MEMORY_ALLOCATION_PARAMS)
                    )
                if s.hClass == nv_gpu.NV1_MEMORY_USER:
                    dump_struct(
                        get_struct(s.pAllocParms, nv_gpu.NV_MEMORY_ALLOCATION_PARAMS)
                    )
                ...
                if s.hClass == nv_gpu.GT200_DEBUGGER:
                    dump_struct(
                        get_struct(s.pAllocParms, nv_gpu.NV83DE_ALLOC_PARAMETERS)
                    )
                if s.hClass == nv_gpu.AMPERE_CHANNEL_GPFIFO_A:
                    sx = get_struct(
                        s.pAllocParms, nv_gpu.NV_CHANNELGPFIFO_ALLOCATION_PARAMETERS
                    )
                    dump_struct(sx)
                    gpus_fifo.append((sx.gpFifoOffset, sx.gpFifoEntries))
                if s.hClass == nv_gpu.KEPLER_CHANNEL_GROUP_A:
                    dump_struct(
                        get_struct(
                            s.pAllocParms, nv_gpu.NV_CHANNEL_GROUP_ALLOCATION_PARAMETERS
                        )
                    )
            if s.hClass == nv_gpu.TURING_USERMODE_A:
                gpus_user_modes.append(s.hObjectNew)
        elif nr == nv_gpu.NV_ESC_RM_MAP_MEMORY:
            # nv_ioctl_nvos33_parameters_with_fd
            if getenv("IOCTL", 0) >= 1:
                s = get_struct(argp, nv_gpu.NVOS33_PARAMETERS)
                print(
                    f"NV_ESC_RM_MAP_MEMORY   hClient={s.hClient}, hDevice={s.hDevice}, hMemory={s.hMemory}, length={s.length} flags={s.flags} pLinearAddress={s.pLinearAddress}"
                )
        elif nr == nv_gpu.NV_ESC_RM_UPDATE_DEVICE_MAPPING_INFO:
            if getenv("IOCTL", 0) >= 1:
                s = get_struct(argp, nv_gpu.NVOS56_PARAMETERS)
                print(
                    f"NV_ESC_RM_UPDATE_DEVICE_MAPPING_INFO   hClient={s.hClient}, hDevice={s.hDevice}, hMemory={s.hMemory}, pOldCpuAddress={s.pOldCpuAddress} pNewCpuAddress={s.pNewCpuAddress} status={s.status}"
                )
        elif nr == nv_gpu.NV_ESC_RM_ALLOC_MEMORY:
            if getenv("IOCTL", 0) >= 1:
                s = get_struct(argp, nv_gpu.nv_ioctl_nvos02_parameters_with_fd)
                print(
                    f"NV_ESC_RM_ALLOC_MEMORY  fd={s.fd}, hRoot={s.params.hRoot}, hObjectParent={s.params.hObjectParent}, hObjectNew={s.params.hObjectNew}, hClass={s.params.hClass}, flags={s.params.flags}, pMemory={s.params.pMemory}, limit={s.params.limit}, status={s.params.status}"
                )
        elif nr == nv_gpu.NV_ESC_ALLOC_OS_EVENT:
            if getenv("IOCTL", 0) >= 1:
                s = get_struct(argp, nv_gpu.nv_ioctl_alloc_os_event_t)
                print(
                    f"NV_ESC_ALLOC_OS_EVENT  hClient={s.hClient} hDevice={s.hDevice} fd={s.fd} Status={s.Status}"
                )
        ...
        if getenv("IOCTL", 0) >= 1:
            ...
            if nvuvms.get(request) is not None:
                dump_struct(
                    get_struct(argp, getattr(nv_gpu, nvuvms.get(request) + "_PARAMS"))
                )
...
def _dump_qmd(address, packets):
    ...
    while i < packets:
        ...
        if getenv("IOCTL", 0) >= 1:
            ...
            for j in range(size):
                print(f"\t\t\t{j}: {gpfifo[i+j+1]} | 0x{gpfifo[i+j+1]:x}")
        if mthd == 792:
...
def before_launch():
    _dump_gpfifo("before launch")


def collect_last_launch_state():
    return _dump_gpfifo("after launch")
...
def compare_launch_state(states, good_states):
    ...
    if len(states) != 1 or len(good_states) != 1:
        return (
            False,
            f"Some states not captured. {len(states)}!=1 || {len(good_states)}!=1",
        )
    ...
    for i in range(len(states)):
        ...
        for n in [
            ...
            if getattr(state, n) != getattr(good_state, n):
                return (
                    False,
                    f"Field {n} mismatch: {getattr(state, n)} vs {getattr(good_state, n)}",
                )
        ...
        if (
            state.shader_local_memory_high_size
            < good_state.shader_local_memory_high_size
            and good_state.shader_local_memory_high_size > 0x640
        ):
            return (
                False,
                f"Field shader_local_memory_high_size mismatch: {state.shader_local_memory_high_size}vs{good_state.shader_local_memory_high_size}",
            )

        # TODO: Can't request more, since it might not be optimal, but need to investigate their formula for this.. #7133
        if (
            state.min_sm_config_shared_mem_size
            > good_state.min_sm_config_shared_mem_size
            and good_state.min_sm_config_shared_mem_size > 5
        ):
            return (
                False,
        ...
        if (
            state.target_sm_config_shared_mem_size
            > good_state.target_sm_config_shared_mem_size
            and good_state.target_sm_config_shared_mem_size > 5
        ):
            return (
                False,
        ...
        for i in range(8):
            if i in {1, 7}:
                continue  # shaders don't use that. what's cuda put here?
