...
class RGP:
  ...
  def from_profile(profile_pickled, device:str|None=None):
    ...
    _is_base = lambda d: all(p.isdigit() for p in d.split(":")[1:])
    device_events = {x.device:x for x in profile if isinstance(x, ProfileDeviceEvent) and x.device.startswith('AMD') and _is_base(x.device)}
    if device is None:
