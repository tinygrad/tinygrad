...
class BLAKE3:
  def __init__(self, max_memory: int = 1024**3):
    ...
    self.PERMS = Tensor([2, 6, 3, 10, 7, 0, 4, 13, 1, 11, 12, 5, 9, 14, 15, 8], dtype=dtypes.uint32)
    self.mem_size = max_memory

  @jit.TinyJit
  def permute(self, data: Tensor) -> Tensor: data[:] = data[self.PERMS]
  ...
  def tensor_to_blake_data(self, tensor: Tensor) -> Tuple[Tensor, int, int]:
    ...
    pad_amt = self.mem_size - tensor.nbytes()
...
if __name__ == "__main__":
  ...
  if arg == "warmup":
    ...
    def warmup(size):
      ...
      BLAKE3(max_memory=max_memory).hash(warmup_data)
    warmup(max_memory)
  else:
    def benchmark_size(size_bytes):
      ...
      start = time.time()
      BLAKE3(max_memory=max_memory).hash(data)
      end = time.time()
