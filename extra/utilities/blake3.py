...
PAD, DEFAULT_LEN = 66, 65
PERMUTATIONS = Tensor([2, 6, 3, 10, 7, 0, 4, 13, 1, 11, 12, 5, 9, 14, 15, 8], dtype=dtypes.uint32)
...
def compress_blocks(states: Tensor, data: Tensor, chain_vals: Tensor) -> Tensor:
  for i in range(7):
    ...
    if i < 6: data = data[PERMUTATIONS]
  return (states[:8] ^ states[8:]).cat(chain_vals[:8] ^ states[8:]).realize()

@TinyJit
def init_chain_vals(data: Tensor, info: Tensor) -> Tuple[Tensor, Tensor]:
  ...
  flags = (flags + (8 * (((info < PAD).sum() <= 16) * (info < DEFAULT_LEN)))).cast(dtypes.uint32) # add root flag if <= 1 chunk
  states = (chain_vals.cat(chain_vals[:, :4], counts, lengths, flags, dim=1) * (info < PAD).cast(dtypes.uint32))
  for i in range(16):
...
def tree_hash(chain_vals: Tensor, n_steps: Variable) -> Tensor:
  for _ in range(n_steps.val):
    ...
    chain_vals = ((compress_blocks(states, paired, iv) * pair_mask)[:8] + remainder).realize()
  return chain_vals.realize()

def tensor_to_blake_input(tensor: Tensor, max_memory: int) -> Tuple[Tensor, Tensor, Variable]:
...
def blake3(tensor: Tensor, max_memory: int = 1024**3) -> str:
  data, info, n_steps = tensor_to_blake_input(tensor, max_memory)
  chain_vals = init_chain_vals(data, info)
  chain_vals = tree_hash(chain_vals, n_steps)
