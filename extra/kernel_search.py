...
if __name__ == "__main__":
  ...
  if getenv("OP", 0) == 1:
    ...
    buf0 = GPUBuffer(shape=ShapeTracker((View((32, 8, 112, 112), (12544, 401408, 112, 1), 0),)), hostbuf=GPUBuffer(shape=(8, 32, 112, 112), force_create=True))
    op0 = LazyOp(ReduceOps.SUM, (buf0,), (32, 1, 1, 1))
    buf1 = GPUBuffer(shape=ShapeTracker((View((32, 1, 1, 1), (0, 0, 0, 0), 0),)), hostbuf=GPUBuffer(shape=(1,), backing=np.array([9.964923e-06], dtype=np.float32)))
    op1 = LazyOp(BinaryOps.MUL, (op0,buf1,), None)
    ast = LazyOp(MovementOps.RESHAPE, (op1,), (1, 32, 1, 1))
  elif getenv("CONVW", 0):
    # re_S64_128_3_3
    buf0 = GPUBuffer(shape=ShapeTracker((View((64, 512, 34, 34), (1024, 65536, 32, 1), -33), ZeroView((64, 512, 32, 32), ((0, 64), (0, 512), (-1, 33), (-1, 33))), View((64, 1, 128, 3, 3, 512, 32, 32), (591872, 591872, 0, 34, 1, 1156, 34, 1), 0),)), hostbuf=GPUBuffer(shape=(512, 64, 32, 32), force_create=True))
    buf1 = GPUBuffer(shape=ShapeTracker((View((64, 1, 128, 3, 3, 512, 32, 32), (0, 0, 1024, 0, 0, 131072, 32, 1), 0),)), hostbuf=GPUBuffer(shape=(512, 128, 32, 32), force_create=True))
    op0 = LazyOp(BinaryOps.MUL, (buf0,buf1,), None)
    ...
    buf0 = GPUBuffer(shape=ShapeTracker((View((64, 512, 32, 32), (524288, 1024, 32, 1), 0),)), hostbuf=GPUBuffer(shape=(64, 512, 32, 32), force_create=True))
    op0 = LazyOp(ReduceOps.SUM, (buf0,), (64, 1, 1, 1))
    buf1 = GPUBuffer(shape=ShapeTracker((View((64, 1, 1, 1), (0, 0, 0, 0), 0),)), hostbuf=GPUBuffer(shape=(1,), backing=np.array([1.9073486e-06], dtype=np.float32)))
    op1 = LazyOp(BinaryOps.MUL, (op0,buf1,), None)
    ast = LazyOp(MovementOps.RESHAPE, (op1,), (1, 64, 1, 1))
  elif getenv("GEMM", 0):
    N = 768
    buf0 = GPUBuffer(shape=ShapeTracker((View((1, N, N, 1), (0, 1, N, 0), 0), View((1, 1, N, N, 1, 1, 1, N), (0, 0, 0, 1, 0, 0, 0, N), 0),)), hostbuf=GPUBuffer(shape=(N, N), force_create=True))
    buf1 = GPUBuffer(shape=ShapeTracker((View((1, 1, N, N, 1, 1, 1, N), (0, 0, 1, 0, 0, 0, 0, N), 0),)), hostbuf=GPUBuffer(shape=(N, N), force_create=True))
    op0 = LazyOp(BinaryOps.MUL, (buf0,buf1,), None)
    ...
    buf0 = GPUBuffer(shape=ShapeTracker((View((32, 1, 32, 32, 32, 64, 3, 3), (73984, 73984, 0, 34, 1, 1156, 34, 1), 0),)), hostbuf=GPUBuffer(shape=(32, 64, 34, 34), force_create=True))
    buf1 = GPUBuffer(shape=ShapeTracker((View((32, 1, 32, 32, 32, 64, 3, 3), (0, 0, 576, 0, 0, 9, 3, 1), 0),)), hostbuf=GPUBuffer(shape=(32, 64, 3, 3), force_create=True))
    op0 = LazyOp(BinaryOps.MUL, (buf0,buf1,), None)
    op1 = LazyOp(ReduceOps.SUM, (op0,), (32, 1, 32, 32, 32, 1, 1, 1))
    ast = LazyOp(MovementOps.RESHAPE, (op1,), (32, 32, 32, 32))
  elif getenv("BROKEN", 0):
    buf0 = GPUBuffer(shape=ShapeTracker((View((64, 1, 1, 1), (1, 0, 0, 0), 0),)), hostbuf=GPUBuffer(shape=(64,), force_create=True))
    buf1 = GPUBuffer(shape=ShapeTracker((View((64, 5, 32, 32), (5120, 1024, 32, 1), 0),)), hostbuf=GPUBuffer(shape=(64, 5, 32, 32), force_create=True))
    op0 = LazyOp(ReduceOps.SUM, (buf1,), (64, 1, 1, 1))
    buf2 = GPUBuffer(shape=ShapeTracker((View((64, 1, 1, 1), (0, 0, 0, 0), 0),)), hostbuf=GPUBuffer(shape=(1,), backing=np.array([0.001], dtype=np.float32)))
    op1 = LazyOp(BinaryOps.MUL, (op0,buf2,), None)
    op2 = LazyOp(BinaryOps.SUB, (buf0,op1,), None)
    ast = LazyOp(MovementOps.RESHAPE, (op2,), (64,))
  elif getenv("BROKEN3"):
    buf0 = GPUBuffer(shape=ShapeTracker((View((5, 128, 18, 18), (32768, 256, 16, 1), -17), ZeroView((5, 128, 16, 16), ((0, 5), (0, 128), (-1, 17), (-1, 17))), View((5, 1, 128, 16, 16, 128, 3, 3), (41472, 41472, 0, 18, 1, 324, 18, 1), 0),)), hostbuf=GPUBuffer(shape=(5, 128, 16, 16), force_create=True))
    buf1 = GPUBuffer(shape=ShapeTracker((View((5, 1, 128, 16, 16, 128, 3, 3), (0, 0, 1152, 0, 0, 9, 3, 1), 0),)), hostbuf=GPUBuffer(shape=(128, 128, 3, 3), force_create=True))
    op0 = LazyOp(BinaryOps.MUL, (buf0,buf1,), None)
