...
def pack_kernel_descriptor(text_offset:int, kd:dict) -> bytes:
  ...
  reserved1 = (kd['workgroup_processor_mode'] << 3) | (kd['memory_ordered'] << 4)
  ...
  compute_pgm_rsrc2 = (kd['user_sgpr_count'] << hsa.AMD_COMPUTE_PGM_RSRC_TWO_USER_SGPR_COUNT_SHIFT |
                       kd['system_sgpr_workgroup_id_x'] << hsa.AMD_COMPUTE_PGM_RSRC_TWO_ENABLE_SGPR_WORKGROUP_ID_X_SHIFT |
                       kd['system_sgpr_workgroup_id_y'] << hsa.AMD_COMPUTE_PGM_RSRC_TWO_ENABLE_SGPR_WORKGROUP_ID_Y_SHIFT |
                       kd['system_sgpr_workgroup_id_z'] << hsa.AMD_COMPUTE_PGM_RSRC_TWO_ENABLE_SGPR_WORKGROUP_ID_Z_SHIFT)

  # Pack kernel_code_properties using hsa constants
  kernel_code_properties = (kd['user_sgpr_kernarg_segment_ptr'] << hsa.AMD_KERNEL_CODE_PROPERTIES_ENABLE_SGPR_KERNARG_SEGMENT_PTR_SHIFT |
                            kd['uses_dynamic_stack'] << hsa.AMD_KERNEL_CODE_PROPERTIES_IS_DYNAMIC_CALLSTACK_SHIFT |
  ...
  desc.group_segment_fixed_size = kd['group_segment_fixed_size']
  desc.private_segment_fixed_size = kd['private_segment_fixed_size']
  desc.kernarg_size = kd['kernarg_size']
...
def build_hsaco(insts:list[Inst]) -> bytes:
