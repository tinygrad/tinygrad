...
__global__ __launch_bounds__(512, 2) void hk_bf16_atb_gemm(bf16 *C_ptr, bf16 *A_ptr, bf16 *B_ptr) {
    ...
    using ST_A = st_bf<BLOCK_K, HALF_BLOCK_SIZE_ROW, st_32x32_s>;
    using ST_B = st_bf<BLOCK_K, HALF_BLOCK_SIZE_COL, st_32x32_s>;
    __shared__ ST_A As[2][2];
    __shared__ ST_B Bs[2][2];

    using RT_A = rt_bf<BLOCK_K, REG_BLOCK_M, col_l, rt_32x32_s>;
    using RT_B = rt_bf<BLOCK_K, REG_BLOCK_N, col_l, rt_32x32_s>;
    ...
    constexpr int memcpy_per_tile_A = BLOCK_K * HALF_BLOCK_SIZE_ROW * sizeof(bf16) / bytes_per_memcpy;
    constexpr int memcpy_per_tile_B = BLOCK_K * HALF_BLOCK_SIZE_COL * sizeof(bf16) / bytes_per_memcpy;
    ...
    for (int k = 0; k < k_iters; k++, tic ^= 1, toc ^= 1) {
        if (k + 1 < k_iters) {
            G::load(As[toc][0], A, {0, 0, k + 1, block_row * 2}, swizzled_offsets_A);
            G::load(Bs[toc][0], B, {0, 0, k + 1, block_col * 2}, swizzled_offsets_B);
            G::load(As[toc][1], A, {0, 0, k + 1, block_row * 2 + 1}, swizzled_offsets_A);
            G::load(Bs[toc][1], B, {0, 0, k + 1, block_col * 2 + 1}, swizzled_offsets_B);
        }

        auto as_subtile0 = kittens::subtile_inplace<BLOCK_K, REG_BLOCK_M>(As[tic][0], {0, warp_m});
        load(a, as_subtile0);
        auto bs_subtile0 = kittens::subtile_inplace<BLOCK_K, REG_BLOCK_N>(Bs[tic][0], {0, warp_n});
        load(b0, bs_subtile0);
