...
def flash_attention(xq, xk, xv, attn_mask:Tensor|None=None, is_causal:bool=False):
  ...
  def grad(dou:UOp, _) -> tuple[None, None, UOp, UOp, UOp]:
    ...
    dq = _sharded_zeros((B, H, N, D), xq, axis=0)
    ...
    delta_vec = Tensor.custom_kernel(delta_vec, attn, do, fxn=functools.partial(custom_fa_backward_pre, device=single_device, arch=arch))[0]
...
@functools.cache
def custom_fa_backward_pre(delta_vec:UOp, o:UOp, do:UOp, device:str, arch:str):
  B, N, H, _ = o.shape
  ...
  sink = UOp.sink(delta_vec.base, o.base, do.base,
                  threadIdx_x, blockIdx_x, blockIdx_y, blockIdx_z,
