...
@functools.cache
def custom_fused_qkv_rope_forward(q:UOp, k:UOp, v:UOp, xqkv:UOp, freqs_cis:UOp, device:str, arch:str, B:int, N:int, H:int, H_KV:int, D:int):
  code = (pathlib.Path(__file__).parent / "fused_qkv_rope.cpp").read_text()
  threads = 256
  threadIdx_x = UOp.special(threads, "lidx0")
  blockIdx_x, blockIdx_y = UOp.special(B, "gidx0"), UOp.special(N, "gidx1")
  sink = UOp.sink(q.base, k.base, v.base, xqkv.base, freqs_cis.base, threadIdx_x, blockIdx_x, blockIdx_y,
                  arg=KernelInfo(name="fused_qkv_rope_forward"))
  compile_args = ["-std=c++20", "-ffast-math", f"-DATTN_B={B}", f"-DATTN_N={N}", f"-DATTN_H={H}", f"-DATTN_H_KV={H_KV}", f"-DATTN_D={D}", f"-DTHREADS_PER_BLOCK={threads}"]
...
@functools.cache
def custom_fused_qkv_rope_backward(dxqkv:UOp, dq:UOp, dk:UOp, dv:UOp, freqs_cis:UOp, device:str, arch:str, B:int, N:int, H:int, H_KV:int, D:int):
  code = (pathlib.Path(__file__).parent / "fused_qkv_rope_bwd.cpp").read_text()
  threads = 256
  threadIdx_x = UOp.special(threads, "lidx0")
  blockIdx_x, blockIdx_y = UOp.special(B, "gidx0"), UOp.special(N, "gidx1")
  sink = UOp.sink(dxqkv.base, dq.base, dk.base, dv.base, freqs_cis.base, threadIdx_x, blockIdx_x, blockIdx_y,
...
def _fused_qkv_rope_grad(dq_u:UOp, dk_u:UOp, dv_u:UOp, call:UOp) -> tuple[None, None, None, UOp, None]:
  ...
  xqkv = Tensor(xqkv_u, device=xqkv_u.device)
  freqs_cis = Tensor(freqs_u, device=freqs_u.device)

  B, N, _ = xqkv.shape
  H, H_KV, D = dq.shape[2], dk.shape[2], dq.shape[3]
  num_devices = len(xqkv.device) if isinstance(xqkv.device, tuple) else 1
  is_dp = xqkv.uop.axis == 0
  is_mp = xqkv.uop.axis == 2
  ...
  dxqkv = Tensor.custom_kernel(dxqkv, dq, dk, dv, freqs_cis, fxn=functools.partial(custom_fused_qkv_rope_backward, device=single_device, arch=arch, B=B_local, N=N, H=H_local, H_KV=H_KV_local, D=D))[0]
...
def fused_qkv_rope(xqkv:Tensor, freqs_cis:Tensor, n_heads:int, n_kv_heads:int, head_dim:int) -> tuple[Tensor, Tensor, Tensor]:
  ...
  q, k, v, *_ = Tensor.custom_kernel(q, k, v, xqkv, freqs_cis, fxn=functools.partial(custom_fused_qkv_rope_forward, device=single_device, arch=arch, B=B_local, N=N, H=H_local, H_KV=H_KV_local, D=head_dim), grad_fxn=_fused_qkv_rope_grad)
  return q, k, v
