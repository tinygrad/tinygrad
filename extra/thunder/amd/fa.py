...
def flash_attention(xq, xk, xv, attn_mask:Tensor|None=None, is_causal:bool=False):
  ...
  single_device = xq.device[0] if isinstance(q.device, tuple) else xq.device
  ...
  def grad(dou:UOp, _) -> tuple[None, None, UOp, UOp, UOp]:
    ...
    dq = _sharded_empty((B, H, N, D), xq, axis=0)
    ...
    dq, dk, dv = Tensor.custom_kernel(dq, dk, dv, do, xq, xk, xv, l_vec, delta_vec, fxn=functools.partial(custom_fa_backward, device=single_device, arch=arch))[:3]
    dq = dq.transpose(1, 2)
...
if __name__ == "__main__":
  ...
  # backward pass
  q2 = Tensor.randn(B, N, H, D, device="AMD", dtype="bfloat16", requires_grad=True).contiguous()
  k2 = Tensor.randn(B, N, H_KV, D, device="AMD", dtype="bfloat16", requires_grad=True).contiguous()
  v2 = Tensor.randn(B, N, H_KV, D, device="AMD", dtype="bfloat16", requires_grad=True).contiguous()
  ...
  for _ in range(5):
    st = time.perf_counter()
    q2.grad, k2.grad, v2.grad = fa_bwd(q2, k2, v2)
    Device["AMD"].synchronize()
    et = time.perf_counter() - st
