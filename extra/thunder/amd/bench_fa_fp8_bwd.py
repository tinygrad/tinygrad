...
def main():
  ...
  bf16_buffers, fp8_buffers = bf16, fp8
  ...
  for i in range(warmup+repeat):
    print(f"FA_BWD {'warmup' if i < warmup else 'sample'} {i if i < warmup else i-warmup}", flush=True)
    for schedule, buffers in ((bf16_schedule, bf16_buffers[:1]), (fp8_schedule, fp8_buffers[:5]))[::1 if i%2 == 0 else -1]:
      with Context(DEBUG=0):
        for t in buffers: t.assign(0).realize()
      run_linear(schedule, wait=True)
  ...
  with Context(DEBUG=0):
    ...
    for name,actual,expected in zip(("dQ","dK","dV"),fp8[:3],(ref_q,*bf16[1:3])):
      ...
      print(f"{name}: relative L2 error {error:.6f}", flush=True)
