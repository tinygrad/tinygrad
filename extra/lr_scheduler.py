...
class OneCycleLR(LR_Scheduler):
  ...
  def get_lr(self) -> Tensor:
    return (self.epoch_counter < self.total_steps*self.pct_start).where(
      self._annealing_linear(self.initial_lr, self.max_lr, self.epoch_counter.cast(self.optimizer.lr.dtype)/(self.total_steps*self.pct_start)),
      self._annealing_linear(self.max_lr, self.min_lr, (self.epoch_counter.cast(self.optimizer.lr.dtype)-(self.total_steps*self.pct_start))/(self.total_steps*(1-self.pct_start)))
    )
