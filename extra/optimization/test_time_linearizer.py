...
if __name__ == "__main__":
    ...
    for i, ast_str in enumerate(ast_strs):
        ...
        for k, v in acted_lins.items():
            ...
            print(
                v.colored_shape(50),
                f"{tm1*1e3:10.2f} {tm2*1e3:10.2f} {tm3*1e3:10.2f}  :  {((tm1-tm2)/tm1)*100:5.2f}%  vs  {((tm1-tm3)/tm1)*100:5.2f}%",
            )
            ok_avg += (tm1 - tm2) / tm1
            short_avg += (tm1 - tm3) / tm1
        print(
            f"{ok_avg/len(acted_lins)*100:5.2f}% vs {short_avg/len(acted_lins)*100:5.2f}%"
        )
