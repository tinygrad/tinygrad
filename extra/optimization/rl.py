...
if __name__ == "__main__":
    net = PolicyNet()
    if os.path.isfile("/tmp/policynet.safetensors"):
    ...
    while 1:
        ...
        while 1:
            ...
            for x in get_kernel_actions(lin):
                valid_action_mask[x] = 1
        ...
        # print(rews)
        print(
            f"***** EPISODE {len(rews)} steps, {sum(rews):5.2f} reward, {base_tm*1e6:12.2f} -> {tm*1e6:12.2f} : {lin.colored_shape()}"
        )
        all_feats += feats
        all_acts += acts
        # rewards to go
