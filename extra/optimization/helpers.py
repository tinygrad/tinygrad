...
def ast_str_to_ast(ast_str: str) -> UOp:
    return eval(ast_str)


def ast_str_to_lin(ast_str: str, opts=None):
    return Kernel(ast_str_to_ast(ast_str), opts=opts)
...
def load_worlds(filter_reduce=True, filter_noimage=True, filter_novariable=True):
    ...
    if DEBUG >= 1:
        print(f"loaded {len(ast_strs)=} before filters")
    if filter_reduce:
        ast_strs = [x for x in ast_strs if "REDUCE_AXIS" in x]
    if filter_noimage:
        ast_strs = [x for x in ast_strs if "dtypes.image" not in x]
    if filter_novariable:
        ast_strs = [x for x in ast_strs if "DEFINE_VAR" not in x]
    if DEBUG >= 1:
        print(
            f"loaded {len(ast_strs)=} after filters {filter_reduce=}, {filter_noimage=}, {filter_novariable=}"
        )
    random.seed(1337)
...
def lin_to_feats(lin: Kernel, use_sts=True):
    ...
    if use_sts:
        my_sts = dedup(
            [
                (
                    x.shape == lin.full_shape,
                    x.real_strides(),
                    any(v.mask is not None for v in x.views),
                    len(x.views),
                )
                for x in lin.sts
            ]
