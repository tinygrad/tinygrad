...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run a search for the optimal opts for a kernel",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--ast", type=str, default=None, help="the ast for the kernel to be optimized"
    )
    parser.add_argument(
        "--file",
        type=str,
        default=None,
        help="a file containing asts to be optimized, one per line",
    )
    ...
    for i, ast_str in enumerate(ast_strs):
        ...
        lin = beam_search(
            lin, rawbufs, getenv("BEAM", 8), bool(getenv("BEAM_ESTIMATE", 1))
        )
