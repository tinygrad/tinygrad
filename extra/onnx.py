...
def dtype_parse(onnx_dtype: int) -> DType:
    supported: dict[int, DType] = {
        ...
        TensorProto.FLOAT16: dtypes.float32,
        TensorProto.DOUBLE: dtypes.double,
    ...
    if onnx_dtype in unsupported:
        raise NotImplementedError(
            f"onnx dtype {TensorProto.DataType.Name(onnx_dtype)} is not supported"
        )
    return (
        supported[onnx_dtype]
        if is_dtype_supported(supported[onnx_dtype])
        else dtypes.float
    )
...
def attribute_parse(onnx_attribute: AttributeProto):
    supported: dict[AttributeProto.AttributeType, Callable[[AttributeProto], Any]] = {
        AttributeProto.FLOAT: lambda a: float(a.f),
        AttributeProto.INT: lambda a: int(a.i),
        AttributeProto.STRING: lambda a: a.s.decode("utf-8"),
        AttributeProto.TENSOR: lambda a: buffer_parse(a.t),
        AttributeProto.FLOATS: lambda a: tuple(float(x) for x in a.floats),
        AttributeProto.INTS: lambda a: tuple(int(x) for x in a.ints),
    ...
    if onnx_attribute.type in unsupported:
        raise NotImplementedError(
            f"attribute with type {AttributeProto.AttributeType.Name(onnx_attribute.type)} is not supported"
        )
    return supported[onnx_attribute.type](onnx_attribute)


def buffer_parse(onnx_tensor: TensorProto) -> Tensor:
    if onnx_tensor.string_data:
        raise NotImplementedError(
            "Parsing for buffer with string data is not implemented."
        )
    dtype, shape = dtype_parse(onnx_tensor.data_type), tuple(onnx_tensor.dims)
    if (
        data := list(onnx_tensor.float_data)
        or list(onnx_tensor.int32_data)
        or list(onnx_tensor.int64_data)
        or list(onnx_tensor.double_data)
        or list(onnx_tensor.uint64_data)
    ):
        if len(data) == 1:
            return Tensor(data[0], dtype=dtype).reshape(shape)
        return Tensor(data, dtype=dtype).reshape(shape).realize()
    if onnx_tensor.HasField("raw_data"):
        np_buffer = (
            np.frombuffer(
                onnx_tensor.raw_data,
                dtype=helper.tensor_dtype_to_np_dtype(onnx_tensor.data_type),
            )
            .copy()
            .reshape(shape)
        )
        if np_buffer.size == 1:
            return Tensor(np_buffer.item(), dtype=dtype).reshape(shape)
...
def type_parse(onnx_type: TypeProto):
    elem_type = onnx_type
    if (
        elem_type.HasField("map_type")
        or elem_type.HasField("sparse_tensor_type")
        or elem_type.HasField("opaque_type")
    ):
        raise NotImplementedError(
            "parsing for map_type, sparse_tensor_type and opaque_type are not implemented"
        )
    if is_optional := elem_type.HasField("optional_type"):
        elem_type = elem_type.optional_type.elem_type
    if is_sequence := elem_type.HasField("sequence_type"):
        elem_type = elem_type.sequence_type.elem_type
    if elem_type.HasField("tensor_type"):
        shape = tuple(
            d.dim_param or d.dim_value for d in elem_type.tensor_type.shape.dim
        )
...
class OnnxRunner:
    def __init__(self, model: ModelProto):
        ...
        self.is_training = any(
            n.domain in {"ai.onnx.training", "ai.onnx.preview.training"}
        ...
        self.graph_values = {
            "": None,
            **{x.name: buffer_parse(x) for x in model.graph.initializer},
        }
        self.graph_inputs = {
            x.name: type_parse(x.type)
            for x in model.graph.input
            if x.name not in self.graph_values
        }
    ...
    def _parse_input(self, name: str, value: Any, spec: OnnxValue):
        ...
        if spec.is_sequence:
            ...
            if not all_same(tuple(t.shape for t in sequence)):
                raise RuntimeError(f"Shapes for {name} sequence must be homogeneous")
            return sequence
        tensor = (
            Tensor(value, dtype=spec.dtype, requires_grad=self.is_training)
        ...
        for dim, (onnx_dim, user_dim_input) in enumerate(
            ...
            if user_dim_input != onnx_dim:
                raise RuntimeError(
                    f"{name} has mismatch on {dim=}. Expected {onnx_dim}, received {user_dim_input}."
                )
        return tensor
...
def get_onnx_ops():
    ...
    def _prepare_quantize(x, scale, zero_point, axis=1, block_size=0):
        if axis < 0:
            axis += x.ndim
        if not isinstance(zero_point, Tensor):
            zero_point = Tensor(zero_point, dtype=dtypes.uint8)._broadcast_to(
                scale.shape
            )
        if block_size == 0:
            shape = (*[1] * axis, *scale.shape, *[1] * (x.ndim - axis - scale.ndim))
            return scale.reshape(shape), zero_point.reshape(shape)
        return scale.repeat_interleave(
            block_size, dim=axis
        ), zero_point.repeat_interleave(block_size, dim=axis)
    ...
    def _qlinearop_float(
        ...
        **opts,
        ...
        dequantized_inputs = [
            (inp.int() - zp) * scale
            for inp, zp, scale in zip(inputs, zero_points, scales)
        ]
    ...
    def ImageDecoder(encoded_stream: bytes, pixel_format="RGB"):
        ...
        if pixel_format == "Grayscale":
            return Tensor(np.array(img.convert("L"))).unsqueeze(
                -1
            )  # (H, W) to (H, W, 1)
    ...
    def EyeLike(x: Tensor, dtype: int | None = None, k: int = 0):
        ret = Tensor.eye(
            ...
            dtype=dtype_parse(dtype) if dtype is not None else x.dtype,
        )
        return (
            ret
            if x.size(0) == x.size(1)
                ...
                tuple(
                    None if d == ret.size(0) else (k, d - ret.shape[0] - k)
                    for d in x.shape
                )
            )
        )
    ...
    def Gelu(x: Tensor, approximate: str | None = None):
        return (
            x.gelu()
            if approximate == "tanh"
            else 0.5 * x * (1 + (x / math.sqrt(2)).erf())
        )
    ...
    def Transpose(x: Tensor, perm: list[int] | None = None):
        return x.permute(order=list(range(x.ndim)[::-1]) if perm is None else perm)
    ...
    def AveragePool(
        ...
        return X.avg_pool2d(
            ...
            strides,
            dilations,
            _resolve_pool_pads(X, pads, kernel_shape, dilations, strides, auto_pad),
            ceil_mode=ceil_mode,
    ...
    def MaxPool(
        ...
        indices = (
            (
                (ret.reshape(-1, 1) == X.reshape(1, -1))
                * Tensor.arange(X.numel(), dtype=dtypes.int64).unsqueeze(0)
            )
            .sum(1)
            .reshape(ret.shape)
        )
        return ret.cast(X.dtype), (
            indices.transpose(-2, -1) if storage_order else indices
        )
    ...
    def ConvTranspose(
        ...
        strides, dilations, output_padding = (
            make_tuple(x, len(input_shape))
            for x in (strides, dilations, output_padding)
        )
        if output_shape is not None:  # we pad according to output_shape
            pads = _auto_pad(
                [
                    s * (i - 1) + op + ((k - 1) * d + 1) - os
                    for s, i, op, k, d, os in zip(
                        strides,
                        input_shape,
                        output_padding,
                        kernel_shape,
    ...
    def MaxUnpool(
        ...
        ret = (
            (xI.reshape(-1, 1)._one_hot_along_dim(prod(out_sh)) * xT.reshape(-1, 1))
            .sum(0)
            .reshape(1, 1, *out_sh)
        )
        if outshape is not None and outshape != ret.shape:
            pads = _auto_pad(
                [outshape[-2] - ret.shape[-2], outshape[-1] - ret.shape[-1]],
                "SAME_UPPER",
            )
    ...
    def CumSum(X: Tensor, axis: int | list, exclusive: int = 0, reverse: int = 0):
        ...
        if exclusive:
            X = X.pad(
                tuple((1, 0) if i == axis else None for i in range(X.ndim))
            ).shrink(
    ...
    def Resize(
        ...
        def _apply_nearest_mode(index: Tensor, input_dim, mode: str):
            if mode == "round_prefer_floor":
                index = (index - 0.5).ceil()
            elif mode == "round_prefer_ceil":
                index = (index + 0.5).floor()
            elif mode in ["floor", "ceil"]:
                index = getattr(index, mode)()
            else:
        ...
        def _apply_transformation(index: Tensor, input_dim, scale_dim, roi_dim, mode):
            ...
            if mode == "half_pixel":
                index = (index + 0.5) / scale_dim - 0.5
            elif mode == "align_corners":
                index = (
                    index * (input_dim - 1) / (output_dim - 1)
                    if output_dim != 1
                    else Tensor([0])
                )
            elif mode == "asymmetric":
                index = index / scale_dim
                ...
                index = (
                    input_dim / 2 * (1 - int(output_dim) / output_dim)
                    + (index + 0.5) / scale_dim
                    - 0.5
                )
            elif mode == "tf_crop_and_resize":
                index = roi_dim[0] * (input_dim - 1) + index * (
                    (roi_dim[1] - roi_dim[0]) * (input_dim - 1) / (output_dim - 1)
                )
            else:
        ...
        scales, sizes = (
            None if scales is None else scales[2 - (X.ndim - len(scales)) :]
        ), (None if sizes is None else sizes[2 - (X.ndim - len(sizes)) :])
        # we pre permute the axes and permute back after resize
        (
            axes,
            input_shape,
        ) = (
            (axes or list(range(X.ndim))),
            cast(tuple[int, ...], X.shape[2:]),
        )
        ...
        if sizes is not None:
            if keep_aspect_ratio_policy in ["not_larger", "not_smaller"]:
                ...
                sizes = [
                    (
                        int((scales[0] * input_shape[i]) + 0.5)
                        if i + 2 in axes
                        else input_shape[i]
                    )
                ...
                scales = [
                    size / input_shape for size, input_shape in zip(sizes, input_shape)
                ]
        else:
            sizes = [int(sc * sh) for sc, sh in zip(scales, input_shape)]
        regions = (
            [[st, ed] for st, ed in zip(roi, roi[len(roi) // 2 :])]
            if isinstance(roi, list) and roi
            else [[0.0, 0.0]] * (X.ndim - 2)
        )
        ...
        if mode == "nearest":
            indexes = [
                _apply_nearest_mode(index, shape, nearest_mode)
                for (index, shape) in zip(indexes, input_shape)
            ]
    ...
    def BatchNormalization(
        ...
        if training_mode:
            ...
            return (
                X.batchnorm(scale, B, current_mean, current_invstd),
                running_mean,
                running_var,
            )
    ...
    def LayerNormalization(
        ...
        return (
            x.layernorm(axes, epsilon).mul(scale).add(bias),
            mean,
            (x.sub(mean)).square().mean(axis=axes, keepdim=True).add(epsilon).rsqrt(),
        )

    def GroupNormalization(
        x: Tensor, scale: Tensor, bias: Tensor, num_groups: int, epsilon: float = 1e-05
    ):
        return (
    ...
    def EmbedLayerNormalization(
        ...
        assert (
            mask is None and not mask_index_type
        ), "functionality not supported yet"  # TODO
    ...
    def OneHot(
        indices: Tensor, depth: float | int | list, values: Tensor, axis: int = -1
    ):
    ...
    def AffineGrid(theta: Tensor, size: list[int], align_corners: int = 0):
        ...
        def generate_grid(steps):
            return (
                Tensor.linspace(-1, 1, steps, device=theta.device)
                if align_corners
                else Tensor.linspace(
    ...
    def Attention(
        ...
        assert (
            ...
            is do_rotary
            is past_sequence_length
            is mask_filter_value
            is past_present_share_buffer
            is scale
            is None
        ...
        if unidirectional:  # gpt-style
            ...
            xq, xk, xv = [
                xqkv.shrink([None, None, (i * hidden_size, (i + 1) * hidden_size)])
                for i in range(3)
            ]
        else:  # bert-style
            wq, wk, wv = (
        ...
        if (
            ...
            indices = (
                ...
                if indices.shape == ()
                    ...
                    x_sh[axis] + x if x < 0 else x
                    for x in _cached_to_python_const(indices)
                ]
            )
    ...
    def GatherND(x: Tensor, indices: Tensor, batch_dims: int = 0):
        ...
        return ret.reshape(
            *x_shape[:batch_dims],
            *i_shape[batch_dims:-1],
            *ret.shape[indices.ndim - 1 :],
        )
    ...
    def ScatterND(
        ...
        assert (
            updates.shape
            == indices.shape[:-1] + x.shape[cast(int, indices.shape[-1]) :]
        )
        x = x.contiguous()
    ...
    def ScatterElements(
        ...
        return x.scatter_reduce(
            ...
            indices,
            updates,
            {"add": "sum", "mul": "prod", "min": "amin", "max": "amax"}.get(reduction),
        )
    ...
    def Compress(inp: Tensor, condition: list[bool], axis: int | None = None):
        ...
        if axis < 0:
            axis += inp.ndim
        con = Tensor(
            np.arange(len(condition))[condition]
        )  # no boolean indexing in Tensor
    ...
    def QuantizeLinear(
        ...
        out_dtype = (
            ...
            else dtype_parse(output_dtype) if output_dtype else dtypes.uint8
        )
        y_scale, y_zero_point = _prepare_quantize(
            x, y_scale, y_zero_point, axis, block_size
        )
        return _clamp_cast(
            ((x / y_scale).round() + y_zero_point), out_dtype
        ).contiguous()
    ...
    def QLinearConv(
        x: Tensor,
        x_scale: Tensor,
        x_zero_point: Tensor | int,
        w: Tensor,
        w_scale: Tensor,
        w_zero_point: Tensor | int,
        y_scale: Tensor,
        y_zero_point: Tensor | int,
        B: Tensor | None = None,
        **opts,
    ):
    ...
    def QLinearMatMul(
        a: Tensor,
        a_scale: Tensor,
        a_zero_point: Tensor | int,
        b: Tensor,
        b_scale: Tensor,
        b_zero_point: Tensor | int,
        y_scale: Tensor,
        y_zero_point: Tensor | int,
    ) -> Tensor:
        return _qlinearop_quantized(
    ...
    def MatMulInteger(
        A: Tensor,
        B: Tensor,
        a_zero_point: Tensor | int = 0,
        b_zero_point: Tensor | int = 0,
    ) -> Tensor:
    ...
    def Adam(
        ...
        *inputs: Tensor,
        ...
        G, V, H = (
            ...
            V.detach(),
            H.detach(),
        )  # TODO we shouldn't need these detaches
