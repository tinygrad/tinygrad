...
def get_onnx_ops():
  ...
  def DequantizeLinear(x:Tensor, x_scale:Tensor, x_zero_point:Tensor|int=0, axis:int=1, block_size:int=0):
    if getenv("NHWC") and len(x.shape) == 4 and x.shape[2:] == (1,1) and x.shape[1]%4 == 0:
      # DSP swizzle memory
      x = x.reshape(x.shape[0], x.shape[1]//4, 4).permute(1,0,2).contiguous().permute(1,0,2).reshape(x.shape)
    x_scale, x_zero_point = _prepare_quantize(x, x_scale, x_zero_point, axis, block_size)
    ret = ((x.int() - x_zero_point) * x_scale).cast(x_scale.dtype)
    return ret
