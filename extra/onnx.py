...
from onnx import AttributeProto, ModelProto, TensorProto, helper
import numpy as np
...
def dtype_parse(onnx_dtype: int) -> DType:
  ...
  if onnx_dtype not in mapping: raise NotImplementedError(f"onnx dtype {TensorProto.DataType.Name(onnx_dtype)} is not supported")
  return mapping[onnx_dtype] if is_dtype_supported(mapping[onnx_dtype]) else dtypes.float

# src: onnx/onnx_ml_pb2.pyi
def attribute_parse(onnx_attribute: AttributeProto):
  mapping: dict[AttributeProto.AttributeType, Callable[[AttributeProto], Any]] = {
...
def make_onnx_ops():
  ...
  OnnxOps = {"Identity":lambda x:x, "Constant":Constant, }
  ...
  def ReduceProd(data:Tensor, axes:list[int]|None=None, keepdims:int=1, noop_with_empty_axes:int=0):
    return data.prod(_axes(axes, noop_with_empty_axes), keepdims)
  ...
  return {
    ...
    **{name:obj for name,obj in locals().items() if (callable(obj) and not name.startswith("_")) or isinstance(obj, dict)}
  }


# ***** a single geodude, connected becomes onix *****
...
# https://www.pokemon.com/us/pokedex/onix
# MAIN THING TO IMPORT TO RUN ONNX
class OnnxSession:
  # TODO: BTW!! we can pre-load values that hit to_python_const lower down in the graph. We just search up and see if they originate fron a
  # Constant node or from initializer and load it into self.values and skip the node. a bit of a hack but I think it's correct.
  ...
  # mapping from op names to op implementations
  onnx_ops:dict[str, Callable] = make_onnx_ops()

  def __init__(self, model: ModelProto):
    self.is_training = any(n.HasField("domain") and n.domain == "ai.onnx.preview.training" for n in model.graph.node)
    if self.is_training: Tensor.training = True
    else:
      Tensor.no_grad = True
      Tensor.training = False

    # extract info from model (gets rid of protobuf usage)
    self.values = {x.name:buffer_parse(x) for x in model.graph.initializer}
    self.input_spec = {x.name:MessageToDict(x.type) for x in model.graph.input if x.name not in self.values}
    self.output_spec = {x.name:MessageToDict(x.type) for x in model.graph.output}
    self.nodes = tuple(OnnxNode(n.op_type, tuple(n.input), tuple(n.output), {x.name:attribute_parse(x) for x in n.attribute})
                       for n in model.graph.node)
    self.opset_version = model.opset_import[0].version
    self.variable_dims = {}

    # cool stats thing for cool print
    # self.ops_counter = collections.Counter(n.op_type for n in model.graph.node)

  # *** input parsing ***
  def parse_input(self, name: str, value: Any, spec: dict):
    if "optionalType" in spec: return self._parse_optional(name, value, spec["optionalType"]["elemType"])
    if "sequenceType" in spec: return self._parse_sequence(name, value, spec["sequenceType"])
    if "tensorType" in spec: return self._parse_tensor(name, value, spec["tensorType"])
    raise NotImplementedError(f"Unsupported type for {name}: {spec=}")

  def _parse_optional(self, name: str, value: Any, spec: dict):
    if value is None: return None
    return self.parse_input(name, value, spec)
  def _parse_sequence(self, name: str, value: Any, spec: dict):
    if not isinstance(value, Sequence): raise RuntimeError(f"{name} received {value}, expected a sequence type")
    dtype = dtype_parse(spec["elemType"]["tensorType"]["elemType"])
    sequence = [Tensor(v, dtype=dtype, requires_grad=self.is_training) if not isinstance(v, Tensor) else v for v in value]
    if not all_same([t.shape for t in sequence]): raise RuntimeError(f"Shapes for {name} must be homogeneous")
    # TODO: need true float16 for dtype checking
    # if not all(t.dtype is dtype for t in sequence):
    #   raise RuntimeError(f"{name} has dtype mismatch for sequence type. Expected {dtype}, received {tensor.dtype}.")
    return sequence
  def _parse_tensor(self, name: str, value: Any, spec: dict):
    dtype = dtype_parse(spec["elemType"])
    tensor = Tensor(value, dtype=dtype, requires_grad=self.is_training) if not isinstance(value, Tensor) else value
    # TODO: need true float16 for dtype checking
    # if dtype is not tensor.dtype: raise RuntimeError(f"{name} has mismatch for dtype. Expected {dtype}, received {tensor.dtype}.")
    if spec["shape"]:
      for dim, onnx_dim in enumerate(spec["shape"]["dim"]):
        dim_param, dim_value = onnx_dim.get("dimParam"), int(onnx_dim.get("dimValue"))
        user_dim_input = int(tensor.shape[dim])
        if dim_param:
          dim_value = self.variable_dims[dim_param] if dim_param in self.variable_dims else self.variable_dims.setdefault(dim_param, user_dim_input)
        if user_dim_input != dim_value:
          raise RuntimeError(f"{name} has mismatch for dim={dim_param or dim}. Expected {dim_value}, received {user_dim_input}.")
    return tensor

  # *** main runner ***
  def run(self, inputs={}, debug=int(getenv("DEBUGONNX", "0")), limit=int(getenv("ONNXLIMIT", "-1"))):
    for name, input_spec in self.input_spec.items():
      ...
      self.values[name] = self.parse_input(name, inputs[name], input_spec)
    ...
    for num,node in enumerate(self.nodes):
      ...
      if op in self.onnx_ops:
        # TODO: improve opset parsing so we can just call function
        # ret = self.onnx_ops[op](*inps, **opts)
        ...
        ret = real_fxn(*inps, **opts)
      else:
        print("UNSUPPORTED", node.op, node.inputs, node.outputs)
        raise NotImplementedError(f"{op=} not supported")

      # finalization after running the op
      if not isinstance(ret, tuple): ret = (ret, )
      if len(node.outputs) > len(ret): raise RuntimeError(f"expected output size must be less than {len(ret)}, it's {node.output}")
      for i in range(len(node.outputs)): self.values[node.outputs[i]] = ret[i]
      if num == limit: return {name:self.values[name] for name in node.outputs}
