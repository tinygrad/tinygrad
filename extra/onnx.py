...
class OnnxRunner:
  """
  OnnxRunner class parses and runs an ONNX model using Tinygrad as backend.

  Args:
    model: The ONNX model to run
    float32: If ``True``, forces all floating point inputs and initializer tensors to use float32 precision
            This can help with numerical stability.
            When enabled:
            - All float inputs are cast to float32
            - All float model initializers are cast to float32
            - Internal float computations use float32
            - Final outputs are cast back to the model's original float precision
  """
  def __init__(self, model: ModelProto, float32: bool = False):
    ...
    self.graph_values = {"": None, **{x.name:self.buffer_parse(x) for x in model.graph.initializer}}
    self.graph_inputs = {x.name:self.type_parse(x.type) for x in model.graph.input if x.name not in self.graph_values}
    self.graph_outputs = {x.name:self.type_parse(x.type) for x in model.graph.output}
    self.graph_nodes = tuple(OnnxNode(num, n.op_type, tuple(n.input), tuple(n.output), {x.name:self.attribute_parse(x) for x in n.attribute})
                       for num,n in enumerate(model.graph.node))
    self.opset_version = model.opset_import[0].version
    self.variable_dims: dict[str, int] = {}

  # ***** protobuf parsing ******
  def dtype_parse(self, onnx_dtype: int) -> DType:
    ...
    if (self.float32 and dtypes.is_float(dtype)) or not is_dtype_supported(dtype): dtype = dtypes.float32
    return dtype

  def attribute_parse(self, onnx_attribute: AttributeProto):
    supported: dict[AttributeProto.AttributeType, Callable[[AttributeProto], Any]] = {
      ...
      AttributeProto.STRING: lambda a: a.s.decode("utf-8"), AttributeProto.TENSOR: lambda a: self.buffer_parse(a.t),
  ...
  def buffer_parse(self, onnx_tensor: TensorProto, float32: bool = False) -> Tensor:
    if onnx_tensor.string_data: raise NotImplementedError("Parsing for buffer with string data is not implemented.")
    dtype, shape = self.dtype_parse(onnx_tensor.data_type), tuple(onnx_tensor.dims)
  ...
  def type_parse(self, onnx_type: TypeProto):
    elem_type = onnx_type
    ...
    if elem_type.HasField("tensor_type"):
      ...
      # we want intended dtype for spec
      dtype = self.dtype_parse(elem_type.tensor_type.elem_type)
  ...
  def onnx_ops(self):
    ...
    def EyeLike(x:Tensor, dtype:int|None=None, k:int=0):
      ret = Tensor.eye(cast(int, min(x.shape)), dtype=self.dtype_parse(dtype) if dtype is not None else x.dtype)
    ...
    # ***** Casting Ops *****
    # TODO: saturate
    def Cast(x:Tensor, to:int, saturate:int=1): return x.cast(self.dtype_parse(to))
    ...
    def QuantizeLinear(x:Tensor, y_scale:Tensor, y_zero_point:Tensor|int=0, axis:int=1, block_size:int=0, output_dtype:int=0, saturate=1):
      out_dtype = y_zero_point.dtype if isinstance(y_zero_point, Tensor) else self.dtype_parse(output_dtype) if output_dtype else dtypes.uint8
