...
def get_run_onnx(onnx_model: ModelProto):
  ...
  def buffer_parse(inp: TensorProto) -> Tensor:
    if inp.data_type in (1,10,6,7,9):
      ...
      if len(inp.float_data) > 0:
        ...
        ret = Tensor(np.array(inp.int32_data, dtype=bool if inp.data_type == 9 else np.int32).reshape(inp.dims), requires_grad=False)
      else:
  ...
  def attribute_parse(a: AttributeProto) -> float | int | str | Tensor | tuple[float] | tuple[int]:
    ...
    elif a.type == AttributeProto.GRAPH: return dict(graph=a.g, attribute_dict=prepare_attributes(a.g), output_tensor_names=prepare_output_names(a.g))
  ...
  def prepare_attributes(graph):
    return {num: attribute_to_dict(n.attribute) for num,n in enumerate(graph.node)}

  def prepare_output_names(graph):
    return [x.name for x in graph.output]
  ...
  def run_onnx(inputs={}, debug=False):
    ...
    def run_onnx_graph(graph, attribute_dict, output_tensor_names):
      ...
      for num,n in enumerate(graph.node):
        ...
        # one liners
          ...
          branch = opt["then_branch" if inp[0].numpy().all() else "else_branch"]
          ret = tuple(run_onnx_graph(**branch).values())
        else:
    ...
    return run_onnx_graph(onnx_model.graph, prepare_attributes(onnx_model.graph), prepare_output_names(onnx_model.graph))
  return run_onnx
