...
class ARM64Codegen(AssemblyCodegen):
  def specialize(self, asm):
    ...
    for i, (uop, out, vin, arg) in enumerate(asm):
      if out is not None:
        if out.nm not in live_range:
          live_range[out.nm] = [i,i]
        else: 
          live_range[out.nm][1] = i
      if vin.__class__ is not int:
        for v in vin:
          if v.__class__ is int: continue
          if v.nm not in live_range:
            live_range[v.nm] = [i,i]
          else: 
            live_range[v.nm][1] = i 
    ...
    for i, (uop, out, vin, arg) in enumerate(asm):
      for var, reg in list(rtor.items()):
        ...
        #print(f"i: {i} var: {var} rtor: {rtor[var]} regs:{available_regs}")
        if var[1] != 'B' and i > live_range[var][1]:
      ...
      if uop == UOps.CAST:
        ...
        if arg == BinaryOps.MUL and out.dtype == dtypes.bool:
          ...
          ins.append(f"{'f' if reg == 's' else 's' if arg == BinaryOps.DIV else ''}{alu[arg]} {rtor[out.nm]}, {rtor[vin[0].nm]}, {'x15' if vin[1].__class__ is int else rtor[vin[1].nm]}")
      elif uop == UOps.LOAD:
