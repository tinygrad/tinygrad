...
# ruff: noqa: E501
from extra.assembly.amd.autogen.cdna.enum import DSOp, FLATOp, GLOBALOp, MTBUFOp, MUBUFOp, SCRATCHOp, SMEMOp, SOP1Op, SOP2Op, SOPCOp, SOPKOp, SOPPOp, VOP1Op, VOP2Op, VOP3AOp, VOP3BOp, VOP3POp
...
DSOp_PCODE = {
  ...
  DSOp.DS_MIN_RTN_I32: 'src = DATA.i32;\nMEM[addr].i32 = src < tmp ? src : tmp;\nRETURN_DATA.i32 = tmp\naddr = CalcDsAddr(ADDR.b32, OFFSET0.b32, OFFSET1.b32);\ntmp = MEM[addr].i32;',
  ...
  DSOp.DS_CMPST_RTN_B32: 'cmp = DATA.b32;\nMEM[addr].b32 = tmp == cmp ? src : tmp;\nRETURN_DATA.b32 = tmp\nsrc\ncmp\naddr = CalcDsAddr(ADDR.b32, OFFSET0.b32, OFFSET1.b32);\ntmp = MEM[addr].b32;\nsrc = DATA2.b32;',
  ...
  DSOp.DS_PK_ADD_RTN_F16: 'RETURN_DATA = tmp\ntmp = MEM[ADDR];\nsrc = DATA;\ndst[31 : 16].f16 = tmp[31 : 16].f16 + src[31 : 16].f16;\ndst[15 : 0].f16 = tmp[15 : 0].f16 + src[15 : 0].f16;\nMEM[ADDR] = dst.b32;',
  ...
  DSOp.DS_READ_B128: 'addr = CalcDsAddr(ADDR.b32, 0x0, 0x0);\nRETURN_DATA[31 : 0] = MEM[addr + OFFSET.u32].b32;\nRETURN_DATA[63 : 32] = MEM[addr + OFFSET.u32 + 4U].b32;\nRETURN_DATA[95 : 64] = MEM[addr + OFFSET.u32 + 8U].b32;\nRETURN_DATA[127 : 96] = MEM[addr + OFFSET.u32 + 12U].b32\nÊ   where:\nÊ   OFFSET  = Unsigned immediate byte offset.\nÊ   OFFEN   = Send offset either as VADDR or as zero..\nÊ   IDXEN   = Send index either as VADDR or as zero.\nÊ   LDS     = Data read from/written to LDS or VGPR.\nÊ   OP      = Instruction Opcode.\nÊ   VADDR   = VGPR address source.\nÊ   VDATA   = Destination vector GPR.\nÊ   SRSRC   = Scalar GPR that specifies resource constant.\nÊ   ACC     = Return to ACC VGPRs\nÊ   SC      = Scope\nÊ   NT      = Non-Temporal\nÊ   SOFFSET = Byte offset added to the memory address of an SGPR.',
}
...
FLATOp_PCODE = {
  ...
  FLATOp.FLAT_LOAD_DWORDX3: 'VDATA[95 : 64] = MEM[addr + 8U].b32\naddr = CalcFlatAddr(ADDR.b32, OFFSET.b32);\nVDATA[31 : 0] = MEM[addr].b32;\nVDATA[63 : 32] = MEM[addr + 4U].b32;',
  ...
  FLATOp.FLAT_ATOMIC_UMIN: 'RETURN_DATA.u32 = tmp\naddr = CalcFlatAddr(ADDR.b32, OFFSET.b32);\ntmp = MEM[addr].u32;\nsrc = DATA.u32;\nMEM[addr].u32 = src < tmp ? src : tmp;',
  ...
  FLATOp.FLAT_ATOMIC_SWAP_X2: 'tmp = MEM[addr].b64;\nMEM[addr].b64 = DATA.b64;\nRETURN_DATA.b64 = tmp\naddr = CalcFlatAddr(ADDR.b32, OFFSET.b32);',
  ...
  FLATOp.FLAT_ATOMIC_UMAX_X2: 'MEM[addr].u64 = src >= tmp ? src : tmp;\nRETURN_DATA.u64 = tmp\naddr = CalcFlatAddr(ADDR.b32, OFFSET.b32);\ntmp = MEM[addr].u64;\nsrc = DATA.u64;',
  ...
  FLATOp.FLAT_ATOMIC_INC_X2: 'tmp = MEM[addr].u64;\nsrc = DATA.u64;\nMEM[addr].u64 = tmp >= src ? 0ULL : tmp + 1ULL;\nRETURN_DATA.u64 = tmp\naddr = CalcFlatAddr(ADDR.b32, OFFSET.b32);',
...
GLOBALOp_PCODE = {
  ...
  GLOBALOp.GLOBAL_ATOMIC_CMPSWAP: 'RETURN_DATA.u32 = tmp\naddr = CalcGlobalAddr(ADDR.b32, SADDR.b32, OFFSET.b32);\ntmp = MEM[addr].u32;\nsrc = DATA[31 : 0].u32;\ncmp = DATA[63 : 32].u32;\nMEM[addr].u32 = tmp == cmp ? src : tmp;',
  ...
  GLOBALOp.GLOBAL_ATOMIC_DEC: 'RETURN_DATA.u32 = tmp\naddr = CalcGlobalAddr(ADDR.b32, SADDR.b32, OFFSET.b32);\ntmp = MEM[addr].u32;\nsrc = DATA.u32;\nMEM[addr].u32 = ((tmp == 0U) || (tmp > src)) ? src : tmp - 1U;',
...
MTBUFOp_PCODE = {
  ...
  MTBUFOp.TBUFFER_STORE_FORMAT_XY: 'MEM[addr + ChannelOffsetY()] = ConvertToFormat(VDATA[63 : 32].b32)\naddr = CalcBufferAddr(VADDR.b32, SRSRC.b32, SOFFSET.b32, OFFSET.b32);\nMEM[addr + ChannelOffsetX()] = ConvertToFormat(VDATA[31 : 0].b32);\n// Mem access size depends on format',
  ...
  MTBUFOp.TBUFFER_STORE_FORMAT_D16_XYZW: "Ê   where:\nÊ   OP     = Instruction Opcode.\nÊ   ADDR   = Source of flat address VGPR.\nÊ   DATA   = Source data.\nÊ   VDST   = Destination VGPR.\nÊ   NV     = Access to non-volatile memory.\nÊ   SADDR  = SGPR holding address or offset\nÊ   SEG    = Instruction type: Flat, Scratch, or Global\nÊ   LDS    = Data is transferred between LDS and Memory, not VGPRs.\nÊ   OFFSET = Immediate address byte-offset.\nÊ   SC      = Scope\nÊ   NT      = Non-Temporal\naddr = CalcBufferAddr(VADDR.b32, SRSRC.b32, SOFFSET.b32, OFFSET.b32);\nMEM[addr + ChannelOffsetX()] = ConvertToFormat(32'B(VDATA[15 : 0].b16));\n// Mem access size depends on format\nMEM[addr + ChannelOffsetY()] = ConvertToFormat(32'B(VDATA[31 : 16].b16));\nMEM[addr + ChannelOffsetZ()] = ConvertToFormat(32'B(VDATA[47 : 32].b16));\nMEM[addr + ChannelOffsetW()] = ConvertToFormat(32'B(VDATA[63 : 48].b16))",
}
...
MUBUFOp_PCODE = {
  ...
  MUBUFOp.BUFFER_LOAD_FORMAT_D16_XY: "// Mem access size depends on format\nVDATA[31 : 16].b16 = 16'B(ConvertFromFormat(MEM[addr + ChannelOffsetY()]))\naddr = CalcBufferAddr(VADDR.b32, SRSRC.b32, SOFFSET.b32, OFFSET.b32);\nVDATA[15 : 0].b16 = 16'B(ConvertFromFormat(MEM[addr + ChannelOffsetX()]));",
  ...
  MUBUFOp.BUFFER_STORE_SHORT: 'MEM[addr].b16 = VDATA[15 : 0]\naddr = CalcBufferAddr(VADDR.b32, SRSRC.b32, SOFFSET.b32, OFFSET.b32);',
  ...
  MUBUFOp.BUFFER_LOAD_SBYTE_D16_HI: "VDATA[31 : 16].i16 = 16'I(signext(MEM[addr].i8));\n// VDATA[15:0] is preserved.\naddr = CalcBufferAddr(VADDR.b32, SRSRC.b32, SOFFSET.b32, OFFSET.b32);",
  ...
  MUBUFOp.BUFFER_STORE_FORMAT_D16_HI_X: "// Mem access size depends on format\naddr = CalcBufferAddr(VADDR.b32, SRSRC.b32, SOFFSET.b32, OFFSET.b32);\nMEM[addr + ChannelOffsetX()] = ConvertToFormat(32'B(VDATA[31 : 16].b16));",
  ...
  MUBUFOp.BUFFER_ATOMIC_ADD: 'tmp = MEM[addr].u32;\nMEM[addr].u32 += DATA.u32;\nRETURN_DATA.u32 = tmp\naddr = CalcBufferAddr(VADDR.b32, SRSRC.b32, SOFFSET.b32, OFFSET.b32);',
  ...
  MUBUFOp.BUFFER_ATOMIC_MIN_F64: 'RETURN_DATA.f64 = tmp\naddr = CalcBufferAddr(VADDR.b32, SRSRC.b32, SOFFSET.b32, OFFSET.b32);\ntmp = MEM[addr].f64;\nsrc = DATA.f64;\nMEM[addr].f64 = src < tmp ? src : tmp;',
  ...
  MUBUFOp.BUFFER_ATOMIC_DEC_X2: 'addr = CalcBufferAddr(VADDR.b32, SRSRC.b32, SOFFSET.b32, OFFSET.b32);\ntmp = MEM[addr].u64;\nsrc = DATA.u64;\nMEM[addr].u64 = ((tmp == 0ULL) || (tmp > src)) ? src : tmp - 1ULL;\nRETURN_DATA.u64 = tmp\nÊ   where:\nÊ   OFFSET  = Unsigned immediate byte offset.\nÊ   OFFEN   = Send offset either as VADDR or as zero.\nÊ   IDXEN   = Send index either as VADDR or as zero.\nÊ   LDS     = Data is transferred between LDS and Memory, not VGPRs.\nÊ   OP      = Instruction Opcode.\nÊ   DFMT    = Data format for typed buffer.\nÊ   NFMT    = Number format for typed buffer.\nÊ   VADDR   = VGPR address source.\nÊ   VDATA   = Vector GPR for read/write result.\nÊ   SRSRC   = Scalar GPR that specifies resource constant.\nÊ   SOFFSET = Unsigned byte offset from an SGPR.\nÊ   SC      = Scope\nÊ   NT      = Non-Temporal\nÊ   ACC     = Return to ACC VGPRs',
}
...
SMEMOp_PCODE = {
  ...
  SMEMOp.S_BUFFER_ATOMIC_CMPSWAP: 'cmp = DATA[63 : 32].u32;\nMEM[addr].u32 = tmp == cmp ? src : tmp;\nRETURN_DATA.u32 = tmp\naddr = CalcScalarBufferAddr(SBASE.b32, SOFFSET.b32, OFFSET.i32);\ntmp = MEM[addr].u32;\nsrc = DATA[31 : 0].u32;',
  ...
  SMEMOp.S_BUFFER_ATOMIC_UMIN: 'tmp = MEM[addr].u32;\nsrc = DATA.u32;\nMEM[addr].u32 = src < tmp ? src : tmp;\nRETURN_DATA.u32 = tmp\naddr = CalcScalarBufferAddr(SBASE.b32, SOFFSET.b32, OFFSET.i32);',
  ...
  SMEMOp.S_ATOMIC_OR_X2: 'RETURN_DATA.b64 = tmp\naddr = CalcScalarGlobalAddr(SBASE.b32, SOFFSET.b32, OFFSET.i32);\ntmp = MEM[addr].b64;\nMEM[addr].b64 = (tmp | DATA.b64);',
...
SOP1Op_PCODE = {
  ...
  SOP1Op.S_BCNT0_I32_B64: "D0.i32 = tmp;\nSCC = D0.u64 != 0ULL\ntmp = 0;\nfor i in 0 : 63 do\nÊ   tmp += S0.u64[i] == 1'0U ? 1 : 0\nendfor;",
  ...
  SOP1Op.S_AND_SAVEEXEC_B64: 'EXEC.u64 = (S0.u64 & EXEC.u64);\nD0.u64 = saveexec.u64;\nSCC = EXEC.u64 != 0ULL\nsaveexec = EXEC.u64;',
  ...
  SOP1Op.S_CBRANCH_JOIN: "saved_csp = S0.u32;\nif WAVE_MODE.CSP.u32 == saved_csp then\nÊ   PC += 4LL;\nÊ   // Second time to JOIN: continue with program.\nelse\nÊ   WAVE_MODE.CSP -= 3'1U;\nÊ   // First time to JOIN; jump to other FORK path.\nÊ   { PC, EXEC } = SGPR[WAVE_MODE.CSP.u32 * 4U].b128;\nÊ   // Read 128 bits from 4 consecutive SGPRs.\nendif",
  ...
  SOP1Op.S_ANDN2_WREXEC_B64: 'EXEC.u64 = (S0.u64 & ~EXEC.u64);\nD0.u64 = EXEC.u64;\nSCC = EXEC.u64 != 0ULL\nÊ   // V0 holds the index value per lane\nÊ   // save exec mask for restore at the end\nÊ   s_mov_b64 s2, exec\nÊ   // exec mask of remaining (unprocessed) threads\nÊ   s_mov_b64 s4, exec\nÊ   loop:\nÊ   // get the index value for the first active lane\nÊ   v_readfirstlane_b32  s0, v0\nÊ   // find all other lanes with same index value\nÊ   v_cmpx_eq s0, v0\nÊ   <OP>        // do the operation using the current EXEC mask. S0 holds the index.\nÊ   // mask out thread that was just executed\nÊ   // s_andn2_b64  s4, s4, exec\nÊ   // s_mov_b64    exec, s4\nÊ   s_andn2_wrexec_b64 s4, s4     // replaces above 2 ops\nÊ   // repeat until EXEC==0\nÊ   s_cbranch_scc1  loop\nÊ   s_mov_b64    exec, s2',
...
SOP2Op_PCODE = {
  ...
  SOP2Op.S_XOR_B64: 'SCC = D0.u64 != 0ULL\nD0.u64 = (S0.u64 ^ S1.u64);',
  ...
  SOP2Op.S_LSHR_B64: 'SCC = D0.u64 != 0ULL\nD0.u64 = (S0.u64 >> S1[5 : 0].u32);',
...
SOPCOp_PCODE = {
  ...
  SOPCOp.S_SETVSKIP: 'VSKIP = S0.u32[S1.u32[4 : 0]]\nÊ   s_setvskip 1, 0     // Enable vskip mode.\nÊ   s_setvskip 0, 0     // Disable vskip mode.',
...
SOPPOp_PCODE = {
  ...
  SOPPOp.S_BRANCH: "Ê   s_branch label    // Set SIMM16 = +4 = 0x0004\nÊ   s_nop 0    // 4 bytes\nlabel:\nÊ   s_nop 0    // 4 bytes\nÊ   s_branch label    // Set SIMM16 = -8 = 0xfff8\nPC = PC + signext(SIMM16.i16 * 16'4) + 4LL;\n// short jump.",
  ...
  SOPPOp.S_CBRANCH_EXECNZ: "Ê   PC = PC + signext(SIMM16.i16 * 16'4) + 4LL\nelse\nÊ   PC = PC + 4LL\nendif\nif EXECZ.u1 == 1'0U then",
...
VOP1Op_PCODE = {
  ...
  VOP1Op.V_CVT_F32_FP8: 'Ê   D0.f32 = fp8_to_f32(S0[23 : 16].fp8)\nelsif SDWA_SRC0_SEL == BYTE3.b3 then\nÊ   D0.f32 = fp8_to_f32(S0[31 : 24].fp8)\nelse\nÊ   // BYTE0 implied\nÊ   D0.f32 = fp8_to_f32(S0[7 : 0].fp8)\nendif\nif SDWA_SRC0_SEL == BYTE1.b3 then\nÊ   D0.f32 = fp8_to_f32(S0[15 : 8].fp8)\nelsif SDWA_SRC0_SEL == BYTE2.b3 then',
  VOP1Op.V_CVT_F32_BF8: 'if SDWA_SRC0_SEL == BYTE1.b3 then\nÊ   D0.f32 = bf8_to_f32(S0[15 : 8].bf8)\nelsif SDWA_SRC0_SEL == BYTE2.b3 then\nÊ   D0.f32 = bf8_to_f32(S0[23 : 16].bf8)\nelsif SDWA_SRC0_SEL == BYTE3.b3 then\nÊ   D0.f32 = bf8_to_f32(S0[31 : 24].bf8)\nelse\nÊ   // BYTE0 implied\nÊ   D0.f32 = bf8_to_f32(S0[7 : 0].bf8)\nendif',
  ...
  VOP1Op.V_PERMLANE16_SWAP_B32: 'for pass in 0 : 1 do\nÊ   for lane in 0 : 15 do\nÊ       tmp = VGPR[pass * 32 + lane][SRC0.u32];\nÊ       VGPR[pass * 32 + lane][SRC0.u32] = VGPR[pass * 32 + lane + 16][VDST.u32];\nÊ       VGPR[pass * 32 + lane + 16][VDST.u32] = tmp\nÊ   endfor\nendfor',
  VOP1Op.V_PERMLANE32_SWAP_B32: 'for lane in 0 : 31 do\nÊ   tmp = VGPR[lane][SRC0.u32];\nÊ   VGPR[lane][SRC0.u32] = VGPR[lane + 32][VDST.u32];\nÊ   VGPR[lane + 32][VDST.u32] = tmp\nendfor',
  VOP1Op.V_CVT_F32_BF16: "Ê   where:\nÊ   SRC0  = First operand for instruction.\nÊ   VSRC1 = Second operand for instruction.\nÊ   OP    = Instructions.\nÊ   All VOPC instructions can alternatively be encoded in the VOP3A format.\nD0.f32 = 32'F({ S0.b16, 16'0U })",
}
...
VOP2Op_PCODE = {
  ...
  VOP2Op.V_MAC_F16: "if OPSEL.u4[3] then\nÊ   D0 = { tmp.f16, D0[15 : 0] }\nelse\nÊ   D0 = { 16'0, tmp.f16 }\nendif\ntmp = S0.f16 * S1.f16 + D0.f16;",
...
VOP3AOp_PCODE = {
  ...
  VOP3AOp.V_CMP_NGE_F16: '// With NAN inputs this is not the same operation as <\n// D0 = VCC in VOPC encoding.\nD0.u64[laneId] = !(S0.f16 >= S1.f16);',
  ...
  VOP3AOp.V_CMPX_EQ_F16: '// D0 = VCC in VOPC encoding.\nEXEC.u64[laneId] = D0.u64[laneId] = S0.f16 == S1.f16;',
  ...
  VOP3AOp.V_CMPX_NGT_F16: '// D0 = VCC in VOPC encoding.\nEXEC.u64[laneId] = D0.u64[laneId] = !(S0.f16 > S1.f16);\n// With NAN inputs this is not the same operation as <=',
  ...
  VOP3AOp.V_CMP_NGE_F32: '// With NAN inputs this is not the same operation as <\n// D0 = VCC in VOPC encoding.\nD0.u64[laneId] = !(S0.f32 >= S1.f32);',
  ...
  VOP3AOp.V_CMPX_EQ_F32: '// D0 = VCC in VOPC encoding.\nEXEC.u64[laneId] = D0.u64[laneId] = S0.f32 == S1.f32;',
  ...
  VOP3AOp.V_CMPX_NGT_F32: '// D0 = VCC in VOPC encoding.\nEXEC.u64[laneId] = D0.u64[laneId] = !(S0.f32 > S1.f32);\n// With NAN inputs this is not the same operation as <=',
  ...
  VOP3AOp.V_CMP_NGE_F64: '// With NAN inputs this is not the same operation as <\n// D0 = VCC in VOPC encoding.\nD0.u64[laneId] = !(S0.f64 >= S1.f64);',
  ...
  VOP3AOp.V_CMPX_EQ_F64: '// D0 = VCC in VOPC encoding.\nEXEC.u64[laneId] = D0.u64[laneId] = S0.f64 == S1.f64;',
  ...
  VOP3AOp.V_CMPX_NGT_F64: '// D0 = VCC in VOPC encoding.\nEXEC.u64[laneId] = D0.u64[laneId] = !(S0.f64 > S1.f64);\n// With NAN inputs this is not the same operation as <=',
  ...
  VOP3AOp.V_CMP_LT_I32: '// D0 = VCC in VOPC encoding.\nD0.u64[laneId] = S0.i32 < S1.i32;',
  ...
  VOP3AOp.V_CMPX_LT_U32: '// D0 = VCC in VOPC encoding.\nEXEC.u64[laneId] = D0.u64[laneId] = S0.u32 < S1.u32;',
  ...
  VOP3AOp.V_CMPX_LT_I64: '// D0 = VCC in VOPC encoding.\nEXEC.u64[laneId] = D0.u64[laneId] = S0.i64 < S1.i64;',
  ...
  VOP3AOp.V_MAX_F32: "elsif isNAN(64'F(S0.f32)) then\nÊ   D0.f32 = S1.f32\nelsif isNAN(64'F(S1.f32)) then\nÊ   D0.f32 = S0.f32\nelsif ((64'F(S0.f32) == +0.0) && (64'F(S1.f32) == -0.0)) then\nÊ   D0.f32 = S0.f32\nelsif ((64'F(S0.f32) == -0.0) && (64'F(S1.f32) == +0.0)) then\nÊ   D0.f32 = S1.f32\nelsif WAVE_MODE.IEEE then\nÊ   D0.f32 = S0.f32 >= S1.f32 ? S0.f32 : S1.f32\nelse\nÊ   D0.f32 = S0.f32 > S1.f32 ? S0.f32 : S1.f32\nendif\nif (WAVE_MODE.IEEE && isSignalNAN(64'F(S0.f32))) then\nÊ   D0.f32 = 32'F(cvtToQuietNAN(64'F(S0.f32)))\nelsif (WAVE_MODE.IEEE && isSignalNAN(64'F(S1.f32))) then\nÊ   D0.f32 = 32'F(cvtToQuietNAN(64'F(S1.f32)))",
  ...
  VOP3AOp.V_DOT2C_F32_F16: 'D0.f32 = tmp\ntmp = D0.f32;\ntmp += f16_to_f32(S0[15 : 0].f16) * f16_to_f32(S1[15 : 0].f16);\ntmp += f16_to_f32(S0[31 : 16].f16) * f16_to_f32(S1[31 : 16].f16);',
  ...
  VOP3AOp.V_MOV_B32: 'Ê   v_mov_b32 v0, v1    // Move into v0 from v1\nÊ   v_mov_b32 v0, -v1   // Set v0 to the negation of v1\nÊ   v_mov_b32 v0, abs(v1)   // Set v0 to the absolute value of v1\nD0.b32 = S0.b32',
  ...
  VOP3AOp.V_FFBL_B32: "Ê   // Search from LSB\nÊ   if S0.u32[i] == 1'1U then\nÊ       D0.i32 = i;\nÊ       break\nÊ   endif\nendfor\nV_FFBL_B32(0x00000000) => 0xffffffff\nV_FFBL_B32(0xff000001) => 0\nV_FFBL_B32(0xff000008) => 3\nV_FFBL_B32(0xffff0000) => 16\nV_FFBL_B32(0x80000000) => 31\nD0.i32 = -1;\n// Set if no ones are found\nfor i in 0 : 31 do",
  ...
  VOP3AOp.V_LOG_F16: 'V_LOG_F16(0xfc00) => 0xfe00     // log(-INF) = NAN\nV_LOG_F16(0xbc00) => 0xfe00     // log(-1.0) = NAN\nV_LOG_F16(0x8000) => 0xfc00     // log(-0.0) = -INF\nV_LOG_F16(0x0000) => 0xfc00     // log(+0.0) = -INF\nV_LOG_F16(0x3c00) => 0x0000     // log(+1.0) = 0\nV_LOG_F16(0x7c00) => 0x7c00     // log(+INF) = +INF\nD0.f16 = log2(S0.f16)',
  ...
  VOP3AOp.V_CUBESC_F32: 'Ê       D0.f32 = -S2.f32\nÊ   endif\nendif\n// D0.f = cubemap S coordinate.\n// XYZ coordinate is given in (S0.f, S1.f, S2.f).\n// S0.f = x\n// S1.f = y\n// S2.f = z\nif ((abs(S2.f32) >= abs(S0.f32)) && (abs(S2.f32) >= abs(S1.f32))) then\nÊ   if S2.f32 < 0.0F then\nÊ       D0.f32 = -S0.f32\nÊ   else\nÊ       D0.f32 = S0.f32\nÊ   endif\nelsif abs(S1.f32) >= abs(S0.f32) then\nÊ   D0.f32 = S0.f32\nelse\nÊ   if S0.f32 < 0.0F then\nÊ       D0.f32 = S2.f32\nÊ   else',
  ...
  VOP3AOp.V_CVT_SCALEF32_F16_FP8: "tmp = fp8_to_f16_scale(src, scale.u8);\n// OPSEL[3] controls destination hi/lo\nD0 = tmp.b32\nscale = 32'U(exponent(S1.f32));\nsrcbyte = OPSEL[1 : 0].i32 * 8;\nsrc = VGPR[laneId][SRC0.u32][srcbyte + 7 : srcbyte].fp8;",
  ...
  VOP3AOp.V_CVT_SCALEF32_2XPK16_FP6_F32: "scale = 32'U(exponent(S2.f32));\ndeclare tmp : 192'B;\nfor pass in 0 : 15 do\nÊ   dOffset = pass * 12;\nÊ   sOffset = pass * 32;\nÊ   // Note that S0 and S1 inputs are interleaved in the packed result.\nÊ   tmp[dOffset + 5 : dOffset].fp6 = f32_to_fp6_scale(S0[sOffset + 31 : sOffset].f32, scale.u8);\nÊ   tmp[dOffset + 11 : dOffset + 6].fp6 = f32_to_fp6_scale(S1[sOffset + 31 : sOffset].f32, scale.u8)\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_2XPK16_BF6_F32: "Ê   // Note that S0 and S1 inputs are interleaved in the packed result.\nÊ   tmp[dOffset + 5 : dOffset].bf6 = f32_to_bf6_scale(S0[sOffset + 31 : sOffset].f32, scale.u8);\nÊ   tmp[dOffset + 11 : dOffset + 6].bf6 = f32_to_bf6_scale(S1[sOffset + 31 : sOffset].f32, scale.u8)\nendfor;\nD0[191 : 0] = tmp.b192\nscale = 32'U(exponent(S2.f32));\ndeclare tmp : 192'B;\nfor pass in 0 : 15 do\nÊ   dOffset = pass * 12;\nÊ   sOffset = pass * 32;",
  VOP3AOp.V_CVT_SCALEF32_SR_PK32_FP6_F32: "scale = 32'U(exponent(S2.f32));\nrandomVal = S1.u32;\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 32;\nÊ   tmp[dOffset + 5 : dOffset].fp6 = f32_to_fp6_sr_scale(S0[sOffset + 31 : sOffset].f32, randomVal,\nscale.u8);\nÊ   randomVal = 32'U(v_prng_b32(randomVal.b32))\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_SR_PK32_BF6_F32: "scale = 32'U(exponent(S2.f32));\nrandomVal = S1.u32;\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 32;\nÊ   tmp[dOffset + 5 : dOffset].bf6 = f32_to_bf6_sr_scale(S0[sOffset + 31 : sOffset].f32, randomVal,\nscale.u8);\nÊ   randomVal = 32'U(v_prng_b32(randomVal.b32))\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_PK32_F32_FP6: "scale = 32'U(exponent(S1.f32));\ndeclare tmp : 1024'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 32;\nÊ   sOffset = pass * 6;\nÊ   tmp[dOffset + 31 : dOffset].f32 = fp6_to_f32_scale(S0[sOffset + 5 : sOffset].fp6, scale.u8)\nendfor;\nD0[1023 : 0] = tmp.b1024",
  VOP3AOp.V_CVT_SCALEF32_PK32_F32_BF6: "scale = 32'U(exponent(S1.f32));\ndeclare tmp : 1024'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 32;\nÊ   sOffset = pass * 6;\nÊ   tmp[dOffset + 31 : dOffset].f32 = bf6_to_f32_scale(S0[sOffset + 5 : sOffset].bf6, scale.u8)\nendfor;\nD0[1023 : 0] = tmp.b1024",
  VOP3AOp.V_CVT_SCALEF32_PK32_FP6_F16: "scale = 32'U(exponent(S1.f32));\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 16;\nÊ   tmp[dOffset + 5 : dOffset].fp6 = f16_to_fp6_scale(S0[sOffset + 15 : sOffset].f16, scale.u8)\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_PK32_FP6_BF16: "scale = 32'U(exponent(S1.f32));\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 16;\nÊ   tmp[dOffset + 5 : dOffset].fp6 = bf16_to_fp6_scale(S0[sOffset + 15 : sOffset].bf16, scale.u8)\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_PK32_BF6_F16: "scale = 32'U(exponent(S1.f32));\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 16;\nÊ   tmp[dOffset + 5 : dOffset].bf6 = f16_to_bf6_scale(S0[sOffset + 15 : sOffset].f16, scale.u8)\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_PK32_BF6_BF16: "scale = 32'U(exponent(S1.f32));\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 16;\nÊ   tmp[dOffset + 5 : dOffset].bf6 = bf16_to_bf6_scale(S0[sOffset + 15 : sOffset].bf16, scale.u8)\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_SR_PK32_FP6_F16: "scale = 32'U(exponent(S2.f32));\nrandomVal = S1.u32;\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 16;\nÊ   tmp[dOffset + 5 : dOffset].fp6 = f16_to_fp6_sr_scale(S0[sOffset + 15 : sOffset].f16, randomVal,\nscale.u8);\nÊ   randomVal = 32'U(v_prng_b32(randomVal.b32))\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_SR_PK32_FP6_BF16: "scale = 32'U(exponent(S2.f32));\nrandomVal = S1.u32;\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 16;\nÊ   tmp[dOffset + 5 : dOffset].fp6 = bf16_to_fp6_sr_scale(S0[sOffset + 15 : sOffset].bf16, randomVal,\nscale.u8);\nÊ   randomVal = 32'U(v_prng_b32(randomVal.b32))\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_SR_PK32_BF6_F16: "endfor;\nD0[191 : 0] = tmp.b192\nscale = 32'U(exponent(S2.f32));\nrandomVal = S1.u32;\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 16;\nÊ   tmp[dOffset + 5 : dOffset].bf6 = f16_to_bf6_sr_scale(S0[sOffset + 15 : sOffset].f16, randomVal,\nscale.u8);\nÊ   randomVal = 32'U(v_prng_b32(randomVal.b32))",
  VOP3AOp.V_CVT_SCALEF32_SR_PK32_BF6_BF16: "scale = 32'U(exponent(S2.f32));\nrandomVal = S1.u32;\ndeclare tmp : 192'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 6;\nÊ   sOffset = pass * 16;\nÊ   tmp[dOffset + 5 : dOffset].bf6 = bf16_to_bf6_sr_scale(S0[sOffset + 15 : sOffset].bf16, randomVal,\nscale.u8);\nÊ   randomVal = 32'U(v_prng_b32(randomVal.b32))\nendfor;\nD0[191 : 0] = tmp.b192",
  VOP3AOp.V_CVT_SCALEF32_PK32_F16_FP6: "scale = 32'U(exponent(S1.f32));\ndeclare tmp : 512'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 16;\nÊ   sOffset = pass * 6;\nÊ   tmp[dOffset + 15 : dOffset].f16 = fp6_to_f16_scale(S0[sOffset + 5 : sOffset].fp6, scale.u8)\nendfor;\nD0[511 : 0] = tmp.b512",
  VOP3AOp.V_CVT_SCALEF32_PK32_BF16_FP6: "Ê   dOffset = pass * 16;\nÊ   sOffset = pass * 6;\nÊ   tmp[dOffset + 15 : dOffset].bf16 = fp6_to_bf16_scale(S0[sOffset + 5 : sOffset].fp6, scale.u8)\nendfor;\nD0[511 : 0] = tmp.b512\nscale = 32'U(exponent(S1.f32));\ndeclare tmp : 512'B;\nfor pass in 0 : 31 do",
  VOP3AOp.V_CVT_SCALEF32_PK32_F16_BF6: "scale = 32'U(exponent(S1.f32));\ndeclare tmp : 512'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 16;\nÊ   sOffset = pass * 6;\nÊ   tmp[dOffset + 15 : dOffset].f16 = bf6_to_f16_scale(S0[sOffset + 5 : sOffset].bf6, scale.u8)\nendfor;\nD0[511 : 0] = tmp.b512",
  VOP3AOp.V_CVT_SCALEF32_PK32_BF16_BF6: "scale = 32'U(exponent(S1.f32));\ndeclare tmp : 512'B;\nfor pass in 0 : 31 do\nÊ   dOffset = pass * 16;\nÊ   sOffset = pass * 6;\nÊ   tmp[dOffset + 15 : dOffset].bf16 = bf6_to_bf16_scale(S0[sOffset + 5 : sOffset].bf6, scale.u8)\nendfor;\nD0[511 : 0] = tmp.b512",
  VOP3AOp.V_ASHR_PK_I8_I32: "Ê   elsif n >= 127 then\nÊ       return 8'0x7f\nÊ   else\nÊ       return n[7 : 0].b8\nÊ   endif);\ndeclare tmp : 16'B;\ntmp[7 : 0] = SAT8(S0.i32 >> S2[4 : 0].u32);\ntmp[15 : 8] = SAT8(S1.i32 >> S2[4 : 0].u32);\nD0[15 : 0] = tmp\nSAT8 = lambda(n) (\nÊ   if n <= -128 then\nÊ       return 8'0x80",
  ...
  VOP3AOp.V_WRITELANE_B32: '// Lane select\nVGPR[lane][VDST.u32] = S0.b32\nlane = S1.u32[5 : 0];',
  ...
  VOP3AOp.V_CVT_SR_F16_F32: "prev_mode = ROUND_MODE;\nROUND_MODE = ROUND_NEAREST_EVEN;\nif OPSEL[3].u2 == 2'0U then\nÊ   VGPR[laneId][VDST.u32][15 : 0].f16 = 16'F(f32_to_f16_SR(S0.f32, S1.u32))\nelse\nÊ   VGPR[laneId][VDST.u32][31 : 16].f16 = 16'F(f32_to_f16_sr(S0.f32, S1.u32))\nendif;\n// Unwritten bytes of D are preserved.\nROUND_MODE = prev_mode",
  VOP3AOp.V_CVT_SR_BF16_F32: "prev_mode = ROUND_MODE;\nROUND_MODE = ROUND_NEAREST_EVEN;\nif OPSEL[3].u2 == 2'0U then\nÊ   VGPR[laneId][VDST.u32][15 : 0].bf16 = 16'BF(f32_to_bf16_SR(S0.f32, S1.u32))\nelse\nÊ   VGPR[laneId][VDST.u32][31 : 16].bf16 = 16'BF(f32_to_bf16_sr(S0.f32, S1.u32))\nendif;\n// Unwritten bytes of D are preserved.\nROUND_MODE = prev_mode",
...
VOP3POp_PCODE = {
  ...
  VOP3POp.V_MFMA_F32_32X32X64_F8F6F4: 'A, B: (32*64) elements * 8 bits/element = 16384 bits, divide by 64 lanes * 32 bits/register-lane = 8\nregisters C, D: (32*32) elements * 32 bits/element = 32768 bits, divide by 64 lanes * 32 bits/register-\nlane = 16 registers\nD = A (32x64) * B (64x32) + C (32x32)',
