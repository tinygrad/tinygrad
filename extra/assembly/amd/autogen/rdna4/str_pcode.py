...
# ruff: noqa: E501
from extra.assembly.amd.autogen.rdna4.enum import DSOp, SMEMOp, SOP1Op, SOP2Op, SOPCOp, SOPKOp, SOPPOp, VBUFFEROp, VFLATOp, VGLOBALOp, VIMAGEOp, VINTERPOp, VOP1Op, VOP2Op, VOP3Op, VOP3POp, VOP3SDOp, VOPDOp, VSAMPLEOp, VSCRATCHOp
...
DSOp_PCODE = {
  ...
  DSOp.DS_AND_RTN_B32: 'MEM[addr].b32 = (tmp & DATA.b32);\nRETURN_DATA.b32 = tmp\naddr = CalcDsAddr(vgpr_a.b32, offset.b32);\ntmp = MEM[addr].b32;',
  ...
  DSOp.DS_STOREXCHG_RTN_B32: 'RETURN_DATA.b32 = tmp\naddr = CalcDsAddr(vgpr_a.b32, offset.b32);\ntmp = MEM[addr].b32;\nMEM[addr].b32 = DATA.b32;',
  ...
  DSOp.DS_INC_RTN_U64: 'MEM[addr].u64 = tmp >= src ? 0ULL : tmp + 1ULL;\nRETURN_DATA.u64 = tmp\naddr = CalcDsAddr(vgpr_a.b32, offset.b32);\ntmp = MEM[addr].u64;\nsrc = DATA.u64;',
  ...
  DSOp.DS_MIN_NUM_RTN_F64: 'Ê   MEM[ADDR].f64 = tmp.f64\nelsif isNAN(tmp.f64) then\nÊ   MEM[ADDR].f64 = src.f64\nelsif ((src.f64 < tmp.f64) || ((abs(src.f64) == 0.0) && (abs(tmp.f64) == 0.0) && sign(src.f64) &&\n!sign(tmp.f64))) then\nÊ   // NOTE: -0<+0 is TRUE in this comparison\nÊ   MEM[ADDR].f64 = src.f64\nelse\nÊ   MEM[ADDR].f64 = tmp.f64\nendif;\nRETURN_DATA.f64 = tmp\ntmp = MEM[ADDR].f64;\nsrc = DATA.f64;\nif (isNAN(src.f64) && isNAN(tmp.f64)) then\nÊ   MEM[ADDR].f64 = cvtToQuietNAN(src.f64)\nelsif isNAN(src.f64) then',
  ...
  DSOp.DS_BVH_STACK_PUSH4_POP1_RTN_B32: "endif;\nRETURN_ADDR[31 : 0] = 32'B(ENCODE_ADDR(stack_base, stack_index))\ndeclare stack_base : 32'B;\ndeclare stack_index : 32'U;\ndeclare DATA1 : 32'B;\ndeclare last_node_ptr : 32'B;\ndeclare INVALID_NODE : 32'B;\nDATA_VALID = lambda(data) (\nÊ   if data == INVALID_NODE then\nÊ       return 1'0U\nÊ   elsif ((last_node_ptr != INVALID_NODE) && (data == last_node_ptr)) then\nÊ       // Match last_node_ptr\nÊ       return 1'0U\nÊ   else\nÊ       return 1'1U\nÊ   endif);\n// main code\n{ stack_base, stack_index } = 64'B(DECODE_ADDR(ADDR, OFFSET0));\nlast_node_ptr = DATA0.b32;\n// First 3 passes: push data onto stack\nfor i in 0 : 2 do\nÊ   if DATA_VALID(DATA1[i * 32 + 31 : i * 32]) then\nÊ       MEM[stack_base.u32 + stack_index] = DATA1[i * 32 + 31 : i * 32];\nÊ       stack_index += 1U\nÊ   elsif DATA1[i].b32 == last_node_ptr then\nÊ       // Treat all further data as invalid as well.\nÊ       break\nÊ   endif\nendfor;\n// Last pass: return data or pop\nif DATA_VALID(DATA1[127 : 96]) then\nÊ   RETURN_DATA[31 : 0] = DATA1[127 : 96]\nelse\nÊ   RETURN_DATA[31 : 0] = MEM[stack_base.u32 + stack_index];\nÊ   MEM[stack_base.u32 + stack_index] = INVALID_NODE;\nÊ   stack_index -= 1U",
  DSOp.DS_BVH_STACK_PUSH8_POP1_RTN_B32: "if DATA_VALID(DATA1[255 : 224]) then\nÊ   RETURN_DATA[31 : 0] = DATA1[255 : 224]\nelse\nÊ   RETURN_DATA[31 : 0] = MEM[stack_base.u32 + stack_index];\nÊ   MEM[stack_base.u32 + stack_index] = INVALID_NODE;\nÊ   stack_index -= 1U\nendif;\nRETURN_ADDR[31 : 0] = 32'B(ENCODE_ADDR(stack_base, stack_index))\ndeclare stack_base : 32'B;\ndeclare stack_index : 32'U;\ndeclare DATA1 : 32'B;\ndeclare last_node_ptr : 32'B;\ndeclare INVALID_NODE : 32'B;\nDATA_VALID = lambda(data) (\nÊ   if data == INVALID_NODE then\nÊ       return 1'0U\nÊ   elsif ((last_node_ptr != INVALID_NODE) && (data == last_node_ptr)) then\nÊ       // Match last_node_ptr\nÊ       return 1'0U\nÊ   else\nÊ       return 1'1U\nÊ   endif);\n// main code\n{ stack_base, stack_index } = 64'B(DECODE_ADDR(ADDR, OFFSET0));\nlast_node_ptr = DATA0.b32;\n// First 7 passes: push data onto stack\nfor i in 0 : 6 do\nÊ   if DATA_VALID(DATA1[i * 32 + 31 : i * 32]) then\nÊ       MEM[stack_base.u32 + stack_index] = DATA1[i * 32 + 31 : i * 32];\nÊ       stack_index += 1U\nÊ   elsif DATA1[i].b32 == last_node_ptr then\nÊ       // Treat all further data as invalid as well.\nÊ       break\nÊ   endif\nendfor;\n// Last pass: return data or pop",
  DSOp.DS_BVH_STACK_PUSH8_POP2_RTN_B64: "Ê   elsif DATA1[i].b32 == last_node_ptr then\nÊ       // Treat all further data as invalid as well.\nÊ       break\nÊ   endif\nendfor;\n// Last pass: return data or pop\nif DATA_VALID(DATA1[255 : 224]) then\nÊ   RETURN_DATA[31 : 0] = DATA1[255 : 224]\nelse\nÊ   RETURN_DATA[31 : 0] = MEM[stack_base.u32 + stack_index];\nÊ   MEM[stack_base.u32 + stack_index] = INVALID_NODE;\nÊ   stack_index -= 1U\nendif;\n// Attempt a second pop\nif DATA_VALID(MEM[stack_base.u32 + stack_index]) then\nÊ   RETURN_DATA[63 : 32] = MEM[stack_base.u32 + stack_index];\nÊ   MEM[stack_base.u32 + stack_index] = INVALID_NODE;\nÊ   stack_index -= 1U\nendif;\nRETURN_ADDR[31 : 0] = 32'B(ENCODE_ADDR(stack_base, stack_index))\ndeclare stack_base : 32'B;\ndeclare stack_index : 32'U;\ndeclare DATA1 : 32'B;\ndeclare last_node_ptr : 32'B;\ndeclare INVALID_NODE : 32'B;\nDATA_VALID = lambda(data) (\nÊ   if data == INVALID_NODE then\nÊ       return 1'0U\nÊ   elsif ((last_node_ptr != INVALID_NODE) && (data == last_node_ptr)) then\nÊ       // Match last_node_ptr\nÊ       return 1'0U\nÊ   else\nÊ       return 1'1U\nÊ   endif);\n// main code\n{ stack_base, stack_index } = 64'B(DECODE_ADDR(ADDR, OFFSET0));\nlast_node_ptr = DATA0.b32;\n// First 7 passes: push data onto stack\nfor i in 0 : 6 do\nÊ   if DATA_VALID(DATA1[i * 32 + 31 : i * 32]) then\nÊ       MEM[stack_base.u32 + stack_index] = DATA1[i * 32 + 31 : i * 32];\nÊ       stack_index += 1U",
...
SMEMOp_PCODE = {
  ...
  SMEMOp.S_PREFETCH_INST: "if MODE.SCALAR_PREFETCH_EN.u1 then\nÊ   mem_addr = (64'U(S0[63 : 0].i64 + 64'I(IOFFSET.i24)) & 0xffffffffffffff80ULL);\nÊ   // Force 128B alignment\nÊ   length = S2.u32;\nÊ   // SGPR or M0\nÊ   length += SDATA.u32;\nÊ   // SDATA is an immediate\nÊ   length = (length & 31U);\nÊ   // Length restricted to 0..31\nÊ   length = (length + 1U) * 128U;\nÊ   // Prefetch 1-32 cachelines, units of 128B\nÊ   PrefetchScalarInst(mem_addr, length)\nendif",
  SMEMOp.S_PREFETCH_INST_PC_REL: "Ê   mem_addr = (64'U(PC[63 : 0].i64 + 8LL + 64'I(IOFFSET.i24)) & 0xffffffffffffff80ULL);\nÊ   // Force 128B alignment\nÊ   length = S1.u32;\nÊ   // SGPR or M0\nÊ   length += SDATA.u32;\nÊ   // SDATA is an immediate\nÊ   length = (length & 31U);\nÊ   // Length restricted to 0..31\nÊ   length = (length + 1U) * 128U;\nÊ   // Prefetch 1-32 cachelines, units of 128B\nÊ   PrefetchScalarInst(mem_addr, length)\nendif\nif MODE.SCALAR_PREFETCH_EN.u1 then",
  SMEMOp.S_PREFETCH_DATA: "if MODE.SCALAR_PREFETCH_EN.u1 then\nÊ   mem_addr = (64'U(S0[63 : 0].i64 + 64'I(IOFFSET.i24)) & 0xffffffffffffff80ULL);\nÊ   // Force 128B alignment\nÊ   length = S2.u32;\nÊ   // SGPR or M0\nÊ   length += SDATA.u32;\nÊ   // SDATA is an immediate\nÊ   length = (length & 31U);\nÊ   // Length restricted to 0..31\nÊ   length = (length + 1U) * 128U;\nÊ   // Prefetch 1-32 cachelines, units of 128B\nÊ   PrefetchScalarData(mem_addr, length)\nendif",
  SMEMOp.S_BUFFER_PREFETCH_DATA: "Ê   // SDATA is an immediate\nÊ   length = (length & 31U);\nÊ   // Length restricted to 0..31\nÊ   length = (length + 1U) * 128U;\nÊ   // Prefetch 1-32 cachelines, units of 128B\nÊ   PrefetchScalarData(mem_addr, length)\nendif\nif MODE.SCALAR_PREFETCH_EN.u1 then\nÊ   mem_addr = (64'U(S0[47 : 0].i64 + 64'I(IOFFSET.i24)) & 0xffffffffffffff80ULL);\nÊ   // Force 128B alignment\nÊ   length = S2.u32;\nÊ   // SGPR or M0\nÊ   length += SDATA.u32;",
  SMEMOp.S_PREFETCH_DATA_PC_REL: "if MODE.SCALAR_PREFETCH_EN.u1 then\nÊ   mem_addr = (64'U(PC[63 : 0].i64 + 8LL + 64'I(IOFFSET.i24)) & 0xffffffffffffff80ULL);\nÊ   // Force 128B alignment\nÊ   length = S1.u32;\nÊ   // SGPR or M0\nÊ   length += SDATA.u32;\nÊ   // SDATA is an immediate\nÊ   length = (length & 31U);\nÊ   // Length restricted to 0..31\nÊ   length = (length + 1U) * 128U;\nÊ   // Prefetch 1-32 cachelines, units of 128B\nÊ   PrefetchScalarData(mem_addr, length)\nendif",
}
...
SOP1Op_PCODE = {
  ...
  SOP1Op.S_BARRIER_SIGNAL: "if !InWorkgroup() then\nÊ   // Must be in a workgroup to signal a barrier.\nÊ   s_nop(16'0U)\nelsif ((barrierNumber == -2) && !WAVE_STATUS.PRIV) then\nÊ   // Barrier #-2 is privileged (for traps only).\nÊ   s_nop(16'0U)\nelsif barrierNumber == 0 then\nÊ   // Barrier #0 is a NOP.\nÊ   s_nop(16'0U)\nelse\nÊ   BARRIER_STATE[barrierNumber & 63].signalCnt += 7'1U\nendif;\n// Check for barrier completion.\nCheckBarrierComplete(barrierNumber)\n;\n// M0 cannot reference the negative barrier numbers.\nbarrierNumber = IsM0(SRC0.u32) ? 32'I(M0[4 : 0].u32) : SRC0.i32;",
  SOP1Op.S_BARRIER_SIGNAL_ISFIRST: ";\n// M0 cannot reference the negative barrier numbers.\nbarrierNumber = IsM0(SRC0.u32) ? 32'I(M0[4 : 0].u32) : SRC0.i32;\nif !InWorkgroup() then\nÊ   // Must be in a workgroup to signal a barrier.\nÊ   SCC = 1'0U\nelsif ((barrierNumber == -2) && !WAVE_STATUS.PRIV) then\nÊ   // Barrier #-2 is privileged (for traps only).\nÊ   SCC = 1'0U\nelsif barrierNumber == 0 then\nÊ   // Barrier #0 is a NOP.\nÊ   SCC = 1'0U\nelse\nÊ   // Set SCC if this is the first signaling event for this barrier.\nÊ   SCC = BARRIER_STATE[barrierNumber & 63].signalCnt.u32 == 0U;\nÊ   BARRIER_STATE[barrierNumber & 63].signalCnt += 7'1U\nendif;\nCheckBarrierComplete(barrierNumber)",
  SOP1Op.S_GET_BARRIER_STATE: "63].memberCnt.u7, 3'0, BARRIER_STATE[barrierNumber & 63].valid.u1 })\nbarrierNumber = IsM0(SRC0.u32) ? 32'I(M0[4 : 0].u32) : SRC0.i32;\nD0.u32 = 32'U({ 9'0, BARRIER_STATE[barrierNumber & 63].signalCnt.u7, 5'0, BARRIER_STATE[barrierNumber &",
...
SOPKOp_PCODE = {
  ...
  SOPKOp.S_CMOVK_I32: "Ê   D0.i32 = 32'I(signext(S0.i16))\nendif\nif SCC then",
...
SOPPOp_PCODE = {
  ...
  SOPPOp.S_SLEEP: 'Ê   s_sleep { duration: 0 }       // Wait for 0 clocks.\nÊ   s_sleep { duration: 1 }       // Wait for 1-64 clocks.\nÊ   s_sleep { duration: 2 }       // Wait for 65-128 clocks.\nÊ   s_sleep { sleep_forever: 1 }        // Wait until an event occurs.',
  ...
  SOPPOp.S_CBRANCH_VCCNZ: "Ê   PC = PC + signext(SIMM16.i16 * 16'4) + 4LL\nelse\nÊ   PC = PC + 4LL\nendif\nif VCCZ.u1 == 1'0U then",
...
VBUFFEROp_PCODE = {
  ...
  VBUFFEROp.BUFFER_STORE_D16_HI_B8: 'MEM[addr].b8 = VDATA[23 : 16]\naddr = CalcBufferAddr(vgpr_a.b64, sgpr_o.b64);',
  ...
  VBUFFEROp.BUFFER_ATOMIC_SUB_CLAMP_U32: "old_value = MEM[ADDR].u32;\nif old_value < DATA.u32 then\nÊ   new_value = 0U\nelse\nÊ   new_value = old_value - DATA.u32\nendif;\nMEM[ADDR].u32 = new_value;\nRETURN_DATA.u32 = old_value\ndeclare new_value : 32'U;",
  ...
  VBUFFEROp.BUFFER_ATOMIC_MAX_I32: 'RETURN_DATA.i32 = tmp\naddr = CalcBufferAddr(vgpr_a.b64, sgpr_o.b64);\ntmp = MEM[addr].i32;\nsrc = DATA.i32;\nMEM[addr].i32 = src >= tmp ? src : tmp;',
  ...
  VBUFFEROp.BUFFER_ATOMIC_MIN_I64: 'MEM[addr].i64 = src < tmp ? src : tmp;\nRETURN_DATA.i64 = tmp\naddr = CalcBufferAddr(vgpr_a.b64, sgpr_o.b64);\ntmp = MEM[addr].i64;\nsrc = DATA.i64;',
  ...
  VBUFFEROp.BUFFER_ATOMIC_MIN_NUM_F32: "RETURN_DATA.f32 = tmp\ntmp = MEM[ADDR].f32;\nsrc = DATA.f32;\nif (isNAN(64'F(src.f32)) && isNAN(64'F(tmp.f32))) then\nÊ   MEM[ADDR].f32 = 32'F(cvtToQuietNAN(64'F(src.f32)))\nelsif isNAN(64'F(src.f32)) then\nÊ   MEM[ADDR].f32 = tmp.f32\nelsif isNAN(64'F(tmp.f32)) then\nÊ   MEM[ADDR].f32 = src.f32\nelsif ((src.f32 < tmp.f32) || ((abs(src.f32) == 0.0F) && (abs(tmp.f32) == 0.0F) && sign(src.f32) &&\n!sign(tmp.f32))) then\nÊ   // NOTE: -0<+0 is TRUE in this comparison\nÊ   MEM[ADDR].f32 = src.f32\nelse\nÊ   MEM[ADDR].f32 = tmp.f32\nendif;",
  ...
  VBUFFEROp.TBUFFER_STORE_FORMAT_XY: 'MEM[addr + ChannelOffsetY()] = ConvertToFormat(VDATA[63 : 32].b32)\naddr = CalcBufferAddr(vgpr_a.b64, sgpr_o.b64);\nMEM[addr + ChannelOffsetX()] = ConvertToFormat(VDATA[31 : 0].b32);\n// Mem access size depends on format',
...
VFLATOp_PCODE = {
  ...
  VFLATOp.FLAT_STORE_D16_HI_B8: 'MEM[addr].b8 = VDATA[23 : 16]\naddr = CalcFlatAddr(v_addr.b64, flat_scratch.b64);',
  ...
  VFLATOp.FLAT_ATOMIC_ADD_U32: 'RETURN_DATA.u32 = tmp\naddr = CalcFlatAddr(v_addr.b64, flat_scratch.b64);\ntmp = MEM[addr].u32;\nMEM[addr].u32 += DATA.u32;',
  ...
  VFLATOp.FLAT_ATOMIC_ADD_U64: 'MEM[addr].u64 += DATA.u64;\nRETURN_DATA.u64 = tmp\naddr = CalcFlatAddr(v_addr.b64, flat_scratch.b64);\ntmp = MEM[addr].u64;',
  ...
  VFLATOp.FLAT_ATOMIC_MAX_NUM_F32: "Ê   MEM[ADDR].f32 = tmp.f32\nendif;\nRETURN_DATA.f32 = tmp\ntmp = MEM[ADDR].f32;\nsrc = DATA.f32;\nif (isNAN(64'F(src.f32)) && isNAN(64'F(tmp.f32))) then\nÊ   MEM[ADDR].f32 = 32'F(cvtToQuietNAN(64'F(src.f32)))\nelsif isNAN(64'F(src.f32)) then\nÊ   MEM[ADDR].f32 = tmp.f32\nelsif isNAN(64'F(tmp.f32)) then\nÊ   MEM[ADDR].f32 = src.f32\nelsif ((src.f32 > tmp.f32) || ((abs(src.f32) == 0.0F) && (abs(tmp.f32) == 0.0F) && !sign(src.f32) &&\nsign(tmp.f32))) then\nÊ   // NOTE: +0>-0 is TRUE in this comparison\nÊ   MEM[ADDR].f32 = src.f32\nelse",
...
VGLOBALOp_PCODE = {
  ...
  VGLOBALOp.GLOBAL_LOAD_B96: 'VDATA[95 : 64] = MEM[addr + 8U].b32\naddr = CalcGlobalAddr(v_addr.b64, s_saddr.b64);\nVDATA[31 : 0] = MEM[addr].b32;\nVDATA[63 : 32] = MEM[addr + 4U].b32;',
  ...
  VGLOBALOp.GLOBAL_LOAD_D16_I8: "// VDATA[31:16] is preserved.\naddr = CalcGlobalAddr(v_addr.b64, s_saddr.b64);\nVDATA[15 : 0].i16 = 16'I(signext(MEM[addr].i8));",
  ...
  VGLOBALOp.GLOBAL_ATOMIC_SUB_U32: 'RETURN_DATA.u32 = tmp\naddr = CalcGlobalAddr(v_addr.b64, s_saddr.b64);\ntmp = MEM[addr].u32;\nMEM[addr].u32 -= DATA.u32;',
  ...
  VGLOBALOp.GLOBAL_ATOMIC_OR_B32: 'tmp = MEM[addr].b32;\nMEM[addr].b32 = (tmp | DATA.b32);\nRETURN_DATA.b32 = tmp\naddr = CalcGlobalAddr(v_addr.b64, s_saddr.b64);',
  ...
  VGLOBALOp.GLOBAL_LOAD_BLOCK: 'Ê   // Algorithm can skip over "holes" both in the memory block and VGPR block.\nÊ   if M0[i].u1 then\nÊ       VGPR[laneId][VDST.u32 + i.u32] = MEM[VGPR[laneId][ADDR].i32 + i * 4].b32\nÊ   endif\nendfor\nfor i in 0 : 31 do',
  VGLOBALOp.GLOBAL_STORE_BLOCK: 'for i in 0 : 31 do\nÊ   // Algorithm can skip over "holes" both in the memory block and VGPR block.\nÊ   if M0[i].u1 then\nÊ       MEM[VGPR[laneId][ADDR].i32 + i * 4] = VGPR[laneId][VDATA.u32 + i.u32].b32\nÊ   endif\nendfor',
...
VIMAGEOp_PCODE = {
  ...
  VIMAGEOp.IMAGE_ATOMIC_AND: 'RETURN_DATA.b32 = tmp\naddr = CalcImageAddr(v_addr.b128);\ntmp = MEM[addr].b32;\nMEM[addr].b32 = (tmp & DATA.b32);',
...
VOP1Op_PCODE = {
  ...
  VOP1Op.V_RCP_IFLAG_F32: '// Can only raise integer DIV_BY_ZERO exception\nD0.f32 = 1.0F / S0.f32;',
  ...
  VOP1Op.V_CVT_F32_FP8: "if OPSEL[1 : 0].u2 == 2'0U then\nÊ   D0.f32 = fp8_to_f32(VGPR[laneId][SRC0.u32][7 : 0].fp8)\nelsif OPSEL[1 : 0].u2 == 2'2U then\nÊ   // Byte select bits are reversed\nÊ   D0.f32 = fp8_to_f32(VGPR[laneId][SRC0.u32][15 : 8].fp8)\nelsif OPSEL[1 : 0].u2 == 2'1U then\nÊ   D0.f32 = fp8_to_f32(VGPR[laneId][SRC0.u32][23 : 16].fp8)\nelse\nÊ   D0.f32 = fp8_to_f32(VGPR[laneId][SRC0.u32][31 : 24].fp8)\nendif",
  VOP1Op.V_CVT_F32_BF8: "if OPSEL[1 : 0].u2 == 2'0U then\nÊ   D0.f32 = bf8_to_f32(VGPR[laneId][SRC0.u32][7 : 0].bf8)\nelsif OPSEL[1 : 0].u2 == 2'2U then\nÊ   // Byte select bits are reversed\nÊ   D0.f32 = bf8_to_f32(VGPR[laneId][SRC0.u32][15 : 8].bf8)\nelsif OPSEL[1 : 0].u2 == 2'1U then\nÊ   D0.f32 = bf8_to_f32(VGPR[laneId][SRC0.u32][23 : 16].bf8)\nelse\nÊ   D0.f32 = bf8_to_f32(VGPR[laneId][SRC0.u32][31 : 24].bf8)\nendif",
...
VOP2Op_PCODE = {
  ...
  VOP2Op.V_MIN_NUM_F32: "if (isSignalNAN(64'F(S0.f32)) || isSignalNAN(64'F(S1.f32))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(64'F(S0.f32)) && isNAN(64'F(S1.f32))) then\nÊ   D0.f32 = 32'F(cvtToQuietNAN(64'F(S0.f32)))\nelsif isNAN(64'F(S0.f32)) then\nÊ   D0.f32 = S1.f32\nelsif isNAN(64'F(S1.f32)) then\nÊ   D0.f32 = S0.f32\nelsif ((S0.f32 < S1.f32) || ((abs(S0.f32) == 0.0F) && (abs(S1.f32) == 0.0F) && sign(S0.f32) &&\n!sign(S1.f32))) then\nÊ   // NOTE: -0<+0 is TRUE in this comparison\nÊ   D0.f32 = S0.f32\nelse\nÊ   D0.f32 = S1.f32\nendif",
  VOP2Op.V_MAX_NUM_F32: "if (isSignalNAN(64'F(S0.f32)) || isSignalNAN(64'F(S1.f32))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(64'F(S0.f32)) && isNAN(64'F(S1.f32))) then\nÊ   D0.f32 = 32'F(cvtToQuietNAN(64'F(S0.f32)))\nelsif isNAN(64'F(S0.f32)) then\nÊ   D0.f32 = S1.f32\nelsif isNAN(64'F(S1.f32)) then\nÊ   D0.f32 = S0.f32\nelsif ((S0.f32 > S1.f32) || ((abs(S0.f32) == 0.0F) && (abs(S1.f32) == 0.0F) && !sign(S0.f32) &&\nsign(S1.f32))) then\nÊ   // NOTE: +0>-0 is TRUE in this comparison\nÊ   D0.f32 = S0.f32\nelse\nÊ   D0.f32 = S1.f32\nendif",
  ...
  VOP2Op.V_LSHLREV_B64: 'D0.u64 = (S1.u64 << S0[5 : 0].u32)',
  ...
  VOP2Op.V_MIN_NUM_F16: "if (isSignalNAN(64'F(S0.f16)) || isSignalNAN(64'F(S1.f16))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(64'F(S0.f16)) && isNAN(64'F(S1.f16))) then\nÊ   D0.f16 = 16'F(cvtToQuietNAN(64'F(S0.f16)))\nelsif isNAN(64'F(S0.f16)) then\nÊ   D0.f16 = S1.f16\nelsif isNAN(64'F(S1.f16)) then\nÊ   D0.f16 = S0.f16\nelsif ((S0.f16 < S1.f16) || ((abs(S0.f16) == 16'0.0) && (abs(S1.f16) == 16'0.0) && sign(S0.f16) &&\n!sign(S1.f16))) then\nÊ   // NOTE: -0<+0 is TRUE in this comparison\nÊ   D0.f16 = S0.f16\nelse\nÊ   D0.f16 = S1.f16\nendif",
  VOP2Op.V_MAX_NUM_F16: "Ê   // NOTE: +0>-0 is TRUE in this comparison\nÊ   D0.f16 = S0.f16\nelse\nÊ   D0.f16 = S1.f16\nendif\nif (isSignalNAN(64'F(S0.f16)) || isSignalNAN(64'F(S1.f16))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(64'F(S0.f16)) && isNAN(64'F(S1.f16))) then\nÊ   D0.f16 = 16'F(cvtToQuietNAN(64'F(S0.f16)))\nelsif isNAN(64'F(S0.f16)) then\nÊ   D0.f16 = S1.f16\nelsif isNAN(64'F(S1.f16)) then\nÊ   D0.f16 = S0.f16\nelsif ((S0.f16 > S1.f16) || ((abs(S0.f16) == 16'0.0) && (abs(S1.f16) == 16'0.0) && !sign(S0.f16) &&\nsign(S1.f16))) then",
...
VOP3Op_PCODE = {
  ...
  VOP3Op.V_CMP_NGT_F16: '// With NAN inputs this is not the same operation as <=\n// D0 = VCC in VOPC encoding.\nD0.u64[laneId] = !(S0.f16 > S1.f16);',
  ...
  VOP3Op.V_CMPX_CLASS_F64: "if isSignalNAN(S0.f64) then\nÊ   result = S1.u32[0]\nelsif isQuietNAN(S0.f64) then\nÊ   result = S1.u32[1]\nelsif exponent(S0.f64) == 2047 then\nÊ   // +-INF\nÊ   result = S1.u32[sign(S0.f64) ? 2 : 9]\nelsif exponent(S0.f64) > 0 then\nÊ   // +-normal value\nÊ   result = S1.u32[sign(S0.f64) ? 3 : 8]\nelsif abs(S0.f64) > 0.0 then\nÊ   // +-denormal value\nÊ   result = S1.u32[sign(S0.f64) ? 4 : 7]\nelse\nÊ   // +-0.0\nÊ   result = S1.u32[sign(S0.f64) ? 5 : 6]\nendif;\nEXEC.u64[laneId] = result\ndeclare result : 1'U;",
  ...
  VOP3Op.V_MIN_NUM_F64: 'Ê   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(S0.f64) && isNAN(S1.f64)) then\nÊ   D0.f64 = cvtToQuietNAN(S0.f64)\nelsif isNAN(S0.f64) then\nÊ   D0.f64 = S1.f64\nelsif isNAN(S1.f64) then\nÊ   D0.f64 = S0.f64\nelsif ((S0.f64 < S1.f64) || ((abs(S0.f64) == 0.0) && (abs(S1.f64) == 0.0) && sign(S0.f64) &&\n!sign(S1.f64))) then\nÊ   // NOTE: -0<+0 is TRUE in this comparison\nÊ   D0.f64 = S0.f64\nelse\nÊ   D0.f64 = S1.f64\nendif\nif (isSignalNAN(S0.f64) || isSignalNAN(S1.f64)) then',
  VOP3Op.V_MAX_NUM_F64: 'if (isSignalNAN(S0.f64) || isSignalNAN(S1.f64)) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(S0.f64) && isNAN(S1.f64)) then\nÊ   D0.f64 = cvtToQuietNAN(S0.f64)\nelsif isNAN(S0.f64) then\nÊ   D0.f64 = S1.f64\nelsif isNAN(S1.f64) then\nÊ   D0.f64 = S0.f64\nelsif ((S0.f64 > S1.f64) || ((abs(S0.f64) == 0.0) && (abs(S1.f64) == 0.0) && !sign(S0.f64) &&\nsign(S1.f64))) then\nÊ   // NOTE: +0>-0 is TRUE in this comparison\nÊ   D0.f64 = S0.f64\nelse\nÊ   D0.f64 = S1.f64\nendif',
  ...
  VOP3Op.V_MIN_NUM_F32: "if (isSignalNAN(64'F(S0.f32)) || isSignalNAN(64'F(S1.f32))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(64'F(S0.f32)) && isNAN(64'F(S1.f32))) then\nÊ   D0.f32 = 32'F(cvtToQuietNAN(64'F(S0.f32)))\nelsif isNAN(64'F(S0.f32)) then\nÊ   D0.f32 = S1.f32\nelsif isNAN(64'F(S1.f32)) then\nÊ   D0.f32 = S0.f32\nelsif ((S0.f32 < S1.f32) || ((abs(S0.f32) == 0.0F) && (abs(S1.f32) == 0.0F) && sign(S0.f32) &&\n!sign(S1.f32))) then\nÊ   // NOTE: -0<+0 is TRUE in this comparison\nÊ   D0.f32 = S0.f32\nelse\nÊ   D0.f32 = S1.f32\nendif",
  VOP3Op.V_MAX_NUM_F32: "if (isSignalNAN(64'F(S0.f32)) || isSignalNAN(64'F(S1.f32))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(64'F(S0.f32)) && isNAN(64'F(S1.f32))) then\nÊ   D0.f32 = 32'F(cvtToQuietNAN(64'F(S0.f32)))\nelsif isNAN(64'F(S0.f32)) then\nÊ   D0.f32 = S1.f32\nelsif isNAN(64'F(S1.f32)) then\nÊ   D0.f32 = S0.f32\nelsif ((S0.f32 > S1.f32) || ((abs(S0.f32) == 0.0F) && (abs(S1.f32) == 0.0F) && !sign(S0.f32) &&\nsign(S1.f32))) then\nÊ   // NOTE: +0>-0 is TRUE in this comparison\nÊ   D0.f32 = S0.f32\nelse\nÊ   D0.f32 = S1.f32\nendif",
  ...
  VOP3Op.V_MIN_NUM_F16: "Ê   D0.f16 = 16'F(cvtToQuietNAN(64'F(S0.f16)))\nelsif isNAN(64'F(S0.f16)) then\nÊ   D0.f16 = S1.f16\nelsif isNAN(64'F(S1.f16)) then\nÊ   D0.f16 = S0.f16\nelsif ((S0.f16 < S1.f16) || ((abs(S0.f16) == 16'0.0) && (abs(S1.f16) == 16'0.0) && sign(S0.f16) &&\n!sign(S1.f16))) then\nÊ   // NOTE: -0<+0 is TRUE in this comparison\nÊ   D0.f16 = S0.f16\nelse\nÊ   D0.f16 = S1.f16\nendif\nif (isSignalNAN(64'F(S0.f16)) || isSignalNAN(64'F(S1.f16))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(64'F(S0.f16)) && isNAN(64'F(S1.f16))) then",
  VOP3Op.V_MAX_NUM_F16: "if (isSignalNAN(64'F(S0.f16)) || isSignalNAN(64'F(S1.f16))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(64'F(S0.f16)) && isNAN(64'F(S1.f16))) then\nÊ   D0.f16 = 16'F(cvtToQuietNAN(64'F(S0.f16)))\nelsif isNAN(64'F(S0.f16)) then\nÊ   D0.f16 = S1.f16\nelsif isNAN(64'F(S1.f16)) then\nÊ   D0.f16 = S0.f16\nelsif ((S0.f16 > S1.f16) || ((abs(S0.f16) == 16'0.0) && (abs(S1.f16) == 16'0.0) && !sign(S0.f16) &&\nsign(S1.f16))) then\nÊ   // NOTE: +0>-0 is TRUE in this comparison\nÊ   D0.f16 = S0.f16\nelse\nÊ   D0.f16 = S1.f16\nendif",
  ...
  VOP3Op.V_MOV_B32: 'Ê   v_mov_b32 v0, v1    // Move into v0 from v1\nÊ   v_mov_b32 v0, -v1   // Set v0 to the negation of v1\nÊ   v_mov_b32 v0, abs(v1)   // Set v0 to the absolute value of v1\nD0.b32 = S0.b32',
  ...
  VOP3Op.V_FLOOR_F64: 'if ((S0.f64 < 0.0) && (S0.f64 != D0.f64)) then\nÊ   D0.f64 += -1.0\nendif\nD0.f64 = trunc(S0.f64);',
  ...
  VOP3Op.V_RCP_F32: 'V_RCP_F32(0xff800000) => 0x80000000     // rcp(-INF) = -0\nV_RCP_F32(0xc0000000) => 0xbf000000     // rcp(-2.0) = -0.5\nV_RCP_F32(0x80000000) => 0xff800000     // rcp(-0.0) = -INF\nV_RCP_F32(0x00000000) => 0x7f800000     // rcp(+0.0) = +INF\nV_RCP_F32(0x7f800000) => 0x00000000     // rcp(+INF) = +0\nD0.f32 = 1.0F / S0.f32',
  VOP3Op.V_RCP_IFLAG_F32: 'D0.f32 = 1.0F / S0.f32;\n// Can only raise integer DIV_BY_ZERO exception',
  VOP3Op.V_RSQ_F32: 'V_RSQ_F32(0xff800000) => 0xffc00000     // rsq(-INF) = NAN\nV_RSQ_F32(0x80000000) => 0xff800000     // rsq(-0.0) = -INF\nV_RSQ_F32(0x00000000) => 0x7f800000     // rsq(+0.0) = +INF\nV_RSQ_F32(0x40800000) => 0x3f000000     // rsq(+4.0) = +0.5\nV_RSQ_F32(0x7f800000) => 0x00000000     // rsq(+INF) = +0\nD0.f32 = 1.0F / sqrt(S0.f32)',
  VOP3Op.V_RCP_F64: 'D0.f64 = 1.0 / S0.f64',
  VOP3Op.V_RSQ_F64: 'D0.f64 = 1.0 / sqrt(S0.f64)',
  VOP3Op.V_SQRT_F32: 'V_SQRT_F32(0xff800000) => 0xffc00000     // sqrt(-INF) = NAN\nV_SQRT_F32(0x80000000) => 0x80000000     // sqrt(-0.0) = -0\nV_SQRT_F32(0x00000000) => 0x00000000     // sqrt(+0.0) = +0\nV_SQRT_F32(0x40800000) => 0x40000000     // sqrt(+4.0) = +2.0\nV_SQRT_F32(0x7f800000) => 0x7f800000     // sqrt(+INF) = +INF\nD0.f32 = sqrt(S0.f32)',
  ...
  VOP3Op.V_CLZ_I32_U32: "for i in 0 : 31 do\nÊ   // Search from MSB\nÊ   if S0.u32[31 - i] == 1'1U then\nÊ       D0.i32 = i;\nÊ       break\nÊ   endif\nendfor\nD0.i32 = -1;\n// Set if no ones are found",
  ...
  VOP3Op.V_SQRT_F16: 'V_SQRT_F16(0xfc00) => 0xfe00     // sqrt(-INF) = NAN\nV_SQRT_F16(0x8000) => 0x8000     // sqrt(-0.0) = -0\nV_SQRT_F16(0x0000) => 0x0000     // sqrt(+0.0) = +0\nV_SQRT_F16(0x4400) => 0x4000     // sqrt(+4.0) = +2.0\nV_SQRT_F16(0x7c00) => 0x7c00     // sqrt(+INF) = +INF\nD0.f16 = sqrt(S0.f16)',
  ...
  VOP3Op.V_CVT_F32_FP8: "if OPSEL[1 : 0].u2 == 2'0U then\nÊ   D0.f32 = fp8_to_f32(VGPR[laneId][SRC0.u32][7 : 0].fp8)\nelsif OPSEL[1 : 0].u2 == 2'2U then\nÊ   // Byte select bits are reversed\nÊ   D0.f32 = fp8_to_f32(VGPR[laneId][SRC0.u32][15 : 8].fp8)\nelsif OPSEL[1 : 0].u2 == 2'1U then\nÊ   D0.f32 = fp8_to_f32(VGPR[laneId][SRC0.u32][23 : 16].fp8)\nelse\nÊ   D0.f32 = fp8_to_f32(VGPR[laneId][SRC0.u32][31 : 24].fp8)\nendif",
  VOP3Op.V_CVT_F32_BF8: "if OPSEL[1 : 0].u2 == 2'0U then\nÊ   D0.f32 = bf8_to_f32(VGPR[laneId][SRC0.u32][7 : 0].bf8)\nelsif OPSEL[1 : 0].u2 == 2'2U then\nÊ   // Byte select bits are reversed\nÊ   D0.f32 = bf8_to_f32(VGPR[laneId][SRC0.u32][15 : 8].bf8)\nelsif OPSEL[1 : 0].u2 == 2'1U then\nÊ   D0.f32 = bf8_to_f32(VGPR[laneId][SRC0.u32][23 : 16].bf8)\nelse\nÊ   D0.f32 = bf8_to_f32(VGPR[laneId][SRC0.u32][31 : 24].bf8)\nendif",
  VOP3Op.V_CVT_PK_F32_FP8: 'tmp = OPSEL[0].u1 ? VGPR[laneId][SRC0.u32][31 : 16] : VGPR[laneId][SRC0.u32][15 : 0];\nD0[31 : 0].f32 = fp8_to_f32(tmp[7 : 0].fp8);\nD0[63 : 32].f32 = fp8_to_f32(tmp[15 : 8].fp8)',
  VOP3Op.V_CVT_PK_F32_BF8: 'tmp = OPSEL[0].u1 ? VGPR[laneId][SRC0.u32][31 : 16] : VGPR[laneId][SRC0.u32][15 : 0];\nD0[31 : 0].f32 = bf8_to_f32(tmp[7 : 0].bf8);\nD0[63 : 32].f32 = bf8_to_f32(tmp[15 : 8].bf8)',
  ...
  VOP3Op.V_SAD_U16: 'tmp += ABSDIFF(S0[31 : 16].u16, S1[31 : 16].u16);\nD0.u32 = tmp\nABSDIFF = lambda(x, y) (\nÊ   x > y ? x - y : y - x);\n// UNSIGNED comparison\ntmp = S2.u32;\ntmp += ABSDIFF(S0[15 : 0].u16, S1[15 : 0].u16);',
  ...
  VOP3Op.V_S_RSQ_F32: 'V_RSQ_F32(0xff800000) => 0xffc00000     // rsq(-INF) = NAN\nV_RSQ_F32(0x80000000) => 0xff800000     // rsq(-0.0) = -INF\nV_RSQ_F32(0x00000000) => 0x7f800000     // rsq(+0.0) = +INF\nV_RSQ_F32(0x40800000) => 0x3f000000     // rsq(+4.0) = +0.5\nV_RSQ_F32(0x7f800000) => 0x00000000     // rsq(+INF) = +0\nD0.f32 = 1.0F / sqrt(S0.f32)',
  VOP3Op.V_S_RSQ_F16: "D0.f16 = 16'1.0 / sqrt(S0.f16);\nD0[31 : 16] = 16'0x0\nV_RSQ_F16(0xfc00) => 0xfe00     // rsq(-INF) = NAN\nV_RSQ_F16(0x8000) => 0xfc00     // rsq(-0.0) = -INF\nV_RSQ_F16(0x0000) => 0x7c00     // rsq(+0.0) = +INF\nV_RSQ_F16(0x4400) => 0x3800     // rsq(+4.0) = +0.5\nV_RSQ_F16(0x7c00) => 0x0000     // rsq(+INF) = +0",
  VOP3Op.V_S_SQRT_F32: 'V_SQRT_F32(0xff800000) => 0xffc00000     // sqrt(-INF) = NAN\nV_SQRT_F32(0x80000000) => 0x80000000     // sqrt(-0.0) = -0\nV_SQRT_F32(0x00000000) => 0x00000000     // sqrt(+0.0) = +0\nV_SQRT_F32(0x40800000) => 0x40000000     // sqrt(+4.0) = +2.0\nV_SQRT_F32(0x7f800000) => 0x7f800000     // sqrt(+INF) = +INF\nD0.f32 = sqrt(S0.f32)',
  ...
  VOP3Op.V_MINIMUM_F16: "Ê   D0.f16 = S1.f16\nendif\nif (isSignalNAN(64'F(S0.f16)) || isSignalNAN(64'F(S1.f16))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif isSignalNAN(64'F(S0.f16)) then\nÊ   D0.f16 = 16'F(cvtToQuietNAN(64'F(S0.f16)))\nelsif isSignalNAN(64'F(S1.f16)) then\nÊ   D0.f16 = 16'F(cvtToQuietNAN(64'F(S1.f16)))\nelsif isQuietNAN(64'F(S0.f16)) then\nÊ   D0.f16 = S0.f16\nelsif isQuietNAN(64'F(S1.f16)) then\nÊ   D0.f16 = S1.f16\nelsif ((S0.f16 < S1.f16) || ((abs(S0.f16) == 16'0.0) && (abs(S1.f16) == 16'0.0) && sign(S0.f16) &&\n!sign(S1.f16))) then\nÊ   // NOTE: -0<+0 is TRUE in this comparison\nÊ   D0.f16 = S0.f16\nelse",
  VOP3Op.V_MAXIMUM_F16: "if (isSignalNAN(64'F(S0.f16)) || isSignalNAN(64'F(S1.f16))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif isSignalNAN(64'F(S0.f16)) then\nÊ   D0.f16 = 16'F(cvtToQuietNAN(64'F(S0.f16)))\nelsif isSignalNAN(64'F(S1.f16)) then\nÊ   D0.f16 = 16'F(cvtToQuietNAN(64'F(S1.f16)))\nelsif isQuietNAN(64'F(S0.f16)) then\nÊ   D0.f16 = S0.f16\nelsif isQuietNAN(64'F(S1.f16)) then\nÊ   D0.f16 = S1.f16\nelsif ((S0.f16 > S1.f16) || ((abs(S0.f16) == 16'0.0) && (abs(S1.f16) == 16'0.0) && !sign(S0.f16) &&\nsign(S1.f16))) then\nÊ   // NOTE: +0>-0 is TRUE in this comparison\nÊ   D0.f16 = S0.f16\nelse\nÊ   D0.f16 = S1.f16\nendif",
  VOP3Op.V_CVT_PK_FP8_F32: 'prev_mode = ROUND_MODE;\nROUND_MODE = ROUND_NEAREST_EVEN;\nif OPSEL[3].u32 == 0U then\nÊ   VGPR[laneId][VDST.u32][15 : 0].b16 = { f32_to_fp8(S1.f32), f32_to_fp8(S0.f32) };\nÊ   // D0[31:16] are preserved\nelse\nÊ   VGPR[laneId][VDST.u32][31 : 16].b16 = { f32_to_fp8(S1.f32), f32_to_fp8(S0.f32) };\nÊ   // D0[15:0] are preserved\nendif;\nROUND_MODE = prev_mode',
  VOP3Op.V_CVT_PK_BF8_F32: 'prev_mode = ROUND_MODE;\nROUND_MODE = ROUND_NEAREST_EVEN;\nif OPSEL[3].u32 == 0U then\nÊ   VGPR[laneId][VDST.u32][15 : 0].b16 = { f32_to_bf8(S1.f32), f32_to_bf8(S0.f32) };\nÊ   // D0[31:16] are preserved\nelse\nÊ   VGPR[laneId][VDST.u32][31 : 16].b16 = { f32_to_bf8(S1.f32), f32_to_bf8(S0.f32) };\nÊ   // D0[15:0] are preserved\nendif;\nROUND_MODE = prev_mode',
  VOP3Op.V_CVT_SR_FP8_F32: "e = exponent(S0.f32);\nm = 23'U(32'U(23'B(mantissa(S0.f32))) + S1[31 : 12].u32);\ntmp = float32(s, e, m);\n// Add stochastic value to mantissa, wrap around on overflow\nif OPSEL[3 : 2].u2 == 2'0U then\nÊ   VGPR[laneId][VDST.u32][7 : 0].fp8 = f32_to_fp8(tmp.f32)\nelsif OPSEL[3 : 2].u2 == 2'1U then\nÊ   VGPR[laneId][VDST.u32][15 : 8].fp8 = f32_to_fp8(tmp.f32)\nelsif OPSEL[3 : 2].u2 == 2'2U then\nÊ   VGPR[laneId][VDST.u32][23 : 16].fp8 = f32_to_fp8(tmp.f32)\nelse\nÊ   VGPR[laneId][VDST.u32][31 : 24].fp8 = f32_to_fp8(tmp.f32)\nendif;\n// Unwritten bytes of D are preserved.\nROUND_MODE = prev_mode\nprev_mode = ROUND_MODE;\nROUND_MODE = ROUND_NEAREST_EVEN;\ns = sign(S0.f32);",
  VOP3Op.V_CVT_SR_BF8_F32: "prev_mode = ROUND_MODE;\nROUND_MODE = ROUND_NEAREST_EVEN;\ns = sign(S0.f32);\ne = exponent(S0.f32);\nm = 23'U(32'U(23'B(mantissa(S0.f32))) + S1[31 : 11].u32);\ntmp = float32(s, e, m);\n// Add stochastic value to mantissa, wrap around on overflow\nif OPSEL[3 : 2].u2 == 2'0U then\nÊ   VGPR[laneId][VDST.u32][7 : 0].bf8 = f32_to_bf8(tmp.f32)\nelsif OPSEL[3 : 2].u2 == 2'1U then\nÊ   VGPR[laneId][VDST.u32][15 : 8].bf8 = f32_to_bf8(tmp.f32)\nelsif OPSEL[3 : 2].u2 == 2'2U then\nÊ   VGPR[laneId][VDST.u32][23 : 16].bf8 = f32_to_bf8(tmp.f32)\nelse\nÊ   VGPR[laneId][VDST.u32][31 : 24].bf8 = f32_to_bf8(tmp.f32)\nendif;\n// Unwritten bytes of D are preserved.\nROUND_MODE = prev_mode",
}
...
VOPDOp_PCODE = {
  ...
  VOPDOp.V_DUAL_MAX_NUM_F32: "elsif ((S0.f32 > S1.f32) || ((abs(S0.f32) == 0.0F) && (abs(S1.f32) == 0.0F) && !sign(S0.f32) &&\nsign(S1.f32))) then\nÊ   // NOTE: +0>-0 is TRUE in this comparison\nÊ   D0.f32 = S0.f32\nelse\nÊ   D0.f32 = S1.f32\nendif\nif (isSignalNAN(64'F(S0.f32)) || isSignalNAN(64'F(S1.f32))) then\nÊ   TRAPSTS.INVALID = 1\nendif;\nif (isNAN(64'F(S0.f32)) && isNAN(64'F(S1.f32))) then\nÊ   D0.f32 = 32'F(cvtToQuietNAN(64'F(S0.f32)))\nelsif isNAN(64'F(S0.f32)) then\nÊ   D0.f32 = S1.f32\nelsif isNAN(64'F(S1.f32)) then\nÊ   D0.f32 = S0.f32",
...
VSCRATCHOp_PCODE = {
  ...
  VSCRATCHOp.SCRATCH_STORE_B128: 'MEM[addr + 4U].b32 = VDATA[63 : 32];\nMEM[addr + 8U].b32 = VDATA[95 : 64];\nMEM[addr + 12U].b32 = VDATA[127 : 96]\naddr = CalcScratchAddr(v_addr_off.b64, s_saddr_off.b64);\nMEM[addr].b32 = VDATA[31 : 0];',
  ...
  VSCRATCHOp.SCRATCH_LOAD_BLOCK: 'Ê   // Algorithm can skip over "holes" both in the memory block and VGPR block.\nÊ   if M0[i].u1 then\nÊ       VGPR[laneId][VDST.u32 + i.u32] = MEM[VGPR[laneId][ADDR].i32 + i * 4].b32\nÊ   endif\nendfor\nfor i in 0 : 31 do',
...
PSEUDOCODE_STRINGS = {
  ...
  SOPKOp: SOPKOp_PCODE,
  SOPPOp: SOPPOp_PCODE,
  VBUFFEROp: VBUFFEROp_PCODE,
  VFLATOp: VFLATOp_PCODE,
  VGLOBALOp: VGLOBALOp_PCODE,
  VIMAGEOp: VIMAGEOp_PCODE,
  ...
  VOPDOp: VOPDOp_PCODE,
  VSAMPLEOp: VSAMPLEOp_PCODE,
  VSCRATCHOp: VSCRATCHOp_PCODE,
}