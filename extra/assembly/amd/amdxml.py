...
def parse_xml(filename: str, arch: str):
  ...
  return encodings, enums

def write_enum(enums, path):
  lines = ["# autogenerated from AMD ISA XML - do not edit", "from enum import Enum", ""]
  # Add BufFmt first (hardcoded buffer formats)
...
if __name__ == "__main__":
  ...
  for filename, arch in ARCH_MAP.items():
    ...
    encodings, enums = parse_xml(filename, arch)
    # Apply fixes for instructions missing from XML
    ...
    print(f"  {len(encodings)} encodings, {sum(len(v) for v in enums.values())} instructions")
