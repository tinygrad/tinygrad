...
def parse_xml(filename: str, arch: str):
  root = ET.fromstring(nn.state.zip_extract(Tensor.from_url(XML_URL))[filename].to("CPU").data())
...
def write_info(types, enums, arch, path):
  # Build reverse map: instruction name -> (enum_class, member_name)
  name_to_enum = {}
  for fmt, ops in enums.items():
    suffix = "_E32" if fmt in ("VOP1", "VOP2", "VOPC") else "_E64" if fmt == "VOP3" else ""
    for op, name in ops.items():
      msuf = suffix if fmt != "VOP3" or op < 512 else ""
      name_to_enum[name] = (f"{fmt}Op", f"{name}{msuf}")
  ...
  for name, fields in sorted(types.items()):
    if name not in name_to_enum: continue
    enum_cls, enum_mem = name_to_enum[name]
    def fmt_val(v):
      fmt, size, otype = v
      fmt_s = f"Fmt.{fmt}" if fmt else "None"
      otype_s = f"OpType.{otype}" if otype else "None"
      return f"({fmt_s}, {size}, {otype_s})"
    fstr = ", ".join(f'"{k}": {fmt_val(v)}' for k, v in sorted(fields.items()))
    lines.append(f'  {enum_cls}.{enum_mem}: {{{fstr}}},')
  lines.append("}")
  with open(path, "w") as f: f.write("\n".join(lines))
...
if __name__ == "__main__":
  ...
  for arch, (encodings, enums, types) in arch_data.items():
    ...
    write_info(types, enums, arch, base / "info.py")
