...
pcode_pm = PatternMatcher([
  ...
  # Special register aliases (expanded to operations on base registers)
  (UPat(Ops.DEFINE_VAR, arg=('EXEC_LO', -float('inf'), float('inf'))),
   lambda: UOp(Ops.AND, dtypes.uint64, (UOp(Ops.DEFINE_VAR, dtypes.uint64, arg=('EXEC', 0, 0xffffffffffffffff)), UOp.const(dtypes.uint64, 0xffffffff))).cast(dtypes.uint32)),
  (UPat(Ops.DEFINE_VAR, arg=('VCCZ', -float('inf'), float('inf'))),
   lambda: UOp(Ops.DEFINE_VAR, dtypes.uint64, arg=('VCC', 0, 0xffffffffffffffff)).eq(0).cast(dtypes.uint32)),
  (UPat(Ops.DEFINE_VAR, arg=('EXECZ', -float('inf'), float('inf'))),
   lambda: UOp(Ops.DEFINE_VAR, dtypes.uint64, arg=('EXEC', 0, 0xffffffffffffffff)).eq(0).cast(dtypes.uint32)),
  (UPat(Ops.DEFINE_VAR, arg=('ThreadMask', -float('inf'), float('inf'))),
   lambda: UOp(Ops.DEFINE_VAR, dtypes.uint64, arg=('EXEC', 0, 0xffffffffffffffff)).cast(dtypes.uint32)),
  (UPat(Ops.DEFINE_VAR, arg=('DST', -float('inf'), float('inf'))),
   lambda: UOp(Ops.DEFINE_VAR, dtypes.uint32, arg=('VDST', 0, 0xffffffff))),
  (UPat(Ops.DEFINE_VAR, arg=('laneID', -float('inf'), float('inf'))),
   lambda: UOp(Ops.DEFINE_VAR, dtypes.uint32, arg=('laneId', 0, 0xffffffff))),
