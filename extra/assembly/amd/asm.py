...
def _disasm_vop1(inst: VOP1) -> str:
  ...
  dst = _vreg(inst.vdst, inst.dst_regs()) if inst.dst_regs() > 1 else _fmt_v16(inst.vdst, 0, 128) if inst.is_dst_16 else f"v{inst.vdst}"
  src = _fmt_src(inst.src0, inst.src_regs(0)) if inst.src_regs(0) > 1 else _src16(inst, inst.src0) if inst.is_src_16(0) else inst.lit(inst.src0)
...
def _disasm_vop3(inst: VOP3) -> str:
  ...
  any_hi = inst.opsel != 0
  s0 = _vop3_src(inst, inst.src0, inst.neg&1, inst.abs&1, inst.opsel&1, inst.src_regs(0), inst.is_src_16(0), any_hi)
  s1 = _vop3_src(inst, inst.src1, inst.neg&2, inst.abs&2, inst.opsel&2, inst.src_regs(1), inst.is_src_16(1), any_hi)
  s2 = _vop3_src(inst, inst.src2, inst.neg&4, inst.abs&4, inst.opsel&4, inst.src_regs(2), inst.is_src_16(2), any_hi)

  if op == VOP3Op.V_READLANE_B32: dst = _fmt_sdst(inst.vdst, 1)
  elif inst.dst_regs() > 1: dst = _vreg(inst.vdst, inst.dst_regs())
  elif inst.is_dst_16: dst = f"v{inst.vdst}.h" if (inst.opsel & 8) else f"v{inst.vdst}.l" if any_hi else f"v{inst.vdst}"
  ...
  need_opsel = nonvgpr_opsel or (inst.opsel and not inst.is_src_16(0))

  if inst.op < 256:  # VOPC
    return f"{name}_e64 {s0}, {s1}" if name.startswith('v_cmpx') else f"{name}_e64 {_fmt_sdst(inst.vdst, 1)}, {s0}, {s1}"
  if inst.op < 384:  # VOP2
    os = _opsel_str(inst.opsel, 3, need_opsel, inst.is_dst_16) if 'cndmask' in name else _opsel_str(inst.opsel, 2, need_opsel, inst.is_dst_16)
    return f"{name}_e64 {dst}, {s0}, {s1}, {s2}{os}{cl}{om}" if 'cndmask' in name else f"{name}_e64 {dst}, {s0}, {s1}{os}{cl}{om}"
  if inst.op < 512:  # VOP1
    return f"{name}_e64" if op in (VOP3Op.V_NOP, VOP3Op.V_PIPEFLUSH) else f"{name}_e64 {dst}, {s0}{_opsel_str(inst.opsel, 1, need_opsel, inst.is_dst_16)}{cl}{om}"
  # Native VOP3
  ...
  os = _opsel_str(inst.opsel, 3 if is3 else 2, need_opsel, inst.is_dst_16)
...
def _disasm_sop2(inst: SOP2) -> str:
  name = inst.op_name.lower()
  return f"{name} {_fmt_sdst(inst.sdst, inst.dst_regs())}, {inst.lit(inst.ssrc0) if inst.ssrc0 == 255 else _fmt_src(inst.ssrc0, inst.src_regs(0))}, {inst.lit(inst.ssrc1) if inst.ssrc1 == 255 else _fmt_src(inst.ssrc1, inst.src_regs(1))}"

def _disasm_sopc(inst: SOPC) -> str:
  name = inst.op_name.lower()
  return f"{name} {_fmt_src(inst.ssrc0, inst.src_regs(0))}, {_fmt_src(inst.ssrc1, inst.src_regs(1))}"
