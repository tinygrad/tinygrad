...
FIELD_ORDER = {
  ...
  'VOP3A': ['op', 'vdst', 'src0', 'src1', 'src2', 'omod', 'neg', 'abs', 'clmp', 'opsel'],  # CDNA VOP3A
  'VOP3B': ['op', 'vdst', 'sdst', 'src0', 'src1', 'src2', 'omod', 'neg', 'clmp'],  # CDNA VOP3B
...
def _generate_enum_py(enums, src_enum, doc_name, buf_fmt=None) -> str:
  ...
  is_cdna = 'CDNA' in doc_name or 'MI' in doc_name
  def enum_lines(name, items):
    # CDNA VOP3AOp: VOP1 promotions in PDF are at 0x180+ but should be 0x140+ (subtract 64 from values >= 384)
    if is_cdna and name == 'VOP3AOp': items = {(v - 64 if v >= 384 else v): n for v, n in items.items()}
...
def _generate_str_pcode_py(enums, pseudocode, arch) -> str:
  ...
  # For CDNA, adjust pseudocode keys to match the modified VOP3AOp values (PDF has 0x180+, we use 0x140+)
  # VOP3BOp instructions (DIV_SCALE, MAD_U64, MAD_I64) have same values in PDF and enum, so exclude them
  if arch == "cdna":
    vop3b_names = {'V_DIV_SCALE_F32', 'V_DIV_SCALE_F64', 'V_MAD_U64_U32', 'V_MAD_I64_I32'}
    pseudocode = {((name, val - 64) if val >= 384 and name not in vop3b_names else (name, val)): pc for (name, val), pc in pseudocode.items()}
