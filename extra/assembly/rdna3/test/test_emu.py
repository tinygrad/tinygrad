...
class TestVOPD(unittest.TestCase):
  def test_vopd_add_nc_u32(self):
    ...
    kernel = VOPD(opx=VOPDOp.V_DUAL_MOV_B32, srcx0=256+1, vsrcx1=VGPR(0), vdstx=VGPR(3),
                  opy=VOPDOp.V_DUAL_ADD_NC_U32, srcy0=256+1, vsrcy1=VGPR(2), vdsty=VGPR(2)).to_bytes()
    kernel += s_endpgm().to_bytes()
    prog = decode_program(kernel)
    exec_wave(prog, state, bytearray(65536), 1)
  ...
  def test_vopd_lshlrev(self):
    ...
    # vdsty = (vdsty_enc << 1) | ((vdstx & 1) ^ 1), so for vdstx=3 (odd), vdsty_enc=2 gives vdsty=4
    kernel = VOPD(opx=VOPDOp.V_DUAL_MOV_B32, srcx0=256+1, vsrcx1=VGPR(0), vdstx=VGPR(3),
                  opy=VOPDOp.V_DUAL_LSHLREV_B32, srcy0=132, vsrcy1=VGPR(1), vdsty=VGPR(2)).to_bytes()  # V4 = V1 << 4
    kernel += s_endpgm().to_bytes()
    prog = decode_program(kernel)
    exec_wave(prog, state, bytearray(65536), 1)
  ...
  def test_vopd_and(self):
    ...
    # vdsty = (vdsty_enc << 1) | ((vdstx & 1) ^ 1), so for vdstx=3 (odd), vdsty_enc=2 gives vdsty=4
    kernel = VOPD(opx=VOPDOp.V_DUAL_MOV_B32, srcx0=256+1, vsrcx1=VGPR(0), vdstx=VGPR(3),
                  opy=VOPDOp.V_DUAL_AND_B32, srcy0=256+1, vsrcy1=VGPR(2), vdsty=VGPR(2)).to_bytes()
    kernel += s_endpgm().to_bytes()
    prog = decode_program(kernel)
    exec_wave(prog, state, bytearray(65536), 1)
  ...
  def test_vopd_parallel_read(self):
    ...
    kernel = VOPD(opx=VOPDOp.V_DUAL_MOV_B32, srcx0=256+0, vsrcx1=VGPR(0), vdstx=VGPR(7),
                  opy=VOPDOp.V_DUAL_ADD_NC_U32, srcy0=256+4, vsrcy1=VGPR(7), vdsty=VGPR(3)).to_bytes()
    kernel += s_endpgm().to_bytes()
    prog = decode_program(kernel)
    exec_wave(prog, state, bytearray(65536), 1)
...
class TestDecoder(unittest.TestCase):
  def test_vopd_literal_handling(self):
    ...
    vopd_bytes = VOPD(opx=8, srcx0=255, vsrcx1=VGPR(0), vdstx=VGPR(1),  # MOV: V1 = literal
                      opy=8, srcy0=128, vsrcy1=VGPR(0), vdsty=VGPR(2)).to_bytes()  # MOV: V2 = 0
