...
FIELD_TYPES = {
  'OP': lambda fmt, enums: f"{fmt}Op" if f"{fmt}Op" in enums else None,
  'SSRC0': 'SSrc', 'SSRC1': 'SSrc', 'SOFFSET': 'SSrc', 'SADDR': 'SSrc',
  'SRC0': 'Src', 'SRC1': 'Src', 'SRC2': 'Src',
  'SDST': 'SGPR', 'SBASE': 'SGPR', 'SDATA': 'SGPR', 'SRSRC': 'SGPR',
  'VDST': 'VGPR', 'VSRC1': 'VGPR', 'VDATA': 'VGPR', 'VADDR': 'VGPR', 'ADDR': 'VGPR', 'DATA': 'VGPR', 'DATA0': 'VGPR', 'DATA1': 'VGPR',
  'SIMM16': 'SImm', 'OFFSET': 'Imm',
}
FIELD_ORDER = {
  'SOP2': ['op', 'sdst', 'ssrc0', 'ssrc1'], 'SOP1': ['op', 'sdst', 'ssrc0'], 'SOPC': ['op', 'ssrc0', 'ssrc1'],
  'SOPK': ['op', 'sdst', 'simm16'], 'SOPP': ['op', 'simm16'],
...
FLOAT_MAP = {'0.5': 'POS_HALF', '-0.5': 'NEG_HALF', '1.0': 'POS_ONE', '-1.0': 'NEG_ONE',
             '2.0': 'POS_TWO', '-2.0': 'NEG_TWO', '4.0': 'POS_FOUR', '-4.0': 'NEG_FOUR', '1/(2*PI)': 'INV_2PI', '0': 'ZERO'}
...
def parse_fields_table(table: list, fmt: str, enums: set[str]) -> list[tuple[str, int, int, int | None, str | None]]:
  fields = []
  for row in table[1:]:
...
def is_fields_table(t) -> bool:
  return t and len(t) > 1 and t[0] and 'Field' in str(t[0][0] or '')
...
if __name__ == "__main__":
  ...
  for i, page in enumerate(pdf.pages[150:200]):
    ...
    for m in re.finditer(r'\d+\.\d+\.\d+\.\s+(\w+)\s*\n?Description', text):
      ...
      # find fields table AFTER header on this page, or on next page
      fields = None
      field_pos = text.find('Field Name', header_pos)
      if field_pos > header_pos:  # table is after header on same page
        for t in page.find_tables():
          if is_fields_table(t_data := t.extract()):
            fields = parse_fields_table(t_data, fmt_name, set(enums.keys()))
            if fields and any(f[0] == 'ENCODING' for f in fields): break
            fields = None
      if not fields and 150+i+1 < len(pdf.pages):  # check next page
        for t in pdf.pages[150+i+1].find_tables():
          if is_fields_table(t_data := t.extract()):
            ...
            fields = None
      if fields:
        # merge continuation from next page if table was on same page
        if field_pos > header_pos and 150+i+1 < len(pdf.pages):
          for nt in pdf.pages[150+i+1].extract_tables():
            if is_fields_table(nt) and (extra := parse_fields_table(nt, fmt_name, set(enums.keys()))):
              if not any(f[0] == 'ENCODING' for f in extra): fields.extend(extra)
              break
        formats[fmt_name] = fields

  # generate output
  lines = ["# autogenerated from AMD RDNA3.5 ISA PDF - do not edit",
           "from enum import IntEnum", "from extra.assembly.rdna3.lib import bits, Inst32, Inst64, SGPR, VGPR, TTMP, s, v",
           "from extra.assembly.rdna3.lib import SSrc, Src, SImm, Imm", "import functools", ""]
  ...
  for fmt_name, fields in sorted(formats.items()):
    ...
    if enc := next((f for f in fields if f[0] == 'ENCODING'), None):
      bits_str = f"bits[{enc[1]}]" if enc[1] == enc[2] else f"bits[{enc[1]}:{enc[2]}]"
      lines.append(f"  encoding = {bits_str} == 0b{enc[3]:b}" if enc[3] else f"  encoding = {bits_str} == 0")
    order = FIELD_ORDER.get(fmt_name, [])
    for name, hi, lo, _, ftype in sorted([f for f in fields if f[0] != 'ENCODING'],
                                          key=lambda f: order.index(f[0].lower()) if f[0].lower() in order else 1000):
  ...
  for cls_name, ops in sorted(enums.items()):
    fmt = cls_name[:-2]
    if fmt == "GLOBAL":
      for _, name in sorted(ops.items()): lines.append(f"{name.lower()} = functools.partial(FLAT, GLOBALOp.{name}, seg=2)")
    elif fmt == "SCRATCH":
      for _, name in sorted(ops.items()): lines.append(f"{name.lower()} = functools.partial(FLAT, SCRATCHOp.{name}, seg=2)")
    elif fmt in formats:
      suffix = "_e32" if fmt in ("VOP1", "VOP2") else ""
      for _, name in sorted(ops.items()): lines.append(f"{name.lower()}{suffix} = functools.partial({fmt}, {cls_name}.{name})")
  lines.append("")
