...
def parse_bits(s: str) -> tuple[int, int] | None:
  if m := re.match(r'\[(\d+)(?::(\d+))?\]', s): return int(m.group(1)), int(m.group(2) or m.group(1))
  return None
...
if __name__ == "__main__":
  ...
  lines = ["# autogenerated from AMD RDNA3.5 ISA PDF - do not edit", "from enum import IntEnum",
           ...
           "class SrcEnum(IntEnum):"] + [f"  {name} = {val}" for val, name in sorted(src_enum.items())] + [""]
  for cls_name, ops in sorted(enums.items()):
    lines += [f"class {cls_name}(IntEnum):"] + [f"  {name} = {opcode}" for opcode, name in sorted(ops.items())] + [""]
