...
# Designed to be shared between functional and pipeline emulators
...
VOP2_BASE, VOP1_BASE = 0x100, 0x180  # VOP3 already has these baked in (256, 384)
...
# Helper for S_CVT_I32_F32 / V_CVT_I32_F32
def _cvt_i32_f32(v):
  if math.isinf(v): return 0x7fffffff if v > 0 else (-0x80000000 & 0xffffffff)
  if math.isnan(v): return 0
  return max(-0x80000000, min(0x7fffffff, int(v))) & 0xffffffff

# Helper for S_CVT_U32_F32 / V_CVT_U32_F32
def _cvt_u32_f32(v):
  if math.isinf(v) and v > 0: return 0xffffffff
  if math.isinf(v) or math.isnan(v) or v < 0: return 0
  return min(0xffffffff, int(v))
...
SALU: dict[int, callable] = {
  ...
  # Signed arithmetic with overflow
  ...
  # Min/max with SCC = (s0 was selected)
  ...
  # Bit field extract with SCC = (result != 0)
  SOP2_BASE + SOP2Op.S_BFE_U32: lambda a, b, scc: ((r := ((a >> (b & 0x1f)) & ((1 << ((b >> 16) & 0x7f)) - 1)) if (b >> 16) & 0x7f else 0), int(r != 0)),
  SOP2_BASE + SOP2Op.S_BFE_I32: lambda a, b, scc: ((r := (sext((a >> (b & 0x1f)) & ((1 << (w := (b >> 16) & 0x7f)) - 1), w) & 0xffffffff) if w else 0), int(r != 0)),
  # Pack operations (no SCC change)
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # SOP1 - Unary operations (s1 ignored)
  # ─────────────────────────────────────────────────────────────────────────────
  ...
  # Float conversions/rounding (no SCC change)
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # SOPC - Comparison operations (result is 0, only scc_out matters)
  # ─────────────────────────────────────────────────────────────────────────────
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # SOPK - Immediate operations (s1 is the 16-bit immediate)
  # ─────────────────────────────────────────────────────────────────────────────
...
# Carry ops are handled separately by the caller
# ═══════════════════════════════════════════════════════════════════════════════
VALU: dict[int, callable] = {
  # ─────────────────────────────────────────────────────────────────────────────
  # VOP2 - Binary operations (at VOP2_BASE + opcode, or VOP3 256 + opcode)
  # ─────────────────────────────────────────────────────────────────────────────
  # Float binary
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # VOP1 - Unary operations (at VOP1_BASE + opcode, or VOP3 384 + opcode)
  # ─────────────────────────────────────────────────────────────────────────────
  ...
  # Transcendentals
  VOP1_BASE + VOP1Op.V_RCP_F32: lambda a, b, c: i32(1.0 / (v := f32(a)) if v != 0 else math.copysign(float('inf'), v)),
  VOP1_BASE + VOP1Op.V_RCP_IFLAG_F32: lambda a, b, c: i32(1.0 / (v := f32(a)) if v != 0 else math.copysign(float('inf'), v)),
  VOP1_BASE + VOP1Op.V_RSQ_F32: lambda a, b, c: i32(1.0 / math.sqrt(v) if (v := f32(a)) > 0 else (float('nan') if v < 0 else float('inf'))),
  VOP1_BASE + VOP1Op.V_SQRT_F32: lambda a, b, c: i32(math.sqrt(v) if (v := f32(a)) >= 0 else float('nan')),
  VOP1_BASE + VOP1Op.V_LOG_F32: lambda a, b, c: i32(math.log2(v) if (v := f32(a)) > 0 else (float('-inf') if v == 0 else float('nan'))),
  VOP1_BASE + VOP1Op.V_EXP_F32: lambda a, b, c: i32(float('inf') if (v := f32(a)) > 128 else (0.0 if v < -150 else math.pow(2.0, v))),
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  # VOP3-only - 3-source ops (these use VOP3Op values directly, already offset)
  # ─────────────────────────────────────────────────────────────────────────────
  ...
  VOP3Op.V_BFE_I32: lambda a, b, c: (sext((a >> (b & 0x1f)) & ((1 << (w := c & 0x1f)) - 1), w) & 0xffffffff) if w else 0,
...
def salu(op: int, s0: int, s1: int, scc_in: int) -> tuple[int, int]:
  """Scalar ALU - pure combinational. Returns (result, scc_out)."""
  if (fn := SALU.get(op)): return fn(s0, s1, scc_in)
  return (s0, scc_in)

def valu(op: int, s0: int, s1: int, s2: int) -> int | None:
  """Vector ALU (single lane) - pure combinational. Returns result or None if op not found."""
  if (fn := VALU.get(op)): return fn(s0, s1, s2)
  return None

def vopc(op: int, s0: int, s1: int) -> int:
  """Vector comparison (single lane) - pure combinational. Returns comparison result (0 or 1)."""
  base = op & 0x7f
  # F32 comparisons (16-31)
  if 16 <= base <= 31:
    f0, f1, cmp = f32(s0), f32(s1), base - 16
    nan = math.isnan(f0) or math.isnan(f1)
    return int([False, f0<f1, f0==f1, f0<=f1, f0>f1, f0!=f1, f0>=f1, not nan, nan,
                f0<f1 or nan, f0==f1 or nan, f0<=f1 or nan, f0>f1 or nan, f0!=f1 or nan, f0>=f1 or nan, True][cmp])
  # I16 comparisons (49-54)
  if 49 <= base <= 54:
    cmp, s0s, s1s = base - 49, sext(s0 & 0xffff, 16), sext(s1 & 0xffff, 16)
    return int([s0s<s1s, s0s==s1s, s0s<=s1s, s0s>s1s, s0s!=s1s, s0s>=s1s][cmp])
  # U16 comparisons (57-62)
  if 57 <= base <= 62:
    cmp, s0u, s1u = base - 57, s0 & 0xffff, s1 & 0xffff
    return int([s0u<s1u, s0u==s1u, s0u<=s1u, s0u>s1u, s0u!=s1u, s0u>=s1u][cmp])
  # I32/U32 comparisons (64-79)
  if 64 <= base <= 79:
    cmp = (base - 64) % 8
    if base < 72:  # I32
      s0s, s1s = sext(s0, 32), sext(s1, 32)
      return int([False, s0s<s1s, s0s==s1s, s0s<=s1s, s0s>s1s, s0s!=s1s, s0s>=s1s, True][cmp])
    else:  # U32
      return int([False, s0<s1, s0==s1, s0<=s1, s0>s1, s0!=s1, s0>=s1, True][cmp])
  # CLASS_F32 (126)
  if base == 126:
    f, mask = f32(s0), s1
