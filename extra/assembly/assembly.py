...
class AssemblyLanguage:
    ...
    def render_numnode(self, b) -> Register:
        ...
        if key not in self.tor:
            self.ins.append(
                AssemblyInstruction(
                    Ops.LOAD, self.newreg(key, scalar=True, dtype=dtypes.int32), [], b
                )
            )
        return self.tor[key]
    ...
    def render_cast(self, a: Register, new_dtype: DType) -> Register:
        ...
        if key not in self.tor:
            self.ins.append(
                AssemblyInstruction(Ops.CAST, self.newreg(key, dtype=new_dtype), [a])
            )
        return self.tor[key]
    ...
    def addr_w_offset(self, args):
        ...
        if self.supports_load3:
            if reg.scalar:
                ...
                self.ins.append(
                    AssemblyInstruction(Ops.ALU, new_reg, [reg], UnaryOps.NOOP)
                )
                reg = new_reg
...
def uops_to_asmstyle(lang, function_name: str, uops: List[UOp]):
    ...
    lang.cnts.clear()
    buf_to_dtype = {
        args: dtype for uop, dtype, _, args, _ in uops if uop == Ops.DEFINE_GLOBAL
    }
    global_size, local_size = [], []
    skipload_branch = 0
    lang.ins += [
        AssemblyInstruction(
            Ops.SPECIAL, lang.newreg(buf, dtype=dtypes.uint64, scalar=True), [], buf
        )
        ...
        if uop == Ops.DEFINE_LOCAL:
            ...
            if args[1] not in ["global", "local", "global+local"]:
                ...
                for i, var in enumerate(reversed(args[0])):
                    lang.ins.append(
                        AssemblyInstruction(
                            Ops.ENDLOOP, None, [lang.tor[var]], (var.max + 1, f"gid{i}")
                        )
                    )
            elif args[1] == "local":
                for i, var in enumerate(reversed(args[0])):
                    lang.ins.append(
                        AssemblyInstruction(
                            Ops.ENDLOOP, None, [lang.tor[var]], (var.max + 1, f"lid{i}")
                        )
                    )
        elif uop == Ops.CAST:
            ...
            for i, sr in enumerate(out.subregs()):
                lang.ins.append(
                    AssemblyInstruction(Ops.ALU, sr, [lang.tor[vin[i]]], UnaryOps.NOOP)
                )
        elif uop == Ops.ALU:
            ...
            if args in [BinaryOps.CMPLT]:
                ...
                lang.ins.append(
                    AssemblyInstruction(
                        Ops.ALU, pred_reg, [lang.tor[x] for x in vin], args
                    )
                )
                ...
                lang.ins.append(
                    AssemblyInstruction(
                        Ops.ALU, tmp, [lang.tor[vin[1]]], UnaryOps.RECIP
                    )
                )
                lang.ins.append(
                    AssemblyInstruction(
                        Ops.ALU, out, [lang.tor[vin[0]], tmp], BinaryOps.MUL
                    )
                )
                ...
                lang.ins.append(
                    AssemblyInstruction(Ops.ALU, out, [lang.tor[x] for x in vin], args)
                )
            ...
            lang.ins.append(
                AssemblyInstruction(Ops.LOAD, lang.newreg(u, dtype=dtype), [], args)
            )
        elif uop == Ops.LOAD:
            ...
            if args.valid.min == 0 and args.valid.max == 1:
                lang.ins.append(
                    AssemblyInstruction(
                        Ops.LABEL, None, [], f"$skipload_{skipload_branch}"
                    )
                )
                skipload_branch += 1
        elif uop == Ops.STORE:
            if args is None:
                lang.ins.append(
                    AssemblyInstruction(
                        Ops.ALU, lang.tor[vin[0]], [lang.tor[vin[1]]], UnaryOps.NOOP
                    )
                )
            else:
                idx, treg, off = lang.addr_w_offset(args)
