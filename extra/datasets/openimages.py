...
def fetch_openimages(output_fn: str, base_dir: Path, subset: str):
    ...
    if subset == "train":
        ...
        annotations_fn = (
            annotations_dir / VALIDATION_BBOX_ANNOTATIONS_URL.split("/")[-1]
        )
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(download_image, bucket, subset, image_id, data_dir)
...
def iterate(coco, base_dir, bs=8):
    ...
    for i in range(0, len(image_ids), bs):
        ...
        for img_id in image_ids[i : i + bs]:
            ...
            x, original_size = image_load(
                base_dir, img_dict["subset"], img_dict["file_name"]
            )
            X.append(x)
...
def download_dataset(base_dir: Path, subset: str) -> Path:
    if (ann_file := base_dir / f"{subset}/labels/openimages-mlperf.json").is_file():
