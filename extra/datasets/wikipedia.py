    ...
    def tokenize(self, text: str) -> list[str]:
        ...
        for token in whitespace_tokenize(
            _tokenize_chinese_chars(
                _clean_text(
                    text.decode("utf-8", "ignore") if isinstance(text, bytes) else text
                )
            )
        ):
...
def truncate_seq_pair(
    tokens_a: list[str], tokens_b: list[str], max_num_tokens: int, rng: random.Random
) -> None:
    while True:
...
def create_masked_lm_predictions(
    tokens: list[str], tokenizer: Tokenizer, rng: random.Random, vocab_words: list[str]
    ...
    num_to_predict = min(
        getenv("MAX_PREDICTIONS_PER_SEQ", 76),
        max(1, int(round(len(tokens) * getenv("MASKED_LM_PROB", 0.15)))),
    )
            ...
            if current_chunk:
                ...
                tokens, masked_lm_positions, masked_lm_labels = (
                    create_masked_lm_predictions(
                        tokens, tokenizer, rng, list(tokenizer.vocab.keys())
                    )
                )
                instances.append(
                    {
...
def get_instances(
    rng: random.Random, tokenizer: Tokenizer, documents: list[list[str]]
) -> list[dict]:
    instances = []
    for _ in range(getenv("DUPE_FACTOR", 10)):
        for di, doc in enumerate(documents):
            instances.extend(
                create_instances_from_document(rng, tokenizer, doc, di, documents)
            )
...
def instance_to_features(instance: dict, tokenizer: Tokenizer) -> dict:
    ...
    return {
        ...
        "masked_lm_positions": np.expand_dims(
            np.array(masked_lm_positions, dtype=np.int32), 0
        ),
        "masked_lm_ids": np.expand_dims(np.array(masked_lm_ids, dtype=np.int32), 0),
        "masked_lm_weights": np.expand_dims(
            np.array(masked_lm_weights, dtype=np.float32), 0
        ),
        "next_sentence_labels": np.expand_dims(
            np.array([next_sentence_label], dtype=np.int32), 0
        ),
    }
...
def process_part(part: int):
    tokenizer = Tokenizer(
        getenv("BASEDIR", Path(__file__).parent / "wiki") / "vocab.txt"
    )
    os.makedirs(BASEDIR / "train", exist_ok=True)

    if os.path.exists(BASEDIR / f"train/{str(part)}.pkl"):
        return
...
def get_features_from_part(
    tokenizer: Tokenizer, val: bool = False, part: int = 0
) -> list[dict]:  # Convert raw text to masked NSP samples
    ...
    if val:
        ...
        return [
            instance_to_features(instances[int(inst * pick_ratio)], tokenizer)
            for inst in range(10000)
        ]
    else:
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "pre-eval":  # Generate 10000 eval samples
        ...
        if sys.argv[2] == "all":  # Use all 500 parts for training generation
            process_map(
                process_part,
                [part for part in range(500)],
                max_workers=getenv("NUM_WORKERS", min(os.cpu_count(), 32)),
                chunksize=1,
            )
