...
def get_train_files():
    return sorted(
        [
            ...
            if x.stem.startswith("case")
            and int(x.stem.split("_")[-1]) < 210
            and x not in get_val_files()
        ]
    )
...
def get_val_files():
    data = fetch(
        "https://raw.githubusercontent.com/mlcommons/training/master/retired_benchmarks/unet3d/pytorch/evaluation_cases.txt"
    ).read_text()
    return sorted(
...
def preprocess_dataset(filenames, preprocessed_dir, val):
    ...
    for fn in tqdm(
        filenames, desc=f"preprocessing {'validation' if val else 'training'}"
    ):
    ...
    for i in range(0, len(files), bs):
        ...
        for i in order[i : i + bs]:
            if preprocessed_dir is not None:
                x_cached_path, y_cached_path = (
                    preprocessed_dir / f"{os.path.basename(files[i])}_x.npy",
                    preprocessed_dir / f"{os.path.basename(files[i])}_y.npy",
                )
                if x_cached_path.exists() and y_cached_path.exists():
                    samples += [(np.load(x_cached_path), np.load(y_cached_path))]
            else:
                samples += [preprocess(files[i])]
...
def sliding_window_inference(
    model, inputs, labels, roi_shape=(128, 128, 128), overlap=0.5, gpus=None
):
...
def gaussian_noise(image, mean=0.0, std=0.1, prob=0.1):
    if random.random() < prob:
        ...
        noise = np.random.normal(loc=mean, scale=scale, size=image.shape).astype(
            image.dtype
        )
        image += noise
    return image
