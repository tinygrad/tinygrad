...
def verify_checksum(folder_path: str, checksum_path: str):
    ...
    with open(checksum_path, "r") as f:
        for line in f:
            ...
            )  # remove './' from the start of the folder name
            hasher = hashlib.md5()
...
def download_wikipedia(path: str):
    ...
    gdrive_download(
        "https://drive.google.com/uc?id=1fbGClQMi2CoMv7fwrwTC5YYPooQBdcFW",
        os.path.join(path, "bert_config.json"),
    )
    gdrive_download(
        "https://drive.google.com/uc?id=1USK108J6hMM_d27xCHi738qBL8_BT1u1",
        os.path.join(path, "vocab.txt"),
    )
    gdrive_download(
        "https://drive.google.com/uc?id=1chiTBljF0Eh1U5pKs6ureVHgSbtU8OG_",
        os.path.join(path, "model.ckpt-28252.data-00000-of-00001"),
    )
    gdrive_download(
        "https://drive.google.com/uc?id=1Q47V3K3jFRkbJ2zGCrKkKk-n0fvMZsa0",
        os.path.join(path, "model.ckpt-28252.index"),
    )
    gdrive_download(
        "https://drive.google.com/uc?id=1vAcVmXSLsLeQ1q7gvHnQUSth5W_f_pwv",
        os.path.join(path, "model.ckpt-28252.meta"),
    )
    with open(os.path.join(path, "checkpoint"), "w") as f:
        f.write(
            'model_checkpoint_path: "model.ckpt-28252"\nall_model_checkpoint_paths: "model.ckpt-28252"'
        )
    if getenv("WIKI_TRAIN", 0):
        gdrive_download(
            "https://drive.google.com/uc?id=1tmMgLwoBvbEJEHXh77sqrXYw5RpqT8R_",
            os.path.join(path, "bert_reference_results_text_md5.txt"),
        )
        gdrive_download(
            "https://drive.google.com/uc?id=14xV2OUGSQDG_yDBrmbSdcDC-QGeqpfs_",
            os.path.join(path, "results_text.tar.gz"),
        )
        wikipedia_uncompress_and_extract(
            os.path.join(path, "results_text.tar.gz"), path
        )
        if getenv("VERIFY_CHECKSUM", 0):
            verify_checksum(
                os.path.join(path, "results4"),
                os.path.join(path, "bert_reference_results_text_md5.txt"),
            )
