...
# a packet field is a machine immediate: its width comes from the packet schema, so the const states it (a link-time value passes through)
def _imm(x:sint, dtype=dtypes.uint32): return x.cast(dtype) if isinstance(x, UOp) else UOp(Ops.CONST, dtype, arg=dtype.const(x))

def pkt3(ctx, op:PM4Ops, *vals):
  return UOp(Ops.INS, arg=op, src=tuple(_imm(x) for x in (ctx.pm4.PACKET3(getattr(ctx.pm4, f"PACKET3_{op.name}"), len(vals) - 1), *vals)))
...
def pm4_submit(ctx, lin):
  ...
  done_idx, submit_idx = _imm(size_dw + 0, dtypes.int), _imm(size_dw + 1, dtypes.int)
  init_counters = make_patches(ib, [((size_dw + i) * 4, _imm(0, dtypes.uint32)) for i in range(2)]).rtag("link")
  ...
  put = put_ptr.index(zero:=_imm(0, dtypes.int))
  ...
  write_pkt = UOp.barrier(*[ring.index(((put + off) % q.ring.size).cast(dtypes.int)).store(_imm(x)) for off,x in enumerate(pkt)])
...
def sdma_copy(ctx, call):
  ...
  return call.ins(SDMAOps.COPY, src=tuple(_imm(x) for off in range(0, sz, ctx.max_copy_size) for x in (
...
def sdma_wait(ctx, ins, dst, val):
  ...
  return ins.ins(SDMAOps.POLL_REGMEM, src=tuple(_imm(x) for x in (
...
def sdma_store(ctx, ins, dst, val):
  ...
  return UOp(Ops.LINEAR, src=(
    ins.ins(SDMAOps.FENCE, src=tuple(_imm(x) for x in (op, *data64_le(dst.getaddr(ctx.devs)), val))),
    ins.ins(SDMAOps.TRAP, src=tuple(_imm(x) for x in (ctx.sdma.SDMA_OP_TRAP, 0)))))
...
def sdma_timestamp(ctx, ins, dst):
  ...
  return ins.ins(SDMAOps.TIMESTAMP, src=tuple(_imm(x) for x in (op, *data64_le(dst.getaddr(ctx.devs)))))
...
def sdma_submit(cmdbuf, devs):
  ...
  size_dw, zero = cmdbuf.nbytes() // dtypes.uint32.itemsize, _imm(0, dtypes.int)
  ...
  zero_tail = ring.index(tail_off_dw + zi).store(_imm(0)).end(zi)
  i = UOp.range(_imm(size_dw, dtypes.int), 0, dtype=dtypes.int, src=(cmdbuf,))
