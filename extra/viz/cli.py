...
from tinygrad.helpers import temp, ansistrip, colored, time_to_str
...
if __name__ == "__main__":
  ...
  parser.add_argument('--agg', action="store_true", default=True,
                      help="Profile only: View aggregate table or all kernels (like DEBUG=2) (default: true)")
    ...
    for k,v in profile["layout"].items():
      ...
      for e in v.get("events", []):
        ...
        if not args.agg:
          ...
          print(e["name"], ptm,  " "+e["fmt"].replace("\n", " | ")+"  ")
