...
class TestTorchBackendInplace(unittest.TestCase):
  ...
  def test_topk_inplace(self):
    x = torch.tensor([1, 3, 2, 4], dtype=torch.float32)
    sorted_tensor = torch.empty(2, dtype=torch.float32)
    indices_tensor = torch.empty(2, dtype=torch.int32)
  ...
  def test_sort_inplace(self):
    x = torch.tensor([3, 1, 4, 2], dtype=torch.float32)
    sorted_tensor = torch.empty_like(x)
    indices_tensor = torch.empty_like(x, dtype=torch.int32)
