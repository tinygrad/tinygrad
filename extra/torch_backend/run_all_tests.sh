...
echo -e "\n>>> Unit Tests\n"
PYTHONPATH=. CPU=1 TINY_BACKEND=1 "$PYTHON_BIN" -m pytest -x test/test_ops.py 
PYTHONPATH=. CPU=1 CPU_LLVM=1 LLVMOPT=0 TINY_BACKEND=1 "$PYTHON_BIN" -m pytest -x test/test_ops.py 
PYTHONPATH=. CPU=1 TINY_BACKEND=1 "$PYTHON_BIN" -m pytest -x test/unit/test_linalg.py -v
PYTHONPATH=. CPU=1 CPU_LLVM=1 LLVMOPT=0 TINY_BACKEND=1 "$PYTHON_BIN" -m pytest -x test/unit/test_linalg.py -v
...
echo -e "\n>>> Multi-GPU Test\n"
PYTHONPATH=. CPU=1 CPU_LLVM=1 GPUS=4 TORCH_DEBUG=1 "$PYTHON_BIN" extra/torch_backend/test_multigpu.py

echo -e "\n>>> Beautiful MNIST Test\n"
PYTHONPATH=. STEPS=20 CPU=1 TARGET_EVAL_ACC_PCT=90.0 TINY_BACKEND=1 "$PYTHON_BIN" examples/other_mnist/beautiful_mnist_torch.py

echo -e "\n>>> Some Torch Tests (may fail)\n"
PYTHONPATH=. "$PYTHON_BIN" -m pytest extra/torch_backend/torch_tests.py -v --tb=no || true
