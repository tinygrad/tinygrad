...
def kfd_ioctl(idir, nr, user_struct, fd, **kwargs):
    ...
    ret = fcntl.ioctl(
        fd,
        (idir << 30) | (ctypes.sizeof(user_struct) << 16) | (ord("K") << 8) | nr,
        made,
    )
    if ret != 0:
...
def ioctls_from_header():
    ...
    for name, idir, nr, sname in matches:
        fxns[name.replace("AMDKFD_IOC_", "").lower()] = functools.partial(
            kfd_ioctl, idirs[idir], int(nr, 0x10), getattr(kfd, "struct_" + sname)
        )
...
def gpu_alloc_userptr(fd, size, flags):
    ...
    mem = kio.alloc_memory_of_gpu(
        fd, va_addr=addr, size=size, gpu_id=GPU_ID, flags=flags, mmap_offset=addr
    )
    return mem
...
def gpu_alloc(fd, size, flags):
    addr = libc.mmap(
        0, size, 0, mmap.MAP_PRIVATE | mmap.MAP_ANONYMOUS | MAP_NORESERVE, -1, 0
    )
    assert addr != 0xFFFFFFFFFFFFFFFF
    mem = kio.alloc_memory_of_gpu(
        fd, va_addr=addr, size=size, gpu_id=GPU_ID, flags=flags
    )
    buf = libc.mmap(
        mem.va_addr,
        mem.size,
...
if __name__ == "__main__":
    ...
    with open(
        f"/sys/devices/virtual/kfd/kfd/topology/nodes/{1+gpu_num}/gpu_id", "r"
    ) as f:
        GPU_ID = int(f.read())
    ...
    stm = kio.map_memory_to_gpu(
        fd,
        handle=rw_ptr.handle,
        device_ids_array_ptr=ctypes.addressof(arr),
        n_devices=1,
    )
    assert stm.n_success == 1
    event_page = gpu_alloc(
        fd,
        0x8000,
        kfd.KFD_IOC_ALLOC_MEM_FLAGS_GTT
        | kfd.KFD_IOC_ALLOC_MEM_FLAGS_WRITABLE
    ...
    stm = kio.map_memory_to_gpu(
        fd,
        handle=event_page.handle,
        device_ids_array_ptr=ctypes.addressof(arr),
        n_devices=1,
    )
    assert stm.n_success == 1
    ring_base = gpu_alloc_userptr(
        fd,
        0x1000,
    ...
    stm = kio.map_memory_to_gpu(
        fd,
        handle=ring_base.handle,
        device_ids_array_ptr=ctypes.addressof(arr),
        n_devices=1,
    )
    assert stm.n_success == 1
    signals = gpu_alloc_userptr(
        fd,
        0x1000,
        kfd.KFD_IOC_ALLOC_MEM_FLAGS_USERPTR
        | kfd.KFD_IOC_ALLOC_MEM_FLAGS_WRITABLE
    ...
    stm = kio.map_memory_to_gpu(
        fd,
        handle=eop_buffer.handle,
        device_ids_array_ptr=ctypes.addressof(arr),
        n_devices=1,
    )
    assert stm.n_success == 1
    ctx_save_restore_address = gpu_alloc(
        fd,
        0x2C02000,
        kfd.KFD_IOC_ALLOC_MEM_FLAGS_VRAM
        | kfd.KFD_IOC_ALLOC_MEM_FLAGS_WRITABLE
    ...
    stm = kio.map_memory_to_gpu(
        fd,
        handle=ctx_save_restore_address.handle,
        device_ids_array_ptr=ctypes.addressof(arr),
        n_devices=1,
    )
    assert stm.n_success == 1
    ...
    nq = kio.create_queue(
        fd,
        ring_base_address=ring_base.va_addr,
        ring_size=0x1000,
        gpu_id=GPU_ID,
        ...
        ctx_save_restore_address=ctx_save_restore_address.va_addr,
        ...
        write_pointer_address=rw_ptr.va_addr + 0,
        read_pointer_address=rw_ptr.va_addr + 0x8,
    )
    doorbell = libc.mmap(
        0,
        8192,
  ...
  """
    ...
    packet.reserved1 = 0
    for i in range(5):
        packet.dep_signal[i] = EMPTY_SIGNAL
    ...
    to_mv(signals.va_addr, 0x40).cast("Q")[2] = (
        event_page.va_addr + evt.event_slot_index * 8
    )  # HWData2=HWAddress
