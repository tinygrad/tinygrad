...
if __name__ == "__main__":
    ...
    sdma_ring = dev._gpu_alloc(
        1 << 22, kfd.KFD_IOC_ALLOC_MEM_FLAGS_USERPTR, uncached=True
    )
    gart = dev._gpu_alloc(0x1000, kfd.KFD_IOC_ALLOC_MEM_FLAGS_GTT, uncached=True)
    sdma_queue = kio.create_queue(
        AMDDevice.kfd,
        ...
        write_pointer_address=gart.va_addr + 0x100,
        read_pointer_address=gart.va_addr + 0x108,
    )
    ...
    sdma_doorbell = to_mv(
        doorbells + sdma_queue.doorbell_offset - doorbells_base, 4
    ).cast("I")

    test_write_page = dev._gpu_alloc(
        0x1000, kfd.KFD_IOC_ALLOC_MEM_FLAGS_USERPTR, uncached=True
    )
    cmd = sdma_pkts.timestamp(
        op=amd_sdma.SDMA_OP_TIMESTAMP,
        sub_op=amd_sdma.SDMA_SUBOP_TIMESTAMP_GET_GLOBAL,
        addr=test_write_page.va_addr,
    )
    ...
    def blit_sdma_command(cmd):
        ctypes.memmove(
            sdma_ring.va_addr + (sdma_doorbell_value % sdma_ring.size),
            ctypes.addressof(cmd),
            sz := ctypes.sizeof(cmd),
        )
        return sz
    ...
    while True:
        ...
        while sdma_read_pointer[0] != sdma_write_pointer[0]:
            continue
        tm = to_mv(test_write_page.va_addr, 0x1000).cast("Q")[0] / 1e8
        print(
            f"{tm:.3f} s @ 0x{sdma_ring.va_addr + (sdma_doorbell_value % sdma_ring.size):X} R:0x{sdma_queue.read_pointer_address:X} W:0x{sdma_queue.write_pointer_address:X}"
        )
        time.sleep(0.01)
