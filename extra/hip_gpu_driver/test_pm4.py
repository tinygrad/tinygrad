...
if __name__ == "__main__":
    ...
    a = Tensor([0.0, 1.0, 2.0], device="KFD").realize()
    b = a + 7
    ...
    gart_compute = dev._gpu_alloc(
        0x1000, kfd.KFD_IOC_ALLOC_MEM_FLAGS_GTT, uncached=True
    )
    eop_buffer = dev._gpu_alloc(0x1000, kfd.KFD_IOC_ALLOC_MEM_FLAGS_VRAM)
    compute_ring = dev._gpu_alloc(
        0x800000, kfd.KFD_IOC_ALLOC_MEM_FLAGS_GTT, uncached=True
    )
    ctx_save_restore_address = dev._gpu_alloc(
        0x2C02000, kfd.KFD_IOC_ALLOC_MEM_FLAGS_VRAM
    )
    compute_queue = kio.create_queue(
        dev.kfd,
        ring_base_address=compute_ring.va_addr,
        ring_size=compute_ring.size,
        gpu_id=dev.gpu_id,
        queue_type=kfd.KFD_IOC_QUEUE_TYPE_COMPUTE,
        ...
        write_pointer_address=gart_compute.va_addr,
        read_pointer_address=gart_compute.va_addr + 8,
    )
    compute_doorbell = to_mv(
        dev.doorbells + compute_queue.doorbell_offset - dev.doorbells_base, 4
    ).cast("I")
    ...
    pm4_cmd = [
        amd_gpu.PACKET3(amd_gpu.PACKET3_SET_SH_REG, 6),
        regCOMPUTE_PGM_LO,
        code_ptr & 0xFFFFFFFF,
        code_ptr >> 32,
        0,
    ...
    pm4_cmd += [
        amd_gpu.PACKET3(amd_gpu.PACKET3_SET_SH_REG, 2),
        regCOMPUTE_PGM_RSRC1,
        code.compute_pgm_rsrc1,
        code.compute_pgm_rsrc2,
    ]
    pm4_cmd += [
        amd_gpu.PACKET3(amd_gpu.PACKET3_SET_SH_REG, 2),
        regCOMPUTE_USER_DATA_0,
        dev.kernargs_ptr & 0xFFFFFFFF,
        dev.kernargs_ptr >> 32,
    ]
    # pm4_cmd += [amd_gpu.PACKET3(amd_gpu.PACKET3_SET_SH_REG, 2), regCOMPUTE_USER_DATA_0, 0, 0]
    pm4_cmd += [
        amd_gpu.PACKET3(amd_gpu.PACKET3_SET_SH_REG, 8),
        regCOMPUTE_START_X,
        0,
        0,
        0,
    ...
    pm4_cmd += [
        amd_gpu.PACKET3(amd_gpu.PACKET3_DISPATCH_DIRECT, 3),
        runner.global_size[0],
        runner.global_size[1],
        runner.global_size[2],
        CS_W32_EN | COMPUTE_SHADER_EN,
    ]
  ...
  """
    ...
    for j in range(0x80000):
        for i, value in enumerate(pm4_cmd):
            pm4_buffer_view[wptr + i] = value
        wptr += len(pm4_cmd)
