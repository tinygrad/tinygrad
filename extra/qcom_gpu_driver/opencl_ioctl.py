...
for child in xml.getroot():
    if "name" in child.attrib and child.attrib["name"] == "adreno_pm4_type3_packets":
        for sc in child:
            if "name" in sc.attrib and (
                "variants" not in sc.attrib or sc.attrib["variants"] != "A2XX"
            ):
...
for k, v in adreno.__dict__.items():
    if k.startswith("REG_") and isinstance(v, int) and v > 1024:
        REGS[v] = k
...
def ioctls_from_header():
    hdr = (
        (
            ...
            / "extra/qcom_gpu_driver/msm_kgsl.h"
        )
        .read_text()
        .replace("\\\n", "")
    )
    ...
    return {
        int(nr, 0x10): (name, getattr(msm_kgsl, "struct_" + sname))
        for name, nr, sname in matches
    }
...
def format_struct(s):
    ...
    for field_name, field_type in s._fields_:
        if field_name in {"__pad", "PADDING_0"}:
            continue
        dat = getattr(s, field_name)
        if isinstance(dat, int):
            sdats.append(f"{field_name}:0x{dat:X}")
        else:
            sdats.append(f"{field_name}:{dat}")
    return sdats
...
def parse_cmd_buf(dat):
    ...
    while ptr < len(dat):
        ...
        if (cmd >> 24) == 0x70:
            ...
            if IOCTL > 0:
                print(
                    f"{ptr:3X} -- typ 7: {size=:3d}, {opcode=:#x} {ops[opcode]}",
                    hprint(vals),
                )
            if (
                ops[opcode] == "CP_LOAD_STATE6_FRAG"
            ):  # for compute shaders CP_LOAD_STATE6_FRAG is used
                ...
                if IOCTL > 0:
                    print(
                        f"{num_unit=} {state_block=} {state_src=} {state_type=} {dst_off=}"
                    )

                if "LOAD_FRAGS" not in CAPTURED_STATE:
                    CAPTURED_STATE["LOAD_FRAGS"] = []
                CAPTURED_STATE["LOAD_FRAGS"].append(
                    (state_block, state_type, num_unit, dst_off)
                )
                ...
                if state_block == SB6_CS_SHADER:
                    ...
                    if state_type == ST6_IBO:
                        ibos_bytes = get_mem(
                            (vals[2] << 32) | vals[1], num_unit * 16 * 4
                        )
                    ...
                    if state_type == ST6_SHADER:
                        samplers_bytes = get_mem(
                            (vals[2] << 32) | vals[1], num_unit * 4 * 4
                        )
...
def ioctl(fd, request, argp):
    ret = libc.syscall(
        0x1D, ctypes.c_int(fd), ctypes.c_ulong(request), ctypes.c_void_p(argp)
    )
    ...
    if nr in nrs and itype == 9:
        ...
        if name == "IOCTL_KGSL_GPU_COMMAND":
            for i in range(s.numcmds):
                cmd = get_struct(
                    s.cmdlist + ctypes.sizeof(msm_kgsl.struct_kgsl_command_object) * i,
                    msm_kgsl.struct_kgsl_command_object,
                )
                if IOCTL > 0:
            ...
            for i in range(s.numobjs):
                obj = get_struct(
                    s.objlist + s.objsize * i, msm_kgsl.struct_kgsl_command_object
                )
                if IOCTL > 0:
                    print(f"obj {i}:", format_struct(obj))
                    print(
                        format_struct(
                            msm_kgsl.struct_kgsl_cmdbatch_profiling_buffer.from_buffer_copy(
                                get_mem(obj.gpuaddr, obj.size)
                            )
                        )
                    )
...
def install_hook(c_function, python_function):
    ...
    tramp += struct.pack(
        "Q",
        ctypes.cast(
            ctypes.byref(python_function), ctypes.POINTER(ctypes.c_ulong)
        ).contents.value,
    )

    # get real ioctl address
    ioctl_address = ctypes.cast(
        ctypes.byref(c_function), ctypes.POINTER(ctypes.c_ulong)
    )

    # hook ioctl
    libc = ctypes.CDLL(ctypes.util.find_library("libc"))
    ret = libc.mprotect(
        ctypes.c_ulong((ioctl_address.contents.value // 0x1000) * 0x1000), 0x2000, 7
    )
    assert ret == 0
...
def compare_launch_state(state, good_state):
    cmp = [
        ...
        (
            adreno.REG_A6XX_SP_CS_CTRL_REG0,
            adreno.A6XX_SP_CS_CTRL_REG0_FULLREGFOOTPRINT__MASK,
        ),
        (
            adreno.REG_A6XX_SP_CS_CTRL_REG0,
        ...
        (
            adreno.REG_A6XX_SP_CS_PVT_MEM_PARAM,
            adreno.A6XX_SP_CS_PVT_MEM_PARAM_MEMSIZEPERITEM__MASK,
        ),
        (
            adreno.REG_A6XX_SP_CS_PVT_MEM_PARAM,
            adreno.A6XX_SP_CS_PVT_MEM_PARAM_HWSTACKSIZEPERTHREAD__MASK,
        ),
        ...
        (
            adreno.REG_A6XX_HLSQ_CS_NDRANGE_0,
            adreno.A6XX_HLSQ_CS_NDRANGE_0_LOCALSIZEX__MASK,
        ),
        (
            adreno.REG_A6XX_HLSQ_CS_NDRANGE_0,
            adreno.A6XX_HLSQ_CS_NDRANGE_0_LOCALSIZEY__MASK,
        ),
        (
            adreno.REG_A6XX_HLSQ_CS_NDRANGE_0,
            adreno.A6XX_HLSQ_CS_NDRANGE_0_LOCALSIZEZ__MASK,
        ),
    ...
    for x, m in cmp:
        print(
            f"Field {REGS[x]}, mask: 0x{m:X} cmp: {state.get(x, 0) & m} vs {good_state.get(x, 0) & m}"
        )
        if state.get(x, 0) & m != good_state.get(x, 0) & m:
            return (
                False,
                f"Field {REGS[x]}, mask: 0x{m:X} mismatch: {state.get(x, 0) & m} vs {good_state.get(x, 0) & m}",
            )

    for n in ["descriptors", "ibos"]:
        if n not in good_state:
            continue
        mv1, mv2 = state.get(n), good_state.get(n)

        if len(mv1) != len(mv2):
            return False, f"{n}: len mismatch {len(mv1)} != {len(mv2)}"
        ...
        for i in range(len(mv2)):
            if i % 8 == 5 or i % 8 == 4:
                continue  # addresses
            if mv1[i] != mv2[i]:
                return False, f"{n}: content mismatch {i} {mv1[i]} {mv2[i]}"
    ...
    for n in ["samplers"]:
        ...
        if len(mv1) != len(mv2):
            return False, f"{n}: len mismatch {len(mv1)} != {len(mv2)}"
        if any(mv1[i] != mv2[i] for i in range(len(mv1))):
            return False, f"{n}: content mismatch"
