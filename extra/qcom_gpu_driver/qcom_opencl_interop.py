...
gdev = GPUDevice()
cl_buf = cl.clCreateBuffer(
    gdev.context, cl.CL_MEM_READ_WRITE, 0x100, None, status := ctypes.c_int32()
)
...
cl.clEnqueueWriteBuffer(
    gdev.queue, cl_buf, False, 0, 0x100, mv_address(data), 0, None, None
)
...
rawbuf_ptr = to_mv(cl_buf_desc_ptr, 0x100).cast("Q")[
    20
]  # offset 0xA0 is a raw gpu pointer.
...
for i in range(4):
    cl_buf = cl.clCreateBuffer(
        gdev.context, cl.CL_MEM_READ_WRITE, 2 * 2 * 4, None, status := ctypes.c_int32()
    )
    assert status.value == 0
    data = memoryview(array.array("I", [x + i for x in range(2 * 2)]))
    cl.clEnqueueWriteBuffer(
        gdev.queue, cl_buf, False, 0, 2 * 2 * 4, mv_address(data), 0, None, None
    )
    cl.clFinish(gdev.queue)  # wait writes to complete
    ...
    y = calc(
        x=Tensor.from_blob(rawbuf_ptr, (2, 2), dtype=dtypes.int, device="QCOM")
    ).numpy()
    print(f"jit {i}\n", y)
...
cl_img = cl.clCreateImage2D(
    ...
    cl.cl_image_format(cl.CL_RGBA, cl.CL_FLOAT),
    w,
    h,
    0,
    None,
...
x = Tensor.from_blob(
    rawbuf_ptr, (h * w * 4,), dtype=dtypes.imagef((h, w)), device="QCOM"
)
y = (x + 1).numpy()
print(y)
