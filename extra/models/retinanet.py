...
class RetinaNet:
  ...
  def postprocess_one(self, predictions_per_image:Tensor, image_size, orig_image_size, score_thresh, topk_candidates, nms_thresh,
                      anchors, split_idx):
    h, w = image_size
    ...
    if orig_image_size is not None:
      resized_x = image_boxes[:, 0::2] * orig_image_size[1] / w
      resized_y = image_boxes[:, 1::2] * orig_image_size[0] / h
  ...
  def postprocess_detections(self, predictions, input_size=(800, 800), image_sizes=None, orig_image_sizes=None, score_thresh=0.05, topk_candidates=1000, nms_thresh=0.5):
    ...
    for predictions_per_image, image_size, orig_image_size in zip(predictions, image_sizes, orig_image_sizes):
      detection = self.postprocess_one(predictions_per_image, image_size, orig_image_size, score_thresh, topk_candidates, nms_thresh, anchors, split_idx)
