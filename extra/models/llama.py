...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0) -> Tensor:
    ...
    return Tensor.stack(freqs.cos(), freqs.sin(), dim=-1).reshape(
        1, end, 1, dim // 2, 2
    )
...
class Attention:
    ...
    def __call__(
        ...
        keys = (
            self.cache_kv[0].shrink((None, (0, start_pos + seqlen), None, None))
            if start_pos > 0
            else xk
        )
        values = (
            self.cache_kv[1].shrink((None, (0, start_pos + seqlen), None, None))
            if start_pos > 0
            else xv
        )
    ...
    if af or ap:
        if not hasattr(sample, "alpha_counter"):
            setattr(
                sample,
                "alpha_counter",
                Tensor.zeros_like(logits, dtype=dtypes.int32).contiguous(),
            )
