...
class BertForPretraining:
    def __init__(
        ...
        self.cls = BertPreTrainingHeads(
            hidden_size, vocab_size, self.bert.embeddings.word_embeddings.weight
        )
    ...
    def sparse_categorical_crossentropy(
        self, predictions: Tensor, labels: Tensor, ignore_index=-1
    ):
        log_probs, loss_mask = predictions.log_softmax(dtype=dtypes.float), (
            labels != ignore_index
        )
        y_counter = (
            Tensor.arange(
                predictions.shape[-1], requires_grad=False, device=predictions.device
            )
            .unsqueeze(0)
            .expand(labels.numel(), predictions.shape[-1])
        )
        y = (
            (y_counter == labels.flatten().reshape(-1, 1)) * loss_mask.reshape(-1, 1)
        ).reshape(*labels.shape, predictions.shape[-1])
        return -((log_probs * y).sum()) / (
            loss_mask.sum() + 1e-5
    ...
    def loss(
        ...
        masked_lm_loss = self.sparse_categorical_crossentropy(
            prediction_logits, masked_lm_ids, ignore_index=masked_lm_weights
        )
        next_sentence_loss = seq_relationship_logits.binary_crossentropy_logits(
    ...
    def accuracy(
        ...
        return (
            masked_lm_correct.sum() / valid.sum(),
            seq_relationship_correct.mean(),
            masked_lm_loss,
            next_sentence_loss,
        )

    def load_from_pretrained(
        self, tf_weight_path: str = Path(__file__).parent.parent / "datasets" / "wiki"
    ):
        ...
        for k, v in state_dict.items():
            ...
            n = m[
                -1
            ]  # this is just to stop python from complaining about possibly unbound local variable
...
class BertPreTrainingHeads:
    def __init__(self, hidden_size: int, vocab_size: int, embeddings_weight: Tensor):
        self.predictions = BertLMPredictionHead(
            hidden_size, vocab_size, embeddings_weight
        )
        self.pooler = BertPooler(hidden_size)
        self.seq_relationship = Linear(hidden_size, 2)

    def __call__(self, sequence_output: Tensor, masked_lm_positions: Tensor):
        prediction_logits = self.predictions(
            gather(sequence_output, masked_lm_positions)
        )
...
def gather(prediction_logits: Tensor, masked_lm_positions: Tensor):
    counter = (
        ...
        .reshape(1, 1, prediction_logits.shape[1])
        .expand(*masked_lm_positions.shape, prediction_logits.shape[1])
    )
    onehot = counter == masked_lm_positions.unsqueeze(2).expand(
        *masked_lm_positions.shape, prediction_logits.shape[1]
    )
...
class BertEmbeddings:
    ...
    def __call__(self, input_ids, token_type_ids):
        ...
        position_ids = (
            Tensor.arange(seq_length, requires_grad=False, device=input_ids.device)
            .unsqueeze(0)
            .expand(*input_shape)
        )
