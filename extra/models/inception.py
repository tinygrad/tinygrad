...
class InceptionA:
    ...
    def __call__(self, x: Tensor) -> Tensor:
        outputs = [
            ...
            x.sequential(
                [self.branch3x3dbl_1, self.branch3x3dbl_2, self.branch3x3dbl_3]
            ),
...
class InceptionC:
    def __init__(self, in_ch, ch_7x7):
        ...
        self.branch7x7_2 = BasicConv2d(
            ch_7x7, ch_7x7, kernel_size=(1, 7), padding=(0, 3)
        )
        ...
        self.branch7x7dbl_2 = BasicConv2d(
            ch_7x7, ch_7x7, kernel_size=(7, 1), padding=(3, 0)
        )
        ...
        self.branch7x7dbl_5 = BasicConv2d(
            ch_7x7, 192, kernel_size=(1, 7), padding=(0, 3)
        )
...
class InceptionE:
    ...
    def __call__(self, x: Tensor) -> Tensor:
        ...
        outputs = [
            self.branch1x1(x),
            Tensor.cat(
                self.branch3x3_2a(branch3x3), self.branch3x3_2b(branch3x3), dim=1
            ),
            Tensor.cat(
...
class InceptionAux:
    ...
    def __call__(self, x: Tensor) -> Tensor:
        x = x.avg_pool2d(kernel_size=5, stride=3, padding=1).sequential(
    ...
    def __init__(self, num_classes: int = 1008, cls_map: Optional[Dict] = None):
        def get_cls(key1: str, key2: str, default):
            return (
                default
                if cls_map is None
                else cls_map.get(key1, cls_map.get(key2, default))
            )
        ...
        self.maxpool1 = lambda x: Tensor.max_pool2d(
            x, kernel_size=(3, 3), stride=2, padding=1
        )
        self.Conv2d_3b_1x1 = BasicConv2d(64, 80, kernel_size=1)
        self.Conv2d_4a_3x3 = BasicConv2d(80, 192, kernel_size=(3, 3))
        self.maxpool2 = lambda x: Tensor.max_pool2d(
            x, kernel_size=(3, 3), stride=2, padding=1
        )
...
class FidInceptionA(InceptionA):
    def __call__(self, x: Tensor) -> Tensor:
        outputs = [
            ...
            self.branch_pool(
                x.avg_pool2d(
                    kernel_size=(3, 3), stride=1, padding=1, count_include_pad=False
                )
            ),
        ]
        return Tensor.cat(*outputs, dim=1)
    ...
    def __call__(self, x: Tensor) -> Tensor:
        ...
        x = x.sequential(
            [
                ...
                self.Conv2d_2a_3x3,
                self.Conv2d_2b_3x3,
                lambda x: Tensor.max_pool2d(
                    x, kernel_size=(3, 3), stride=2, dilation=1
                ),
                self.Conv2d_3b_1x1,
                self.Conv2d_4a_3x3,
                lambda x: Tensor.max_pool2d(
