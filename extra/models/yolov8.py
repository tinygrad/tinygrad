...
class YOLOv8:
  def __init__(self, yolo_variant, num_classes): #width_multiple, ratio_multiple, depth_multiple
    d, w, r = get_variant_multiples(yolo_variant)
    self.net = Darknet(w, r, d)
    self.fpn = Yolov8NECK(w, r, d)
    self.head = DetectionHead(num_classes, filters=(int(256*w), int(512*w), int(512*w*r)))
    self.variant = yolo_variant
  ...
  def load_from_pretrained(self, ):
    url = f'https://gitlab.com/r3sist/yolov8_weights/-/raw/master/yolov8{self.variant}.safetensors'
    from extra.utils import fetch_as_file
    from tinygrad.nn.state import load_state_dict, safe_load
    state_dict = safe_load(fetch_as_file(url))
