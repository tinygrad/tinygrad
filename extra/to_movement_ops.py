...
def apply_mop(st: ShapeTracker, mop_arg: Tuple[MovementOps, Tuple]) -> ShapeTracker:
    ...
    if mop == MovementOps.RESHAPE:
        ...
        if arg == (-1,):
            return st.reshape((prod(st.views[-1].shape),))
        return st.reshape(arg)
    if mop == MovementOps.PERMUTE:
        return st.permute(arg)
    if mop == MovementOps.EXPAND:
        if len(arg) != len(st.shape):
            st = st.reshape((1, *st.shape))
        return st.expand(arg)
    if mop == MovementOps.PAD:
        return st.pad(arg)
    if mop == MovementOps.SHRINK:
        return st.shrink(arg)
    if mop == MovementOps.STRIDE:
        return st.stride(arg)
...
def to_movement_ops(st: ShapeTracker) -> List[Tuple[MovementOps, Tuple]]:
    ...
    for i, v in enumerate(st.views):
        ...
        offset = v.offset + sum(
            st * (s - 1) for s, st in zip(real_shape, v.strides) if st < 0
        )
        real_offset = offset + (
            sum(x * st for (x, _), st in zip(v.mask, v.strides)) if v.mask else 0
        )
        real_real_shape = [s for s, st in zip(real_shape, v.strides) if st]
        strides: List[Node | int] = [
            abs(st) if isinstance(st, int) else st for st in v.strides if st
        ]
        ...
        def sort_by_strides(shape, strides):
            return sorted(
                zip(shape, strides), key=lambda k: (k[1], -k[0]), reverse=True
            ), sorted(
                range(len(strides)),
                key=lambda k: (strides[k], -real_real_shape[k]),
                reverse=True,
            )
        ...
        to_apply.extend(
            [
                (MovementOps.RESHAPE, (-1,)),
                (MovementOps.SHRINK, ((real_offset, real_offset + buffer_size),)),
            ]
        )
        if strides:
            if (
                ordered_shape_strides[0][0] * ordered_shape_strides[0][1]
            ) - buffer_size > 0:
                to_apply.append(
                    (
                        MovementOps.PAD,
                        (
                            (
                                0,
                                (
                                    ordered_shape_strides[0][0]
                                    * ordered_shape_strides[0][1]
                                )
                                - buffer_size,
                            ),
                        ),
            ...
            for i, shape_stride in enumerate(ordered_shape_strides):
                if (
                    ...
                    and shape_stride[1]
                    < ordered_shape_strides[i + 1][0] * ordered_shape_strides[i + 1][1]
                ):
                    remaining_buffer = (
                        ordered_shape_strides[i - 1][1] if i > 0 else buffer_size
                    )
                    to_apply.append(
                        (
                            MovementOps.EXPAND,
                            (
                                shape_stride[0],
                                *(s[0] for s in ordered_shape_strides[:i]),
                                remaining_buffer,
                            ),
                        )
                    )
                    ...
                    to_apply.append(
                        (
                            ...
                            (
                                *(s[0] for s in ordered_shape_strides[:i]),
                                shape_stride[0] * remaining_buffer,
                            ),
                        )
                    )
                    to_apply.append(
                        (
                            ...
                            (
                                *((0, 0) for _ in range(i)),
                                (0, shape_stride[0] * shape_stride[1]),
                            ),
                        )
                    )
                    to_apply.append(
                        (
                            MovementOps.RESHAPE,
                            (
                                *(s[0] for s in ordered_shape_strides[: i + 1]),
                                remaining_buffer + shape_stride[1],
                            ),
                        )
                    )
                    ordered_shape_strides[i] = (
                        ordered_shape_strides[i][0],
                        remaining_buffer + shape_stride[1],
                    )
                else:
                    to_apply.append(
                        (
                            MovementOps.SHRINK,
                            (
                                *((0, s[0]) for s in ordered_shape_strides[:i]),
                                (0, shape_stride[0] * shape_stride[1]),
                            ),
                        )
                    )
            ...
            to_apply.extend(
                [
                    (
                        MovementOps.SHRINK,
                        (*[(0, s[0]) for s in ordered_shape_strides], (0, 1)),
                    ),
            ...
            if order != list(range(len(order))):
                to_apply.append(
                    (
                        MovementOps.PERMUTE,
                        tuple(order.index(i) for i in range(len(strides))),
                    )
                )
        to_apply.append(
            (
                MovementOps.RESHAPE,
                tuple(s if st else 1 for s, st in zip(real_shape, v.strides)),
            )
        )
        if any(i < 0 for i in v.strides):
            to_apply.append(
                (MovementOps.STRIDE, tuple(-1 if st < 0 else 1 for st in v.strides))
            )
...
def get_real_view(shape, strides, offset, mask):
    ...
    real_offset = offset + (
        sum(x * st for (x, _), st in zip(mask, strides)) if mask else 0
    )
...
def get_buffer_size(shape, strides, offset, mask):
    ...
    return (
        real_offset + sum((s - 1) * st for s, st in zip(real_real_shape, strides)) + 1
    )
...
def st_equivalent(st1: ShapeTracker, st2: ShapeTracker):
    ...
    for i, ranges in enumerate(
        itertools.product(*[range(v.min, v.max + 1) for v in vs])
    ):
        if i > 1000:
...
def test_rebuild(st: ShapeTracker):
    ...
    c[len(mops)] += 1
    for mop_arg in mops:
        rebuilt_st = apply_mop(rebuilt_st, mop_arg)
    rebuilt_st = rebuilt_st.simplify()
    # why is the "all(x == 0 for x in rebuilt_st.views[-1].strides)" hack needed?
    assert st_equivalent(st, rebuilt_st) or all(
        x == 0 for x in rebuilt_st.views[-1].strides
    ), f"mismatch {st} {rebuilt_st}"
    last_v1 = st.views[-1]
