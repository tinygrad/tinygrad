...
def compile_net(run: TinyJit, special_names: Dict[int, str]) -> Tuple[
    Dict[str, str],
    ...
    for ji in run.jit_cache:
        ...
        functions[fxn.function_name] = (
            fxn.src
        )  # NOTE: this assumes all with the same name are the same
        cargs = []
...
def export_model_webgpu(
    functions, statements, bufs, weight_names, input_names, output_names, model_name
) -> Tuple[str, int, int]:
    ...
    create_bind_group_layouts = ",".join(
        [
            "device.createBindGroupLayout({{entries: [{{binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: {{ type: 'uniform' }}}}, {}]}})".format(
                ",".join(
                    [
                        f"{{binding: {argIdx+1}, visibility: GPUShaderStage.COMPUTE, buffer: {{ type: 'storage' }} }}"
    ...
    kernel_calls = "\n        ".join(
        [
            f"addComputePass(device, commandEncoder, pipelines[{i}], layouts[{i}], infinityBuf, [{', '.join(args)}], {global_size});"
    ...
    input_buffer_types = [
        dtype_to_js_type(bufs[inp_name][1]) for inp_name in input_names
    ]
    output_buffer_types = [
        dtype_to_js_type(bufs[out_name][1]) for out_name in output_names
    ]
    input_writers = "\n    ".join(
        [
            f"await gpuWriteBuffer{i}.mapAsync(GPUMapMode.WRITE);\n        new {input_buffer_types[i]}(gpuWriteBuffer{i}.getMappedRange()).set("
            + f"_{inp_name});"
