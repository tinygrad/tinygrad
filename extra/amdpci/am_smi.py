...
def color_temp(temp):
    if temp >= 87:
        return colored(f"{temp:>3}", "red")
    elif temp >= 80:
        return colored(f"{temp:>3}", "yellow")
    return f"{temp:>3}"


def color_voltage(voltage):
    return colored(f"{voltage/1000:>5.3f}V", "cyan")
    ...
    for i in range(max_height):
        ...
        for bid, block in enumerate(strs):
            if i < len(block):
                line.append(
                    block[i]
                    + " " * (split + max_width_in_block[bid] - ansilen(block[i]))
                )
            else:
                line.append(" " * (split + max_width_in_block[bid]))
...
def get_bar0_size(pcibus):
    ...
    if len(bar0_info) < 3:
        raise ValueError("Unexpected resource file format for BAR0.")
...
class AMSMI(AMDev):
    ...
    def _init_from_d0(self):
        ...
        if self.reg("regSCRATCH_REG7").read() != AM_VERSION:
            raise Exception(
                f"Unsupported AM version: {self.reg('regSCRATCH_REG7').read():x}"
            )
    ...
    def read_pci_state(self):
        with open(f"/sys/bus/pci/devices/{self.pcibus}/power_state", "r") as f:
    ...
    def __init__(self):
        ...
        for k, v in self.lspci.items():
            for part in remove_parts:
                self.lspci[k] = self.lspci[k].replace(part, "").strip().rstrip()

    def _open_am_device(self, pcibus):
        if pcibus not in self.opened_pci_resources:
            bar_fds = {
                bar: os.open(
                    f"/sys/bus/pci/devices/{pcibus}/resource{bar}",
                    os.O_RDWR | os.O_SYNC,
                )
                for bar in [0, 2, 5]
            }
            bar_size = {
                0: get_bar0_size(pcibus),
                2: os.fstat(bar_fds[2]).st_size,
                5: os.fstat(bar_fds[5]).st_size,
            }
            ...
            self.opened_pci_resources[pcibus] = (
                map_pci_range(0),
                None,
                map_pci_range(5).cast("I"),
            )
        ...
        try:
            ...
            if DEBUG >= 2:
                print(f"Failed to open AM device {pcibus}: {e}")
            return

        self.opened_pcidevs.append(pcibus)
        if DEBUG >= 2:
            print(f"Opened AM device {pcibus}")
    ...
    def rescan_devs(self):
        ...
        for d in self.devs:
            ...
            if d.pci_state == "D0" and d.reg("regSCRATCH_REG7").read() != AM_VERSION:
                ...
                os.system("clear")
                if DEBUG >= 2:
                    print(f"Removed AM device {d.pcibus}")

    def collect(self):
        return {
            d: d.smu.read_metrics() if d.pci_state == "D0" else None for d in self.devs
        }
    ...
    def draw(self):
        ...
        for dev, metrics in dev_metrics.items():
            if dev.pci_state != "D0":
                dev_content.append(
                    [
                        f"{colored('(sleep)', 'yellow')} {bold(dev.pcibus)}: {self.lspci[dev.pcibus[5:]]}"
                    ]
                    + [f"PCI State: {dev.pci_state}"]
                    + [" " * 107]
                )
                continue
            ...
            activity_line = (
                [
                    f"GFX Activity {draw_bar(metrics.SmuMetrics.AverageGfxActivity / 100, 50)}"
                ]
                + [
            ...
            temps_keys = [
                (k, name)
                for k, name in smu_v13_0_0.c__EA_TEMP_e__enumvalues.items()
                if k < smu_v13_0_0.TEMP_COUNT
                and metrics.SmuMetrics.AvgTemperature[k] != 0
            ]
            temps_table = ["=== Temps (C) ==="] + [
                f"{name:<15}: {color_temp(metrics.SmuMetrics.AvgTemperature[k])}"
                for k, name in temps_keys
            ]

            voltage_keys = [
                (k, name)
                for k, name in smu_v13_0_0.c__EA_SVI_PLANE_e__enumvalues.items()
                if k < smu_v13_0_0.SVI_PLANE_COUNT
            ]
            power_table = (
                ["=== Power ==="]
                ...
                + [
                    ...
                    + draw_bar(
                        metrics.SmuMetrics.AverageSocketPower
                        / metrics.SmuMetrics.dGPU_W_MAX,
                        16,
                    )
                ]
                + ["", "=== Voltages ==="]
                + [
                    f"{name:<20}: {color_voltage(metrics.SmuMetrics.AvgVoltage[k])}"
                    for k, name in voltage_keys
                ]
            )
            ...
            dev_content.append(
                device_line
                + activity_line
                + same_line([temps_table, power_table, frequency_table])
            )

        raw_text = (
            "AM Monitor".center(terminal_width) + "\n" + "=" * terminal_width + "\n\n"
        )
        for i in range(0, len(dev_content), 2):
            if i + 1 < len(dev_content):
                raw_text += "\n".join(same_line([dev_content[i], dev_content[i + 1]]))
            else:
                raw_text += "\n".join(dev_content[i])
            if i + 2 < len(dev_content):
                raw_text += "\n" + "=" * terminal_width + "\n\n"
