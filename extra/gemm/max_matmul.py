...
from tinygrad.ops import UOp, UOps, BinaryOps, UnaryOps, TernaryOps, KernelInfo
...
if __name__ == "__main__":
  ...
  if getenv("CUDA") == 1:
    ...
    if GEMM_VARIATION == "max" and (M%64)==0 and (N%128)==0 and (K%64)==0 and DTYPE_IN == dtypes.half and DTYPE_OUT == dtypes.float and DTYPE_ACC == dtypes.float:
      ...
      prog = CUDAProgram(device, "wmma_example", compiler.compile(open(os.path.join(script_dir, 'max_kernels/nv.max.cu')).read()))
      args = (c, a, b)
      kwargs = {
      ...
      prog = CUDAProgram(device, "wmma_example", compiler.compile(open(os.path.join(script_dir, 'max_kernels/nv.hcopt.cu')).read()))
      args = (c, a, b)
      kwargs = {
        'global_size': [32, 64, 1],
