...
def block_128x128_gemm(a:UOp, b:UOp) -> UOp:
  ...
  barrier = UOp.barrier(A_local.store(a[k_tile_range]), B_local.store(b[k_tile_range]))
  ...
  c_regs = c_regs.rearrange("(wpb_m lpw_m wpb_n lpw_n) rt_m t_m rt_n t_n -> (wpb_m rt_m lpw_m t_m) (wpb_n rt_n lpw_n t_n)",
                            lpw_m=LANES_PER_WAVE_M, lpw_n=LANES_PER_WAVE_N)
  ...
  A_col = A_col.rearrange("(wpb lpw) rt t -> (wpb rt lpw t)", lpw=LANES_PER_WAVE_M)
  B_row = B_row.rearrange("(wpb lpw) rt t -> (wpb rt lpw t)", lpw=LANES_PER_WAVE_N)
