...
dtype_in = (
    dtypes.half
    if getenv("HALF")
    else dtypes.bfloat16 if getenv("BFLOAT16") else dtypes.float
)
acc_dtype = (
    dtypes.half
    if getenv("ACC_HALF")
    else dtypes.bfloat16 if getenv("ACC_BFLOAT16") else None
)
...
def rand_input():
    return (
        Tensor.rand(BS, CIN, HW, HW, dtype=dtype_in).realize(),
        Tensor.rand(COUT, CIN, K, K, dtype=dtype_in).realize(),
    )
...
if __name__ == "__main__":
    ...
    if COMP:
        ...
        ta, tb = torch.from_numpy(a.numpy()).to(torch_device), torch.from_numpy(
            b.numpy()
        ).to(torch_device)
