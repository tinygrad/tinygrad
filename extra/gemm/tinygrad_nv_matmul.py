...
if __name__ == "__main__":
    if getenv("GEMV"):
        A, B = (
            Tensor.empty(1, N, dtype=dtypes.float),
            Tensor.empty(14336, N, dtype=dtypes.float16).T,
        )
    else:
        A, B = Tensor.empty(N, N, dtype=dtypes.float16), Tensor.empty(
            N, N, dtype=dtypes.float16
        )
    C = A.matmul(B)
    si = C.schedule()[-1]
    ast = si.ast
