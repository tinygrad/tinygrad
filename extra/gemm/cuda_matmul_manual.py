...
extern "C" __global__ void simple_matmul(half *a, half *b, float *c) {{
  ...
  for (int i = 0; i < {N}; i += {WMMA_K}) {{
    ...
    // the reason this is weird is because in a m16n16k16 wmma, nvidia's fragments arrange data into 4 quadrants like so:
    ...
    //  [1 1 1 1 1 1 1 1 3 3 3 3 3 3 3 3]
    //  [1 1 1 1 1 1 1 1 3 3 3 3 3 3 3 3]]
    // Where every thread in the warp is responsible for two values in each quadrant.
    // This can be seen below, where each value contains the thread ID ("t") responsible for it:
