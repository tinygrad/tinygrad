...
def custom_gemm_bw(gradient:UOp, kernel:UOp):
  ...
  if len(inputs) == 5:
    ...
    # wgrad: uses g_scale * x_scale
    grad_b = asm_gemm(g_fp8.permute(2, 0, 1).reshape(g_t.shape[-1], -1), a_t.reshape(-1, a_t.shape[-1]), x_scale=g_scale * s_x_t, w_scale=Tensor(1.0, dtype=dtypes.float, device=a.device))
...
def asm_gemm(a:Tensor, b:Tensor, x_scale:Tensor|None=None, w_scale:Tensor|None=None) -> Tensor:
  ...
  if arch.startswith("gfx950") and getenv("USE_ASM", 1):
    ...
    if a.dtype == FP8_DTYPE:
      xs = x_scale if x_scale is not None else Tensor(1.0, dtype=dtypes.float, device=a.device)
      ws = w_scale if w_scale is not None else Tensor(1.0, dtype=dtypes.float, device=a.device)
