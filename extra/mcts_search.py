...
from tinygrad.engine.search import (
    _ensure_buffer_alloc,
    get_kernel_actions,
    _time_program,
)
...
def expand_node(node: MCTSNode):
    ...
    node.children = [
        MCTSNode(x, node)
        for x in get_kernel_actions(node.kernel, include_0=False).values()
    ]
...
def _sample_tree(node: MCTSNode, best_tm: float) -> MCTSNode:
    ...
    # safe softmax
    ucb_exp = np.exp(
        (np.array(ucb_explored_children) - max(ucb_explored_children)) / TEMP
    )
    return _sample_tree(
...
def mcts_search(lin: Kernel, rawbufs: List[Buffer], amt: int) -> Kernel:
    ...
    if (
        not getenv("IGNORE_MCTS_CACHE")
        and CACHELEVEL >= 1
        and (val := diskcache_get("mcts_search", key)) is not None
    ):
        ret = lin.copy()
    ...
    for i in range(amt):
        ...
        if DEBUG >= 2:
            print(
                f"\r{et:7.2f}s {colored(f'{compile_time*100/et:3.0f}%', 'cyan')} {colored(f'{runtime_time*100/et:3.0f}%', 'red')}: {tm:12.2f} us     best: {best_tm:12.2f} us @ {best_idx+1:4d}      {i+1:4d}/{amt:4d}  {int(round((i+1)/et)):4d}/s     {node.kernel.colored_shape()}\033[K",
                end="",
            )  # noqa: E501
    ...
    if getenv("MCTSGRAPH"):
        ...
        def add_node(node: MCTSNode):
            ...
            edge_lbl = (
                f"{str(gopts[-1].op)[7:]} {gopts[-1].axis} {gopts[-1].arg}"
                if len(gopts)
                else "ROOT"
            )
            G.add_node(
                node,
                label=f"{node.i+1}\n{node.tm:.2f} us\n{edge_lbl}\nt {node.t:.2f}\nn {node.n}",
                fillcolor="#80ff8080" if node.tm == best_tm else "#ffff8080",
                style="filled" if node.t == best_tm else "",
            )
