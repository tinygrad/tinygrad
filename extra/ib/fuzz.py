...
def send(name, tensor: Tensor):
  tensor.contiguous().to(f"{ibd}/{name}").realize()

def receive(name, shape, d=Device.DEFAULT):
  return Tensor.empty(shape, device=f"{ibd}/{name}").contiguous().to(d).realize()
