...
if __name__ == "__main__":
  ...
  with Timing("weights -> model: "):
    nn.state.load_state_dict(model, {k: v.to("CPU") for k, v in convert_from_huggingface(part1, model, 32, 8).items()}, strict=False)
    nn.state.load_state_dict(model, {k: v.to("CPU") for k, v in convert_from_huggingface(part2, model, 32, 8).items()}, strict=False)
