...
class Synthesizer:
  def __init__(
    ...
    **kwargs,
    ...
    self.dp = (
      ...
      if use_sdp
      else DurationPredictor(hidden_channels, 256, 3, 0.5, gin_channels=gin_channels)
    )
    if n_speakers > 1:
  ...
  def infer(
    ...
    x, m_p, logs_p, x_mask = self.enc_p.forward(
      x.realize(), x_lengths.realize(), emotion_embedding.realize() if emotion_embedding is not None else emotion_embedding
    )
    ...
    if max_y_length_estimate_scale is not None:
      ...
      if real_max_y_length < max_y_length:
        ...
        if overestimation > 10:
          logging.warning("Warning: max length overestimated by more than 10%")
...
class StochasticDurationPredictor:
  def __init__(self, in_channels, filter_channels, kernel_size, p_dropout, n_flows=4, gin_channels=0):
    ...
    self.in_channels, self.filter_channels, self.kernel_size, self.p_dropout, self.n_flows, self.gin_channels = (
      in_channels,
      filter_channels,
      kernel_size,
      p_dropout,
...
class DurationPredictor:
  def __init__(self, in_channels, filter_channels, kernel_size, p_dropout, gin_channels=0):
    self.in_channels, self.filter_channels, self.kernel_size, self.p_dropout, self.gin_channels = (
      in_channels,
      filter_channels,
      kernel_size,
      p_dropout,
...
class TextEncoder:
  def __init__(self, n_vocab, out_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, emotion_embedding):
    self.n_vocab, self.out_channels, self.hidden_channels, self.filter_channels, self.n_heads, self.n_layers, self.kernel_size, self.p_dropout = (
      n_vocab,
      out_channels,
      hidden_channels,
      filter_channels,
...
class ResidualCouplingBlock:
  def __init__(self, channels, hidden_channels, kernel_size, dilation_rate, n_layers, n_flows=4, gin_channels=0):
    self.channels, self.hidden_channels, self.kernel_size, self.dilation_rate, self.n_layers, self.n_flows, self.gin_channels = (
      channels,
      hidden_channels,
      kernel_size,
      dilation_rate,
...
class PosteriorEncoder:
  def __init__(self, in_channels, out_channels, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=0):
    self.in_channels, self.out_channels, self.hidden_channels, self.kernel_size, self.dilation_rate, self.n_layers, self.gin_channels = (
      in_channels,
      out_channels,
      hidden_channels,
      kernel_size,
...
class WN:
  def __init__(self, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=0, p_dropout=0):
    ...
    self.hidden_channels, self.kernel_size, self.dilation_rate, self.n_layers, self.gin_channels, self.p_dropout = (
      hidden_channels,
      kernel_size,
      dilation_rate,
      n_layers,
    ...
    for i in range(n_layers):
      ...
      self.in_layers.append(
        nn.Conv1d(hidden_channels, 2 * hidden_channels, kernel_size, dilation=dilation, padding=int((kernel_size * dilation - dilation) / 2))
      )
...
class ResBlock1:
  def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5)):
    self.convs1 = [
      nn.Conv1d(channels, channels, kernel_size, 1, dilation=dilation[i], padding=get_padding(kernel_size, dilation[i])) for i in range(3)
    ]
...
class ResBlock2:
  def __init__(self, channels, kernel_size=3, dilation=(1, 3)):
    self.convs = [
      nn.Conv1d(channels, channels, kernel_size, 1, dilation=dilation[i], padding=get_padding(kernel_size, dilation[i])) for i in range(2)
    ]
...
class ConvFlow:
  def __init__(self, in_channels, filter_channels, kernel_size, n_layers, num_bins=10, tail_bound=5.0):
    self.in_channels, self.filter_channels, self.kernel_size, self.n_layers, self.num_bins, self.tail_bound = (
      in_channels,
      filter_channels,
      kernel_size,
      n_layers,
  ...
  def forward(self, x, x_mask, g=None, reverse=False):
    ...
    x1, log_abs_det = piecewise_rational_quadratic_transform(
      x1, un_normalized_widths, un_normalized_heights, un_normalized_derivatives, inverse=reverse, tails="linear", tail_bound=self.tail_bound
    )
...
class ResidualCouplingLayer:
  def __init__(self, channels, hidden_channels, kernel_size, dilation_rate, n_layers, p_dropout=0, gin_channels=0, mean_only=False):
    ...
    self.channels, self.hidden_channels, self.kernel_size, self.dilation_rate, self.n_layers, self.mean_only = (
      channels,
      hidden_channels,
      kernel_size,
      dilation_rate,
...
class MultiHeadAttention:
  def __init__(
    ...
    if window_size is not None:
      self.emb_rel_k, self.emb_rel_v = [
        Tensor.randn(1 if heads_share else n_heads, window_size * 2 + 1, self.k_channels) * (self.k_channels**-0.5) for _ in range(2)
      ]
  ...
  def _matmul_with_relative_values(self, x, y):
    return x.matmul(y.unsqueeze(0))  # x: [b, h, l, m], y: [h or 1, m, d], ret: [b, h, l, d]

  def _matmul_with_relative_keys(self, x, y):
    return x.matmul(y.unsqueeze(0).transpose(-2, -1))  # x: [b, h, l, d], y: [h or 1, m, d], re, : [b, h, l, m]
  ...
  def _get_relative_embeddings(self, relative_embeddings, length):
    ...
    padded_relative_embeddings = (
      relative_embeddings if pad_length <= 0 else relative_embeddings.pad(convert_pad_shape([[0, 0], [pad_length, pad_length], [0, 0]]))
    )
...
class FFN:
  def __init__(self, in_channels, out_channels, filter_channels, kernel_size, p_dropout=0.0, activation=None, causal=False):
    self.in_channels, self.out_channels, self.filter_channels, self.kernel_size, self.p_dropout, self.activation, self.causal = (
      in_channels,
      out_channels,
      filter_channels,
      kernel_size,
  ...
  def _causal_padding(self, x):
    return x if self.kernel_size == 1 else x.pad(convert_pad_shape([[0, 0], [0, 0], [self.kernel_size - 1, 0]]))

  def _same_padding(self, x):
    return x if self.kernel_size == 1 else x.pad(convert_pad_shape([[0, 0], [0, 0], [(self.kernel_size - 1) // 2, self.kernel_size // 2]]))


class Encoder:
  def __init__(self, hidden_channels, filter_channels, n_heads, n_layers, kernel_size=1, p_dropout=0.0, window_size=4, **kwargs):
    self.hidden_channels, self.filter_channels, self.n_heads, self.n_layers, self.kernel_size, self.p_dropout, self.window_size = (
      hidden_channels,
      filter_channels,
      n_heads,
      n_layers,
...
def piecewise_rational_quadratic_transform(
  ...
  if tails is None:
    ...
    spline_fn, spline_kwargs = unconstrained_rational_quadratic_spline, {"tails": tails, "tail_bound": tail_bound}
  return spline_fn(
    inputs=inputs,
...
def unconstrained_rational_quadratic_spline(
  ...
  output, log_abs_det = rational_quadratic_spline(
    ...
    unnormalized_derivatives=un_normalized_derivatives.squeeze(dim=0).squeeze(dim=0),
    inverse=inverse,
    left=-tail_bound,
    right=tail_bound,
    bottom=-tail_bound,
...
def rational_quadratic_spline(
  ...
  if inverse:
    a = (inputs - input_cum_heights) * (input_derivatives + input_derivatives_plus_one - 2 * input_delta) + input_heights * (
    ...
    derivative_numerator = input_delta.square() * (
      input_derivatives_plus_one * root.square() + 2 * input_delta * theta_one_minus_theta + input_derivatives * (1 - root).square()
    )
...
def sequence_mask(length: Tensor, max_length):
  return Tensor.arange(max_length, dtype=length.dtype, device=length.device).unsqueeze(0) < length.unsqueeze(1)
...
def cat_lr(t, left, right):
  return Tensor.full(get_shape(t), left).cat(t, dim=-1).cat(Tensor.full(get_shape(t), right), dim=-1)
  ...
  return (
    (
      (
        ...
        == Tensor.arange(x.shape[-1]).reshape(*reshape_arg).expand(*indices.shape, x.shape[-1])
      )
      * x
    )
    .sum(indices.ndim)
...
def load_model(symbols, hps, model) -> Synthesizer:
  net_g = Synthesizer(
    len(symbols), hps.data.filter_length // 2 + 1, hps.train.segment_size // hps.data.hop_length, n_speakers=hps.data.n_speakers, **hps.model
  )
...
class TextMapper:  # Based on https://github.com/keithito/tacotron
  ...
  def phonemize(self, text, strip=True):
    return _phonemize(self.phonemizer, text, default_separator, strip, 1, False, False)

  def filter_oov(self, text):
    return "".join(list(filter(lambda x: x in self._symbol_to_id, text)))

  def base_english_cleaners(self, text):
    return self.collapse_whitespace(self.phonemize(self.expand_abbreviations(unidecode(text.lower()))))
  ...
  def cjke_cleaners(self, text):
    return re.sub(
      r"([^\.,!\?\-…~])$",
      r"\1.",
      re.sub(r"\s+$", "", self.english_to_ipa2(text).replace("ɑ", "a").replace("ɔ", "o").replace("ɛ", "e").replace("ɪ", "i").replace("ʊ", "u")),
    )

  def cjke_cleaners2(self, text):
    return re.sub(r"([^\.,!\?\-…~])$", r"\1.", re.sub(r"\s+$", "", self.english_to_ipa2(text)))

  def cjks_cleaners(self, text):
    return re.sub(r"([^\.,!\?\-…~])$", r"\1.", re.sub(r"\s+$", "", self.english_to_lazy_ipa(text)))
  ...
  def expand_abbreviations(self, text):
    return reduce(lambda t, abbr: re.sub(abbr[0], abbr[1], t), self._abbreviations, text)
...
if __name__ == "__main__":
  ...
  if args.emotion_path is not None:
    if args.emotion_path.endswith(".npy"):
      emotion_embedding = Tensor(np.load(args.emotion_path), dtype=dtypes.int64).unsqueeze(0)
    else:
  ...
  audio_tensor = net_g.infer(
    ...
    max_y_length_estimate_scale=Y_LENGTH_ESTIMATE_SCALARS[args.model_to_use] if args.estimate_max_y_length else None,
  )[0, 0].realize()
  ...
  out_path = Path(
    args.out_path
    or Path(args.out_dir) / f"{args.model_to_use}{f'_sid_{args.speaker_id}' if model_has_multiple_speakers else ''}_{args.base_name}.wav"
  )
