...
class StableDiffusionV2:
    def __init__(
        self, unet_config: Dict, cond_stage_config: Dict, parameterization: str = "v"
    ):
    ...
    def denoise(self, x: Tensor, sigma: Tensor, cond: Dict) -> Tensor:
        ...
        return run(
            ...
            *prep(x * c_in, c_noise, cond["crossattn"], c_out, x * c_skip),
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run Stable Diffusion v2.X",
    ...
    parser.add_argument(
        "--height", type=int, default=768, help="The output image height"
    )
    parser.add_argument("--weights-fn", type=str, help="Filename of weights to use")
    parser.add_argument(
        "--weights-url", type=str, help="Custom URL to download weights from"
    )
