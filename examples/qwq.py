...
def download_weights(total_num_weights: int) -> Path:
    model = fetch(
        "https://huggingface.co/Qwen/QwQ-32B-Preview/resolve/main/model.safetensors.index.json?download=true",
        "model.safetensors.index.json",
        subdir=(subdir := "qwq_32b_preview"),
    )

    for i in range(1, total_num_weights + 1):
        filename = f"model-{i:05d}-of-{total_num_weights:05d}.safetensors"
        fetch(
            f"https://huggingface.co/Qwen/QwQ-32B-Preview/resolve/main/{filename}?download=true",
            filename,
            subdir=subdir,
        )
...
def load_model(
    ...
    for layer in model.layers:
        ...
        layer.attention.wq = nn.Linear(
            model_params["dim"], model_params["n_heads"] * head_dim, bias=True
        )
        layer.attention.wk = nn.Linear(
            model_params["dim"], model_params["n_kv_heads"] * head_dim, bias=True
        )
        layer.attention.wv = nn.Linear(
            model_params["dim"], model_params["n_kv_heads"] * head_dim, bias=True
        )
    ...
    weights = fix_bf16(
        convert_from_huggingface(
            load(str(model_path / "model.safetensors.index.json")),
            model,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run QwQ in tinygrad",
    ...
    parser.add_argument(
        "--prompt", type=str, default="Hello.", help="Phrase to start with"
    )
    parser.add_argument(
        "--weights", type=str, default=None, help="Path to the downloaded weights"
    )
    ...
    model_path = (
        Path(args.weights)
        if args.weights
        else download_weights(model_info["total_num_weights"])
    )
    transformer = load_model(model_path, model_info["model_params"])
    tokenizer = AutoTokenizer.from_pretrained(model_info["tokenizer"])
    param_bytes = sum(
        x.lazydata.size * x.dtype.itemsize for x in get_parameters(transformer)
    )
    ...
    for i in range(args.count):
        ...
        next_tok = (
            ...
            if tok_tensor is None or (len(toks) - start_pos) > 1
            else tok_tensor.reshape(1, 1)
        )
        with Timing(
            "total ",
            enabled=args.timing,
    ...
    if args.temperature == 0:
        ...
        try:
            assert (
                text == expected[key]
            ), f"invalid output: `{colored(text, 'red')}` != `{expected[key]}`"
