...
class Slicer:  # from https://github.com/svc-develop-team/so-vits-svc/
  def __init__(
    self, sr: int, threshold: float = -40.0, min_length: int = 5000, min_interval: int = 300, hop_size: int = 20, max_sil_kept: int = 5000
  ):
  ...
  def slice(self, waveform):
    ...
    if len(sil_tags) == 0:
      return {"0": {"slice": False, "split_time": f"0,{len(waveform)}"}}  # Apply and return slices.
    chunks = []
    if sil_tags[0][0]:
...
class Resample:
  ...
  def _apply_sinc_resample_kernel(self, waveform: Tensor):
    if not waveform.is_floating_point():
      raise TypeError(f"Waveform tensor expected to be of type float, but received {waveform.dtype}.")
...
def sinc_interp_resample(
  x: Tensor, orig_freq: int = 16000, new_freq: int = 1600, lowpass_filter_width: int = 6, rolloff: float = 0.99, beta: Optional[float] = None
):
