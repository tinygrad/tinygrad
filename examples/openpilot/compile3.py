...
def compile(onnx_file):
  ...
  run_onnx_jit = TinyJit(
    lambda **kwargs: next(iter(run_onnx({k: v.to(Device.DEFAULT) for k, v in kwargs.items()}).values())).cast("float32"), prune=True
  )
  for i in range(3):
...
if __name__ == "__main__":
  ...
  new_inputs = {
    ...
    for nm, (st, _, dtype, _) in sorted(zip(pickle_loaded.captured.expected_names, pickle_loaded.captured.expected_st_vars_dtype_device))
  }
