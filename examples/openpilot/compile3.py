...
def compile(onnx_file):
    ...
    input_types = {
        k: (np.float32 if v == np.float16 else v) for k, v in input_types.items()
    }
    ...
    if (allowed_kernel_count := getenv("ALLOWED_KERNEL_COUNT", -1)) != -1:
        ...
        ), f"too many kernels! {kernel_count=}, {allowed_kernel_count=}"
...
def test_vs_compile(run, new_inputs, test_val=None):
    ...
    inputs = {
        **{k: v for k, v in new_inputs.items() if "img" in k},
        **{
            k: Tensor(v, device="NPY").realize()
            for k, v in new_inputs_numpy.items()
            if "img" not in k
    ...
    for v in new_inputs_numpy.values():
        v *= 2
    out = run(**inputs)
...
def test_vs_onnx(new_inputs, test_val, onnx_file, ort=False):
    ...
    if ort:
        ...
        for _ in range(1 if test_val is not None else 5):
            ...
            onnx_output = onnx_session.run(
                ...
                {k: v.astype(np.float16) for k, v in new_inputs_numpy.items()},
            )
    ...
    if test_val is not None:
        np.testing.assert_allclose(
            new_torch_out.reshape(test_val.shape), test_val, atol=1e-4, rtol=1e-2
        )
...
if __name__ == "__main__":
    ...
    Tensor.manual_seed(100)
    new_inputs = {
        nm: Tensor.randn(*st.shape, dtype=dtype).mul(8).realize()
        for nm, (st, _, dtype, _) in sorted(
            zip(
                pickle_loaded.captured.expected_names,
                pickle_loaded.captured.expected_st_vars_dtype_device,
            )
        )
    }
    ...
    if not getenv("FLOAT16"):
        test_vs_onnx(new_inputs, test_val, onnx_file, getenv("ORT"))
