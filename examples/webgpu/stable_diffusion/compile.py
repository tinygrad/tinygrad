...
def split_safetensor(fn):
    ...
    for i, end_pos in enumerate(part_end_offsets):
        with open(
            os.path.join(os.path.dirname(__file__), f"./net_part{i}.safetensors"), "wb+"
        ) as f:
            f.write(net_bytes[cur_pos:end_pos])
            cur_pos = end_pos

    with open(
        os.path.join(os.path.dirname(__file__), f"./net_textmodel.safetensors"), "wb+"
    ) as f:
...
if __name__ == "__main__":
    fetch_dep(
        os.path.join(os.path.dirname(__file__), "clip_tokenizer.js"),
        "https://huggingface.co/wpmed/tinygrad-sd-f16/raw/main/clip_tokenizer.js",
    )
    fetch_dep(
    ...
    sub_steps = [
        ...
        Step(
            name="f16tof32",
            input=[Tensor.randn(2097120, dtype=dtypes.uint32)],
            forward=u32_to_f16,
        ),
    ]
    ...
    def compile_step(model, step: Step):
        ...
        kernel_code = "\n\n".join(
            [
                f"const {key} = `{fixup_code(code, key)}`;"
    ...
    for step in sub_steps:
        ...
        if step.name == "diffusor":
            if args.remoteweights:
                base_url = (
                    "https://huggingface.co/wpmed/stable-diffusion-f16-new/resolve/main"
                )
            else:
                state = get_state_dict(model)
                safe_save(
                ...
                convert_f32_to_f16(
                    os.path.join(os.path.dirname(__file__), "./net.safetensors"),
                    os.path.join(os.path.dirname(__file__), "./net_conv.safetensors"),
                )
                split_safetensor(
                    os.path.join(os.path.dirname(__file__), "./net_conv.safetensors")
                )
