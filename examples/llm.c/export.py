...
if __name__ == "__main__":
    ...
    for p in nn.state.get_parameters(model):
        p.replace(Tensor.empty(p.shape, dtype=p.dtype))  # fake load pretrained
    ...
    for k, v in state_dict.items():
        if v.lazydata.base.buffer not in used_buffers:
            print(f"UNUSED: {k}")
        if v.grad is not None:
            grad_state_dict["grad_" + k] = v.grad
    state_dict.update(grad_state_dict)
    state_dict.update(
        {
            "adam_b1_t": optimizer.b1_t,
            "adam_b2_t": optimizer.b2_t,
            "adam_lr": optimizer.lr,
        }
    )
    ...
    named_buffers = {
        v.lazydata.base.buffer: k.replace(".", "_") for k, v in state_dict.items()
    }

    c_code = ["#include <stdlib.h>", "#include <tgmath.h>", "#include <stdbool.h>"]
    if TIMING:
        c_code += ["#include <stdio.h>", "#include <time.h>"]
    ...
    for i, si in enumerate(sched):
        ...
        if si.ast.op is not Ops.SINK:
            ...
            if TIMING:
                main.append(
                    f"  struct timespec tm{i+1}; clock_gettime(CLOCK_MONOTONIC, &tm{i+1});"
                )
                main.append(
                    f'  printf("%10.2f ms + %7.2f ms @ {to_function_name(srcs[si.ast][0])}\\n",'
    ...
    mallocs = [
        f"  {b.dtype.name}* {n} = ({b.dtype.name}*)malloc({b.nbytes});"
