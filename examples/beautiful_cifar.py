...
if __name__ == "__main__":
    ...
    X_train, Y_train, X_test, Y_test = [
        x.contiguous() for x in [X_train, Y_train, X_test, Y_test]
    ]
    cifar10_std, cifar10_mean = X_train.float().std_mean(axis=(0, 2, 3))

    def preprocess(X: Tensor, Y: Tensor) -> Tuple[Tensor, Tensor]:
        return (
            (X - cifar10_mean.view(1, -1, 1, 1)) / cifar10_std.view(1, -1, 1, 1)
        ).cast(dtypes.default_float), Y.one_hot(depths["num_classes"])
    ...
    params_bias, params_non_bias = partition(
        state_dict.items(), lambda x: "bias" in x[0]
    )
    opt_bias = nn.optim.SGD(
        [x[1] for x in params_bias],
        lr=0.01,
        momentum=0.85,
        nesterov=True,
        weight_decay=hyp["opt"]["bias_decay"],
    )
    opt_non_bias = nn.optim.SGD(
        [x[1] for x in params_non_bias],
        lr=0.01,
        momentum=0.85,
        nesterov=True,
    ...
    def loss_fn(out, Y):
        return (
            out.cross_entropy(Y, reduction="none", label_smoothing=0.2)
    ...
    def val_step() -> Tuple[Tensor, Tensor]:
        ...
        for i in range(0, X_test.size(0), eval_batchsize):
            X, Y = preprocess(
                X_test[i : i + eval_batchsize], Y_test[i : i + eval_batchsize]
            )
            out = model(X)
            loss.append(loss_fn(out, Y))
            acc.append(
                (out.argmax(-1).one_hot(depths["num_classes"]) * Y).sum()
                / eval_batchsize
            )
        ret = (
            Tensor.stack(*loss).mean() / (batchsize * loss_batchsize_scaler),
            Tensor.stack(*acc).mean(),
        )
    ...
    for epoch in range(math.ceil(hyp["misc"]["train_epochs"])):
        ...
        tidxs = Tensor(idxs, dtype="int")[: num_steps_per_epoch * batchsize].reshape(
            num_steps_per_epoch, batchsize
        )  # NOTE: long doesn't fold
        train_loss: float = 0
        for epoch_step in (t := trange(num_steps_per_epoch)):
            ...
            t.set_description(
                f"*** loss: {loss:5.3f}   lr: {opt_non_bias.lr.item():.6f}"
        ...
        print(
            f"\033[F*** epoch {epoch:3d}       tm: {(gmt-gst):5.2f} s    val_tm: {(get-gmt):5.2f} s   train_loss: {train_loss/num_steps_per_epoch:5.3f}   val_loss: {val_loss:5.3f}   eval acc: {acc*100:5.2f}%  @ {get-start_tm:6.2f} s  "
        )
