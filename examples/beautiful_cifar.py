...
if __name__ == "__main__":
  ...
  def preprocess(X: Tensor) -> Tensor:
    return ((X - cifar10_mean.view(1, -1, 1, 1)) / cifar10_std.view(1, -1, 1, 1)).cast(dtypes.default_float)
  ...
  opt_bias = nn.optim.SGD([x[1] for x in params_bias], lr=0.01, momentum=0.85, nesterov=True, weight_decay=hyp["opt"]["bias_decay"])
  opt_non_bias = nn.optim.SGD([x[1] for x in params_non_bias], lr=0.01, momentum=0.85, nesterov=True, weight_decay=hyp["opt"]["non_bias_decay"])
  ...
  def train_step(idxs: Tensor) -> Tensor:
    ...
    opt.zero_grad()
    loss.backward()
    return (loss / (batchsize * loss_batchsize_scaler)).realize(
      *opt.schedule_step(), *lr_sched_bias.schedule_step(), *lr_sched_non_bias.schedule_step()
    )
