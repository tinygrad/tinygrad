...
if __name__ == "__main__":
  for BS in [64,128, 256]:
    ...
    print(f'BS={BS} opt_tm {opt_tm:7.2f}')
    mdl = ResNet50()
    seen = set()
  # the device we are optimizing for
    device: Compiled = Device[Device.DEFAULT]
    print(f"optimizing for {Device.DEFAULT}, BS={BS} aim for opt_tm {opt_tm:7.2f} ms")

    # first model run to init the weights, they are saved in seen
    mdl(Tensor.empty(BS, 3, 224, 224)).lazydata.schedule(seen)

    # run model again to get only what changes, these are the kernels of the model
    x = Tensor.empty(BS, 3, 224, 224)
    out = mdl(x)
    ...
    print(f"*******BS={BS} SUCCESS={opt_tm<=total_tm} total {total_tm*1000:.2f} ms, {running_gflops/total_tm:6.0f} GFLOPS {(total_tm*1000)-opt_tm:7.2f}ms slower than opt_tm")
