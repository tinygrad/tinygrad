...
class Attention:
    ...
    def __call__(
        ...
        if start_pos > 0:
            ...
            values = self.cache_kv[1].shrink(
                (None, (0, start_pos + seqlen), None, None)
            )
        else:
            keys = xk
            values = xv
...
class Transformer:
    ...
    def forward(
        self, tokens: Union[Tensor, UOp], start_pos: Variable, temperature: float = 0.0
    ):
    ...
    def __call__(
        ...
        forward = (
            self.forward_jit
            if JIT and (isinstance(tokens, UOp) or tokens.shape[1] == 1)
            else self.forward
        )
...
if __name__ == "__main__":
    ...
    gpt2 = (
        GPT2.build_gguf(args.model_size)
        if args.model_size.startswith("gpt2_gguf_")
        else GPT2.build(args.model_size)
    )
