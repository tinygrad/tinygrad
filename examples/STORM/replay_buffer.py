...
class ReplayBuffer():
    ...
    def sample_external(self, batch_size, batch_length, to_device="cuda"):
        ...
        if self.store_on_gpu:
            ...
            obs = Tensor.stack([self.external_buffer["obs"][idx:idx+batch_length] for idx in indexes])
            # action = torch.stack([self.external_buffer["action"][idx:idx+batch_length] for idx in indexes])
            action = Tensor.stack([self.external_buffer["action"][idx:idx+batch_length] for idx in indexes])
            # reward = torch.stack([self.external_buffer["reward"][idx:idx+batch_length] for idx in indexes])
            reward = Tensor.stack([self.external_buffer["reward"][idx:idx+batch_length] for idx in indexes])
        ...
        if self.store_on_gpu:
            ...
            if batch_size > 0:
                for i in range(self.num_envs):
                    ...
                    obs.append(Tensor.stack([self.obs_buffer[idx:idx+batch_length, i] for idx in indexes]))
                    # action.append(torch.stack([self.action_buffer[idx:idx+batch_length, i] for idx in indexes]))
                    action.append(Tensor.stack([self.action_buffer[idx:idx+batch_length, i] for idx in indexes]))
                    # reward.append(torch.stack([self.reward_buffer[idx:idx+batch_length, i] for idx in indexes]))
                    reward.append(Tensor.stack([self.reward_buffer[idx:idx+batch_length, i] for idx in indexes]))
