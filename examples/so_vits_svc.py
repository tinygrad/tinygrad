...
class TransformerEncoder:
  def __init__(self, cfg: HParams):
    ...
    self.dropout, self.embedding_dim, self.layer_norm_first, self.layerdrop, self.num_layers, self.num_layers_1 = (
      cfg.dropout,
...
class TransformerEncoderLayer:
  def __init__(
    ...
    (
      ...
      self.activation_fn,
    ) = embedding_dim, dropout, activation_dropout, layer_norm_first, num_attention_heads, cond_layer_norm, get_activation_fn(activation_fn)
    ...
    def block(n_in, n_out, k, stride, is_layer_norm=False, is_group_norm=False, conv_bias=False):
      ...
      if is_layer_norm:
        return [
          ...
          [partial(Tensor.transpose, dim0=-2, dim1=-1), nn.LayerNorm(dim, elementwise_affine=True), partial(Tensor.transpose, dim0=-2, dim1=-1)],
          Tensor.gelu,
        ]
  ...
  def __init__(
    ...
    **kwargs,
    ...
    self.enc_p = TextEncoder(
      inter_channels, hidden_channels, kernel_size, n_layers, filter_channels=filter_channels, n_heads=n_heads, p_dropout=p_dropout
    )
    self.dec = Generator(
      sampling_rate,
      inter_channels,
...
class TextEncoder:
  def __init__(self, out_channels, hidden_channels, kernel_size, n_layers, gin_channels=0, filter_channels=None, n_heads=None, p_dropout=None):
    self.out_channels, self.hidden_channels, self.kernel_size, self.n_layers, self.gin_channels = (
      out_channels,
      hidden_channels,
      kernel_size,
      n_layers,
...
class SineGen:
  def __init__(self, samp_rate, harmonic_num=0, sine_amp=0.1, noise_std=0.003, voice_threshold=0, flag_for_pulse=False):
    self.sine_amp, self.noise_std, self.harmonic_num, self.sampling_rate, self.voiced_threshold, self.flag_for_pulse = (
      sine_amp,
      noise_std,
      harmonic_num,
      samp_rate,
  ...
  def _f02sine(self, f0_values):
    ...
    def mod(x: Tensor, n: int) -> Tensor:
      return x - n * x.div(n).floor()  # this is what the % operator does in pytorch.
  ...
  def __init__(
    ...
    for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
      ...
      self.noise_convs.append(
        nn.Conv1d(1, c_cur, kernel_size=stride_f0 * 2, stride=stride_f0, padding=(stride_f0 + 1) // 2)
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--speaker", default=None, help="If not specified, the first available speaker is chosen. Usually there is only one speaker per model."
  )
