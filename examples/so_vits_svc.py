...
class SpeechEncoder:
    def __init__(self, hidden_dim, model: ContentVec):
        self.hidden_dim, self.model = hidden_dim, model
    ...
    def load_from_pretrained(
        cls, checkpoint_path: str, checkpoint_url: str
    ) -> ContentVec:
...
class ContentVec:
    ...
    def __init__(self, cfg: HParams):
        self.feature_grad_mult, self.untie_final_proj = (
            cfg.feature_grad_mult,
            cfg.untie_final_proj,
        )
        ...
        self.final_proj = (
            nn.Linear(cfg.encoder_embed_dim, final_dim * 1)
            if self.untie_final_proj
            else nn.Linear(cfg.encoder_embed_dim, final_dim)
        )
    ...
    def forward_features(self, source, padding_mask):
        if self.feature_grad_mult > 0:
            ...
            if self.feature_grad_mult != 1.0:
                pass  # training: GradMultiply.forward(features, self.feature_grad_mult)
        else:
            features = self.feature_extractor(source, padding_mask)
        return features

    def forward_padding_mask(
        self, features, padding_mask
    ):  # replaces original forward_padding_mask for batch inference
        lengths_org = (
            tilde(padding_mask.cast(dtypes.bool)).cast(dtypes.int64).sum(1)
        )  # ensure its bool for tilde
        lengths = (lengths_org - 400).float().div(320).floor().cast(
            dtypes.int64
        ) + 1  # intermediate float to divide
...
class TransformerEncoder:
    def __init__(self, cfg: HParams):
        def make_conv() -> nn.Conv1d:
            ...
            layer.weight, layer.bias = (Tensor.normal(*layer.weight.shape, std=std)), (
                Tensor.zeros(*layer.bias.shape)
            )
        ...
        (
            ...
            cfg.encoder_layerdrop,
            cfg.encoder_layers,
            cfg.encoder_layers_1,
        )
        self.pos_conv, self.pos_conv_remove = [make_conv()], (
            1 if cfg.conv_pos % 2 == 0 else 0
        )
        self.layers = [
            TransformerEncoderLayer(
                ...
                cfg.activation_fn,
                self.layer_norm_first,
                cond_layer_norm=(i >= cfg.encoder_layers),
            )
            for i in range(cfg.encoder_layers + cfg.encoder_layers_1)
        ]
        self.layer_norm = nn.LayerNorm(self.embedding_dim)
        self.cond_layer_norm = (
            CondLayerNorm(self.embedding_dim) if cfg.encoder_layers_1 > 0 else None
        )
    ...
    def __call__(self, x, spk_emb, padding_mask=None, layer=None, tap=False):
        ...
        if self.layer_norm_first and layer is None:
            x = (
                self.cond_layer_norm(x, spk_emb)
                if (self.num_layers_1 > 0)
                else self.layer_norm(x)
            )
        return x, layer_results

    def extract_features(
        self, x: Tensor, spk_emb: Tensor, padding_mask=None, tgt_layer=None, tap=False
    ):
        ...
        if padding_mask is not None:
            # x[padding_mask] = 0
            assert (
                padding_mask.shape == x.shape[: len(padding_mask.shape)]
            )  # first few dims of x must match padding_mask
        ...
        if self.pos_conv_remove > 0:
            x_conv = x_conv[:, :, : -self.pos_conv_remove]
...
class TransformerEncoderLayer:
    def __init__(
        ...
        self.self_attn_layer_norm = (
            ...
            if not cond_layer_norm
            else CondLayerNorm(self.embedding_dim)
        )
    ...
    def __call__(
        ...
        if self.layer_norm_first:
            x = (
                ...
                if not self.cond_layer_norm
                else self.self_attn_layer_norm(x, emb)
            )
            ...
            x = (
                ...
                if not self.cond_layer_norm
                else self.final_layer_norm(x, emb)
            )
...
class MultiHeadAttention:
    def __init__(self, n_state, n_head):
        ...
        self.q_proj, self.k_proj, self.v_proj, self.out_proj = [
            nn.Linear(n_state, n_state) for _ in range(4)
        ]
    ...
    def __init__(self, conv_layers, dropout=0.0, mode="default", conv_bias=False):
        ...
        def block(
            ...
            if is_layer_norm:
                return [
                    make_conv(),
                    partial(Tensor.dropout, p=dropout),
                    [
                        partial(Tensor.transpose, dim0=-2, dim1=-1),
        ...
        for i, cl in enumerate(conv_layers):
            ...
            self.conv_layers.append(
                block(
                    ...
                    is_group_norm=(
                        (mode == "default" or mode == "group_norm_masked") and i == 0
                    ),
                    conv_bias=conv_bias,
                )
            )
    ...
    def __call__(self, x: Tensor, padding_mask: Tensor):
        ...
        if self.mode == "group_norm_masked":
            if padding_mask is not None:
                ...
                lengths_org = (
                    tilde(padding_mask.cast(dtypes.bool)).cast(dtypes.int64).sum(1)
                )  # ensure padding_mask is bool for tilde
                lengths = (((lengths_org - k) / stride) + 1).floor().cast(dtypes.int64)
                padding_mask = tilde(lengths_to_padding_mask(lengths)).cast(
                    dtypes.int64
                )  # lengths_to_padding_mask returns bool tensor
        ...
        for _, conv in enumerate(self.conv_layers[1:], start=1):
            conv = reduce(
                lambda a, b: operator.iconcat(a, b if isinstance(b, list) else [b]),
                conv,
                [],
            )  # flatten
            x = x.sequential(conv)
...
class CondLayerNorm:  # https://github.com/auspicious3000/contentvec/blob/main/contentvec/modules/cond_layer_norm.py#L10
    def __init__(self, dim_last, eps=1e-5, dim_spk=256, elementwise_affine=True):
        self.dim_last, self.eps, self.dim_spk, self.elementwise_affine = (
            dim_last,
            eps,
            dim_spk,
            elementwise_affine,
        ...
        if self.elementwise_affine:
            ...
            self.weight_ln.weight, self.bias_ln.weight = (
                Tensor.ones(*self.weight_ln.weight.shape)
            ), (Tensor.zeros(*self.bias_ln.weight.shape))
...
class GroupNormMasked:  # https://github.com/auspicious3000/contentvec/blob/d746688a32940f4bee410ed7c87ec9cf8ff04f74/contentvec/modules/fp32_group_norm.py#L16
    def __init__(self, num_groups, num_channels, eps=1e-5, affine=True):
        ...
        self.weight, self.bias = (Tensor.ones(num_channels)), (
            (Tensor.zeros(num_channels)) if self.affine else (None, None)
        )
    ...
    def __call__(self, x: Tensor, mask: Tensor):
        ...
        if mask is None:
            ...
            mask = mask.reshape(bsz, 1, 1, length)
        x = x * mask
        ...
        var = (
            ((x.std(axis=3, keepdim=True) ** 2) + mean_**2) * length / lengths - mean**2
        ) + self.eps
        return (
            x.add(-mean)
            .div(var.sqrt())
            .reshape(bsz, n_c, length)
            .mul(self.weight.reshape(1, -1, 1))
            .add(self.bias.reshape(1, -1, 1))
        )
...
class Synthesizer:
    ...
    def infer(
        ...
        x = (
            self.pre(c) * x_mask
            + self.emb_uv(uv.cast(dtypes.int64)).transpose(1, 2)
            + vol
        )
        z_p, _, _, c_mask = self.enc_p.forward(
            x, x_mask, f0=self._f0_to_coarse(f0), noise_scale=noise_scale
        )
    ...
    @classmethod
    def load_from_pretrained(
        cls, config_path: str, config_url: str, weights_path: str, weights_url: str
    ) -> Synthesizer:
...
class TextEncoder:
    def __init__(
        ...
        (
            ...
            self.n_layers,
            self.gin_channels,
        ) = (out_channels, hidden_channels, kernel_size, n_layers, gin_channels)
...
class SineGen:
    ...
    def _f02sine(self, f0_values):
        def padDiff(x: Tensor):
            return (x.pad((0, 0, -1, 1)) - x).pad((0, 0, 0, -1))

        def mod(x: Tensor, n: int) -> Tensor:
            return (
                x - n * x.div(n).floor()
            )  # this is what the % operator does in pytorch.

        rad_values = mod((f0_values / self.sampling_rate), 1)  # convert to F0 in rad
        rand_ini = Tensor.rand(
            f0_values.shape[0], f0_values.shape[2], device=f0_values.device
        )  # initial phase noise

        # rand_ini[:, 0] = 0
        m = (
            Tensor.ones(f0_values.shape[0])
            .unsqueeze(1)
            .pad((0, f0_values.shape[2] - 1, 0, 0))
            .cast(dtypes.bool)
        )
        m = tilde(m)
        ...
        m = (
            Tensor.ones(tmp.shape)
            .pad((0, 0, 0, rad_values.shape[1] - 1, 0))
            .cast(dtypes.bool)
        )
        m = tilde(m)
    ...
    def forward(self, f0, upp=None):
        fn = f0.mul(
            Tensor([[range(1, self.harmonic_num + 2)]], dtype=dtypes.float32).to(
                f0.device
            )
        )
...
class Generator:
    def __init__(
        ...
        self.conv_pre = nn.Conv1d(
            inter_channels, upsample_initial_channel, 7, 1, padding=3
        )
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            ...
            self.noise_convs.append(
                ...
                else nn.Conv1d(1, c_cur, kernel_size=1)
            )
...
def randn_like(x: Tensor) -> Tensor:
    return Tensor.randn(*x.shape, dtype=x.dtype).to(device=x.device)


def tilde(x: Tensor) -> Tensor:
    if x.dtype == dtypes.bool:
        return (1 - x).cast(dtypes.bool)
    return (
        x + 1
    ) * -1  # this seems to be what the ~ operator does in pytorch for non bool
...
def load_checkpoint_enc(
    checkpoint_path, model: ContentVec, optimizer=None, skip_list=[]
):
    assert Path(checkpoint_path).is_file()
    ...
    for key, v in saved_state_dict.items():
        ...
        try:
            ...
            if not skip and obj.shape == v.shape:
                if "feature_extractor" in key and (
                    isinstance(parent, nn.GroupNorm) or isinstance(parent, nn.LayerNorm)
                ):  # cast
            ...
            raise e
    logging.info(
        f"Loaded checkpoint '{checkpoint_path}' in {time.time() - start_time:.4f}s"
    )
...
def get_sid(spk2id: HParams, speaker: str) -> Tensor:
    ...
    if not speaker_id and type(speaker) is int:
        if len(spk2id.__dict__) >= speaker:
            speaker_id = speaker
    if speaker_id is None:
        raise RuntimeError(f"speaker={speaker} not in the speaker list")
    return Tensor([int(speaker_id)], dtype=dtypes.int64).unsqueeze(0)


def get_encoder(ssl_dim) -> Type[SpeechEncoder]:
    if ssl_dim == 256:
        return ContentVec256L9
    if ssl_dim == 768:
        return ContentVec768L12
...
VITS_MODELS = {  # config_path, weights_path, config_url, weights_url
    "saul_goodman": (
        SO_VITS_SVC_PATH / "config_saul_gman.json",
        SO_VITS_SVC_PATH / "pretrained_saul_gman.pth",
        "https://huggingface.co/Amo/so-vits-svc-4.0_GA/resolve/main/ModelsFolder/Saul_Goodman_80000/config.json",
        "https://huggingface.co/Amo/so-vits-svc-4.0_GA/resolve/main/ModelsFolder/Saul_Goodman_80000/G_80000.pth",
    ),
    "drake": (
        SO_VITS_SVC_PATH / "config_drake.json",
        SO_VITS_SVC_PATH / "pretrained_drake.pth",
        "https://huggingface.co/jaspa/so-vits-svc/resolve/main/aubrey/config_aubrey.json",
        "https://huggingface.co/jaspa/so-vits-svc/resolve/main/aubrey/pretrained_aubrey.pth",
    ),
    "cartman": (
        SO_VITS_SVC_PATH / "config_cartman.json",
        SO_VITS_SVC_PATH / "pretrained_cartman.pth",
        "https://huggingface.co/marcoc2/so-vits-svc-4.0-models/resolve/main/EricCartman/config.json",
        "https://huggingface.co/marcoc2/so-vits-svc-4.0-models/resolve/main/EricCartman/G_10200.pth",
    ),
    "tf2spy": (
        SO_VITS_SVC_PATH / "config_tf2spy.json",
        SO_VITS_SVC_PATH / "pretrained_tf2spy.pth",
        "https://huggingface.co/Amo/so-vits-svc-4.0_GA/resolve/main/ModelsFolder/TF2_spy_60k/config.json",
        "https://huggingface.co/Amo/so-vits-svc-4.0_GA/resolve/main/ModelsFolder/TF2_spy_60k/G_60000.pth",
    ),
    "tf2heavy": (
        SO_VITS_SVC_PATH / "config_tf2heavy.json",
        SO_VITS_SVC_PATH / "pretrained_tf2heavy.pth",
        "https://huggingface.co/Amo/so-vits-svc-4.0_GA/resolve/main/ModelsFolder/TF2_heavy_100k/config.json",
        "https://huggingface.co/Amo/so-vits-svc-4.0_GA/resolve/main/ModelsFolder/TF2_heavy_100k/G_100000.pth",
    ),
    "lady_gaga": (
        SO_VITS_SVC_PATH / "config_gaga.json",
        SO_VITS_SVC_PATH / "pretrained_gaga.pth",
        "https://huggingface.co/marcoc2/so-vits-svc-4.0-models/resolve/main/LadyGaga/config.json",
        "https://huggingface.co/marcoc2/so-vits-svc-4.0-models/resolve/main/LadyGaga/G_14400.pth",
    ),
}
ENCODER_MODELS = {  # weights_path, weights_url
    "contentvec": (
        SO_VITS_SVC_PATH / "contentvec_checkpoint.pt",
        "https://huggingface.co/lj1995/VoiceConversionWebUI/resolve/main/hubert_base.pt",
    )
}
ENCODER_MODEL = "contentvec"
DEMO_PATH, DEMO_URL = (
    Path(__file__).parents[1] / "temp/LJ037-0171.wav",
    "https://keithito.com/LJ-Speech-Dataset/LJ037-0171.wav",
)
if __name__ == "__main__":
    logging.basicConfig(
        stream=sys.stdout, level=(logging.INFO if DEBUG < 1 else logging.DEBUG)
    )
    ...
    parser.add_argument(
        ...
        "--model",
        default=None,
        help=f"Specify the model to use. All supported models: {VITS_MODELS.keys()}",
        required=True,
    )
    parser.add_argument(
        "-f", "--file", default=DEMO_PATH, help=f"Specify the path of the input file"
    )
    parser.add_argument(
        "--out_dir",
    ...
    parser.add_argument(
        "--speaker",
        default=None,
        help="If not specified, the first available speaker is chosen. Usually there is only one speaker per model.",
    )
    parser.add_argument("--noise_scale", default=0.4)
    parser.add_argument(
        "--tran",
        default=0.0,
        help="Pitch shift, supports positive and negative (semitone) values. Default 0.0",
    )
    ...
    encoder_location, vits_location = (
        ENCODER_MODELS[ENCODER_MODEL],
        VITS_MODELS[vits_model],
    )

    Tensor.no_grad, Tensor.training = True, False
    # Get Synthesizer and ContentVec
    net_g, hps = Synthesizer.load_from_pretrained(
        vits_location[0], vits_location[2], vits_location[1], vits_location[3]
    )
    Encoder = get_encoder(hps.model.ssl_dim)
    encoder = Encoder.load_from_pretrained(encoder_location[0], encoder_location[1])

    # model config args
    target_sample, spk2id, hop_length, target_sample = (
    ...
    vol_embedding = (
        hps.model.vol_embedding
        if hasattr(hps.data, "vol_embedding") and hps.model.vol_embedding is not None
        else False
    )

    # args
    slice_db, clip_seconds, lg_num, pad_seconds, tran, noise_scale, audio_path = (
        args.slice_db,
        args.clip_seconds,
        args.lg_num,
        args.pad_seconds,
    ...
    speaker = (
        args.speaker if args.speaker is not None else list(hps.spk.__dict__.keys())[0]
    )

    ### Loading audio and slicing ###
    if audio_path == DEMO_PATH:
        download_if_not_present(DEMO_PATH, DEMO_URL)
    ...
    for slice_tag, data in audio_data:
        ...
        for k, dat in enumerate(datas):
            per_length = (
                int(np.ceil(len(dat) / audio_sr * target_sample))
                if clip_seconds != 0
                else length
            )
            ...
            wav16k, f0, uv = preprocess.get_unit_f0(
                wav, tran, hop_length, target_sample
            )
            ...
            c = repeat_expand_2d_left(
                c.squeeze(0).realize(), f0.shape[1]
            )  # interpolate speech encoding to match f0
            c = c.unsqueeze(0).realize()
            enc_time = time.time() - start

            # VITS infer
            vits_start = time.time()
            out_audio, f0 = net_g.infer(
                c, f0=f0, uv=uv, g=sid, noise_scale=noise_scale, vol=None
            )
            ...
            logging.info(
                "total infer time:{:.2f}s, speech_enc time:{:.2f}s, vits time:{:.2f}s".format(
                    infer_time, enc_time, vits_time
                )
            )
            ### Infer END ###
    ...
    out_path = Path(
        args.out_path
        or Path(args.out_dir) / f"{args.model}{f'_spk_{speaker}'}_{args.base_name}.wav"
    )
