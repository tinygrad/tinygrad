...
class SPPF:
    def __init__(self, c1, c2, k=5):
        ...
        self.maxpool = lambda x: x.pad((k // 2, k // 2, k // 2, k // 2)).max_pool2d(
    ...
    for v in metadata.values():
        if v["dtype"] == "F16":
            v.update(
                {
                    "dtype": "F32",
                    "data_offsets": [offset * 2 for offset in v["data_offsets"]],
                }
            )
...
def get_weights_location(yolo_variant: str) -> Path:
    weights_location = (
        Path(__file__).parents[1] / "weights" / f"yolov8{yolo_variant}.safetensors"
    )
    fetch(
    ...
    if not is_dtype_supported(dtypes.half):
        f32_weights = weights_location.with_name(
            f"{weights_location.stem}_f32.safetensors"
        )
        if not f32_weights.exists():
