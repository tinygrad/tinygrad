...
def convert_f16_safetensor_to_f32(input_file: Path, output_file: Path):
  ...
  for v in metadata.values():
    if v["dtype"] == "F16":
      v.update({"dtype": "F32", "data_offsets": [offset * 2 for offset in v["data_offsets"]]})
...
if __name__ == "__main__":
  ...
  yolo_variant = (
    ...
    if len(sys.argv) >= 3
      ...
      print("No variant given, so choosing 'n' as the default. Yolov8 has different variants, you can choose from ['n', 's', 'm', 'l', 'x']") or "n"
    )
  )
