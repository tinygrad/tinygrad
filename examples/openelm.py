...
class Attention:
    def __init__(self, model_dim, num_query_heads, num_kv_heads, head_dim):
        self.qkv_proj = nn.Linear(
            model_dim, (num_query_heads + num_kv_heads * 2) * head_dim, bias=False
        )
    ...
    def __call__(self, x: Tensor) -> Tensor:
        ...
        qkv = qkv.reshape(
            batch_size,
            seq_len,
            self.num_query_heads + self.num_kv_heads * 2,
            self.head_dim,
        ).transpose(1, 2)
        xq, xk, xv = qkv.split(
            [self.num_query_heads, self.num_kv_heads, self.num_kv_heads], dim=1
        )
        xq = self.q_norm(xq)
        xk = self.k_norm(xk)
        ...
        freq_constant = 10000
        inv_freq = 1.0 / (
            freq_constant ** (Tensor.arange(0, self.head_dim, 2) / self.head_dim)
        )
        ...
        attn_output = xq.scaled_dot_product_attention(xk, xv, is_causal=True).transpose(
            1, 2
        )
...
class Layer:
    def __init__(
        self, model_dim, intermediate_dim, num_query_heads, num_kv_heads, head_dim
    ):
    ...
    def __init__(self, cfg):
        ...
        self.layers = [
            Layer(
                cfg["model_dim"],
                make_divisible(
                    int(cfg["model_dim"] * cfg["ffn_multipliers"][i]),
                    cfg["ffn_dim_divisor"],
                ),
                cfg["num_query_heads"][i],
                cfg["num_kv_heads"][i],
                cfg["head_dim"],
            )
...
if __name__ == "__main__":
    ...
    model = Transformer(
        json.loads(
            fetch(
                f"https://huggingface.co/apple/{model_name}/resolve/main/config.json?download=true"
            ).read_bytes()
        )
    )
    weights = nn.state.safe_load(
        fetch(
            f"https://huggingface.co/apple/{model_name}/resolve/main/model.safetensors?download=true"
        )
    )
    if DEBUG >= 3:
    ...
    nn.state.load_state_dict(
        model, {k.removeprefix("transformer."): v for k, v in weights.items()}
    )
