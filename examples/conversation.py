...
def llama_generate(
  llama: LLaMa, toks: list[int], outputted: str, prompt: str, start_pos: int, user_delim: str, resp_delim: str, temperature=0.7, max_tokens=1000
):
...
def tts(
  ...
  audio_tensor = synth.infer(
    ...
    max_y_length_estimate_scale=Y_LENGTH_ESTIMATE_SCALARS[model_to_use] if estimate_max_y_length else None,
    pad_length=vits_pad_length,
  )[0, 0]
...
def init_vits(
  ...
  if emotion_path is not None:
    if emotion_path.endswith(".npy"):
      emotion_embedding = Tensor(np.load(emotion_path), dtype=dtypes.int64).unsqueeze(0)
    else:
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--llama_model", type=Path, default=None, help="Folder with the original weights to load, or single .index.json, .safetensors or .bin file"
  )
  ...
  synth, emotion_embedding, text_mapper, hps, model_has_multiple_speakers = init_vits(
    args.vits_model_to_use, args.vits_emotion_path, args.vits_speaker_id, args.vits_seed
  )
