...
def tts(
    ...
    x_tst, x_tst_lengths = stn_tst.pad(
        ((0, pad_length - init_shape[0]),), value=1
    ).unsqueeze(0), Tensor([init_shape[0]], dtype=dtypes.int64)
    sid = (
    ...
    audio_tensor = synth.infer(
        ...
        max_y_length_estimate_scale=(
            Y_LENGTH_ESTIMATE_SCALARS[model_to_use] if estimate_max_y_length else None
        ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Quantize the weights to int8 or nf4 in memory",
    )
    parser.add_argument(
        "--llama_model",
        type=Path,
