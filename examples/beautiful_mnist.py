...
class FusedAdam(Optimizer):
  ...
  def fused_step(self, t:Tensor, g:Tensor) -> tuple[Tensor, list[Tensor]]:
    ...
    return out, [self.b1_t, self.b2_t, self.m, self.v]
  ...
  def schedule_step_with_grads(self, grads:list[Tensor]) -> list[Tensor]:
    ...
    # NOTE: this should be the only function the user needs to change to implement an optimizer
    out, to_realize = self.fused_step(t, g)
