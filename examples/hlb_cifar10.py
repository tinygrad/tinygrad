...
def train_cifar():
  ...
  for X, Y in fetch_batches(X_train, Y_train, BS=BS, is_train=True):
    ...
    loss_cpu = loss.numpy()
    cl = time.monotonic()
    optimizer.lr = optimizer.lr * (1-lr_decay)
    print(f"{i:3d} {(cl-st)*1000.0:7.2f} ms run, {(et-st)*1000.0:7.2f} ms python, {(cl-et)*1000.0:7.2f} ms CL, {loss_cpu:7.2f} loss, {optimizer.lr} LR, {GlobalCounters.mem_used/1e9:.2f} GB used, {GlobalCounters.global_ops*1e-9/(cl-st):9.2f} GFLOPS")
    i += 1
