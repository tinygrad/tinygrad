...
class UnsyncedBatchNorm:
  def __init__(self, sz: int, eps=1e-5, affine=True, track_running_stats=True, momentum=0.1, num_devices=len(GPUS)):
    ...
    if affine:
      self.weight, self.bias = Tensor.ones(sz, dtype=dtypes.float32), Tensor.zeros(sz, dtype=dtypes.float32)
    else:
    ...
    if Tensor.training:
      ...
      if self.track_running_stats:
        ...
        self.running_var.assign(
          (1 - self.momentum) * self.running_var + self.momentum * batch_var_adjust * batch_var.detach().cast(self.running_var.dtype)
        )
...
def train_cifar():
  ...
  def train_step(model, optimizer, lr_scheduler, X, Y):
    ...
    loss = (
      cross_entropy(out, Y, reduction="none", label_smoothing=hyp["opt"]["label_smoothing"])
