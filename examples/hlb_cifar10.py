...
assert (
    BS % len(GPUS) == 0
), f"{BS=} is not a multiple of {len(GPUS)=}, uneven multi GPU is slow"
assert (
    EVAL_BS % len(GPUS) == 0
), f"{EVAL_BS=} is not a multiple of {len(GPUS)=}, uneven multi GPU is slow"
...
class UnsyncedBatchNorm:
    def __init__(
        ...
        if affine:
            self.weight, self.bias = Tensor.ones(
                sz, dtype=dtypes.float32
            ), Tensor.zeros(sz, dtype=dtypes.float32)
        else:
            self.weight, self.bias = None, None

        self.running_mean = Tensor.zeros(
            num_devices, sz, dtype=dtypes.float32, requires_grad=False
        )
        self.running_var = Tensor.ones(
            num_devices, sz, dtype=dtypes.float32, requires_grad=False
        )
        self.num_batches_tracked = Tensor.zeros(
            1, dtype=dtypes.int, requires_grad=False
        )
    ...
    def calc_stats(self, x: Tensor):
        if Tensor.training:
            ...
            y = x - batch_mean.detach().reshape(
                shape=[batch_mean.shape[0], 1, -1, 1, 1]
            )  # d(var)/d(mean) = 0
            ...
            if self.track_running_stats:
                self.running_mean.assign(
                    ...
                    + self.momentum * batch_mean.detach().cast(self.running_mean.dtype)
                )
                ...
                self.running_var.assign(
                    ...
                    + self.momentum
                    * batch_var_adjust
                    * batch_var.detach().cast(self.running_var.dtype)
                )
            ...
            batch_invstd = (
                self.running_var.reshape(self.running_var.shape[0], 1, -1, 1, 1)
                .expand(x.shape)
                .add(self.eps)
                .rsqrt()
            )
...
class SpeedyResNet:
    ...
    def __call__(self, x, training=True):
        ...
        forward = (
            lambda x: x.conv2d(self.whitening).pad((1, 0, 0, 1)).sequential(self.net)
        )
        return forward(x) if training else (forward(x) + forward(x[..., ::-1])) / 2.0
...
def train_cifar():
    ...
    def whitening(X, kernel_size=hyp["net"]["kernel_size"]):
        ...
        return Tensor(W.astype(np.float32), requires_grad=False).cast(
    ...
    def pad_reflect(X, size=2) -> Tensor:
        X = (
            X[..., :, 1 : size + 1]
            .flip(-1)
            .cat(X, X[..., :, -(size + 1) : -1].flip(-1), dim=-1)
        )
        X = (
            X[..., 1 : size + 1, :]
            .flip(-2)
            .cat(X, X[..., -(size + 1) : -1, :].flip(-2), dim=-2)
        )
        return X
        ...
        while True:
            ...
            if is_train:
                ...
                if getenv("RANDOM_FLIP", 1):
                    X = (Tensor.rand(X.shape[0], 1, 1, 1) < 0.5).where(
                        X.flip(-1), X
                    )  # flip LR
                if getenv("CUTMIX", 1):
            ...
            et = time.monotonic()
            print(
                f"shuffling {'training' if is_train else 'test'} dataset in {(et-st)*1e3:.2f} ms ({epoch=})"
            )
            for i in range(0, X.shape[0], BS):
                # pad the last batch  # TODO: not correct for test
                batch_end = min(i + BS, Y.shape[0])
                x = Tensor(
                    X[batch_end - BS : batch_end], device=X_in.device, dtype=X_in.dtype
                )
                y = Tensor(
                    Y[batch_end - BS : batch_end], device=Y_in.device, dtype=Y_in.dtype
                )
                step += 1
                yield x, y
            epoch += 1
    ...
    transform = [
        ...
        - Tensor(cifar_mean, device=x.device, dtype=x.dtype).reshape((1, 3, 1, 1)),
        lambda x: x
        / Tensor(cifar_std, device=x.device, dtype=x.dtype).reshape((1, 3, 1, 1)),
    ]
    ...
    X_train, Y_train = X_train.cast(dtypes.default_float), Y_train.cast(
        dtypes.default_float
    )
    X_test, Y_test = X_test.cast(dtypes.default_float), Y_test.cast(
        dtypes.default_float
    )
    ...
    with Tensor.train():
        ...
        while i <= STEPS:
            if (
                i % getenv("EVAL_STEPS", STEPS) == 0
                and i > 1
                and not getenv("DISABLE_BACKWARD")
            ):
            ...
            with Context(
                BEAM=getenv("LATEBEAM", BEAM.value), WINO=getenv("LATEWINO", WINO.value)
            ):
                loss = train_step_jitted(
                    model,
                    optim.OptimizerGroup(opt_bias, opt_non_bias),
            ...
            if (
                getenv("EMA")
                and i > hyp["ema"]["steps"]
                and (i + 1) % hyp["ema"]["every_n_steps"] == 0
            ):
            ...
            print(
                f"{i:3d} {(cl-st)*1000.0:7.2f} ms run, {(et-st)*1000.0:7.2f} ms python, {(cl-et)*1000.0:7.2f} ms {device_str}, {loss_cpu:7.2f} loss, {opt_non_bias.lr.numpy()[0]:.6f} LR, {GlobalCounters.mem_used/1e9:.2f} GB used, {GlobalCounters.global_ops*1e-9/(cl-st):9.2f} GFLOPS, {GlobalCounters.global_ops*1e-9:9.2f} GOPS"
            )
            st = cl
            i += 1
