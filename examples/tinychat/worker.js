...
const q6k_to_f16_byteFactor = 1 / 210 * 256 * 2;
const q6k_to_f16_chunk_size = 430080; // compiled kernel input size; GCD of all Q6_K weight sizes
...
self.addEventListener('message', async (event) => {
  ...
  if (k.includes("feed_forward") || k.includes("attention.w")) {
    ...
    const [scale, decomped_bytes] = await decomp(v.bytes);
    v.dtype = "int8"
    ...
    const decomp = await self.q6k_to_f16();
    const wasm = decomp.wasm;
    const result = new Uint8Array(parseInt(v.bytes.length * q6k_to_f16_byteFactor));
    if (v.bytes.length % q6k_to_f16_chunk_size !== 0) {throw new Error(`Expected buffer size: ${v.bytes.length} to be divisible by chunk size: ${q6k_to_f16_chunk_size}`)}
    const num_chunks = v.bytes.length / q6k_to_f16_chunk_size;
    ...
    const inputPtr0 = wasm._malloc(q6k_to_f16_chunk_size);
    const output_size = parseInt(q6k_to_f16_chunk_size * q6k_to_f16_byteFactor);
    const outputPtr0 = wasm._malloc(output_size);

    for (let i = 0; i < num_chunks; i++) {
      const input_cursor = i * q6k_to_f16_chunk_size;
      const output_cursor = i * output_size;
      wasm.HEAPU8.set(v.bytes.subarray(input_cursor, input_cursor + q6k_to_f16_chunk_size), inputPtr0);
