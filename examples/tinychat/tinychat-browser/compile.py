...
def prepare_browser_chunks(model):
  ...
  for info in split_t_infos:
    ...
    for file in files:
      if sum(i[0] for i in file) + info[0] <= chunk_size:
        if info[3] and any(i[3] for i in file):
          continue  # no two split tensors can touch the same file, due to wasm loading constraints
        file.append(info)
        placed = True
        break
    if not placed:
  ...
  if len(hashes) != len(files):
    print(f"WARNING: {len(files)} files were exported, but only {len(hashes)} are unique: something may have gone wrong")
  ...
  with open(os.path.join(os.path.dirname(__file__), f"./net_metadata.json"), "w") as writer:
    json.dump(metadata, writer, indent=4)
  return metadata
...
def validate_model(model, tokenizer):
  ...
  while True:
    ...
    start_pos += 1
    if tok in tokenizer.stop_tokens or len(result) > len(expected):
      break
...
if __name__ == "__main__":
  ...
  tokenizer_path = fetch(
    "https://huggingface.co/bofenghuang/Meta-Llama-3-8B/resolve/main/original/tokenizer.model", "tokenizer.model", subdir="llama3-1b-instruct"
  )
  ...
  with open(os.path.join(os.path.dirname(__file__), f"{model_name}.c"), "w") as f:
    f.write(cprog)
  with open(os.path.join(os.path.dirname(__file__), "net_clang.js"), "w") as f:
    f.write(js_wrapper)
  ...
  with open(os.path.join(os.path.dirname(__file__), "net.js"), "w") as f:
    f.write(prg)
