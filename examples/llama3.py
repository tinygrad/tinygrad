...
from extra.models.llama import (
    Transformer,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--download_model", action="store_true", help="Download a model"
    )
    parser.add_argument("--model", type=Path, help="Model path")
    parser.add_argument(
        "--size", choices=["1B", "8B", "70B"], default="1B", help="Model size"
    )
    parser.add_argument(
        "--shard", type=int, default=1, help="Shard the model across multiple devices"
    )
    parser.add_argument(
        "--quantize", choices=["int8", "nf4", "float16"], help="Quantization method"
    )
    parser.add_argument(
        "--no_api",
        action="store_true",
        help="Disable the api and run a cli test interface",
    )
    parser.add_argument(
        "--host", type=str, default="0.0.0.0", help="Web server bind address"
    )
    ...
    tokenizer = Tokenizer(
        str(
            (args.model if args.model.is_dir() else args.model.parent)
            / "tokenizer.model"
        )
    )
    ...
    def encode_message(role: str, content: str):
        return (
            encode_role(role)
            + tokenizer.encode(content.strip())
            + [tokenizer.special_tokens["<|eot_id|>"]]
        )

    device = (
        tuple(f"{Device.DEFAULT}:{i}" for i in range(args.shard))
        if args.shard > 1
        else Device.DEFAULT
    )
    model = build_transformer(
        args.model, model_size=args.size, quantize=args.quantize, device=device
    )
    ...
    if not args.no_api and not args.benchmark:
        ...
        def server_static(filename):
            return static_file(
                filename, root=(Path(__file__).parent / "tinychat").as_posix()
            )
        ...
        def server_assets(filename):
            return static_file(
                filename,
                root=(Path(__file__).parent / "tinychat" / "assets").as_posix(),
            )

        @app.route("/")
        def index():
            return static_file(
                "index.html", root=(Path(__file__).parent / "tinychat").as_posix()
            )
        ...
        def completions():
            ...
            toks = [tokenizer.bos_id] + tokenizer.encode(
                rjson.get("prompt", ""), allow_special=True
            )
        ...
        for _ in range(20):
            ...
            print(generated)
        if "LLaMA-3/8B-SF-DPO" in args.model.as_posix() and (
            TEMPERATURE == 0.85 or TEMPERATURE == 0
        ):
            if TEMPERATURE == 0.85:
                EXPECTED_TEXT = {
                ...
                EXPECTED_TEXT = {
                    k: "Hello! How can I assist you today? If you have any questions or need help with something,"
                    for k in range(1, 7)
                }
            assert (
                generated == EXPECTED_TEXT[args.shard]
            ), f"{generated=} {EXPECTED_TEXT[args.shard]}"
            print(
                "\n" + colored("output validated", "green")
            )  # NOTE: "\n" inside colored does not render the color in github action
    else:
        prompt = [tokenizer.bos_id] + encode_message(
