...
class LastLayer:
    def __init__(self, hidden_size: int, patch_size: int, out_channels: int):
        ...
        self.adaLN_modulation: List[Callable[[Tensor], Tensor]] = [
            Tensor.silu,
...
class Flux:
    ...
    def __init__(
        ...
        self.guidance_in: Callable[[Tensor], Tensor] = (
        ...
        self.double_blocks = [
            DoubleStreamBlock(
                self.hidden_size, self.num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias
            )
...
def load_flow_model(name: str, model_path: str):
    ...
    if not model_path:
        model_path = fetch(urls[name])
    state_dict = {
        k.replace("scale", "weight"): v for k, v in safe_load(model_path).items()
    }
...
def prepare(
    T5: T5Embedder, clip: ClipEmbedder, img: Tensor, prompt: Union[str, List[str]]
) -> Dict[str, Tensor]:
    bs, _, h, w = img.shape
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", type=str, default="", help="path of the model file"
    )
    parser.add_argument(
        "--width",
        type=int,
    ...
    # prepare input noise
    x = Tensor.randn(
        1, 16, 2 * math.ceil(height / 16), 2 * math.ceil(width / 16), dtype="bfloat16"
    )
