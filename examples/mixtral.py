...
class MixtureFeedForward:
  ...
  def __call__(self, x: Tensor) -> Tensor:
    ...
    ret = e1(x.to(e1.w1.weight.device)).to(x.device) * scale[0] + e2(x.to(e2.w1.weight.device)).to(x.device) * scale[1]
    return ret
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--weights",
    type=str,
    default=(pathlib.Path(__file__).parent.parent / "weights/mixtral-8x7b-32kseqlen").as_posix(),
  ...
  if CI:
    print(f"ram used: {GlobalCounters.mem_used / 1e9:5.2f} GB")
    ...
    with Profiling(sort="time", frac=0.1, enabled=args.profile):
      with Timing("total ", enabled=args.timing, on_exit=lambda x: f", {1e9 / x:.2f} tok/sec"):
        with WallTimeEvent(BenchEvent.STEP):
          tok = model(
            Tensor([toks[start_pos:]]), 0 if start_pos == 0 else Variable("start_pos", 1, 1024 - 1).bind(start_pos), args.temperature
          ).item()
    toks.append(tok)
    start_pos += 1
