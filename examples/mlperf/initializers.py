...
def rand_truncn(*shape, dtype=None, truncstds=2, **kwargs) -> Tensor:
    ...
    take = (
        (x.abs() <= truncstds).where(ctr, CNT).min(axis=-1, keepdim=True)
    )  # set to 0 if no good samples
...
class Linear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True):
        ...
        self.weight = Tensor.normal(
            (out_features, in_features), mean=0.0, std=0.01, dtype=dtypes.float32
        )
        if bias:
    ...
    def __call__(self, x: Tensor):
        return x.linear(
            self.weight.cast(dtypes.default_float).transpose(),
            self.bias.cast(dtypes.default_float) if self.bias is not None else None,
        )
...
class LayerNormBert:
    def __init__(
        ...
        self.weight, self.bias = (
            (
                Tensor.ones(*self.normalized_shape, dtype=dtypes.float32),
                Tensor.zeros(*self.normalized_shape, dtype=dtypes.float32),
            )
    ...
    def __call__(self, x: Tensor):
        ...
        xn = (
            x.cast(dtypes.float32).layernorm(eps=self.eps, axis=self.axis).cast(x.dtype)
        )
        if not self.elementwise_affine:
            return xn
        return xn * self.weight.cast(dtypes.default_float) + self.bias.cast(
