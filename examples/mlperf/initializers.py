...
class LinearBert(nn.Linear):
  ...
  def __call__(self, x:Tensor):
    ...
    return x.linear(self.weight.contiguous().cast(dtypes.half if getenv("HALF_LINEAR", 0) else dtypes.default_float).transpose(), self.bias.cast(dtypes.default_float) if self.bias is not None else None)
