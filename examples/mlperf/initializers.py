...
class Conv2dHeNormal(nn.Conv2d):
  def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
    ...
    if bias:
      self.bias = self.bias.cast(dtypes.float32)

  def __call__(self, x: Tensor):
    return x.conv2d(
      self.weight.cast(dtypes.default_float),
...
class Linear(nn.Linear):
  def __init__(self, in_features, out_features, bias=True):
    ...
    if bias:
      self.bias = Tensor.zeros(out_features, dtype=dtypes.float32)
...
class LinearBert(nn.Linear):
  ...
  def __call__(self, x: Tensor):
    return x.cast(dtypes.default_float).linear(
      self.weight.cast(dtypes.default_float).transpose(), self.bias.cast(dtypes.default_float) if self.bias is not None else None
    )
...
class EmbeddingBert(nn.Embedding):
  ...
  def __call__(self, idx: Tensor) -> Tensor:
    if idx.numel() == 0:
      return Tensor.empty(idx.shape + (self.embed_sz,), dtype=self.weight.dtype, device=self.weight.device)
    arange_shp, weight_shp, big_shp = (
      (1, 1, self.vocab_sz, 1),
      (1, 1, self.vocab_sz, self.embed_sz),
      idx.shape
      + (
        self.vocab_sz,
        self.embed_sz,
    ...
    if not hasattr(self, "arange"):
      self.arange = Tensor.arange(self.vocab_sz, requires_grad=False, device=self.weight.device).reshape(arange_shp)
    arange, idx, vals = (
      ...
      self.weight.cast(dtypes.default_float).reshape(weight_shp).expand(big_shp),
    )
...
class LayerNormBert:
  def __init__(self, normalized_shape: Union[int, tuple[int, ...]], eps: float = 1e-12, elementwise_affine: bool = True):
    ...
    self.weight, self.bias = (
      (Tensor.ones(*self.normalized_shape, dtype=dtypes.float32), Tensor.zeros(*self.normalized_shape, dtype=dtypes.float32))
  ...
  def __init__(self, sz: int, eps=1e-5, affine=True, track_running_stats=True, momentum=0.1):
    ...
    if track_running_stats:
      self.running_mean, self.running_var = (
        Tensor.zeros(sz, dtype=dtypes.float32, requires_grad=False),
        Tensor.ones(sz, dtype=dtypes.float32, requires_grad=False),
      )
  ...
  def __call__(self, x: Tensor) -> Tensor:
    ...
    if self.track_running_stats and Tensor.training:
      ...
      self.running_var.assign(
        ...
        + self.momentum * x.numel() / (x.numel() - x.shape[1]) * batch_var.detach().cast(self.running_var.dtype)
      )
  ...
  def __init__(
    ...
    if bias:
      if prior_prob:
        ...
        self.bias = Tensor.zeros_like(self.bias, dtype=dtypes.float32)
