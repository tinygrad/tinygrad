...
def train_resnet():
    ...
    if getenv("LOGMLPERF"):
        ...
        )  # truncate to log this. "file": "tinygrad/examples/mlperf/model_train.py"
        MLLOGGER = mllog.get_mllogger()
        if INITMLPERF:
            # common.yaml
            MLLOGGER.event(key=mllog_constants.SUBMISSION_ORG, value="tinycorp")
            MLLOGGER.event(
                key=mllog_constants.SUBMISSION_PLATFORM,
                value=getenv("SUBMISSION_PLATFORM", "tinybox"),
            )
            MLLOGGER.event(
                key=mllog_constants.SUBMISSION_DIVISION, value=mllog_constants.CLOSED
            )
            MLLOGGER.event(
                key=mllog_constants.SUBMISSION_STATUS, value=mllog_constants.ONPREM
            )
            # closed_common.yaml
            MLLOGGER.event(
                key=mllog_constants.SUBMISSION_BENCHMARK, value=mllog_constants.RESNET
            )
            diskcache_clear()
    ...
    loss_scaler = config["LOSS_SCALER"] = getenv(
        "LOSS_SCALER", 256.0 if dtypes.default_float == dtypes.float16 else 1.0
    )
    ...
    steps_in_train_epoch = config["steps_in_train_epoch"] = (
        round_up(len(get_train_files()), BS) // BS
    )
    steps_in_val_epoch = config["steps_in_val_epoch"] = (
        round_up(len(get_val_files()), EVAL_BS) // EVAL_BS
    )
    ...
    skip_list = [
        ...
        if "bn" in k or "bias" in k or "downsample.1" in k
    ]
    parameters = [x for x in parameters if x not in set(skip_list)]
    optimizer = LARS(parameters, base_lr, momentum=0.9, weight_decay=decay)
    optimizer_skip = SGD(
        skip_list, base_lr, momentum=0.9, weight_decay=0.0, classic=True
    )
    ...
    scheduler_skip = PolynomialDecayWithWarmup(
        optimizer_skip,
        initial_lr=base_lr,
        end_lr=1e-4,
        train_steps=epochs * steps_in_train_epoch,
    ...
    if MLLOGGER:
        if RUNMLPERF:
            ...
            MLLOGGER.event(
                key=mllog_constants.TRAIN_SAMPLES, value=len(get_train_files())
            )
            ...
            MLLOGGER.event(
                key=mllog_constants.LARS_OPT_BASE_LEARNING_RATE,
                value=scheduler.initial_lr,
            )
            MLLOGGER.event(key=mllog_constants.LARS_OPT_END_LR, value=scheduler.end_lr)
            MLLOGGER.event(
                key=mllog_constants.LARS_OPT_LR_DECAY_POLY_POWER, value=scheduler.power
            )
            MLLOGGER.event(key=mllog_constants.LARS_OPT_LR_DECAY_STEPS, value=epochs)
            MLLOGGER.event(
                key=mllog_constants.LARS_EPSILON, value=0
            )  # does not support epsilon != 0
            MLLOGGER.event(
                key=mllog_constants.LARS_OPT_LEARNING_RATE_WARMUP_EPOCHS,
                value=lr_warmup_epochs,
            )
            MLLOGGER.event(
                key=mllog_constants.LARS_OPT_MOMENTUM, value=optimizer.momentum
            )
            MLLOGGER.event(
                key=mllog_constants.LARS_OPT_WEIGHT_DECAY, value=optimizer.wd
            )
    ...
    # ** jitted steps **
    input_mean = Tensor(
        [123.68, 116.78, 103.94], device=GPUS, dtype=dtypes.float32
    ).reshape(1, -1, 1, 1)

    # mlperf reference resnet does not divide by input_std for some reason
    # input_std = Tensor([0.229, 0.224, 0.225], device=GPUS, dtype=dtypes.float32).reshape(1, -1, 1, 1)
    def normalize(x):
        return (x.permute([0, 3, 1, 2]) - input_mean).cast(dtypes.default_float)
    ...
    def train_step(X, Y):
        ...
        X = normalize(X)
        out = model.forward(X)
        loss = out.cast(dtypes.float32).sparse_categorical_crossentropy(
            Y, label_smoothing=0.1
        )
        top_1 = (out.argmax(-1) == Y).sum()
        (loss * loss_scaler).backward()
        for t in optimizer_group.params:
            t.grad = t.grad.contiguous() / loss_scaler
    ...
    def fake_data_get(batch_size):
        ...
        y = [0] * batch_size
        return (
            x.shard(GPUS, axis=0).realize(),
            Tensor(y, requires_grad=False).shard(GPUS, axis=0),
            y,
            None,
        )
    ...
    for e in range(start_epoch, epochs):
        ...
        if INITMLPERF:
            ...
            batch_loader = batch_load_resnet(
                ...
                shuffle=True,
                seed=seed * epochs + e,
                pad_first_batch=True,
            )
            it = iter(
                tqdm(
                    batch_loader,
                    total=steps_in_train_epoch,
                    desc=f"epoch {e}",
                    disable=BENCHMARK,
                )
            )
        ...
        while proc is not None:
            ...
            if len(prev_cookies) == getenv("STORE_COOKIES", 1):
                prev_cookies = (
                    []
                )  # free previous cookies after gpu work has been enqueued
            try:
                if INITMLPERF:
            ...
            device_str = (
                loss.device
                if isinstance(loss.device, str)
                else f"{loss.device[0]} * {len(loss.device)}"
            )
            ...
            if i == BENCHMARK:
                ...
                median_step_time = sorted(step_times)[
                    (BENCHMARK + 1) // 2
                ]  # in seconds
                estimated_total_minutes = int(
                    median_step_time * steps_in_train_epoch * epochs / 60
                )
                print(
                    f"Estimated training time: {estimated_total_minutes // 60}h{estimated_total_minutes % 60}m"
                )
                print(
                    f"epoch global_ops: {steps_in_train_epoch * GlobalCounters.global_ops:_}, "
                    f"epoch global_mem: {steps_in_train_epoch * GlobalCounters.global_mem:_}"
                )
                # if we are doing beam search, run the first eval too
                if (TRAIN_BEAM or EVAL_BEAM) and e == start_epoch:
                    break
                return
        if MLLOGGER and RUNMLPERF:
            MLLOGGER.event(
        ...
        if steps_in_val_epoch > 0 and (
            (e + 1 - eval_start_epoch) % eval_freq == 0 or e + 1 >= 33
        ):
            if MLLOGGER and RUNMLPERF:
                MLLOGGER.start(
            ...
            while proc is not None:
                ...
                (loss, top_1), y, proc = (
                    eval_step(proc[0], proc[1]),
                    proc[2],
                    proc[3],
                )  # drop inputs, keep cookie
            ...
            if getenv("RESET_STEP", 1):
                eval_step.reset()
            if not BENCHMARK:
                assert eval_num_samples == len(
                    get_val_files()
                ), f"eval sample count mismatched. {eval_num_samples=} != {len(get_val_files())}"
            total_loss = eval_loss / eval_num_samples
            total_top_1 = eval_top_1 / eval_num_samples
            total_fw_time = sum(eval_times) / len(eval_times)
            tqdm.write(
                f"eval loss: {total_loss:.2f}, eval time: {total_fw_time:.2f}, eval top 1 acc: {total_top_1:.3f}"
            )
            if WANDB:
                wandb.log(
                    {
            ...
            if not achieved and total_top_1 >= target:
                ...
                if MLLOGGER and RUNMLPERF:
                    MLLOGGER.event(
                        ...
                        metadata=dict(status=mllog_constants.SUCCESS),
                    )
            ...
            if getenv("CKPT"):
                ...
                safe_save(
                    get_training_state(model, optimizer_group, scheduler_group), fn
                )
...
def train_unet3d():
    ...
    START_EVAL_AT = getenv(
        "START_EVAL_AT", ceil(1000 * TRAIN_DATASET_SIZE / (SAMPLES_PER_EPOCH * BS))
    )
    EVALUATE_EVERY = getenv(
        "EVALUATE_EVERY", ceil(20 * TRAIN_DATASET_SIZE / (SAMPLES_PER_EPOCH * BS))
    )
    TRAIN_BEAM, EVAL_BEAM = getenv("TRAIN_BEAM", BEAM.value), getenv(
        "EVAL_BEAM", BEAM.value
    )
    BENCHMARK = getenv("BENCHMARK")
    CKPT = getenv("CKPT")
    ...
    def lr_warm_up(optim, init_lr, lr, current_epoch, warmup_epochs):
        ...
        optim.lr.assign(
            Tensor([init_lr + (lr - init_lr) * scale], device=GPUS)
        ).realize()
    ...
    if WANDB:
        wandb.init(config=config, project=PROJ_NAME)

    step_times, start_epoch = [], 1
    is_successful, diverged = False, False
    start_eval_at, evaluate_every = 1 if BENCHMARK else START_EVAL_AT, (
        1 if BENCHMARK else EVALUATE_EVERY
    )
    ...
    if BENCHMARK:
        ...
        print(
            f"Start evaluation at epoch {start_eval_at} and every {evaluate_every} epoch(s) afterwards"
        )

    if not TRAIN_PREPROCESSED_DIR.exists():
        preprocess_dataset(get_train_files(), TRAIN_PREPROCESSED_DIR, False)
    if not VAL_PREPROCESSED_DIR.exists():
        preprocess_dataset(get_val_files(), VAL_PREPROCESSED_DIR, True)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        with Context(BEAM=TRAIN_BEAM):
            ...
            train_dataloader = batch_load_unet3d(
                TRAIN_PREPROCESSED_DIR,
                batch_size=BS,
                val=False,
                shuffle=True,
            ...
            while proc is not None:
                    ...
                    wandb.log(
                        {
                            ...
                            "train/data_time": dt - pt,
                            "train/cl_time": cl - dt,
                            "train/GFLOPS": GlobalCounters.global_ops
                            * 1e-9
                            / (cl - st),
                            "epoch": epoch + (i + 1) / SAMPLES_PER_EPOCH,
                        }
                    )
                ...
                if i == BENCHMARK:
                    ...
                    estimated_total_minutes = int(
                        median_step_time * SAMPLES_PER_EPOCH * NUM_EPOCHS / 60
                    )
                    print(
                        f"Estimated training time: {estimated_total_minutes // 60}h{estimated_total_minutes % 60}m"
                    )
                    if (TRAIN_BEAM or EVAL_BEAM) and epoch == start_epoch:
                        break
                    return
        ...
        with Context(BEAM=EVAL_BEAM):
            if epoch == next_eval_at:
                ...
                for x, y in tqdm(
                    iterate(get_val_files(), preprocessed_dir=VAL_PREPROCESSED_DIR),
                ...
                tqdm.write(
                    f"{l1_dice} L1 dice, {l2_dice} L2 dice, {mean_dice:.3f} mean_dice, {eval_loss:5.2f} eval_loss"
                )
...
def train_step_bert(
    ...
    lm_logits, seq_relationship_logits = model(
        input_ids, attention_mask, masked_positions, segment_ids
    )
    loss = model.loss(
        lm_logits,
    ...
    global_norm = Tensor(
        [0.0], dtype=dtypes.float32, device=optimizer[0].device
    ).realize()
...
def eval_step_bert(
    ...
    return (
        masked_lm_accuracy.realize(),
        seq_relationship_accuracy.realize(),
        masked_lm_loss.realize(),
        next_sentence_loss.realize(),
    )
...
def train_bert():
    ...
    if getenv("LOGMLPERF"):
        ...
        if INITMLPERF:
            ...
            MLLOGGER.event(
                key=mllog_constants.SUBMISSION_BENCHMARK, value=mllog_constants.BERT
            )
    ...
    # ** hyperparameters **
    BS = config["GLOBAL_BATCH_SIZE"] = getenv(
        "BS",
        (
            11 * len(GPUS)
            if dtypes.default_float in (dtypes.float16, dtypes.bfloat16)
            else 8 * len(GPUS)
        ),
    )
    EVAL_BS = config["EVAL_BS"] = getenv("EVAL_BS", 1 * len(GPUS))
    max_lr = config["OPT_BASE_LEARNING_RATE"] = getenv(
        "OPT_BASE_LEARNING_RATE", 0.00011 * math.sqrt(BS / 66)
    )
    ...
    max_eval_steps = config["MAX_EVAL_STEPS"] = getenv(
        "MAX_EVAL_STEPS", (10000 + EVAL_BS - 1) // EVAL_BS
    )  # EVAL_BS * MAX_EVAL_STEPS >= 10000
    eval_step_freq = config["EVAL_STEP_FREQ"] = getenv(
        "EVAL_STEP_FREQ",
        int((math.floor(0.05 * (230.23 * BS + 3000000) / 25000) * 25000) / BS),
    )  # Round down
    ...
    loss_scaler = config["LOSS_SCALER"] = getenv(
        "LOSS_SCALER", 2.0**10 if dtypes.default_float == dtypes.float16 else 1.0
    )
    ...
    assert 10000 <= (
        EVAL_BS * max_eval_steps
    ), "Evaluation batchsize * max_eval_steps must greater or equal 10000 to iterate over full eval dataset"
    ...
    # ** Optimizer **
    parameters_no_wd = [
        v for k, v in get_state_dict(model).items() if "bias" in k or "LayerNorm" in k
    ]
    parameters = [x for x in parameters if x not in set(parameters_no_wd)]
    optimizer_wd = LAMB(
        parameters, lr=max_lr, eps=epsilon, weight_decay=decay, adam=False
    )
    optimizer_no_wd = LAMB(
        parameters_no_wd, lr=max_lr, eps=epsilon, weight_decay=0.0, adam=False
    )
    optimizer_group = OptimizerGroup(optimizer_wd, optimizer_no_wd)

    # ** LR scheduler **
    scheduler_wd = PolynomialDecayWithWarmup(
        optimizer_wd, max_lr, 0, train_steps, warmup_steps, power=poly_power
    )
    scheduler_no_wd = PolynomialDecayWithWarmup(
        optimizer_no_wd, max_lr, 0, train_steps, warmup_steps, power=poly_power
    )
    ...
    if MLLOGGER:
        if RUNMLPERF:
            MLLOGGER.event(
                key=mllog_constants.GLOBAL_BATCH_SIZE, value=config["GLOBAL_BATCH_SIZE"]
            )
            ...
            MLLOGGER.event(
                key=mllog_constants.OPT_BASE_LR, value=config["OPT_BASE_LEARNING_RATE"]
            )
            MLLOGGER.event(
                key=mllog_constants.OPT_LAMB_WEIGHT_DECAY, value=config["DECAY"]
            )
            ...
            MLLOGGER.event(
                key=mllog_constants.OPT_LAMB_LR_DECAY_POLY_POWER,
                value=config["POLY_POWER"],
            )
            MLLOGGER.event(
                key=mllog_constants.OPT_LAMB_EPSILON, value=config["EPSILON"]
            )

            MLLOGGER.event(
                key=mllog_constants.OPT_LR_WARMUP_STEPS,
                value=config["NUM_WARMUP_STEPS"],
            )
            MLLOGGER.event(
                key=mllog_constants.NUM_WARMUP_STEPS, value=config["NUM_WARMUP_STEPS"]
            )
            MLLOGGER.event(key="start_warmup_step", value=0)
            MLLOGGER.event(
                key="opt_learning_rate_training_steps", value=config["TRAIN_STEPS"]
            )
            MLLOGGER.event(key=mllog_constants.GRADIENT_ACCUMULATION_STEPS, value=1)
            MLLOGGER.event(
                key=mllog_constants.EVAL_SAMPLES,
                value=config["EVAL_BS"] * config["MAX_EVAL_STEPS"],
            )
            MLLOGGER.event(
                key=mllog_constants.TRAIN_SAMPLES,
                value=config["GLOBAL_BATCH_SIZE"] * config["TRAIN_STEPS"],
            )
    ...
    if RUNMLPERF:
        ...
        train_it = iter(
            tqdm(batch_load_train_bert(BS), total=train_steps, disable=BENCHMARK)
        )
        for _ in range(start_step):
            next(train_it)  # Fast forward
    ...
    while train_data is not None and i < train_steps and not achieved:
        ...
        st = time.perf_counter()
        GlobalCounters.reset()
        loss, global_norm = train_step_bert(
            model,
            optimizer_group,
            scheduler_group,
            loss_scaler,
            train_data["input_ids"],
            train_data["segment_ids"],
            train_data["input_mask"],
            train_data["masked_lm_positions"],
            train_data["masked_lm_ids"],
            train_data["masked_lm_weights"],
            train_data["next_sentence_labels"],
        )
        ...
        if WANDB:
            wandb.log(
                {
                    "lr": optimizer_wd.lr.numpy(),
                    "train/loss": loss,
                    "train/global_norm": global_norm.item(),
                    "train/step_time": cl - st,
                    "train/python_time": pt - st,
                    "train/data_time": dt - pt,
                    "train/cl_time": cl - dt,
        ...
        if i == BENCHMARK:
            ...
            print(
                f"epoch global_ops: {train_steps * GlobalCounters.global_ops:_}, "
        ...
        if (
            ...
            if MLLOGGER and RUNMLPERF:
                MLLOGGER.start(
                    key=mllog_constants.EVAL_START,
                    value=None,
                    metadata={"epoch_num": i * BS, "step_num": i},
                )
            if getenv("RESET_STEP", 0):
            ...
            for j in tqdm(
                ...
                lm_acc, clsf_acc, lm_loss, clsf_loss = eval_step_bert(
                    model,
                    eval_data["input_ids"],
                ...
                lm_acc, clsf_acc, lm_loss, clsf_loss = (
                    lm_acc.item(),
                    clsf_acc.item(),
                    lm_loss.item(),
                    clsf_loss.item(),
            ...
            if WANDB:
                wandb.log(
                    {
                        "eval/lm_loss": avg_lm_loss,
                        "eval/clsf_loss": avg_clsf_loss,
                        "eval/lm_accuracy": avg_lm_acc,
                        "eval/clsf_accuracy": avg_clsf_acc,
                        "eval/forward_time": avg_fw_time,
                    }
                )
            ...
            if MLLOGGER and RUNMLPERF:
                MLLOGGER.end(
                    ...
                    metadata={
                        "epoch_count": i * BS,
                        "step_num": i,
                        "samples_count": config["EVAL_BS"] * config["MAX_EVAL_STEPS"],
                    },
                )
                MLLOGGER.event(
                    key=mllog_constants.EVAL_ACCURACY,
                    value=avg_lm_acc,
                    metadata={"epoch_num": i * BS, "masked_lm_accuracy": avg_lm_acc},
                )
            ...
            if not achieved and avg_lm_acc >= target:
                ...
                if getenv("CKPT"):
                    if not os.path.exists(ckpt_dir := save_ckpt_dir):
                        os.mkdir(ckpt_dir)
                ...
                print(
                    f"Reference Convergence point reached after {i * BS} datasamples and {hours}h{minutes}m{seconds:.2f}s."
                )
                achieved = True
                if MLLOGGER and RUNMLPERF:
                    MLLOGGER.event(
        ...
        if getenv("CKPT") and i % save_ckpt_freq == 0:
            if MLLOGGER and RUNMLPERF:
                if previous_step:
                    MLLOGGER.end(
                        key=mllog_constants.BLOCK_STOP,
                        value=None,
                        metadata={
                            "first_epoch_num": 1,
                            "epoch_num": 1,
                            "first_step_num": i,
                            "step_num": i,
                            "step_count": i - previous_step,
                        },
                    )
                MLLOGGER.start(
                    key="checkpoint_start", value=None, metadata={"step_num": i}
                )
            if not os.path.exists(ckpt_dir := save_ckpt_dir):
                os.mkdir(ckpt_dir)
            if WANDB and wandb.run is not None:
            ...
            if MLLOGGER and RUNMLPERF:
                MLLOGGER.end(
                    key="checkpoint_stop", value=None, metadata={"step_num": i}
                )
                MLLOGGER.start(
