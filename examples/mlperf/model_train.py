...
def train_llama3():
  from examples.mlperf.models.flat_llama import FlatTransformer, apply_grad, FP8_DTYPE, FP8_GRAD_DTYPE
  ...
  for p in optim.params:
    p.grad = Tensor.zeros(p.shape, dtype=FP8_GRAD_DTYPE if p.dtype == FP8_DTYPE else p.dtype, device=p.device).contiguous()
