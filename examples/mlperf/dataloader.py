    ...
    with Context(DEBUG=0):
        while (_recv := q_in.get()) is not None:
            ...
            if fn is not None:
                ...
                img = np.tile(
                    np.array([[[123.68, 116.78, 103.94]]], dtype=np.uint8),
                    (224, 224, 1),
                )
            ...
            # faster
            X[idx].contiguous().realize().lazydata.base.realized.as_buffer(
...
def batch_load_resnet(
    batch_size=64, val=False, shuffle=True, seed=None, pad_first_batch=False
):
    ...
    def _gen():
        for _ in range(FIRST_BATCH_PAD):
            yield -1
        yield from (
            shuffled_indices(len(files), seed=seed)
            if shuffle
            else iter(range(len(files)))
        )
    ...
    if os.path.exists("/dev/shm/resnet_X"):
...
def process_batch_bert(data: List[dict]) -> dict[str, Tensor]:
    return {
        "input_ids": Tensor(
            np.concatenate([s["input_ids"] for s in data], axis=0), dtype=dtypes.float32
        ),
        "input_mask": Tensor(
            np.concatenate([s["input_mask"] for s in data], axis=0),
            dtype=dtypes.default_float,
        ),
        "segment_ids": Tensor(
            np.concatenate([s["segment_ids"] for s in data], axis=0),
            dtype=dtypes.float32,
        ),
        "masked_lm_positions": Tensor(
            np.concatenate([s["masked_lm_positions"] for s in data], axis=0),
            dtype=dtypes.float32,
        ),
        "masked_lm_ids": Tensor(
            np.concatenate([s["masked_lm_ids"] for s in data], axis=0),
            dtype=dtypes.float32,
        ),
        "masked_lm_weights": Tensor(
            np.concatenate([s["masked_lm_weights"] for s in data], axis=0),
            dtype=dtypes.float32,
        ),
        "next_sentence_labels": Tensor(
            np.concatenate([s["next_sentence_labels"] for s in data], axis=0),
            dtype=dtypes.float32,
        ),
    }
...
def batch_load_train_bert(BS: int):
    ...
    cycle_length = min(
        getenv("NUM_CPU_THREADS", min(os.cpu_count(), 8)), len(train_files)
    )
...
def batch_load_val_bert(BS: int):
    file = (
        getenv(
            "BASEDIR", Path(__file__).parent.parents[1] / "extra" / "datasets" / "wiki"
        )
        / "eval.pkl"
    )
...
def load_unet3d_data(
    preprocessed_dataset_dir, seed, queue_in, queue_out, X: Tensor, Y: Tensor
):
    ...
    while (data := queue_in.get()) is not None:
        ...
        x, y = np.load(preprocessed_dataset_dir / f"{case_name}_x.npy"), np.load(
            preprocessed_dataset_dir / f"{case_name}_y.npy"
        )
        ...
        Y[idx].contiguous().realize().lazydata.base.realized.as_buffer(
            force_zero_copy=True
        )[:] = y.tobytes()
...
def batch_load_unet3d(
    ...
    try:
        X = Tensor.empty(
            *sz, dtype=dtypes.float32, device=f"disk:/dev/shm/{shm_name_x}"
        )
        Y = Tensor.empty(*sz, dtype=dtypes.uint8, device=f"disk:/dev/shm/{shm_name_y}")

        for _ in range(cpu_count()):
            proc = Process(
                target=load_unet3d_data,
                args=(preprocessed_dataset_dir, seed, queue_in, queue_out, X, Y),
            )
            proc.daemon = True
            proc.start()
        ...
        for _ in range(len(files) // batch_size):
            ...
            data_out_count[bc] = 0
            yield X[bc * batch_size : (bc + 1) * batch_size], Y[
                bc * batch_size : (bc + 1) * batch_size
            ], Cookie(bc)
    finally:
        shutdown = True
...
if __name__ == "__main__":
    ...
    def load_unet3d(val):
        assert (
            not val
        ), "validation set is not supported due to different sizes on inputs"
