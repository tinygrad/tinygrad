...
def eval_unet3d():
  ...
  from extra.datasets.kits19 import iterate, sliding_window_inference, get_eval_files
  from examples.mlperf.metrics import dice_score
  mdl = UNet3D()
  if (fn:=getenv("UNET3D_MODEL", "")): load_state_dict(mdl, safe_load(fn))
  else: mdl.load_from_pretrained()
  s = 0
  st = time.perf_counter()
  for i, (image, label) in enumerate(iterate(get_eval_files()), start=1):
