...
def tlog(x):
  print(f"{x:25s}  @ {time.perf_counter() - start:5.2f}s")
  ...
  with WallTimeEvent(BenchEvent.FULL):
    ...
    class ResnetRunner:
      def __init__(self, device=None):
        self.mdl = ResNet50()
        for x in get_parameters(self.mdl) if device else []:
          x.to_(device)
        if fn := getenv("RESNET_MODEL", ""):
          load_state_dict(self.mdl, safe_load(fn))
        else:
    ...
    while proc is not None:
      ...
      tlog(
        f"****** {n:5d}/{d:5d}  {n * 100.0 / d:.2f}% -- {(run - st) * 1000:7.2f} ms to enqueue, {(et - run) * 1000:7.2f} ms to realize ({(nd - run) * 1000:7.2f} ms fetching). {(len(proc)) / (et - st):8.2f} examples/sec. {GlobalCounters.global_ops * 1e-12 / (et - st):5.2f} TFLOPS"
      )
      st = et
...
def eval_retinanet():
  ...
  while proc is not None:
    ...
    coco_results = [
      {"image_id": img_ids[i], "category_id": label, "bbox": box.tolist(), "score": score}
    ...
    et = time.perf_counter()
    tlog(
      f"****** {(run - st) * 1000:7.2f} ms to enqueue, {(et - run) * 1000:7.2f} ms to realize ({(nd - run) * 1000:7.2f} ms fetching, {(pd - run) * 1000:4.2f} ms postprocess_detections). {(len(proc)) / (et - st):8.2f} examples/sec. {GlobalCounters.global_ops * 1e-12 / (et - st):5.2f} TFLOPS"
    )
    st = et
    proc, next_proc = next_proc, None
...
def eval_llama3():
  ...
  if (llama_layers := getenv("LLAMA_LAYERS")) != 0:
    params["n_layers"] = llama_layers
