...
def fclip_grads(grads:list[Tensor], grad_acc, clip_norm) -> Tensor:
  normalized = grads / grad_acc
  total_norm = Tensor.stack(*[g.float().square().sum() for g in normalized]).sum().sqrt().contiguous()
  scale = (clip_norm / (total_norm + 1e-6)).clamp(max_=1.0)
  return [(g * scale).cast(g.dtype) for g in normalized], total_norm
