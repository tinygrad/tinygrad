...
def functional_adam(g:Tensor, m:Tensor, v:Tensor, b1_t:Tensor, b2_t:Tensor, lr=0.001, b1=0.9, b2=0.999, eps=1e-6):
  b1_t *= b1
  b2_t *= b2
  m.assign(b1 * m + (1.0 - b1) * g)
  v.assign(b1 * v + (1.0 - b2) * (g * g))
  m_hat = m / (1.0 - b1_t)
  v_hat = v / (1.0 - b2_t)
...
if __name__ == "__main__":
  ...
  for i in (t:=trange(getenv("STEPS", 20))):
    ...
    for i,tt in enumerate(params):
      tt.assign(tt.detach() - delta[pos_params[i]:pos_params[i+1]].reshape(tt.shape).to(Device.DEFAULT))
