...
if __name__ == "__main__":
    ...
    def train_step(
        ...
        with Tensor.train():
            ...
            action_mask = (
                ...
                == Tensor.arange(log_dist.shape[1])
                .reshape(1, -1)
                .expand(selected_action.shape[0], -1)
            ).float()
            ...
            clipped_ratio = masked_advantage * ratios.clip(
                1 - PPO_EPSILON, 1 + PPO_EPSILON
            )
    ...
    for episode_number in (t := trange(EPISODES)):
        ...
        Xn, An, Rn = (
            Xn[-REPLAY_BUFFER_SIZE:],
            An[-REPLAY_BUFFER_SIZE:],
            Rn[-REPLAY_BUFFER_SIZE:],
        )
        ...
        old_log_dist = model(X)[
            0
        ].detach()  # TODO: could save these instead of recomputing
        for i in range(TRAIN_STEPS):
            samples = Tensor.randint(
                BATCH_SIZE, high=X.shape[0]
        ...
        t.set_description(
            f"sz: {len(Xn):5d} steps/s: {steps/(time.perf_counter()-st):7.2f} action_loss: {action_loss.item():7.3f} entropy_loss: {entropy_loss.item():7.3f} critic_loss: {critic_loss.item():8.3f} reward: {sum(rews):6.2f}"
        )
