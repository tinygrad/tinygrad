...
class LLaMa:
    ...
    def build(
        ...
        assert (
            tokenizer.vocab_size() == params["args"]["vocab_size"]
        ), f"{tokenizer.vocab_size()=} not equal to {params['args']['vocab_size']}"
        ...
        model = Transformer(
            **params["args"], linear=linear, max_context=MAX_CONTEXT, jit=bool(JIT)
        )
            ...
            if isinstance(device, tuple):
                for k, v in nn.state.get_state_dict(model).items():
                    if "scale" in k:
                        ...
                        if getenv("SHARD_KVCACHE") and (
                            ".wq." in k or ".wk." in k or ".wv." in k
                        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--shard", type=int, default=1, help="number of devices to load the weights to"
    )
...
"""
    ...
    LLAMA_SUFFIX = {"1": "", "2": "-2", "3": "-3", "code": "-code", "tiny": "-tiny"}[
        args.gen
    ]
    MODEL_PATH = (
        args.model
    ...
    param_bytes = sum(
        x.lazydata.size * x.dtype.itemsize for x in get_parameters(llama.model)
    )
    ...
    if chatbot:
        ...
        with Timing():
            llama.model(
                Tensor([toks], device=device), 0, args.temperature
    ...
    while 1:
        ...
        for i in range(args.count):
            ...
            next_tok = (
                Tensor([toks[start_pos:]], device=device)
