  ...
  def __init__(self, version: str | None = None, pretrained: str | None = None):
    ...
    if version != "v2-mlperf-train":
      self.first_stage_model = (
        AutoencoderKL()
      )  # only needed for decoding generated latents to images; not needed in mlperf training from preprocessed moments
    ...
    if not version:
      ...
      if pretrained:
        ...
        weights: dict[str, Tensor] = {
          k.replace("cond_stage_model.", "", 1): v for k, v in torch_load(pretrained)["state_dict"].items() if k.startswith("cond_stage_model.")
        }
        ...
        if version == "v2-mlperf-eval":
          ...
          weights = {
            k.replace("first_stage_model.", "", 1): v for k, v in torch_load(pretrained)["state_dict"].items() if k.startswith("first_stage_model.")
          }
    ...
    if version == "v2-mlperf-train":
      ...
      for bb in (
        flatten(self.model.diffusion_model.input_blocks) + self.model.diffusion_model.middle_block + flatten(self.model.diffusion_model.output_blocks)
      ):
...
if __name__ == "__main__":
  ...
  alphas_np, alphas_prev_np = alphas.numpy().tolist(), alphas_prev.numpy().tolist()
      ...
      with Timing("step in ", enabled=args.timing, on_exit=lambda _: f", using {GlobalCounters.mem_used / 1e9:.2f} GB"):
        with WallTimeEvent(BenchEvent.STEP):
          latent = run(
            ...
            latent,
            Tensor([timestep]),
            Tensor([alphas_np[index]]),
            Tensor([alphas_prev_np[index]]),
