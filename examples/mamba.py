...
class MambaMixer:
  def __init__(
    ...
    self.conv1d = nn.Conv1d(
      in_channels=self.d_inner, out_channels=self.d_inner, bias=conv_bias, kernel_size=d_conv, groups=self.d_inner, padding=d_conv - 1
    )
  ...
  def __call__(self, hidden_states: Tensor):
    ...
    if not hasattr(self, "conv_state"):
      ...
      y = selective_scan_ref(x, dt, A, B, C, self.D, z=z, delta_bias=self.dt_proj.bias, delta_softplus=True, return_last_state=True)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--size", type=str, default="370m", help=f"Size of model to use [{', '.join([k for k in MODELS.keys()])}]")
