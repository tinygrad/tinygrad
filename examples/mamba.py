...
class MambaMixer:
    def __init__(
        ...
        dt = (
            Tensor.uniform(self.d_inner, low=math.log(dt_min), high=math.log(dt_max))
            .exp()
    ...
    def __call__(self, hidden_states: Tensor):
        ...
        if not hasattr(self, "conv_state"):
            self.conv_state = (
                Tensor.zeros(batch, self.dim * self.expand, self.d_conv)
                .contiguous()
                .realize()
            )
            self.ssm_state = Tensor.zeros(
                batch, self.dim * self.expand, self.d_state
            ).realize()

            xz = self.in_proj.weight @ hidden_states.permute(2, 0, 1).reshape(
                hidden_states.shape[2], hidden_states.shape[1] * hidden_states.shape[0]
            )
            ...
            x_dbl = self.x_proj(
                x.permute(0, 2, 1).reshape(x.shape[0] * x.shape[2], x.shape[1])
            )
            dt, B, C = Tensor.split(
    ...
    def step(self, hidden_states: Tensor):
        ...
        self.conv_state.assign(
            self.conv_state[:, :, 1:].cat(x.unsqueeze(-1), dim=-1).realize()
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt",
        type=str,
        default="Why is gravity ",
        help="Prompt for LLM completion",
    )
    parser.add_argument(
        "--size",
        type=str,
    ...
    parser.add_argument(
        "--n_tokens", type=int, default=10, help="Number of tokens to generate"
    )
    parser.add_argument(
        "--sample", dest="sample", action="store_true", help="Sample flag"
    )
    parser.add_argument(
        "--temp", type=float, default=1.0, help="Sampling temp has to be <=1.0"
    )
    ...
    s = time.time()
    tinyoutput = generate(
        model, tokenizer, prompt, n_tokens_to_gen=num_toks, sample=sample, temp=temp
    )
    print(tinyoutput)
    print("TIME: ", time.time() - s)
    TORCHOUTPUT = "Why is gravity \nso important?\nBecause it's the only"
    if ORIG_PROMPT == prompt and not sample and num_toks == 10 and args.size == "370m":
