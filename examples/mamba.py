...
def selective_scan_ref(
  ...
  # parallel prefix scan: h_t = deltaA_t * h_{t-1} + deltaB_u_t
  # associative operator: (A2, Bu2) . (A1, Bu1) = (A2*A1, A2*Bu1 + Bu2)
  def combine(left, right):
    return (right[0] * left[0], right[0] * left[1] + right[1])
  _, all_states = Tensor.associative_scan(combine, (deltaA, deltaB_u), axis=2)
  last_state = all_states[:, :, -1] if return_last_state else None
  # compute outputs: contract hidden states with C
  if not is_variable_C:
    y = Tensor.einsum("bdln,dn->bdl", all_states, C)
  else:
    if len(C.shape) == 3:
      y = Tensor.einsum("bdln,bnl->bdl", all_states, C)
    else:
      y = Tensor.einsum("bdln,bdnl->bdl", all_states, C)
