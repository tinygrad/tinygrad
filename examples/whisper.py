...
class TextDecoder:
    ...
    def __call__(self, x: Tensor, pos: int, encoded_audio: Tensor):
        pos = (
            Variable("self_attn_cache_len", 1, self.max_self_attn_cache_len).bind(pos)
            if pos
            else 0
        )
    ...
    def forward(
        self, x: Tensor, pos: Union[Variable, Literal[0]], encoded_audio: Tensor
    ):
        seqlen = x.shape[-1]
        x = self.token_embedding(x) + self.positional_embedding.shrink(
            ((pos, pos + seqlen), None, None)
        )
...
def transcribe_waveform(model: Whisper, enc, waveforms, truncate=False):
    ...
    def inferloop(ctx: Union[np.ndarray, List[np.ndarray]], encoded_audio):
        ...
        for i in range((nsample - len(start_tokens)) * 2):
            next_tokens = (
                model.decoder(Tensor(next_tokens), pos, encoded_audio)[:, -1]
                .argmax(axis=-1)
                .numpy()
                .astype(np.int32)
                .reshape(-1, 1)
    ...
    def gettexttoks(line):
        return [
            tok
            for tok in line
            if tok < eot or tok > enc._special_tokens["<|notimestamps|>"]
        ][-nsample + len(start_tokens) :]
    ...
    for curr_frame in range(0, log_spec.shape[-1], FRAMES_PER_SEGMENT):
        ...
        if all(len(c) == len(ctx[0]) for c in ctx):
            ...
            ctx = [
                inferloop((np.array([c] * model.batch_size)), encoded_audio)[i]
        ...
        for i, (res, arr) in enumerate(zip(transcriptions, ctx)):
            if curr_frame * HOP_LENGTH <= len(waveforms[i]):
                res.extend(
                    arr[
                        np.where(arr == start_tokens[-1])[0][0]
                        + 1 : eoti[0] if len(eoti := np.where(arr == eot)[0]) else None
                    ]
                )
        ctx = [
            [enc._special_tokens["<|startofprev|>"]] + gettexttoks(cs) + start_tokens
            for cs in ctx
        ]

    transcriptions = list(
        map(lambda tokens: enc.decode(tokens).strip(), transcriptions)
    )
